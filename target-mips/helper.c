...
static inline void tlb_refill_exception_prepare(CPUMIPSState *env)
{
    ...
    if (unlikely(tlb_refill_info.state == PROBE)) {
        ...
        for (i = 0; i < TLB_HANDLER_SIZE; i++) {
            tlb_refill_info.handler[i] = ldl_phys(tlb_refill_info.ebase - 0x80000000 + (i << 2));
            CPU.do_tlbwr = 1;
        }
 ...
 */
static inline int tlb_exception_return(CPUMIPSState *env, target_ulong address, int rw,
                                      int mmu_idx)
{
 ...
 */
static inline int do_tlb_refill(CPUMIPSState *env, vaddr address, int rw, int mmu_idx)
{
    ...
    tlbret = tlb_exception_return(env, address, rw, mmu_idx);
    ...
    if (ret != TLBRET_MATCH) {
        ...
        if (likely(tlb_refill_info.state == USEFASTTLB)) {
            ...
            if (tlb_exception_interpreter(env, tlb_refill_info.handler, TLB_HANDLER_SIZE) != 0) {
                phys_addr = -1;
                fprintf(stderr, "cpu_get_phys_page_debug() fails for vaddr: 0x" TARGET_FMT_lx "\n", addr);
            } else {
                target_ulong mask = env->CP0_PageMask | ~(TARGET_PAGE_MASK << 1);
                              ...
                              int mmu_idx)
{
    ...
    if (ret == TLBRET_MATCH) {
        ...
        ret = do_tlb_refill(env, address, rw, mmu_idx);
