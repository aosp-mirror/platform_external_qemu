    ...
    if (unlikely(linux_pte_info.state == PROBE)) {
        ...
        static struct {
            ...
            /* 3.10+ No HugeTLB support*/
            {
                ...
                {0x8c, 0x403b2000, 0xfffff000}, /* 0x403b2000 : dmfc0 k1,c0_context */
        ...
        if (is_64bit) {
            ...
            address |= 0x540;
            /* dror k1,k1,0xb */
            address = ((uint64_t)address >> 11) | ((address & 0x7ff) << 53);
                                      ...
                                      int mmu_idx, int is_softmmu)
{
    ...
    if (linux_pte_info.is_64bit) {
        pgd_addr = env->CP0_Context;
        pgd_addr = ((uint64_t)pgd_addr >> 23) << 23;
        /* 1 0  1 0 1  << 6  xkphys cached */
        pgd_addr |= 0x540;
        /* dror k1,k1,0xb */
        pgd_addr = ((uint64_t)pgd_addr >> 11) | ((pgd_addr & 0x7ff) << 53);
    }
    ...
    if (rw == 0 || (lo & (1 << CP0ENTRYLO_D))) {
        hwaddr physical = (lo >> CP0ENTRYLO_PFN) << 12;
