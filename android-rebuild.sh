...
PREBUILT_DLLS_DIR=$OUT_DIR/build/prebuilt_dlls
PREBUILT_DLL_FILES=
PREBUILT_DLL_SYMLINKS=
# Copy a prebuilt file
# $1: Source file path.
# $2: Destination file path (not a directory!) relative to PREBUILT_DLLS_DIR
copy_prebuilt_dll () {
    local SRC_FILE="$1"
    local DST_FILE="$2"
    copy_file "$SRC_FILE" "$PREBUILT_DLLS_DIR/$DST_FILE"
    PREBUILT_DLL_FILES="$PREBUILT_DLL_FILES $DST_FILE"
}

# Install a prebuilt symlink
# $1: Source file path.
# $2: Destination file path (not a directory!) relative to PREBUILT_DLLS_DIR
symlink_prebuilt_dll () {
    local SRC_FILE="$1"
    local DST_FILE="$2"
    run ln -sf "$SRC_FILE" "$PREBUILT_DLLS_DIR/$DST_FILE"
    PREBUILT_DLL_SYMLINKS="$PREBUILT_DLL_SYMLINKS $DST_FILE"
}
    ...
    for LIBNAME in $MESA_LIBNAME; do
        for MESA_ARCH in x86 x86_64; do
            ...
            if [ "$MESA_LIBRARY" ]; then
                ...
                echo "Copying $MESA_HOST-$MESA_ARCH $LIBNAME $MESA_LIBRARY library to $MESA_DSTDIR"
                copy_prebuilt_dll "$MESA_LIBRARY" "$MESA_DSTDIR/$MESA_DSTLIB"
                if [ "$MESA_HOST" = "linux" -a "$LIBNAME" = "libGL.so" ]; then
                    ...
                    # by SDL.
                    symlink_prebuilt_dll libGL.so "$MESA_DSTDIR/libGL.so.1"
                fi
            fi
        done
    done
...
for QT_ARCH in x86 x86_64; do
    ...
    for QT_LIB in $QT_LIBS; do
        ...
        copy_prebuilt_dll "$QT_SRCDIR/$QT_LIB" "$QT_DSTDIR/$QT_DST_LIB"
    done
done

# Build prebuilt_dlls,make
prebuilt_dlls_mk=$PREBUILT_DLLS_DIR/prebuilt_dlls.make
echo "Building $prebuild_mk"
echo "# This file was autogenerated by $PROGNAME. Do not edit !" > $prebuilt_dlls_mk
echo "PREBUILT_DLL_FILES := \\" >> $prebuilt_dlls_mk
for PREBUILT_DLL_FILE in $PREBUILT_DLL_FILES; do
    echo "    $PREBUILT_DLL_FILE \\" >> $prebuilt_dlls_mk
done
echo -e "\nPREBUILT_DLL_SYMLINKS := \\" >> $prebuilt_dlls_mk
for PREBUILT_DLL_SYMLINK in $PREBUILT_DLL_SYMLINKS; do
    echo "    $PREBUILT_DLL_SYMLINK \\" >> $prebuilt_dlls_mk
done
echo -e "\n# Done autogenerated file." >> $prebuilt_dlls_mk
...
EXECUTABLES="$EXECUTABLES emulator64-ranchu-arm64 emulator64-ranchu-mips64 emulator64-ranchu-mips"
...
if [ "$HOST_OS" != "Darwin" ]; then
    ...
    EXECUTABLES="$EXECUTABLES emulator-ranchu-arm64 emulator-ranchu-mips64 emulator64-ranchu-mips"
