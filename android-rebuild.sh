...
if [ -z "$NO_TESTS" ]; then
    ...
    if [ "$RUN_32BIT_TESTS" ]; then
        ...
        for UNIT_TEST in android_emu_unittests emugl_common_host_unittests \
                         emulator_libui_unittests emulator_crashreport_unittests\
                         qemu-android_*_unittests; do
            for TEST in "$OUT_DIR/$UNIT_TEST$EXE_SUFFIX"; do
                echo "   - $TEST"
                run $TEST_SHELL $TEST || FAILURES="$FAILURES $TEST"
            done
        done
    fi
    ...
    if [ "$RUN_64BIT_TESTS" ]; then
        ...
        for UNIT_TEST in android_emu64_unittests emugl64_common_host_unittests \
                         emulator64_libui_unittests emulator64_crashreport_unittests\
                         qemu-android64_*_unittests; do
             for TEST in "$OUT_DIR/$UNIT_TEST$EXE_SUFFIX"; do
                 echo "   - ${TEST#$OUT_DIR/}"
