...
static VOID WINAPI icmpwin_callback_apc(void *ctx, PIO_STATUS_BLOCK iob,
                                        ULONG reserved);
...
static int m_append(Slirp *slirp, struct mbuf *m0, int len, caddr_t cp) {
  ...
  */
  ...
  if (space > 0) {
     ...
     */
    if (space > remainder) space = remainder;
...
void icmpwin_ping(Slirp *slirp, struct mbuf *m, int hlen) {
  ...
  if (slirp->cbIcmpPending + pongsize > 1024 * 1024) return;
  ...
  if (m->m_next == NULL) {
    ...
    reqdata = pong->buf;
    m_copydata(m, sizeof(struct ip) + sizeof(struct icmp_echo), (int)reqsize,
               reqdata);
  }
  ...
  status = IcmpSendEcho2(slirp->icmp_handle, NULL, g_pfnIcmpCallback, pong, dst,
                         reqdata, (WORD)reqsize, &opts, pong->buf,
                         (DWORD)pong->bufsize, 5 * 1000 /* ms */);
  if (status != 0) {
    fprintf(stderr, "Slirp: IcmpSendEcho2: unexpected status %d\n", status);
  } else if ((status = GetLastError()) != ERROR_IO_PENDING) {
    int code;

    printf("Slirp: IcmpSendEcho2: error %d\n", status);
    switch (status) {
  ...
  if (pong != NULL) free(pong);
}

static VOID WINAPI icmpwin_callback_apc(void *ctx, PIO_STATUS_BLOCK iob,
                                        ULONG reserved) {
  struct pong *pong = (struct pong *)ctx;
  if (pong != NULL) icmpwin_callback(pong);
  (void)iob;
  (void)reserved;
}
...
void icmpwin_process(Slirp *slirp) {
  ...
  if (TAILQ_EMPTY(&slirp->pongs_received)) return;
...
void icmpwin_pong(struct pong *pong) {
  ...
  if (nreplies == 0) {
    ...
    if (error == IP_REQ_TIMED_OUT)
      fprintf(stderr, "Slirp: ping %p timed out\n", (void *)pong);
    else
      fprintf(stderr, "Slirp: ping %p: IcmpParseReplies: error %d\n",
              (void *)pong, (int)error);
    return;
  }
  ...
  if (reply->Status == IP_SUCCESS) {
    ...
    switch (reply->Status) {
      ...
      case IP_TTL_EXPIRED_REASSEM:
        ...
        break;
      default:
        printf("Slirp: ping reply status %d, dropped\n", (int)reply->Status);
        return;
    }
