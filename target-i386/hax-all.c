...
static int hax_get_capability(struct hax_state *hax)
{
    ...
    if (cap->wstatus & HAX_CAP_MEMQUOTA)
    {
        ...
        {
          dprint("The memory needed by this AVD exceeds the max specified in your HAXM configuration.");
          dprint("AVD      RAM size = %"PRId64" MB", hax->mem_quota >> 20);
          dprint("HAXM max RAM size = %"PRId64" MB", cap->mem_quota >> 20);
          dprint("You might want to adjust your AVD RAM size and/or HAXM configuration to run in fast virt mode.\n");
