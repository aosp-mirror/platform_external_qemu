...
void print_hvf_error(int r) {
    switch (r) {
        case HV_SUCCESS:
            fprintf(stderr, "%s: error: HV_SUCCESS\n", __func__); break;
        case HV_ERROR:
            fprintf(stderr, "%s: error: HV_ERROR\n", __func__); break;
        case HV_BUSY:
            fprintf(stderr, "%s: error: HV_BUSY\n", __func__); break;
        case HV_BAD_ARGUMENT:
            fprintf(stderr, "%s: error: HV_BAD_ARGUMENT\n", __func__); break;
        case HV_NO_RESOURCES:
            fprintf(stderr, "%s: error: HV_NO_RESOURCES\n", __func__); break;
        case HV_NO_DEVICE:
            fprintf(stderr, "%s: error: HV_NO_DEVICE\n", __func__); break;
        case HV_UNSUPPORTED:
            fprintf(stderr, "%s: error: HV_UNSUPPORTED\n", __func__); break;
        default:
            fprintf(stderr, "%s: Unknown error heh\n", __func__); break;
            break;
    }
}
// Memory slots/////////////////////////////////////////////////////////////////
...
int __hvf_set_memory(hvf_slot *slot) {
    ...
    if (mapres) {
        // fprintf(stderr, "register failed. err: 0x%x\n", mapres);
        print_hvf_error(mapres);
        abort();
        return -1;
    }
    pthread_rwlock_unlock(&mem_lock);
...
int hvf_enabled() { return 1; }
void hvf_disable(int shouldDisable) { }

void vmx_reset_vcpu(CPUState *cpu) {
    uint64_t apic_msr_val = 0xfee00000 | MSR_IA32_APICBASE_ENABLE;
    fprintf(stderr, "%s:%d arrive. msr apicbase +enable 0x%llx\n", __func__, __LINE__, apic_msr_val);
    if (cpu_is_bsp(X86_CPU(cpu))) {
        apic_msr_val |= MSR_IA32_APICBASE_BSP;
        fprintf(stderr, "%s:%d arrive. msr apicbase +enable +bsp 0x%llx\n", __func__, __LINE__, apic_msr_val);
    }
    ...
    memset(cpu->hvf_x86->apic_page, 0, 4096);
    ((char*)cpu->hvf_x86->apic_page)[0x390] = 0xdeadbeef;

    apic_msr_val = cpu_get_apic_base(X86_CPU(cpu)->apic_state);
    fprintf(stderr, "%s: writing 0x%llx to IA32 APICBASE MSR\n", __func__, apic_msr_val);
    hv_vcpu_write_msr(cpu->hvf_fd, MSR_IA32_APICBASE, apic_msr_val);
...
int hvf_init_vcpu(CPUState * cpu) {
    ...
    fprintf(stderr, "%s: attempt hv_vcpu_create\n", __func__);
    r = hv_vcpu_create(&cpu->hvf_fd, HV_VCPU_DEFAULT);
    cpu->hvf_vcpu_dirty = 1;
    if (r) {
        fprintf(stderr, "%s: create vcpu failed. errcode 0x%x\n", __func__, r);
        print_hvf_error(r);
    }
    else
        fprintf(stderr, "%s: success create vcpu. fd %llu\n", __func__, cpu->hvf_fd);
    ...
    if (!cpu->hvf_x86->apic_page) {
        posix_memalign(&cpu->hvf_x86->apic_page, 4096, 4096);
        memset(cpu->hvf_x86->apic_page, 0, 4096);
        int mapres = hv_vm_map((hv_uvaddr_t)cpu->hvf_x86->apic_page, apic_gpa, 4096, HV_MEMORY_READ | HV_MEMORY_WRITE);
        if (mapres) {
            fprintf(stderr, "%s: err map apic page %p\n", __func__, cpu->hvf_x86->apic_page);
            print_hvf_error(mapres);
        }
        int apicaddrset_res = hv_vmx_vcpu_set_apic_address(cpu->hvf_fd, apic_gpa);
        if (apicaddrset_res) {
            fprintf(stderr, "%s: err set apic addr to 0x%llx 0x%llx\n", __func__, apic_gpa, apicaddrset_res);
            print_hvf_error(apicaddrset_res);
        }
        if (!cpu->hvf_x86->apic_page) {
            fprintf(stderr, "%s: wtf? no apic page\n", __func__);
        } else {
            fprintf(stderr, "%s: inited apic page.\n", __func__);
        }
    } else {
        fprintf(stderr, "%s: did NOT init apic page\n", __func__);
    }
    ...
    fprintf(stderr, "%s: x86cpu starts at %p\n", __func__, x86cpu);
    ...
    r = hv_vcpu_enable_native_msr(cpu->hvf_fd, MSR_IA32_APICBASE, 1);
    fprintf(stderr, "%s: 0x%x enable apicbase native msr\n", __func__);
    if (r) print_hvf_error(r);
    //hv_vcpu_enable_native_msr(cpu->hvf_fd, MSR_IA32_TSC, 1);
    hv_vcpu_enable_native_msr(cpu->hvf_fd, MSR_IA32_SYSENTER_CS, 1);
    hv_vcpu_enable_native_msr(cpu->hvf_fd, MSR_IA32_SYSENTER_EIP, 1);
    hv_vcpu_enable_native_msr(cpu->hvf_fd, MSR_IA32_SYSENTER_ESP, 1);

    uint64_t apic_msr_val = cpu_get_apic_base(X86_CPU(cpu)->apic_state);
    r = hv_vcpu_write_msr(cpu->hvf_fd, MSR_IA32_APICBASE, apic_msr_val);
    fprintf(stderr, "%s: err 0x%x writing 0x%llx to IA32 APICBASE MSR\n", __func__, r, apic_msr_val);
...
static void hvf_handle_interrupt(CPUState * cpu, int mask) {
    // fprintf(stderr, "%s: call mask 0x%x\n", __func__, mask);
        ...
        switch (exit_reason) {
            case EXIT_REASON_HLT: {
                ...
                if (!((cpu->interrupt_request & CPU_INTERRUPT_HARD) && (EFLAGS(cpu) & IF_MASK)) && !(cpu->interrupt_request & CPU_INTERRUPT_NMI) && !(idtvec_info & VMCS_IDT_VEC_VALID)) {
                    cpu->halted = 1;
                    // fprintf(stderr, "%s: EXIT HALT EXCP_HLT\n", __func__);
                    ret = EXCP_HLT;
                }
                    // fprintf(stderr, "%s: EXIT HALT EXCP_INTERRUP\n", __func__);
                ret = EXCP_INTERRUPT;
                break;
            }
            case EXIT_REASON_MWAIT: {
                // fprintf(stderr, "%s: EXIT MWAIT\n", __func__);
                // fprintf(stderr, "%s: EXIT_REASON_MWAT -> EXCP_INTERRUPT\n", __func__);
                ret = EXCP_INTERRUPT;
                break;
            }
            ...
            {
                // fprintf(stderr, "%s: EXIT EPTFAULT\n", __func__);
                hvf_slot *slot;
                ...
                if (ept_emulation_fault(exit_qual) && !slot) {
                    // fprintf(stderr, "%s: EXIT EPTFAULT: do mmio\n", __func__);
            ...
            case EXIT_REASON_CPUID: {
                ...
                // fprintf(stderr, "cpuid %lx at %lx\n", rax, rip);
                macvm_set_rip(cpu, rip + ins_len);
                break;
            }
            case EXIT_REASON_XSETBV: {
                // fprintf(stderr, "%s: EXIT XSETBV\n", __func__);
                X86CPU *x86_cpu = X86_CPU(cpu);
                CPUX86State *env = &x86_cpu->env;
            ...
            case EXIT_REASON_INTR_WINDOW:
                // fprintf(stderr, "%s: EXIT_REASON_INTR_WINDOW -> EXCP_INTERRUPT\n", __func__);
                // fprintf(stderr, "%s: EXIT INTR_WINDOW\n", __func__);
                vmx_clear_int_window_exiting(cpu);
                ret = EXCP_INTERRUPT;
                break;
            case EXIT_REASON_NMI_WINDOW:
                // fprintf(stderr, "%s: EXIT_REASON_NMI_WINDOW -> EXCP_INTERRUPT\n", __func__);
                // fprintf(stderr, "%s: EXIT NMI_WINDOW\n", __func__);
                vmx_clear_nmi_window_exiting(cpu);
                ret = EXCP_INTERRUPT;
                break;
            case EXIT_REASON_EXT_INTR:
                // fprintf(stderr, "%s: EXIT_REASON_EXT_INTR -> EXCP_INTERRUPT\n", __func__);
                // fprintf(stderr, "%s: EXIT EXT_INTR\n", __func__);
            ...
            {
                // fprintf(stderr, "%s: EXIT RDMSR | WRMSR\n", __func__);
                load_regs(cpu);
            ...
            case EXIT_REASON_CR_ACCESS: {
                // fprintf(stderr, "%s: EXIT CR_ACCESS\n", __func__);
                int cr;
                int reg;
                ...
                switch (cr) {
                    ...
                    case 8: {
                        ...
                        if (exit_qual & 0x10) {
                            ...
                            int tpr = RRX(cpu, reg);
                            cpu_set_apic_tpr(x86_cpu->apic_state, tpr);
                            fprintf(stderr, "%s: EXIT_REASON_CR_ACCESS -> EXCP_INTERRUPT\n", __func__);
                            ret = EXCP_INTERRUPT;
                        }
                        break;
                    }
            ...
            case EXIT_REASON_APIC_ACCESS: { // TODO
                // fprintf(stderr, "%s: EXIT APIC_ACCESS\n", __func__);
                struct x86_decode decode;
            ...
            case EXIT_REASON_TPR: {
                // fprintf(stderr, "%s: EXIT TPR\n", __func__);
                ret = 1;
                break;
            }
            case EXIT_REASON_TASK_SWITCH: {
                // fprintf(stderr, "%s: EXIT TASKSWITCH\n", __func__);
            ...
            case EXIT_REASON_TRIPLE_FAULT: {
                ...
                usleep(1000 * 100);
                // fprintf(stderr, "%s: EXIT_REASON_3xFAULT -> EXCP_INTERRUPT\n", __func__);
                ret = EXCP_INTERRUPT;
                break;
            }
            case EXIT_REASON_RDPMC:
                // fprintf(stderr, "%s: EXIT RDPMC\n", __func__);
                wreg(cpu->hvf_fd, HV_X86_RAX, 0);
                wreg(cpu->hvf_fd, HV_X86_RDX, 0);
                macvm_set_rip(cpu, rip + ins_len);
                break;
            case VMX_REASON_VMCALL:
                // fprintf(stderr, "%s: EXIT VMCALL\n", __func__);
...
static int hvf_accel_init(MachineState *ms) {
    int x;
    fprintf(stderr, "%s: call. hv vm create?\n", __func__);
