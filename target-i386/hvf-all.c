...
int hvf_init_vcpu(CPUState * cpu) {
    ...
    // TODO: Think through the APIC setup
    addr_t apic_gpa = 0xfec00000; // we seem to use 0xfec00000.
    if (!cpu->hvf_x86->apic_page) {
        posix_memalign(&cpu->hvf_x86->apic_page, 4096, 4096);
        memset(cpu->hvf_x86->apic_page, 0, 4096);
        hv_vm_map((hv_uvaddr_t)cpu->hvf_x86->apic_page, apic_gpa, 4096, HV_MEMORY_READ | HV_MEMORY_WRITE);
...
int hvf_handle_io(CPUArchState * env, uint16_t port, void* buffer,
    ...
    for (i = 0; i < count; i++) {
        address_space_rw(&address_space_io, port, cpu_get_mem_attrs(env),
                         ptr, size,
                         direction);
        ptr += size;
    }
    ...
    do {
        ...
        switch (exit_reason) {
            ...
            case EXIT_REASON_CPUID: {
                ...
                // TODO: get_cpuid_func(cpu, rax, rcx, &rax, &rbx, &rcx, &rdx);
