...
int __hvf_set_memory(hvf_slot *slot) {
    ...
    if (mapres) {
        // DPRINTF("register failed. err: 0x%x\n", mapres);
...
void vmx_reset_vcpu(CPUState *cpu) {
    ...
    DPRINTF("%s:%d arrive. msr apicbase +enable 0x%llx\n", __func__, __LINE__, apic_msr_val);
    if (cpu_is_bsp(X86_CPU(cpu))) {
        apic_msr_val |= MSR_IA32_APICBASE_BSP;
        DPRINTF("%s:%d arrive. msr apicbase +enable +bsp 0x%llx\n", __func__, __LINE__, apic_msr_val);
    }
    ...
    DPRINTF("%s: writing 0x%llx to IA32 APICBASE MSR\n", __func__, apic_msr_val);
...
int hvf_init_vcpu(CPUState * cpu) {
    ...
    if (r) {
        DPRINTF("%s: create vcpu failed. errcode 0x%x\n", __func__, r);
        print_hvf_error(r);
    }
    else
        DPRINTF("%s: success create vcpu. fd %llu\n", __func__, cpu->hvf_fd);
...
static int hvf_accel_init(MachineState *ms) {
    ...
    if (r) {
        DPRINTF("%s: hv_vm_create failed with err code %d\n", __func__, r);
    }
