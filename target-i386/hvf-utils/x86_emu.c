...
static bool reg_actually_in_host(struct CPUState* cpu, addr_t ptr) {
...
void write_val_ext(struct CPUState* cpu, addr_t ptr, addr_t val, int size)
{
    if (ptr > (addr_t)cpu && ptr < (addr_t)cpu + sizeof(struct CPUState))  {
        ...
        return;
    } else if (ptr > 0x7f0000000000) {
        fprintf(stderr, "%s: about to abort: cpuregrange is [0x%llx 0x%llx] us: 0x%llx\n", __func__, cpu, cpu + sizeof(struct CPUState), ptr);
        if (ptr > (addr_t)cpu->hvf_x86 && ptr < (addr_t)(cpu->hvf_x86 + sizeof(struct hvf_x86_state))) {
            fprintf(stderr, "%s: narrowly avoided\n", __func__);
            write_val_to_reg(ptr, val, size);
            return;
        }
    }
...
addr_t read_val_ext(struct CPUState* cpu, addr_t ptr, int size)
{
    ...
    if (reg_actually_in_host(cpu, ptr)) {
...
void simulate_rdmsr(struct CPUState *cpu)
{
    ...
    switch (msr) {
        ...
        case MSR_IA32_APICBASE:
            ...
            fprintf(stderr, "%s: read apicbase. val 0x%llx\n", __func__, (unsigned long long)val);
            break;
        ...
        case MSR_MTRRdefType:
            ...
            break;
        default:
            fprintf(stderr, "%s: unknoown msr 0x%x\n", __func__, msr);
            //printf("read msr %llx\n", RCX(cpu));
            val = 0;
            break;
    }
    ...
    if (msr == 0xfe) {
        fprintf(stderr, "%s: the hell kind of msr read val is this? 0x%llx\n", __func__, val);
    }
...
void simulate_wrmsr(struct CPUState *cpu)
{
    ...
    switch (msr) {
        case MSR_IA32_TSC:
            ...
            break;
        case MSR_IA32_APICBASE:
            fprintf(stderr, "MSR_IA32_APICBASE %llx\n", data);
