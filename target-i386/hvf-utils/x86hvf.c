...
void hvf_get_msrs(CPUState *cpu_state)
{
    ...
    fprintf(stderr, "%s: ret 0x%x apicbase msr: 0x%llx\n", __func__, r, tmp);
...
uint8_t* cpu_get_apic_vmx_page(DeviceState* dev) {
    if (dev) {
        ...
        fprintf(stderr, "%s: get apic page of %p\n", __func__, s);
        return s->vmx_apic_page;
    } else {
        fprintf(stderr, "%s: no apic dev!\n", __func__);
        return NULL;
    }
}
...
int hvf_put_registers(CPUState *cpu_state)
{
    ...
    fprintf(stderr, "%s: apic timer vals %d %d\n", __func__,
            (int32_t*)((char*)cpu_state->hvf_x86->apic_page) + 0x390,
            (int32_t*)((char*)vmx_apic) + 0x390);
    if (vmx_apic && cpu_state->hvf_x86->apic_page) {
        memcpy(cpu_state->hvf_x86->apic_page, vmx_apic, 4096);
    fprintf(stderr, "%s: apic timer vals %d %d\n", __func__,
            (int32_t*)((char*)cpu_state->hvf_x86->apic_page) + 0x390,
            (int32_t*)((char*)vmx_apic) + 0x390);
    } else {
        fprintf(stderr, "%s: NO apic page!\n", __func__);
        if (!vmx_apic) {
            fprintf(stderr, "%s: no apic page from apic state\n", __func__);
        } else {
            fprintf(stderr, "%s: no apic page from hvf state\n", __func__);
        }
    }
...
int hvf_get_registers(CPUState *cpu_state)
{
    ...
    uint8_t *vmx_apic = cpu_get_apic_vmx_page(x86cpu->apic_state);
    fprintf(stderr, "%s: apic timer vals %d %d\n", __func__,
            ((char*)cpu_state->hvf_x86->apic_page) + 0x390,
            vmx_apic + 0x390);
    if (vmx_apic && cpu_state->hvf_x86->apic_page) {
        memcpy(vmx_apic, cpu_state->hvf_x86->apic_page, 4096);
    fprintf(stderr, "%s: apic timer vals %d %d\n", __func__,
            (int32_t*)((char*)cpu_state->hvf_x86->apic_page) + 0x390,
            (int32_t*)((char*)vmx_apic) + 0x390);
    } else {
...
void hvf_inject_interrupts(CPUState *cpu_state)
{
    ...
    if (cpu_state->hvf_x86->interruptable && (cpu_state->interrupt_request & CPU_INTERRUPT_HARD) &&
        ...
        // fprintf(stderr, "%s: get pic interrupt :}. line 0x%x\n", __func__, line);
...
int hvf_process_events(CPUState *cpu_state)
{
    ...
    if (cpu_state->interrupt_request & CPU_INTERRUPT_INIT) {
        // fprintf(stderr, "%s: init interrupt\n", __func__);
    ...
    if (cpu_state->interrupt_request & CPU_INTERRUPT_POLL) {
        ...
        // fprintf(stderr, "%s: apic poll\n", __func__);
        apic_poll_irq(cpu->apic_state);
    }
    if (((cpu_state->interrupt_request & CPU_INTERRUPT_HARD) && (EFLAGS(cpu_state) & IF_MASK)) ||
        (cpu_state->interrupt_request & CPU_INTERRUPT_NMI)) {
        // fprintf(stderr, "%s: some other hard interrupt\n", __func__);
        cpu_state->halted = 0;
    }
    if (cpu_state->interrupt_request & CPU_INTERRUPT_SIPI) {
        // fprintf(stderr, "%s: SIPI interrupt\n", __func__);
        hvf_cpu_synchronize_state(cpu_state);
        do_cpu_sipi(cpu);
    }
    if (cpu_state->interrupt_request & CPU_INTERRUPT_TPR) {
        // fprintf(stderr, "%s: tpr interrupt\n", __func__);
