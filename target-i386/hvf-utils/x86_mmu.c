...
static bool test_pt_entry(struct CPUState *cpu, struct gpt_translation *pt, int level, bool *is_large, bool pae)
{
    ...
    if (!pte_present(pte)) {
        addr_t page_mask = pae ? PAE_PTE_PAGE_MASK : LEGACY_PTE_PAGE_MASK;
        fprintf(stderr, "%s:%d fail access bc pte not present? continue anyway? pae? %d level %d toplevel %d pte-gpa %llx (%llx)\n", __func__, __LINE__, pae, level, gpt_top_level(cpu, pae), pte, pte & page_mask);
        return false;
    }
    ...
    if (cr0 & CR0_WP) {
        if (pt->write_access && !pte_write_access(pte)) {
            fprintf(stderr, "%s:%d fail access\n", __func__, __LINE__);
            return false;
        }
    }
...
void vmx_write_mem(struct CPUState* cpu, addr_t gva, void *data, int bytes)
{
    ...
    while (bytes > 0) {
        ...
        if (gva > 0x7f0000000000) {
            fprintf(stderr, "%s: the hell kind of gva is htis? 0x%llx cpustate %p\n", __func__, gva, cpu);
            abort();
        }
        if (!mmu_gva_to_gpa(cpu, gva, &gpa)) {
            fprintf(stderr, "%s: mmu_gva_to_gpa %llx -> %llx failed. Continue anyway :)\n",  __FUNCTION__, gva, gpa);
            if (gva >= 0xffff2000 && gva <= 0xffff2000 + 0x1000) {
                gpa = 0xfee00000 + (gva - 0xffff2000);
                fprintf(stderr, "%s: Well, looks like a apic field. Set gpa = 0x%llx :)\n", __func__, gpa);
            } 
            // abort();
        } else {
...
void vmx_read_mem(struct CPUState* cpu, void *data, addr_t gva, int bytes)
{
    ...
    while (bytes > 0) {
        ...
        if (gpa == 0xfee00000 + 0x390) {
            fprintf(stderr, "%s: APIC TIMER GPA! (gva %llx)\n", __func__, gva);
        }
