...
void get_cpuid_func(struct CPUState* cpu, int func, int cnt, uint32_t *eax, uint32_t *ebx, uint32_t *ecx, uint32_t *edx)
{
   ...
   // TODO: Newer CPUs are going to have features that can break emulation if the guest kernel
   // detects them and tries to run with it. Either we need support in the HVF while running in vcpu level,
   ...
   // cpuid level	: 13
   // wp		: yes
   // flags		: fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush dts acpi mmx fxsr sse sse2 ss ht tm pbe syscall nx lm constant_tsc pebs bts eagerfpu pni pclmulqdq dtes64 ds_cpl ssse3 fma cx16 xtpr pdcm movbe popcnt aes xsave avx f16c rdrand hypervisor lahf_lm xsaveopt fsgsbase tsc_adjust bmi1 avx2 smep bmi2 erms
