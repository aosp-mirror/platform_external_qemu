...
addr_t get_reg_ref(CPUState *cpu, int reg, int is_extended, int size)
{
    ...
    if (ptr > 0x7f0000000000) {
        fprintf(stderr, "%s: ptr=0x%llx was set to addr of %d . cpu range: [0x%llx 0x%llx] hvfx86 range: [0x%llx 0x%llx]\n", __func__, ptr, which, cpu, cpu + sizeof(struct CPUState), cpu->hvf_x86, cpu->hvf_x86 + sizeof(struct hvf_x86_state));
    }
    return ptr;
}
