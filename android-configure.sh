...
if [ "$HOST_OS" = darwin ]; then
    ...
    OSX_SDK_SUPPORTED_ARRAY=($OSX_SDK_SUPPORTED)
    OSX_SDK_INSTALLED_ARRAY=($OSX_SDK_INSTALLED_LIST)
    OSX_SDK_VERSION=

    # compare each SDK version supported with each SDK version installed
    for SUPPORTED_VERSION in "${OSX_SDK_SUPPORTED_ARRAY[@]}"; do
        for INSTALLED_VERSION in "${OSX_SDK_INSTALLED_ARRAY[@]}"; do
