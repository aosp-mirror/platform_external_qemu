...
# $1: Source file path.
# $2: Destination file path (not a directory!) relative to OUT_DIR
install_prebuilt_dll () {
    ...
    if [ ! -L $SRC_PATH ]; then
        if [ ! -f $SRC_SYMBOL_PATH ]; then
            panic "Couldn't find symbol for $SRC_PATH : $SRC_SYMBOL_PATH"
        fi
...
for E2FS_ARCH in x86 x86_64; do
    ...
    mkdir -p "$E2FS_DSTDIR" || panic "Could not create sub-directory: $E2FS_DSTDIR"
    if [ ! -d "$E2FS_SRCDIR" ]; then
        continue
    fi
    cp -a "$E2FS_SRCDIR"/sbin/* "$E2FS_DSTDIR" || "Could not copy e2fsprogs binaries!"
done
