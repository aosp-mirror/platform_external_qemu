...
###
###  LibANGLE probe (Linux only currently)
###
if [ "$HOST_OS" = "linux" ]; then
    probe_prebuilts_dir "LibANGLE" ANGLE_PREBUILTS_DIR common/ANGLE
fi
...
if [ "$HOST_OS" = "linux" ]; then
    echo "ANGLE_PREBUILTS_DIR := $ANGLE_PREBUILTS_DIR" >> $config_mk
fi
