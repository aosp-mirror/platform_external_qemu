 ...
 * tlb_refill_info[cs->cpu_index].handler buffer which will later
...
static inline void tlb_refill_exception_prepare(CPUMIPSState *env)
{
    ...
    if (unlikely(tlb_refill_info[cs->cpu_index].state == PROBE)) {
        tlb_refill_info[cs->cpu_index].ebase = env->CP0_EBase & ~0x3FF;
        for (i = 0; i < TLB_HANDLER_SIZE; i++) {
            tlb_refill_info[cs->cpu_index].handler[i] = ldl_phys(cs->as, tlb_refill_info[cs->cpu_index].ebase - 0x80000000 + (i << 2));
            env->CPU.do_tlbwr = 1;
        }
        tlb_refill_info[cs->cpu_index].state = USEFASTTLB;
        dump_handler(&tlb_refill_info[cs->cpu_index]);
    }
}
...
static inline void tlb_exception_interpreter_prepare(CPUMIPSState *env, target_ulong address, int exception)
{
    ...
    if (likely(tlb_refill_info[cs->cpu_index].state != PROBE))
        return;
...
static inline int do_tlb_refill(CPUState *cs, vaddr address, int rw, int mmu_idx)
{
    ...
    if (unlikely(tlb_refill_info[cs->cpu_index].state == USESLOWTLB))
        goto out;

    tlb_exception_interpreter_prepare(env, address, EXCP_TLBF);

    if (tlb_exception_interpreter(env, tlb_refill_info[cs->cpu_index].handler, TLB_HANDLER_SIZE))
        goto out;
...
hwaddr mips_cpu_get_phys_page_debug(CPUState *cs, vaddr addr)
{
    ...
    if (ret != TLBRET_MATCH) {
        ...
        if (likely(tlb_refill_info[cs->cpu_index].state == USEFASTTLB)) {
            env->CPU.do_tlbwr = 0;
            if (tlb_exception_interpreter(env, tlb_refill_info[cs->cpu_index].handler, TLB_HANDLER_SIZE) != 0) {
                phys_addr = -1;
