...
void x86_cpu_build_from_features(const X86CPUDefinition *tmpl,
                                 X86CPUDefinition *dst,
                                 const char *name,
                                 uint32_t ecx,
                                 uint32_t edx)
{
    *dst = *tmpl;
    dst->name = name;
    dst->features[FEAT_1_ECX] |= ecx;
    dst->features[FEAT_1_EDX] |= edx;
}
...
{
    const X86CPUDefinition *tmpl = x86_cpu_find_tmpl(cpus, cpus_size, cpu_name);
    if (tmpl) {
        uint32_t eax, ebx, ecx, edx;
        host_cpuid(1, 0, &eax, &ebx, &ecx, &edx);

        x86_cpu_build_from_features(tmpl, dst, new_cpu_name,
