...
int hax_set_ram(uint64_t start_pa, uint64_t size, uint64_t host_va, int flags)
{
    ...
    if (hax_global.supports_64bit_setram) {
        ...
        ret = DeviceIoControl(hDeviceVM, HAX_VM_IOCTL_SET_RAM2,
                              &info, sizeof(info), NULL, 0, &dSize,
                              (LPOVERLAPPED) NULL);
    } else {
...
int hax_gpa_prot(uint64_t start_pa, uint64_t size, int flags)
{
    HANDLE hDeviceVM = hax_global.vm->fd;
    DWORD dSize = 0;
    int ret;
    ...
    ret = DeviceIoControl(hDeviceVM, HAX_VM_IOCTL_GPA_PROT,
                          &info, sizeof(info), NULL, 0, &dSize,
                          (LPOVERLAPPED) NULL);
    if (!ret) {
        return -EFAULT;
