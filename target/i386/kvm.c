...
extern mword ASMCALL asm_vmread(uint32_t component);
...
static uint64_t vmread(component_index_t component) {
    return asm_vmread(component);
}


uint64_t vmread_dump(uint32_t enc, const char *name)
{
    uint64_t val = 0;
...
void dump_vmcs()
{
    ...
    for (i = 0; i < n; i++) {
        ...
        printf("dumping 0x%x, name %s\n", enc, name);
        vmread_dump(enc, name);
    }
}
