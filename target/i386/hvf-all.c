...
int hvf_handle_io(CPUArchState *env, uint16_t port, void *buffer, int direction,
                  int size, int count);
...
int hvf_hva2gpa(void *hva, uint64_t length, int array_size, uint64_t *gpa,
                uint64_t *size) {
    struct mac_slot *mslot;
    int count = 0;
    ...
    for (uint32_t i = 0; i < MAC_SLOTS_COUNT; i++) {
        ...
        if (hva_num >= hva_start_num && hva_num < hva_start_num + mslot->size) {
            if (count < array_size) {
                gpa[count] = mslot->gpa_start + (hva_num - hva_start_num);
                size[count] =
                    min(length, mslot->size - (hva_num - hva_start_num));
            }
            count++;
...
int __hvf_set_memory(hvf_slot *slot) {
    ...
    DPRINTF("%s: hv_vm_map for hva 0x%llx gpa [0x%llx 0x%llx]\n", __func__,
            ...
            (unsigned long long)(macslot->gpa_start + macslot->size));
    int mapres =
        (hv_vm_map((hv_uvaddr_t)slot->mem, slot->start, slot->size, flags));
...
static MemoryListener hvf_memory_listener = {
    .priority = 10, .region_add = hvf_region_add, .region_del = hvf_region_del,
};
...
void hvf_disable(int shouldDisable) { hvf_disabled = shouldDisable; }
...
void vmx_reset_vcpu(CPUState *cpu) {
    ...
    for (int i = 0; i < 8; i++) wreg(cpu->hvf_fd, HV_X86_R8 + i, 0x0);
...
int hvf_init_vcpu(CPUState *cpu) {
    ...
    cpu->hvf_caps =
        (struct hvf_vcpu_caps *)g_malloc0(sizeof(struct hvf_vcpu_caps));
    cpu->hvf_x86 =
        (struct hvf_x86_state *)g_malloc0(sizeof(struct hvf_x86_state));
    ...
    wvmcs(
        cpu->hvf_fd, VMCS_PRI_PROC_BASED_CTLS,
        cap2ctrl(cpu->hvf_caps->vmx_cap_procbased,
                 VMCS_PRI_PROC_BASED_CTLS_HLT | VMCS_PRI_PROC_BASED_CTLS_MWAIT |
    ...
    wvmcs(cpu->hvf_fd, VMCS_ENTRY_CTLS,
          cap2ctrl(cpu->hvf_caps->vmx_cap_entry, 0));
    ...
    x86cpu = X86_CPU(cpu);
    x86cpu->env.kvm_xsave_buf =
        qemu_memalign(4096, sizeof(struct hvf_xsave_buf));
...
int hvf_handle_io(CPUArchState *env, uint16_t port, void *buffer, int direction,
    ...
    for (i = 0; i < count; i++) {
        address_space_rw(&address_space_io, port, MEMTXATTRS_UNSPECIFIED, ptr,
                         size, direction);
        ptr += size;
    }
}
...
void __hvf_cpu_synchronize_state(void *data) {
    ...
    if (cpu_state->hvf_vcpu_dirty == 0) hvf_get_registers(cpu_state);
...
void hvf_cpu_clean_state(CPUState *cpu_state) { cpu_state->hvf_vcpu_dirty = 0; }
...
static bool ept_emulation_fault(uint64_t ept_qual) {
    ...
    if (ept_qual & EPT_VIOLATION_INST_FETCH) return false;

    /* EPT fault must be a read fault or a write fault */
    read = ept_qual & EPT_VIOLATION_DATA_READ ? 1 : 0;
    write = ept_qual & EPT_VIOLATION_DATA_WRITE ? 1 : 0;
    if ((read | write) == 0) return false;
...
static void load_state_from_tss32(CPUState *cpu,
                                  struct x86_tss_segment32 *tss) {
...
static void vmx_handle_task_switch(CPUState *cpu, x68_segment_selector tss_sel,
    ...
    int ret;
    x68_segment_selector old_tss_sel =
        vmx_read_segment_selector(cpu, REG_SEG_TR);
    ...
    if (reason == TSR_IDT_GATE && gate_valid) {
        ...
        if (tss_sel.rpl > dpl || cs.rpl > dpl) DPRINTF("emulate_gp");
    }
    ...
    if (reason == TSR_IRET) EFLAGS(cpu) &= ~RFLAGS_NT;

    if (reason != TSR_CALL && reason != TSR_IDT_GATE) old_tss_sel.sel = 0xffff;
...
static int apic_fls_bit(uint32_t value) { return 31 - clz32(value); }

/* Find first bit starting from lsb */
static int apic_ffs_bit(uint32_t value) { return ctz32(value); }
...
void vmx_update_tpr(CPUState *cpu) {
    ...
    if (irr == -1)
        ...
        wvmcs(cpu->hvf_fd, VMCS_TPR_THRESHOLD,
              (irr > tpr) ? tpr >> 4 : irr >> 4);
}
...
again:
    ...
    do {
        ...
        uint32_t ins_len =
            (uint32_t)rvmcs(cpu->hvf_fd, VMCS_EXIT_INSTRUCTION_LENGTH);
...
#ifdef DIRTY_VGA_TRACKING  // Don't try to build HVF with dirty vga tracking for
                           // now.
#error
#endif
                if (slot) {
            ...
            case EXIT_REASON_APIC_ACCESS: {  // TODO
                ...
                DPRINTF("apic fetched %s, %x %x len %d\n",
                        decode_cmd_to_string(decode.cmd), decode.opcode[0],
                        decode.opcode[1], decode.len);
                exec_instruction(cpu, &decode);
                store_regs(cpu);
                break;
            }
            ...
            case EXIT_REASON_TRIPLE_FAULT: {
                ...
                DPRINTF(
                    "triple fault at %llx (%llx, %llx), cr0 %llx, qual %llx, "
                    "gpa %llx, ins len %d, cpu %p\n",
                    linear_rip(cpu, rip), RIP(cpu),
                    linear_addr(cpu, rip, REG_SEG_CS),
                    rvmcs(cpu->hvf_fd, VMCS_GUEST_CR0), exit_qual, gpa, ins_len,
                    cpu);
            ...
            case VMX_REASON_VMCALL:
                ...
                //     RIP(cpu) += rvmcs(cpu->hvf_fd,
                //     VMCS_EXIT_INSTRUCTION_LENGTH); store_regs(cpu);
                // }
                break;
            default:
