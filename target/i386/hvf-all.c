...
int __hvf_set_memory_with_flags(hvf_slot *slot, hv_memory_flags_t flags);
...
int hvf_map_safe(void* hva, uint64_t gpa, uint64_t size, uint64_t flags) {
    ...
    int res = __hvf_set_memory_with_flags(mem, (hv_memory_flags_t)flags);
...
int __hvf_set_memory(hvf_slot *slot) {
    return __hvf_set_memory_with_flags(slot, HV_MEMORY_READ | HV_MEMORY_WRITE | HV_MEMORY_EXEC);
}
