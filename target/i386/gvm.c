...
#ifdef GVM_CAP_PIT_STATE2
    has_pit_state2 = gvm_check_extension(s, GVM_CAP_PIT_STATE2);
#endif
...
#if 0
/* Classic GVM device assignment interface. Will remain x86 only. */
int gvm_device_pci_assign(GVMState *s, PCIHostDeviceAddress *dev_addr,
                          uint32_t flags, uint32_t *dev_id)
{
    struct gvm_assigned_pci_dev dev_data = {
    ...
    ret = gvm_vm_ioctl(s, GVM_ASSIGN_PCI_DEVICE, &dev_data);
    if (ret < 0) {
        return ret;
    }
...
int gvm_device_pci_deassign(GVMState *s, uint32_t dev_id)
{
    ...
    return gvm_vm_ioctl(s, GVM_DEASSIGN_PCI_DEVICE, &dev_data);
}

static int gvm_assign_irq_internal(GVMState *s, uint32_t dev_id,
                                   uint32_t irq_type, uint32_t guest_irq)
{
    struct gvm_assigned_irq assigned_irq = {
        .assigned_dev_id = dev_id,
        .guest_irq = guest_irq,
        .flags = irq_type,
    };

    if (gvm_check_extension(s, GVM_CAP_ASSIGN_DEV_IRQ)) {
        return gvm_vm_ioctl(s, GVM_ASSIGN_DEV_IRQ, &assigned_irq);
    } else {
        return gvm_vm_ioctl(s, GVM_ASSIGN_IRQ, &assigned_irq);
    }
}

int gvm_device_intx_assign(GVMState *s, uint32_t dev_id, bool use_host_msi,
                           uint32_t guest_irq)
{
    uint32_t irq_type = GVM_DEV_IRQ_GUEST_INTX |
        (use_host_msi ? GVM_DEV_IRQ_HOST_MSI : GVM_DEV_IRQ_HOST_INTX);

    return gvm_assign_irq_internal(s, dev_id, irq_type, guest_irq);
}
...
int gvm_device_intx_set_mask(GVMState *s, uint32_t dev_id, bool masked)
{
    struct gvm_assigned_pci_dev dev_data = {
        .assigned_dev_id = dev_id,
        .flags = masked ? GVM_DEV_ASSIGN_MASK_INTX : 0,
    };

    return gvm_vm_ioctl(s, GVM_ASSIGN_SET_INTX_MASK, &dev_data);
}

static int gvm_deassign_irq_internal(GVMState *s, uint32_t dev_id,
                                     uint32_t type)
{
    struct gvm_assigned_irq assigned_irq = {
        .assigned_dev_id = dev_id,
    ...
    return gvm_vm_ioctl(s, GVM_DEASSIGN_DEV_IRQ, &assigned_irq);
}

int gvm_device_intx_deassign(GVMState *s, uint32_t dev_id, bool use_host_msi)
{
    return gvm_deassign_irq_internal(s, dev_id, GVM_DEV_IRQ_GUEST_INTX |
        (use_host_msi ? GVM_DEV_IRQ_HOST_MSI : GVM_DEV_IRQ_HOST_INTX));
}

int gvm_device_msi_assign(GVMState *s, uint32_t dev_id, int virq)
{
    return gvm_assign_irq_internal(s, dev_id, GVM_DEV_IRQ_HOST_MSI |
                                              GVM_DEV_IRQ_GUEST_MSI, virq);
}

int gvm_device_msi_deassign(GVMState *s, uint32_t dev_id)
{
    return gvm_deassign_irq_internal(s, dev_id, GVM_DEV_IRQ_GUEST_MSI |
                                                GVM_DEV_IRQ_HOST_MSI);
}

bool gvm_device_msix_supported(GVMState *s)
{
    /* The kernel lacks a corresponding GVM_CAP, so we probe by calling
     * GVM_ASSIGN_SET_MSIX_NR with an invalid parameter. */
    return gvm_vm_ioctl(s, GVM_ASSIGN_SET_MSIX_NR, NULL) == -EFAULT;
}

int gvm_device_msix_init_vectors(GVMState *s, uint32_t dev_id,
                                 uint32_t nr_vectors)
{
    struct gvm_assigned_msix_nr msix_nr = {
        .assigned_dev_id = dev_id,
        .entry_nr = nr_vectors,
    };

    return gvm_vm_ioctl(s, GVM_ASSIGN_SET_MSIX_NR, &msix_nr);
}

int gvm_device_msix_set_vector(GVMState *s, uint32_t dev_id, uint32_t vector,
                               int virq)
{
    struct gvm_assigned_msix_entry msix_entry = {
        .assigned_dev_id = dev_id,
        .gsi = virq,
        .entry = vector,
    };

    return gvm_vm_ioctl(s, GVM_ASSIGN_SET_MSIX_ENTRY, &msix_entry);
}

int gvm_device_msix_assign(GVMState *s, uint32_t dev_id)
{
    return gvm_assign_irq_internal(s, dev_id, GVM_DEV_IRQ_HOST_MSIX |
                                              GVM_DEV_IRQ_GUEST_MSIX, 0);
}

int gvm_device_msix_deassign(GVMState *s, uint32_t dev_id)
{
    return gvm_deassign_irq_internal(s, dev_id, GVM_DEV_IRQ_GUEST_MSIX |
                                                GVM_DEV_IRQ_HOST_MSIX);
}
                             ...
                             uint64_t address, uint32_t data, PCIDevice *dev)
{
    ...
    if (iommu) {
        ...
        if (ret) {
            trace_gvm_x86_fixup_msi_error(route->gsi);
            return 1;
        }
                                      ...
                                      uint32_t index, uint32_t mask)
{
    ...
    trace_gvm_x86_update_msi_routes(cnt);
}
                                ...
                                int vector, PCIDevice *dev)
{
    ...
    trace_gvm_x86_add_msi_route(route->gsi);
...
int gvm_arch_release_virq_post(int virq)
{
    ...
    QLIST_FOREACH_SAFE(entry, &msi_route_list, list, next) {
        if (entry->virq == virq) {
            trace_gvm_x86_remove_msi_route(virq);
