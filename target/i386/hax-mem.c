...
static void hax_dump_memslots(void)
{
    ...
    for (; x < HAX_MAX_SLOTS; ++x) {
        slot = &hax_global.memslots[x];
...
#define HAX_SET_PHYS_MEM_DUMP(fmt,...) do {\
    ...
    fprintf(stderr, "%s: msg: " fmt "n", __func__, ##__VA_ARGS__); \
    ...
    fprintf(stderr, "%s: split slot id: %d needed free for split: %d adding to middle: %d\n", __func__, split_slot_id, split_needed_free_slots, add_to_middle); \
} while (0); \
...
#define HAX_SET_PHYS_MEM_DUMP(fmt,...)
    ...
    for (i = 0; i < HAX_MAX_SLOTS; i++) {
        ...
        free_slots[i] = mem->flags & HAX_RAM_INFO_INVALID;
    ...
    // slot.
    //
    // It's possible to exit early here if we are adding a region and it wouldn't
    // change the set of regions at all.
    for (i = 0; i < HAX_MAX_SLOTS; i++) {
        int overlap;
        int will_split;
        uint64_t slot_start;
        ...
        {
            int removing = !add;
            int flag_changing = add && (flags != slot->flags);
            int should_split = removing || flag_changing;
            ...
            if (will_split) {
                split_slot_id = i;
                split_needed_free_slots = flag_changing ? 2 : 1;
            } else {
                split_slot_id = -1;
                split_needed_free_slots = 0;
            }

            // Record whether we are adding the slot to the middle of an existing slot
            // with the same flags (in other words, no-op)
            add_to_middle = add && (flags == slot->flags);
        }
    ...
    // Validation: check that if we have more than one overlap, we should not need to split.
    if (overlap_slot_count > 1 &&
        split_slot_id != -1) {
        qemu_abort("FATAL: Have more than one overlap slot, but we are splitting a single slot.\n");
    }
    ...
    for (i = 0; i < HAX_MAX_SLOTS; ++i) {
        ...
        // This is not the split region, so we can use the following calculation of
        // overlap interval and remaining interval.
        ...
        if (hax_set_ram(remaining_start,
                        remaining_end - remaining_start,
                        (uint64_t)(uintptr_t)((char*)slot->hva + (uintptr_t)(remaining_start - slot_start)),
                        slot->flags)) {
    ...
    if (split_needed_free_slots > free_slot_count) {
        qemu_abort("FATAL: needed to split a slot, but we have no free slots (%d needed).\n", split_needed_free_slots);
    }
    ...
    if (split_slot_id != -1) {
        ...
        if (free_slot_id == -1) {
            qemu_abort("FATAL: splitting slots, could not find a free slot.\n");
        }
        ...
        right_slot->hva = slot->hva + (uintptr_t)(split_right_start - slot_start);
        right_slot->flags = flags;

        if (hax_set_ram(right_slot->start,
                        right_slot->size,
                        (uint64_t)(uintptr_t)right_slot->hva,
                        right_slot->flags)) {
            qemu_abort("FATAL: splitting slots, could not allocate new slot\n");
        }
    ...
    if (free_slot_count == 0) {
        qemu_abort("FATAL: no free slots for adding new region\n");
    }
    ...
    // Obtain the free slot.
    {
        ...
        if (!slot) {
            qemu_abort("FATAL: free slot count does not match free slot state\n");
        }

        // Validation: Check that the slot itself is marked as free.
        if (!(slot->flags & HAX_RAM_INFO_INVALID)) {
            qemu_abort("FATAL: free slot state does not match slot flags\n");
        }
