...
static void hvf_handle_mmio(CPUState* cpu) {
    ...
    if (is_write) {
        uint64_t guest_reg_val = env->xregs[rt == 31 ? 0 : rt];
        switch (len) {
            case 1:
        ...
        DPRINTF("%s: mmio read val 0x%llx to rt %lu %lu\n", __func__, (unsigned long long)val, rt, rt == 31 ? 0 : rt);
        env->xregs[rt == 31 ? 0 : rt] = val;
        DPRINTF("%s: after %llu\n", __func__,
               (unsigned long long)env->xregs[rt == 31 ? 0 : rt]);
    }
...
static void hvf_handle_exception(CPUState* cpu) {
    ...
    };
    {
        ARMCPU* armcpu = ARM_CPU(cpu);
        CPUARMState* env = &armcpu->env;
        DPRINTF("%s: post put regs 0x%llx\n", __func__, (unsigned long long)env->xregs[4]);
    }
    hvf_put_registers(cpu);
