...
void HangDetector::LooperWatcher::process(bool crashOnHang) {
    base::AutoLock l(*mLock);
...
HangDetector::HangDetector(bool crashOnHangs)
    : mCrashOnHangs(crashOnHangs), mWorkerThread([this]() { workerThread(); }) {
    mWorkerThread.start();
}
