...
void StructuredInfo::setHostHwInfo(const HostHwInfo::Info& info) {
    ...
    android_studio::EmulatorHost* hostinfo = sCrashProto.get().mutable_metrics_host_info();

    hostinfo->set_virt_support(info.virt_support);
    hostinfo->set_running_in_vm(info.running_in_vm);

    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    for (const auto& hostgpu : info.gpuinfolist->infos) {
        android_studio::EmulatorGpuInfo* outGpu =
            details->mutable_metrics_emulator_details()->add_host_gpu();
...
toProtoGuestArch(const char* hw_cpu_arch) {
    static constexpr std::pair<StringView,
                               android_studio::EmulatorDetails::GuestCpuArchitecture>
            map[] = {
                    {"x86", android_studio::EmulatorDetails::X86},
                    {"x86_64", android_studio::EmulatorDetails::X86_64},
                    {"arm", android_studio::EmulatorDetails::ARM},
                    {"arm64", android_studio::EmulatorDetails::ARM_64},
                    {"mips", android_studio::EmulatorDetails::MIPS},
                    {"mips64", android_studio::EmulatorDetails::MIPS_64},
            };
    ...
    return android_studio::EmulatorDetails::UNKNOWN_GUEST_CPU_ARCHITECTURE;
}
...
void StructuredInfo::setAvdInfo(AvdInfo* info) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    android_studio::EmulatorAvdInfo* avd =
        details->mutable_metrics_emulator_details()->mutable_avd_info();
    ...
    if (avdInfo_isPhone(info)) {
        details->add_avd_props(EmulatorCrashInfoDetails::PHONE_AVD);
    }
    if (avdInfo_isTv(info)) {
        details->add_avd_props(EmulatorCrashInfoDetails::TV_AVD);
    }
    if (avdInfo_isWear(info)) {
        details->add_avd_props(EmulatorCrashInfoDetails::WEAR_AVD);
    }
    if (avdInfo_isAndroidAuto(info)) {
        details->add_avd_props(EmulatorCrashInfoDetails::ANDROIDAUTO_AVD);
    }
}
...
void StructuredInfo::setSessionPhase(AndroidSessionPhase phase) {
    ...
    details->mutable_metrics_emulator_details()->set_session_phase(
...
void StructuredInfo::setHypervisor(CpuAccelerator accel) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    EmulatorCrashInfoDetails::Hypervisor protoAccel =
        EmulatorCrashInfoDetails::UNKNOWN_HYPERVISOR;
    switch (accel) {
        case CPU_ACCELERATOR_NONE:
            protoAccel = EmulatorCrashInfoDetails::NO_HYPERVISOR;
            break;
        case CPU_ACCELERATOR_KVM:
            protoAccel = EmulatorCrashInfoDetails::KVM;
            break;
        case CPU_ACCELERATOR_HAX:
            protoAccel = EmulatorCrashInfoDetails::HAXM;
            break;
...
void StructuredInfo::setRenderer(SelectedRenderer renderer) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    details->mutable_metrics_emulator_details()->set_renderer((android_studio::EmulatorDetails::EmulatorRenderer)renderer);
}
...
void StructuredInfo::setUptime(uint64_t uptimeMs) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    details->mutable_metrics_emulator_details()->set_wall_time((int64_t)uptimeMs);
}
...
void StructuredInfo::setGuestGlVendor(const char* vendor) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    details->mutable_metrics_emulator_details()->mutable_guest_gl()->set_vendor(vendor);
}
...
void StructuredInfo::setGuestGlRenderer(const char* renderer) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    details->mutable_metrics_emulator_details()->mutable_guest_gl()->set_renderer(renderer);
}
...
void StructuredInfo::setGuestGlVersion(const char* version) {
    ...
    AutoLock lock(mLock);
    EmulatorCrashInfoDetails* details = sCrashProto.get().mutable_emulator_details();
    details->mutable_metrics_emulator_details()->mutable_guest_gl()->set_version(version);
}
