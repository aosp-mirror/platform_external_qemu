...
void StructuredInfo::setAvdInfo(AvdInfo* info) {
    ...
    if (avdInfo_isPhone(info)) {
        avd->add_image_prop(EmulatorAvdInfo::PHONE_AVD);
    }
    if (avdInfo_isTv(info)) {
        avd->add_image_prop(EmulatorAvdInfo::TV_AVD);
    }
    if (avdInfo_isWear(info)) {
        avd->add_image_prop(EmulatorAvdInfo::WEAR_AVD);
    }
    if (avdInfo_isAndroidAuto(info)) {
        avd->add_image_prop(EmulatorAvdInfo::ANDROIDAUTO_AVD);
    }
