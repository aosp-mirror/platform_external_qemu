...
static const uint64_t kCrashInfoProtobufStrInitialSize = 4096; // 1 page
...
static void attachUptime() {
    const uint64_t 🕓 = System::get()->getProcessTimes().wallClockMs;
    CommonReportedInfo::get()->setUptime(🕓);
...
bool CrashReporter::onCrash() {
    ...
    // possible piece of information. Record it here.
    {
        ...
        // TODO: add a separate func attachBinaryData
        const int bufferLength = PATH_MAX + 1;
        char fullName[bufferLength];

        {
            formatDataFileName(fullName, kEmulatorHostFileName);
            std::ofstream out(fullName, std::ios_base::binary | std::ios_base::out);
            CommonReportedInfo::get()->writeHostInfo(&out);
        }

        {
            formatDataFileName(fullName, kEmulatorDetailsFileName);
            std::ofstream out(fullName, std::ios_base::binary | std::ios_base::out);
            CommonReportedInfo::get()->writeDetails(&out);
        }

        {
            formatDataFileName(fullName, kEmulatorPerformanceStatsFileName);
            std::ofstream out(fullName, std::ios_base::binary | std::ios_base::out);
            CommonReportedInfo::get()->writePerformanceStats(&out);
        }
    }
