...
bool HostCrashService::getHWInfo() {
    ...
    std::string utf8Path = PathUtils::join(dataDirectory, kHwInfoName);
    {
        ...
        if (!System::get()->runCommand(
            ...
            std::ofstream out(base::PathUtils::asUnicodePath(utf8Path));
    ...
    do {
        if (System::get()->pathCanRead(utf8Path)) {
            std::ifstream in(base::PathUtils::asUnicodePath(utf8Path));
            if (in) {
                return true;
            }
        }
...
bool HostCrashService::getMemInfo() {
    ...
    std::ofstream fout(base::PathUtils::asUnicodePath(path));
    if (!fout) {
