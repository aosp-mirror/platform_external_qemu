...
inline bool ProtoCompare(const ProtoComparison& comp,
    ...
    return ProtoCompare(comp, actual,
                        MakePartialProtoFromAscii<Proto>(expected));
}
...
public:
    ...
    virtual const google::protobuf::Message* CreateExpectedProto(
            const google::protobuf::Message&
                    arg,  // For determining the type of the
                          // expected protobuf.
    ...
    void SetCompareTreatingNaNsAsEqual() {
        comp_->treating_nan_as_equal = true;
    }
                ...
                *os << "(";
                if (comp_->has_custom_margin) {
                    ...
                    ss << std::setprecision(
                       ...
                       << comp_->float_margin;
                    *os << "absolute error of float or double fields <= "
                        << ss.str();
                }
                if (comp_->has_custom_margin && comp_->has_custom_fraction) {
                    *os << " or ";
                ...
                if (comp_->has_custom_fraction) {
                    ...
                    *os << "relative error of float or double fields <= "
                        << ss.str();
                }
                *os << ") ";
            }
    ...
    void DescribeNegationTo(::std::ostream* os) const {
        *os << "is "
            << (must_be_initialized_ ? "not fully initialized or " : "")
            << "not ";
        DescribeRelationToExpectedProto(os);
    }
    ...
    virtual const google::protobuf::Message* CreateExpectedProto(
        ...
        if (ParsePartialFromAscii(expected_, expected_proto, &error_text)) {
            ...
            if (listener->IsInterested()) {
                ...
                *listener << " doesn't parse as a "
                          << arg.GetDescriptor()->full_name() << ":\n"
                          << error_text;
            }
            return NULL;
        }
...
public:
    ...
    bool MatchAndExplain(google::protobuf::io::ZeroCopyInputStream* arg,
        ...
        *os << "which deserializes to ";
        ...
        const bool match = proto_matcher_.MatchAndExplain(*deserialized_arg,
                                                          &inner_listener);
...
public:
    explicit WhenDeserializedMatcher(
            const PolymorphicProtoMatcher& proto_matcher)
...
inline ::testing::PolymorphicMatcher<internal::ProtoStringMatcher> EqualsProto(
    ...
    internal::ProtoComparison comp;
    comp.field_comp = internal::kProtoEqual;
    return ::testing::MakePolymorphicMatcher(internal::ProtoStringMatcher(
            x, internal::kMayBeUninitialized, comp));
}
template <class Proto>
...
EqualsInitializedProto(const std::string& x) {
    ...
    comp.field_comp = internal::kProtoEqual;
    return ::testing::MakePolymorphicMatcher(internal::ProtoStringMatcher(
            x, internal::kMustBeInitialized, comp));
}
template <class Proto>
inline internal::PolymorphicProtoMatcher EqualsInitializedProto(
...
inline InnerProtoMatcher IgnoringRepeatedFieldOrdering(
        InnerProtoMatcher inner_proto_matcher) {
    inner_proto_matcher.mutable_impl()
            .SetCompareRepeatedFieldsIgnoringOrdering();
    return inner_proto_matcher;
}
