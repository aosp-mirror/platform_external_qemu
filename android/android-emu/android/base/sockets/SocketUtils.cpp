...
static int socketTcpLoopbackClientFor(int port, int domain) {
    ...
    if (connres < 0) {
        ...
        // EINPROGRESS macro is screwed up, just define it ourselves here
        int inprogress = 115;
        if (selectRes > 0 && errno != inprogress) {
            if (domain == AF_INET) {
            ...
            fprintf(stderr, "%s: inprogress %d\n", __func__, inprogress);
            if (errno == inprogress) {
                if (domain == AF_INET) {
                    fprintf(stderr, "%s: ipv4 loopback connection already in progress, bail %d %d\n", __func__, selectRes, errno);
                } else {
                    fprintf(stderr, "%s: ipv6 loopback connection already in progress, bail %d %d\n", __func__, selectRes, errno);
                }
            } else {
                if (domain == AF_INET) {
