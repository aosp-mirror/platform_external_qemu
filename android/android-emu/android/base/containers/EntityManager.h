...
public:
    ...
    ComponentHandle add(
        ...
        auto res = static_cast<ComponentHandle>(mData.add(h, type));
    ...
    void removeByComponent(ComponentHandle h) {
        auto data = getByComponent(h);

        if (!data) return;
        if (data->tracked) mEntityToComponentMap.erase(data->entityHandle);
