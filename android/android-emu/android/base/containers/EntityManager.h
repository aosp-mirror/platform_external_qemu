...
public:
    ...
    EntityHandle addFixed(EntityHandle fixedHandle, const Item& item, size_t type) {
        ...
        if (isFreeListHead) {
            ...
            auto& prevEntry = mEntries[mFirstFreeIndex];

            while (prevEntry.nextFreeIndex != newIndex) {
                prevEntry = mEntries[prevEntry.nextFreeIndex];
            }

            prevEntry.nextFreeIndex = mEntries[newIndex].nextFreeIndex;
