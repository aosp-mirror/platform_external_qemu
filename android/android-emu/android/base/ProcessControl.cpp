...
void initializeEmulatorRestartParameters(int argc, char** argv, const char* outPath) {
    ...
    fprintf(stderr, "%s: saved launch parameters\n", __func__);
    fprintf(stderr, "%s\n", params.str().c_str());
}
...
void finalizeEmulatorRestartParameters(const char* dir) {
    ...
    // Add or replace the "-restart" option to make the launch wait for process exit next time
    ...
    if (hasRestart) {
        fprintf(stderr, "%s: has restart arg already\n", __func__);
        params.argv[restartArgIndex + 1] = ss.str();
    } else {
        params.argv.push_back("-is-restart");
        params.argv.push_back(ss.str());
    }
    
    fprintf(stderr, "%s: finalized launch parameters\n", __func__);
    fprintf(stderr, "%s\n", params.str().c_str());
