...
class QuitAndRestart {
public:
    QuitAndRestart() = default;
...
static android::base::LazyInstance<QuitAndRestart> sQuitAndRestart = LAZY_INSTANCE_INIT;

void registerEmulatorQuitCallback(FunctionView<void()> func) {
    sQuitAndRestart->registerQuit(func);
}

void restartEmulator() {
    sQuitAndRestart->restart();
}
