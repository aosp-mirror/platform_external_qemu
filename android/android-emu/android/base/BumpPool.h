...
class BumpPool {
public:
    BumpPool(size_t startingBytes = 4096) : mStorage(startingBytes)  { }
    ...
    void* alloc(size_t wantedSize) {
        mTotalWantedThisGeneration += wantedSize;
        if (mAllocPos + wantedSize > mStorage.size()) {
            mNeedRealloc = true;
            void* fallbackPtr = malloc(wantedSize);
            mFallbackPtrs.insert(fallbackPtr);
            return fallbackPtr;
        }
        size_t avail = mStorage.size() - mAllocPos;
        void* allocPtr = (void*)(mStorage.data() + mAllocPos);
        mAllocPos += wantedSize;
        return allocPtr;
    }
