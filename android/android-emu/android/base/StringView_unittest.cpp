...
TEST(StringView, NullTerminated) {
    ...
    // But when wrapped in NullTerminated, it is copied.
    EXPECT_STREQ(NullTerminated(firstByte).c_str(), "0");

    // If the string is already null terminated, it doesn't copy.
    EXPECT_EQ(NullTerminated(str).c_str(), kString);

    EXPECT_STREQ(NullTerminated(StringView()).c_str(), "");
    EXPECT_STREQ(NullTerminated(StringView("")).c_str(), "");
    EXPECT_STREQ(NullTerminated(StringView(kString, 0ul)).c_str(), "");
}
