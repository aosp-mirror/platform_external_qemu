...
long ring_buffer_view_write(
    ...
    for (i = 0; i < steps; ++i) {
        ...
        if (step_size > available_at_end) {
            uint32_t remaining = step_size - available_at_end;
            memcpy(
                &r->buf[ring_buffer_view_get_ring_pos(v, r->write_pos)],
                data_bytes + i * step_size,
                available_at_end);
            memcpy(
                &r->buf[ring_buffer_view_get_ring_pos(v, r->write_pos + available_at_end)],
                data_bytes + i * step_size + available_at_end,
                remaining);
        } else {
            memcpy(
...
long ring_buffer_view_read(
    ...
    for (i = 0; i < steps; ++i) {
        ...
        if (step_size > available_at_end) {
            ...
            memcpy(
                data_bytes + i * step_size,
                &r->buf[ring_buffer_view_get_ring_pos(v, r->read_pos)],
                available_at_end);
            memcpy(
                data_bytes + i * step_size + available_at_end,
                &r->buf[ring_buffer_view_get_ring_pos(v, r->read_pos + available_at_end)],
                remaining);
        } else {
...
void ring_buffer_write_fully(
    ...
    while (processed < bytes) {
        ...
        if (v) {
            processed_here = ring_buffer_view_write(r, v, data, candidate_step, 1);
        } else {
            processed_here = ring_buffer_write(r, data, candidate_step, 1);
        }
...
void ring_buffer_read_fully(
    ...
    while (processed < bytes) {
        ...
        if (v) {
            processed_here = ring_buffer_view_read(r, v, data, candidate_step, 1);
        } else {
            processed_here = ring_buffer_read(r, data, candidate_step, 1);
        }
