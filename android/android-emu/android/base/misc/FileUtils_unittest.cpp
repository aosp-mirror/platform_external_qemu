...
// Tests that buffers containing null terminator bytes can be read
// successfully into a std::vector<uint8_t>.
TEST(FileUtils, fileToVector) {
    const int bytesToTest = 100;

    std::vector<uint8_t> testPattern;

    for (int i = 0; i < bytesToTest; i++) {
        testPattern.push_back(i % 3 == 0 ? 0x0 : 0xaf);
    }
    ...
    const auto testOutput = readFileIntoVector(tempfile_path(tf));

    EXPECT_TRUE(testOutput);

    for (int i = 0; i < bytesToTest; i++) {
        EXPECT_EQ(testPattern[i], testOutput->at(i));
    }
}
