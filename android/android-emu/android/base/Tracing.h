...
public:
    ThresholdTrace() = default;
    void begin(const char* name, uint64_t thresholdUs = 1000) {
        beginThresholdTrace(name, thresholdUs);
    }
    void end() {
        endThresholdTrace();
    }
...
public:
    ...
    void begin(const char* name, uint32_t printInterval = 1000) {
        beginIntervalTrace(name, printInterval);
    }
    void end() {
        endIntervalTrace();
    }
