...
struct FuncRange {
    FuncRange(std::string name, void* addr, size_t length)
        : mName(name), mAddr(addr), mLength(length) {}
    std::string mName;
    void* mAddr;
    size_t mLength;
    MallocStats mStats;
...
public:
    bool addToGroup(std::string group, std::string func) {
        ...
        std::unique_ptr<FuncRange> fp(new FuncRange(
                func, (void*)info.start_ip, info.end_ip - info.start_ip));
        mData[key] = std::move(fp);
        // for (auto& it : mData) {
    ...
    void newHook(const void* ptr, size_t size) {
        ...
        for (int i = 5; i < ret; i++) {
            ...
            for (auto& it : mData) {
                if (std::abs((intptr_t)results[i] -
                             (intptr_t)it.second->mAddr) < it.second->mLength) {
                    auto& funcRange = it.second;
                    funcRange->mStats.mAllocated += size;
                    funcRange->mStats.mLive += size;
                    funcRange->mStats.mPeak =
                            std::max(funcRange->mStats.mPeak.load(),
                                     funcRange->mStats.mLive.load());
                    found = true;
                    break;
                }
            }
...
private:
    std::unordered_map<std::string, std::unique_ptr<FuncRange>> mData;
