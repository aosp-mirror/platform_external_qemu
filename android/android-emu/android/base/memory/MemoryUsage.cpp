...
MallocStats MemoryUsage::aggregate(std::string label) {
    return mImpl->aggregate(label);
}

std::string MemoryUsage::printUsage() {
    return mImpl->printUsage();
}
/*std::std::vector<MallocStats> MemoryUsage::aggregateAll() {
        return mImpl->aggregateAll();
}*/
