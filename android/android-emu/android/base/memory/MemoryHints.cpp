...
public:
    ...
    bool offerRange(void* start, uint64_t length) {
        ...
        return ERROR_SUCCESS == OfferVirtualMemory((PVOID)start, (SIZE_T)length, VmOfferPriorityVeryLow);
    }
    ...
    bool reclaimRange(void* start, uint64_t length) {
        ...
        if (mOfferedRanges.find(range) != mOfferedRanges.end()) {
            ...
            return ERROR_SUCCESS == ReclaimVirtualMemory(range.first, range.second);
        }
        return true;
    }
