...
bool memoryHint(void* start, uint64_t length, MemoryHint hint) {
    ...
    case MemoryHint::DontNeed:
        ...
        // You could Virtual­Unlock memory that was already unlocked in
        // order to remove it from your working set. This was a trick,
        // because it took what used to be a programming error and gave it
        // additional meaning, but in a way that didn't break backward
        // compatibility because the contractual behavior of the memory did
        // not change: The contents of the memory remain valid and the
        // program is still free to access it at any time. The new behavior
        // is that unlocking unlocked memory also takes it out of the
        // process's working set, so that it becomes a prime candidate for
        // being paged out and used to satisfy another memory allocation.
