...
TEST(MemoryHints, Basic) {
    ...
    char memory[small];
    EXPECT_TRUE(memoryHint(memory, small, MemoryHint::DontNeed));
    EXPECT_TRUE(memoryHint(memory, small, MemoryHint::Normal));
    EXPECT_TRUE(memoryHint(memory, small, MemoryHint::Random));
    EXPECT_TRUE(memoryHint(memory, small, MemoryHint::Sequential));

    char* heap = new char[large];
    EXPECT_TRUE(memoryHint(heap, large, MemoryHint::DontNeed));
    EXPECT_TRUE(memoryHint(heap, large, MemoryHint::Normal));
    EXPECT_TRUE(memoryHint(heap, large, MemoryHint::Random));
    EXPECT_TRUE(memoryHint(heap, large, MemoryHint::Sequential));
    delete [] heap;
}
