...
Optional<System::DiskKind> nativeDiskKind(int st_dev)
{
    ...
    CFMutableDictionaryRef classesToMatch = IOBSDNameMatching(kIOMasterPortDefault, 0, devName);
    if (!classesToMatch) {
    ...
    while ((serviceEntry = IOIteratorNext(entryIterator)) != 0)
    {
        ...
        do
        {
            ...
            if (res)
            {
                ...
                if ([@kIOPropertyMediumTypeSolidStateKey isEqualToString:type]) {
                    CFRelease(res);
                    return System::DiskKind::Ssd;
                } else if ([@kIOPropertyMediumTypeRotationalKey isEqualToString:type]) {
                    CFRelease(res);
