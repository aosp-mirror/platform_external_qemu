...
static pthread_mutex_t g_recreate_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_cond_t g_recreate_cond = PTHREAD_COND_INITIALIZER;

static bool g_has_event_loop = false;

// maintain a list of all created windows
std::list<DisplayWindow *> g_windows;
...
static void close_all_windows()
{
    ...
    for (std::list<DisplayWindow*>::const_iterator it = g_windows.begin(); it != g_windows.end(); ++it) {
        DisplayWindow* pWin = *it;
        if (pWin->tex != NULL) {
            ...
            case SDL_USEREVENT:
                switch (e.user.code) {
                    ...
                    case USER_EVENT_CLOSE_WINDOW:
                        pthread_mutex_lock(&g_recreate_lock);
                        DisplayWindow* pWin = (DisplayWindow*)e.user.data1;
                        if (pWin->tex != NULL) {
                            SDL_DestroyTexture(pWin->tex);
                            pWin->tex = NULL;
                        ...
                        pthread_cond_signal(&g_recreate_cond);
                        pthread_mutex_unlock(&g_recreate_lock);
                        break;
                }
                break;
        }
...
#ifdef _WIN32
    DisplayWindow* win = pData->window;
    if (win) {
...
void SDLRenderer::delete_sdl_window(DisplayWindow* pWin) {
    ...
    me.user.data1 = pWin;
    SDL_PushEvent(&me);
    pthread_cond_wait(&g_recreate_cond, &g_recreate_lock);
    pthread_mutex_unlock(&g_recreate_lock);
}
