...
void SDLRenderer::draw_frame_internal(DisplayWindow* pWin, AVFrame *frame) 
{
	...
	video_rect.x = 0;
	video_rect.y = 0;
	video_rect.w = frame->width; // lastwidth; // frame size might change, e.g., chrome tab mirroring
	video_rect.h = frame->height; //lastheight;
	SDL_UpdateYUVTexture(pWin->tex, &video_rect,
	...
	while (!quit) {
		...
		case SDL_USEREVENT:
			...
			case USER_EVENT_CLOSE_WINDOW:
				...
				if (g_num_windows <= 0) {
					// if there is only one window, quit the loop. We need to keep this loop if there is a window still exists
					quit = true;
				}
...
DisplayWindow* SDLRenderer::recreateSDLWindow(int width, int height)
{
	...
	pWin->win = SDL_CreateWindow("ExternalDisplay", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, flags);
	if (NULL == pWin->win) {
		derror("cannot create window.\n");
		delete pWin;
		return NULL;
	}
	pWin->ren = SDL_CreateRenderer(pWin->win, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	if (NULL == pWin->ren) {
		derror("cannot create renderer.\n");
		delete pWin;
		return NULL;
	}
	pWin->tex = SDL_CreateTexture(pWin->ren, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING, width, height);
	if (NULL == pWin->tex) {
...
void SDLRenderer::resize_sdl_window(DisplayWindow* pWin, int width, int height)
{
	if (pWin && pWin->win) {
		//scale_to_fit_screen(width, height, &width, &height);

		SDL_SetWindowSize(pWin->win, width, height);
		SDL_SetWindowPosition(pWin->win, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED);
