...
FFmpegDecoder* FFmpegDecoder::create(int is_vp8, const char* extradata, int extradata_size, int width, int height)
{
...
int FFmpegDecoder::decode(DisplayWindow* window, const char* data, int data_size)
{
	if (data == NULL)
		return -1;
	...
	{		
		decode_video(window, data, data_size, 
	...
	int output_width, int output_height)
{
	...
	{		
		...
		len = avcodec_decode_video2(this->codec_context, this->frame, &decoded, &this->pkt);

		if (len < 0)
		{
			VERBOSE_PRINT(extdisplay, "data_size %d, avcodec_decode_video failed (%d)\n", data_size, len);
			ret = false;
			break;
		}	
		else if (!decoded)
		...
		else
		{
			...
			/*
			VERBOSE_PRINT(extdisplay, "linesize[0] %d linesize[1] %d linesize[2] %d linesize[3] %d "
				"pix_fmt %d width %d height %d",
				this->frame->linesize[0], this->frame->linesize[1],
				this->frame->linesize[2], this->frame->linesize[3],
			...
			*/
			...
			this->decoded_size = avpicture_get_size(dest_pix_fmt, output_width, output_height);
			this->decoded_data = xzalloc(this->decoded_size);
			avpicture_fill((AVPicture* )avFrameRGB, this->decoded_data, dest_pix_fmt, output_width, output_height);			
			av_picture_copy((AVPicture* )avFrameRGB, (AVPicture*)this->frame, dest_pix_fmt, output_width, output_height);			
			update_window_yuv(window, avFrameRGB, 
				avFrameRGB->linesize[0], avFrameRGB->data[0],
				avFrameRGB->linesize[1], avFrameRGB->data[1],
				avFrameRGB->linesize[2], avFrameRGB->data[2],
			...
			if ((count % 100) == 0)
			{
				VERBOSE_PRINT(extdisplay, "decode_video2(%dms): size=%d len=%d picture_copy\n", total_decode_time/count, this->pkt.size, len);
			}
			done = true;
			...
			if (!done && (this->codec_context->pix_fmt != dest_pix_fmt || output_width != this->codec_context->width || output_height != this->codec_context->height))
			{				
				...
				{					
					...
					avpicture_fill((AVPicture *)avFrameRGB, (const uint8_t *)this->decoded_data, dest_pix_fmt, output_width, output_height);					
					sws_scale(this->img_convert_ctx, this->frame->data, this->frame->linesize, 0, this->codec_context->height, avFrameRGB->data, avFrameRGB->linesize);								
					
#ifndef _USE_SDL_
					update_window(window, this->decoded_data, this->decoded_size, output_width, output_height);		
#endif
					...
					//VERBOSE_PRINT(extdisplay, "sws_scale(%dms), decode(%dms), total_decode_time(%dms)\n", end_time - start_time, decode_time, total_decode_time);
