...
int external_display_reply_packet(Mirroring *mirroring, uint8_t *buffer, size_t size);
...
int Mirroring::sendSinkStatus(bool attached)
{
	...
	if (attached) {
		...
		this->video_decoder = FFmpegDecoder::create(0, nullptr, 0, this->resolution_width, this->resolution_height);
	...
	this->state = (this->video_decoder) ? MIRRORING_DISPLAY_ATTACHED : MIRRORING_DISPLAY_DETACHED;
...
{	
	if (this->video_decoder != nullptr)
		this->video_decoder->decode(this->window, (const char *)h264_buf, h264_buf_size);
...
{	
	...
	VERBOSE_PRINT(extdisplay, "%s(): Message type: %d, size: %d, state=%d, header=%p",
		__FUNCTION__,
		message_type,  mhdr->size, this->state, mhdr);
...
{
	VERBOSE_PRINT(extdisplay, "%s(): Read (%d bytes) from bulk endpoint, packetbuf_pos=%d", __FUNCTION__, packet_len, this->packetbuf_pos);

	if (this->state == MIRRORING_DISPLAY_DETACHED || this->state == MIRRORING_DEVICE_DISCONNECTED) {
		VERBOSE_PRINT(extdisplay, "%s(): ignore message", __FUNCTION__);
		return 0;
	}

	// allocate a buf to store a whole packet, one packet may come with a few reads, and one read may contain a few packets
...
#ifdef _WIN32	
		this->packetbuf = (unsigned char *)_aligned_malloc(this->packetbuf_size, 32);
#elif __APPLE__
	...
	{
		derror("%s(): Packet size is too small. packet_len=%d, packetbuf_pos=%d, packetbuf_size=%d\n", __FUNCTION__, packet_len, this->packetbuf_pos, this->packetbuf_size);
	...
	while (pos < packet_len && remaining > 0)
	{
		...
		if (pos + size > packet_len)
		{
			...
			VERBOSE_PRINT(extdisplay, "%s(): Packet is incomplete, expecting %d bytes, but we only got %d bytes, pos=%d.\n", 
				__FUNCTION__,
				size, remaining, pos);
			break;
		}
