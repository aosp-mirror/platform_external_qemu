...
static void outputBlacklistResults(GpuInfoList* gpulist) {
    ...
    blacklistIni.setInt64("gpu_blacklisted", gpulist->blacklist_status);
    blacklistIni.setInt64("sync_blacklisted", gpulist->SyncBlacklist_status);
    blacklistIni.write();
}
...
static bool blacklistInfoUpdated = false;
...
static bool getCachedBlacklistStatus(const std::string& key) {
    android::base::IniFile blacklistIni(kBlacklistCacheFileName);
    if (!blacklistIni.read()) {
        return false;
    }
    bool res = blacklistIni.getInt64(key, 0);
    return res;
}
