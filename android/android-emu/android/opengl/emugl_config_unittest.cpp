...
TEST(EmuglConfig, initFromUISetting) {
    ...
    makeLibSubDir(myDir, "gles_angle11");
    makeLibSubFile(myDir, "gles_angle11/" LIB_NAME("EGL"));
    makeLibSubFile(myDir, "gles_angle11/" LIB_NAME("GLESv2"));
    ...
    for (int i = 1; i < 5; i++) {
        ...
        switch (i) {
        case 1:
            EXPECT_STREQ("angle11", config.backend);
            break;
        case 2:
