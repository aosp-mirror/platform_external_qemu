    ...
    virtual int onGuestRecv(AndroidPipeBuffer* buffers,
        ...
        while (buff != buffEnd) {
            if (mDataForReadingLeft == 0) {
                if (android::opengl::recvMode == android::opengl::RecvMode::Android) {
                ...
                } else if (android::opengl::recvMode == android::opengl::RecvMode::Fuchsia) { // Fuchsia, default (Not using switch because of break; overload)
                    ...
                    // if (nextUs - currUs > 1000) {
                        // fprintf(stderr, "%s: opengles pipe recv mode virtio: long wait. %f ms\n", __func__,
                                // (float)(nextUs - currUs) / 1000.0f);
                    // }
