...
- (MacCamera*)init
{
    ...
    output_queue = dispatch_queue_create("com.google.goldfish.camera.video", DISPATCH_QUEUE_SERIAL );
...
- (int)read_frame:(ClientFrameBuffer*)framebuffers:(int)fbs_num:(float)r_scale:(float)g_scale:(float)b_scale:(float)exp_comp
{
    ...
    if (current_frame != nil) {
        ...
        int frame_width, frame_height, bpr, bpp, frame_size;
        ...
        @synchronized (self)
        {
            ...
            bpr = CVPixelBufferGetBytesPerRow(current_frame);
            bpp = 4;
            frame_size = bpr * frame_height;
