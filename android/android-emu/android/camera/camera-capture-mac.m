...
// 
//     /* Create an input device and register it with the capture session. */
//     input_device = [[AVCaptureDeviceInput alloc] initWithDevice:capture_device];
//     success = [capture_session addInput:input_device error:&error];
//     if (!success) {
//         E("Unable to initialize input device:");
//         [input_device release];
//         input_device = nil;
//         [self free];
//         [self release];
...
// 
    if (true) {
        E("Unable to open camera device");
        [self free];
        [self release];
        return nil;
    }
...
- (void)captureOutput:(AVCaptureOutput*) captureOutput
                      didOutputVideoFrame:(CVImageBufferRef)videoFrame
                      withSampleBuffer:(void*) sampleBuffer
