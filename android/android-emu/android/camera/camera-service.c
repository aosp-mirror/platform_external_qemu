...
/* version number for camera snapshots for compatibility. */
#define CAMERA_FILE_VERSION    1
...
static void _camera_client_save(Stream* f, QemudClient* client, void* opaque) {
    ...
    stream_put_be32(f, CAMERA_FILE_VERSION);
...
static int _camera_client_load(Stream* f, QemudClient* client, void* opaque) {
    ...
    int version_number = stream_get_be32(f);
    if (version_number != CAMERA_FILE_VERSION) {
        D("%s: cannot load: snapshot requires camera file version %d.  Saved file is version %d\n",
          __FUNCTION__, CAMERA_FILE_VERSION, version_number);
        return -EIO;
    }
