...
public:
    ...
    bool supportsAdvancedVideoProcessor() const { return mSupportsAdvancedVideoProcessor; }
...
public:
    ...
    static int enumerateDevices(MFInitialize& mf, CameraInfo* cameraInfoList, int maxCount);
                ...
                if (mMF.getMFApi().supportsAdvancedVideoProcessor()) {
                    ...
                    if (width > static_cast<UINT32>(mFramebufferWidth)) {
                        ...
                        score += static_cast<UINT32>(125.0f * width /
                                                        mFramebufferWidth) * ratioMultiplier;
                    }
                } else {
...
HRESULT MediaFoundationCameraDevice::createSourceReader(
        const ComPtr<IMFMediaSource>& source) {
    ComPtr<IMFAttributes> attributes;
    HRESULT hr = mMF.getMFApi().mfCreateAttributes(&attributes, 1);
    if (SUCCEEDED(hr)) {
        if (mMF.getMFApi().supportsAdvancedVideoProcessor()) {
...
int camera_enumerate_devices(CameraInfo* cis, int max) {
    ...
    const int result = MediaFoundationCameraDevice::enumerateDevices(mf, cis, max);
    if (result < 0) {
