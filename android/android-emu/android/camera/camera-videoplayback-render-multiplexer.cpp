...
// TODO: Figure out if we actually need a notifier.
...
void VideoplaybackNotifier::initTimer() {
  mLoopTimer = loopTimer_new(mLooper,
                             _on_videoplayback_time_out,
                             this);
}
...
bool RenderMultiplexer::initialize(const GLESv2Dispatch* gles2,
    ...
    mVideoRenderer = std::unique_ptr<VideoRenderer>(new VideoRenderer());
      ...
      LOG(INFO) << "Request found";
        switch (maybe_next_request->function_case()) {
            ...
            case ::offworld::VideoInjectionRequest::kPlay:
              if (mPlayer == nullptr) {
                LOG(ERROR) << "No video loaded, cannot play video.";
                return -1;
              }
              mCurrentRenderer = mVideoRenderer.get();
              mPlayer->start();
            ...
            case ::offworld::VideoInjectionRequest::kStop:
              if (mPlayer == nullptr) {
                LOG(ERROR) << "No video loaded, so there is no video to stop.";
                return -1;
              }
              mCurrentRenderer = mVideoRenderer.get();
              mPlayer->stop();
            ...
            case ::offworld::VideoInjectionRequest::kPause:
              LOG(ERROR) << "Pause is unimplemented.";
              return -1;
...
void RenderMultiplexer::loadVideo(const char* video_data) {
  mPlayer = videoplayer::VideoPlayer::create(
      "/tmp/coke_barcode.mp4",
      mVideoRenderer->renderTarget(),
