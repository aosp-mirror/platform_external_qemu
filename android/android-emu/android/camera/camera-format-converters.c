...
int convert_frame_fast(const void* src_frame,
    ...
    if (has_resize) {
        ...
        uint8_t* source_y = *result_frame->staging_framebuffer;
        uint8_t* source_u = source_y + info.y_size;
        uint8_t* source_v = source_u + info.u_or_v_size;
        ...
        int result = I420Scale(
                source_y, info.y_stride, source_u, info.u_or_v_stride, source_v,
                info.u_or_v_stride, src_width, src_height, dest_y,
    ...
    uint8_t* y_staging = *result_frame->staging_framebuffer + staging_offset;
    uint8_t* u_staging = y_staging + info.y_size;
    uint8_t* v_staging = u_staging + info.u_or_v_size;
