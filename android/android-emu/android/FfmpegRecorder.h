...
//    ffmpeg_recorder_init_output_context(&recordingInfo);
//
...
// Basic recording flow is as follows:
//    FfmpegRecorder* recorder = FfmpegRecorder::create(fbWidth, fbHeight);
//
//    // Initialize the output context
//    recorder->initOutputContext(&recordingInfo);
//    // Add the video and audio contexts
//    recorder->addVideoTrack(...);
//    recorder->addAudioTrack(...); // audio track is optional
//    recorder->start(); // Sets the current time as the start time of recording
//
// And then from other threads that get the audio/video frames:
//    recorder->encodeVideoFrame(...);
//
//    recorder->encodeAudioFrame(...);
//
// Once we are finished recording:
//    recorder->stop();
//
