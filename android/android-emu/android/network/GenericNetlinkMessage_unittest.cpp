...
TEST(GenericNetlinkMessageTest, BasicOperationsTest) {
	GenericNetlinkMessage msg(GenericNetlinkMessage::NL_AUTO_PORT, 
								...
								family, hdrlen, flags, cmd, version);
	EXPECT_TRUE(msg.isValid());
	EXPECT_EQ(msg.netlinkHeader()->nlmsg_flags, flags);
	EXPECT_EQ(msg.netlinkHeader()->nlmsg_type, family);
	EXPECT_EQ(msg.netlinkHeader()->nlmsg_seq, GenericNetlinkMessage::NL_AUTO_SEQ);
	EXPECT_EQ(msg.netlinkHeader()->nlmsg_pid, GenericNetlinkMessage::NL_AUTO_PORT);
	EXPECT_EQ(msg.dataLen(), msg.netlinkHeader()->nlmsg_len);
	const genlmsghdr* gehdr = msg.genericNetlinkHeader();
	...
	EXPECT_TRUE(msg.putAttribute(HWSIM_ATTR_FRAME, frame, kSize));
	IOVector iov = msg.getAttribute(HWSIM_ATTR_FRAME);
	EXPECT_EQ(iov.size(), 1);
	EXPECT_EQ(std::memcmp(frame, iov[0].iov_base, kSize), 0);
}
