    ...
    switch( reason ) {
        ...
        case LWS_CALLBACK_SERVER_WRITEABLE:
            ...
            while( !self->connections[fd]->buffer.empty( ) )
            {
                ...
                unsigned int charsSent = lws_write( wsi, (unsigned char*)message.c_str( ), message.length( ), LWS_WRITE_TEXT );
...
WebSocketServer::WebSocketServer( int port, const string certPath, const string& keyPath )
{
    ...
    fprintf(stderr, "%s\n started on port: %d", __func__, _port);

    //::log( "Server started on port " + Util::toString( this->_port ) );
...
{
    fprintf(stderr, "%s\n", __func__);
    //::log( "Error: " + message + " on socketID '" + Util::toString( socketID ) + "'" );
...
void WebSocketServer::wait( uint64_t timeout )
{
    if( lws_service( this->_context, timeout ) < 0 )
      fprintf(stderr, "%s: Error polling for socket activity.\n", __func__);
}
