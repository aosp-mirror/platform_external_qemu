...
void AdbVsockPipe::Service::onHostConnection(android::base::ScopedSocket&& socket,
                                             AdbPortType portType) {
    {
        ...
        const auto i = std::find_if(mPipes.begin(), mPipes.end(),
                                    [portType](const std::unique_ptr<AdbVsockPipe>& p){
            return p->mFromSnapshot && p->mProxy && (p->mProxy->portType() == portType);
        });

        if (i != mPipes.end()) {
            AdbVsockPipe* pipe = &**i;
            pipe->mSocket = std::move(socket);
            pipe->mFromSnapshot = false;
            return;
        }
    }
...
void AdbVsockPipe::Service::saveImpl(base::Stream* stream) const {
    if (featurecontrol::isEnabled(featurecontrol::Feature::SnapshotAdb)) {
        std::unique_lock<std::mutex> guard(mPipesMtx);
...
AdbVsockPipe::~AdbVsockPipe() {
    ...
    if (mProxy && mProxy->canSave()) {
        CrossSessionSocket::recycleSocket(std::move(mSocket));
    }
}
...
void AdbVsockPipe::onHostSocketEvent(unsigned events) {
    processProxyEventBits(mProxy->onHostSocketEvent(mSocket.fd(),
...
bool AdbVsockPipe::canSave() const {
    return mProxy && mProxy->canSave()
           && mSocket.valid()
           && mVsockCallbacks.isValid();
}
...
void AdbVsockPipe::save(base::Stream* stream) const {
    ...
    CrossSessionSocket::registerForRecycle(mSocket.fd());
}
...
bool AdbVsockPipe::loadImpl(base::Stream* stream) {
    ...
    mSocket = CrossSessionSocket::reclaimSocket(stream->getBe32());
