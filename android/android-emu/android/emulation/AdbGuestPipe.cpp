...
bool AdbGuestPipe::Service::canLoad() const {
    bool ret = android::featurecontrol::isEnabled(
            android::featurecontrol::Feature::SnapshotAdb) || sJdwpPipes.size();
...
void AdbGuestPipe::Service::onHostConnection(ScopedSocket&& socket, AdbPortType portType) {
    D("%s", __func__);
...
void AdbGuestPipe::Jdwp::Buffer::readBytes(const AndroidPipeBuffer *buffers, int numBuffers,
        int *bufferIdx, size_t *bufferPst,
        size_t bytesToRead,
        bool skipData) {
    while (bytesToRead > 0 && *bufferIdx < numBuffers) {
        ...
        if (skipData) {
            ...
            memcpy(mBuffer + mBufferTail,
                buffers[*bufferIdx].data + *bufferPst,
                currentReadSize);
...
void AdbGuestPipe::Jdwp::onGuestRecvData(const AndroidPipeBuffer *buffers,
    ...
    while (currentBuffer < numBuffers) {
        if (mGuestRecvBuffer.mBytesToSkip) {
            mGuestRecvBuffer.readBytes(buffers, numBuffers, &currentBuffer, &currentPst,
            ...
            msg = (AdbMessageSniffer::amessage*)mGuestRecvBuffer.mBuffer;
            mGuestRecvBuffer.readBytes(buffers, numBuffers, &currentBuffer, &currentPst,
                kHeaderSize + msg->data_length - mGuestRecvBuffer.mBufferTail, false);
            if (kHeaderSize + msg->data_length == mGuestRecvBuffer.mBufferTail) {
                switch (mProxyStatus) {
                    case Uninitialized: {
                        int jdwpConnect = sscanf(mGuestRecvBuffer.mBuffer + kHeaderSize, "jdwp:%d", &mGuestPid);
                        if (jdwpConnect == 0) {
                            mIsJdwp = IsJdwp::No;
                            return;
                        }
                    ...
                    case ConnectOk: {
                        if (msg->data_length != 14 ||
                                0 != memcmp("JDWP-Handshake", mGuestRecvBuffer.mBuffer + kHeaderSize, 14)) {
                            mIsJdwp = IsJdwp::No;
                            return;
                        }
