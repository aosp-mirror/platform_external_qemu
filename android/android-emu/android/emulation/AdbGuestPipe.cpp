...
void AdbGuestPipe::onSave(android::base::Stream* stream) {
    ...
    if (mHostSocket) {
        ...
        printf("save socket %d needSaveBuffer %d\n", mHostSocket->fd(),
                needSaveBuffer);
...
void AdbGuestPipe::onGuestClose(PipeCloseReason reason) {
    ...
    //mHostSocket.reset();
    //android::base::AutoLock lock(mRecvLock);
    //DD("%s: [%p] acquired lock", __func__, this);
    mHostSocket.release();
...
void AdbGuestPipe::onGuestWantWakeOn(int flags) {
    ...
    if (flags & PIPE_WAKE_READ) {
        if (mHostSocket.get() && !shouldUseRecvBuffer()) {
            mHostSocket->wantRead();
        }
    }
...
void AdbGuestPipe::onHostSocketEvent(unsigned events) {
    ...
    if ((events & FdWatch::kEventRead) != 0) {
        if (shouldUseRecvBuffer() && mHostSocket) {
            recvOnce();
        } else {
        ...
        while (dataSize > 0) {
            ssize_t len;
            {
                ...
                if (mHostSocket) {
                    ...
                    //        data, dataSize);
                    //android::base::Thread::sleepMs(20);
                    android::base::AutoLock lock(mRecvLock);
                    DD("%s: [%p] recv buffer %d %d", __func__, this,
                            mRecvBufferBegin, mRecvBufferEnd);
...
void AdbGuestPipe::recvOnce() {
    ...
    ssize_t recvSize;
    {
        ...
        recvSize = android::base::socketRecv(mHostSocket->fd(),
                    localBuffer.data(),
    ...
    if (recvSize != 0) {
        if (-1 != recvSize) {
            mRecvBufferErrno = EWOULDBLOCK;
            memcpy(mRecvBuffer.data() + mRecvBufferEnd,
                    localBuffer.data(), recvSize);
            mRecvBufferEnd += recvSize;
            DD("%s: [%p] mRecvBuffer total %d", __func__, this,
                    (int)mRecvBufferEnd);
            wake = true;
        }
    }
    ...
    if (!mHostSocket) {
        printf("mHostSocket released!\n");
        abort();
    }
    if (wake) signalWake(PIPE_WAKE_READ);
    mHostSocket->wantRead();
    //DD("%s: [%p] done", __func__, this);
}
