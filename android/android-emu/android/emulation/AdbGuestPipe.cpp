...
void AdbMessage::readToBuffer(AndroidPipeBuffer* buffers, int numBuffers, int count) {
    mBufferP = mBuffer;
...
void AdbMessage::read(AndroidPipeBuffer* buffers, int numBuffers, int count) {
    if (count <= 0) return;
...
void AdbMessage::printMessage() {
    amessage & msg = mPacket.mesg;
    printf("command: %s ", getCommandName(msg.command));
...
int AdbGuestPipe::onGuestSend(const AndroidPipeBuffer* buffers,
    ...
    if (mState == State::ProxyingData) {
        ...
        // print_sent_packet(buffers, numBuffers, count);
        return count;
