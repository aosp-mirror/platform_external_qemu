...
void AdbGuestPipe::Service::recycleSocket(AdbScopedSocketWatcher&& socketWatch) {
    if (!socketWatch.valid()) return;
    ...
    if (ite != mRecycledSockets.end()) {
        ...
        // skip the custom dtor so that we don't close the socket
        socketWatch.swapSocketAndClose(&ite->second);
    }
}
...
AdbGuestPipe::AdbScopedSocketWatcher& AdbGuestPipe::AdbScopedSocketWatcher::operator=(
        AdbGuestPipe::AdbScopedSocketWatcher&& other) {
    mFdWatcher = std::move(other.mFdWatcher);
    mSocket = std::move(other.mSocket);
    return *this;
...
AdbGuestPipe::AdbScopedSocketWatcher::AdbScopedSocketWatcher(
    ...
    : mFdWatcher(fdWatcher)
    , mSocket(fdWatcher->fd()) { }

AdbGuestPipe::AdbScopedSocketWatcher::AdbScopedSocketWatcher(
        android::base::Looper::FdWatch* fdWatcher, ScopedSocket&& socket)
    : mFdWatcher(fdWatcher)
    , mSocket(std::move(socket)) { }

void AdbGuestPipe::AdbScopedSocketWatcher::swapSocketAndClose(ScopedSocket* socket) {
    mFdWatcher.reset();
    mSocket.swap(socket);
    mSocket.reset(-1);
}
...
android::base::Looper::FdWatch* AdbGuestPipe::AdbScopedSocketWatcher::fdWatcher() {
...
AdbGuestPipe::AdbGuestPipe(void* mHwPipe, Service* service,
        AdbHostAgent* hostAgent, android::base::Stream* stream)
            ...
            if (scopedSocket.valid()) {
                auto fdWatch = android::base::ThreadLooper::get()->createFdWatch(
                    ...
                    [](void* opaque, int fd, unsigned events) {
                        static_cast<AdbGuestPipe*>(opaque)->onHostSocketEvent(
                                events);
                    },
                    this);
                assert(fdWatch);
                ...
                mRecvBuffer.drainSocket(mHostSocket.fdWatcher()->fd(), false);
...
void AdbGuestPipe::onSave(android::base::Stream* stream) {
    if (shouldUseRecvBuffer()) {
        mRecvBuffer.drainSocket(mHostSocket.fdWatcher()->fd(), true);
    }
    ...
    if (mHostSocket.valid()) {
        stream->putBe32(mHostSocket.fdWatcher()->fd());
        ...
        while (dataSize > 0) {
            ssize_t len;
            {
                ...
                if (mRecvBuffer.hasData()) {
                    ...
                    len = android::base::socketRecv(mHostSocket.fdWatcher()->fd(),
                            data, dataSize);
                } else {
    ...
    while (numBuffers > 0) {
        ...
        while (dataSize > 0) {
            ssize_t len;
            {
                ...
                // Possible that the host socket has been reset.
                if (mHostSocket.valid()) {
                    len = android::base::socketSend(mHostSocket.fdWatcher()->fd(),
                                                    data, dataSize);
#ifdef _DEBUG
                    if (msgSize) {
                        msgSize -= dataSize;
...
void AdbGuestPipe::BufferedSocketReader::drainSocket(int socket,
    ...
    if (appendToRecvBuffer) {
        ...
        ssize_t recvSize = 0;
        do {
            recvSize = ::recv(socket,
                (char*)mRecvBuffer.data() + mRecvBufferEnd,
                mRecvBuffer.size() - mRecvBufferEnd,
                recvFlags);
        ...
        while (::recv(socket,
                ...
                localBuffer.size(),
                recvFlags) > 0) {
            DD("%s: [%p] clearing data from previous session", __func__,
                    this);
        }
    }
#ifdef _WIN32
...
size_t AdbGuestPipe::BufferedSocketReader::readData(void* data, size_t dataSize) {
