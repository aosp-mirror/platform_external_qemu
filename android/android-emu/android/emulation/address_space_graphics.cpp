...
void AddressSpaceGraphicsContext::echoAsync() {
    if ((ConsumerState)(*mConsumerState) == ConsumerState::NeedNotify) {
    ...
    while (1) {
        ...
        switch (threadCmd) {
            case ThreadCommand::Wakeup:
                *mConsumerState = ConsumerState::CanConsume;
                break;
            case ThreadCommand::Sleep:
                *mConsumerState = ConsumerState::NeedNotify;
                break;
            case ThreadCommand::Exit:
                return;
        }

        switch (*mConsumerState) {
            case ConsumerState::CanConsume:
                while (1) {
                    uint32_t availableToEcho =
                        ring_buffer_available_read(
    ...
    case Command::AllocOrGetOffset:
        if (!mBuffer) {
            ...
            initGraphicsContextRings(
                mBuffer,
                mToHost, mFromHost);
            mConsumerState = &(mToHost.ring->state);
        }
