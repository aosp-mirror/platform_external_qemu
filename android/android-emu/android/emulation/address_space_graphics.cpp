...
const int AddressSpaceGraphicsContext::kContextAllocationSize = 3 * kRingInfoSize + 3 * kRingSize + kWriteBufferSize;
...
const int AddressSpaceGraphicsContext::kWriteBufferOffset = kToHostTransferRingBufferOffset + kRingSize;
...
static void initGraphicsContextRings(
    ...
    initGraphicsContextConfig(
        AddressSpaceGraphicsContext::kWriteBufferSize,
        AddressSpaceGraphicsContext::kWriteStepSize,
        toHost.ring->config);
...
int AddressSpaceGraphicsContext::onUnavailableRead() {
    static const uint32_t kMaxUnavailableReads = 20;
...
void AddressSpaceGraphicsContext::getPtrAndSize(uint64_t data, char** ptrOut, size_t* sizeOut) {
    ...
    *ptrOut = mBuffer + kWriteBufferOffset + offsetRelativeToWriteBufferStart;
    *sizeOut = size;
}
...
void AddressSpaceGraphicsContext::perform(AddressSpaceDevicePingInfo *info) {
    ...
    case Command::AllocOrGetOffset:
        if (!mBuffer) {
            mWriteBufferSize = android_hw->hw_gltransport_asg_writeBufferSize;
            mWriteStepSize = android_hw->hw_gltransport_asg_writeStepSize;
            mAllocSize = 3 * kRingInfoSize + 3 * kRingSize + mWriteBufferSize;
