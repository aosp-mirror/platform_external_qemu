...
AddressSpaceGraphicsContext::consumeReadbackLoop(
    ...
    while (1) {
        ...
        if (!availableToEcho) {
            ...
            if (true &&
                unavailableCount == maxUnavailableUntilHangup) {
...
void AddressSpaceGraphicsContext::perform(AddressSpaceDevicePingInfo *info) {
    ...
    case Command::AllocOrGetOffset:
        ...
        ASGFX_LOG("AllocOrGetOffset: Offset: 0x%llx",
    ...
    case Command::NotifyAvailable:
        ASGFX_LOG("NotifyAvailable");
        if (!mThreadMessages.trySend(ThreadCommand::Wakeup)) {
            ASGFX_LOG("Warning: thread messages full, likely stalled in render thread");
        }
        result = 0;
        break;
    case Command::Echo:
