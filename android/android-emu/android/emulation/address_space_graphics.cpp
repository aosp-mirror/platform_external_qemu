...
public:
    Globals() :
        ...
        mPerContextBufferSize(
                android_hw->hw_gltransport_asg_writeBufferSize) { fprintf(stderr, "%s: bufsize: 0x%llx\n", __func__, (unsigned long long)mPerContextBufferSize); }
...
private:
    ...
    void fillBlockLocked(Block& block) {
        ...
        fprintf(stderr, "%s: shared region start: 0x%llx\n", __func__,
