...
void AddressSpaceGraphicsContext::perform(AddressSpaceDevicePingInfo *info) {
    ...
    case GraphicsCommand::AllocOrGetOffset:
        fprintf(stderr, "%s: AllocOrGetOffset\n", __func__);
        if (!mBuffer) {
            fprintf(stderr, "%s: AllocOrGetOffset init\n", __func__);
        ...
        break;
    case GraphicsCommand::GetSize:
        result = ADDRESS_SPACE_GRAPHICS_CONTEXT_ALLOCATION_SIZE;
        break;
    case GraphicsCommand::GuestInitializedRings: {
