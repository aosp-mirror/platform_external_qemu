...
public:
    Globals() :
        mPerContextRingStorageSize( 2 * ADDRESS_SPACE_GRAPHICS_RING_SIZE),
    ...
    void fillBlockLocked(Block& block) {
        ...
        int allocRes = get_address_space_device_hw_funcs()->
            allocSharedHostRegionLocked(
                ADDRESS_SPACE_GRAPHICS_PAGE_SIZE, &offsetIntoPhys);
...
AddressSpaceGraphicsContext::AddressSpaceGraphicsContext() :
    ...
    mHostContext.to_host =
        reinterpret_cast<struct ring_buffer*>(
            mRingAllocation.buffer);
    mHostContext.from_host =
        reinterpret_cast<struct ring_buffer*>(
            mRingAllocation.buffer +
            ADDRESS_SPACE_GRAPHICS_RING_SIZE);

    mHostContext.buffer = mBufferAllocation.buffer;
    mHostContext.host_state =
        reinterpret_cast<asg_host_state*>(
            &mHostContext.to_host->state);
    mHostContext.ring_config =
        reinterpret_cast<asg_ring_config*>(
            mHostContext.to_host->config);
    ...
    mHostContext.ring_config->known_transfer_size_mode = 0;
...
void AddressSpaceGraphicsContext::perform(AddressSpaceDevicePingInfo* info) {
    ...
    case ASG_SET_VERSION: {
        auto guestVersion = (uint32_t)info->metadata;
...
int AddressSpaceGraphicsContext::onUnavailableRead() {
    ...
    if (mUnavailableReadCount == kMaxUnavailableReads) {
        ...
        if (cmd == ConsumerCommand::Exit) {
            return -1;
        }

        mConsumerReturnMessages.trySend(cmd);
        return 1;
    }
    return 0;
}
