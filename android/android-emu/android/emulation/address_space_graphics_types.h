...
// This file defines common types for address space graphics.

#define ADDRESS_SPACE_GRAPHICS_PAGE_SIZE 4096
#define ADDRESS_SPACE_GRAPHICS_RING_SIZE ADDRESS_SPACE_GRAPHICS_PAGE_SIZE
...
struct asg_ring_storage { // directly shared with guest
    ...
    char from_host[ADDRESS_SPACE_GRAPHICS_PAGE_SIZE];
};
...
enum asg_host_state {
    ...
    // without notification. The format of elements in the ring_buffer is of
    // type 1 (8 bytes; 4 bytes offset, 4 bytes size, relative to write buffer)
    ASG_HOST_STATE_CAN_CONSUME = 1,

    // 2 (CanConsume2): Like CanConsume, but the format of elements in
    // the ring_buffer is of type 2 (16 bytes; 8 bytes guest physical address,
    // 8 bytes size).
    ASG_HOST_STATE_CAN_CONSUME2 = 2,

    // 3 (Error): Something weird happened and we need to exit.
...
//
// Each of |to_host| and |from_host| can contain elements of type 1 or 2:
// Type 1: 8 bytes: 4 bytes offset, 4 bytes size. Relative to write buffer.
...
struct asg_ring_config {
    ...
    // config[2]: the highest position in the auxiliary buffer
    // that the host has read so far
    uint32_t host_consumed_pos;

    // config[2]: 0 if in command / unknown transfer size mode, 1 if known transfer size
    uint32_t known_transfer_size_mode;

    // config[3]: the size of the transfer, used if transfer size is known.
    // Set before setting config[2] to 1.
