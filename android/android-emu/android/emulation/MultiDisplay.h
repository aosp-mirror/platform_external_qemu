...
struct MultiDisplayInfo {
    ...
    bool     enabled;
    MultiDisplayInfo() :
      id(0), pos_x(0), pos_y(0), width(0), height(0), dpi(0), flag(0), cb(0), enabled(false) {}
    MultiDisplayInfo(int32_t i, int32_t x, int32_t y, uint32_t w, uint32_t h,
                     uint32_t d, uint32_t f, bool e) :
      id(i), pos_x(x), pos_y(y), width(w), height(h), dpi(d), flag(f), cb(0), enabled(e) {}
...
public:
    ...
    bool isMultiDisplayEnabled() { return mMultiDisplay.size() > 1; }
...
private:
    ...
    std::vector<MultiDisplayInfo> parseConfig();
};
} // namespace android
