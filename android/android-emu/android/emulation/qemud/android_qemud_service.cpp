...
QemudService* qemud_service_new(const char* name,
    ...
    QemudService* s =
            static_cast<QemudService*>(android_alloc0(sizeof(*s)));  // NOLINT
...
void qemud_service_remove_client(QemudService* s, QemudClient* c) {
    ...
    for (;;) {
        node = *pnode;
        if (node == nullptr) {
            D("%s: could not find client for service '%s'", __FUNCTION__,
              s->name);
            return;
        }
        if (node == c) {
...
QemudClient* qemud_service_connect_client(QemudService* sv,
    ...
    if (client == nullptr) {
        D("%s: registration failed for '%s' service", __FUNCTION__, sv->name);
        return nullptr;
    }
    D("%s: registered client channel %d for '%s' service", __FUNCTION__,
      channel_id, sv->name);
    return client;
}

QemudService* qemud_service_find(QemudService* service_list,
                                 const char* service_name) {
    QemudService* sv = nullptr;
    for (sv = service_list; sv != nullptr; sv = sv->next) {
...
void qemud_service_save_name(Stream* f, QemudService* s) {
    ...
    stream_write(f, reinterpret_cast<const uint8_t*>(s->name), len);
}

char* qemud_service_load_name(Stream* f) {
    int ret;
    int name_len = stream_get_be32(f);
    auto* service_name = static_cast<char*>(android_alloc(name_len));
    if ((ret = stream_read(f, reinterpret_cast<uint8_t*>(service_name),
                           name_len) != name_len)) {
...
void qemud_service_broadcast(QemudService* sv, const uint8_t* msg, int msglen) {
    QemudClient* c;
