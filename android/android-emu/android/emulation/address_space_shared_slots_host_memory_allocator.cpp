...
struct MemBlock {
    ...
    MemBlock(): physBase(0), physBaseLoaded(0), bits(nullptr), bitsSize(0) {}

    MemBlock(uint32_t sz) {
        bits = android::aligned_buf_alloc(alignment, sz);
        bitsSize = sz;
        physBase = 77; // TODO: allocate address block
        physBaseLoaded = 0;
        // TODO: bind physBase to bits
    ...
    ~MemBlock() {
        ...
        // TODO: unbind physBase from bits

        if (physBase) {
            // TODO: free address block
        }
    ...
    static bool load(base::Stream* stream, MemBlock* block) {
        ...
        const uint64_t physBase = 42; // TODO: allocate address block
        // TODO: bind physBase to bits
...
bool AddressSpaceSharedSlotsHostMemoryAllocatorContext::globalStateLoad(base::Stream* stream) {
    AutoLock lock(g_blocksLock);

    for (uint32_t sz = stream->getBe32(); sz > 0; --sz) {
        MemBlock block;
        if (!MemBlock::load(stream, &block)) { return false; }
