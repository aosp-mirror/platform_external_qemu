...
TEST_F(AdbHubTest, JdwpReconnect) {
    ...
    apacket packetOpen = buildPacketHostToGuest(ADB_OPEN);
    packetOpen.data.resize(strlen(kOpenData) + 1);
    packetOpen.mesg.data_length = packetOpen.data.size();
    EXPECT_TRUE(sendPacket(mTesterSocket.get(), &packetOpen));
    mHub.onHostSocketEvent(mHubSocket.get(), FdWatch::kEventRead, [] { FAIL(); });
    expectGuestRecvPacket(packetOpen);
