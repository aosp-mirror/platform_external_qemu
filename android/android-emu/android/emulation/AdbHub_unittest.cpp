...
TEST_F(AdbHubTest, RecvPacketSeparateHeader) {
    ...
    EXPECT_TRUE(base::socketSendAll(mTesterSocket.get(), &packet.mesg, kHeaderSize));
    mHub.onHostSocketEvent(mHubSocket.get(), base::Looper::FdWatch::kEventRead,
        [] { FAIL(); });
    EXPECT_EQ(mPipeWakeFlag, 0);
    EXPECT_EQ(mHub.onGuestRecvData(&buffer, 1), PIPE_ERROR_AGAIN);

    EXPECT_TRUE(base::socketSendAll(mTesterSocket.get(), packet.data.data(), packet.mesg.data_length));
    mHub.onHostSocketEvent(mHubSocket.get(), base::Looper::FdWatch::kEventRead,
        [] { FAIL(); });
    EXPECT_EQ(mPipeWakeFlag, PIPE_WAKE_READ);
...
TEST_F(AdbHubTest, RecvPacketMultiplePacketMultipleBuffers) {
    ...
    mHub.onHostSocketEvent(mHubSocket.get(), base::Looper::FdWatch::kEventRead,
        [] { FAIL(); });
    EXPECT_EQ(mPipeWakeFlag, PIPE_WAKE_READ);
    EXPECT_EQ(mHub.onGuestRecvData(buffers, 4), packetSize(packet0) + packetSize(packet1));

    EXPECT_EQ(0, memcmp(buffers[0].data, &packet0.mesg, kHeaderSize));
    EXPECT_EQ(0, memcmp(buffers[1].data, packet0.data.data(), packet0.mesg.data_length));
    EXPECT_EQ(0, memcmp(buffers[2].data, &packet1.mesg, kHeaderSize));
    EXPECT_EQ(0, memcmp(buffers[3].data, packet1.data.data(), packet1.mesg.data_length));
