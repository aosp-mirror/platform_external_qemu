...
//
// Usage:
//       At vm starts up, register this enigne by calling the AudioOutputEngine::set() static method
...
public:
     ...
     //     format - audio format, one of the AudioFormat enums, AUDIO_FMT_S16 is the most common format
     //     freq - audio sampling rate, usually 44100 or 48000
     //     nchannels - number of channels, 2 for stero audio
     //     callback - audio output callback, write() call should be made inside this callback
     //     callback_opaque - arbitrary parameter for user to pass custom data reference
     // return:
     //     0 if successful
     //     < 0  if failed
     virtual int open(const char *name, AudioFormat format, int freq, int nchannels, audio_callback_fn callback, void *callback_opaque) = 0;

     // render the buffer, this function should be called from inside the audio audio_callback_fn function
     ...
     static void set(AudioOutputEngine* engine) {
         mInstance = engine;
     }
