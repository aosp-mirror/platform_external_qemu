...
static vpx_codec_ctx_t* getContext(void* ctx) {
    vpx_codec_ctx_t *context = (vpx_codec_ctx_t*)(ctx);
    return context;
}
...
void MediaVpxDecoder::destroyVpxContext(void* ptr) {
    vpx_codec_destroy(getContext(mCtx));
    delete getContext(mCtx);
    mCtx = nullptr;
}
...
void MediaVpxDecoder::decodeFrame(void* ptr) {
    ...
    vpx_codec_decode(getContext(mCtx), data, len, user_data, 0);
