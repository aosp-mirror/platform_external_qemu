...
public:
    ...
    void setHostAddr(uint32_t handle, size_t off, void* hva) {
        fprintf(stderr, "%s: set to 0x%llx %p\n", __func__, off, hva);
        auto& entry = mEntries[handle];
    ...
    void setHostAddrByPhysAddr(uint64_t physAddr, void* hva) {
        ...
        for (auto it : mEntries) {
            fprintf(stderr, "%s: handle %u\n", __func__, it.first);
            for (auto it2 : it.second.blocks) {
                if (it2.first == off) {
        fprintf(stderr, "%s: %p off 0x%llx 0x%llx\n", __func__, this, off, (unsigned long long)hva);
                    it2.second.hva = hva;
        fprintf(stderr, "%s: %p\n", __func__, it2.second.hva);
        mEntries[it.first].blocks[off].hva = hva;
                    return;
                }
            }
        }

        fprintf(stderr, "%s: off 0%llx not found\n", __func__, off);
    }
    ...
    void* getHostAddr(uint64_t physAddr) {
        fprintf(stderr, "%s: %p 0x%llx\n", __func__, this, (unsigned long long)physAddr);
        ...
        for (auto it : mEntries) {
            ...
            for (auto it2 : it.second.blocks) {
                if (it2.first == off) {
        fprintf(stderr, "%s: off 0x%llx 0x%llx\n", __func__, off, (unsigned long long)it2.second.hva);
