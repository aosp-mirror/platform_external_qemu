...
TEST_F(HostAddressSpaceTest, SharedRegionClaiming) {
    ...
    int claimRes = mDevice->claimShared(handle, offset);
    EXPECT_EQ(0, claimRes);
    claimRes = mDevice->claimShared(handle2, offset);
    EXPECT_EQ(0, claimRes);
...
TEST_F(HostAddressSpaceTest, SharedRegionClaimingNegative) {
    ...
    int claimRes = mDevice->claimShared(handle, offset + 1);
    EXPECT_EQ(-EINVAL, claimRes);
    claimRes = mDevice->claimShared(handle, offset);
    EXPECT_EQ(0, claimRes);
    claimRes = mDevice->claimShared(handle, offset);
    EXPECT_EQ(-EINVAL, claimRes);
