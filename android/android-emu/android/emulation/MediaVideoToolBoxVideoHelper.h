...
public:
    MediaVideoToolBoxVideoHelper(int w, int h, OutputTreatmentMode outMode,
...
private:
    struct InputFrame {
        InputFrame(H264NaluParser::H264NaluType in_type, const uint8_t* in_data, int in_size):
...
private:
    ...
    // TODO: video toolbox does not provide such
    // output parameters, need to get it from
    // somewhere else, such as ffmpeg
    unsigned int mColorPrimaries = 2;
    unsigned int mColorRange = 0;
    unsigned int mColorTransfer = 2;
    unsigned int mColorSpace = 2;

    MediaSnapshotState::ColorAspects mColorAspects;
