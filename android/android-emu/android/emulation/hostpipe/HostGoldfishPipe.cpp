...
public:
    HostGoldfishPipeDevice() {
        mPipeDevice = new TestAndroidPipeDevice;
    ...
    void* connect(const char* name) {
        ...
        fprintf(stderr, "%s: opened a pipe %p\n", __func__, pipe);
        if (mGuest->connect(pipe, name) == 0) { // success
            void* hwpipe = mGuest->getHwSidePipe(pipe);
            fprintf(stderr, "%s: sucesful connect, reutrn %p\n", __func__, hwpipe);
            return hwpipe;
        } else {
...
private:
    TestAndroidPipeDevice* mPipeDevice = nullptr;
