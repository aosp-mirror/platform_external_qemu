...
MultiDisplay::MultiDisplay(const QAndroidEmulatorWindowAgent* const agent) {
    mWindowAgent = agent;
    SkinRect monitor;
...
int MultiDisplay::setMultiDisplay(uint32_t id,
    ...
    if (mWindowAgent->getRotation() != SKIN_ROTATION_0) {
    ...
    // Service inside guest already started in QemuMiscPipe when guesst
    // bootCompleted. But this service may be killed, e.g., Android low
    // memory. Send broadcast again to gurantee servce running.
    // P.S. Service has check to avoid duplication.
...
bool MultiDisplay::getMultiDisplay(uint32_t id,
                                  ...
                                  bool* enabled) {
    AutoLock lock(mLock);
    LOG(VERBOSE) << "getMultiDisplay " << id;
...
int MultiDisplay::destroyDisplay(uint32_t displayId) {
    ...
    bool needUIUpdate;
    {
     ...
     //TODO: dpi value? needed, directly add event to UI event?
    mWindowAgent->setUIMultiDisplay(displayId, 0, 0, 0, 0, false, 0);
...
int MultiDisplay::setDisplayPose(uint32_t displayId,
    ...
    uint32_t height;
    {
        ...
        if (mMultiDisplay[displayId].cb != 0) {
            ...
            getCombinedDisplaySizeLocked(nullptr, &height);
            UIUpdate = true;
        }
    }
    if (UIUpdate) {
...
int MultiDisplay::setDisplayColorBuffer(uint32_t displayId, uint32_t colorBuffer) {
    ...
    bool noSkin = false;
    {
        ...
        // the disable skin when first display set, index 0 is the default one.
        if (mMultiDisplay.size() == 2 && mMultiDisplay[displayId].cb == 0) {
            noSkin = true;
        }
...
void MultiDisplay::setDisplayPoseInSkinUI(int totalHeight) {
    ...
    int32_t pos_x, pos_y;
    uint32_t width, height, dpi, cb;
    for (int i = 0; i < s_maxNumMultiDisplay; i++) {
        if (getMultiDisplay(i, &pos_x, &pos_y, &width, &height, &dpi, nullptr, &cb, nullptr)) {
            continue;
        }
        if (i == 0 || cb != 0) {
            mWindowAgent->setUIMultiDisplay(
                i, pos_x,
                totalHeight - height - pos_y,
                width, height,
                true, dpi);
        }
    }
...
std::vector<MultiDisplayInfo> MultiDisplay::parseConfig() {
...
//    if (agent->multiDisplay->tryLockMultiDisplayOnLoad()) {
        std::vector<MultiDisplayInfo> info = parseConfig();
        if (info.size()) {
            LOG(VERBOSE) << "config multidisplay with command-line";
            for (const auto& i : info) {
                setMultiDisplay(i.id,
                                -1,
                                -1,
                                i.width,
                                i.height,
                                ...
                                i.flag,
                                true);
                mWindowAgent->updateUIMultiDisplayPage(i.id);
            }
        } else {
...
void android_init_multi_display(const QAndroidEmulatorWindowAgent* const agent) {
    android::sMultiDisplay = new android::MultiDisplay(agent);
