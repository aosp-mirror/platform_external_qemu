...
int MultiDisplay::destroyDisplay(uint32_t displayId) {
    ...
    setUIMultiDisplay(displayId, 0, 0, 0, 0, false, 0);
...
void MultiDisplay::setDisplayPoseInSkinUI(int totalHeight) {
    ...
    int32_t pos_x, pos_y, start_id = -1;
    uint32_t id, width, height, dpi, cb;
    while(getNextMultiDisplay(start_id, &id, &pos_x, &pos_y, &width, &height, &dpi, nullptr, &cb)) {
        if (id == 0 || cb != 0) {
            setUIMultiDisplay(id,
                              pos_x, 
...
void MultiDisplay::setUIMultiDisplay(uint32_t id,
                                     int32_t x,
                                     int32_t y,
                                     uint32_t w,
                                     uint32_t h,
