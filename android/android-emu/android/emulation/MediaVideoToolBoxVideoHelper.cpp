...
MediaVideoToolBoxVideoHelper::MediaVideoToolBoxVideoHelper(
        int w, int h, OutputTreatmentMode oMode, FrameStorageMode fMode)
    : mOutputWidth(w), mOutputHeight(h), mUseGpuTexture(fMode == FrameStorageMode::USE_GPU_TEXTURE) {
...
const uint8_t* MediaVideoToolBoxVideoHelper::parseOneFrame(const uint8_t* frame, size_t szBytes) {
    ...
    if (naluType == H264NaluType::SPS 
            || naluType == H264NaluType::PPS) { 
        mInputFrames.push_back(InputFrame(naluType, data, dataSize));
    } else {
        mInputFrames.push_back(InputFrame(naluType, currentNalu, currentNaluSize));
    }
...
void MediaVideoToolBoxVideoHelper::copyFrame() {
    ...
    mSavedDecodedFrames.push_back(MediaSnapshotState::FrameInfo{
                ...
                (int)mOutputHeight, (uint64_t)(mOutputPts), ColorAspects{mColorAspects}});
