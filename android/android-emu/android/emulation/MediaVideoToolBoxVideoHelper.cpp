...
void MediaVideoToolBoxVideoHelper::extractFrameInfo() {
    ...
    if (success) {
        ...
        VTB_DPRINT("gfot ffmpeg w %d h %d", mOutputWidth, mOutputHeight);
...
const uint8_t* MediaVideoToolBoxVideoHelper::parseOneFrame(const uint8_t* frame,
    ...
    // usually guest sends one frame a time, but sometime, SEI prefixes IDR frame
    // as one frame from guest, we need to separate SEI out from IDR: video tool box
    // cannot handle SEI+IDR combo-frame; for other cases, there is no need to check.
    ...
    if(naluType == H264NaluType::Undefined) return nullptr;
...
void MediaVideoToolBoxVideoHelper::recreateDecompressionSession() {
    ...
    if (mDecoderSession != nullptr) {
        // TODO: Once we implement async frame readback, we'll need to flush all
        // of the frames here and store them somewhere for the guest to read
        // later.
        VTDecompressionSessionInvalidate(mDecoderSession);
        CFRelease(mDecoderSession);
        mDecoderSession = nullptr;
