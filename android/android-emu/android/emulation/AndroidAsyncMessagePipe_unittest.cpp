...
public:
    ...
    void wait() {
        ...
        if (mSignaledCount > 0 ||
            mCv.wait_for(lock, 5s, [this] { return mSignaledCount > 0; })) {
...
protected:
    ...
    void pumpLooperUntilEvent(base::Looper::Duration timeoutMs) {
        const base::Looper::Duration deadline = mLooper->nowMs() + timeoutMs;
        while (!mLooper->runOneIterationWithDeadlineMs(deadline) &&
