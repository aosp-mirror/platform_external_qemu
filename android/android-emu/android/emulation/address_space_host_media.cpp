...
static uint64_t getAddrSlot(uint64_t metadata) {
    uint64_t ret = metadata >> 8; //get rid of opcode
    ret = ret & (0xFFFF); //take the slot
    return ret;
}
...
void AddressSpaceHostMediaContext::handleMediaRequest(AddressSpaceDevicePingInfo *info) {
    ...
    fprintf(stderr, "Got media request (type=%u, op=%u, slot=%lld)", static_cast<uint8_t>(codecType),
                     static_cast<uint8_t>(op), (long long)(getAddrSlot(info->metadata)));
    ...
    switch (codecType) {
        ...
        case MediaCodecType::H264Codec:
            ...
            mH264Decoder->handlePing(codecType,
                                     op,
                                     (uint8_t*)(mControlOps->get_host_ptr(info->phys_addr)) + offSetAddr);
            break;
        default:
