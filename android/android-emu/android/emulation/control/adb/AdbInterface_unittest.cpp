...
class AdbTestSystem : public TestSystem {
    public:
    AdbTestSystem(bool success=true) : TestSystem("/progdir", System::kProgramBitness, "/homedir",
                      "/appdir"), mSuccess(success) {}
...
TEST_F(AdbInterfaceTest, findAdbFromAndroidSdkRootIgnoreBadExec) {
    // We should not find an ADB executable that does not provide a proper version string.
    // or fails to execute.
