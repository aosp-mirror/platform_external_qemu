...
void AdbInterfaceImpl::selectAdbPath() {
    ...
    // Sort available adb installations by protocol version. Unknown versions are least preferred.
    // Note that we sort using >=, so biggest numbers first.
    std::sort(mAvailableAdbInstalls.begin(), mAvailableAdbInstalls.end(), [](const AdbEntry& fst, const AdbEntry& snd) {
        return fst.second && (!snd.second || fst.second >= snd.second);
    });
