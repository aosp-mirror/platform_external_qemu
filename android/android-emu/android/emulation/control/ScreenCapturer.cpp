...
static std::vector<uint8_t>& convert_dma_byte(std::vector<uint8_t>& source, std::vector<uint8_t>& dest) {
...
static std::vector<uint8_t>& convert_dma_hexlet(std::vector<uint8_t>& source, std::vector<uint8_t>& dest) {
    // This only works if we have "native" support for uint128_t (which clang has)
    static_assert(sizeof(__uint128_t) == 16);

    // Dest should be large enough to hold what we need.
    assert(dest.size() >= source.size() / 4 * 3 + 16);
    ...
    while (src + 16 < src_end) {
        ...
        // Note that endianness is very important! the most significant bits end up
        // in the address furthest away resulting in 4 zero bytes! which will be filled
        // up in the next round (or will get chopped up in the end).
        *to_write = newValue;

        dst += 12;  // We wrote 12 bytes. (well 16 but the last 4 bytes we don't care for)
        src += 16;  // We read 16 bytes.
    }

    // Move the last 16 bytes if needed, this happens if we are not aligned to a 128 bit boundary.
    int j = 0;
    while (src < src_end) {
