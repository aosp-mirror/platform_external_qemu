...
public:
    ...
    Status setGps(ServerContext* context,
        ...
        location->gpsSendLoc(request->latitude(), request->longitude(),
                             request->elevation(), request->satellites(), &tVal);
    ...
    Status getGps(ServerContext* context,
        ...
        // TODO(jansene):Underlying agent does not implement this!
        reply->set_passiveupdate(location->gpsGetPassiveUpdate());
        location->gpsGetLoc(&lat, &lon, &elevation, &count);
        ...
        LOG(VERBOSE) << reply->DebugString();
		fprintf(stderr, "%s:%d, %lf %lf %lf\n", __func__, __LINE__, lat, lon, elevation);
        return Status::OK;
    }
    ...
    Status sendKey(ServerContext* context,
                     ...
                     ::google::protobuf::Empty* reply) override {
        // Let's send a up and down..
        mAgents->user_event->sendKey(request->key(), true);
        mAgents->user_event->sendKey(request->key(), false);
        return Status::OK;
    }
