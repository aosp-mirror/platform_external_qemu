...
class AdbCommandImpl : public AdbCommand {
...
private:
    AdbCommandImpl(android::base::Looper* looper,
...
class AdbCommandConnectionImpl : public AdbCommand {
public:
    AdbCommandConnectionImpl(const std::vector<std::string>& args,
    ...
    ~AdbCommandConnectionImpl() { LOG(INFO) << "Dead"; }
    // Returns true if the command is currently in the process of execution.
    ...
    void start(int timeout_ms = 1000) override {
        ...
        mFunctor.reset(
                new base::FunctorThread([this, shared]() { execute(); }));
        mFunctor->start();
    }
...
private:
    void execute() {
        ...
        std::vector<char> stderr;
        int exitcode;
        while (shell.good() && System::get()->getUnixTimeUs() < mTimeout) {
            shell.read(stdout, stderr, exitcode);
            if (mWantOutput) {
                output->write(stdout.data(), stdout.size());
                output->write(stderr.data(), stderr.size());
            }
        }
    ...
    std::string mCmd;
    ResultCallback mResultCallback;
    std::unique_ptr<base::FunctorThread> mFunctor;
    AdbCommandPtr mSelf;
};
...
AdbCommandPtr AdbInterfaceImpl::runAdbCommand(
    ...
    if (android_cmdLineOptions->legacy_adb) {
        command = std::shared_ptr<AdbCommandImpl>(new AdbCommandImpl(
                mLooper, adbPath(), mSerialString, args, want_output,
                timeout_ms, std::move(result_callback)));

    } else {
        command = std::shared_ptr<AdbCommandConnectionImpl>(
                new AdbCommandConnectionImpl(args, std::move(result_callback),
                                             want_output));
    }
    command->start();
    return command;
...
AdbCommandImpl::AdbCommandImpl(Looper* looper,
                               const std::string& adb_path,
                               const std::string& serial_string,
                               const std::vector<std::string>& command,
                               bool want_output,
...
void AdbCommandImpl::start(int checkTimeoutMs) {
    if (!mTask && !mFinished) {
        auto shared =
                std::static_pointer_cast<AdbCommandImpl>(shared_from_this());
...
void AdbCommandImpl::taskDoneFunction(const OptionalAdbCommandResult& result) {
    if (!mCancelled) {
        mResultCallback(result);
    }
    mFinished = true;
...
void AdbCommandImpl::taskFunction(OptionalAdbCommandResult* result) {
