...
static const int kAdbQueryTimeoutMs = 200;
...
static std::string readResponse(int fd) {
    ...
    waiter->update(fd, SocketWaiter::kEventRead);
    char buffer[512];
    if (waiter->wait(kAdbQueryTimeoutMs)) {
        socket_recv(fd, (void*) buffer, base::arraySize(buffer));
        std::string msg(buffer);
        LOG(VERBOSE) << "readResponse [" << msg << "]";
        return msg;
    }
...
static int getAdbServerVersion() {
    ...
    // version.
    if (readResponse(fd.get()).compare("OKAY") == 0) {
        std::string version_result = readResponse(fd.get());
        // Protocol format is: 4 bytes indicating size of message in bytes.
        // Actual message.. We are expecting for bytes with the protocol
        // version.
...
void AdbInterfaceImpl::configureAdbPaths() {
    ...
    int protocol = getAdbServerVersion();
    ...
    if (adb_exe && System::get()->pathCanExec(adb_exe.get())) {
        options.push_back(std::make_pair("", std::string(adb_exe.get())));
    }
    ...
    if (protocol) {
        for (auto option : options) {
            auto adb = option.second;
            if (getAdbVersion(adb) == protocol) {
                // Matching protocol versions! Nice, we definitely want to use this
                // one as we will not be restarting adb.
                LOG(INFO) << "Found matching protocol: " << protocol <<  " adb:" << adb;
                mAutoAdbPath = adb;
