...
public:
    ...
    bool read(std::vector<char>& stdout,
              std::vector<char> stderr,
              int& exitCode);

    // Write bytes to stdin
    bool write(std::string stdin);

    // Write bytes to stdin
    bool write(std::vector<char> stdin);

    // True when using shell v1.
    bool isV1() { return !mShellV2; }

private:
    bool readV1(std::vector<char>& stdout,
                std::vector<char> stderr,
                int& exitCode);
    bool readV2(std::vector<char>& stdout,
                std::vector<char>& stderr,
                int& exitCode);
