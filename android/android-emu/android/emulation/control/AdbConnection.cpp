...
public:
    ...
    AdbState state() const override {
        if (mSocket->connected()) {
            // Due to async nature of socket updates we check for socket state
            // here. (It could be that the socket opened before we registered a
            // callback)
            if (mState == AdbState::disconnected) {
                return AdbState::socket;
            }
    ...
    bool connectToEmulator(int timeoutMs) {
        ...
        auto waitUntil = System::get()->getHighResTimeUs() + (timeoutMs * 1000);
        ...
        while (state() != AdbState::connected &&
               System::get()->getHighResTimeUs() < waitUntil) {
...
bool AdbStreambuf::waitForUnlock(uint64_t timeoutms) {
    base::AutoLock lock(mWriterLock);
    auto waitUntil = base::System::get()->getHighResTimeUs() + timeoutms * 1000;
    while (!mReady && System::get()->getHighResTimeUs() < waitUntil) {
...
bool AdbStreambuf::readyForWrite() {
    auto waitUntilUs = System::get()->getHighResTimeUs() + mWriteTimeout * 1000;
    while (!mReady && isOpen() &&
           System::get()->getHighResTimeUs() < waitUntilUs) {
