...
bool AdbShellStream::read(std::vector<char>& stdout,
                          std::vector<char> stderr,
                          int& exitCode) {
    if (mShellV2) {
        return readV2(stdout, stderr, exitCode);
    }
    return readV1(stdout, stderr, exitCode);
}

bool AdbShellStream::write(std::string stdin) {
    size_t size = stdin.size();
    bool ok = true;
    if (mShellV2) {
    ...
    mAdbStream->write(stdin.data(), size);
    return mAdbStream->gcount() == size && mAdbStream->good();
}

bool AdbShellStream::write(std::vector<char> stdin) {
    size_t size = stdin.size();
    bool ok = true;
    if (mShellV2) {
        ShellHeader hdr{.id = ShellHeader::kIdStdin,
...
bool AdbShellStream::readV1(std::vector<char>& stdout,
                            std::vector<char> stderr,
                            int& exitCode) {
    if (stdout.capacity() < 32)
        stdout.reserve(128);

    auto strmbuf = mAdbStream->rdbuf();
    // Max chunk size we are willing to collect before sending.
    int chunksize = stdout.capacity();
    char* end = stdout.data();
...
bool AdbShellStream::readV2(std::vector<char>& stdout,
                            std::vector<char>& stderr,
                            int& exitCode) {
    ShellHeader hdr;
    stdout.clear();
    ...
    if (mAdbStream->read((char*)&hdr, sizeof(hdr))) {
        ...
        switch (hdr.id) {
            case ShellHeader::kIdStderr:
                stderr.insert(stderr.end(), std::begin(msg), std::end(msg));
                break;
            case ShellHeader::kIdStdout:
                stdout.insert(stdout.end(), std::begin(msg), std::end(msg));
                break;
