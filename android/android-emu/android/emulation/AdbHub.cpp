...
int AdbHub::onGuestSendData(const AndroidPipeBuffer* buffers,
    ...
    while (currentBuffer < numBuffers && bytesToParse > 0) {
        if (mGuestSendBuffer.mBytesToSkip) {
            ...
            if (mGuestSendBuffer.mBufferTail == kHeaderSize) {
                ...
                //if (mIgnoreHostIds.count(mesg->arg1)) {
                if (false) {
    ...
    if (mNeedTranslate) {
        ...
        if (!mSocket || !mSocket->valid()) {
            ...
            //} else {
            if (translatedBuffer.size() == 0) {
                D("Ignore all guest send buffers");
            } else {
                mSendToHostQueue.push(std::move(translatedBuffer));
...
int AdbHub::onGuestRecvData(AndroidPipeBuffer* buffers,
    ...
    int currentBuffer = 0;
    size_t currentPst = 0;
    //int lastPacketBuffer = 0;
    //size_t lastPacketPst = 0;
    //if (mGuestRecvBuffer.mBytesToSkip || mGuestRecvBuffer.mBufferTail) {
    //    lastPacketBuffer = -1;
    //}
    while (currentBuffer < numBuffers && bytesToParse > 0) {
        ...
        if (mGuestRecvBuffer.mBytesToSkip) {
            ...
            if (mGuestRecvBuffer.mBytesToSkip == 0) {
                //lastPacketBuffer = currentBuffer;
                //lastPacketPst = currentPst;
            }
            ...
            if (kHeaderSize + mesg->data_length ==
                ...
                if (mesg->command == ADB_OPEN) {
                    ...
                    if (shouldReuseConnection(packet)) {
                        ...
                        //CHECK(lastPacketBuffer >= 0);
                        //currentBuffer = lastPacketBuffer;
                        //currentPst = lastPacketPst;
...
AdbProxy* AdbHub::onNewConnection(const apacket &packet) {
    ...
    if (mJdwpProxies.count(jdwpId) == 0) {
        ...
        mProxies.emplace(packet.mesg.arg0, proxy);
        return proxy;
