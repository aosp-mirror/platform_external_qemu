...
bool InstallPipe::hasDataForGuest() const {
    return mFp != NULL;
}

void InstallPipe::openFile() {
...
int InstallPipe::copyData(unsigned char* buf, int buf_size) {
    int copied = 0;
    if (buf_size <= 0) {
        return copied;
    }
    ...
    copied += fread(buf, 1, buf_size, mFp);
    if (copied < buf_size) {
        if (feof(mFp)) {
            fclose(mFp);
            mFp = NULL;
...
int InstallPipe::onGuestSend(const AndroidPipeBuffer* buffers,
    ...
    while (numBuffers > 0) {
        unsigned char* pch = static_cast<unsigned char*>(buffers->data);
        int pos = static_cast<int>(buffers->size);
        replyComplete = (pch[pos -1] == '\0');
        //mResult += std::string(static_cast<unsigned char*>(buffers->data), static_cast<int>(buffers->size));
