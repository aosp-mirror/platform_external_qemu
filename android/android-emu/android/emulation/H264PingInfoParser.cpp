...
#define WHT   "\x1B[37m"
#define RESET "\x1B[0m"
#define H264_PRINT(color,fmt,...) fprintf(stderr, color "H264PingInfoParser: %s:%d " fmt "\n" RESET, __func__, __LINE__, ##__VA_ARGS__);
#else
...
H264PingInfoParser::H264PingInfoParser(uint32_t version): mVersion(version) {
}

void H264PingInfoParser::parseInitContextParams(void* ptr, InitContextParam& param) {
...
uint64_t H264PingInfoParser::getHostDecoderId(void* ptr) {
    if (nullptr == ptr)
        return 0;
...
void H264PingInfoParser::parseDecodeFrameParams(void* ptr, DecodeFrameParam& param) {
            param.hostDecoderId = getHostDecoderId(ptr);
...
uint32_t H264PingInfoParser::getHostColorBufferId(void* ptr) {
...
void H264PingInfoParser::parseGetImageParams(void* ptr, GetImageParam& param) {
    param.hostDecoderId = getHostDecoderId(ptr);
    if (mVersion == 200) {
        param.hostColorBufferId = getHostColorBufferId(ptr);
    }
    uint8_t* retptr = (uint8_t*)getReturnAddress(ptr);
