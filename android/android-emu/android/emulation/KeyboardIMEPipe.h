...
class KeyboardIMEPipe final : public AndroidPipe {
public:
    using GuestKeyboardIMECallback = std::function<void(const uint8_t*, size_t)>;
    using Ptr = std::shared_ptr<KeyboardIMEPipe>;
    ...
    public:
        ...
        static KeyboardIMEPipe::Ptr getPipe();
        static void closePipe();
    };

    KeyboardIMEPipe(void* hwPipe, Service* svc);
    ...
    static void setGuestKeyboardIMECallback(GuestKeyboardIMECallback cb);
    void setText(const uint8_t* buf, size_t len);
...
void registerKeyboardIMEService();
