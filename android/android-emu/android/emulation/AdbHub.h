...
private:
    ...
    bool tryReuseConnection(const apacket &packet);
    //int pushToSenderQueue(std::vector<uint8_t>&& buffer);
    void pushToSenderQueue(apacket&& packet);
    ...
    base::ConditionVariable mSendToHostQueueEmptyCV;
    std::queue<apacket> mSendToHostQueue;
    apacket mCurrentSendPacket;
    size_t mCurrentSendPacketPst = 0;
    bool mSendToHostThreadShouldKill = false;
    std::unique_ptr<base::Thread> mRecvFromHostThread;
    base::Lock mRecvFromHostLock;
    base::ConditionVariable mRecvFromHostCV;
    std::queue<apacket> mRecvFromHostQueue;
    std::atomic<bool> mWantRecv = {false};
    apacket mCurrentRecvPacket;
    size_t mCurrentRecvPacketPst = 0;
    std::atomic<bool> mRecvFromHostThreadShouldKill = {false};
