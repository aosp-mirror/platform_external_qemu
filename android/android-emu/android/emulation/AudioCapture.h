...
// A subclass must override that onSample() method which receives the audio byte stream from the vm engine.
//
// Usage:
//    1) Create a new AudioCapturer instance with the specified audio format, which implements onSample() method
//
//    2) Call QemuAudioCaptureEngine::start() with the instance created in (1) to start capture.
//       After this call, audio byte stream will arrive in AudioCapturer::onSample() method
//
//    3) Call QemuAudioCaptureEngine::stop() with the instance created in (1) to stop capture
//
