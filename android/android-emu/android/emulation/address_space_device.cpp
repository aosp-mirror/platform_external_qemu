...
private:
    ...
    bool addMemoryMappingLocked(uint64_t gpa, void *ptr, uint64_t size) {
        if (memoryMappings.insert({gpa, {ptr, size}}).second) {
    ...
    void *getHostPtrLocked(uint64_t gpa) const {
        auto i = memoryMappings.lower_bound(gpa);
        if (i == memoryMappings.end()) {
            return nullptr;
        } else if (i->first == gpa) {
    ...
    std::map<uint64_t, std::pair<void *, uint64_t>> memoryMappings;  // do not save/load
};
