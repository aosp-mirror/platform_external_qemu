...
public:
    ...
    uint32_t genHandle() {
        ...
        ++mHandleIndex;
        fprintf(stderr, "%s: new handle: %u\n", __func__, res);
        return res;
    }

    void destroyHandle(uint32_t handle) {
        fprintf(stderr, "%s: erase handle: %u\n", __func__, handle);
        AutoLock lock(mLock);
        mContexts.erase(handle);
    }

    void tellPingInfo(uint32_t handle, void* data) {
        AutoLock lock(mLock);
        auto& contextDesc = mContexts[handle];
        contextDesc.pingInfo = (AddressSpaceDevicePingInfo*)data;
    }
    ...
    void perform(uint32_t handle,
        ...
        fprintf(stderr, "%s: handle %u data %p size %u meta 0x%llx\n", __func__,
                handle, (void*)(uintptr_t)phys_addr, (uint32_t)size, (unsigned long long)metadata);
...
static void sAddressSpaceDeviceTellPingInfo(uint32_t handle, void* info) {
    sAddressSpaceDeviceState->tellPingInfo(handle, info);
}
