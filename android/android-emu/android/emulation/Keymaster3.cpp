...
void keymasterDecodeAndExecute(const std::vector<uint8_t> &input, std::vector<uint8_t> *output) {
    keymaster_ipc_call(input, output);
}

void registerKeymaster3PipeService() {
    android::AndroidPipe::Service::add(new android::AndroidMessagePipe::Service("KeymasterService3", keymasterDecodeAndExecute));
}
} // namespace android
