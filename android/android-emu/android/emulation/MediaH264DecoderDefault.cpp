...
void MediaH264DecoderDefaultImpl::initH264Context(unsigned int width,
    ...
    mCodec = NULL;
    auto useCuvidEnv = android::base::System::getEnvironmentVariable("ANDROID_EMU_CODEC_USE_CUVID_DECODER");
    if (useCuvidEnv != "") {
    ...
    if (!mImageReady) {
        ...
        if (mIsInFlush) {
            ...
            if (retframe != 0) {
                ...
                H264_DPRINT("WARNING: some unknown error %d: %s", retframe, tmp);
                *retErr = static_cast<int>(Err::NoDecodedFrame);
                return;
            }
...
void MediaH264DecoderDefault::updateDecoder(uint64_t key, MediaH264DecoderDefaultImpl* val) {
    std::lock_guard<std::mutex>  g(mMapLock);
    if (mDecoders.find(key) == mDecoders.end()) {
...
void MediaH264DecoderDefault::removeDecoder(uint64_t key) {
    {
        ...
        auto iter = mDecoders.find(key);
        if (iter != mDecoders.end()) {
            H264_DPRINT("removed decoder key %" PRIx64 ", val: %p", key, mDecoders[key]);
