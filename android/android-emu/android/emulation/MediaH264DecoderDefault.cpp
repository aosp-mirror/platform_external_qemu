...
MediaH264DecoderDefaultImpl* MediaH264DecoderDefault::getDecoder(void*ptr) {
    void* key = reinterpret_cast<void*>(*reinterpret_cast<uint64_t*>(ptr));
    if (mDecoders.find(key) != mDecoders.end()) {
        H264_DPRINT("found decoder %p", mDecoders[key]);
        return mDecoders[key];
    }
    H264_DPRINT("cannot find decoder with key %p", key);
    return nullptr;
}

void MediaH264DecoderDefault::addDecoder(void* key, MediaH264DecoderDefaultImpl* val) {
    if (mDecoders.find(key) == mDecoders.end()) {
        mDecoders[key] = val;
        H264_DPRINT("added decoder key %p val: %p", key, val);
        return;
    }
...
void MediaH264DecoderDefault::removeDecoder(void* ptr) {
        void* key = reinterpret_cast<void*>(*reinterpret_cast<uint64_t*>(ptr));
        if (mDecoders.find(key) != mDecoders.end()) {
            H264_DPRINT("removed decoder key %p, val: %p", key, mDecoders[key]);
            mDecoders.erase(key);
            return;
        }
