...
void MediaH264DecoderDefault::decodeFrame(void* ptr, const uint8_t* frame, size_t szBytes) {
    ...
    *retErr = (int32_t)h264Err;
    ...
    if (got_frame) {
      ...
      H264_DPRINT("Failed to decode a frame");
    }
}
...
void MediaH264DecoderDefault::copyFrame() {
    ...
    for (int i=0; i < h / 2; ++i) {
      memcpy(w * h + mDecodedFrame + i * mFrame->linesize[1], mFrame->data[1] + i * mFrame->linesize[0], w / 2);
    }
    for (int i=0; i < h / 2; ++i) {
      memcpy(w * h + w * h / 4 + mDecodedFrame + i * mFrame->linesize[2], mFrame->data[2] + i * mFrame->linesize[0], w / 2);
    }
}
