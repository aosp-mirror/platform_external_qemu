...
void MediaH264DecoderDefault::initH264Context(unsigned int width,
    ...
    //mCodecCtx->flags |= AV_CODEC_FLAG_DROPCHANGED;
...
void MediaH264DecoderDefault::decodeFrame(void* ptr, const uint8_t* frame, size_t szBytes) {
    ...
    //avcodec_decode_video2(mCodecCtx, mFrame, &got_frame, &mPacket);
...
void MediaH264DecoderDefault::copyFrame() {
    ...
    int h = mFrame->height;
    for (int i = 0; i < h; ++i) {
      memcpy(mDecodedFrame + i * mFrame->linesize[0], mFrame->data[0] + i * mFrame->linesize[0], w);
    }
    for (int i=0; i < h / 2; ++i) {
      memcpy(w * h + mDecodedFrame + i * mFrame->linesize[1], mFrame->data[1] + i * mFrame->linesize[1], w / 2);
    }
    for (int i=0; i < h / 2; ++i) {
      memcpy(w * h + w * h / 4 + mDecodedFrame + i * mFrame->linesize[2], mFrame->data[2] + i * mFrame->linesize[2], w / 2);
    }
...
void MediaH264DecoderDefault::flush(void* ptr) {
    ...
    while (1) {
        ...
        if (retframe == AVERROR_EOF) {
            ...
            H264_DPRINT("This should not happen in flushing mode");
        } else {
            H264_DPRINT("Discarding frame in flush mode");
        }
    }
