...
int AdbMessageSnifferImpl::readHeader(int dataSize) {
    ...
    copyFromBuffer(need);
    if (!packetSeemsValid()) {
        *mLogStream << mName << " Received invalid packet.. Disabling logging.";
        mLevel = 0;
        return -1;
    }
...
bool AdbMessageSnifferImpl::checkForDummyShellCommand() {
    ...
    if (msg.command == ADB_OPEN) {
        ...
        auto isShellV2 = [&purpose](const std::string& substr) {
            return purpose.find(substr) != std::string::npos;
        };

        if (std::find(kShellV1.begin(), kShellV1.end(), purpose) !=
                    kShellV1.end() ||
            std::find_if(kShellV2.begin(), kShellV2.end(), isShellV2) !=
                    kShellV2.end()) {
