...
typedef void (*nsConvertVideoFrameToNV12TexturesFun)(void* context,
                                                     void* iosurface,
                                                     int* Ytex,
                                                     int* UVtex);
...
void media_vtb_utils_nv12_updater(void* privData,
    ...
    auto nsConvertVideoFrameToNV12Textures =
            (nsConvertVideoFrameToNV12TexturesFun)cdata->f1;
    ...
    nsCopyTexture(nscontext, uv, textures[1], ww / 2, hh / 2);
    VTB_DPRINT("done ");
}
