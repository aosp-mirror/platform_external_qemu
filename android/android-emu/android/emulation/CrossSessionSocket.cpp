...
android::base::StaticMap<int, android::base::ScopedSocket> sRecycledSockets;
...
void CrossSessionSocket::recycleSocket(CrossSessionSocket&& socket) {
    ...
    if (!sRecycledSockets.isPresent(fd)) return;
    D("%s: recycling %d", __func__, fd);
    //sRecycledSockets.set(fd, std::move(socket.mSocket));
    socket.reset();
}

void CrossSessionSocket::registerForRecycle(int socket) {
    if (!sRecycledSockets.isPresent(socket)) {
        //sRecycledSockets.set(socket, -1);
    }
}

CrossSessionSocket CrossSessionSocket::reclaimSocket(int fd) {
    if (!sRecycledSockets.isPresent(fd)) return CrossSessionSocket();
    //return CrossSessionSocket(std::move(sRecycledSockets.get(fd).value()));
    return CrossSessionSocket();
}
