...
static int socketPipe_sendBuffers(void* opaque,
    ...
    while (count > 0) {
        ...
        int  len = HANDLE_EINTR(send(pipe->socket.fd(),
                                     buff->data + buffStart,
                                     avail,
                                     0));
#else
        // Solution #1: Spin on EAGAIN (WSAEWOULDBLOCK)
        int  len = qemu_windows_send(pipe->socket.fd(),
                                     buff->data + buffStart,
                                 avail);
#endif
...
#ifndef _WIN32
        int  len = HANDLE_EINTR(recv(pipe->socket.fd(),
                                buff->data + buffStart,
                                avail,
                                0));
#else
        int  len = qemu_windows_recv(pipe->socket.fd(),
                                     buff->data + buffStart,
                                     avail,
                                     true);
#endif
...
static void socketPipe_save(void* service_pipe, Stream* file) {
    ...
    if (pipe->socket.valid()) {
        ...
        pipe->socket.drainSocket(
                android::emulation::CrossSessionSocket::DrainBehavior::AppendToBuffer);
        android::emulation::CrossSessionSocket::registerForRecycle(pipe->socket.fd());
...
static void* socketPipe_load(void* hwpipe, void* serviceOpaque,
        const char* args, Stream* file) {
    auto stream = reinterpret_cast<android::base::Stream*>(file);
    SocketPipe* pipe = new SocketPipe;
    ...
    if (fd > 0) {
        pipe->socket = android::emulation::CrossSessionSocket::reclaimSocket(fd);
...
static AndroidPipeFuncs s_unix_pipe_funcs = {
        ...
        socketPipe_poll,
        socketPipe_wakeOn,
        // Save & load will be enabled based on android::featurecontrol::Feature::SnapshotAdb
        nullptr,
        nullptr,
};
