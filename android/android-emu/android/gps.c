...
android_gps_send_nmea( const char*  sentence )
{
    ...
    if (s_enable_gnssgrpcv1) {
        if (s_gnssgrpcv1_send_nmea) {
            //fprintf(stderr, "android_gps_send_nmea is %p\n", android_gps_send_nmea);
            //fprintf(stderr, "s_gnssgrpcv1_send_nmea is %p\n", s_gnssgrpcv1_send_nmea);
            s_gnssgrpcv1_send_nmea(sentence, strlen(sentence));
        } else {
            fprintf(stderr, "s_gnssgrpcv1_send_nmea is null\n");
        }
        return;
    }
...
void
android_gps_set_send_nmea_func(void* fptr) {
    android_gnssgrpcv1_send_nmea_func myf = (android_gnssgrpcv1_send_nmea_func)(fptr);
