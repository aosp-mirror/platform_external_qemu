        ...
        case Proxying:
            ...
            if (mesg->command == ADB_WRTE && mShouldSendCachePacket) {
                ...
                if ((jdwpCmd.flags & 0x80) == 0 && jdwpCmd.command_set < ExtensionBegin) {
                    bool isBreakpointEvent = false;
                    if (jdwpCmd.command_set == 15 && jdwpCmd.command == 1 &&
                        data[11] == 0x4 && data[12] != SuspendPolicy::None) {
                        isBreakpointEvent = true;
                    }
                    mPendingGuestReplyCommands.emplace(jdwpCmd.id, isBreakpointEvent);
            ...
            if (mesg->command == ADB_WRTE) {
                ...
                if (mShouldSendCachePacket) {
                    ...
                    if (jdwpCmd.flags & 0x80 && jdwpCmd.command_set < ExtensionBegin) {
                        const auto ite = mPendingGuestReplyCommands.find(jdwpCmd.id);
                        if (ite != mPendingGuestReplyCommands.end()) {
                            if (ite->second) { // breakpoint event
                                memcpy(&mBreakpointEventId, data + 11, 4);
                            }
                            mPendingGuestReplyCommands.erase(ite);
                        }
                        printf("recv reply id %d, remaining %d\n", jdwpCmd.id, (int)mPendingGuestReplyCommands.size());
                        if (mPendingGuestReplyCommands.size() == 0 && mDebuggerActivated && mBreakpointEventId > 0) {
...
#endif
    memcpy(packet.data.data() + 11 + 6, &proxy->mBreakpointEventId, 4);
