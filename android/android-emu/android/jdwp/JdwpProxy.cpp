...
void JdwpProxy::repeaterCallback(void* opaque, base::Looper::Timer* timer) {
    ...
    assert(timer == proxy->mRepeaterTimer.get());
    assert(proxy->mShouldSendCachePacket);
    assert(proxy->mloadedCachedPacket);
    proxy->mPushToSenderQueue(std::move(*proxy->mloadedCachedPacket.get()));
    proxy->mSendAll();
