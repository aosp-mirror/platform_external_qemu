                ...
                if (mShouldSendCachePacket) {
                    ...
                    if (jdwpCmd.flags & 0x80 && jdwpCmd.command_set < ExtensionBegin) {
                        ...
                        if (jdwpCmd.id == mBreakpointRequestId) {
                            memcpy(&mBreakpointEventId, data + sJdwpHeaderSize, 4);
                        }
                        DD("recv reply id %d, remaining %d\n", jdwpCmd.id, (int)mPendingGuestReplyCommands.size());
                        if (mPendingGuestReplyCommands.size() == 0 && mBreakpointEventId != 0) {
                            *shouldForwardSend = false;
                            ...
                            // 02 (suspend policy: all) 000000001 (number of events) 02 (event kind: break point) xxxxxxxx (request id) ...
                            // We need to overwrite the request id to match those set by JDI
                            memcpy(packet1.data.data() + sJdwpHeaderSize + 6, &mBreakpointEventId, 4);
