...
void JdwpProxy::onGuestRecvData(const android::emulation::amessage* mesg, const uint8_t* data,
            ...
            if (clientProxyDifferent) {
                ...
                *shouldForwardRecv = false;
                ...
                if (mProxyState > HandshakeSentOk) {
                    ...
                    memcpy(packet.data.data(), "JDWP-Handshake", 14);
                    toSends->push(packet);
...
void JdwpProxy::onGuestSendData(const android::emulation::amessage* mesg, const uint8_t* data) {
    if (mesg->command == ADB_CLSE) {
        mShouldClose = true;
    }
    switch (mProxyState) {
