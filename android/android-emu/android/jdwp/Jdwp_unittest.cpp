...
TEST(Jdwp, ReadBuffer) {
    const uint8_t kByteBuffer[8] = {0, 0, 0, 0, 0, 0, 0, 0x7};
    EXPECT_EQ(0x7, readValFromBuffer<uint32_t>(kByteBuffer + 4, 4));
    EXPECT_EQ(0x7, readValFromBuffer<uint32_t>(kByteBuffer, 8));
    EXPECT_EQ(0x7, uint32FromBuffer(kByteBuffer + 4));
}
...
TEST(Jdwp, WriteBuffer) {
    ...
    EXPECT_EQ(0, memcmp(actualBuffer, kExpectedByteBuffer, sizeof(kExpectedByteBuffer)));
}

TEST(Jdwp, WriteStrBuffer) {
    const uint8_t kExpectedByteBuffer[8] = {0, 0, 0, 0x4, 'H', 'E', 'L', 'O'};
...
TEST(Jdwp, EventRequest) {
    ...
    EXPECT_EQ(0, memcmp(kExpected4BytesId, buffer, sizeof(kExpected4BytesId)));
    ...
    EXPECT_EQ(0, memcmp(kExpected8BytesId, buffer, sizeof(kExpected8BytesId)));
}
