...
#include "android/videoplayback/videoplayback-render-target.h"
...
void VideoplaybackRenderTarget::getRenderTargetSize(
    ...
    if (width > mRenderWidth) {
        width = mRenderWidth;
        height = (int)(width / sampleAspectRatio);
    }
...
void VideoplaybackRenderTarget::renderBuffer() {
    ...
    if (mBuffer != nullptr) {
        ...
        PPMInfo frameInfo;
        int headerSize = parsePPMHeader(&frameInfo, mBuffer, mBufferLen);
        const unsigned char* frameBuf = &mBuffer[headerSize];

        // Load the processed frame into the texture.
        mGles2->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, frameInfo.width,
                             frameInfo.height, 0, GL_RGB, GL_UNSIGNED_BYTE,
                             frameBuf);
        ...
        GLint posAttrib =
            mGles2->glGetAttribLocation(program, "in_position");
