...
static void _hwCarClient_recv(void* opaque,
    ...
    if (hwCar->callback != nullptr && hwCar->callback->callback != nullptr) {
        hwCar->callback->callback((char*)msg, msglen, hwCar->callback->context);
    }
}
