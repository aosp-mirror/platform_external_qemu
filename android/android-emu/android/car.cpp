...
std::function<void(const char*, int)> _callback;
...
//        _callback((char *)msg);
 //    }
    if (_callback != NULL) {
        _callback((char*) msg, msglen);
    }
    android_send_car_data((char*) msg, msglen);
}
...
//    qemud_client_send(_car->client->client, (const uint8_t*)msgString.c_str(), msgString.length());
    qemud_client_send(_car->client->client, (const uint8_t*)msg, msgLen);

   D("Yao: android_send_car_data %s %d", msg, msgLen);
}
