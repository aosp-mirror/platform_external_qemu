...
static void on_screenrecord_stop(void* opaque, int success) {
    if (success != 0) {
...
static int do_screenrecord_start(ControlClient client, char* args) {
    ...
    while (true) {
        int optionIndex = 0;
        int ic = getopt_long(splitArgs.size(), (char**)sarray, "", longOptions,
                             &optionIndex);
        if (ic == -1) {
            D(("Got ic=-1\n"));
            break;
            ...
            case 'b':
                ...
                if (info.video_bitrate < kMinVideoBitrate ||
                    ...
                    control_write(client,
                                  ...
                                  "[%d,%d]\r\n",
                                  info.video_bitrate, kMinVideoBitrate,
                                  kMaxVideoBitrate);
                    return -1;
                }
                break;
...
static int do_screenrecord_stop(ControlClient client, char* args) {
    if (client->global->record_state != RecordState::Recording) {
        control_write(client,
                      "KO: Either recording has not started or is in process "
                      "of stopping.\r\n");
        return -1;
    }
