 ...
 *
 *  this console is enabled automatically at emulator startup, on port 5554 by
 * default, unless some other emulator is already running. See
 * (android_emulation_start in android_sdl.c for details)
 *
 *  you can telnet to the console, then use commands like 'help' or others to
 * dynamically change emulator settings.
 *
 */
...
static bool isCommandEnabled(const char* command_name) {
    ...
    for (int i = 0; command_flags[i].name != nullptr; ++i) {
        if (!strcmp(command_name, command_flags[i].name)) {
            return android::featurecontrol::isEnabled(command_flags[i].feature);
        }
    }
...
static int control_global_add_redir(ControlGlobal global,
    ...
    if (global->num_redirs >= global->max_redirs) {
        ...
        auto new_redirs = (Redir)realloc(global->redirs,
                                         new_max * sizeof(global->redirs[0]));
        if (new_redirs == NULL)
            return -1;
 ...
 */
static int control_client_detach(ControlClient client) {
    int result;
...
static void control_client_destroy(ControlClient client) {
...
static void control_control_write(ControlClient client,
                                  const char* buff,
                                  int len) {
    if (len < 0)
        len = strlen(buff);
...
static int control_vwrite(ControlClient client,
                          const char* format,
                          va_list args) {
...
static ControlClient control_client_create(Socket socket,
                                           ControlGlobal global) {
...
static CommandDef find_command(char* input,
                               CommandDef commands,
                               char** pend,
                               char** pargs) {
    int nn;
...
static void dump_help(ControlClient client,
                      CommandDef cmd,
                      const char* prefix) {
    if (cmd->description) {
...
static void control_client_do_command(ControlClient client) {
    ...
    for (;;) {
        ...
        if (cmd->subcommands == NULL) {
            control_write(
                    client,
                    "KO: internal error: buggy command table for '%.*s'\r\n",
...
static int do_help(ControlClient client, char* args) {
    char* line;
    char* start = args;
    char* end = start;
...
static int do_help_verbose(ControlClient client, char* args) {
    CommandDef cmd;
...
static int do_ping(ControlClient client, char* args) {
    control_write(client, "I am alive!\r\n");
    return 0;
}

static void control_client_read_byte(ControlClient client, unsigned char ch) {
    if (ch == '\r') {
        /* filter them out */
    } else if (ch == '\n') {
...
#define ANDROID_CONSOLE_COPY_AGENT(type, name) \
    ...
    if (console_auth_status == CONSOLE_AUTH_STATUS_ERROR) {
        ...
        char* emulator_console_auth_token_path =
                android_console_auth_token_path_dup();
        derror("Unable to access '%s': emulator console will not "
               "work",
...
static int do_quit(ControlClient client, char* args) {
    client->finished = 1;
    return -1;
}
...
static void dump_network_speeds(ControlClient client) {
    ...
    control_write(client, format, "<num>",
                  "selects both upload and download speed");
    control_write(client, format, "<up>:<down>",
                  "select individual upload/download speeds");
}

static int do_network_speed(ControlClient client, char* args) {
    if (!args) {
        control_write(
                client,
    ...
    if (!android_network_set_speed(args)) {
        control_write(client,
                      "KO: invalid <speed> argument, see 'help network speed' "
                      "for valid values\r\n");
        return -1;
    }
...
static void describe_network_speed(ControlClient client) {
    control_write(client,
                  "'network speed <speed>' allows you to dynamically change "
                  "the speed of the emulated\r\n"
                  "network on the device, where <speed> is one of the "
                  "following:\r\n\r\n");
    dump_network_speeds(client);
}

static int do_network_delay(ControlClient client, char* args) {
    if (!args) {
        control_write(
                client,
                "KO: missing <delay> argument, see 'help network delay'\r\n");
        return -1;
    }
    if (!android_network_set_latency(args)) {
        control_write(client,
                      "KO: invalid <delay> argument, see 'help network delay' "
                      "for valid values\r\n");
        return -1;
    }
...
static void describe_network_delay(ControlClient client) {
    control_write(client,
                  "'network delay <latency>' allows you to dynamically change "
                  "the latency of the emulated\r\n"
                  "network on the device, where <latency> is one of the "
                  "following:\r\n\r\n");
    /* XXX: TODO */
}
...
static int do_network_capture_stop(ControlClient client, char* args) {
...
static const CommandDefRec network_capture_commands[] = {
        {"start", "start network capture",
         ...
         NULL, do_network_capture_start, NULL},
        ...
        {"stop", "stop network capture",
         ...
         NULL, do_network_capture_stop, NULL},

        {NULL, NULL, NULL, NULL, NULL, NULL}};

static const CommandDefRec network_commands[] = {
        {"status", "dump network status", NULL, NULL, do_network_status, NULL},
...
static void describe_wifi_add(ControlClient client) {
    control_write(client,
                  "'wifi add <ssid> [password]' will add a new SSID named "
                  "<ssid> that will be visible\n"
                  "on the device. If the optional [password] is provided the "
                  "SSID will be set up\n"
                  "with this WPA2-PSK password. If an SSID with the same name "
                  "already exists\n"
                  "the password will be updated if different, otherwise "
                  "nothing will happen.\n");
}
...
static void describe_wifi_block(ControlClient client) {
    control_write(client,
                  "'wifi block <ssid>' will block network access on the given "
                  "SSID. The device\n"
                  "will still be able to connect to the SSID but will not be "
                  "able to access the\n"
...
static void describe_wifi_unblock(ControlClient client) {
    control_write(
            client,
            "'wifi unblock <ssid>' will unblock network access on the given "
            "SSID. The device\n"
...
static const CommandDefRec wifi_commands[] = {
        {"add", "add new WiFi SSID", NULL, describe_wifi_add, do_wifi_add,
         NULL},
        {"block", "block network access on SSID", NULL, describe_wifi_block,
         do_wifi_block, NULL},
        {"unblock", "unblock network access on SSID", NULL,
         describe_wifi_unblock, do_wifi_unblock, NULL},
        {NULL, NULL, NULL, NULL, NULL, NULL}};
...
static int do_redir_list(ControlClient client, char* args) {
    ControlGlobal global = client->global;
...
static int redir_parse_proto_port(char* args, int* pport, int* pproto) {
    int proto = -1;
    int len = 0;
    char* end;
...
static int redir_parse_guest_port(char* arg, int* pport) {
    char* end;
...
BadFormat:
    control_write(client,
                  "KO: bad redirection format, try (tcp|udp):hostport\r\n");
    return -1;
}
...
static void dump_subscription_sources(ControlClient client) {
    int i;
    for (i = 0; i < sizeof(_cdma_subscription_sources) /
...
static void describe_subscription_source(ControlClient client) {
    control_write(client,
                  "'cdma ssource <ssource>' allows you to specify where to "
                  "read the subscription from\r\n");
    dump_subscription_sources(client);
}

static int do_cdma_ssource(ControlClient client, char* args) {
    int nn;
    if (!args) {
        control_write(client,
...
static int do_cdma_prl_version(ControlClient client, char* args) {
    int version = 0;
    char* endptr;
...
static int do_gsm_status(ControlClient client, char* args) {
    if (args) {
...
static int do_gsm_data(ControlClient client, char* args) {
    int nn;

    if (!args) {
        control_write(client,
                      "KO: missing argument, try 'gsm data <state>'\r\n");
        return -1;
    }
    ...
    control_write(client,
                  "KO: bad GSM data state name, try 'help gsm data' for list "
                  "of valid values\r\n");
    return -1;
}

static void help_gsm_voice(ControlClient client) {
    int nn;
    control_write(client,
...
static int do_gsm_voice(ControlClient client, char* args) {
    int nn;

    if (!args) {
        control_write(client,
                      "KO: missing argument, try 'gsm voice <state>'\r\n");
        return -1;
    }
    ...
    control_write(client,
                  "KO: bad GSM data state name, try 'help gsm voice' for list "
                  "of valid values\r\n");
    return -1;
}
...
static int do_gsm_call(ControlClient client, char* args) {
    ...
    if (!args) {
        control_write(client,
                      "KO: missing argument, try 'gsm call <phonenumber>'\r\n");
        return -1;
    }
...
static int do_gsm_cancel(ControlClient client, char* args) {
    if (!args) {
        control_write(client,
                      "KO: missing argument, try 'gsm call <phonenumber>'\r\n");
        return -1;
...
static int do_gsm_list(ControlClient client, char* args) {
    /* check that we have a phone number made of digits */
...
static int do_gsm_busy(ControlClient client, char* args) {
    ACall call;

    if (!args) {
        control_write(client,
                      "KO: missing argument, try 'gsm busy <phonenumber>'\r\n");
        return -1;
    }
...
static int do_gsm_hold(ControlClient client, char* args) {
    ACall call;
...
static int do_gsm_accept(ControlClient client, char* args) {
    ACall call;
...
static int do_gsm_signal(ControlClient client, char* args) {
...
static int do_gsm_signal_profile(ControlClient client, char* args) {
    char* end;
    const char* p = args;
    if (!p)
        p = "";
...
static const CommandDefRec cdma_commands[] = {
        {"ssource", "Set the current CDMA subscription source", NULL,
         describe_subscription_source, do_cdma_ssource, NULL},
        {"prl_version", "Dump the current PRL version", NULL, NULL,
...
static int do_sms_send(ControlClient client, char* args) {
    char* p;
    int textlen;
    ...
    if (sms_address_from_str(&sender, args, p - args) < 0) {
        control_write(client,
                      "KO: bad phone number format, must be [+](0-9)* or "
                      "[a-zA-Z0-9]+\r\n");
        return -1;
    }
...
static int do_sms_sendpdu(ControlClient client, char* args) {
    SmsPDU pdu;
    ...
    if (!args) {
        control_write(
                client,
                "KO: missing argument, try 'sms sendpdu <hexstring>'\r\n");
        return -1;
    }
...
static int do_power_display(ControlClient client, char* args) {
    client->global->battery_agent->batteryDisplay(client, control_write_out_cb);
    return 0;
}

static int do_ac_state(ControlClient client, char* args) {
    if (args) {
...
static int do_battery_status(ControlClient client, char* args) {
    if (args) {
...
static int do_battery_present(ControlClient client, char* args) {
    if (args) {
        ...
        if (strcasecmp(args, "false") == 0) {
            client->global->battery_agent->setIsBatteryPresent(false);
            return 0;
        }
    }
...
static int do_battery_health(ControlClient client, char* args) {
    if (args) {
        if (strcasecmp(args, "unknown") == 0) {
...
static int do_event_send(ControlClient client, char* args) {
    char* p;
    ...
    while (*p) {
        ...
        if (ret < 0) {
            if (ret == -1) {
                ...
                control_write(client,
                              "KO: invalid event code in '%.*s', try 'event "
                              "list codes <type>' for valid values\r\n",
                              q - p, p);
            } else {
                control_write(client,
...
static int do_event_types(ControlClient client, char* args) {
    int count = android_event_get_type_count();
    int nn;

    control_write(client,
                  "event <type> can be an integer or one of the following "
                  "aliases\r\n");
    for (nn = 0; nn < count; nn++) {
        char tmp[16];
        char* p = tmp;
...
static int do_event_codes(ControlClient client, char* args) {
    int count;
    int nn, type, dummy;

    if (!args) {
        control_write(client,
                      "KO: argument missing, try 'event codes <type>'\r\n");
        return -1;
    }
    ...
    if (count == 0) {
        ...
        control_write(client,
                      "type '%s' accepts the following <code> aliases:\r\n",
                      args);
        for (nn = 0; nn < count; nn++) {
...
static int do_event_text(ControlClient client, char* args) {
    if (!args) {
        control_write(client,
    ...
    textlen = sms_utf8_from_message_str(args, textlen, (unsigned char*)p,
                                        textlen);
    if (textlen < 0) {
        control_write(
                client,
...
static int do_snapshot_save(ControlClient client, char* args) {
    if (getConsoleAgents()->settings->android_cmdLineOptions()->read_only) {
        control_write(client,
                      "KO: Snapshot save is disabled because \"-read-only\" "
                      "was specified\n");
        return -1;
    }
...
static int do_snapshot_load(ControlClient client, char* args) {
    if (getConsoleAgents()->settings->android_cmdLineOptions()->read_only) {
        control_write(client,
                      "KO: Snapshot load is disabled because \"-read-only\" "
                      "was specified\n");
        return -1;
    }
...
static int do_snapshot_del(ControlClient client, char* args) {
    if (getConsoleAgents()->settings->android_cmdLineOptions()->read_only) {
        control_write(client,
                      "KO: Snapshot delete is disabled because \"-read-only\" "
                      "was specified\n");
        return -1;
    }
...
static int do_snapshot_last_loaded(ControlClient client, char* args) {
    bool success = vmopers(client)->snapshotLastLoaded(
            client, control_write_out_cb, control_write_err_cb);
    return success ? 0 : -1;
}
...
static int do_avd_stop(ControlClient client, char* args) {
...
static int do_avd_start(ControlClient client, char* args) {
    if (vmopers(client)->vmIsRunning()) {
...
static int do_avd_status(ControlClient client, char* args) {
    control_write(client, "virtual device is %s\r\n",
                  vmopers(client)->vmIsRunning() ? "running" : "stopped");
    return 0;
}

static int do_avd_heartbeat(ControlClient client, char* args) {
...
static int do_avd_rewind_audio(ControlClient client, char* args) {
...
static int do_avd_snapshotpath(ControlClient client, char* args) {
    if (!args) {
...
static int do_avd_pause(ControlClient client, char* args) {
    bool success = vmopers(client)->vmPause();
    return success ? 0 : -1;
}

static int do_avd_resume(ControlClient client, char* args) {
    bool success = vmopers(client)->vmResume();
    return success ? 0 : -1;
}

static int do_avd_hostmicon(ControlClient client, char* args) {
    (void)args;
    vmopers(client)->allowRealAudio(true /* pass through host mic data */);
    return 0;
}

static int do_avd_hostmicoff(ControlClient client, char* args) {
    (void)args;
    vmopers(client)->allowRealAudio(
            false /* send 0 instead of host mic data */);
    return 0;
}

static int do_avd_bugreport(ControlClient client, char* args) {
...
static const CommandDefRec vm_commands[] = {
        ...
        {"heartbeat", "query the heart beat number of the guest system",
         "'avd heartbeat' will report the number of heart beats from guest "
         "system running inside this avd\r\n",
         NULL, do_avd_heartbeat, NULL},

        {"rewindaudio", "rewind the input audio to the beginning",
         "'avd rewindaudio' will rewind the input of audio to "
         "beginning(applicable only to wav audio driver)\r\n",
        ...
        {"pause", "pause the virtual device",
         "'avd pause' pauses the virtual device. This is useful for saving "
         "system resources\r\n"
         "when the virtual device is not immediately needed.",
         NULL, do_avd_pause, NULL},

        {"hostmicon", "activate the host audio input device",
         "'avd hostmicon' allows real audio to go through the guest microphone "
         "input.\r\n",
         NULL, do_avd_hostmicon, NULL},

        {"hostmicoff", "deactivate the host audio input device",
         "'avd hostmicoff' sends zeroes to the guest microphone input "
         "regardless of host microphone signals.\r\n",
        ...
        {"bugreport", "generate bug report info.",
         "'avd bugreport' will print out bug report information which is used "
         "for the filling the template in issue tracker.\r\n",
         NULL, do_avd_bugreport, NULL},

        {"id", "query virtual device ID",
         "'avd id' will return the ID of this virtual device (possibly "
         "separate from the name)\r\n",
         NULL, do_avd_id, NULL},

        {"windowtype", "query virtual device headless or qtwindow",
         "'avd windowtype' will return either windowtype=headless or "
         "windowtype=qtwindow.\r\n",
         NULL, do_avd_windowtype, NULL},

        {"path", "query AVD path",
         "'avd path' will return the path to the AVD files. If no AVD can be "
         "found, 'NO_AVD_INFO' is returned. If the path cannot be queried, an "
         "empty string will be returned.\r\n",
         NULL, do_avd_path, NULL},

        {"discoverypath", "query AVD discovery path",
         "'avd discoverypath' will return the path to the discovery file (for "
         "use with Studio embedded). If no AVD can be found, 'NO_AVD_INFO' is "
         "returned. If the path cannot be queried, an empty string will be "
         "returned.\r\n",
         NULL, do_avd_discoverypath, NULL},

        {"snapshotspath", "query AVD snapshots path",
         "'avd snapshotspath' will return the path where snapshots are stored "
         "for the current AVD. If no AVD can be found, 'NO_AVD_INFO' is "
         "returned. If the path cannot be queried, an empty string will be "
         "returned.\r\n",
         NULL, do_avd_snapshotspath, NULL},

        {"snapshotpath", "query path to a particular AVD snapshot",
         "'avd snapshotpath <snapshotname>' will return the directory where a "
         "particular snapshot is stored. Requires one argument: the name of "
         "the snapshot on disk (The user-specified name is currently not "
         "supported). If no AVD can be found, 'NO_AVD_INFO' is returned. If "
         "the snapshot does not exist at that directory, 'NO_SNAPSHOT' is "
         "returned. If the path cannot be formed for some other reason, an "
...
static int do_geo_nmea(ControlClient client, char* args) {
    if (!args) {
        control_write(client,
    ...
    if (!client->global->location_agent->gpsIsSupported()) {
        control_write(client,
                      "KO: no GPS emulation in this virtual device\r\n");
        return -1;
    }
...
static int do_geo_fix(ControlClient client, char* args) {
    enum {
        GEO_LONG = 0,
        GEO_LAT,
        GEO_ALT,
    ...
    /* check longitude and latitude */
    {
        ...
        if (longitude < -180.0 || longitude > 180.0) {
            control_write(client,
                          "KO: invalid longitude value. Should be in "
                          "[-180,+180] range\r\n");
            return -1;
        }
        ...
        if (latitude < -90.0 || latitude > 90.0) {
            control_write(client,
                          "KO: invalid latitude value. Should be in [-90,+90] "
                          "range\r\n");
            return -1;
        }
    }
    ...
    if (top_param >= GEO_SAT) {
        ...
        if (n_satellites != params[GEO_SAT] || n_satellites < 1 ||
            n_satellites > 12) {
            control_write(client,
                          "KO: invalid number of satellites. Must be an "
    ...
    if (location_agent_qt_settings_func) {
        location_agent_qt_settings_func(params[GEO_LAT], params[GEO_LONG],
...
static int do_geo_gnss(ControlClient client, char* args) {
    if (!args) {
        control_write(client,
                      "KO: GNSS sentence missing, try 'help geo gnss'\r\n");
        return -1;
    }
    if (!client->global->location_agent->gpsIsSupported()) {
        control_write(client,
...
static const CommandDefRec geo_commands[] = {
        ...
        {"gnss", "send a GNSS sentence",
         ...
         "GnssClockFlags,count,Svid,ConstellationType,State,"
...
static int do_sensors_get(ControlClient client, char* args) {
    if (!args) {
...
SENSOR_STATUS_ERROR:
    switch (status) {
        ...
        case SENSOR_STATUS_UNKNOWN:
            snprintf(buffer, sizeof(buffer),
                     "KO: unknown sensor name: %s, run 'sensor status' to get "
                     "available sensors.\r\n",
                     sensor);
            break;
        default:
            snprintf(buffer, sizeof(buffer),
                     "KO: '%s' sensor: exception happens.\r\n", sensor);
    }
    do_control_write(client, buffer);
    return -1;
}
...
static int do_sensors_set(ControlClient client, char* args) {
    if (!args) {
        control_write(client,
                      "KO: Usage: \"set <sensorname> "
...
static int do_sensors_status(ControlClient client, char* args) {
    uint8_t id, status;
    char buffer[SENSORS_INFO_SIZE] = {0};
...
static const CommandDefRec physics_commands[] = {
        {"record-gt",
         ...
         "'record-gt <filename>': start recording of ground truth to the given "
         "file.\r\n",
...
static int do_fingerprint_touch(ControlClient client, char* args) {
    if (args) {
        char* endptr;
...
static int do_fingerprint_remove(ControlClient client, char* args) {
...
static int do_screenrecord_screenshot(ControlClient client, char* args) {
    ...
    static const struct option longOptions[] = {
            {"display", required_argument, NULL, 'd'}, {NULL, 0, NULL, 0}};
    ...
    if (optind != splitArgs.size() - 1) {
        return client->global->record_agent->doSnap(nullptr, displayId) ? 0
                                                                        : -1;
    }

    return client->global->record_agent->doSnap(sarray[optind], displayId) ? 0
                                                                           : -1;
}
...
static int do_screenrecord_webrtc(ControlClient client, char* args) {
    ...
    if (splitArgs[0] == "start") {
        ...
        const char* name =
                client->global->record_agent->startSharedMemoryModule(fps);
        if (name) {
...
static int do_qemu_monitor(ControlClient client, char* args) {
...
#define HELP_VERBOSE_COMMAND                                                \
    {                                                                       \
        "help-verbose", "print a list of commands with descriptions", NULL, \
                NULL, do_help_verbose, NULL                                 \
    }
...
static int do_crash(ControlClient client, char* args) {
    ...
    if (args) {
        ...
        if (sscanf(args, "%d", &port) == 1 && port >= 0 && port < 65536) {
            ...
            if (port != 0 && active != port) {
                control_write(
                        client,
                        "OK: Port has already been activated at port: %d\n",
                        active);
            } else {
                control_write(client,
                              "OK: gRPC endpoint available at port %d\r\n",
                              active);
            }
            return 0;
        }
...
static int do_crash_on_exit(ControlClient client, char* args) {
    control_write(client, "OK: crashing emulator on exit, bye bye\r\n");
    crashhandler_exitmode("console-simulated crash on exit");
    crash();
}

static int do_kill(ControlClient client, char* args) {
    if (client->global->record_agent->getRecorderState().state ==
        RECORDER_RECORDING) {
        D(("Stopping the recording ...\n"));
    ...
    if (needRequestClose) {
        ...
        client->global->libui_agent->requestExit(0,
                                                 "Killed by console command");
#else
        kill(getpid(), SIGINT);
#endif
        return 0;
...
static int do_restart(ControlClient client, char* args) {
    control_write(client, "OK: restarting emulator, bye bye\r\n");
    DINIT("Emulator restart by console restart command.\n");
    fflush(stdout);
    client->global->libui_agent->requestRestart(0,
                                                "Restarted by console command");
    return 0;
}
...
static int do_debug(ControlClient client, char* args) {
    ...
    if (!android_parse_debug_tags_option(args, /*parse_as_suffix*/ false)) {
        control_write(client,
                      "KO: bad tags, see command line help for "
                      ""
                      "-debug"
                      " option for a list of valid ones\r\n");
        return -1;
    }
    return 0;
}
...
static int do_no_draw(ControlClient client, char* args) {
    ...
    std::transform(args_lower.begin(), args_lower.end(), args_lower.begin(),
                   ::tolower);
    if (args_lower == "on" || args_lower == "true" || args_lower == "yes") {
        android::featurecontrol::setEnabledOverride(
                android::featurecontrol::NoDraw, true);
    } else if (args_lower == "off" || args_lower == "false" ||
               args_lower == "no") {
        android::featurecontrol::setEnabledOverride(
                android::featurecontrol::NoDraw, false);
    } else {
        control_write(client, "KO: expecting nodraw parameter: on/off.\r\n");
        return -1;
    }
...
extern const CommandDefRec main_commands[] = {
        ...
        {"nodraw", "turn on/off NoDraw mode. (experimental)", NULL, NULL,
         do_no_draw, NULL},
