...
static int do_icebox_run(ControlClient client, char* args) {
    if (android::icebox::run_async(args)) {
        ...
        control_write(client, "KO: Failed to run command, might have pening icebox operations, please try again\r\n");
        return -1;
    }
}
...
static int do_icebox_track(ControlClient client, char* args) {
    ...
    if (android::icebox::track_async(pid, "tmp_snapshot")) {
        ...
        control_write(client, "KO: Failed to track PID %d, might have pening icebox operations, please try again\r\n", pid);
        return -1;
    }
}
