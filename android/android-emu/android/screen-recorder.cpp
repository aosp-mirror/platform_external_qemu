...
static void screen_recorder_fb_update(void* opaque, int x, int y, int width, int height) {
    D("[%d] %s(x=%d, y=%d, w=%d, h=%d)", ++sGlobals->frameCount, __func__, x, y, width, height);
...
void screen_recorder_init(int w, int h, const QAndroidDisplayAgent* dpy_agent) {
    ...
    D("%s(w=%d, h=%d, isGuestMode=%d)", __func__, w, h, sGlobals->isGuestMode);
}
...
int screen_recorder_start(const RecordingInfo* info) {
    ...
    if (sGlobals->isGuestMode) {
        sGlobals->displayAgent.registerUpdateListener(&screen_recorder_fb_update, nullptr);
...
void screen_recorder_stop(void) {
    ...
    if (sGlobals->isGuestMode) {
        sGlobals->displayAgent.unregisterUpdateListener(&screen_recorder_fb_update);
        AutoLock lock(sGlobals->guestFBLock);
        if (sGlobals->guestFrame) {
