                              ...
                              int* glesMinorVersion_out)
{
    ...
    ConsumerInterface interface = {
        ...
        // global preload
        []() {
            // This wants to address that when using asg, pipe wants to clean up all render threads and wait for gl objects, but framebuffer notices that there is a render thread info that is still not cleaned up because these render threads come from asg.

            fprintf(stderr, "%s: asg global preload\n", __func__);
