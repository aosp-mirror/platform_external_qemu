...
void FoldableModel::initFoldableRoll() {
    ...
    if (rollRangeTokens.size() != numRolls ||
        ...
        E("Incorrect rollable configs for ranges %s, defaults %s, or "
          "radius %s\n",
          rollRanges.c_str(), rollDefaults.c_str(), rollRadius.c_str());
    } else {
...
bool FoldableModel::sendFoldedAreaToSystem() {
    auto adbInterface = emulation::AdbInterface::getGlobal();
    if (!adbInterface) return false;

    int xOffset, yOffset, width, height; 
    char foldedArea[64];
    if (!getFoldedArea(&xOffset, &yOffset, &width, &height)) {
        return false;
    }
    ...
    adbInterface->enqueueCommand(
            ...
            [](const android::emulation::OptionalAdbCommandResult& result) {
                if (result && result->exit_code == 0) {
                    VLOG(foldable) << "foldable roll: 'fold-area' command succeeded";
                }});
