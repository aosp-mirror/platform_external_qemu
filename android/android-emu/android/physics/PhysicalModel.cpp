...
#undef PHYSICAL_PARAMETER_
    ...
    const uint32_t kRecordingFileVersion = 1;
    ...
    if (filename != nullptr) {
        if (!shouldRecord) {
            ...
            if (!recordingSaveFile) {
                ...
                fwrite(&kRecordingFileVersion,
                       sizeof(uint32_t), 1,
                       recordingSaveFile.get());
            }
        }
    }
...
int PhysicalModelImpl::load(Stream* f) {
    ...
    if (mPlaybackCommands.size() > 0) {
        D("%s: cannot playback physics motion after a snapshot load.  Physics "
          "playback will not occur.", __FUNCTION__);
        mPlaybackCommands.resize(0);
    }
...
void PhysicalModelImpl::playback(const char* filename) {
    ...
    uint32_t version;
    if (1 != fread(&version, sizeof(uint32_t), 1, fp.get())) {
        E("%s: Error unable to read version number from file %s.  "
          "Physical Motion will not be played back.", __FUNCTION__, filename);
        return;
    }
    ...
    if (version != kRecordingFileVersion) {
        E("%s: Unsupported version %d in physics file %s.  Required Version "
          ...
          __FUNCTION__, version, filename, kRecordingFileVersion);
        return;
    }

    fseek(fp.get(), 0 , SEEK_END);
    const uint64_t fileSize = ftell(fp.get());
    const uint64_t numCommands =
            (fileSize - sizeof(uint32_t)) / sizeof(TargetCommand);
    fseek(fp.get(), sizeof(uint32_t) , SEEK_SET);
    if (numCommands) {
