...
public:
     ...
     * Can be called from any thread.
     */
    void setTargetPosition(vec3 position, PhysicalInterpolation mode);

    /*
     * Sets the rotation that the modeled object should move toward.
     * Can be called from any thread.
     */
    void setTargetRotation(vec3 rotation, PhysicalInterpolation mode);
     ...
     * Can be called from any thread.
     */
    void setTargetMagneticField(vec3 field, PhysicalInterpolation mode);
     ...
     * Can be called from any thread.
     */
    void setTargetTemperature(float celsius, PhysicalInterpolation mode);
     ...
     * Can be called from any thread.
     */
    void setTargetProximity(float centimeters, PhysicalInterpolation mode);
     ...
     * Can be called from any thread.
     */
    void setTargetLight(float lux, PhysicalInterpolation mode);
     ...
     * Can be called from any thread.
     */
    void setTargetPressure(float hPa, PhysicalInterpolation mode);
     ...
     * Can be called from any thread.
     */
    void setTargetHumidity(float percentage, PhysicalInterpolation mode);
...
#undef GET_PHYSICAL_NAME
     ...
     */
    template<class T>
    void setOverride(PhysicalSensor sensor,
     ...
     */
    template<class T>
    T getSensorValue(PhysicalSensor sensor,
...
void PhysicalModelImpl::OVERRIDE_FUNCTION_NAME(y)(z override_value) {\
    setOverride(SENSOR_NAME(x), &OVERRIDE_NAME(y), override_value);\
}
PHYSICAL_SENSORS_LIST
#undef SENSOR_
...
z PhysicalModelImpl::GET_FUNCTION_NAME(y)(\
        long* measurement_id) const {\
    return getSensorValue<z>(SENSOR_NAME(x),\
                             &OVERRIDE_NAME(y),\
                             std::bind(&PhysicalModelImpl::PHYSICAL_NAME(y),\
                                       this),\
                             measurement_id);\
}
PHYSICAL_SENSORS_LIST
    ...
    for (int parameter = 0;
        ...
        switch (parameter) {
            ...
            case PHYSICAL_PARAMETER_MAGNETIC_FIELD: {
                ...
                setTargetMagneticField(
                        targetMagneticField, PHYSICAL_INTERPOLATION_STEP);
                break;
            }
            case PHYSICAL_PARAMETER_TEMPERATURE:
                setTargetTemperature(
                        stream_get_float(f), PHYSICAL_INTERPOLATION_STEP);
                break;
            case PHYSICAL_PARAMETER_PROXIMITY:
                setTargetProximity(
                        stream_get_float(f), PHYSICAL_INTERPOLATION_STEP);
...
void physicalModel_setTargetPosition(PhysicalModel* model,
        vec3 position, PhysicalInterpolation mode) {
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetPosition(position, mode);
    }
}

void physicalModel_setTargetRotation(PhysicalModel* model,
        vec3 rotation, PhysicalInterpolation mode) {
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetRotation(rotation, mode);
    }
}
...
void physicalModel_setTargetMagneticField(PhysicalModel* model,
    ...
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetMagneticField(field, mode);
    }
}
...
void physicalModel_setTargetTemperature(struct PhysicalModel* model,
    ...
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetTemperature(celsius, mode);
    }
}
...
void physicalModel_setTargetProximity(struct PhysicalModel* model,
    ...
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetProximity(centimeters, mode);
    }
}
...
void physicalModel_setTargetPressure(struct PhysicalModel* model,
    ...
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetPressure(hPa, mode);
    }
}
...
void physicalModel_setTargetHumidity(struct PhysicalModel* model,
    ...
    PhysicalModelImpl* impl = PhysicalModelImpl::getImpl(model);
    if (impl != nullptr) {
        impl->setTargetHumidity(percentage, mode);
    }
}
