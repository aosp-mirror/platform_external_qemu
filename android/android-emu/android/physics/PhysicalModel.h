...
void _physicalModel_setMagneticValue(struct PhysicalModel* model,
        float north, float east, float vertical, bool instantaneous);

// TODO(tpsiaki): add remaining ambient values.

/* Get current simulated state of the modeled object */
struct vec3 _physicalModel_getPosition(struct PhysicalModel* model);
struct vec3 _physicalModel_getRotation(struct PhysicalModel* model);
