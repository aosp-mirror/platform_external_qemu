...
constexpr uint64_t kStateChangeTimeNs =
        secondsToNs(kStateChangeTimeSeconds);
...
glm::quat InertialModel::getRotation(
    ...
    // Rotation length should not be zero, but it may be possible by driving
    // the inertial model in an extreme way (i.e. well timed oscilations) to
    // hit this case.  In this degenerate case, we will simply return an
    // identity rotation and the user may see a discontinuity in rotations.
    const float rotationVecLength = glm::length(rotationVec);
    if (rotationVecLength == 0.f) {
        return glm::quat();
    }

    const glm::vec4 rotation = (1.f / rotationVecLength) * rotationVec;
