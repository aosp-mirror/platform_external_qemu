...
void InertialModel::setTargetVelocity(
    ...
    if (mode == PHYSICAL_INTERPOLATION_STEP) {
        ...
        const glm::vec3 quarticTerm = (1.f / (4.f * time_3)) * (
                1.f * time_1 * a_init +
                2.f * v_init +
                -2.f * v_target);
        const glm::vec3 cubicTerm = (1.f / (3.f * time_2)) * (
                -2.f * time_1 * a_init +
                -3.f * v_init +
                3.f * v_target);
        const glm::vec3 quadraticTerm = (1.f / 2.f) * a_init;
...
void InertialModel::setInertialTransforms(
    ...
    constexpr float time_4 = time_2 * time_2;
    constexpr float time_5 = time_2 * time_3;
    glm::vec2 quinticTimeVec(time_5, time_4);
    glm::vec4 cubicTimeVec(time_3, time_2, time_1, 1.f);
    
    glm::vec3 endPosition = mPositionCubic * cubicTimeVec +
            mPositionQuintic * quinticTimeVec;
    glm::vec3 endVelocity = mVelocityCubic * cubicTimeVec +
            mVelocityQuintic * quinticTimeVec;
