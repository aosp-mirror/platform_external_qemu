...
void InertialModel::setTargetRotation(
    ...
    if (mode == PHYSICAL_INTERPOLATION_STEP) {
        ...
        mInitialRotation = getRotation(PARAMETER_VALUE_CURRENT);
    }
    ...
    mRotationTimeSeconds = kTargetRotationTime < minTime ?
            minTime : kTargetRotationTime;
    mRotationChangeEndTime = mRotationChangeStartTime +
            secondsToNs(mRotationTimeSeconds);

    assert(!isnan(mRotationAngleRadians));
    assert(!isnan(mRotationAxis.x));
    assert(!isnan(mRotationAxis.y));
    assert(!isnan(mRotationAxis.z));
    assert(!isnan(mRotationTimeSeconds));
    assert(!isnan(mRotationChangeEndTime));
}
...
glm::quat InertialModel::getRotation(ParameterValue parameterValue) const {
    ...
    if (parameterValue == PARAMETER_VALUE_CURRENT &&
            mModelTimeNs < mRotationChangeEndTime) {
