...
constexpr float kTargetStateChangeTime = 0.5f;
...
void InertialModel::setTargetPosition(
    ...
    if (mode == PHYSICAL_INTERPOLATION_STEP) {
        ...
        mAccelPhasePositionTransform = glm::mat4x3(
                glm::vec3(0.f), glm::vec3(0.f), glm::vec3(0.f), position);
        mAccelPhasePositionQuarticScale = glm::vec3(0.f);
        ...
        mStateChangeEndTime = mModelTimeNs + static_cast<uint64_t>(kTargetStateChangeTime * 1000000000.0);
    } else {
        ...
        glm::vec3 x_init = getPosition();
        glm::vec3 v_init = getVelocity();
        glm::vec3 a_init = getAcceleration();
        glm::vec3 x_target = position;

        // constexprs for powers of the state change time which are used
        // repeated.
        constexpr float time_1 = kTargetStateChangeTime;
        ...
        // kTargetStateChangeTime spent in the first phase (here called
        ...
        glm::vec3 accelQuarticTerm = (1.f / (12.f * time_4)) * (
                11.f * time_2 * a_init +
                54.f * time_1 * v_init +
                96.f * x_init +
                -96.f * x_target);
        glm::vec3 accelCubicTerm = (1.f / (3.f * time_3)) * (
                -4.f * time_2 * a_init +
                -15.f * time_1 * v_init +
                -24.f * x_init +
                24.f * x_target);
        glm::vec3 accelQuadraticTerm = (1.f / 2.f) * a_init;
        glm::vec3 accelLinearTerm = v_init;
        glm::vec3 accelConstantTerm = x_init;

        glm::vec3 decelQuarticTerm = (1.f / (12.f * time_4)) * (
                -5.f * time_2 * a_init +
                -42.f * time_1 * v_init +
                -96.f * x_init +
                96.f * x_target);
        glm::vec3 decelCubicTerm = (1.f / (3.f * time_3)) * (
                4.f * time_2 * a_init +
                33.f * time_1 * v_init +
                72.f * x_init +
                -72.f * x_target);
        glm::vec3 decelQuadraticTerm = (-3.f / (2.f * time_2)) * (
                time_2 * a_init +
                8.f * time_1 * v_init +
                16.f * x_init +
                -16.f * x_target);
        glm::vec3 decelLinearTerm = (1.f / (3.f * time_1)) * (
                2.f * time_2 * a_init +
                15.f * time_1 * v_init +
                24.f * x_init +
                -24.f * x_target);
        glm::vec3 decelConstantTerm = (1.f / 12.f) * (
                -1.f * time_2 * a_init +
                -6.f * time_1 * v_init +
                12.f * x_target);
...
glm::vec3 InertialModel::calculateInertialState(
    ...
    float time_1 = static_cast<float>(
            ((mModelTimeNs < mStateChangeEndTime ?
                    mModelTimeNs : mStateChangeEndTime) -
            mStateChangeStartTime) / 1000000000.0);
    float time_2 = time_1 * time_1;
    float time_3 = time_2 * time_1;
    float time_4 = time_3 * time_1;
