...
TEST(InertialModel, ManyTargets) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        inertialModel.setTargetPosition(((i % 1) == 0) ? secondPosition : firstPosition, PHYSICAL_INTERPOLATION_SMOOTH);
    }
...
TEST(InertialModel, ManyTargetVelocities) {
    ...
    for (int i = 0; i < 990; i++) {
        inertialModel.setCurrentTime(i * 50000000UL);
        inertialModel.setTargetVelocity(((i % 1) == 0) ? secondVelocity : firstVelocity, PHYSICAL_INTERPOLATION_SMOOTH);
    }
