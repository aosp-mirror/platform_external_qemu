...
public:
    FeaturePatternQueryThread() :
        mThread([this]() { queryFeaturePatternFn(); }) {
        mThread.start();
    }
...
static void doFeatureAction(const FeatureAction& action) {
    ...
    setEnabledOverride(feature, action.enable);
...
// 24 hour download interval
static const uint32_t kFeaturePatternDownloadIntervalSeconds = 86400;

static void queryFeaturePatternFn() {

    uint32_t currTime = System::get()->getUnixTime();

    if (!sCachedFeaturePatterns->has_last_download_time() ||
        currTime - sCachedFeaturePatterns->last_download_time() <
