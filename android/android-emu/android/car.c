...
static void _hwCarClient_recv(void* opaque,
                                 ...
                                 int msglen,
                                 QemudClient* client) {
     D(" Yao: %s: '%.*s'", __FUNCTION__, msglen, msg);

     if (msglen == 5 && !memcmp(msg, "Hello", 5)) {
         const char* msg_back = "Hello back";
         qemud_client_send(client, (const uint8_t*)msg_back, strlen(msg_back));
     }
}
...
void android_car_init(void) {
  HwCar* car = _car;
  if (car->service == NULL) {
    qemud_service_register("car", 0, car, _hwCar_connect,
...
void android_send_car_data(const char* msg)
{
   ...
   D("Yao: android_send_car_data %s", msg);
