...
VirtualSceneCamera::VirtualSceneCamera() {
    ...
    mPositionParameterId =
            android_physical_model_get_parameter_id_from_name("position");
    mRotationParameterId =
            android_physical_model_get_parameter_id_from_name("rotation");

    const glm::mat4 openGlFromSensors = poseToOpenGl(
            glm::quat(0.70710678118654757f, 0.0f, 0.0f, 0.70710678118654746),
            glm::vec3());
...
void VirtualSceneCamera::update() {
    glm::vec3 position;
    android_physical_model_get(mPositionParameterId, &position.x, &position.y,
                               &position.z);

    glm::vec3 rotationEuler;
    android_physical_model_get(mRotationParameterId, &rotationEuler.x,
