...
std::unique_ptr<PosterSceneObject> PosterSceneObject::create(
    ...
    renderer.getTextureInfo(texture, &result.get()->mWidth,
                            &result.get()->mHeight);

    return result;
}

void PosterSceneObject::setPosterLocation(const glm::vec3& position,
    ...
    mBaseTransform = glm::translate(glm::mat4(), position) *
    ...
    mMinScale = kMinimumSizeMeters / posterSize;
    if (mMinScale > 1.0f) {
