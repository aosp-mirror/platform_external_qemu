...
void Scene::unregisterSceneObjects(Renderer& renderer) {
    for (auto& sceneObject : mSceneObjects) {
        renderer.unregisterSceneObject(sceneObject.get());
    }
...
int64_t Scene::update(std::vector<RenderableObject>& renderables) {
    mCamera.update();
