...
bool Scene::initialize() {
    ...
    for (auto& poster : mPosters) {
        if (!poster.second.default_filename.empty()) {
            loadPoster(poster.first.c_str(),
                       poster.second.default_filename.c_str());
        }
    }
...
std::vector<RenderableObject> Scene::getRenderableObjects() const {
    ...
    for (auto& sceneObject : mSceneObjects) {
        createRenderableObjects(viewProjection, sceneObject, renderables);
    }

    for (auto& poster : mPosters) {
        if (poster.second.sceneObject) {
            createRenderableObjects(viewProjection, poster.second.sceneObject,
                                    renderables);
        }
    }
...
bool Scene::loadPoster(const char* posterName, const char* filename) {
    ...
    const glm::mat4 rotation = glm::eulerAngleXYZ(
            glm::radians(poster.rotation.x), glm::radians(poster.rotation.y),
            glm::radians(poster.rotation.z));
    newSceneObject->setTransform(
            glm::translate(glm::mat4(), poster.position) * rotation *
            glm::scale(glm::mat4(),
...
bool Scene::loadPostersFile(const char* filename) {
    ...
    for (in >> str; !in.eof(); in >> str) {
        ...
        if (str == "poster") {
            ...
            in >> poster.rotation.x >> poster.rotation.y >> poster.rotation.z;
            if (!in) {
...
void Scene::createRenderableObjects(
