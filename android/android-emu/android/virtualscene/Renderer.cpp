...
public:
    ...
    // Renderer public API.
    void unregisterSceneObject(const SceneObject* sceneObject) override;
...
RendererImpl::RendererImpl(const GLESv2Dispatch* gles2, int width, int height)
    ...
    mRenderTargets[0] = RenderTarget::createTextureTarget(
            mGles2,
            createEmptyTexture(width * kSuperSampleMultiple,
                               height * kSuperSampleMultiple),
...
bool RendererImpl::initialize() {
    Material fxaaEffect =
            createMaterialScreenSpace(nullptr, kFxaaFragmentShader);
    if (!fxaaEffect.isValid()) {
        W("%s: Failed creating fxaa effect.", __FUNCTION__);
        return false;
    }
    mEffectsChain.push_back(std::move(fxaaEffect));

    Material blitEffect =
            createMaterialScreenSpace(nullptr, kBlitFragmentShader);
    if (!blitEffect.isValid()) {
        W("%s: Failed creating blit effect.", __FUNCTION__);
        return false;
    }
    mEffectsChain.push_back(std::move(blitEffect));

    mEffectsMesh =
            Renderer::createMesh(nullptr, kScreenQuadVerts, kScreenQuadIndices);
    if (!mEffectsMesh.isValid()) {
        W("%s: Failed effects mesh.", __FUNCTION__);
        return false;
    }
...
void RendererImpl::unregisterSceneObject(const SceneObject* sceneObject) {
    auto objectDataIt = mObjectData.find(sceneObject);

    if (objectDataIt == mObjectData.end()) {
        E("%s: Scene object 0x08%" PRIXPTR " not registered.", __FUNCTION__,
          sceneObject);
        return;
    }
