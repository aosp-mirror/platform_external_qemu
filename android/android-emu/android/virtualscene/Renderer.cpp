...
#define MOVING_SQUARE_VELOCITY vec2(6.5/5.0,9.0/5.0) // Velocity of the moving
                                                     // square expressed in
...
private:
    ...
    Material mMaterialCheckerboard;
};
...
RendererImpl::~RendererImpl() {
    ...
    if (mMaterialCheckerboard.isValid()) {
        releaseMaterial(mMaterialCheckerboard);
    }
...
void RendererImpl::render(const std::vector<RenderableObject>& renderables,
    ...
    for (size_t i = 0; i < mEffectsChain.size(); i++) {
        ...
        processRenderable(renderable, [&, time](const MaterialData& material) {
