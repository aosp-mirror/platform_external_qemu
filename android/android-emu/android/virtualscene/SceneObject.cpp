...
SceneObject::SceneObject(std::vector<RenderCommand>&& renderCommands)
    : mRenderCommands(std::move(renderCommands)) {}
...
std::unique_ptr<SceneObject> SceneObject::loadFromObj(Renderer& renderer,
    ...
    auto getOrLoadTexture = [&textures, &renderer](std::string filename) {
        auto it = textures.find(filename);
        if (it == textures.end()) {
            std::shared_ptr<Texture> texture = Texture::load(
                    renderer.getGLESv2Dispatch(), filename.c_str());
    ...
    std::shared_ptr<Material> material = renderer.createMaterialTextured();
    ...
    for (const tinyobj::shape_t& shape : shapes) {
        ...
        command.mesh = renderer.createMesh(vertices, indices);
        command.texture = texture;

        renderCommands.emplace_back(std::move(command));
    }

    std::unique_ptr<SceneObject> result(
            new SceneObject(std::move(renderCommands)));
    return result;
}
...
const std::vector<RenderCommand>& SceneObject::getRenderCommands() const {
