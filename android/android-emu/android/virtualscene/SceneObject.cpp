...
std::unique_ptr<SceneObject> SceneObject::loadFromObj(Renderer& renderer,
    ...
    D("%s: Creating scene object with %d vertices", __FUNCTION__,
      vertices.size());

    std::unique_ptr<SceneObject> result(
            new SceneObject(renderer, vertexBuffer, vertices.size()));
...
void SceneObject::render() {
    ...
    gles2->glDrawArrays(GL_TRIANGLES, 0, mVertexCount);
    gles2->glBindBuffer(GL_ARRAY_BUFFER, 0);
