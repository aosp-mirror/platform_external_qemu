...
public:
    ...
    // Before teardown, uninitialize all SceneObjects so that Renderer resources
    // are cleaned up.
    void unregisterSceneObjects(Renderer& renderer);

    // Get the scene camera.
    const SceneCamera& getCamera() const;

    // Update the scene for the next frame and creates the list of renderables
    // to pass to the Renderer.
    //
    // Returns the timestamp for the frame.
    int64_t update(std::vector<RenderableObject>& renderables);
