...
void VideoInjectionController::trySendAsyncResponse(uint32_t async_id,
    ...
    if (sInstance) {
        if(result.ok()){
            sInstance->sendFollowUpAsyncResponse(async_id, Ok(), isCompleted);
        }
        else if (result.err()) {
            sInstance->sendFollowUpAsyncResponse(async_id, Err(result.unwrapErr()), isCompleted);
        }
        else {
            LOG(ERROR) << "Illegal result status, it should be ok or error.";
        }
    }
    else{
...
void VideoInjectionControllerImpl::sendFollowUpAsyncResponse(
    ...
    if (itr == mAsyncRequestContextMap.end()) {
        ...
        if (result.ok()) {
            ...
            asyncResponse = createAsyncResponse(
                    itr->second.asyncId, itr->second.request.sequence_id(),
                    Err(result.unwrapErr()), isCompleted);
        }
