...
{
    struct tm local = *localtime(time_p);
    struct tm utc = *gmtime(time_p);
    static const int kSecondsPerDay = 60 * 60 * 24;
    long utcInSecs = utc.tm_sec + 60 * (utc.tm_min + 60 * utc.tm_hour);
    long localInSecs = local.tm_sec + 60 * (local.tm_min + 60 * local.tm_hour);
    if (utc.tm_year > local.tm_year) {
        utcInSecs += kSecondsPerDay;
    } else if (utc.tm_year < local.tm_year) {
        localInSecs += kSecondsPerDay;
    } else {
        if (utc.tm_mon > local.tm_mon) {
            utcInSecs += kSecondsPerDay;
        } else if (utc.tm_mon < local.tm_mon) {
            localInSecs += kSecondsPerDay;
        } else {
            utcInSecs += kSecondsPerDay * utc.tm_mday;
            localInSecs += kSecondsPerDay * local.tm_mday;
        }
    }
    return utcInSecs - localInSecs;
}
