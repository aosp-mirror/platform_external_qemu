...
// WARNING WARNING!
// Windows does not have the same security model as posix for the
// posix compliant functions. Here we just translate down from what
// user level security is desired, and we drop group/world
static int win_mode_convert(mode_t mode) {
   return mode & (_S_IREAD | _S_IWRITE);
}
