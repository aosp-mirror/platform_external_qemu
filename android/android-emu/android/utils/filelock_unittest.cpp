...
#define GTEST_TEST_WITH_NAME(test_case_name, test_name, parent_class, parent_id)\
    ...
    public:
        ...
        void SetUp() {
            ...
            if (sIsParentProcess) {
                ...
                char cmdBuffer[MAX_PATH + 100];
                ASSERT_GT(sizeof(cmdBuffer), snprintf(cmdBuffer, MAX_PATH,
                        "%s --gtest_filter=%s.%s --child --read=%p --write=%p",
                ...
                si.cb = sizeof(si);
                //si.hStdOutput = mChildWrite;
                //si.hStdInput = mChildRead;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (!strncmp("--read", argv[i], strlen("--read"))) {
            sscanf(argv[i], "--read=%p", &sChildRead);
        } else if (!strncmp("--write", argv[i], strlen("--write"))) {
            sscanf(argv[i], "--write=%p", &sChildWrite);
        }
    }
#endif
