...
mapfile_open(const char* path, int oflag, int share_mode)
{
    ...
    } else if ((oflag & (O_RDONLY | O_WRONLY | O_RDWR)) == O_RDONLY) {
            ...
            size_t* mapped_size)
{
  ...
  /* Get the mask for mapping offset alignment. */
    ...
    /* Map the section. */
#ifdef _WIN32
    // TODO: no PROT_* in windows. Looks like were not even using this. I'll
    // just define it here from gcc. Hopefully it won't break anything.
#ifndef PROT_READ
