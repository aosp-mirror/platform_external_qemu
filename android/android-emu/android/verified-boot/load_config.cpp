...
static const char kConfigFilename[] = "VerifiedBootConfig.textproto";
...
public:
    ...
    void AddError(int line,
        ...
        if (mFirstError) {
            mFirstError = false;
            LOG(WARNING) << "Could not parse " << kConfigFilename << ": ";
        }
...
//     identify the paramter number (zero indexed).
static Status checkParam(const std::string& param,
                         const char* prefix,
                         int param_idx) {
    ++param_idx;  // Make the index 1-based
    if (param.empty()) {
        LOG(WARNING) << kConfigFilename << " " << prefix << " parameter "
                     << param_idx << " is empty.";
...
static Status checkVersion(const VerifiedBootConfig& config) {
    if (!config.has_major_version()) {
        LOG(WARNING) << kConfigFilename
                     << " is missing a required key: major_version";
        return Status::MissingVersion;
    }

    if (config.major_version() > kMaxSupportedMajorVersion) {
        LOG(WARNING) << kConfigFilename
                     << " version is too large to be supported. "
...
static Status buildDMParam(const VerifiedBootConfig& config,
    ...
    if (config.dm_param_size() < 1) {
        LOG(WARNING) << kConfigFilename
                     << " does not contain any dm_param options";
...
//
// If any paramater fails it's check, logs the error and
...
//
// Returns true if successful.  If there was a problem, |params|
...
// Public interface, see header for docs.
Status getParametersFromFile(std::vector<std::string>* params) {
    const std::string config_path = ::android::base::PathUtils::join(
            ::android::ConfigDirs::getUserDirectory(), kConfigFilename);

    const int fd = open(config_path.c_str(), O_RDONLY);
    if (fd < 0) {
        LOG(INFO) << "Could not open " << kConfigFilename << ": "
                  << strerror(errno);
