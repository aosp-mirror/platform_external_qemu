...
FoldableState* android_foldable_initialize(const struct FoldableConfig* config) {
    ...
    android_sensors_override_set(ANDROID_SENSOR_HINGE_ANGLE0,
...
float android_foldable_get_with_1d_parameter() {
    ...
    if (_foldableState->config.numHinges < 1)
        return 0.0;
    return (_foldableState->currentHingeDegrees[0] - _foldableState->config.hingeParams[0].minDegrees) /
           (_foldableState->config.hingeParams[0].maxDegrees - _foldableState->config.hingeParams[0].minDegrees);
}
