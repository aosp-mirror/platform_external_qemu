...
static void myTask(int * outResult) {
    *outResult = 0;
    fprintf(stderr, "going to sleep %d seconds\n", s_SleepTime);
    android::base::System::get()->sleepMs(s_SleepTime * 1000);
    *outResult = 1;
}

static void myTaskDone(const int& outResult) {
    //quite emulator if we are not dead yet
    fprintf(stderr, "too bad, timeout, quit\n");
    qemu_system_shutdown_request();
}

bool android_test_start_boot_complete_timer(int time_out_seconds) {
    s_SleepTime = time_out_seconds;
    return android::base::runParallelTask<int>(android::base::ThreadLooper::get(), &myTask, &myTaskDone); 
}
