...
static constexpr uint8_t REPLAY_PAUSE = 3;
...
void SensorSessionPlayback::reset() {
    D("reset elapse time");
    current_elapsed_time_ = 0;
    D("get the first iter");
...
SensorSessionPlayback::SensorSessionStatus SensorSessionPlayback::StartReplay(
    ...
    playThread_ = new android::base::FunctorThread([this]() { playSensor(); });
