...
SensorSessionPlayback::SensorSessionStatus SensorSessionPlayback::LoadFrom(std::string filename) {
    ...
    D("metadata %d", session_.session_metadata().subscribed_sensors_size());
    // D(session_.session_metadata().DebugString().c_str());
    D("first record %" PRIu64, current_record_iterator_->timestamp_ns());
    D("Frist record event counts %d", current_record_iterator_->sensor_events_size());
    D("Frist record property counts %d", current_record_iterator_->car_property_values_size());
    D("Frist record, %s", current_record_iterator_->DebugString().c_str());
    for (auto iter = current_record_iterator_; iter != session_.sensor_records().end();
            ++iter) {
        if (iter->car_property_values_size() > 0)
        {
            D("print record, %s", iter->DebugString().c_str());
        }
    }
...
SensorSessionPlayback::SensorSessionStatus SensorSessionPlayback::StartReplay(float multiplier) {
    D("start play with multiplier %f", multiplier);
    if (multiplier < 0)
...
void SensorSessionPlayback::playSensor() {
    D("start runing pthread playsensor");

    float multiplier;
    int replay_state;
    carVhalReplayMultiMsg_.tryReceive(&multiplier);
    carVhalReplayMsg_.tryReceive(&replay_state);
    D("get the multipier %f", multiplier);
    D ("replaystate %d", replay_state);
    ...
    while (current_record_iterator_ != session_.sensor_records().end() && replay_state == REPLAY_START) {
        current_elapsed_time_ = (getUnixTimeNs() - current_start_time) * multiplier;
        ...
        while ((current_record_iterator_ != session_.sensor_records().end()) &&
                current_elapsed_time_ > current_record_iterator_->timestamp_ns()) {
            ...
            // D ("print sensor record count %d, time stamp %" PRIu64, sensorrecordcount++, current_record_iterator_->timestamp_ns());
            // Advance to the next record
            current_record_iterator_++;
        }
        // Sleep until the next record is ready.
        if (current_record_iterator_ != session_.sensor_records().end()) {
            carVhalReplayCV_.timedWait(&carVhalReplaytLock_, (current_start_time + current_record_iterator_->timestamp_ns() - getUnixTimeNs()) / 1000);
        }

        carVhalReplayMsg_.tryReceive(&replay_state);
    }
    carVhalReplayMsg_.trySend(REPLAY_STOP);
    D("Stop play %" PRIu64, current_elapsed_time_);
}
