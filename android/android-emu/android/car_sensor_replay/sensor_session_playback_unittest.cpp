...
static void checkLocationsRecord(emulator::SensorSession::SensorRecord record) {
    ...
    ASSERT_EQ(record.locations_size(), 1);
    ASSERT_EQ(record.locations(0).accuracy_mm(), 39599);
    ASSERT_EQ(record.locations(0).altitude_m(), -22.799999237060547);
    ASSERT_EQ(record.locations(0).vertical_accuracy_meters(), 2);
    ASSERT_EQ(record.locations(0).point().lat_e7(), 374179327);
    ASSERT_EQ(record.locations(0).point().lng_e7(), 3074104962);
}

TEST_F(SensorSessionPlaybackTest, TestLoadProto) {
    // The duration of the test proto is 3331143561ns
    ASSERT_EQ(sensorSessionPlayback->session_duration(), 0xC68D3789);
    // There are 685 events in the test proto
    ASSERT_EQ(sensorSessionPlayback->event_count(), 685);
}
...
TEST_F(SensorSessionPlaybackTest, TestSeekToTime) {
    ...
    sensorSessionPlayback->SeekToTime(0x40C60B3D);
    emulator::SensorSession::SensorRecord record =
