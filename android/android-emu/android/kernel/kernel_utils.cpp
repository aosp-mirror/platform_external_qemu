...
bool parseKernelVersion(const char* const kernelBits, size_t size,
                        KernelVersion* kernelVersion) {
    constexpr unsigned kOffest1 = 526;
    constexpr unsigned kOffest2 = 0x200;
    ...
    const unsigned versionOffset = read16le(kernelBits + 526) + kOffest2;
...
bool android_getKernelVersion(const char* kernelPath,
    ...
    return parseKernelVersion(static_cast<const char*>(kernelFileMap.get()),
                              size, kernelVersion);
}
