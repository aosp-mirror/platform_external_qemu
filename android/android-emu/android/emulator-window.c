...
bool emulator_window_start_recording(const char* filename) {
    ...
    stralloc_reset(&qRecordingInfo->filename);
    printf("setting recording save path=%s\n", filename);
    stralloc_add_str(&qRecordingInfo->filename, filename);
...
static void _recording_callback_on_gpu_frame(void* context,
    ...
    if (qRecordingInfo->running) {
        if (qRecordingInfo->recorder == NULL) {
            qRecordingInfo->recorder = ffmpeg_create_recorder(
                    stralloc_cstr(&qRecordingInfo->filename));
        ...
        if (!qRecordingInfo->request_stop) {
