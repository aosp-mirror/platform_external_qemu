...
class Registry {
public:
    Registry() : mElementIdx(0) {}

    ~Registry() {}

    int Register(Type& element, int elementIdx = std::numeric_limits<int>::min()) {
        android::base::AutoLock lock(mLock);

        if (elementIdx == std::numeric_limits<int>::min()) {
            while (mElements.find(mElementIdx) != mElements.end()) {
                mElementIdx++;
            }
...
#define SEND_ERROR_AND_RETURN(errorString, errorStatus) \
    ...
    void onMessage(const std::vector<uint8_t>& input) override {
        ...
        const unsigned char* message_buffer = &input[0];
        ...
        switch (request.request_type_case()) {
            ...
            default:
                LOG(ERROR) << "Unexpected message type. Possible darwinn proto "
                              "type mismatch found";
                break;
        }
    }
    ...
    void CreateExecutable(const std::string& executableString) {
        ...
        int executableId = mExecutableReferences.Register(
                executableReference.ValueOrDie());
        darwinn::Response response;
    ...
    void CreateRequest(int executableId) {
        ...
        int requestId = mRequests.Register(request, request->id());
        darwinn::Response response;
    ...
    void AddInputToRequest(
        ...
        if (bufferSize != str.size()) {
            SEND_ERROR_AND_RETURN("Buffer size in the proto and the size field does not match",
                                  darwinn::Response::RESULT_UNKNOWN_ERROR);
        }
    ...
    void SubmitRequest(
        ...
        std::function<void(int, const platforms::darwinn::util::Status&)>
                callback = [outBuffers, asyncId, this](
                                   int requestId,
                    ...
                    for (const auto& outBuffer : outBuffers) {
                        ...
                        *response.mutable_output()->mutable_output()->add_output_buffer() = buffer;
                    }
                    response.mutable_output()->set_request_id(requestId);
                    ...
                    if (s.ok()) {
                        ...
                        response.set_result(darwinn::Response::RESULT_UNKNOWN_ERROR);
                    }
        ...
        // Submit the request and send a response. If there is no error, we send the 
        // response message with the outputs in the callback
