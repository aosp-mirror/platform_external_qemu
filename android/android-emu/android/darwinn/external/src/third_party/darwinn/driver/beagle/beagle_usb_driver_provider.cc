...
DEFINE_int32(usb_app_mode_product_id, 0x089F,
             "USB product ID in application mode.");
DEFINE_int32(
...
constexpr uint16_t kTargetVendorId = 0x1A6E;
...
std::vector<Device> BeagleUsbDriverProvider::Enumerate() {
  ...
  auto usb_dfu_device_list_or_error =
      usb_device_factory.EnumerateDevices(kTargetVendorId, kTargetDfuProductId);

  auto usb_app_device_list_or_error = usb_device_factory.EnumerateDevices(
      kTargetVendorId, FLAGS_usb_app_mode_product_id);
