...
TEST_F(DeviceBufferMapperTest, MapUnmapAll) {
  ...
  EXPECT_CALL(*address_space_,
              MapMemory(Eq(input["input"][0]), Eq(DmaDirection::kToDevice)))
      .WillOnce(
          Invoke([input_device_address](
                     const Buffer &buffer,
                     DmaDirection direction) -> util::StatusOr<DeviceBuffer> {
            return DeviceBuffer(input_device_address, buffer.size_bytes());
          }));
  EXPECT_CALL(*address_space_,
              MapMemory(Eq(output["output"][0]), Eq(DmaDirection::kFromDevice)))
      .WillOnce(
          Invoke([output_device_address](
                     const Buffer &buffer,
                     DmaDirection direction) -> util::StatusOr<DeviceBuffer> {
            return DeviceBuffer(output_device_address, buffer.size_bytes());
          }));
  EXPECT_CALL(*address_space_,
              MapMemory(Eq(instruction[0]), Eq(DmaDirection::kToDevice)))
      .WillOnce(Invoke([instruction_device_address](const Buffer &buffer,
                                                    DmaDirection direction)
                           -> util::StatusOr<DeviceBuffer> {
...
TEST_F(DeviceBufferMapperTest, MapInputUnmap) {
  ...
  for (int i = 0; i < kBatch; ++i) {
    EXPECT_CALL(*address_space_,
                MapMemory(Eq(input["input"][i]), Eq(DmaDirection::kToDevice)))
        .WillOnce(
            Invoke([&device_addresses, i](
                       const Buffer &buffer,
                       DmaDirection direction) -> util::StatusOr<DeviceBuffer> {
...
TEST_F(DeviceBufferMapperTest, MapOutputUnmap) {
  ...
  for (int i = 0; i < kBatch; ++i) {
    ...
    EXPECT_CALL(*address_space_,
                MapMemory(Eq(output0), Eq(DmaDirection::kFromDevice)))
        .WillOnce(
            Invoke([device_address0](
                       const Buffer &buffer,
                       DmaDirection direction) -> util::StatusOr<DeviceBuffer> {
              return DeviceBuffer(device_address0, buffer.size_bytes());
            }));
    EXPECT_CALL(*address_space_,
                MapMemory(Eq(output1), Eq(DmaDirection::kFromDevice)))
        .WillOnce(
            Invoke([device_address1](
                       const Buffer &buffer,
                       DmaDirection direction) -> util::StatusOr<DeviceBuffer> {
...
TEST_F(DeviceBufferMapperTest, MapInstructionUnmap) {
  ...
  for (int i = 0; i < kNumChunk; ++i) {
    EXPECT_CALL(*address_space_,
                MapMemory(Eq(instructions[i]), Eq(DmaDirection::kToDevice)))
        .WillOnce(
            Invoke([&device_addresses, i](
                       const Buffer &buffer,
                       DmaDirection direction) -> util::StatusOr<DeviceBuffer> {
