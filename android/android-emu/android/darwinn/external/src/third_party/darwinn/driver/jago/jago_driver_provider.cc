...
#include "third_party/darwinn/driver/config/jago/jago_chip_config.h"
...
class JagoDriverProvider : public DriverProvider {
 public:
  static std::unique_ptr<DriverProvider> CreateDriverProvider() {
    return gtl::WrapUnique<DriverProvider>(new JagoDriverProvider());
  }

  ~JagoDriverProvider() override = default;
 ...
 private:
  JagoDriverProvider() = default;
};

REGISTER_DRIVER_PROVIDER(JagoDriverProvider);

std::vector<Device> JagoDriverProvider::Enumerate() {
  return EnumerateSysfs("jago", Chip::kJago, Device::Type::PLATFORM);
}

bool JagoDriverProvider::CanCreate(const Device& device) {
  return device.type == Device::Type::PLATFORM && device.chip == Chip::kJago;
}

util::StatusOr<std::unique_ptr<api::Driver>> JagoDriverProvider::CreateDriver(
  ...
  // Number of wires.
  constexpr int kNumWires = 1;

  auto config = gtl::MakeUnique<config::JagoChipConfig>();
