...
BeaglePciDriverProvider::CreateDriver(const Device& device,
  ...
  auto address_space_factory =
      gtl::MakeUnique<BuddyAddressSpaceFactory>(mmu_mapper.get());
  ...
  return {gtl::MakeUnique<MmioDriver>(
      ...
      std::move(address_space_factory), std::move(allocator),
      std::move(host_queue), std::move(interrupt_handler),
