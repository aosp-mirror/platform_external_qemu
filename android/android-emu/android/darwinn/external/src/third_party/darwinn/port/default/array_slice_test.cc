...
// NOTE(jnjoseph): Enable when supporting InlinedVector.
...
TEST(IntSlice, ConstStaticArrayConversion) {
  IntVec vec;
  Fill(&vec, ARRAYSIZE(test_const_array));
...
TEST(MutableIntSlice, StaticArrayConversion) {
  ...
  static_cast<void>(v);
  TestImplicitConversion(array, array, ARRAYSIZE(array));
}
