...
TEST_F(DmaInfoExtractorTest, SingleChunk) {
...

  {
    ...
    for (const int device_address : device_addresses) {
      EXPECT_CALL(*address_space_, MapMemory(_, _))
          .WillOnce(Invoke([device_address](const Buffer& buffer, DmaDirection)
...
TEST_F(DmaHintExtractorTest, SimpleOffsetByte) {
  ...
  EXPECT_CALL(*address_space_, MapMemory(_, _))
      .WillOnce(Invoke([](const Buffer& buffer,
                          DmaDirection) -> util::StatusOr<DeviceBuffer> {
...
TEST_F(DmaHintExtractorTest, SimpleExecutable) {
      ...
      DeviceBuffer(instruction_device_addresses[0], instruction_bytes[0]));
  {
    ...
    for (const auto& device_buffer : map_sequence) {
      EXPECT_CALL(*address_space_, MapMemory(_, _))
          .WillOnce(Invoke([&device_buffer](const Buffer& buffer, DmaDirection)
                               -> util::StatusOr<DeviceBuffer> {
  ...
  auto* package_reference = dynamic_cast<PackageReference*>(
      const_cast<api::ExecutableReference*>(api_executable_reference));
  ASSERT_NE(package_reference, nullptr);
