 ...
 public:
  TestDriver()
      : Driver(api::Chip::kBeagle,
               gtl::MakeUnique<ExecutableRegistry>(api::Chip::kBeagle)) {}
...
class DriverTest : public ::testing::Test {
 public:
  ...
  MOCK_METHOD2(RequestDone, void(int, util::Status));

  void WrappedDone(int id, util::Status status) {
    RequestDone(id, std::move(status));
  }
};
...
util::StatusOr<std::vector<std::shared_ptr<api::Request>>> CreateRequestVector(
    TestDriver& driver, const api::ExecutableReference* executable_ref,
    int count) {
