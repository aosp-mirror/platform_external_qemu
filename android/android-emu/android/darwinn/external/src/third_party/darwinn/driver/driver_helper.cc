...
util::Status DriverHelper::Submit(std::shared_ptr<api::Request> request,
  ...
  // available when done.
  auto wrapped_done = [this, done_callback](int id, util::Status status) {
...
util::Status DriverHelper::Submit(const TestVector& test_vector) {
  ...
  if (!input_names.empty()) {
    ...
    for (const auto& input_name : input_names) {
      ...
      for (int i = 0; i < executable_ref->BatchSize(); ++i) {
    ...
    CHECK_EQ(input_pos, input_string.size());
  }
  ...
  if (!output_names.empty()) {
    ...
    CHECK_EQ(output_pos, expected_output_string.size());
  }
...
util::Status DriverHelper::Submit(
    const std::string& tag, const api::ExecutableReference* executable_ref,
...
util::Status DriverHelper::Submit(
    const std::string& tag, const api::ExecutableReference* executable_ref,
    const Buffer::NamedMap& input, const Buffer::NamedMap& expected_output,
