#include "third_party/darwinn/driver/jago/jago_top_level_handler.h"
...
JagoTopLevelHandler::JagoTopLevelHandler(const config::ChipConfig& config,
                                         Registers* registers)
...
util::Status JagoTopLevelHandler::QuitReset() {
...
util::Status JagoTopLevelHandler::EnableReset() {
  ...
  RETURN_IF_ERROR(registers_->Write(reset_offsets_.memoryShutdownReg, 0x1fff));
  ASSIGN_OR_RETURN(uint64 dummy,
  ...
  // Enable clamp.
  RETURN_IF_ERROR(registers_->Write(reset_offsets_.clampEnableReg, 0x1fff));

  // Disable tile control for deepsleep, and read the register to force write
  // to commit.
  RETURN_IF_ERROR(registers_->Write(reset_offsets_.tileMemoryRetnReg, 0xfff));
  ...
  // write to commit.
  RETURN_IF_ERROR(
      registers_->Write(reset_offsets_.logicShutdownAllReg, 0x1fff));
  ...
  // write to commit.
  RETURN_IF_ERROR(
      registers_->Write(reset_offsets_.logicShutdownPreReg, 0x1fff));
