...
ExecutableRegistry::RegisterPackage(const Buffer& package_buffer) {
  ...
  auto executable_reference =
      std::unique_ptr<api::ExecutableReference>(package_reference);
  auto api_reference =
      registrations_
...
util::StatusOr<const api::ExecutableReference*>
ExecutableRegistry::RegisterFile(const std::string& executable_filename) {
  std::ifstream ifs;
...
util::Status ExecutableRegistry::Unregister(
    const api::ExecutableReference* executable_reference) {
...
util::StatusOr<int> ExecutableReference::InputIndex(
  ...
  return util::NotFoundError("Layer not found.");
}
...
uint64 ExecutableReference::ParameterCachingToken() const {
  if (!flatbuffers::IsFieldPresent(&executable(),
