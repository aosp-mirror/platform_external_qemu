...
using platforms::darwinn::api::ExecutableReference;
using ::testing::_;
...
class SingleEndpointDmaSchedulerTest : public ::testing::Test {
 protected:
  SingleEndpointDmaSchedulerTest() {
    ...
    ON_CALL(address_space_, MapMemory(_, _))
  ...
  std::shared_ptr<Request> MakeRequest(Request::Done done) {
    auto request = std::make_shared<Request>(
        ...
        GetDeviceBufferMapper(), &extractor_, std::move(done));
...
TEST_F(SingleEndpointDmaSchedulerTest, RequestCompleteWithPendingDmasFail) {
  ...
  EXPECT_OK(scheduler.Submit(MakeRequest([](int, util::Status) {})));
...
TEST_F(SingleEndpointDmaSchedulerTest, SameDmas) {
  ...
  EXPECT_OK(scheduler.Submit(MakeRequest([&done](int, util::Status status) {
    EXPECT_OK(status);
    done = true;
  })));
...
TEST_F(SingleEndpointDmaSchedulerTest, TwoRequests) {
  ...
  for (int i = 0; i < dmas.size(); ++i) {
    EXPECT_OK(
        scheduler.Submit(MakeRequest([&done, i](int, util::Status status) {
          EXPECT_OK(status);
          done[i] = true;
        })));
  }
...
TEST_F(SingleEndpointDmaSchedulerTest, CancelPendingRequests) {
  ...
  EXPECT_OK(
      scheduler.Submit(MakeRequest([&cancelled](int, util::Status status) {
