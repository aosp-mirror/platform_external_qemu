...
// TODO(jnjoseph): Implement the following.
...
class BenchmarkFactory {
 public:
  ...
  T* TrivialFactory() ATTRIBUTE_NOINLINE { return value_; }
  ...
  Status ArgumentFactory(T** result) ATTRIBUTE_NOINLINE {
    *result = value_;
    return Status::OK();
  }

  Status ArgumentFactoryFail(T** result) ATTRIBUTE_NOINLINE {
    *result = NULL;
    return Status(error::CANCELLED, "");
  }

  Status ArgumentFactoryFailShortMsg(T** result) ATTRIBUTE_NOINLINE {
    *result = NULL;
    return Status(error::INTERNAL, "");
  }

  Status ArgumentFactoryFailLongMsg(T** result) ATTRIBUTE_NOINLINE {
    *result = NULL;
    return Status(error::INTERNAL,
  ...
  StatusOr<T*> StatusOrFactory() ATTRIBUTE_NOINLINE {
    return implicit_cast<T*>(value_);
  }

  StatusOr<T*> StatusOrFactoryFail() ATTRIBUTE_NOINLINE {
    return Status(error::CANCELLED, "");
  }

  StatusOr<T*> StatusOrFactoryFailShortMsg() ATTRIBUTE_NOINLINE {
    return Status(error::INTERNAL, "");
  }

  StatusOr<T*> StatusOrFactoryFailLongMsg() ATTRIBUTE_NOINLINE {
    return Status(error::INTERNAL,
                  "a big string of message junk that will never be read");
  }
 ...
 private:
  T* volatile value_;
  DISALLOW_COPY_AND_ASSIGN(BenchmarkFactory);
};
...
class BenchmarkType {
 public:
  ...
  virtual void DoWork() ATTRIBUTE_NOINLINE {}

 private:
  DISALLOW_COPY_AND_ASSIGN(BenchmarkType);
};
