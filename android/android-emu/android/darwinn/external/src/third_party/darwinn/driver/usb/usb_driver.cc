...
#include "third_party/darwinn/driver/usb/usb_dfu_util.h"
#include "third_party/darwinn/driver/usb/usb_latest_firmware.h"
...
// TODO(macwang): revisit this setting after we finalize PHY tuning.
...
UsbDriver::UsbDriver(
    ...
    std::unique_ptr<ExecutableRegistry> executable_registry,
    const UsbDriverOptions& options)
    : UsbDriver(std::move(chip_config), std::move(registers),
...
// TODO(b/63907676): review the sequence with hardware team and convert them to
// use named constants.
...
util::Status UsbDriver::RegisterAndEnableAllInterrupts() {
  // TODO(jasonjk): Register interrupts to interrupt EP.
...
void UsbDriver::HandleEvent(const util::Status& status,
  ...
  if (status.ok()) {
    // TODO(macwang): analyze if there is any failure case we can recover from.
...
// TODO(b/65263529): breaks up this function according to functionality.
bool UsbDriver::ProcessIo() {
  ...
  while (auto* dma_info = dma_scheduler_.GetNextDma()) {
  ...
  for (auto& io_request : io_requests_) {
    ...
    if (io_type == UsbIoRequest::Type::kBulkOut) {
      ...
      if (options_.mode == OperatingMode::kMultipleEndpointsSoftwareQuery) {
        // TODO(macwang): add some mechansim to slowly poll for available
        // credits.
        ...
        if (credits <= options_.software_credits_lower_limit_in_bytes) {
          ...
          // the limit.
          // TODO(macwang): allow a different endpoint to proceed.
          break;
        }
        ...
        if (status.ok()) {
          ...
          // TODO(macwang): terminate the task early, as there is no
        ...
        util::Status async_request_status = usb_device_->AsyncBulkOutTransfer(
            ...
            [this, &io_request, tag, transfer_size](util::Status status) {
              ...
              callback_queue_.push([this, &io_request, tag, status,
                                    transfer_size] {
      ...
      if (!options_.usb_enable_overlapping_bulk_in_and_out &&
        ...
        util::Status async_request_status = usb_device_->AsyncBulkInTransfer(
            ...
            [this, &io_request, tag, transfer_size](
              ...
              // invalidated when the async transfer is cancelled.
              StdMutexLock queue_lock(&callback_mutex_);
              callback_queue_.push([this, &io_request, status,
                                    num_bytes_transferred, tag, transfer_size] {
                // Starting from here is an functor which would be executed
                ...
                if (status.ok()) {
                  ...
                  // invalidated when the async transfer is cancelled.
                  // TODO(macwang): fail the task and allow reset of the
                  // chip.
                  LOG(FATAL)
...
void UsbDriver::HandleQueuedBulkIn(const util::Status& status, int buffer_index,
  ...
  if (status.ok()) {
    ...
    if (!IsCancelled(status) && !IsDeadlineExceeded(status)) {
      // TODO(macwang): convert to driver error.
    ...
    if (state_ == kClosing) {
      ...
      if (options_.usb_enable_queued_bulk_in_requests) {
        while (!available_bulk_in_buffers_.empty()) {
          ...
          if (!async_request_status.ok()) {
            // TODO(macwang): convert to some driver error.
            LOG(FATAL) << "Bulk-in failed. Abort";
          }
        }
      }

      reevaluation_needed = ProcessIo();

      // TODO(macwang): Enter kPaused state when dma_scheduler_.IsEmpty(). Any
...
UsbDriver::CreateRawUsbDeviceWithRetry() {
  ...
  for (int i = 0; i < kMaxNumOfRetryAfterReset; ++i) {
    ...
    // back-to-back tests.
    // TODO(macwang): revisit this after the connection issue has been resolved.
    usleep(kSleepTimeMicroSecondsBeforeRetry);
...
util::Status UsbDriver::DoClose(bool in_error) {
  {
    StdMutexLock state_lock(&mutex_);
  ...
  RETURN_IF_ERROR(dma_scheduler_.Close());
...
util::Status UsbDriver::DoCancelAndWaitRequests(bool in_error) {
  ...
  // The USB driver controls the data transfer and only needs to stop the dma
  // scheduler but doesn't have to wait for requests, which is needed in PCIe.
...
util::StatusOr<std::shared_ptr<api::Request>> UsbDriver::DoCreateRequest(
    const ExecutableReference* executable_ref) {
  StdMutexLock lock(&mutex_);
  RETURN_IF_ERROR(ValidateStates({kOpen}));

  // TODO(macwang): find a way to mix models, switching on and off descriptors
  // on the fly.
  if (!options_.usb_enable_bulk_descriptors_from_device) {
  ...
  return {std::make_shared<Request>(
      next_id_++, executable_ref, allocator_.get(),
      gtl::MakeUnique<DeviceBufferMapper>(&address_space_),
      &dma_info_extractor_,
      chip_config_->GetChipStructures().minimum_alignment_bytes)};
}

util::Status UsbDriver::DoSubmit(std::shared_ptr<api::Request> request_in,
...
const ExecutableReference* UsbDriver::DoGetExecutableReference(
...
void UsbDriver::CheckFatalError(const util::Status& status) {
  // TODO(wgulland, jnjoseph): Forward to the client application for handling.
