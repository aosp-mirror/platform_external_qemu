...
MmioDriver::MmioDriver(
    ...
    std::unique_ptr<AddressSpaceFactory> address_space_factory,
    ...
    : Driver(
      ...
      address_space_factory_(std::move(address_space_factory)),
...
util::Status MmioDriver::DoOpen(bool debug_mode) {
  ...
  if (chip_structure_.num_page_table_entries == 0) {
    // With no MMU, there is no extended address space.
    const uint64 address_space_size =
        1ULL << chip_structure_.physical_address_bits;
    simple_address_space_ =
        address_space_factory_->Create(0, address_space_size);
  } else {
    simple_address_space_ =
        address_space_factory_->Create(0, kHostPageSize * num_simple_entries);

    extended_address_space_ = address_space_factory_->Create(
        kExtendedAddressSpaceStart,
        kExtendedPageTableEntryAddressableBytes *
            GetNumExtendedPageTableEntries(
                chip_structure_.num_page_table_entries));
  }
  ...
  RETURN_IF_ERROR(instruction_queue_->Open(simple_address_space_.get()));
...
util::Status MmioDriver::DoClose(bool in_error) {
  ...
  simple_address_space_.reset();
  extended_address_space_.reset();
...
util::StatusOr<MappedDeviceBuffer> MmioDriver::DoMapBuffer(
  ...
  // Prefer the extended address space if there is one, but fall back to simple
  // if that's all we have available.
  AddressSpace* address_space = extended_address_space_ != nullptr
                                    ? extended_address_space_.get()
                                    : simple_address_space_.get();

  if (buffer.IsValid()) {
    ASSIGN_OR_RETURN(auto device_buffer,
                     address_space->MapMemory(buffer, direction));
    // TODO (ijsung): this is dangerous: the std::bind captures a raw pointer to
    // the underlying object of one of the two unique_ptr'd address spaces.
    // This will break if executable registry outlives address space in the
    // driver. A better way is to at least use share_ptr for address
    // spaces, and here let the std::bind capture a weak_ptr.
    return MappedDeviceBuffer(
        device_buffer, std::bind(&AddressSpace::UnmapMemory, address_space,
                                 std::placeholders::_1));
  }
  return MappedDeviceBuffer();
}
...
util::StatusOr<std::shared_ptr<api::Request>> MmioDriver::DoCreateRequest(
  ...
  return {std::make_shared<Request>(
      ...
      gtl::MakeUnique<DeviceBufferMapper>(address_space),
      &dma_info_extractor_)};
}
