...
// TODO(macwang) use status update to record the first failure.
  ...
  return AutoRetryLibUsbCommand(
      [=] {
        int result = libusb_control_transfer(
            ...
            command.value, command.index, nullptr, 0, timout_msec);
...
util::Status LocalUsbDevice::SendControlCommandWithDataOut(
    const SetupPacket& command, ConstBuffer data_out, TimeoutMillis timout_msec,
    const char* context) {
  VLOG(10) << __func__;
  StdMutexLock lock(&mutex_);
  RETURN_IF_ERROR(CheckForNullHandle(__func__));
...
util::Status LocalUsbDevice::SendControlCommandWithDataIn(
    ...
    size_t* num_bytes_transferred, TimeoutMillis timout_msec,
    const char* context) {
  VLOG(10) << __func__;
  StdMutexLock lock(&mutex_);
  RETURN_IF_ERROR(CheckForNullHandle(__func__));
...
util::Status LocalUsbDevice::BulkOutTransfer(uint8_t endpoint,
  ...
  const int result =
      libusb_bulk_transfer(libusb_handle_, endpoint | LIBUSB_ENDPOINT_OUT,
                           const_cast<uint8_t*>(data_out.data()),
                           data_out.length(), &amount_transferred, timout_msec);
...
util::Status LocalUsbDevice::BulkInTransfer(uint8_t endpoint,
  ...
  *num_bytes_transferred = 0;
  ...
  const int result = libusb_bulk_transfer(
      ...
      data_in.length(), &amount_transferred, timout_msec);
...
util::Status LocalUsbDevice::AsyncBulkOutTransfer(uint8_t endpoint,
  ...
  libusb_fill_bulk_transfer(
      ...
      LibUsbDataOutCallback, callback_obj, timout_msec);
...
util::Status LocalUsbDevice::AsyncInterruptInTransfer(uint8_t endpoint,
                                                      MutableBuffer data_in,
                                                      TimeoutMillis timout_msec,
                                                      DataInDone callback,
                                                      const char* context) {
...
LocalUsbDeviceFactory::EnumerateDevices(uint16_t vendor_id,
  ...
  auto context_cleaner = gtl::MakeCleanup([context] { libusb_exit(context); });
  ...
  auto device_list_cleaner = gtl::MakeCleanup([device_list] {
