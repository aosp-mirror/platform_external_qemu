  ...
  for (auto& region : mmap_region_) {
    if (region.registers != nullptr) {
      VLOG(1) << StringPrintf(
          ...
          device_path_.c_str(),
          region.offset,   // NOLINT(runtime/int)
...
util::StatusOr<uint64*> KernelRegisters::GetMappedOffset(uint64 offset,
                                                         int alignment) const {
  const size_t end_of_region = offset + alignment;

  if (end_of_region < offset) {
    return util::OutOfRangeError(StringPrintf(
        "Offset (0x%016llx) + size_bytes is larger than 64-bit", offset));
  }
  ...
  for (const auto& region : mmap_region_) {
    if ((offset >= region.offset) &&
      ...
      if (region.registers != nullptr) {
        return region.registers + ((offset - region.offset) / alignment);
      } else {
  ...
  return util::OutOfRangeError(
      StringPrintf("Offset (0x%016llx) is not covered by any region", offset));
}
