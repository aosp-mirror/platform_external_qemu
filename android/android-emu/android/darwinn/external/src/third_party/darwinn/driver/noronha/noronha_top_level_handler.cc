#include "third_party/darwinn/driver/noronha/noronha_top_level_handler.h"
...
NoronhaTopLevelHandler::NoronhaTopLevelHandler(const config::ChipConfig& config,
                                               Registers* registers)
...
util::Status NoronhaTopLevelHandler::QuitReset() {
  // Disable in following orders:
  // 1. Disable logic shutdown.
  constexpr int kGroupSize = 1;
  // 17 bits = 16 tiles + scalar core.
  int value = 0x1ffff;
  while (value != 0) {
    // Power up tiles 1 at a time
    value >>= kGroupSize;

    // 1a. Disable first pass switches, and wait until ack matches.
    RETURN_IF_ERROR(
        registers_->Write(reset_offsets_.logicShutdownPreReg, value));

    config::registers::ShutdownReg shutdown_helper(value);
    shutdown_helper.set_logic_shutdown_ack(shutdown_helper.logic_shutdown());
    RETURN_IF_ERROR(registers_->Poll(reset_offsets_.logicShutdownPreReg,
                                     shutdown_helper.raw()));

    // 1b. Disable second pass switches, and wait until ack matches.
    RETURN_IF_ERROR(
        registers_->Write(reset_offsets_.logicShutdownAllReg, value));
    RETURN_IF_ERROR(registers_->Poll(reset_offsets_.logicShutdownAllReg,
                                     shutdown_helper.raw()));
  }
  ...
  while (value != 0) {
    ...
    // 6. Disable memory shutdown (1 tile at a time), and wait for ack.
    RETURN_IF_ERROR(registers_->Write(reset_offsets_.memoryShutdownReg, value));
    RETURN_IF_ERROR(
        registers_->Poll(reset_offsets_.memoryShutdownAckReg, value));

    // 7. Enable tile control for deepsleep.
    RETURN_IF_ERROR(registers_->Write(reset_offsets_.tileMemoryRetnReg, value));
  }
...
util::Status NoronhaTopLevelHandler::EnableReset() {
  ...
  // Enable memory shutdown, and read the register to force write to commit.
  RETURN_IF_ERROR(registers_->Write(reset_offsets_.memoryShutdownReg, 0x1ffff));
  ASSIGN_OR_RETURN(uint64 dummy,
                   registers_->Read(reset_offsets_.memoryShutdownReg));
  (void)dummy;

  // Enable clock, to allow reset to propagate.
  helper.set_clock_enable(1);
  helper.set_idle_override(1);
  RETURN_IF_ERROR(
      registers_->Write(reset_offsets_.clockEnableReg, helper.raw()));
  // Read clock enable to force previous write to commit.
  ASSIGN_OR_RETURN(dummy, registers_->Read(reset_offsets_.clockEnableReg));
  (void)dummy;
  ...
  // Enable clamp.
  RETURN_IF_ERROR(registers_->Write(reset_offsets_.clampEnableReg, 0x1ffff));

  // Disable tile control for deepsleep, and read the register to force write
  // to commit.
  RETURN_IF_ERROR(registers_->Write(reset_offsets_.tileMemoryRetnReg, 0xffff));
  ASSIGN_OR_RETURN(dummy, registers_->Read(reset_offsets_.tileMemoryRetnReg));
  (void)dummy;

  // Enable second pass shutdown switches, and read the register to force
  // write to commit.
  RETURN_IF_ERROR(
      registers_->Write(reset_offsets_.logicShutdownAllReg, 0x1ffff));
  ASSIGN_OR_RETURN(dummy, registers_->Read(reset_offsets_.logicShutdownAllReg));
  (void)dummy;
  // Enable first pass shutdown switches, and read the register to force
  // write to commit.
  RETURN_IF_ERROR(
      registers_->Write(reset_offsets_.logicShutdownPreReg, 0x1ffff));
