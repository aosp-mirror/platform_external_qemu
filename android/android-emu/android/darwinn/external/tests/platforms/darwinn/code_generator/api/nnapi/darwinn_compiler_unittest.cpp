...
#include "android/darwinn/external/proto/platforms/darwinn/model/config/representation.pb.h"
#else
#include "android/darwinn/external/proto/platforms/darwinn/model/config/representation.proto.h"
#endif
...
void CheckPackagesMatch(const DataArray& serialized_package,
  ...
  EXPECT_TRUE(package_verifier.VerifyBuffer<Package>());
  ...
  EXPECT_TRUE(embedded_serialized_expected_package != nullptr);
  ...
  EXPECT_TRUE(expected_package_verifier.VerifyBuffer<Package>());
  ...
  int num_wrong_positions = 0;
  EXPECT_EQ(generated_string.length(), expected_string.length());
  for (int i = 0; i < std::min(expected_string.length(), generated_string.length()); i++) {
    if (expected_string.c_str()[i] != generated_string.c_str()[i]) {
      num_wrong_positions++;
    }
  }

  EXPECT_EQ(num_wrong_positions, 0);
}

TEST(DarwinnTest, TestSerializeAndDeserialize) {
  ...
  EXPECT_TRUE(model.ParseFromArray(embedded_serialized_model->data,
