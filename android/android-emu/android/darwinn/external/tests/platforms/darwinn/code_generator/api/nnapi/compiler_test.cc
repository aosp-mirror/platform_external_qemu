...
#include "platforms/darwinn/code_generator/package_generator.h"
...
using platforms::darwinn::code_generator::PackageGenerator;
...
string CleanupRandomData(const char* serialized_package) {
    const auto* package =
            flatbuffers::GetRoot<Package>(reinterpret_cast<const uint8_t*>(serialized_package));
    const auto* multi_executable =
            flatbuffers::GetRoot<MultiExecutable>(package->serialized_multi_executable()->data());
    PackageGenerator package_generator;

    for (const auto* executable_serialized : *multi_executable->serialized_executables()) {
        ...
        string new_executable_serialized(reinterpret_cast<const char*>(builder.GetBufferPointer()),
                                         builder.GetSize());
        package_generator.AddExecutable(new_executable_serialized);
    }

    CHECK_OK(package_generator.Finish());
    return package_generator.PackageSerialized();
}
...
void CheckPackagesMatch(const DataArray& serialized_package,
  ...
  auto expected_string = CleanupRandomData(embedded_serialized_expected_package->data);
...
int main() {
  ...
  DataArray serialized_parameter_caching_package;
  CompileModel(platforms::darwinn::api::Chip::kBeagle,
               /*enable_parameter_caching=*/true, reserialized_model,
               &serialized_package, &serialized_parameter_caching_package);
  CheckPackagesMatch(serialized_package,
                     "mobilenet_quantized_224-executable.bin");
  CheckPackagesMatch(
      serialized_parameter_caching_package,
      "mobilenet_quantized_224-parameter-caching-executable.bin");

  delete[] reserialized_model.data;
  delete[] serialized_package.data;
  delete[] serialized_parameter_caching_package.data;
  return 0;
}
