...
function build_google3_target {
    ...
    echo $client_dir 
    cd $client_dir
    blaze build $build_target -c opt --stripopt=--strip-unneeded $additional_opts
    popd
}
...
build_google3_target //platforms/darwinn/code_generator/api/nnapi:compiler_test
cp -af $GOOGLE3_BIN/platforms/darwinn/code_generator/api/nnapi/libembedded_compiler_test_vector.a ./
