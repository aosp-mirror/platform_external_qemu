...
class ShutdownReg {
 public:
  // Defaults to reset value.
  ShutdownReg() : ShutdownReg(0x0) { set_logic_shutdown(0x1ffff); }
 ...
 private:
  union {
    uint64 raw_;
    // These are named after fields in the spec.
    platforms::darwinn::driver::Bitfield<0, 17> logic_shutdown_;
    platforms::darwinn::driver::Bitfield<17, 17> logic_shutdown_ack_;
  } reg_;
};
