...
class MockDriver : public api::Driver {
 public:
  ...
  MOCK_METHOD1(
      ...
      util::StatusOr<const api::ExecutableReference*>(const std::string&));
  MOCK_METHOD1(
      RegisterExecutableSerialized,
      util::StatusOr<const api::ExecutableReference*>(const std::string&));
  MOCK_METHOD2(RegisterExecutableSerialized,
               util::StatusOr<const api::ExecutableReference*>(const char*,
                                                               size_t));
  MOCK_METHOD1(UnregisterExecutable,
               util::Status(const api::ExecutableReference*));

  MOCK_METHOD1(Open, util::Status(bool));
  MOCK_METHOD0(Close, util::Status());
  MOCK_CONST_METHOD1(MakeBuffer, Buffer(size_t));
  MOCK_METHOD1(CreateRequest, util::StatusOr<std::shared_ptr<api::Request>>(
                                  const api::ExecutableReference*));
...
std::unique_ptr<MockDriver> MakeDriver() {
  ...
  auto create_request = [](const api::ExecutableReference* reference)
