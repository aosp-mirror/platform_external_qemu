...
class Driver : public api::Driver {
 public:
  ...
  util::StatusOr<std::shared_ptr<api::Request>> CreateRequest(
      const api::ExecutableReference* executable_ref) override;
 ...
 private:
  ...
  util::Status MapParameters(api::ExecutableReference* api_executable_ref)
      EXCLUSIVE_LOCKS_REQUIRED(state_mutex_);
  ...
  std::unique_ptr<ExecutableRegistry> executable_registry_;
