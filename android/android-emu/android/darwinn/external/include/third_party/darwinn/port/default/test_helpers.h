#ifndef THIRD_PARTY_DARWINN_PORT_DEFAULT_TEST_HELPERS_H_
#define THIRD_PARTY_DARWINN_PORT_DEFAULT_TEST_HELPERS_H_
...
std::function<bool(const std::string&)> ContainsRegex(const std::string& regex);
...
template <typename T>
std::vector<T> PBToVec(const RepeatedField<T> rf) {
...
#define EXPECT_IS_OK(expression) \
  EXPECT_EQ(util::Status::OK(), testing::internal_status::GetStatus(expression))
#undef ASSERT_IS_OK
#define ASSERT_IS_OK(expression) \
  ASSERT_EQ(util::Status::OK(), testing::internal_status::GetStatus(expression))
...
#endif  // THIRD_PARTY_DARWINN_PORT_DEFAULT_TEST_HELPERS_H_
