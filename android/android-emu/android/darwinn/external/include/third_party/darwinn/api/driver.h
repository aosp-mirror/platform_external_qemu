...
class Driver {
 public:
  ...
  virtual util::StatusOr<const ExecutableReference*> RegisterExecutableFile(
  ...
  // when driver is either open or closed. In the future registration would only
  // be possible when the driver is in open state.
  virtual util::StatusOr<const ExecutableReference*>
  RegisterExecutableSerialized(const std::string& executable_content) = 0;
  virtual util::StatusOr<const ExecutableReference*>
  RegisterExecutableSerialized(const char* executable_content,
                               size_t length) = 0;
  ...
  virtual util::Status UnregisterExecutable(
      const ExecutableReference* executable_ref) = 0;
