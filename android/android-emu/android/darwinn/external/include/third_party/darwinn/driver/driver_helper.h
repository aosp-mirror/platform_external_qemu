...
class DriverHelper : public api::Driver {
 public:
  ...
  util::StatusOr<const api::ExecutableReference*> RegisterExecutableFile(
      const std::string& executable_filename) final;

  util::StatusOr<const api::ExecutableReference*> RegisterExecutableSerialized(
  ...
  util::Status UnregisterExecutable(
      const api::ExecutableReference* executable_ref) final;
  ...
  util::Status Submit(const TestVector& test_vector) LOCKS_EXCLUDED(mutex_);
