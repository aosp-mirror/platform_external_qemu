    ...
    while (numBuffers > 0) {
        ...
        switch (mState) {
            case State::WaitingForGuestSend: {
                if (buffers->size < 8) {
                    fprintf(stderr,
                        "ERROR: KeymasterPipe: received incomplete command,"
            ...
            default: // this is not expected
                fprintf(stderr, "ERROR: KeymasterPipe: received command"
...
void KeymasterPipe::decodeAndExecute() {
    ...
    switch (cmd) {
        ...
        case VerifyData:
            ...
            break;
        default:
            fprintf(stderr, "ERROR: KeymasterPipe: received bad command %d,"
