...
static int s_genParamLength(int type) {
    switch (type) {
        case TYPE_DSA:
            return sizeof(keymaster_dsa_keygen_params_t);
            break;
        case TYPE_EC:
            return sizeof(keymaster_ec_keygen_params_t);
            break;
        case TYPE_RSA:
            return sizeof(keymaster_rsa_keygen_params_t);
            break;
        default:
...
static int s_signParamLength(int type) {
    switch (type) {
        case TYPE_DSA:
        ...
        case GenerateKeypair:
            {
                ...
                int expectedKeyParamsLen = s_genParamLength(keyType);
                if (!expectedKeyParamsLen == keyParamsLen) {
                    D("Key param size not match: %d, expect %d, actual%d\n",
                            keyType, expectedKeyParamsLen, keyParamsLen);
                }
                void* keyParams = s_readGenParam(data, keyType);
                data += keyParamsLen;
                assert(data == msg + msglen);
                D("%s: data decode done\n", __FUNCTION__);
                // output parameters
                uint8_t* keyBlob;
                uint32_t keyBlobLength;
                int ret = Keymaster::generateKeypair(keyType,
                        keyParams, &keyBlob, &keyBlobLength);
                D("%s: key generation done\n", __FUNCTION__);
        ...
        case SignData:
            {
                ...
                keymaster_keypair_t keyType =
                        (keymaster_keypair_t)unpack<int32_t>(&data);
                uint32_t keyParamsLen = unpack<uint32_t>(&data);
                int expectedKeyParamsLen = s_signParamLength(keyType);
                if (!expectedKeyParamsLen == keyParamsLen) {
                    D("Key param size not match: %d, expect %d, actual%d\n",
                            keyType, expectedKeyParamsLen, keyParamsLen);
                }
