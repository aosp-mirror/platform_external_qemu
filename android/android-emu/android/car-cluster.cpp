...
static void _hwCarClusterClient_recv(void* opaque,
    ...
    if (msglen == 4) {
        ...
        hwCarCluster->callback((char*)msg, hwCarCluster->nextSize);
    }
}
...
void android_car_cluster_init(void) {
    ...
    if (carCluster->service == nullptr) {
        carCluster->service = qemud_service_register("carCluster", 0, carCluster, _hwCarCluster_connect,
                                              _hwCarCluster_save, _hwCarCluster_load);
...
void set_car_cluster_call_back(car_cluster_callback_t func) { //, void* context) {
    HwCarCluster* carCluster = _carCluster;
    if (func == nullptr) {
...
void android_send_car_cluster_data(const char* msg, int msgLen) {
    ...
    if (_carCluster->client == nullptr) {
        D("no carCluster client is connected");
        return;
    }
    T("%s: Sent message length of %d", __func__, );
    qemud_client_send(_carCluster->client, (const uint8_t*)msg, msgLen);
}
