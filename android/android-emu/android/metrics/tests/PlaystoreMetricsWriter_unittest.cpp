...
static const int SOCK_WAIT = 50;  // Increase if you see test timeout issues.
...
protected:
    bool runMockHttpServer() {
        ...
        while (mRunning) {
            ...
            mRequestsMade++;
            int rec = 0;
            setSockTimeout(s.get(), SOCK_WAIT);
            do {
                char buffer[512];
    ...
    void waitForServerThread() {
        mRunning = false;
        // Just close the server socket as not
        // all platforms handle timeouts on "accept"
        // in the same way.
        System::get()->sleepMs(2 * SOCK_WAIT);
        close(mFd);
        mServer.join();
    }
...
static std::string messageBody(std::string httpMessage) {
    ...
    // We ignore content-length header..
