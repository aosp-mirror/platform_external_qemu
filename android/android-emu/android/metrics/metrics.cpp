...
static void fillAvdMetrics(android_studio::AndroidStudioEvent* event) {
    ...
    if (avdInfo_isPhone(android_avdInfo)) {
        eventAvdInfo->add_props(android_studio::EmulatorAvdInfo::PHONE_AVD);
    }
    if (avdInfo_isTv(android_avdInfo)) {
        eventAvdInfo->add_props(android_studio::EmulatorAvdInfo::TV_AVD);
    }
    if (avdInfo_isWear(android_avdInfo)) {
        eventAvdInfo->add_props(android_studio::EmulatorAvdInfo::WEAR_AVD);
    }
    if (avdInfo_isAndroidAuto(android_avdInfo)) {
        eventAvdInfo->add_props(android_studio::EmulatorAvdInfo::ANDROIDAUTO_AVD);
    }
}
