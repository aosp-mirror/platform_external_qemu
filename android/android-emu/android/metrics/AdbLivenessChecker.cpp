...
printf("AdbLivenessChecker::runCheckNonBlocking()\n");
    if (mDevicesCommand || mShellExitCommand) {
        // already in progress
        return;
    }

printf("AdbLivenessChecker::runCheckNonBlocking() 1\n");
    mDevicesCommand =
        mAdb->runAdbCommand(
            {"devices"},
            [this](const android::emulation::OptionalAdbCommandResult& result) {
                if (!result || result->exit_code) {
printf("CheckResult::kFailureAdbServerDead()\n");
...
printf("CheckResult::kFailureEmulatorDead()\n");
                                reportCheckResult(CheckResult::kFailureEmulatorDead);
                            } else {
printf("CheckResult::kOnline()\n");
