...
#include "android/metrics/proto/clientanalytics.pb.h"  // for LogEvent, LogR...
...
PlaystoreMetricsWriter::PlaystoreMetricsWriter(const std::string& sessionId,
    ...
    : MetricsWriter(sessionId), mUrl(url), mCookieFile(cookieFile) {
...
void PlaystoreMetricsWriter::writeCookie(std::string proto) {
    ...
    if (response.ParseFromString(proto)) {
        response.set_next_request_wait_millis(
                response.next_request_wait_millis() +
                base::System::get()->getUnixTime() * 1000);
...
void PlaystoreMetricsWriter::commit() {
    ...
    bool success = curl_post(mUrl.c_str(), buffer.str().data(),
                             buffer.str().size(), headers, 2, curlWriteCallback,
                             &protobuf_response, &error);
    if (success) {
