...
#include "android/metrics/proto/clientanalytics.pb.h"  // for android_studio
...
// A Metrics writer that reports events to clearcut when commit is called.
// The metrics writer will read in a cookie that contains the backoff time as
// not to violate the clearcut backoff requirements.
//
...
public:
    ...
    PlaystoreMetricsWriter(const std::string& sessionId,
                           ...
                           const std::string url = PLAYURL "?format=raw");
