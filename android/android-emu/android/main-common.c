...
static bool is_cros_image(const char* image, const uint8_t* init_image) {
     ...
     * partition table header. Then it follows partition entries with 128
     * bytes per entry. chrome OS kernel partition is in partition 2.
     */
    ...
    /* Check if it's a valid GPT partition table header */
    if (strncmp(buf + sector, "EFI PART", 8)) return false;
    /* Check if partition 2 is a chrome OS kernel partition */
