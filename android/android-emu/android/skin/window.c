...
static void background_redraw(Background* back,
    ...
    if (skin_rect_intersect(&r, rect, &back->rect)) {
        ...
        skin_surface_blit(surface, &r.pos, skin_image_surface(back->image),
...
static int adisplay_init(ADisplay* disp,
    ...
    disp->surface = skin_surface_create(disp->rect.size.w, disp->rect.size.h,
                                        disp->rect.size.w, disp->rect.size.h);
...
static void adisplay_redraw(ADisplay* disp,
    ...
    if (disp->brightness == LCD_BRIGHTNESS_OFF) {
        ...
        skin_surface_blit(surface, &r.pos, disp->surface, &src_r,
                          SKIN_BLIT_COPY);
    }
    ...
    if (disp->onion != NULL) {
        ...
        if (skin_rect_intersect(&r2, &r, &disp->onion_rect)) {
            ...
            skin_surface_blit(surface, &r2.pos, skin_image_surface(disp->onion),
                              &src_rect, SKIN_BLIT_SRCOVER);
        }
    }
    ...
    if (skin_rect_intersect(&r, rect, &button->rect)) {
        if (button->down && button->image != SKIN_IMAGE_NONE) {
            ...
            if (button->image != SKIN_IMAGE_NONE) {
                skin_surface_blit(surface, &r.pos,
...
static void skin_window_resize(SkinWindow* window, int resize_container) {
    ...
    window->surface = skin_surface_resize(window->surface, window_w, window_h,
                                          layout_w, layout_h);
    ...
    skin_surface_fill(window->surface, &layout->rect, layout->color);
    ...
    skin_surface_create_window(window->surface, window_x, window_y, window_w,
                               window_h);
    ...
    if (window != NULL && window->surface != NULL) {
...

        {
            ...
            if (skin_rect_intersect(&r, rect, &layout->rect)) {
                skin_surface_fill(window->surface, &r, layout->color);
            }
        }
