                 ...
                 unsigned state)
{
    ...
    if (finger->display) {
        if (skin_winsys_is_folded()) {
            ...
            SkinRect rect;
            uint32_t height, cb;
            multiDisplayAgent->getCombinedDisplaySize(NULL, &height);
            while (multiDisplayAgent->getNextMultiDisplay(startId, &id,
                                                        &rect.pos.x,
                                                        &rect.pos.y,
                                                        (uint32_t*)&rect.size.w,
                                                        (uint32_t*)&rect.size.h,
                                                        NULL, NULL, &cb)) {
                ...
                rect.pos.y = height - rect.size.h - rect.pos.y;
                // Default display (id = 0), or other valid display (cb != 0)
                if ((id == 0 || cb != 0) && skin_rect_contains(&rect, posX, posY)) {
                    posX = posX - rect.pos.x;
                    posY = posY - rect.pos.y;
                    break;
                }
                startId = id;
            }
