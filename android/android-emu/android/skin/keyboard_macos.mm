...
const char* skin_translate_host_kb_layout_to_android(const char* name) {
    char buffer[64];
    if (SscanfWithCLocale(name, "com.apple.keylayout.%s", buffer) == 1) {
        int size = 0;
        const char** androidKbLayoutFile = skin_get_android_kb_layout(&size);
        for (int i = 0; i < size; i++) {
            if (strcasestr(androidKbLayoutFile[i], buffer)) {
