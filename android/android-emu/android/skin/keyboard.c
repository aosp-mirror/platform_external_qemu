...
static int map_cros_key(int* code) {
    ...
    // calls it as "number". For most keys, "number" is as same as
    // linux key, but it's different for some keys. Since Android emulator
    // doesn't really use those special keys, it doesn't care about this bug.
    // As a quick fix, we remap them here. Also remap some special cros keys
    // here.
