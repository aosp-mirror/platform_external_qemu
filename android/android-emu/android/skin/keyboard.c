...
skin_keyboard_process_event(SkinKeyboard*  kb, SkinEvent* ev, int  down)
{
    if (ev->type == kEventTextInput) {
        ...
        // TODO(@wdu) Need to test this change with Chrome OS.
        int doCapL = ev->u.text.mod & kKeyModLShift;
        ...
        D("event type: %s key code=%d mod=0x%x str=%s\n", "kTextInput", ev->u.text.keycode, ev->u.text.mod, skin_key_pair_to_string(ev->u.text.keycode, ev->u.text.mod));
        ...
        D("event type: %s ignoring keycode %d\n", down? "keyDown" : "keyUp", keycode);
    }
}
