...
static int map_cros_key(int* code) {
    ...
    // Android emulator use Qt, and it maps qt key to linux key and
    // send it here.
    // Actually underlying qemu is expecting something else and it
    // called it as "number", for most keys, "number" is same as
    // linux key, but it's different for some keys. Since Android emulator
    // don't really use those special keys, so it doesn't care about this bug.
    // As a quick fix, we also remmap them here. Also remap some special cros
    // keys here.
    // This mapping can be generated from 2 stage mapping:
    // linux_to_qcode in ui/input-linux.c  can map linux key -> qcode
    // qcode_to_number in ui/input-keymap.c can map qcode -> number
