...
int skin_native_scancode_to_linux(int32_t scancode) {
    for (size_t i = 0; i < ARRAYLEN(linux_keycode_map); i++) {
        if (linux_keycode_map[i].native_scancode == scancode) {
            return linux_keycode_map[i].linux_keycode;
        }
    }
...
bool skin_keycode_native_is_keypad(int32_t scancode) {
    for (size_t nn = 0; nn < ARRAYLEN(sKeypadNumMapping); nn++) {
