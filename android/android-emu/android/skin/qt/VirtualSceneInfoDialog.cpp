...
void VirtualSceneInfoDialog::resize(const QSize& parentSize) {
    if (parentSize != mLastSize) {
        ...
        QSize overlaySize = QSize(std::min(hint.width(), parentSize.width()),
                                  std::min(hint.height(), parentSize.height()));
        QWidget::resize(overlaySize);

        const auto diff = std::max(parentSize.width() - overlaySize.width(),
                                   parentSize.height() - overlaySize.height());
