...
glm::quat quaternionTwist(const glm::quat& rotation,
...
void VirtualSceneControlWindow::updateMouselook() {
    ...
    if (offset.x() != 0 || offset.y() != 0) {
        mEulerRotationRadians.x -= glm::radians(static_cast<float>(offset.y()));
        mEulerRotationRadians.y -= glm::radians(static_cast<float>(offset.x()));

        // Clamp up/down rotation to -90 and +90 degrees, like a FPS camera.
        ...
        const glm::quat quatX = glm::angleAxis(mEulerRotationRadians.x,
                                               glm::vec3(1.0f, 0.0f, 0.0f));
        const glm::quat quatY = glm::angleAxis(mEulerRotationRadians.y,
                                               glm::vec3(0.0f, 1.0f, 0.0f));
        const glm::quat quatZ = glm::angleAxis(mEulerRotationRadians.z,
        ...
        if (mSensorsAgent) {
            ...
            glm::extractEulerAngleXYZ(glm::mat4(quatY * quatX * quatZ),
...
void VirtualSceneControlWindow::updateVelocity() {
    const glm::vec3 lookDirectionVelocity(
