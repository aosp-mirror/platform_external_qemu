...
static glm::vec3 clampPosition(glm::vec3 position) {
    return glm::clamp(
        position,
        glm::vec3(Accelerometer3DWidget::MinX, Accelerometer3DWidget::MinY, Accelerometer3DWidget::MinZ),
        glm::vec3(Accelerometer3DWidget::MaxX, Accelerometer3DWidget::MaxY, Accelerometer3DWidget::MaxZ));
}
...
void Accelerometer3DWidget::mouseMoveEvent(QMouseEvent* event) {
    if (mTracking && mOperationMode == OperationMode::Rotate) {
        ...
        mTargetRotation = glm::mat4_cast(q) * mTargetRotation;
        renderFrame();
...
void Accelerometer3DWidget::wheelEvent(QWheelEvent* event) {
    if (mOperationMode == OperationMode::Move) {
        ...
        mTargetPosition.z = clamp(MinZ, MaxZ, mTargetPosition.z +
