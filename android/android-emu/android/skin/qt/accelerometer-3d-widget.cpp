...
glm::vec3 Accelerometer3DWidget::screenToWorldCoordinate(int x, int y) const {
    ...
    const glm::vec3 cameraPosition = glm::vec3(0.f, 0.f, CameraDistance);

    // move the near plane position out to the camera plane.
    const glm::vec3 worldSpaceCoordinate = cameraPosition +
            (worldSpaceNearPlaneCoordinate - cameraPosition) *
