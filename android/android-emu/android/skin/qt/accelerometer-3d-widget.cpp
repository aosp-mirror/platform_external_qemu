...
void Accelerometer3DWidget::recalcRotationUpdateInterval() {
    uint64_t currTimeMs = (uint64_t)android::base::System::get()->getHighResTimeUs() / 1000;
    if (currTimeMs - mLastRotationUpdateMs > ROTATION_UPDATE_RESET_TIME_MS)
        memset(&mLastUpdateIntervals[0], 0, ROTATION_UPDATE_TIME_WINDOW_SIZE * sizeof(uint64_t));

    mLastUpdateIntervals[mRotationUpdateTimeWindowElt] = (currTimeMs - mLastRotationUpdateMs);
    ...
    if (nontrivialUpdateIntervals) {
        mUpdateIntervalMs /= ROTATION_UPDATE_TIME_WINDOW_SIZE;
    } else {
        mUpdateIntervalMs = 16;
    }
