...
/*     LOG(VERBOSE) << "switchMultiDisplay id " << id << " "
    ...
    if (enabled && !multiDisplayParamValidate(id, width, height, dpi, flag)) {
        return false;
    }
...
void EmulatorQtWindow::setUIDisplayRegion(int x, int y, int w, int h) {
    resizeAndChangeAspectRatio(x, y, w, h);
#ifdef __APPLE__
    // on Mac, delete and recreate opengl subwindow is needed.
    // Seen this issue for Mac SDK > 10.13
    SkinEvent* event = new SkinEvent();
    event->type = kEventScreenChanged;
    queueSkinEvent(event);
#endif
