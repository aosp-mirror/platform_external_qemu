...
SkinSurfaceBitmap::SkinSurfaceBitmap(const char* path) : reader(path) {
    ...
    auto result = TextureUtils::loadPNG(reader.fileName().toStdString().c_str(),
                                        false);
    if (result) {
        QImage::Format format = (result->mFormat == TextureUtils::Format::RGB24)
                                        ? QImage::Format_RGB888
                                        : QImage::Format_RGBA8888;
        image = QImage(
                reinterpret_cast<const unsigned char*>(result->mBuffer.data()),
