...
void EmulatorQtWindow::mouseReleaseEvent(QMouseEvent* event) {
    ...
    static bool longText = false;

    mContainer.messageCenter().addMessage(longText ?
                                              "Test message that is long enough to be multi-line. I hope Qt is smart enough for that\notherwise I need to manually add a newline"
                                            : "Short text message", icon, timeout);
    longText ^= true;
