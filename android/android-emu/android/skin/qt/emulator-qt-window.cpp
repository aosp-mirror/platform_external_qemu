...
SkinSurfaceBitmap::SkinSurfaceBitmap(int w, int h)
    : image(w, h, QImage::Format_ARGB32_Premultiplied), cachedSize(w, h) {
    image.fill(0);
}
...
SkinSurfaceBitmap::SkinSurfaceBitmap(const SkinSurfaceBitmap& other,
    ...
    pendingBlend = (blend * other.pendingBlend + 255) / 256;
...
void SkinSurfaceBitmap::applyPendingTransformations() {
    ...
    readImage();
    if (pendingBlend == 256 && pendingRotation == SKIN_ROTATION_180) {
        // can shortcut it with mirroring
        image = image.mirrored(true, true);
    } else if (pendingBlend == 256) {
        ...
        if (pendingBlend != 256) {
            painter.setOpacity(pendingBlend / 256.0);
        }
...
bool SkinSurfaceBitmap::hasPendingTransformations() const {
    return pendingRotation != 0 || pendingBlend != 256;
}
