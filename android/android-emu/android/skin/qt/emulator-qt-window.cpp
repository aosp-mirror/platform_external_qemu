...
void EmulatorQtWindow::handleNativeKeyEvent(int keycode,
    ...
    D("Raw keycode %d modifiers 0x%x\n", keycode, modifiers);
    ...
    if ((linuxKeyCode = skin_native_to_linux_keycode(keycode)) == -1 ||
        skin_keycode_is_modifier(linuxKeyCode)) {
        delete skin_event;
        return;
    }
...
#undef NSEventModifierFlagCapsLock
#undef NSEventModifierFlagShift
#undef NSEventModifierFlagControl
#undef NSEventModifierFlagOption
...
#endif
    D("Processed keycode %d modifiers 0x%x", linuxKeyCode,
      textInputData.mod);
    queueSkinEvent(skin_event);
}
