...
void EmulatorQtWindow::handleNativeKeyEvent(int keycode,
    ...
    int linuxKeyCode;
    if ((linuxKeyCode = skin_native_to_linux_keycode(keycode)) == -1) {
        delete skin_event;
        return;
    }
    D("keycode %d modifiers 0x%x", linuxKeyCode, modifiers);
    ...
    // Emulator uses qwerty2.kl in Android which doesn't recognize Caps lock key.
    // Thus, we simulate the same effect using shift when alt key is not pressed.
    // Caution: alt+shift+base key will output different symbol from alt+base.
    if ((modifiers & XCB_MOD_MASK_LOCK) &&
        skin_keycode_is_aplha(linuxKeyCode) &&
    ...
    if ((modifiers & kMacOSAlphaLock) &&
        skin_keycode_is_aplha(linuxKeyCode) &&
        !(modifiers & kMacOSControlKey)) {
        textInputData.mod |= kKeyModLShift;
    }
#else
...
void EmulatorQtWindow::switchMultiDisplay(bool enabled,
...

    {
      ...
      mMultiDisplay[id].pos_x = x;
      mMultiDisplay[id].pos_y = y;
      mMultiDisplay[id].width = width;
      mMultiDisplay[id].height = height;
      mMultiDisplay[id].dpi= dpi;
      mMultiDisplay[id].flag = flag;
      mMultiDisplay[id].enabled = enabled;
    }
    getAdbInterface()->enqueueCommand(
