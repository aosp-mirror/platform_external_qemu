    ...
    mEmulatorWindow(NULL)
{
    ...
    if (hw_rotary) {
        const struct {
            ...
            {mUi->carrotary_counterclockwiseButton, "cmd car_service inject-rotary", "CCW rotation"},
            {mUi->carrotary_clockwiseButton, "cmd car_service inject-rotary -c true", "CW rotation"},
...
void CarRotaryPage::toggleButtonPressed(QPushButton* button, const string cmd, const string label) {
    ...
    // 0 could be the parse from carrotary_longPressInterval failed or the user did input the
    // value 0 into the Qtextedit.
    ...
    if (!mLastPushButtonCmd.empty()) {
        ...
        mAdb->runAdbCommand(
                ...
                [this](const OptionalAdbCommandResult& result) {
                    ...
                    debug("ADB execute elapsed: " + std::to_string(mAdbExecuteTime.elapsed()));
                },
...
void CarRotaryPage::debug(const string message) {
    LOG(INFO) << message;
}