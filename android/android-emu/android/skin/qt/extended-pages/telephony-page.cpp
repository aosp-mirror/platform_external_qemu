...
static TelephonyPage* tpInstance = NULL;
...
static void telephony_callback(int value) {
    if (tpInstance) {
        tpInstance->eventLauncher(value);
    }
}
ANDROID_END_HEADER
...
void TelephonyPage::setTelephonyAgent(const QAndroidTelephonyAgent* agent) {
    ...
    if (mTelephonyAgent  &&  mTelephonyAgent->setNotifyCallback) {
        ...
        mTelephonyAgent->setNotifyCallback(telephony_callback);
    }
}

void TelephonyPage::eventLauncher(int theValue) {
    QEvent* newTelephonyEvent = new TelephonyEvent(mCustomEventType, theValue);
...
void TelephonyPage::customEvent(QEvent* cEvent) {
    ...
    // ->code is the number of calls active on the device
    if (tEvent->code == 0) {
        if (mCallActivity != CallActivity::Inactive) {
