...
CarSensorData::CarSensorData(QWidget *parent) :
    QWidget(parent),
    mUi(new Ui::CarSensorData)
{
    mUi->setupUi(this);
}

static EmulatorMessage getSetPropMsg() {
...
void CarSensorData::sendGearChangeMsg(int gear, string gearName) {
    if (sendEmulatorMsg == nullptr) {
        return;
    }

    EmulatorMessage emulatorMsg = getSetPropMsg();
    ...
    sendEmulatorMsg(emulatorMsg, log);
}

void CarSensorData::sendDrivingStatusChangeMsg(int status, string statusName) {
    if (sendEmulatorMsg == nullptr) {
        return;
    }
...
void CarSensorData::collectDrivingStatusAndReport() {
    ...
    if (mUi->checkBox_no_keyboard->isChecked()) {
        drivingStatus |= static_cast<int32_t>(VehicleDrivingStatus::NO_KEYBOARD_INPUT);
        statusMsg += "NO_KEYBOARD_INPUT ";
    }

    if (mUi->checkBox_limit_msg_len->isChecked()) {
        drivingStatus |= static_cast<int32_t>(VehicleDrivingStatus::LIMIT_MESSAGE_LEN);
    ...
    if (mUi->checkBox_no_voice->isChecked()) {
        drivingStatus |= static_cast<int32_t>(VehicleDrivingStatus::NO_VOICE_INPUT);
...
void CarSensorData::sendIgnitionChangeMsg(int ignition, string ignitionName) {
    if (sendEmulatorMsg == nullptr) {
        return;
    }
...
void CarSensorData::on_car_speedSlider_valueChanged(int speed)
{
    ...
    if (sendEmulatorMsg != nullptr) {
        EmulatorMessage emulatorMsg = getSetPropMsg();
        VehiclePropValue* value = emulatorMsg.add_value();
        value->set_prop(static_cast<int32_t>(VehicleProperty::PERF_VEHICLE_SPEED));
...
void CarSensorData::on_radioButton_lock_toggled() {
    if (mUi->radioButton_lock->isChecked()) {
        sendIgnitionChangeMsg(static_cast<int32_t>(VehicleIgnitionState::LOCK), "LOCK");
    }
}

void CarSensorData::on_radioButton_off_toggled() {
    if (mUi->radioButton_off->isChecked()) {
        sendIgnitionChangeMsg(static_cast<int32_t>(VehicleIgnitionState::OFF), "OFF");
    }
}

void CarSensorData::on_radioButton_start_toggled() {
    if (mUi->radioButton_start->isChecked()) {
        sendIgnitionChangeMsg(static_cast<int32_t>(VehicleIgnitionState::START), "START");
    }
}

void CarSensorData::on_radioButton_acc_toggled() {
    if (mUi->radioButton_acc->isChecked()) {
        sendIgnitionChangeMsg(static_cast<int32_t>(VehicleIgnitionState::ACC), "ACC");
    }
}

void CarSensorData::on_radioButton_on_toggled() {
    if (mUi->radioButton_on->isChecked()) {
        sendIgnitionChangeMsg(static_cast<int32_t>(VehicleIgnitionState::ON), "ON");
    }
}
...
void CarSensorData::on_checkBox_unrestricted_toggled() {
    ...
    if (mUi->checkBox_unrestricted->isChecked()) {
        ...
        sendDrivingStatusChangeMsg(static_cast<int32_t>(VehicleDrivingStatus::UNRESTRICTED),
            "UNRESTRICTED");
    }
}
