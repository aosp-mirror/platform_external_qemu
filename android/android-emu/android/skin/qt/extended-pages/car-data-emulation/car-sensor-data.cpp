...
bool VhalEventLoaderThread::parseJsonFile(const char *filePath, std::vector<emulator::EmulatorMessage> *emulatorMessages) {
    QString jsonString;
    if (filePath) {
    ...
    if (eventDoc.isNull()) {
        ...
        return loadEmulatorEvents(eventDoc, emulatorMessages);
    }
    return false;
}

bool VhalEventLoaderThread::loadEmulatorEvents(const QJsonDocument& eventDoc, std::vector<emulator::EmulatorMessage> *emulatorMessages) {
