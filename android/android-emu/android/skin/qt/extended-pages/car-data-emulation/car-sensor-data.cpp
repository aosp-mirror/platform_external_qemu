...
void CarSensorData::prepareVhalLoader() {
    ...
    connect(mVhalEventLoader.get(), &VhalEventLoaderThread::started, this, &CarSensorData::vhalEventThreadStarted);

    connect(mVhalEventLoader.get(), 
            &VhalEventLoaderThread::loadingFinished,
            this,  
            &CarSensorData::startupVhalEventThreadFinished
            );

    // Make sure new_instance gets cleaned up after the thread exits.
    connect(mVhalEventLoader.get(), &VhalEventLoaderThread::finished, mVhalEventLoader.get(),
            &QObject::deleteLater);
}
