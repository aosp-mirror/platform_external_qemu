...
CarSensorData::CarSensorData(QWidget* parent)
    ...
    QObject::connect(&mTimer, &QTimer::timeout, this, &CarSensorData::VhalTimeout);
}
...
void CarSensorData::VhalTimeout(){
    ...
    std::vector<emulator::EmulatorMessage> events = mTimedEmulatorMessages.getEvents(VHAL_REPLAY_INTERVAL);
...
void VhalEventLoaderThread::loadVhalEventFromFile(const QString& file_name, TimedEmulatorMessages* events) {
...
bool VhalEventLoaderThread::parseJsonFile(const char *filePath, TimedEmulatorMessages* timedEmulatorMessages) {
    QString jsonString;
    if (filePath) {
...
bool VhalEventLoaderThread::loadEmulatorEvents(const QJsonDocument& eventDoc, TimedEmulatorMessages* timedEmulatorMessages) {
    if(eventDoc.isNull()) {
    ...
    for (int eventIdx = 0; eventIdx < eventArray.size(); eventIdx++) {
        ...
        QJsonObject carPropertyValues = sensorrecord.value("car_property_values").toObject();
        ...
        int areaId = carPropertyValues.value("value").toObject().value("area_id").toInt();
        ...
        if(propertyValue.contains("int32_values")) {
            ...
            value->add_float_values((float)propertyValue.value("float_values").toDouble());
        }

        char* error = nullptr;
        timedEmulatorMessages->addEvents(strtol(sensorrecord.value("timestamp_ns").toString().toStdString().c_str(), &error,0), emulatorMsg);

        D("parse json timestampe is %s", sensorrecord.value("timestamp_ns").toString().toStdString().c_str());
        D("parse json key %d", sensorrecord.value("car_property_values").toObject().value("key").toInt());
        D("parse json areaid %d", sensorrecord.value("car_property_values").toObject().value("value").toObject().value("area_id").toInt());
    }
...
std::vector<emulator::EmulatorMessage> TimedEmulatorMessages::getEvents(int64_t interval) {
    std::vector<emulator::EmulatorMessage> res;
    D("TimedEmulatorMessages checking playing %d, %d", mCurrIndex, mEmulatorMessages.size());
    ...
    if(mCurrIndex == mEmulatorMessages.size()) {
        D("TimedEmulatorMessages end playing %d, %d", mCurrIndex, mEmulatorMessages.size());
        mCurrIndex = 0;
        mBaseTimeStamp = -1;
        mStatus = STOP;
        return res;
    ...
    for(; mCurrIndex < mEmulatorMessages.size(); mCurrIndex++) {
        ...
        D("TimedEmulatorMessages checking timestamp %lld, %lld, %lld", (long long)timestamp, (long long)mBaseTimeStamp, (long long)nextTimeStamp);
...
void TimedEmulatorMessages::addEvents(int64_t timestamp, emulator::EmulatorMessage &msg) {
    if(mBaseTimeStamp == -1) {
