    ...
    if (emulatorMsg.prop_size() > 0 || emulatorMsg.value_size() > 0) {
        ...
        for (int valIndex = 0; valIndex < emulatorMsg.value_size(); valIndex++) {
            ...
            if (!propMap.count(val.prop())) {
                ...
                propMap[val.prop()] = { std::to_string(val.prop()) };
            }

            QString label = QString::fromStdString(propMap[val.prop()].label);
            QString value = QString::fromStdString(carpropertyutils::getValueString(val));
            QString area = QString::fromStdString(carpropertyutils::getAreaString(val));
            ...
            // Also, all entries will be entered on the first initialization,
            // since we process all properties, so we don't have to worry about
            // about keeping track of next empty row or something like that.
