...
void CarPropertyTable::on_table_cellClicked(int row, int column) {
    ...
    switch (type) {
        ...
        case (int32_t) VehiclePropertyType::INT32_VEC :
            const std::unique_ptr<std::vector<int32_t>> int32VecValue =
                    std::make_unique<std::vector<int32_t>>(
                            *getUserInt32VecValue(propDesc, row, &pressedOk));
            if (!pressedOk) {
                return;
            }
            writeMsg = makeSetPropMsgInt32Vec(
                    prop,
                    std::make_unique<std::vector<int32_t>>(*int32VecValue),
                    areaId);
            writeLog = "Setting value for " + getLabel(row).toStdString();
            mSendEmulatorMsg(writeMsg, writeLog);
            break;
...
const std::unique_ptr<std::vector<int32_t>>
CarPropertyTable::getUserInt32VecValue(
...
EmulatorMessage CarPropertyTable::makeSetPropMsgInt32Vec(
        int32_t propId,
        const std::unique_ptr<std::vector<int32_t>> vals,
        int areaId) {
    VehiclePropValue* value;
    EmulatorMessage emulatorMsg = makeSetPropMsg(propId, &value, areaId);
