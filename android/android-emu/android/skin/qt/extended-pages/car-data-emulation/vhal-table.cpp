...
void VhalTable::showEvent(QShowEvent* event) {
    ...
    // Let search fram and property frame have same border as list widget
    mUi->search_frame->setStyleSheet(
            "border: 1px solid " +
            Ui::stylesheetValues(theme)["LIST_WIDGET_BORDER"]);
    mUi->property_frame->setStyleSheet(
            "border: 1px solid " +
            Ui::stylesheetValues(theme)["LIST_WIDGET_BORDER"] +
            "; background-color: " +
            Ui::stylesheetValues(theme)["VHAL_PROPERTY_BKG"]);
}
