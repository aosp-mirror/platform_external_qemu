...
const std::unique_ptr<std::vector<int32_t>> CheckboxDialog::getVec() {
    return std::make_unique<std::vector<int32_t>>(*mCheckedValues);
}

void CheckboxDialog::confirm(){
    mCheckedValues = std::make_unique<std::vector<int32_t>>();
    for(auto &checkbox : mCheckboxsVec){
