...
static constexpr int REFRESH_RESTART = 3;
...
WorkerProcessingResult CarClusterConnector::workerProcessFrame(FrameInfo& frameInfo) {
    if (!frameInfo.size) {
    ...
    if (!mCarClusterWindow->isVisible() && !mCarClusterWindow->isDismissed()) {
        ...
        mRefreshMsg.trySend(REFRESH_RESTART);
    }
    ...
    if (frameFinished > 0) {
        ...
        if(mCarClusterWindow) {
            mCarClusterWidget = mCarClusterWindow->findChild<CarClusterWidget*>("render_window");
            mCarClusterWidget->updatePixmap(QImage(mRgbData, FRAME_WIDTH, FRAME_HEIGHT, QImage::Format_RGB32));
        }
    } 
...
android::base::System::Duration CarClusterConnector::nextRefreshAbsolute() {
