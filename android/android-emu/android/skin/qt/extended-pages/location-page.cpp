...
void LocationPage::timeout() {
    ...
    if (nextRow < mUi->loc_pathTable->rowCount()) {
        ...
        printf("l-p: timeout: [%2d]: Last point.  Heading %6.1f\n", mRowToSend, prevHeading);
                                          ...
                                          double* pOutHeading)
{
    ...
    if (avdPath) {
        ...
        *pOutAltitude  = settings.value(Ui::Settings::LOCATION_RECENT_ALTITUDE,
                                     kGplexAlt).toDouble();
        *pOutVelocity = 0.0; // No global setting for velocity
        *pOutHeading  = 0.0; // No global setting for heading
    }
}
