...
bool BugreportPage::saveBugReportTo(const std::string& savingPath) {
    ...
    if (mUi->bug_bugReportCheckBox->isChecked() &&
        ...
        StringView bugreportBaseName("bugreport.txt");
...
void BugreportPage::loadAdbBugreport() {
    ...
    mAdbBugreport = mAdb->runAdbCommand(
            (isNougatOrHigher) ? std::vector<std::string>{"shell", "bugreport"}
                               : std::vector<std::string>{"bugreport"},
            [this, filePath](const OptionalAdbCommandResult& result) {
                mAdbBugreport.reset();
                bool success = false;
...
/*
Crash server only accepts file of size less than 1MB at this point.
bugreport.txt is usually at least 18MB. So we will need to chunk bugreport.txt
and compress them individually. Assume the compression ratio of 5. Each chunk
should be 5MB.
*/
void BugreportPage::compressBugReport() {
    if (!mSavingStates.adbBugreportSucceed ||
        mSavingStates.adbBugreportFilePath.empty())
        return;
    const size_t kChunkSize = 1024 * 1024 * 5;
    std::ifstream stream(mSavingStates.adbBugreportFilePath.c_str(),
    ...
    do {
        src.push_back(buf->sgetc());
    } while (buf->snextc() != EOF);
    stream.close();

    for (size_t i = 0; i <= src.size() / kChunkSize; i += 1) {
        auto filePath =
                PathUtils::join(mSavingStates.crashReporterLocation,
                                StringFormat("bugreport%d.lz4", (int)i));
        size_t srcLength = ((i + 1) * kChunkSize < src.size())
                                   ? kChunkSize
                                   : src.size() - i * kChunkSize;
        if (!compresstoFile(src.data() + (i * kChunkSize), srcLength,
                            filePath)) {
            break;
        }
    }
...
bool BugreportPage::compresstoFile(uint8_t* src,
                                   size_t srcLength,
                                   StringView filePath) {
    if (srcLength == 0) {
        return true;
    ...
    pref.compressionLevel = 9;
    size_t dstLength = LZ4F_compressFrameBound(srcLength, &pref);
    uint8_t* dst = (uint8_t*)malloc(dstLength);
    size_t outSize = 0;
    outSize = LZ4F_compressFrame(dst, dstLength, src, srcLength, &pref);
    if (outSize > 0) {
        saveToFile(filePath, (const char*)dst, outSize);
    }
    free(dst);
    return outSize > 0;
}
