...
void BugreportPage::on_bug_sendToGoogle_clicked() {
    ...
    auto logcatPathFile = PathUtils::join(mSavingStates.crashReporterLocation,
                                          "adb_logcat.lz4");
    compressToFile((uint8_t*)mSavingStates.adbLogcatOutput.c_str(),
                   mSavingStates.adbLogcatOutput.length(), logcatPathFile);
...
bool BugreportPage::compressToFile(const uint8_t* src,
                                   size_t srcLength,
                                   StringView filePath) {
