...
BugreportPage::BugreportPage(QWidget* parent)
    ...
    mUploadDialog = new UploadDialog(
            this, CrashService::makeCrashService(
...
void BugreportPage::on_bug_sendToGoogle_clicked() {
    ...
    auto task = createBugreportFolderSavingTask(
            mSavingStates.crashReporterLocation);
    task->run();
    mUploadDialog->exec();
...
UploadDialog::UploadDialog(QWidget* parent, CrashService* service)
    ...
    mDialogLabel =
            new QLabel(tr("Do you want to send the bug report to Google?"));
...
void UploadDialog::collectSysInfo() {
    mDialogLabel->setText("Collecting info ...");
    mProgressBar->show();
    mButtonBox->hide();
...
void UploadDialog::sendBugReport() {
    collectSysInfo();
    mDialogLabel->setText("Sending bug report...");
