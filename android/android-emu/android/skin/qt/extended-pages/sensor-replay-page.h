...
private:
    ...
    void registerSensorSessionPlaybackCallback(
        ...
        QString getPreviewText(const std::vector<std::string> sensorList,
                               const std::vector<int> carPropertyIdList);
        QString toTimeString(DurationNs time);
        QString toPlaybackTimeString(DurationNs timeline1, DurationNs timeline2,
                                     const QString& connector);
        void addTimeLineTableRow(QTableWidget * table, const QString& timeline,
                                 const QString& eventCount, int tableRow);
        ...
        static void sendCarEmulatorMessageLogged(
                const emulator::EmulatorMessage& msg, const std::string& log);
