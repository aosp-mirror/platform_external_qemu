...
void RecordScreenPage::on_rec_playStopButton_clicked() {
    if (mState == RecordUiState::Stopped) {
        auto videoPlayerNotifier = std::unique_ptr<android::videoplayer::VideoPlayerNotifier>(new android::videoplayer::VideoPlayerNotifier());
        connect(videoPlayerNotifier.get(), SIGNAL(updateWidget()), this, SLOT(updateVideoView()));
        connect(videoPlayerNotifier.get(), SIGNAL(videoFinished()), this, SLOT(videoPlayingFinished()));
        mVideoPlayer = android::videoplayer::VideoPlayer::create(mTmpFilePath, mVideoWidget.get(), std::move(videoPlayerNotifier));
