...
void SettingsPage::on_set_multiDisplay_toggled(bool checked) {
    if (mMultiDisplay[mCurrentDisplay].enabled == checked)
        return;
    ...
    mMultiDisplay[mCurrentDisplay].width = width;
    mMultiDisplay[mCurrentDisplay].height = height;
    mMultiDisplay[mCurrentDisplay].dpi = dpi;
    mMultiDisplay[mCurrentDisplay].enabled = checked;

    emit enableMultiDisplayChanged(checked, mCurrentDisplay, width, height, dpi);
}
...
void SettingsPage::onMultiDisplayIdChanged(int id) {
    ...
    mUi->set_multiDisplay->setChecked(mMultiDisplay[mCurrentDisplay].enabled);
}
