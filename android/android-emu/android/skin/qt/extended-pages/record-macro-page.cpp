...
void RecordMacroPage::on_playButton_clicked() {
    ...
    mTmpMacroName = macroName;
    mMacroPlaying = true;
    setMacroUiState(MacroUiState::Playing);
}

void RecordMacroPage::on_macroList_itemClicked(QListWidgetItem* listItem) {
    RecordMacroSavedItem* macroSavedItem = qobject_cast<RecordMacroSavedItem*>(
            mUi->macroList->itemWidget(listItem));
    const std::string macroName = macroSavedItem->getName();

    if (mMacroPlaying && mTmpMacroName == macroName) {
        setMacroUiState(MacroUiState::Playing);
    } else {
...
void RecordMacroPage::showPreview(std::string previewName) {
    previewName += ".gif";
    const std::string previewPath =
            PathUtils::join(getMacroPreviewsDirectory(), previewName);
