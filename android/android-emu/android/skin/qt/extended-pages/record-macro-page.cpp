...
void RecordMacroPage::loadUi() {
    ...
    QObject::connect(this, &RecordMacroPage::syncWithThreadRequired,
            this, &RecordMacroPage::playbackFinished);
...
void RecordMacroPage::playButtonClicked(QListWidgetItem* listItem) {
    ...
    auto result = sAutomationAgent->startPlaybackWithCallback(macroAbsolutePath, &RecordMacroPage::stopCurrentMacro);
    if (result.err()) {
...
void RecordMacroPage::stopCurrentMacro() {
    sInstance->emitStopSignal();
}

void RecordMacroPage::emitStopSignal() {
    emit syncWithThreadRequired();
}
