...
void MultiDisplayArrangement::paintEvent(QPaintEvent* e) {
    ...
    LOG(INFO) << w << " " << h << " " << width() << " " << height() << " " << geometry().width() << " " << geometry().height();
    float wRatio = ((float) width() - 2 * kBorderX) / (float) w;
    float hRatio = ((float) height() - 2 * kBorderY - kTextHeight) / (float) h;
    mRatio = (wRatio < hRatio) ? wRatio : hRatio;
    LOG(INFO) << "mRatio " << mRatio;
    QPen pen(kMultiDisplayBorderColor, 1);
    for (const auto& iter : mLayout) {
        p.setRenderHint(QPainter::Antialiasing);
        QPainterPath path;
        QRectF rect = QRectF((float)iter.second.x*mRatio + kBorderX,
                             (float)iter.second.y*mRatio + kBorderY,
                             (float)iter.second.w*mRatio,
                             (float)iter.second.h*mRatio);
