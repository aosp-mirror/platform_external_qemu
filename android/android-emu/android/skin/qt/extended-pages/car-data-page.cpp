...
void CarDataPage::onReceiveData(const char* msg, int length) {
    DINIT("Yao callback.... %s %d", msg, strlen(msg));
    string protoStr(msg);
    EmulatorMessage* emulatorMsg = new EmulatorMessage();
    string receivedMsg;
        if (emulatorMsg->ParseFromString(protoStr)) {
            receivedMsg = "Message Type: " + emulatorMsg->msg_type();
            receivedMsg += " Status: " + emulatorMsg->status();
            if (emulatorMsg->prop_size() > 0) {
                receivedMsg += " Prop: " + emulatorMsg->prop(0).prop();
            }
        } else {
