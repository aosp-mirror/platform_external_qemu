...
void VirtualSensorsPage::onTargetStateChanged() {
    ...
    // equality.
    const glm::quat newRotation = glm::eulerAngleXYZ(
            glm::radians(eulerDegrees.x), glm::radians(eulerDegrees.y),
            glm::radians(eulerDegrees.z));
    ...
    mSlidersUseCurrent = !Accelerometer3DWidget::vecNearEqual(
                                 position, mSlidersTargetPosition) ||
                         !Accelerometer3DWidget::quaternionNearEqual(
