...
void VirtualSensorsPage::setupRollableUI() {
    ...
    if (android_foldable_rollable_configured()) {
        ...
        switch (foldableState.config.numRolls) {
            case 3:
                mUi->roll2Slider->setRange(
                        foldableState.config.rollableParams[2].minRolledPercent,
                        foldableState.config.rollableParams[2].maxRolledPercent, false);
                mUi->roll2Label->setHidden(false);
                mUi->roll2Slider->setHidden(false);
            case 2:
