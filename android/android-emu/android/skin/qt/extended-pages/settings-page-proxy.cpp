...
void SettingsPage::grayOutProxy() {
    ...
    if (!mUi->set_useStudio->isChecked()) {
        ...
        radioEnabled = true;
        if (mUi->set_autoDetect->isChecked()) {
            autoCheckEnabled = true;
            if (mUi->set_autoProxy->isChecked()) {
                autoFieldsEnabled = true;
            }
    ...
    mUi->set_autoDetect->setEnabled(radioEnabled);
    mUi->set_manualConfig->setEnabled(radioEnabled);

    mUi->set_autoProxy->setEnabled(autoCheckEnabled);

    mUi->set_proxyUrl->setEnabled(autoFieldsEnabled);
    mUi->set_clearPasswords->setEnabled(autoFieldsEnabled);
