...
// }
 ...
 void SnapshotPage::highlightItemWithFilename(const QString& fileName) {
    // Try the main list
    QTreeWidgetItemIterator iter1(mUi->snapshotDisplay);
    for ( ; *iter1; iter1++) {
        WidgetSnapshotItem* item = static_cast<WidgetSnapshotItem*>(*iter1);
    ...
    QTreeWidgetItemIterator iter2(mUi->defaultSnapshotDisplay);
    for ( ; *iter2; iter2++) {
        WidgetSnapshotItem* item = static_cast<WidgetSnapshotItem*>(*iter2);
        if (fileName == item->fileName()) {
