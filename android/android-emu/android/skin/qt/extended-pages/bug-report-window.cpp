...
static const char BUG_REPORT_TEMPLATE[] = R"(
Please Read:
https://developer.android.com/studio/report-bugs.html#emulator-bugs
...
void BugReportWindow::saveBugReportFolder(const QObject* handler,
    ...
    if (dirName.isNull()) {
        ...
        if (started_slot) {
            connect(task, SIGNAL(started()), handler, started_slot);
        }
        if (finished_slot) {
            connect(task, SIGNAL(finished(bool, QString, bool)), handler,
                    finished_slot);
        }
...
void BugReportWindow::saveBugreportFolderFinished(bool result,
    ...
    mSavingStates.bugreportSavedSucceed = result;
    if (result) {
...
IssueTrackerTask::IssueTrackerTask(ReportingFields reportingFields)
