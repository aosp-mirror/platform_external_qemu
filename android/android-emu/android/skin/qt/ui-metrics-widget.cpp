...
void UiMetricsWidget::onFrameAvailableCallback(unsigned char* pixel,
                                               uint64_t timeStamp) {
    int x = globalData()->mX;
    int y = globalData()->mY;
    int bpr = globalData()->mBytesPerRow;
    int bpp = globalData()->kBytesPerPixel;
    int32_t r = pixel[y * bpr + x * bpp];
    int32_t g = pixel[y * bpr + x * bpp + 1];
    int32_t b = pixel[y * bpr + x * bpp + 2];
    ...
    if (globalData()->mPrevDataPoint.ptr()) {
...
bool UiMetricsWidget::winGetData() {
    ...
    if (!res)
        return;
    onFrameAvailableCallback(mPixelData.data(), now / 1000);
#endif
    return true;
}
...
void UiMetricsWidget::startScreenCapture(int x, int y) {
    ...
    globalData()->mX = x;
    globalData()->mY = y;
    globalData()->mBytesPerRow = globalData()->kBytesPerPixel * mScreenWidth;
#ifdef __APPLE__
    startDisplayStream(&onFrameAvailableCallback);
#elif defined(_WIN32)
    mPixelData.resize(
            mScreenWidth * mScreenHeight * globalData()->kBytesPerPixel, 0);
