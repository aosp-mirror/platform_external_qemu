...
UiMetricsWidget::UiMetricsWidget(QWidget* parent)
    : GLWidget(parent),
      mScreenCapTask((android::base::Looper*)android_getMainLooper(),
...
bool UiMetricsWidget::winGetData() {
    ...
    int pixel = r + (g << 8) + (b << 16);
    int64_t now = System::get()->getHighResTimeUs() / 1000;
    sGlobal->updateFrame(
            {.mTimeInterval = 0, .mPixelValue = pixel, .mTimeStamp = now});
    int64_t phasedWaitMs = 0;
    if (mLastTimeMs < 0) {
        phasedWaitMs = now + kScreenCapIntervalMs;
    } else {
        // fprintf(stderr, " Time interval ms %lld\n", now - mLastTimeMs);
        phasedWaitMs =
                kScreenCapIntervalMs *
                        ((now - mLastTimeMs) / kScreenCapIntervalMs + 1) +
                mLastTimeMs;
    }
    mLastTimeMs = phasedWaitMs;
    mScreenCapTask.setTaskIntervalMs(phasedWaitMs - now);
...
bool UiMetricsWidget::saveToPath(const std::string& path) {
    ...
    fprintf(stderr, "Save to file path %s\n", path.c_str());
    for (const auto& iter : sGlobal->mDataStore) {
        outFile << iter.mTimeStamp << "," << iter.mTimeInterval << "\n";
    }
    double avg = 0;
    for (const auto& iter : sGlobal->mDataStore) {
        avg += iter.mTimeInterval;
    }
    avg = avg / sGlobal->mDataStore.size();
    double sampleVar = 0;
    for (const auto& iter : sGlobal->mDataStore) {
        sampleVar += ((iter.mTimeInterval - avg) * (iter.mTimeInterval - avg));
    }
    sampleVar /= (sGlobal->mDataStore.size() - 1);
    sampleVar = std::sqrt(sampleVar);

    outFile << "#avg: " << avg << "\n";
    outFile << "#sample variance: " << sampleVar << "\n";
    fprintf(stderr, "#avg %f sample variance %f\n", avg, sampleVar);
    outFile.close();
    return true;
}
