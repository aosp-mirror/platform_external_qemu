...
UiMetricsWidget::UiMetricsWidget(QWidget* parent) : GLWidget(parent) {
    ...
    bpr = kBytesPerPixel * mScreenWidth;
...
static void onFrameAvailableCallback(unsigned char* pixel, uint64_t timeStamp) {
    int32_t r = pixel[sY * bpr + sX * kBytesPerPixel];
    int32_t g = pixel[sY * bpr + sX * kBytesPerPixel + 1];
    int32_t b = pixel[sY * bpr + sX * kBytesPerPixel + 2];
...
void UiMetricsWidget::winGetData() {
    static std::vector<unsigned char> pixelData(
            mScreenWidth * mScreenHeight * kBytesPerPixel, 0);
#if defined(_WIN32)
    bool res = takeScreenshot(pixelData.data(), mScreenWidth, mScreenHeight);
    if (!res)
        return;
    const auto now = System::get()->getHighResTimeUs();
    onFrameAvailableCallback(pixelData.data(), now);
#endif
}
...
#elif defined(_WIN32)
    const int kWinScreenCapTimeInterval = 10;  // ms
    mWinScreenCapTimer.setInterval(kWinScreenCapTimeInterval);
    connect(&mWinScreenCapTimer, SIGNAL(timeout()), this, SLOT(winGetData()));
#else
#endif
}
