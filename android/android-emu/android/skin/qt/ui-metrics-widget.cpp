...
void UiMetricsWidget::getData() {
    ...
    for (auto& dataSource : mDataSources) {
        ...
        if (sGlobal->mDataStore.empty())
            continue;

        int origSize = sGlobal->mDataStore.size();
        ...
        for (int i = totalCopy; i > 0; i--) {
            dataSource.values[kCollectionHistory - i] =
                    sGlobal->mDataStore[i - 1].mTimeInterval;
        }
        sGlobal->mDataStore.erase(sGlobal->mDataStore.begin(),
                                  sGlobal->mDataStore.begin() + totalCopy);
