...
void EmulatorContainer::moveEvent(QMoveEvent* event) {
    ...
    adjustMessagesOverlayGeometry(false);
}
...
void EmulatorContainer::resizeEvent(QResizeEvent* event) {
    ...
    adjustMessagesOverlayGeometry(true);
...
void EmulatorContainer::adjustMessagesOverlayGeometry(bool) {
    ...
    /*void EmulatorContainer::adjustMessagesOverlayGeometry(bool resized) {
        QSignalBlocker blocker(mMessages.ptr());
        mMessages->setUpdatesEnabled(false);

        if (resized) {
            mMessages->adjustSize();
        }
        const auto w = std::min(width(), std::max(300, (width() - 2 * 100)));
        if (mMessages->minimumWidth() != w || mMessages->maximumWidth() != w) {
            mMessages->setFixedWidth(w);
        }
        auto newPos = pos() += {(width() - mMessages->width()) / 2,
                               height() - mMessages->height()};
        if (mMessages->pos() != newPos) {
            mMessages->move(newPos);
        }

        mMessages->setUpdatesEnabled(true);
    */
}
