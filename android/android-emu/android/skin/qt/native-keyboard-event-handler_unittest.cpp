...
const struct {
    ...
    {XCB_MOD_MASK_SHIFT, kKeyModLShift},
    {XCB_MOD_MASK_LOCK, kKeyModCapsLock},
    {XCB_MOD_MASK_CONTROL, kKeyModLCtrl},
    {XCB_MOD_MASK_SHIFT | XCB_MOD_MASK_LOCK, kKeyModLShift | kKeyModCapsLock}
}
...
const struct {
    ...
    {NSEventModifierFlagShift | NSEventModifierFlagCapsLock, kKeyModLShift | kKeyModCapsLock}
}

#endif

TEST(keyboardEventHandler, handleKeyEvent) {
    for (size_t i = 0; i < ARRAY_SIZE(kHandleKeyEventTestData); i++) {
        NativeKeyboardEventHandler::KeyEvent keyEv;
        keyEv.scancode = kHandleKeyEventTestData[i].scancode;
        keyEv.modifiers = kHandleKeyEventTestData[i].inputModifier;
        SkinEvent* output =
                NativeKeyboardEventHandler::getInstance()->handleKeyEvent(
                        keyEv);
        EXPECT_NE(nullptr, output);
        EXPECT_EQ(kEventTextInput, output->type);
        EXPECT_EQ(kHandleKeyEventTestData[i].linuxKeyCode, output->u.text.keycode);
    }
}

TEST(keyboardEventHandler, translateModifierState) {
    for (size_t i = 0; i < ARRAY_SIZE(kModifierTestData); i++) {
        int modifer = NativeKeyboardEventHandler::getInstance()->translateModifierState(0, kModifierTestData[i].inputModifier);
