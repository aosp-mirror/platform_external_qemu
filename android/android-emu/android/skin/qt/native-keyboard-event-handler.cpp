...
public:
    X11KeyboardEventHandler() : mAltMask(XCB_MOD_MASK_1) {
        // Alt modifier could be mapped to any one of MOD_MASK1 - MOD_MASK5
    ...
    int translateModifierState(int keycode, int modifiers) override {
        ...
        // TODO (wdu@) Remove the code below after bug has been fixed in
        // prebuilt QT 5.12.1
...
SkinEvent* NativeKeyboardEventHandler::handleKeyEvent(
    ...
    if (!skin_keycode_native_to_linux(&scancode, &modifiers)) {
        return nullptr;
    }

    // when keyboard layout is non-English, 
    // Alt-Gr bit is always set. This is due to mysterious bug in prebuilt libXKB.
    // Thus, right-alt key needs to be hanlded specially in 
    // NativeKeyboardEventHandler::handleKeyEvent
#if defined(__linux__)
    if (skin_keycode_is_modifier(scancode) && scancode != LINUX_KEY_RIGHTALT) {
        return nullptr;
    }
#else
    if (skin_keycode_is_modifier(scancode)) {
        return nullptr;
    }
#endif
    SkinEvent* skinEvent =
    ...
    textInputData.keycode = scancode;
    textInputData.mod =
