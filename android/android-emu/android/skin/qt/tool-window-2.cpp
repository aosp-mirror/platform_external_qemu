...
// only way to make a window that does not have its own taskbar item.
    ...
    // Assume we're starting fully expanded.
    // Disable the expand buttons.
    mTools2Ui->expandHoriz->setVisible(false);
    mTools2Ui->expandVert ->setVisible(false);

    QSettings settings;
    bool foldableEnabled = settings.value(Ui::Settings::FOLDABLE_ENABLE, false).toBool();
    printf("foldableEnabled %d\n", foldableEnabled);
    //TODO: need 29
    mHide = !foldableEnabled || avdInfo_getApiLevel(android_avdInfo) < 28;
}
...
void ToolWindow2::on_expandVert_clicked() {
    // Enlarge to full height
    mTools2Ui->compressHoriz->setVisible(true);
    mTools2Ui->compressVert ->setVisible(true);
    mTools2Ui->expandHoriz  ->setVisible(false);
    mTools2Ui->expandVert   ->setVisible(false);

    mEmulatorWindow->resizeAndChangeAspectRatio(3);
...
void ToolWindow2::on_compressVert_clicked() {
    // Reduce to 3/4 height
    mTools2Ui->compressHoriz->setVisible(false);
    mTools2Ui->compressVert ->setVisible(false);
    mTools2Ui->expandHoriz  ->setVisible(false);
    mTools2Ui->expandVert   ->setVisible(true);

    mEmulatorWindow->resizeAndChangeAspectRatio(2);

    char sizeString[16];
    sprintf(sizeString, "%dx%d", android_hw->hw_lcd_width , (android_hw->hw_lcd_height * 3) / 4);
    mEmulatorWindow->getAdbInterface()->enqueueCommand({ "shell", "wm", "size", sizeString });
}

void ToolWindow2::on_expandHoriz_clicked() {
    // Enlarge to double width
    mTools2Ui->compressHoriz->setVisible(true);
    mTools2Ui->compressVert ->setVisible(true);
    mTools2Ui->expandHoriz  ->setVisible(false);
    mTools2Ui->expandVert   ->setVisible(false);

    mEmulatorWindow->resizeAndChangeAspectRatio(1);
...
void ToolWindow2::on_compressHoriz_clicked() {
    ...
    mEmulatorWindow->resizeAndChangeAspectRatio(0);

    char sizeString[16];
    sprintf(sizeString, "%dx%d", (android_hw->hw_lcd_width * 3) / 4, android_hw->hw_lcd_height);
    mEmulatorWindow->getAdbInterface()->enqueueCommand({ "shell", "wm", "size", sizeString });
}
