...
// only way to make a window that does not have its own taskbar item.
    ...
    printf("foldableEnabled %d\n", foldableEnabled);

    mFoldXOffset = android_hw->hw_displayRegion_0_1_xOffset;
    mFoldYOffset = android_hw->hw_displayRegion_0_1_yOffset;
    mFoldWidth   = android_hw->hw_displayRegion_0_1_width;
    mFoldHeight  = android_hw->hw_displayRegion_0_1_height;
    if (mFoldXOffset < 0 || mFoldXOffset > 9999 ||
        mFoldYOffset < 0 || mFoldYOffset > 9999 ||
        mFoldWidth   < 1 || mFoldWidth   > 9999 ||
        mFoldHeight  < 1 || mFoldHeight  > 9999 ||
        !foldableEnabled              ||
        //TODO: need 29
...
void ToolWindow2::forwardKeyToEmulator(uint32_t keycode, bool down) {
    // TODO: Remove this?
}
...
//    mEmulatorWindow->resizeAndChangeAspectRatioNew(false);

    QSettings settings;
    settings.setValue(Ui::Settings::FOLDABLE_IS_FOLDED, false);

    printf("sending SW_LID false\n");
    forwardGenericEventToEmulator(EV_SW, SW_LID, false);
    forwardGenericEventToEmulator(EV_SYN, 0, 0);

//    char sizeString[16];
//    sprintf(sizeString, "%dx%d", android_hw->hw_lcd_width , android_hw->hw_lcd_height);
//    mEmulatorWindow->getAdbInterface()->enqueueCommand({ "shell", "wm", "size", sizeString });
//    printf("sending wm size %s\n", sizeString);
}
...
//    mEmulatorWindow->resizeAndChangeAspectRatioNew(true);
    ...
    if (!mFoldAreaSent) {
        ...
        sprintf(foldedArea, "folded-area %d,%d,%d,%d",
                mFoldXOffset,
                mFoldYOffset,
                mFoldXOffset + mFoldWidth,
                mFoldYOffset + mFoldHeight);
        mEmulatorWindow->getAdbInterface()->enqueueCommand({"shell", "wm", foldedArea});
        printf("send folding area %s\n", foldedArea);
        mFoldAreaSent = true;
    }

    printf("sending SW_LID true\n");
    forwardGenericEventToEmulator(EV_SW, SW_LID, true);
    forwardGenericEventToEmulator(EV_SYN, 0, 0);

//
//    char sizeString[16];
//    sprintf(sizeString, "%dx%d", android_hw->hw_displayRegion_0_1_width,
//                                 android_hw->hw_displayRegion_0_1_height);
//    mEmulatorWindow->getAdbInterface()->enqueueCommand({ "shell", "wm", "size", sizeString });
//    printf("sending wm size %s\n", sizeString);
}
