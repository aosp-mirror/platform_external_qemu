...
void ModalOverlay::showButtonFunc(QString text,
    ...
    clickableLabel->setStyleSheet(
        QString("font-size: %1;")
                .arg(Ui::stylesheetFontSize(Ui::FontSize::Huge)));
    clickableLabel->setText(
        QString("<center><a href=\"unused.html\" style=\"%1\">%2</a></center>")
                .arg("text-decoration:none;color:#00bea4")
                .arg(text));
    connect(clickableLabel, SIGNAL (linkActivated(const QString&)),
            this, SLOT(slot_handleButtonFunc()));
