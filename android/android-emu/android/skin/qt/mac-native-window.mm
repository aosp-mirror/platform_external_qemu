...
static void (*sCallback)(const uint8_t*, uint64_t) = NULL;
...
static void frameAvailable(CGDisplayStreamFrameStatus status,
    ...
    if (kCGDisplayStreamFrameStatusFrameComplete == status &&
        ...
        const uint8_t* pix = (uint8_t*)IOSurfaceGetBaseAddress(frameSurface);
        if (NULL != pix) {
            sCallback(pix, displayTime / 1000000); /*time in Ms*/
        }
        IOSurfaceUnlock(frameSurface, kIOSurfaceLockReadOnly, NULL);
    }
}

bool startDisplayStream(void (*callback)(const uint8_t*, uint64_t)) {
    sCallback = callback;
    ...
    CGDirectDisplayID display_ids[1]; /* just a typedef'd uint32_t */
    uint32_t found_displays = 0;
    CGError err = CGGetActiveDisplayList(1, display_ids, &found_displays);
