...
static void s_frameAvailable(CGDisplayStreamFrameStatus status, uint64_t displayTime,
    ...
    if (kCGDisplayStreamFrameStatusFrameComplete == status
            ...
            if (NULL != pix) {
                uint32_t r = pix[s_y * bpr + s_x * bpp];
                uint32_t g = pix[s_y * bpr + s_x * bpp + 1];
                uint32_t b = pix[s_y * bpr + s_x * bpp + 2];
                uint32_t a = pix[s_y * bpr + s_x * bpp + 3];
                //uint32_t fr = r + g << 8 + b << 16;
                //fprintf(stderr, "r %u g %u b %u a %u and time %llu\n", r, g, b, a, displayTime);
            }
            IOSurfaceUnlock(frameSurface, kIOSurfaceLockReadOnly, NULL);
    }                                              
}

void* retrieveDisplayPixels(uint32_t* size) {
    
    *size = 1;
    void* ret = android_alloc(sizeof(float));    
    float f = 100.0f;
