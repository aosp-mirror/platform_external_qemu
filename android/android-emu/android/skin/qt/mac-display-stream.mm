...
static void s_frameAvailable(CGDisplayStreamFrameStatus status, uint64_t displayTime,
    ...
    if (kCGDisplayStreamFrameStatusFrameComplete == status
            ...
            if (NULL != pix) {
                uint8_t r = pix[s_x * bpr + s_y * bpp];
                uint8_t g = pix[s_x * bpr + s_y * bpp+1];
                uint8_t b = pix[s_x * bpr + s_y * bpp+2];
                uint8_t a = pix[s_x * bpr + s_y * bpp+3];
                uint32_t fr = r + g << 8 + b << 16;
                fprintf(stderr, "r %u g %u b %u a %u and frame %u\n", r, g, b, a, fr);
            }
