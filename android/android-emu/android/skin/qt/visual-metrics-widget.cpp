...
#include "android/skin/qt/mac-display-stream.h"

class QWidget;

using namespace android::base;

VisualMetricsWidget::VisualMetricsWidget(QWidget* parent)
    : GLWidget(parent) {
    ...
    mDataSources.emplace_back(
            (DataSource){currentGraphIndex,
                         ...
                         minTime,
                         maxTime,
                         [minTime, maxTime]() mutable {
                         	 std::vector<DataSourcePoint> res;
                             uint32_t size = 0;
                             float* dp = reinterpret_cast<float*>(
                                    retrieveDisplayPixels(&size));
...
VisualMetricsWidget::~VisualMetricsWidget() {
...
void VisualMetricsWidget::onCollectionEnabled() {
    startDisplayStream(1155, 455);
    for (auto& dataSource : mDataSources) {
...
void VisualMetricsWidget::onCollectionDisabled() {
    stopDisplayStream();
...
void VisualMetricsWidget::getData() {
    ...
    for (auto& dataSource: mDataSources) {
        ...
        std::vector<DataSourcePoint> points = dataSource.getFunc();
        if (points.empty()) continue;
    	memmove(dataSource.values.data(),
            dataSource.values.data() + points.size(),
            sizeof(float) * (kCollectionHistory - points.size()));
    	for (int i = points.size(); i > 0; i--) {
    		dataSource.values[kCollectionHistory - i] = points[i-1].val;
    	}
...
bool VisualMetricsWidget::initGL() {
    if (!mGLES2) {
        return false;
    }
...
bool VisualMetricsWidget::initProgram() {
    auto gl = mGLES2;
...
bool VisualMetricsWidget::initXAxisBuffer() {
    auto gl = mGLES2;
...
bool VisualMetricsWidget::initFontBuffer() {
    auto gl = mGLES2;
...
void VisualMetricsWidget::drawText(const std::string& string,
                                 float x,
                                 float y,
                                 float height,
                                 const float color[4]) {
...
bool VisualMetricsWidget::initDataSourceBuffer(GLuint* buf) {
    auto gl = mGLES2;
...
void VisualMetricsWidget::resizeGL(int w, int h) {
    if (!mGLES2) {
        return;
    }
...
void VisualMetricsWidget::repaintGL() {
    if (!mGLES2) {
        return;
    }
