...
PerfStats3DWidget::PerfStats3DWidget(QWidget* parent)
    ...
    DataSourceGetter mainLoopUsageGetter = []() {
        return (DataSourcePoint){
            100.0f * CpuUsage::get()->getSingleAreaUsage(CpuUsage::UsageArea::MainLoop),
            0.0f, 100.0f };
    };
    ...
    for (int i = 0; i < numGuestCores; ++i) {
        DataSourceGetter vcpuUsageGetter = [i]() {
            return (DataSourcePoint){
                100.0f * CpuUsage::get()->getSingleAreaUsage(
...
void PerfStats3DWidget::repaintGL() {
    ...
    for (const auto& dataSource: mDataSources) {
        ...
        if (dataSource.values.size() >= mCollectionHistory) {
            ...
            std::stringstream ss;
            ss << dataSource.name;
            ss << " " << dataSource.values[mCollectionHistory - 1] << " " << dataSource.unit;
            drawText(ss.str(), -1.0f, offset + 2.0f * graphHeight - textHeight,
                     textHeight, dataSource.color);
        }
    }
