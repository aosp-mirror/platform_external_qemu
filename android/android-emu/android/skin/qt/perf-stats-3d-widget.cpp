...
PerfStats3DWidget::PerfStats3DWidget(QWidget* parent)
    ...
    DataSourceControlCallback cpuUsageOnBegin = []() {
        CpuUsage::get()->setMeasurementInterval(kCollectionIntervalMs * 1000);
    };

    DataSourceControlCallback cpuUsageOnEnd = []() {
        CpuUsage::get()->setMeasurementInterval(kCollectionIntervalMs * 1000);
    };

    DataSourceGetter mainLoopUsageGetter = []() {
        return (DataSourcePoint){
            CpuUsage::get()->getSingleAreaUsage(CpuUsage::UsageArea::MainLoop),
            0.0f, 1.0f };
    };
    ...
    mDataSources.emplace_back((DataSource){
        currentGraphIndex, { 0.7f, 0.2f, 0.9f, 1.0f },
        "Main Loop CPU Usage", "%", 0.0f, 1.0f,
        mainLoopUsageGetter,
        cpuUsageOnBegin, cpuUsageOnEnd,
    });
    ...
    for (int i = 0; i < numGuestCores; ++i) {
        DataSourceGetter vcpuUsageGetter = [i]() {
            return (DataSourcePoint){
    ...
    currentGraphIndex += numGuestCores;
}
...
void PerfStats3DWidget::getData() {
    ...
    for (auto& dataSource: mDataSources) {
        if (dataSource.values.size() < mCollectionHistory) {
            dataSource.values.resize(mCollectionHistory);
        }
        ...
        memmove(dataSource.values.data(),
                ...
                sizeof(float) * (mCollectionHistory - 1));

        dataSource.values[mCollectionHistory - 1] = pt.val;
...
void PerfStats3DWidget::drawText(const std::string& string,
    ...
    float scale = height * 0.5f; // each glyph is really 2 units wide
    float currX = x + height * 0.5f;
    float dx = height; // TODO: decide if we want the glyphs to be rectangles
    float yOff = y + height * 0.5f;

    for (size_t i = 0 ; i < string.size(); ++i) {
        gl->glUniform1f(mXScaleUniformLoc, scale);
        ...
        gl->glDrawArrays(GL_LINES, STROKES_PER_GLYPH * 2 * charVal, 10);
        currX += dx;
    }
...
bool PerfStats3DWidget::initDataSourceBuffer(GLuint* buf) {
    ...
    gl->glBufferData(GL_ARRAY_BUFFER, sizeof(float) * mCollectionHistory,
