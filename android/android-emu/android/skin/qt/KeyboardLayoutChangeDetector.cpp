...
// Run check periodically and use "adb shell input physicalkeyboard <layout>" to
...
bool KeyboardLayoutChangeDetector::runKeyboardLayoutChangeCheck() {
    ...
    string curKeyboardLayout(skin_keyboard_current_active_layout());
    if (curKeyboardLayout.compare(mKeyboardLayout) != 0) {
        ...
        string androidKeyboardLayout(skin_translate_host_kb_layout_to_android(
                mKeyboardLayout.c_str()));
        if (!androidKeyboardLayout.empty()) {
            inputCmds.push_back(std::move(androidKeyboardLayout));
            ...
            mChangeLayoutCmd = mAdb->runAdbCommand(
                    ...
                    [shared](const OptionalAdbCommandResult& result) {
                        ...
                        if (result && result->output &&
                            ...
                            // Stop the recurrent checking because input command
                            // doesn't support it.
