...
AEMU_EXPORT typedef void (*OnPostFunc)(void* context, int width, int height, int ydir,
                           int format, int type, unsigned char* pixels);
AEMU_EXPORT void android_setPostCallback(OnPostFunc onPost, void* onPostContext, bool useBgraReadback);

AEMU_EXPORT typedef void (*ReadPixelsFunc)(void* pixels, uint32_t bytes);
...
AEMU_EXPORT typedef void (*ScreenshotFunc)(const char* dirname, uint32_t displayId);
AEMU_EXPORT void android_registerScreenshotFunc(ScreenshotFunc f);
AEMU_EXPORT void android_screenShot(const char* dirname, uint32_t displayId);
