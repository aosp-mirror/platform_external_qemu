...
static offworld::Response createAsyncResponse(
    uint32_t asyncId,
    std::unique_ptr<android::automation::VideoInjectionResult> result) {
...
VideoInjectionManagerImpl::getNextCommand(
    std::unique_ptr<VideoInjectionResult> previousResult) {
    CHECK(!previousResult && mCommandPending)
            << "Video injection worker should finish its prevous task "
               "before requesting new command";
    CHECK(previousResult && !mCommandPending)
            << "Video injection worker should only send back result of a "
               "command once";
    ...
    if (mCommandPending) {
        ...
        pb::Response response = createAsyncResponse(
