...
public:
    ...
    // Get the next command from the event source.  Returns false if there are
    // no events remaining.
    virtual bool getNextCommand(DurationNs* outTime,
                                pb::RecordedEvent* outCommand) = 0;
};
...
public:
    ...
    bool getNextCommand(DurationNs* outTime,
                        pb::RecordedEvent* outCommand) override {
...
public:
    OffworldEventSource(StringView event) { AddEvents(event); }
    ...
    bool getNextCommand(DurationNs* outTime,
        ...
        pb::RecordedEvent event = std::move(mEvents.front());
        mEvents.pop_front();
    ...
    void AddEvents(StringView eventString) {
        using namespace google::protobuf;

        io::ArrayInputStream stream(eventString.data(), eventString.size());
        pb::RecordedEvent event;
        while (TextFormat::Parse(&stream, &event)) {
            mEvents.push_back(event);
        }

        if (stream.ByteCount() != eventString.size()) {
...
private:
    ...
    std::deque<pb::RecordedEvent> mEvents;
};
