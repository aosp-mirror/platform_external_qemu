...
AutomationControllerImpl::AutomationControllerImpl(
    ...
    mVideoInjectionController = VideoInjectionController::create(sendMessageCallback);
}
...
AutomationControllerImpl::getNextVideoInjectionCommand(
    ...
    return mVideoInjectionController->getNextCommand(std::move(previousResult));
};
...
VideoInjectionResult AutomationControllerImpl::videoInjectionHandler(
        ...
        uint32_t asyncId) {
    AutoLock lock(mLock);
    return mVideoInjectionController->addCommand(pipe, command, asyncId);
}
