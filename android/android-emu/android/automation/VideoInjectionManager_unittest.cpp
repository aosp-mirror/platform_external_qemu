...
class VideoInjectionManagerTest : public testing::Test {
protected:
    VideoInjectionManagerTest() : mTestSystem("/", System::kProgramBitness) {
    ...
    void SetUp() override {
        mManager = VideoInjectionManager::create(
    ...
    std::unique_ptr<VideoInjectionManager> mManager;
};

TEST_F(VideoInjectionManagerTest, AddCommand) {
    ...
    pb::VideoInjectionCommand testCommand;
    std::string testCommandString;
    testCommand.SerializeToString(&testCommandString);
    EXPECT_THAT(mManager->addCommand(pipe, testCommandString, 123),
                IsErr(VideoInjectionError::InvalidCommand));

    testCommand.mutable_display_default_frame();
    testCommand.SerializeToString(&testCommandString);
    EXPECT_THAT(mManager->addCommand(pipe, testCommandString, 123),
                IsOk());
}

TEST_F(VideoInjectionManagerTest, GetNextCommand) {
    android::AsyncMessagePipeHandle pipe;
    pipe.id = 123;
    ...
    mManager->addCommand(pipe, testCommandString, 1);
    mManager->addCommand(pipe, testCommandString, 2);
