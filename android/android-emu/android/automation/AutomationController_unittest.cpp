...
TEST_F(AutomationControllerTest, Listen) {
    ...
    EXPECT_CALL(*this, offworldSendResponse(Eq(pipe), EqualsProto(R"(
...
TEST_F(AutomationControllerTest, ListenEvents) {
    ...
    EXPECT_CALL(*this, offworldSendResponse(Eq(pipe), EqualsProto(R"(
        ...
        async {
            ...
            automation {
                event_generated {
                    event: "event_time { timestamp: 500000000 } stream_type: PHYSICAL_MODEL physical_model { type: POSITION current_value { data: [0.5, 10, 0] } } "
                }
            }
        }
    )")))
...
TEST_F(AutomationControllerTest, TimeOnlyReplay) {
    ...
    EXPECT_THAT(mController->replayEvent(
                ...
                IsOk());
    EXPECT_CALL(*this, offworldSendResponse(Eq(pipe), Partially(EqualsProto(R"(
