...
TEST(AvdUtil, propertyFile_findProductName) {
  ...
  EXPECT_TRUE(propertyFile_findProductName(&fd, google_names, false));
  ...
  EXPECT_TRUE(propertyFile_findProductName(&fd, phone_names, false));

  EXPECT_EQ(0, fileData_initFromMemory(&fd, testFileAndroidAuto, strlen(testFileAndroidAuto)));
  const char *car_names[] = {"car_emu"};
  EXPECT_TRUE(propertyFile_findProductName(&fd, car_names, true));

  EXPECT_EQ(0, fileData_initFromMemory(&fd, testFileRandom, strlen(testFileRandom)));
  EXPECT_FALSE(propertyFile_findProductName(&fd, google_names, false));
  EXPECT_FALSE(propertyFile_findProductName(&fd, phone_names,  false));
  EXPECT_FALSE(propertyFile_findProductName(&fd, car_names, true));
}