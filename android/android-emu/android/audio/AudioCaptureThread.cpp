...
public:
    AudioCaptureThreadImpl(AudioCaptureThread::Callback* cb,
        ...
        mBufferQueue.reset(new BufferQueue<AudioPacket, kMaxPackets>(mLock, mNbSamples * mChannels * sz, mBits));
    }
...
AudioCaptureThread* AudioCaptureThread::create(Callback* cb,
                                               ...
                                               int bits,
                                               int nchannels) {
    return new AudioCaptureThreadImpl(cb, opaque, sampleRate, nbSamples, bits, nchannels);
}
}  // namespace audio
}  // namespace android
