// Copyright (C) 2017 The Android Open Source Project
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include "android/audio/AudioCaptureThread.h"

#include "android/audio/AudioPacket.h"
#include "android/base/containers/BufferQueue.h"
#include "android/base/synchronization/Lock.h"
#include "android/base/system/System.h"
#include "android/emulation/AudioCapture.h"
#include "android/emulation/AudioCaptureEngine.h"
#include "android/utils/debug.h"

#include <memory>

namespace android {
namespace audio {

using android::base::BufferQueue;
using Result = android::base::BufferQueueResult;
using android::emulation::AudioCaptureEngine;
using android::emulation::AudioCapturer;

namespace {
class AudioCapturerImpl : public AudioCapturer {
public:
    typedef int(Callback)(void* buf, int size);
    explicit AudioCapturerImpl(std::function<AudioCapturerImpl::Callback> cb,
                               int freq,
                               int bits,
                               int nchannels)
        : AudioCapturer(freq, bits, nchannels), mCallback(cb) {
        assert(mCallback);
    }

    // Start the audio capturer
    bool start() {
        auto engine = AudioCaptureEngine::get();
        if (!engine) {
            return false;
        }

        return engine->start(this) == 0;
    }

    // Stop the audio capturer
    bool stop() {
        auto engine = AudioCaptureEngine::get();
        if (!engine) {
            return false;
        }

        return engine->stop(this) == 0;
    }

    virtual ~AudioCapturerImpl() {
        // Doesn't hurt to stop the audio capture engine even if we stopped it
        // already
        auto engine = AudioCaptureEngine::get();
        if (engine) {
            engine->stop(this);
        }
    }

    // Called from the qemu io thread with the audio data
    virtual int onSample(void* buf, int size) override {
        return mCallback(buf, size);
    }

private:
    std::function<AudioCapturerImpl::Callback> mCallback;
};  // class AudioCapturerImpl

// Real implementation of AudioCaptureThread
class AudioCaptureThreadImpl : public AudioCaptureThread {
public:
    AudioCaptureThreadImpl(std::function<AudioCaptureThread::Callback> cb,
                           int sampleRate,
                           int nbSamples,
                           int bits,
                           int nchannels)
        : mCallback(cb),
          mSampleRate(sampleRate),
          mNbSamples(nbSamples),
          mBits(bits),
          mChannels(nchannels),
          mDataQueue(kMaxPackets, mDataLock),
          mFreeQueue(kMaxPackets, mFreeLock) {
        assert(bits == 8 || bits == 16);
        assert(mCallback);
        size_t szBytes = bits == 8 ? 1 : 2;
        // Prefill the free queue with empty packets
        mFreeLock.lock();
        for (int i = 0; i < kMaxPackets; ++i) {
            mFreeQueue.pushLocked(std::move(AudioPacket(mNbSamples * mChannels * szBytes)));
        }
        mFreeLock.unlock();
    }

    ~AudioCaptureThreadImpl() {}

    intptr_t main() final {
        // Start the audio capturer to start receiving audio frames.
        mAudioCapturer.reset(new AudioCapturerImpl(
                [this](void* buf, int size) -> int {
                    return onSample(buf, size);
                },
                mSampleRate, mBits, mChannels));
        if (!mAudioCapturer->start()) {
            derror("Unable to start audio capturer");
            return -1;
        }

        bool finished = false;
        while (!finished) {
            // Consumer
            AudioPacket pkt;
            mDataLock.lock();
            // Blocks until either we get a packet or the queue is closed.
            auto popRes = mDataQueue.popLocked(&pkt);
            mDataLock.unlock();

            switch (popRes) {
                case Result::Error:
                    // Queue was closed.
                    finished = true;
                    break;
                case Result::Ok: {
                    mCallback(pkt.dataVec.data(), pkt.dataVec.size(), pkt.tsUs);
                    mFreeLock.lock();
                    // Blocks until there is space in the queue or is closed.
                    mFreeQueue.pushLocked(std::move(pkt));
                    mFreeLock.unlock();
                    break;
                }
                case Result::TryAgain:
                default:
                    break;
            }
        }

        return 0;
    }

    void stop() {
        if (!mAudioCapturer) {
            return;
        }

        mAudioCapturer->stop();

        mDataLock.lock();
        mDataQueue.closeLocked();
        mDataLock.unlock();

        mFreeLock.lock();
        mFreeQueue.closeLocked();
        mFreeLock.unlock();
    }

private:
    // Called from the qemu io thread. Keep this as fast as possible.
    int onSample(void* buf, int size) {
        if (buf == nullptr || size <= 0) {
            return -1;
        }

        // Producer
        int remaining = size;
        uint8_t* p = static_cast<uint8_t*>(buf);
        while (remaining > 0) {
            // Get the timestamp first as dequeuing the buffer and memcpying
            // will take some time.
            auto tsUs = android::base::System::get()->getHighResTimeUs();
            AudioPacket pkt;
            mFreeLock.lock();
            auto popRes = mFreeQueue.tryPopLocked(&pkt);
            mFreeLock.unlock();

            switch (popRes) {
                case Result::Ok: {
                    int sz = remaining > pkt.dataVec.capacity()
                                     ? pkt.dataVec.capacity()
                                     : remaining;
                    pkt.dataVec.clear();
                    pkt.dataVec.insert(pkt.dataVec.end(), p, p + sz);
                    p += sz;
                    pkt.tsUs = tsUs;
                    remaining -= sz;
                    mDataLock.lock();
                    mDataQueue.tryPushLocked(std::move(pkt));
                    mDataLock.unlock();
                    break;
                }
                case Result::TryAgain:
                case Result::Error:
                default:
                    // Queue was closed or empty. Just discard the packet.
                    return 0;
            }
        }

        return 0;
    }

    std::function<AudioCaptureThread::Callback> mCallback;
    int mSampleRate = 0;
    int mNbSamples = 0;
    int mBits = 0;
    int mChannels = 0;
    std::unique_ptr<AudioCapturerImpl> mAudioCapturer;
    // mDataQueue contains filled audio packets and mFreeQueue has available
    // audio packets that the producer can use. The workflow is as follows:
    // Producer:
    //   1) get an audio packet from mFreeQueue
    //   2) write data into packet
    //   3) push packet to mDataQueue
    // Consumer:
    //   1) wait for audio packet in mDataQueue
    //   2) process audio packet
    //   3) Push packet back to mFreeQueue
    static constexpr int kMaxPackets = 3;
    android::base::Lock mDataLock;
    android::base::Lock mFreeLock;
    BufferQueue<AudioPacket> mDataQueue;
    BufferQueue<AudioPacket> mFreeQueue;
};  // class AudioCaptureThreadImpl
}  // namespace

AudioCaptureThread* AudioCaptureThread::create(
        std::function<AudioCaptureThread::Callback> cb,
        int sampleRate,
        int nbSamples,
        int bits,
        int nchannels) {
    return new AudioCaptureThreadImpl(cb, sampleRate, nbSamples, bits,
                                      nchannels);
}
}  // namespace audio
}  // namespace android
