...
public:
    explicit AudioCapturerImpl(AudioCaptureThread::Callback cb, void* opaque, int freq, int bits, int nchannels) :
...
public:
    AudioCaptureThreadImpl(Callback* cb, void* opaque, int sampleRate, int bits, int nchannels) :
            mCallback(cb),
            mOpaque(opaque),
    ...
    intptr_t main() override final {
        ...
        mAudioCapturer.reset(new AudioCapturerImpl(
                    &AudioCaptureThreadImpl::onSample,
                    this, mSampleRate, mBits, mChannels));
...
private:
    ...
    static int onSample(void* opaque, void* buf, int size) {
            AudioCaptureThreadImpl* t = reinterpret_cast<AudioCaptureThreadImpl*>(opaque);
...
AudioCaptureThread* AudioCaptureThread::create(Callback* cb,
                                               ...
                                               int bits,
                                               int nchannels) {
    return new AudioCaptureThreadImpl(cb, opaque, sampleRate,
                                        bits, nchannels);
}
}  // namespace audio
}  // namespace android
