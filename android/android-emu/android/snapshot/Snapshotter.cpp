...
void Snapshotter::fillSnapshotMetrics(pb::EmulatorSnapshot* snapshot,
    ...
    fprintf(stderr, "%s: snapshot total sz %lld ram %lld tex %lld\n", __func__,
            int64_t(stats.diskSize +
                stats.ramSize +
                stats.texturesSize) / (1048576),
            int64_t(stats.ramSize) / (1048576),
            int64_t(stats.texturesSize) / (1048576));

    if (stats.forSave) {
    fprintf(stderr, "%s: save times: %llu %lld %lld\n", __func__,
            uint64_t(stats.durationMs),
            int64_t(stats.ramDurationMs),
    ...
    fprintf(stderr, "%s: load times: %llu %lld %lld\n", __func__,
            uint64_t(stats.durationMs),
            int64_t(stats.ramDurationMs),
            int64_t(stats.texturesDurationMs));
...
Snapshotter::SnapshotOperationStats Snapshotter::getSaveStats(const char* name,
    ...
    fprintf(stderr, "%s: snapshot total sz %lld ram %lld tex %lld\n", __func__,
            int64_t(diskSize +
                ramSize +
                texturesSize) / (1048576),
            int64_t(ramSize) / (1048576),
            int64_t(texturesSize) / (1048576));
    ...
    fprintf(stderr, "%s: save times: %llu %lld %lld\n", __func__,
            uint64_t(durationMs),
            int64_t(ramDurationMs),
            int64_t(texturesDurationMs));
    return {
        true /* for save */,
        std::string(name),
        durationMs,
