...
#define _RECV_PACKET_OK(packet) { _RECV_PACKET(packet); _SEND_PACKET(ok_out); }
    ...
    std::vector<uint64_t> exception_reference_type_ids(0);
    {
        ...
        // Class by signature
        {
            ...
            uint8_t* data_end = writeStrToBuffer(packet_out.data + 11, kExceptionClass);
            ...
            for (auto& id : exception_reference_type_ids) {
                data_ptr += 1;
                id = readValFromBuffer<uint64_t>(data_ptr, id_size.reference_typ_id_size);
...
#if DEBUG >= 1
        {
            ...
            for (const auto& clazz: classes.classes) {
                DD("class %s id 0x%x\n", clazz.signature.c_str(), (uint32_t)clazz.type_id);
...

    {
        ...
        jdwp_header.length = 11 + set_request.writeToBuffer(packet_out.data + 11);
        ...
        for (const auto& id: exception_reference_type_ids) {
            jdwp_header.length = 11 + set_request.writeToBuffer(
                packet_out.data + 11, id, &id_size);
            jdwp_header.id = jdwp_id ++;
            jdwp_header.writeToBuffer(packet_out.data);
            packet_out.mesg.data_length = jdwp_header.length;
            _SEND_PACKET_OK(packet_out);
    ...
    while (1) {
        ...
        if (reply.data[11] == 0x2) { // Thread suspend all
            ...
            android::base::ThreadLooper::runOnMainLooper([&snapshot_done, &snapshot_signal, &snapshot_lock]() {
            ...
            snapshot_lock.lock();
            snapshot_signal.wait(&snapshot_lock, [&snapshot_done](){ return snapshot_done; });
