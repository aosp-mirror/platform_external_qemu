...
static bool sign_token(RSA* key_rsa, const char* token, int token_size, char* sig, int& len) {

    if (token_size != TOKEN_SIZE) {
        DD("Unexpected token size %d\n", token_size);
    }

    if (!RSA_sign(NID_sha1, (const unsigned char*)token, (size_t)token_size, (unsigned char*)sig,
                (unsigned int*)&len, key_rsa)) {
        return false;
    }
...
static bool sign_auth_token(const char* token, int token_size, char* sig, int & siglen) {
    // read key
    ...
    if (!read_key(key_path.c_str(), keys)) {
        ...
        std::string key_path = pj(android::base::System::get()->getAppDataDirectory(),
        ".android", "adbkey");
        if (!read_key(key_path.c_str(), keys)) {
            return false;
        }
...
#define _RECV_PACKET(packet)           \
    ...
    }
    {
        ...
        if (pack_recv.mesg.command == ADB_AUTH) {
            ...
            if (!sign_auth_token((const char*)pack_recv.data.data(), pack_recv.mesg.data_length,
...
#define _RECV_PACKET_OK(packet) \
    ...
    std::vector<uint64_t> exception_reference_type_ids(0);
    {
        ...
        JdwpCommandHeader jdwp_query = {
                .length = 11,
                .flags = 0,
                .command_set = CommandSet::VirtualMachine,
                .command = VirtualMachineCommand::IDSizes,
        };
        // Not all compilers support putting the following line into the
        // initializer
    ...
    while (1) {
        ...
        if (reply.mesg.data_length > 11 &&
            ...
            JdwpCommandHeader jdwp_command = {
                    ...
                    .flags = 0,
                    .command_set = CommandSet::VirtualMachine,
                    .command = VirtualMachineCommand::Resume,
            };
