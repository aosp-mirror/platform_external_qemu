...
#define _RECV_PACKET(packet)           \
    ...
    }
    {
        ...
        // TODO: authenticate ADB for playstore images
...
#define _RECV_PACKET_OK(packet) \
    ...
    std::vector<uint64_t> exception_reference_type_ids(0);
    {
        ...
        jdwp_query.flags = 0;
        jdwp_query.command_set = CommandSet::VirtualMachine;
        jdwp_query.command = VirtualMachineCommand::IDSizes;
...
#if DEBUG >= 1
        {
            ...
            _SEND_PACKET_OK(packet_out);
            _RECV_PACKET_OK(reply);
            JdwpCommandHeader class_header(reply.data.data());
