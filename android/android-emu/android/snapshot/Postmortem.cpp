...
static bool recvPacket(int s, apacket* packet) {
    ...
    if (packet->mesg.data_length &&
        !socketRecvAll(s, packet->data, packet->mesg.data_length)) {
        return false;
    }
    return true;
}
...
#define _SEND_PACKET(packet)                                                   \
    {                                                                          \
        if (!base::socketSendAll(s, (const void*)&packet, packetSize(packet))) \
            return false;                                                      \
    }
...
#define _RECV_PACKET(packet)           \
    ...
    }
    {
        ...
        /*if (mesg.command == ADB_AUTH) {
            int siglen = 20;
            sign_auth_token((const char*)pack_recv.data, mesg.data_length,
        (char*)pack_send.data, siglen);
...

    {
        ...
        jdwp_connect.mesg.data_length =
                sprintf((char*)jdwp_connect.data, "jdwp:%d", pid) + 1;
...
#define _RECV_PACKET_OK(packet) \
    ...
    // Send handshake
    packet_out.mesg.data_length =
            sprintf((char*)packet_out.data, "JDWP-Handshake");
    ...
    std::vector<uint64_t> exception_reference_type_ids(0);
    {
        ...
        // Class by signature
        {
            jdwp_query.command = 0x2;
            uint8_t* data_end =
                    writeStrToBuffer(packet_out.data + 11, kExceptionClass);
            jdwp_query.length = data_end - packet_out.data;
            packet_out.mesg.data_length = jdwp_query.length;
            jdwp_query.writeToBuffer(packet_out.data);
            _SEND_PACKET_OK(packet_out);
            _RECV_PACKET_OK(reply);
            uint8_t* data_ptr = reply.data + 11;
...

    {
        ...
        jdwp_header.length =
                11 + set_request.writeToBuffer(packet_out.data + 11);
        jdwp_header.id = jdwp_id++;
        jdwp_header.flags = 0;
        jdwp_header.command_set = 0xf;
        jdwp_header.command = 0x1;
        packet_out.mesg.data_length = jdwp_header.length;
        jdwp_header.writeToBuffer(packet_out.data);
        _SEND_PACKET_OK(packet_out);
        _RECV_PACKET_OK(reply);
        ...
        for (const auto& id : exception_reference_type_ids) {
            jdwp_header.length =
                    11 + set_request.writeToBuffer(packet_out.data + 11, id,
                                                   &id_size);
            jdwp_header.id = jdwp_id++;
            jdwp_header.writeToBuffer(packet_out.data);
            packet_out.mesg.data_length = jdwp_header.length;
    ...
    while (1) {
        apacket reply;
        _RECV_PACKET_OK(reply);
        // TODO: close the socket when disconnected from guest
        if (reply.data[11] == 0x2) {  // Thread suspend all
            ...
            jdwp_command.length = 11;
            jdwp_command.id = jdwp_id++;
            jdwp_command.flags = 0;
            jdwp_command.command_set = 0x1;
            jdwp_command.command = 0x9;
            packet_out.mesg.data_length = jdwp_command.length;
            jdwp_command.writeToBuffer(packet_out.data);
            _SEND_PACKET_OK(packet_out);
            _RECV_PACKET_OK(reply);
            return true;
        }
