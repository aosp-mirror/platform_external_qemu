...
std::set<void*> copiedPtrs;
bool MemoryAccessWatch::fillPage(void* ptr, size_t length, const void* data, uint64_t gpa, bool fromHv) {
    if (gpa == 0x185f6000) {
        fprintf(stderr, "%s: the gpa in question is 0x185f6000\n", __func__);
        fprintf(stderr, "%s: the hva is %p\n", __func__, ptr);
    }
   
    if (fromHv) {
        mprotect(ptr, length, PROT_READ | PROT_WRITE | PROT_EXEC);
        if (copiedPtrs.find(ptr) == copiedPtrs.end()) {
            memcpy(ptr, data, length);
            copiedPtrs.insert(ptr);
        }
