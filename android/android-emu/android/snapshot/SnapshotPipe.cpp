    ...
    static void handleSnapshotPb(const offworld::GuestSend& guestSend,
        ...
        switch (snapshotPb.function_case()) {
            ...
            case MS::FunctionCase::kGotoCheckpointParam: {
                ...
                int32_t recvSize = guestRecv.ByteSize();
                sMetaData.resize(recvSize + sizeof(recvSize));
                memcpy(sMetaData.data(), (void*)&recvSize, sizeof(recvSize));
                guestRecv.SerializeToArray(sMetaData.data() + sizeof(recvSize),
                        recvSize);
