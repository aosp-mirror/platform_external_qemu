...
void RamSaver::savePage(int64_t blockOffset,
    ...
    if (block.pages.empty()) {
        ...
        // Split it up into work units of kCompressBufferBatchSize.
        int32_t start = 0;
        int32_t end = 0;
        for (int32_t i = 0; i != block.nonzeroChangedPages.size(); ++i) {
            if (i == block.nonzeroChangedPages.size() - 1 || (i - start) == kCompressBufferBatchSize) {
                end = i + 1;
...
void RamSaver::writePage(WriteInfo&& wi) {
    ...
    if (incremental()) {
        for (int32_t nzcIndex = wi.nonzeroChangedIndexStart; nzcIndex < wi.nonzeroChangedIndexEnd; ++nzcIndex) {
            int32_t pageIndex = block.nonzeroChangedPages[size_t(nzcIndex)];
            auto& page = block.pages[size_t(pageIndex)];
            ...
            if (page.filePos == 0) {
                if (auto gapPos = mIncStats.measure(StatTime::GapTrackingWriter, [&] {
                            return mGaps->allocate(page.sizeOnDisk);
                            })) {
                    page.filePos = *gapPos;
                    mIncStats.count(StatAction::ReusedPos);
                }
