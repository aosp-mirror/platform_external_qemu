...
RamSaver::RamSaver(base::StdioStream&& stream, Flags flags)
    : mStream(std::move(stream)), mStreamFd(fileno(mStream.get())), mFlags(flags) {
...
bool RamSaver::handlePageSave(QueuedPageInfo&& pi) {
    if (pi.blockIndex == kStopMarkerIndex) {
        mCompressor.clear();
        mIndex.startPosInFile = mCurPos.load(std::memory_order_relaxed);
...
void RamSaver::writeIndex() {
    ...
    mStream.write(stream.data().data(), stream.data().size());
...
void RamSaver::writePage(const QueuedPageInfo& pi,
    ...
    base::pwrite(mStreamFd, dataPtr, dataSize, page.filePos);
}
