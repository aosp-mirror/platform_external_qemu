...
extern "C" int android_check_snapshot_loadable(const char* snapshot_name) {
    ...
    if (exportedSnapshot) {
        ...
        do {
            temp_dir = temp_parent / std::to_string(rand());
        } while (!create_directory(temp_dir));
        android::base::TarReader tr(temp_dir.string(), *stream);
        ...
        printf("tmp folder %s\n", temp_dir.c_str());
        /*std::ifstream pbStream(temp_dir / "snapshot.pb");
        emulator_snapshot::Snapshot snapshotPb;
        snapshotPb.ParseFromIstream(&pbStream);*/
