...
bool Snapshot::save() {
    ...
    if (mName != Quickboot::kDefaultBootSnapshot &&
        ...
        *parentSnapshot != Quickboot::kDefaultBootSnapshot) {
        // If mName == *parentSnapshot, we overwrote the current
        // snapshot. For now, just punt on this situation;
        // don't set the parent to self to prevent loops.
        // We won't use that in generic snapshots anyway
        // (every snapshot is new and unique)
