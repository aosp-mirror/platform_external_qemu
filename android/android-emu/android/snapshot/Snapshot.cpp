...
static void fillImageInfo(pb::Image::Type type,
    ...
    *image->mutable_path() = path;
    if (path.empty() || !path_is_regular(NullTerminated(path).c_str())) {
    ...
    struct stat st;
    if (!android_stat(NullTerminated(path).c_str(), &st)) {
...
static bool verifyImageInfo(pb::Image::Type type,
    ...
    const bool realPresent =
            !path.empty() && path_is_regular(NullTerminated(path).c_str());
    ...
    if (android_stat(NullTerminated(path).c_str(), &st) != 0) {
