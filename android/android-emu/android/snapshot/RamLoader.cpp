...
void RamLoader::touchAllPages() {
    ...
    if (!mLazyLoadingFromFileBacking) return;
    for (const auto& block : mIndex.blocks) {
        fprintf(stderr, "%s: host ptr: %p totalSz 0x%llx\n", __func__, block.ramBlock.hostPtr, block.ramBlock.totalSize);

        ContiguousRangeMapper pageToucher([](uintptr_t start, uintptr_t size) {
            android::base::memoryHint((void*)start, size, MemoryHint::Touch);
        }, kReadChunkSize);

        uint8_t* hostPtr = block.ramBlock.hostPtr;
        for (int64_t i = 0; i < block.ramBlock.totalSize; i += kReadChunkSize) {
            pageToucher.add((uintptr_t)hostPtr + i, kReadChunkSize);
        }
    }
}
