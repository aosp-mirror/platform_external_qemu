...
RamLoader::RamBlockStructure::~RamBlockStructure() {
    for (auto& block : blocks) {
        if (block.path) {
            free((void*)block.path);
        }
    }
}

static RamBlock sDupRamBlockPath(const RamBlock& block) {
    RamBlock res = block;
    if (block.path) {
        res.path = android::base::strDup(block.path);
    }
    return res;
}
...
void RamLoader::applyRamBlockStructure(const RamBlockStructure& blockStructure) {
    ...
    for (const auto ramBlock : blockStructure.blocks) {
        mIndex.blocks.push_back({ sDupRamBlockPath(ramBlock), {}, {}});
    }
}
...
void RamLoader::readBlockPages(base::Stream* stream,
    ...
    std::string activeMemPath = block.ramBlock.path ? block.ramBlock.path : "";
