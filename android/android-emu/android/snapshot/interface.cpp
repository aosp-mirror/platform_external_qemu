...
AndroidSnapshotRamMapMode androidSnapshot_getRamMapInfo() {
    bool hasRamMap = Snapshotter::get().hasRamMap();
    bool isRamMapShared = Snapshotter::get().isRamMapShared();
    if (hasRamMap) {
        if (isRamMapShared) {
            return SNAPSHOT_RAM_MAP_SHARED;
        } else {
            return SNAPSHOT_RAM_MAP_PRIVATE;
        }
    } else {
        return SNAPSHOT_RAM_MAP_NONE;
    }
}
