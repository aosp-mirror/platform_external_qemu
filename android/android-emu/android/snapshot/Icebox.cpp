...
#define _RECV_PACKET(packet)           \
...

    {
        ...
        // "host::features=abb_exec,fixed_push_symlink_timestamp,abb,stat_v2,apex,shell_v2,fixed_push_mkdir,cmd";
        const char* kCnxnData = "";
        to_guest.mesg.command = ADB_CNXN;
        to_guest.mesg.arg0 = 0;
        to_guest.mesg.arg1 = 256 * 1024;
        to_guest.mesg.data_length = strlen(kCnxnData) + 1;
...
#define _RECV_PACKET(packet)                           \
    {                                                  \
    ...
    }
        {
            ...
            connect.mesg.arg1 = 0;
            connect.mesg.data_check = 0;
            connect.mesg.magic = connect.mesg.command ^ 0xffffffff;
            connect.mesg.data_length = cmd_str.length() + 1;
...
#define _RECV_PACKET_OK(packet) \
    ...
    while (1) {
        ...
        if (reply.mesg.data_length > 11 &&
            ...
            // Sleep one second to wait for the guest to process the last ADB_OKAY
            // TODO(yahan): do it in a better way other than sleep
            sleep(1);
