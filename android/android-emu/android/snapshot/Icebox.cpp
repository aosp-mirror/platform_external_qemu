...
bool track_async(int pid, const std::string snapshot_name, int max_snapshot_number) {
    if (s_workerThread) {
        if (!s_workerThread->tryWait(nullptr)) {
            return false;
        }
    }
    s_workerThread.reset(new android::base::FunctorThread([pid, snapshot_name, max_snapshot_number] {
...
#define _RECV_REPLY_PACKET_OK(packet, _id)                    \
    ...
    while (1) {
        ...
        if (reply.mesg.data_length > 11 &&
            ...
            if (max_snapshot_number == -1 || snapshot_num < max_snapshot_number) {
