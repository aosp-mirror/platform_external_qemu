...
bool track(int pid, const std::string snapshot_name, int max_snapshot_number) {
    ...
    reporter->report([max_snapshot_number](android_studio::AndroidStudioEvent* event) {
        auto icebox = event->mutable_emulator_details()->add_icebox();
        icebox->mutable_start_icebox()->set_max_snapshot_number(max_snapshot_number);
    });
    uint32_t local_id = s_id.fetch_add(1);
    uint32_t remote_id = 0;
    // It is ok for some non-thread safe to happen during the following loop,
...
#define _RECV_REPLY_PACKET_OK(packet, _id)                    \
                                ...
                                [&new_snapshot_name, reporter]() {
                                    ...
                                    if (snapshotSkipped) {
                                        ...
                                        if (reporter->isReportingEnabled()) {
                                            ...
                                            reporter->report([stats](android_studio::AndroidStudioEvent* event) {
                                                auto icebox = event->mutable_emulator_details()->add_icebox();
                                                snapshot::Snapshotter::fillSnapshotMetrics(icebox->mutable_take_snapshot()->mutable_snapshot(), stats);
                                            });
                                        }
                                    }
    ...
    reporter->report([max_snapshot_number, snapshot_num](android_studio::AndroidStudioEvent* event) {
        auto icebox = event->mutable_emulator_details()->add_icebox();
