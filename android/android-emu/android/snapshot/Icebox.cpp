                ...
                android::base::ThreadLooper::runOnMainLooper(
                        [&snapshot_done, &snapshot_signal, &snapshot_lock,
                         &new_snapshot_name]() {
                            ...
                            bool snapshotSkipped = getConsoleAgents()->vm->isSnapshotSaveSkipped();
                            if (snapshotSkipped) {
                                D("Temporarily re-enabling snapshot");
                                getConsoleAgents()->vm->setSkipSnapshotSave(false);
                            }
                            ...
                            D("Snapshot done, result %d (expect %d)", result,
                              SNAPSHOT_STATUS_OK);
                            if (snapshotSkipped) {
                                getConsoleAgents()->vm->setSkipSnapshotSave(true);
                            }
