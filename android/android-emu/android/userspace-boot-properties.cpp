...
getUserspaceBootProperties(const AndroidOptions* opts,
    ...
    const char* qemuHwcodecAvcdec;
    const char* qemuHwcodecVpxdec;
    ...
    if (fc::isEnabled(fc::AndroidbootProps)) {
        ...
        qemuHwcodecAvcdec = "androidboot.qemu.hwcodec.avcdec";
        qemuHwcodecVpxdec = "androidboot.qemu.hwcodec.vpxdec";
    } else {
        ...
        qemuHwcodecAvcdec = "qemu.hwcodec.avcdec";
        qemuHwcodecVpxdec = "qemu.hwcodec.vpxdec";
    }
    ...
    if (opts->no_boot_anim) {
        params.push_back({bootanimProp, "0"});
    }
    ...
    params.push_back({"androidboot.logcat", "*:V"});
    ...
    if (fc::isEnabled(fc::HardwareDecoder)) {
        params.push_back({qemuHwcodecAvcdec, "2"});
        params.push_back({qemuHwcodecVpxdec, "2"});
    }
