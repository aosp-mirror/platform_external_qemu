...
struct JdwpEventRequestSet {
    ...
    int writeToBuffer(uint8_t* buffer, uint64_t exception_id = 0,
        ...
        *(buffer++) = suspend_policy;
        ...
        if (exception_id) {
            ...
            // uncaught doesn't really work
            *(buffer++) = 8;
            buffer = writeValToBuffer(buffer, exception_id, id_size->reference_typ_id_size);
            *(buffer++) = 1;
            *(buffer++) = 1;
        }
...
struct JdwpAllClasses {
    ...
    void parseFrom(const uint8_t* buffer, const JdwpIdSize& id_size) {
        ...
        for (auto& clazz : classes) {
            ...
            clazz.type_id = readValFromBuffer<uint64_t>(buffer,
                id_size.reference_typ_id_size);
            buffer += id_size.reference_typ_id_size;
            size_t string_len = readValFromBuffer<size_t>(buffer,
                4);
            buffer += 4;
