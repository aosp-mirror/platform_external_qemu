...
using android::recording::VorbisCodecHelper;
using android::recording::VP9CodecHelper;
...
bool ScreenRecorder::startRecordingWorker() {
    ...
    auto audioProducer = android::recording::createAudioProducer(
            kAudioSampleRate, kSrcNumSamples, kInSampleFormat, kNumChannels);
    CodecParams audioParams = {0};
    audioParams.sample_rate = kAudioSampleRate;
    audioParams.bitrate = kAudioBitrate;
    VorbisCodecHelper audioCodecHelper(
            std::move(audioParams),
            toAVSampleFormat(audioProducer->getFormat().audioFormat));
    if (!ffmpegRecorder->addAudioTrack(audioProducer, &audioCodecHelper)) {
