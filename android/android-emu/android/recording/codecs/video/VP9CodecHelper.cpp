...
#include "android/recording/codecs/video/VP9CodecHelper.h"
...
VP9CodecHelper::VP9CodecHelper(CodecParams&& p,
                               uint32_t fbWidth,
                               uint32_t fbHeight,
                               AVPixelFormat fbFormat)
    : CodecHelper(AV_CODEC_ID_VP9, std::move(p)),
      mFbWidth(fbWidth),
      mFbHeight(fbHeight),
      mFbFormat(fbFormat) {}

VP9CodecHelper::~VP9CodecHelper() {}

// Configures the encoder. Returns true if successful, false otherwise.
bool VP9CodecHelper::configAndOpenEncoder(const AVFormatContext* oc,
...
bool VP9CodecHelper::initSwxContext(const AVCodecContext* c,
