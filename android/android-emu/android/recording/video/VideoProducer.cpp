...
public:
    VideoProducer(uint32_t fbWidth,
        ...
        if (dpyAgent) {
            mIsGuestMode = true;
            mGuestReadbackWorker.reset(
                    GuestReadbackWorker::create(dpyAgent, mFbWidth, mFbHeight));
            mVideoFormat = mGuestReadbackWorker->getPixelFormat();
        } else {
            // The host framebuffer is always using RGBA8888.
            mVideoFormat = VideoFormat::RGBA8888;
        }

        // Prefill the free queue with empty frames
        auto sz = mFbWidth * mFbHeight * getVideoFormatSize(mVideoFormat);
        for (int i = 0; i < kMaxFrames; ++i) {
            Frame f(sz);
            f.videoFormat = mVideoFormat;
...
private:
    ...
    VideoFormat mVideoFormat = VideoFormat::INVALID_FMT;
...
Producer* createVideoProducer(uint32_t fbWidth,
                              uint32_t fbHeight,
                              uint8_t fps,
                              const QAndroidDisplayAgent* dpyAgent) {
    return new VideoProducer(fbWidth, fbHeight, fps, dpyAgent);
}
