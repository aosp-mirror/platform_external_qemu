...
public:
 struct VideoInfo {
    ...
    intptr_t main() final {
        ...
        int err = mMemory.open(O_CREAT | O_RDWR, user_read_only);
        if (err != 0) {
            LOG(FATAL) << "Unable to open shared memory due to " << errno;
            return 0;
        }
        ...
        while (mRecording) {
            ...
            if (px) {
                LOG(VERBOSE)  << "sending video frame: " <<  i++;
            ...
            if (px) {
                LOG(VERBOSE)  << "Video frame sent in [" << (newTimeMs - currTimeMs) << "] ms";
            }

            if (newTimeMs - currTimeMs < mTimeDeltaMs) {
                LOG(VERBOSE)
                        << "Video sending thread sleeping for ["
                        << (mTimeDeltaMs - newTimeMs + currTimeMs) << "] ms";
                android::base::System::get()->sleepMs(mTimeDeltaMs - newTimeMs +
                                                      currTimeMs);
            }
        }

        LOG(INFO) << "Finished sending video frames";
