...
private:
    static void onPost(void* opaque, int x, int y, int width, int height) {
        //        D("[%d] %s(x=%d, y=%d, w=%d, h=%d)", ++globals.frameCount,
        //        __func__, x, y,
        //          width, height);
...
GuestReadbackWorker* GuestReadbackWorker::create(
        const QAndroidDisplayAgent* agent,
        uint32_t fbWidth,
        uint32_t fbHeight) {
    return new GuestReadbackWorkerImpl(agent, fbWidth, fbHeight);
}
