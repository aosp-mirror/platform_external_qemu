...
//
// When screen_recorder_stop() is called, this callback will get called, with
// success set to 0. There is some time elapsed when we want to stop recording
// and when the encoding is actually finished, so we'll get a second call to the
// callback once the encoding is finished, with success set to 1. If any errors
// occur while stopping the recording, success will be set to -1.
typedef void (*RecordingStoppedCallback)(void* opaque, int success);
...
extern void screen_recorder_init(int isGuestMode, int w, int h);
// Starts recording the screen. When stopped, the file will be saved as
// |info->filename|. Returns true if recorder started recording, false if it failed.
