...
int SensorLocationEventProviderImpl::createSensorEvent(AndroidSensor sensor,
    ...
    if (timestampType == Type::FieldInfo_Type_INT_32) {
        auto timestampPtr = reinterpret_cast<int32_t*>(data + timestampOffset);
        recordedEvent.set_delay(static_cast<uint64_t>(*timestampPtr));
    } else if (timestampType == Type::FieldInfo_Type_INT_64) {
        auto timestampPtr = reinterpret_cast<int64_t*>(data + timestampOffset);
        recordedEvent.set_delay(static_cast<uint64_t>(*timestampPtr));
    } else if (timestampType == Type::FieldInfo_Type_FLOAT) {
        auto timestampPtr = reinterpret_cast<float*>(data + timestampOffset);
        recordedEvent.set_delay(static_cast<uint64_t>(*timestampPtr));
    } else if (timestampType == Type::FieldInfo_Type_DOUBLE) {
        auto timestampPtr = reinterpret_cast<double*>(data + timestampOffset);
        recordedEvent.set_delay(static_cast<uint64_t>(*timestampPtr));
    } else {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (valueOffset >= 0) {
            ...
            if (sensorValueType == Type::FieldInfo_Type_INT_32) {
                auto valuePtr = reinterpret_cast<int32_t*>(data + valueOffset);
                float floatValue = static_cast<float>(*valuePtr);
                valueObject->add_data(floatValue);
            } else if (sensorValueType == Type::FieldInfo_Type_INT_64) {
                auto valuePtr = reinterpret_cast<int64_t*>(data + valueOffset);
                valueObject->add_data(static_cast<float>(*valuePtr));
            } else if (sensorValueType == Type::FieldInfo_Type_FLOAT) {
                auto valuePtr = reinterpret_cast<float*>(data + valueOffset);
                valueObject->add_data(*valuePtr);
            } else if (sensorValueType == Type::FieldInfo_Type_DOUBLE) {
                auto valuePtr = reinterpret_cast<double*>(data + valueOffset);
                valueObject->add_data(static_cast<float>(*valuePtr));
            } else {
