...
void SensorLocationEventProviderImpl::startFromTimestamp(uint64_t startTimestamp) {
...
bool SensorLocationEventProviderImpl::getNextCommandDelay(
    ...
    // In the case where a starting timestamp is set to later than mPreviousTimestamp,
    // drop all events that are supposed to happen prior to that
