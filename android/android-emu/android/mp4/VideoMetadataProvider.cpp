...
public:
    ...
    bool hasNextFrameMetadata() override { return mVideoMetadataQueue->size() != 0; }
...
std::unique_ptr<VideoMetadataProvider> VideoMetadataProvider::create(const DatasetInfo& datasetInfo) {
    auto provider = new VideoMetadataProviderImpl();
    if (provider->init(datasetInfo) < 0) {
        LOG(ERROR) << "Failed to initialize video metadata provider!";
        provider = nullptr;
    }
    std::unique_ptr<VideoMetadataProvider> ret;
    ret.reset(provider);
    return std::move(ret);
}
...
int VideoMetadataProviderImpl::init(const DatasetInfo& datasetInfo) {
    if (!datasetInfo.has_video_metadata()) {
        LOG(ERROR) << "Dataset info does not contain video metadata stream info!";
        return -1;
    }

    mVideoMetadataDecodeInfo.streamIndex = datasetInfo.video_metadata().stream_index();
    mVideoMetadataDecodeInfo.timestamp.offset = datasetInfo.video_metadata().video_metadata_packet().timestamp().offset();
    mVideoMetadataDecodeInfo.timestamp.type = datasetInfo.video_metadata().video_metadata_packet().timestamp().type();
    mVideoMetadataDecodeInfo.frameNumber.offset = datasetInfo.video_metadata().video_metadata_packet().frame_number().offset();
    mVideoMetadataDecodeInfo.frameNumber.type = datasetInfo.video_metadata().video_metadata_packet().frame_number().type();
...
VideoMetadata VideoMetadataProviderImpl::getNextFrameMetadata() {
    ...
    LOG(VERBOSE) << "Frame number: " << metadata.frameNumber << ", timestamp: " << metadata.timestamp;
