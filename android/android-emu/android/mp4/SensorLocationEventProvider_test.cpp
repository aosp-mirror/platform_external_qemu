...
TEST_F(SensorLocationEventProviderTest, createConsumeEvents) {
    ...
    EXPECT_EQ(event1.sensor_override().value_size(), 1);
    EXPECT_EQ(event1.sensor_override().value(0), static_cast<float>(300));
    ...
    EXPECT_EQ(event2.sensor_override().value_size(), 1);
    EXPECT_EQ(event2.sensor_override().value(0), static_cast<float>(400));
}