...
11: authenticaion challenge string length (here it is 17 bytes)
...
+CSIM=<length-of-reply-without-quotes>, "<base64-reaponse><status1-in-hexstring><status2-in-hexstring>"
     */

    char * first_double_quote_ptr = strchr(cmd, '"');

    // sanity check: the authenticaion challenge should be at least 1 byte long
    ...
    if (!first_double_quote_ptr || strlen(first_double_quote_ptr) < 17
        ...
        snprintf(s_buffer, sizeof(s_buffer), "+CSIM=4, \"6D00\""); // instruction code not supported or invalid
        return s_buffer;
    }
    ...
    int resp_len = proxy_base64_encode(response, sizeof(response), base64_response, sizeof(base64_response));
