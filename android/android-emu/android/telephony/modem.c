...
amodem_addPhysChanCfgUpdate( AModem  modem )
{
    ...
    for (nn = 0; nn < MAX_DATA_CONTEXTS; nn++) {
        ...
        if (!data->active || data->id <= 0)
            continue;
        amodem_addOnePhysChanCfgUpdate(PRIMARY_SERVING, cellBandwidthDownlink, A_DATA_NETWORK_NR, MMWAVE, data->id, modem);
        amodem_addOnePhysChanCfgUpdate(SECONDARY_SERVING, cellBandwidthDownlink, A_DATA_NETWORK_NR, MMWAVE, data->id, modem);
    }
}
