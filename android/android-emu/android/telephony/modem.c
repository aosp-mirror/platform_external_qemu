...
android_parse_network_type( const char*  speed )
{
    ...
    fprintf(stderr, "find type for %s to %d\n", speed, types[nn].type);
            return types[nn].type;
        }
    }

    fprintf(stderr, "cannot find type for %s defalt to 1\n", speed);
...
//    modem->data_network = A_DATA_NETWORK_NR;
    return NULL;
}
...
amodem_addPhysChanCfgUpdate( AModem  modem )
{
    if (modem->send_phys_channel_cfg_unsol == 0 ) {
        fprintf(stderr, "cannot update phys channel %d %d\n",
                modem->send_phys_channel_cfg_unsol, modem->data_network);
        return;
    }

    //disable for now, as it makes guest to stick with 5g
//        return;

    int PRIMARY_SERVING = 1;
    int SECONDARY_SERVING = 2;
    int cellBandwidthDownlink = 50000;
    int MMWAVE = 1;
    int nn;
    for (nn = 0; nn < MAX_DATA_CONTEXTS; nn++) {
        ...
        if (!data->active || data->id <= 0)
            continue;
        amodem_addOnePhysChanCfgUpdate(PRIMARY_SERVING, cellBandwidthDownlink, modem->data_network, MMWAVE, data->id, modem);
        amodem_addOnePhysChanCfgUpdate(SECONDARY_SERVING, cellBandwidthDownlink, modem->data_network, MMWAVE, data->id, modem);
    }
}
