...
#define OPERATOR_HOME_MCCMNC     \
    STRINGIFY(OPERATOR_HOME_MCC) \
...
#define OPERATOR_ROAMING_MCCMNC     \
    STRINGIFY(OPERATOR_ROAMING_MCC) \
    STRINGIFY(OPERATOR_ROAMING_MNC)

static const char* _amodem_switch_technology(AModem modem,
                                             AModemTech newtech,
                                             int32_t newpreferred);
static int _amodem_set_cdma_subscription_source(AModem modem,
                                                ACdmaSubscriptionSource ss);
...
extern AModemTech android_parse_modem_tech(const char* tech) {
    const struct {
        const char* name;
        AModemTech tech;
    } techs[] = {{"gsm", A_TECH_GSM}, {"lte", A_TECH_LTE}, {NULL, 0}};
    int nn;
...
typedef enum {
    kSimApduGetData =
            0xCA,  // Global Platform SEAC section 4.1 GET DATA Command
} SimApduInstruction;
...
typedef enum { A_DATA_IP = 0, A_DATA_PPP } ADataType;
...
// this function.
static bool parseSimApduCommand(const char* command,
                                int length,
                                SIM_APDU* apdu) {
...
static void amodem_unsol(AModem modem, const char* format, ...) {
    if (modem->unsol_func) {
        va_list args;
...
void amodem_receive_sms(AModem modem, SmsPDU sms) {
...
static const char* amodem_printf(AModem modem, const char* format, ...) {
    va_list args;
    va_start(args, format);
    vsnprintf(modem->out_buff, sizeof(modem->out_buff), format, args);
    va_end(args);
...
static void amodem_add_line(AModem modem, const char* format, ...) {
    va_list args;
    va_start(args, format);
    modem->out_size +=
            vsnprintf(modem->out_buff + modem->out_size,
                      sizeof(modem->out_buff) - modem->out_size, format, args);
    va_end(args);
}

static const char* amodem_end_line(AModem modem) {
...
static AConfig* amodem_load_nvram(AModem modem) {
...
static int amodem_nvram_get_int(AModem modem, const char* nvname, int defval) {
    int value;
...
const char* amodem_nvram_get_str(AModem modem,
                                 const char* nvname,
                                 const char* defval) {
    const char* value;
...
static ACdmaSubscriptionSource _amodem_get_cdma_subscription_source(
        AModem modem) {
    int iss = -1;
    iss = amodem_nvram_get_int(modem, NV_CDMA_SUBSCRIPTION_SOURCE,
                               A_SUBSCRIPTION_RUIM);
...
static void amodem_reset(AModem modem) {
    const char* tmp;
    int i;
    ...
    modem->oper_selection_mode = amodem_nvram_get_int(modem, NV_SELECTION_MODE,
                                                      A_SELECTION_AUTOMATIC);
    ...
    for (i = 1; i < MAX_EMERGENCY_NUMBERS; i++) {
        ...
        modem->emergency_numbers[i] =
                amodem_nvram_get_str(modem, key_name, NULL);
    }
    ...
    if (modem->technology == A_TECH_UNKNOWN) {
        modem->technology = aconfig_int(modem->nvram_config,
                                        NV_MODEM_TECHNOLOGY, A_TECH_GSM);
    }
    // Support GSM, WCDMA, CDMA, EvDo
    modem->preferred_mask =
            amodem_nvram_get_int(modem, NV_PREFERRED_MODE, 0x0f);
...
int amodem_state_load(AModem modem, SysFile* file) {
    ...
    for (nn = call_count; nn > 0; nn--) {
        ...
        sys_file_get_buffer(file, (uint8_t*)call->number,
                            A_CALL_NUMBER_MAX_SIZE + 1);
    }
...
AModem amodem_create(int base_port,
                     AModemUnsolFunc unsol_func,
                     int sim_present,
                     void* unsol_opaque) {
...
void amodem_set_legacy(AModem modem) {
    modem->supportsNetworkDataType = 0;
}

void amodem_destroy(AModem modem) {
    asimcard_destroy(modem->sim);
    modem->sim = NULL;
}

void amodem_set_sim_present(AModem modem, int is_present) {
    asimcard_set_status(modem->sim,
                        is_present ? A_SIM_STATUS_READY : A_SIM_STATUS_ABSENT);
}

static int amodem_has_network(AModem modem) {
    return !(modem->radio_state == A_RADIO_STATE_OFF || modem->oper_index < 0 ||
             modem->oper_index >= modem->oper_count ||
             modem->oper_selection_mode == A_SELECTION_DEREGISTRATION);
}

ARadioState amodem_get_radio_state(AModem modem) {
    return modem->radio_state;
}

void amodem_set_radio_state(AModem modem, ARadioState state) {
    modem->radio_state = state;
}

ASimCard amodem_get_sim(AModem modem) {
    return modem->sim;
}

ARegistrationState amodem_get_voice_registration(AModem modem) {
    return modem->voice_state;
}

void amodem_set_voice_registration(AModem modem, ARegistrationState state) {
    ...
    switch (modem->voice_mode) {
        case A_REGISTRATION_UNSOL_ENABLED:
            amodem_unsol(modem, "+CREG: %d,%d\r", modem->voice_mode,
                         modem->voice_state);
            break;
...
ARegistrationState amodem_get_data_registration(AModem modem) {
    return modem->data_state;
}

void amodem_set_data_registration(AModem modem, ARegistrationState state) {
    modem->data_state = state;

    switch (modem->data_mode) {
        case A_REGISTRATION_UNSOL_ENABLED:
            amodem_unsol(modem, "+CGREG: %d,%d\r", modem->data_mode,
                         modem->data_state);
            break;
        ...
        case A_REGISTRATION_UNSOL_ENABLED_FULL:
            if (modem->supportsNetworkDataType)
                ...
                amodem_unsol(modem, "+CGREG: %d,%d,\"%04x\",\"%04x\"\r",
                             modem->data_mode, modem->data_state,
                             modem->area_code & 0xffff,
                             modem->cell_id & 0xffff);
            break;
...
static AModemTech tech_from_network_type(ADataNetworkType type) {
    switch (type) {
...
void amodem_set_data_network_type(AModem modem, ADataNetworkType type) {
    ...
    if (modem->unsol_func && modemTech != A_TECH_UNKNOWN) {
        if (_amodem_switch_technology(modem, modemTech,
                                      modem->preferred_mask)) {
            modem->unsol_func(modem->unsol_opaque, modem->out_buff);
        }
    }
...
int amodem_get_operator_name(AModem modem,
                             ANameIndex index,
                             char* buffer,
                             int buffer_size) {
    AOperator oper;
...
/* reset one operator name from a user-provided buffer, set buffer_size to -1
 * for zero-terminated strings */
void amodem_set_operator_name(AModem modem,
                              ANameIndex index,
                              const char* buffer,
                              int buffer_size) {
    AOperator oper;
...
/** CALLS
 **/
int amodem_get_call_count(AModem modem) {
    return modem->call_count;
}

ACall amodem_get_call(AModem modem, int index) {
...
static void amodem_free_call(AModem modem, AVoiceCall call) {
    ...
    memmove(modem->calls + nn, modem->calls + nn + 1,
...
static AVoiceCall amodem_find_call(AModem modem, int id) {
    int nn;
...
static void amodem_send_calls_update(AModem modem) {
    /* despite its name, this really tells the system that the call
     * state has changed */
    amodem_unsol(modem, "RING\r");
}

int amodem_add_inbound_call(AModem modem, const char* number) {
    AVoiceCall vcall = amodem_alloc_call(modem);
    ACall call = &vcall->call;
    int len;
...
ACall amodem_find_call_by_number(AModem modem, const char* number) {
...
void amodem_set_signal_strength(AModem modem, int rssi, int ber) {
    modem->rssi = rssi;
    modem->ber = ber;
    modem->use_signal_profile = 0;
}

void amodem_set_signal_strength_profile(AModem modem, int quality) {
...
static void acall_set_state(AVoiceCall call, ACallState state) {
...
int amodem_update_call(AModem modem, const char* fromNumber, ACallState state) {
    AVoiceCall vcall =
...
int amodem_disconnect_call(AModem modem, const char* number) {
...
static const char* unknownCommand(const char* cmd, AModem modem) {
    modem = modem;
...
static AModemTech chooseTechFromMask(AModem modem, int32_t preferred) {
    int i, j;
...
static const char* _amodem_switch_technology(AModem modem,
                                             AModemTech newtech,
                                             int32_t newpreferred) {
    D("_amodem_switch_technology: oldtech: %d, newtech %d, preferred: %d. "
      "newpreferred: %d\n",
...
static int parsePreferred(const char* str, int* preferred) {
    char* endptr = NULL;
    int result = 0;
    if (!str || !*str) {
        *preferred = 0;
...
void amodem_set_cdma_prl_version(AModem modem, int prlVersion) {
...
void amodem_set_cdma_subscription_source(AModem modem,
                                         ACdmaSubscriptionSource ss) {
...
static const char* handleSubscriptionSource(const char* cmd, AModem modem) {
    int newsource;
    ...
    if (cmd[0] == '?') {
        ...
        switch (cmd[1]) {
            ...
            case '1':
                ...
                return amodem_printf(modem, "+CCSS: %d",
                                     modem->subscription_source);
                break;
        }
    }
...
static const char* handleRoamPref(const char* cmd, AModem modem) {
    int roaming_pref = -1;
    char* endptr = NULL;
    D("handleRoamPref(%s)\n", cmd);
    ...
    if (!strcmp(cmd, "=?")) {
        ...
        // (if *endptr is null, it means strtol processed the whole string as a
        // number)
        if (endptr && !*endptr) {
            modem->roaming_pref = roaming_pref;
            aconfig_set(modem->nvram_config, NV_CDMA_ROAMING_PREF, cmd);
            aconfig_save_file(modem->nvram_config,
                              modem->nvram_config_filename);
            return NULL;
        }
    }
...
static const char* handleTech(const char* cmd, AModem modem) {
    ...
    cmd += 5;
    if (cmd[0] == '?') {
        return amodem_printf(modem, "+CTEC: %d,%x", modem->technology,
                             modem->preferred_mask);
    }
...
static const char* handleEmergencyMode(const char* cmd, AModem modem) {
    long arg;
    char* endptr = NULL;
...
static const char* handlePrlVersion(const char* cmd, AModem modem) {
...
static const char* handleRadioPower(const char* cmd, AModem modem) {
...
static const char* handleRadioPowerReq(const char* cmd, AModem modem) {
...
static const char* handleOpenLogicalChannel(const char* cmd, AModem modem) {
    int channel;
    char* df_name = NULL;
...
static const char* handleCloseLogicalChannel(const char* cmd, AModem modem) {
    int channel;
    char dummy;
    ...
    --channel;
    if (channel < 0 || channel >= MAX_LOGICAL_CHANNELS ||
...
static const char* handleTransmitLogicalChannel(const char* cmd, AModem modem) {
    ...
    snprintf(scan_string, sizeof(scan_string), "+CGLA=%%d,%%d,%%%ds%%c",
    ...
    switch (apdu.instruction) {
        case kSimApduGetData:
            if (apduClass == 0x80 && apdu.param1 == 0xFF &&
                apdu.param2 == 0x40) {
                // Get Data (from class and instrcution) ALL (from params)
                // command
...
static const char* handleSIMStatusReq(const char* cmd, AModem modem) {
    ...
    switch (asimcard_get_status(modem->sim)) {
        ...
        case A_SIM_STATUS_NOT_READY:
            answer = "+CMERROR: NOT READY";
            break;
        case A_SIM_STATUS_PIN:
            answer = "+CPIN: SIM PIN";
            break;
        case A_SIM_STATUS_PUK:
            answer = "+CPIN: SIM PUK";
            break;
        case A_SIM_STATUS_NETWORK_PERSONALIZATION:
            answer = "+CPIN: PH-NET PIN";
            break;
        default:
...
static const char* handleNetworkRegistration(const char* cmd, AModem modem) {
    if (!memcmp(cmd, "+CREG", 5)) {
        ...
        if (cmd[0] == '?') {
            if (modem->voice_mode == A_REGISTRATION_UNSOL_ENABLED_FULL)
                ...
                return amodem_printf(modem, "+CREG: %d,%d", modem->voice_mode,
                                     modem->voice_state);
        } else if (cmd[0] == '=') {
            switch (cmd[1]) {
                case '0':
        ...
        if (cmd[0] == '?') {
            if (modem->supportsNetworkDataType)
                return amodem_printf(
                        modem, "+CGREG: %d,%d,\"%04x\",\"%04x\",\"%04x\"",
                        modem->data_mode, modem->data_state, modem->area_code,
                        modem->cell_id, modem->data_network);
            else
...
static const char* handleSetDialTone(const char* cmd, AModem modem) {
    /* XXX: TODO */
    return NULL;
}

static const char* handleDeleteSMSonSIM(const char* cmd, AModem modem) {
    /* XXX: TODO */
    return NULL;
}

static const char* handleSIM_IO(const char* cmd, AModem modem) {
    return asimcard_io(modem->sim, cmd);
}

static const char* handleCSIM(const char* cmd, AModem modem) {
    return asimcard_csim(modem->sim, cmd);
}

static const char* handleOperatorSelection(const char* cmd, AModem modem) {
    ...
    if (cmd[0] == '?') { /* ask for current operator */
        ...
        if (modem->oper_name_index == 2)
            return amodem_printf(modem, "+COPS: %d,2,%s",
                                 modem->oper_selection_mode, oper->name[2]);

        return amodem_printf(modem, "+COPS: %d,%d,\"%s\"",
                             modem->oper_selection_mode, modem->oper_name_index,
                             oper->name[modem->oper_name_index]);
    } else if (cmd[0] == '=' &&
               cmd[1] == '?') { /* ask for all available operators */
        ...
        for (nn = 0; nn < modem->oper_count; nn++) {
            ...
            amodem_add_line(modem, "%s(%d,\"%s\",\"%s\",\"%s\")", comma,
                            oper->status, oper->name[0], oper->name[1],
                            oper->name[2]);
            comma = ", ";
        }
...
static const char* handleRequestOperator(const char* cmd, AModem modem) {
    AOperator oper;
    cmd = cmd;
...
static const char* handleSendSMStoSIM(const char* cmd, AModem modem) {
    /* XXX: TODO */
    return "ERROR: unimplemented";
}

static const char* handleSendSMS(const char* cmd, AModem modem) {
    modem->wait_sms = 1;
    return "> ";
}
...
static const char* handleSendSMSText(const char* cmd, AModem modem) {
    SmsAddressRec address;
    char temp[16];
    char number[16];
    int numlen;
    ...
    do {
        ...
        if (numlen >= 4 && numlen < prefix_len + 4 &&
            !strncmp(temp, &phone_prefix[prefix_len - (numlen - 4)],
                     numlen - 4)) {
        ...
        if (index > 0) {
            ...
            snprintf(temp, sizeof(temp), "%s%d", get_phone_number_prefix(),
                     modem->base_port);
            sms_address_from_str(from, temp, strlen(temp));

            deliver = sms_receiver_create_deliver(modem->sms_receiver, index,
                                                  from);
            if (deliver == NULL) {
            ...
            for (nn = 0; deliver[nn] != NULL; nn++) {
                if (remote_call_sms(number, modem->base_port, deliver[nn]) <
                    0) {
...
static const char* handleChangeOrEnterPIN(const char* cmd, AModem modem) {
...
static const char* handleListCurrentCalls(const char* cmd, AModem modem) {
    int nn;
    amodem_begin_line(modem);
    for (nn = 0; nn < modem->call_count; nn++) {
        AVoiceCall vcall = modem->calls + nn;
 ...
 *
 * it seems reference-ril doesn't parse the comma (,) as anything else than a
 * token separator, so use a column (:) instead, the Java parsing code won't see
 * a difference
 *
 */
static void amodem_addTimeUpdate(AModem modem) {
    ...
    utc = *gmtime(&now);
    tzdiff = android_tzoffset_in_seconds(&now) /
             (15 * 60); /* timezone offset is in number of quater-hours */
     ...
     * string returned with %CTZ. the system should contain special code to
     * detect and deal with this case (since it normally relied on the
     * operator's country code which is hard to simulate on a general-purpose
     * computer
     */
    amodem_add_line(
            modem, "%%CTZV: %02d/%02d/%02d:%02d:%02d:%02d%c%d:%d:%s\r\n",
            (utc.tm_year + 1900) % 100, utc.tm_mon + 1, utc.tm_mday,
            utc.tm_hour, utc.tm_min, utc.tm_sec, (tzdiff >= 0) ? '+' : '-',
            (tzdiff >= 0 ? tzdiff : -tzdiff), (isdst > 0), tzname);
}

static const char* handleEndOfInit(const char* cmd, AModem modem) {
    amodem_begin_line(modem);
    amodem_addTimeUpdate(modem);
    return amodem_end_line(modem);
}

static const char* handleListPDPContexts(const char* cmd, AModem modem) {
    int nn;
...
static const char* handleDefinePDPContext(const char* cmd, AModem modem) {
...
static const char* handleQueryPDPContext(const char* cmd, AModem modem) {
    ...
    const char* gateway =
            feature_is_enabled(kFeature_Wifi) ? "192.168.200.2" : "10.0.2.15";
    int nn;
    ...
    for (nn = 0; nn < MAX_DATA_CONTEXTS; nn++) {
        ...
        amodem_add_line(modem, "+CGDCONT: %d,\"%s\",\"%s\",\"%s\",0,0\r\n",
                        data->id, data->type == A_DATA_IP ? "IP" : "PPP",
                        data->apn,
...
static const char* handleStartPDPContext(const char* cmd, AModem modem) {
    /* XXX: TODO: handle PDP start appropriately */
    return NULL;
}

static void remote_voice_call_event(void* _vcall, int success) {
...
static void voice_call_event(void* _vcall) {
    ...
    switch (call->state) {
        case A_CALL_DIALING:
            ...
            if (vcall->is_remote) {
                if (remote_call_dial(call->number, vcall->modem->base_port,
                     ...
                     * XXX: should we send some sort of message to indicate BAD
                     * NUMBER ?
                     */
                    /* it seems the Android code simply waits for changes in the
                     * list   */
...
static int amodem_is_emergency(AModem modem, const char* number) {
    ...
    for (i = 0; i < MAX_EMERGENCY_NUMBERS; i++) {
        if (modem->emergency_numbers[i] &&
            !strcmp(number, modem->emergency_numbers[i]))
            break;
    }

    if (i < MAX_EMERGENCY_NUMBERS)
        return 1;

    return 0;
}

static const char* handleDial(const char* cmd, AModem modem) {
    AVoiceCall vcall = amodem_alloc_call(modem);
    ACall call = &vcall->call;
    int len;
...
static const char* handleAnswer(const char* cmd, AModem modem) {
    int nn;
    for (nn = 0; nn < modem->call_count; nn++) {
        AVoiceCall vcall = modem->calls + nn;
        ACall call = &vcall->call;
...
static const char* handleSignalStrength(const char* cmd, AModem modem) {
    ...
    /* Sneak time updates into the SignalStrength request, because it's
     * periodic. Ideally, we'd be able to prod the guest into asking immediately
     * on restore from snapshot, but that'd require a driver.
     */
    ...
    if (modem->use_signal_profile) {
        ...
        amodem_add_line(modem,
                        "+CSQ: %i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i\r\n",
                        current_signal.gsm_rssi, current_signal.gsm_ber,
                        current_signal.cdma_dbm, current_signal.cdma_ecio,
                        current_signal.evdo_dbm, current_signal.evdo_ecio,
                        current_signal.evdo_snr, current_signal.lte_rssi,
                        current_signal.lte_rsrp, current_signal.lte_rsrq,
                        current_signal.lte_rssnr, current_signal.lte_cqi,
                        current_signal.lte_timing);
    } else {
        // rssi = 0 (<-113dBm) 1 (<-111) 2-30 (<-109--53) 31 (>=-51) 99 (?!)
        // ber (bit error rate) - always 99 (unknown), apparently.
        // TODO: return 99 if modem->radio_state==A_RADIO_STATE_OFF, once
        // radio_state is in snapshot.
        int rssi = modem->rssi;
        int ber = modem->ber;
...
static const char* handleHangup(const char* cmd, AModem modem) {
    if (!memcmp(cmd, "+CHLD=", 6)) {
        ...
        switch (cmd[0]) {
            ...
            case '2':
                if (cmd[1] == 0) { /* place all active on hold, accept held or
                                      waiting one */
                    for (nn = 0; nn < modem->call_count; nn++) {
                        AVoiceCall vcall = modem->calls + nn;
                        ACall call = &vcall->call;
...
static const struct {
    ...
    ResponseHandler
            handler; /* specific handler, ignored if 'answer' is not NULL,
        ...
        {"!+CTEC", NULL,
         handleTech}, /* Set/get current Technology and preferred mode */

        {"+WRMP=?", "+WRMP: 0,1,2", NULL}, /* Query Roam Preference */
        {"!+WRMP", NULL, handleRoamPref},  /* Set/get Roam Preference */

        {"+CCSS=?", "+CTEC: 0,1",
         NULL}, /* Query available subscription sources */
        {"!+CCSS", NULL,
         handleSubscriptionSource}, /* Set/Get current subscription source */
        ...
        /* see requestOperator() */
        {"+COPS=3,0;+COPS?;+COPS=3,1;+COPS?;+COPS=3,2;+COPS?", NULL,
        ...
        /* see requestDial() */
        {"!D", NULL,
         handleDial}, /* the code says that success/error is ignored,
                   the call state will be polled through +CLCC instead
                 */
        ...
        {"+CIMI", OPERATOR_HOME_MCCMNC "000000000",
         NULL}, /* request internation subscriber identification number */
        {"+CGSN", "000000000000000", NULL}, /* request model version */
        {"+CUSD=2", NULL, NULL},            /* Cancel USSD */
        {"+COPS=0", NULL,
         handleOperatorSelection}, /* set network selection to automatic */
        ...
        {"+CUSD=1", NULL, NULL},
        {"+CGEREP=1,0", NULL, NULL},
        {"+CMGF=0", NULL, handleEndOfInit}, /* now is a good time to send the
...
const char* amodem_send(AModem modem, const char* cmd) {
    ...
    /* TODO: implement command handling */
    {
        ...
        if (!found) {
            ...
            if (!memcmp(answer, "> ", 2) || !memcmp(answer, "ERROR", 5) ||
