...
static void impl_skin_surface_free(SkinSurface* s) {
    D("skin_surface_free %d", s->id);
    s->window->releaseBitmap(s, nullptr);
}

static int impl_skin_surface_height(SkinSurface* s) {
    D("skin_surface_height %d", s->id);
    return s->h;
}

static int impl_skin_surface_is_round(SkinSurface* s) {
    return s->isRound;
}

static void impl_skin_surface_unrefp(SkinSurface** psurface) {
    SkinSurface* surf = *psurface;
    if (surf) {
        D("skin_surface_unref %d", surf->id);
        impl_skin_surface_free(surf);
        *psurface = NULL;
    }
}

static int impl_skin_surface_width(SkinSurface* s) {
...
static SkinSurface* impl_skin_surface_create(int w,
                                        int h,
                                        int original_w,
                                        int original_h) {
    return createSkinSurface([w, h, original_w, original_h](SkinSurface* s) {
        ...
        D("skin_surface_create bitmap(%dx%d) w %d h %d id %d\n", original_w,
          original_h, s->w, s->h, next_id);
    });
}

static SkinSurface* impl_skin_surface_create_from_data(const void* data, int size) {
...
static SkinSurface* impl_skin_surface_create_from_file(const char* path) {
...
static SkinSurface* impl_skin_surface_create_derived(SkinSurface* source,
...
static SkinSurface* impl_skin_surface_resize(SkinSurface* surface,
                                        int w,
                                        int h,
                                        int original_w,
                                        int original_h) {
    ...
    if (surface == NULL) {
        return impl_skin_surface_create(w, h, original_w, original_h);
    } else if (surface->bitmap->size() == QSize(original_w, original_h)) {
        surface->w = w;
        surface->h = h;
        return surface;
    } else {
        impl_skin_surface_unrefp(&surface);
        return impl_skin_surface_create(w, h, original_w, original_h);
    }
}

static void impl_skin_surface_create_window(SkinSurface* surface,
                                       int x,
                                       int y,
                                       int w,
                                       int h) {
...
static void impl_skin_surface_update(SkinSurface* surface, SkinRect* rect) {
#if 0
...
static void impl_skin_surface_blit(SkinSurface* dst,
                              SkinPos* pos,
                              SkinSurface* src,
                              SkinRect* rect,
                              SkinBlitOp op) {
...
static void impl_skin_surface_fill(SkinSurface* dst,
                              SkinRect* rect,
                              uint32_t argb_premul) {
    D("skin_surface_fill %d: %d, %d, %d, %d: %x", dst->id, rect->pos.x,
      rect->pos.y, rect->size.w, rect->size.h, argb_premul);
    QRect qrect(rect->pos.x, rect->pos.y, rect->size.w, rect->size.h);
    QColor color(argb_premul);
    dst->window->fill(dst, qrect, color, nullptr);
}

static void impl_skin_surface_upload(SkinSurface* surface,
                                const SkinRect* rect,
                                const void* pixels,
                                int pitch) {
    D("skin_surface_upload %d: %d,%d,%d,%d", surface->id, rect->pos.x,
      rect->pos.y, rect->size.w, rect->size.h);
    ...
    if (rect->pos.x == 0 && rect->pos.y == 0 &&
        ...
        pitch == rect->size.w * 4) {
        memcpy(surface->bitmap->get().bits(), pixels,
               surface->h * surface->w * 4);
    } else {
        const uint32_t* src = (const uint32_t*)pixels;
        uint32_t* dst = ((uint32_t*)surface->bitmap->get().bits()) +
...
static void impl_skin_surface_get_scaled_rect(SkinSurface* surface,
                                         const SkinRect* from,
                                         SkinRect* to) {
    ...
    double wantedFactor =
            surfaceFactorW > surfaceFactorH ? surfaceFactorW : surfaceFactorH;
    ...
    D("skin_surface_get_scaled_rect %d: %d, %d, %d, %d => %d, %d, %d, %d",
      surface->id, fromx, fromy, fromw, fromh, to->pos.x, to->pos.y, to->size.w,
      to->size.h);
}

static void impl_skin_surface_reverse_map(SkinSurface* surface, int* x, int* y) {
...
static const QAndroidSurfaceAgent sQAndroidSurfaceAgent = {
        .skin_surface_unrefp = &impl_skin_surface_unrefp,
        .skin_surface_width = &impl_skin_surface_width,
        .skin_surface_height = &impl_skin_surface_height,
        .skin_surface_is_round = &impl_skin_surface_is_round,
        .skin_surface_create = &impl_skin_surface_create,
        .skin_surface_create_from_data = &impl_skin_surface_create_from_data,
        .skin_surface_create_from_file = &impl_skin_surface_create_from_file,
        .skin_surface_create_derived = &impl_skin_surface_create_derived,
        .skin_surface_resize = &impl_skin_surface_resize,
        .skin_surface_create_window = &impl_skin_surface_create_window,
        .skin_surface_reverse_map = &impl_skin_surface_reverse_map,
        .skin_surface_get_scaled_rect = &impl_skin_surface_get_scaled_rect,
        .skin_surface_update = &impl_skin_surface_update,
        .skin_surface_upload = &impl_skin_surface_upload,
        .skin_surface_blit = &impl_skin_surface_blit,
        .skin_surface_fill = &impl_skin_surface_fill,
};

extern "C" const QAndroidSurfaceAgent* const gQAndroidSurfaceAgent =
        &sQAndroidSurfaceAgent;
