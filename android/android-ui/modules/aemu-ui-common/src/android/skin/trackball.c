...
trackball_init(TrackBall  ball, int  diameter, int  ring,
    ...
    ball->surface  = getConsoleAgents()->surface->skin_surface_create(diameter2, diameter2, diameter2, diameter2);
...
trackball_done( TrackBall  ball )
{
    ...
    getConsoleAgents()->surface->skin_surface_unrefp(&ball->surface);
...
trackball_refresh( TrackBall  ball )
{
    ...
    getConsoleAgents()->surface->skin_surface_upload(ball->surface, &sRect, pixels, ball->diameter * 4);
}
...
trackball_draw(TrackBall ball, int x, int y, SkinSurface* dst)
{
    ...
    getConsoleAgents()->surface->skin_surface_blit(dst,
                      &dst_pos,
                      ball->surface,
                      &src_rect,
                      SKIN_BLIT_COPY);
