...
static int skin_display_init_from(SkinDisplay* display,
    ...
    display->bpp         = aconfig_int(node, "bpp", bpp);
...
static const char* portrait_auto_layout_names[] =
    { "portrait", "reverse_landscape", "reverse_portrait", "landscape" };

static const char* landscape_auto_layout_names[] =
...
static int skin_file_load_from_v1(SkinFile* file,
    ...
    file->parts = part = skin_part_create_from_v1(
            aconfig, basepath, fb_funcs, bpp);
    if (part == NULL)
        return -1;

    const char** auto_layout_names = portrait_auto_layout_names;
    if (part->rect.size.w > part->rect.size.h) {
        auto_layout_names = landscape_auto_layout_names;
    }
    for (nn = 0; nn < 4; nn++)
    {
        ANEW0(layout);
        ...
        layout->name = auto_layout_names[nn];
...
static int skin_file_load_from_v2(SkinFile* file,
    ...
    else
    {
        ...
        const char** auto_layout_names = portrait_auto_layout_names + 1;
        int SIZE_OF_LAYOUTS = sizeof(portrait_auto_layout_names) / sizeof(const char*);
        if (!strcasecmp("landscape", next->name)) {
            auto_layout_names = landscape_auto_layout_names + 1;
            SIZE_OF_LAYOUTS = sizeof(landscape_auto_layout_names) / sizeof(const char*);
        }
        ...
        for (r = SKIN_ROTATION_90; r <= SKIN_ROTATION_270; r++) {
            ...
            if (layout->next != NULL) {
                ...
                if (layout_name_idx >= 0 &&
                    layout_name_idx < SIZE_OF_LAYOUTS) {
...
SkinFile* skin_file_create_from_aconfig(const AConfig* aconfig,
    ...
    if (aconfig_find_const(aconfig, "parts") != NULL) {
        if (skin_file_load_from_v2(
                file, aconfig, basepath, fb_funcs, bpp) < 0) {
            goto BAD_FILE;
        }
