...
skin_image_free( SkinImage*  image )
{
    ...
    {
        getConsoleAgents()->surface->skin_surface_unrefp(&image->surface);
...
skin_image_load( SkinImage*  image )
{
    ...
    if (path[0] == ':') {
        ...
        image->surface = getConsoleAgents()->surface->skin_surface_create_from_data(base, size);
        if (!image->surface) {
            derror("failed to load built-in image file '%s'", path );
            return -1;
        }
    } else {
        image->surface = getConsoleAgents()->surface->skin_surface_create_from_file(path);
        if (!image->surface) {
            derror("failed to load image file '%s'", path );
            return -1;
        }
    }
    image->w = getConsoleAgents()->surface->skin_surface_width(image->surface);
    image->h = getConsoleAgents()->surface->skin_surface_height(image->surface);
...
skin_image_create( SkinImageDesc*  desc, unsigned  hash )
{
    ...
    else
    {
        ...
        node->surface = getConsoleAgents()->surface->skin_surface_create_derived(parent->surface,
                                                    desc->rotation,
                                                    desc->blend);
...
skin_image_clone_rotated( SkinImage* source, SkinRotation by ) {
    ...
    image->surface = getConsoleAgents()->surface->skin_surface_create_derived(source->surface, by,
                                                 SKIN_BLEND_FULL);
