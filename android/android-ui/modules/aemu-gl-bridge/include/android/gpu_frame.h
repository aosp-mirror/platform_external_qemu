...
// being used, if any.
AEMU_EXPORT void gpu_frame_set_record_mode(bool on, uint32_t displayId = 0);
...
// available.
AEMU_EXPORT void* gpu_frame_get_record_frame(uint32_t displayId = 0);
...
// as possible.
AEMU_EXPORT void gpu_register_shared_memory_callback(FrameAvailableCallback frameAvailable,
                                         void* opaque);
AEMU_EXPORT void gpu_unregister_shared_memory_callback(void* opaque);
...
AEMU_EXPORT void gpu_initialize_recorders();

// Used to inform the recorders that the emulator is terminating.
AEMU_EXPORT void gpu_emulator_shutdown();
