...
#define IMG_WIDTH 1080 / 2
#define IMG_HEIGHT 1920 / 2
...
EmulatorQtWindow::EmulatorQtWindow(std::shared_ptr<grpc::Channel> channel, QWidget* parent, size_t width, size_t height) :
    ...
    image_format.set_width(mWidth);
    image_format.set_height(mHeight);
    ...
    if (g_use_mmap) {
        ...
        if (ftruncate(mmap_fd_, mWidth * mHeight * 3) == -1) {
            perror("Failed to increase size of mmaped file");
            exit(1);
        }

        mmap_ptr = mmap(NULL, mWidth * mHeight * 3, PROT_READ | PROT_WRITE, MAP_SHARED,
                mmap_fd_, 0);
...
EmulatorQtWindow::~EmulatorQtWindow() {
    ...
    munmap(mmap_ptr, mWidth * mHeight * 3);
    ::close(mmap_fd_);
    ::remove(MMAP_FILE);
}
...
void EmulatorQtWindow::paintEvent(QPaintEvent* e) {
    ...
    QImage image((g_use_mmap ? reinterpret_cast<uint8_t*>(mmap_ptr) : img_data_),
            mWidth, mHeight, QImage::Format_RGB888);
