 ...
 * hw_ramSize
 */
static bool handleCommonEmulatorOptions(AndroidOptions* opts,
                                        AndroidHwConfig* hw,
                                        AvdInfo* avd,
                                        bool is_qemu2) {
    int forceArmv7 = 0;
...
static bool use_software_gpu_and_screen_too_large(AndroidHwConfig *hw) {
    if (hw->hw_gpu_enabled) {
        // Screen is never too large when GPU emulation is enabled.
        return false;
    }
    const int kMaxWidth = 768;
    const int kMaxHeight = 1280;

    if (hw->hw_lcd_width * hw->hw_lcd_height > kMaxWidth * kMaxHeight) {
        derror("GPU emulation is disabled.\n"
            "Only screen size of %d x %d or smaller is supported "
            "when GPU emulation is disabled.", kMaxWidth, kMaxHeight);
        return true;
    }
...
int handleEmulatorCommandLine(int* p_argc,
    ...
    if (opts->code_profile) {
        ...
        if (path_mkdir_if_needed(profilePath, 0755) < 0) {
    ...
    if (!handleCommonEmulatorOptions(opts, hw, avd, is_qemu2)) {
        goto FAIL;
    }
    ...
    if (opts->snapstorage) {
        hw->disk_snapStorage_path = strdup(opts->snapstorage);
    ...
    if (opts->camera_back) {
        ...
        hw->hw_camera_back = strdup(opts->camera_back);
    }
