...
{
    if (path == NULL || '\0' == *path || !strncmp(path, "<init>", 6)) {
        return 0;
    }
    return 1;
}
                      ...
                      int inAndroidBuild)
{
    ...
    if (_update_hwconfig_path(&hwConfig->disk_systemParition_initPath, avd, AVD_IMAGE_INITSYSTEM)) {
        dprint("system path %s is invalid", hwConfig->disk_systemParition_initPath);
        exit(1);
    }

    _update_hwconfig_path(&hwConfig->disk_userData_path, avd, AVD_IMAGE_USERDATA);
    _update_hwconfig_path(&hwConfig->disk_userSystem_path, avd, AVD_IMAGE_USERSYSTEM);
    _update_hwconfig_path(&hwConfig->disk_initData_path, avd, AVD_IMAGE_INITDATA);
     ...
     */
    {
        ...
        hwConfig->disk_systemParition_size =
...

    {
        ...
        if (dataPath == NULL || !path_exists(dataPath)) {
            hwConfig->disk_userData_size = defaultBytes;
        }
        else {
            uint64_t  dataBytes;
            path_get_size(dataPath, &dataBytes);

            hwConfig->disk_userData_size =
