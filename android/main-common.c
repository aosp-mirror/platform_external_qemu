                      ...
                      int inAndroidBuild)
{
    ...
    if (_update_hwconfig_path(&hwConfig->systemParition_initPath, avd, AVD_IMAGE_INITSYSTEM)) {
        dprint("system path %s is invalid", hwConfig->systemParition_initPath);
        exit(1);
    }
