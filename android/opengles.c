...
int
android_showOpenglesWindow(void* window, int x, int y, int windowWidth, int windowHeight,
                           int framebufferWidth, int framebufferHeight, float rotation)
{
    ...
    bool success = createOpenGLSubwindow(
            win, x, y, windowWidth, windowHeight, framebufferWidth, framebufferHeight, rotation);
    return success ? 0 : -1;
}
