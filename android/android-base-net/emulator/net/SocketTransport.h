...
template <class T>
class LambdaReader : public ProtocolReader<T> {
public:
    LambdaReader(std::function<void(SocketTransport*, T)> receiver,
                 std::function<void(SocketTransport*, State)> state =
                         [](SocketTransport* socket, State state) {})
        : mReceiver(receiver), mState(state) {}

    void received(SocketTransport* from, T object) override {
        mReceiver(from, object);
    };
    void stateConnectionChange(SocketTransport* connection,
                               State current) override {
        mState(connection, current);
    }

private:
    std::function<void(SocketTransport*, T)> mReceiver;
    std::function<void(SocketTransport*, State)> mState;
};
...
public:
     enum  TransportMode  { Read = 0x1, Write = 0x2 };
    SocketTransport(MessageReceiver* receiver,
                    AsyncSocketAdapter* connection,
                    TransportMode mode = static_cast<TransportMode>(TransportMode::Read | TransportMode::Write));
    ~SocketTransport();
