...
public:
    ...
    void removeSocketEventListener(AsyncSocketEventListener* listener);
...
protected:
    enum Notification { Read, Close, Connected };

    // Subclasses should call these to notify the listeners.
    void notify(Notification type, int err = 0);
    void removeAllEventListeners();

private:
    int mRead = 0;
    std::set<AsyncSocketEventListener*> mListeners;
