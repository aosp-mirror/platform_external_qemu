...
{
    PRINTF(
    "  the -attach-core <port> options attaches the UI to a running emulator core process.\n\n"

    "  the <port> parameter must be in the form [host:]port, where 'host' addresses the machine\n"
    "  on which the core process is running, and 'port' addresses the console port number for\n"
    "  the running core process. Note that <port> value must be in the form that can be resolved\n"
    "  into an IP address.\n\n"
