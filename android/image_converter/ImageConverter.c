 ...
 * For optimal performance the 'bytes' and 'pixels' pointers have to be
 * 8-byte alligned.
 */
static void unpackRgb888Movbe(const uint8_t* bytes, int numPixels,
        uint32_t* pixels) {
    uint64_t* limit = (uint64_t*) (pixels + numPixels);
    uint64_t* p = (uint64_t*) pixels;
    while (p < limit) {
        // Load 24 bytes into three 64-bit variables replacing big endian order
        // by little endian one.
        ...
        bytes += 8;
        // Convert the three 64-bit values into four by adding 0xFF opacity
        // bytes and conpensate for swapping of neighbouring pixels that
        // happened when changing the byte order to little endian.
        *p++ = a >> 40 | (a << 16 & 0xFFFFFFFF00000000) | 0xFF000000FF000000;
        *p++ = ((a << 8 | b >> 56) & 0x00000000FFFFFFFF) |
                (b & 0xFFFFFFFF00000000) | 0xFF000000FF000000;
        *p++ = (b >> 8 & 0x00000000FFFFFFFF) |
                ((b << 48 | c >> 16) & 0xFFFFFFFF00000000) |
                0xFF000000FF000000;
        *p++ = (c  >> 24 & 0x00000000FFFFFFFF) |
                (c << 32 & 0xFFFFFFFF00000000) | 0xFF000000FF000000;
    }
}
#endif

/* Generic unpacking function not using any special instructions. */
static void unpackRgb888Universal(const uint8_t* bytes, int numPixels,
        uint32_t* pixels) {
...
static void throwException(JNIEnv* env, const char* exceptionClassName,
        const char* message) {
 ...
 */
JNIEXPORT void JNICALL Java_com_android_emulator_ImageConverter_initNative(
...
/*
 * Class:  com.android,emulator.ImageConverter
 * Method: static native void unpackRgb888(
 *                 byte[] imageBytes, int offset, int length, int[] pixels)
 */
JNIEXPORT void JNICALL Java_com_android_emulator_ImageConverter_unpackRgb888(
    ...
    if (length % 3 != 0) {
        throwException(env, "java/lang/IllegalArgumentException",
                "The number of bytes is not a multiple of 3");
    }
    if (offset + length > (*env)->GetArrayLength(env, byteArray)) {
        throwException(env, "java/lang/ArrayIndexOutOfBoundsException",
                "Data outside if the input array");
    }
    uint32_t numPixels = length / 3;
    if (numPixels > (*env)->GetArrayLength(env, pixelArray)) {
        throwException(env, "java/lang/ArrayIndexOutOfBoundsException",
                "The output array is too small");
    }
    jboolean isCopy;
    uint8_t* bytes =
            (*env)->GetPrimitiveArrayCritical(env, byteArray, &isCopy);
    if (bytes == NULL || isCopy) {
        ...
        throwException(env, "java/lang/IllegalStateException",
                "The input array cannot be pinned in memory");
    }
    uint32_t* pixels =
            (*env)->GetPrimitiveArrayCritical(env, pixelArray, &isCopy);
    if (pixels == NULL || isCopy) {
        (*env)->ReleasePrimitiveArrayCritical(env, pixelArray, pixels, 0);
        (*env)->ReleasePrimitiveArrayCritical(env, byteArray, bytes, 0);
        throwException(env, "java/lang/IllegalStateException",
                "The output array cannot be pinned in memory");
    }
    ...
    if (hasMovbe) {
        ...
        uint32_t tailLength = numPixels % 8;
