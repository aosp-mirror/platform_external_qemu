...
#define  DURATION_INFINITE       ((Duration)INT64_T_MAX)

/* A special Duration value used to mean "as soon as possible"
 * during event loop processing, i.e. what can be called "idle time"
 * when the event loop is not processing callbacks.
 */
#define  DURATION_INFINITE_IDLE  (ADURATION_INFINITE-1)
...
/* An Looper is an abstraction for an event loop, which can
 ...
 * callback, or nor more watchers are registered.
 *
 ...
 *
 * You can create a new Looper by calling looper_new(). This provides a
 * default implementation that can be used in all threads.
 *
 * For the QEMU core, you can grab an Looper pointer by calling
 * looper_forCore() instead. Its implementation relies on top of
...
/* Initialize an LoopTimer with a callback and an 'opaque' value.
 ...
 *
 * Socket connections are mapped to LOOP_IO_READ events.
 * Socket accepts are mapped to LOOP_IO_WRITE events.
 */
enum {
    LOOP_IO_READ  = (1 << 0),
    LOOP_IO_WRITE = (1 << 1),
 ...
 * to be fired when there is data to be read of when the file is ready to
 * be written to. */
__inline__ void
