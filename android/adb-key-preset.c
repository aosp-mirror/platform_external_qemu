...
static void adb_key_preset_client_recv(void *opaque, uint8_t *msg, int msglen,
                                   QemudClient*  client ) {
    D("%s: Receive adb key reques, message %.*s", __FUNCTION__, msglen, msg);
    if (msglen == 6 && !memcmp(msg, "adbKey", 6)) {
        if (s_adbKeyPreset) {
            D("%s: Sending message %s", __FUNCTION__, s_adbKeyPreset);
            qemud_client_send(client, (const uint8_t*)s_adbKeyPreset,
                          strlen(s_adbKeyPreset));
        } else {
            // send out an empty message
            D("%s: Sending empty message", __FUNCTION__);
            qemud_client_send(client, (const uint8_t*)"", 1);
        }
    } else {
