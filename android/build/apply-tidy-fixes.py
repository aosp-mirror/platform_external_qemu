  ...
  for diagnostic in content.get('Diagnostics',[]):
    if diagnostic['Replacements']:
      for replacement in diagnostic['Replacements']:
        path = os.path.normpath(replacement['FilePath'])
        if ((accept and accept.match(path)) and not
            (reject and reject.match(path))):
          if path not in files:
            files[path] = []
          files[path].append(replacement)
          logging.debug("Adding fixes for %s, matching: %s and not rejecting %s", path, accept.match(path),reject.match(path))
        else:
          logging.debug("Ignoring fixes for %s, matching: %s and not rejecting %s", path, accept.match(path),reject.match(path))
  return files
...
def apply_fixes(file_fixes, accept, reject):
  ...
  """
  # Now we apply them
  logging.info("Applying fixes to %d files", len(file_fixes))
