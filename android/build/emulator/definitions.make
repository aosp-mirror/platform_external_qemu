...
$$(_DST): $$(_TST)
	...
	$(hide) ASAN_OPTIONS=`cat android/asan_overrides` LLVM_PROFILE_FILE=$(call local-test-result-path)/$$(PRIVATE_TST).profraw LD_LIBRARY_PATH=$(BUILD_OBJS_DIR)/testlib64 VK_ICD_FILENAMES=$(BUILD_OBJS_DIR)/testlib64/VkICD_mock_icd.json $(TEST_SHELL) $$(PRIVATE_TST) --gtest_output=xml:$(call local-test-result-path)/$$(PRIVATE_TST).xml
	@touch $$(PRIVATE_DST)
endef
