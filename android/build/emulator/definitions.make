...
$$(_DST): $$(_SRC) $$(_DEPENDS)
	@echo "CMake make $$(PRIVATE_TARGET)"
...
$$(_DST): LIBC_LDPATH := $(BUILD_OBJS_DIR)/lib64
$$(_DST): PRIVATE_DST := $$(_DST)
$$(_DST): $$(_TST)
	@echo "Running $$(PRIVATE_TST)"
	@mkdir -p $$(dir $$(PRIVATE_DST))
	@export WINEPATH=$(BUILD_OBJS_DIR)/lib
	$(hide) $$(call set-host-library-search-path,$$(LIBC_LDPATH)) \
			ASAN_OPTIONS=`cat android/asan_overrides` \
			LLVM_PROFILE_FILE=$(call local-test-result-path)/$$(PRIVATE_TST).profraw \
			$(TEST_SHELL) $$(PRIVATE_TST) \
			--gtest_output=xml:$(call local-test-result-path)/$$(PRIVATE_TST).xml
	@touch $$(PRIVATE_DST)
endef
