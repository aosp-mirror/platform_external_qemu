...
set-vulkan-mock-icd = $(call set-host-library-search-path,$(1)/testlib64) VK_ICD_FILENAMES=$(1)/testlib64/VkICD_mock_icd.json
...
$$(_DST): $$(_TST)
	...
	$(hide) $(call set-vulkan-mock-icd,$(BUILD_OBJS_DIR)/$(if $(LOCAL_INSTALL_DIR),$(LOCAL_INSTALL_DIR))) ASAN_OPTIONS=`cat android/asan_overrides` LLVM_PROFILE_FILE=$(call local-test-result-path)/$$(PRIVATE_TST).profraw $(TEST_SHELL) $$(PRIVATE_TST) --gtest_output=xml:$(call local-test-result-path)/$$(PRIVATE_TST).xml
	@touch $$(PRIVATE_DST)
endef
