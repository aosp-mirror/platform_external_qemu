...
# Madness to work around concurrent invocation..
define make-all-cmake-targets
_SRC := $(1)
_DST := $(2)
cmake-stamp: PRIVATE_SRC := $$(_SRC)
cmake-stamp: $(ELFILES)
		@rm -f cmake-temp
		@touch cmake-temp
		@$(hide) +$(MAKE) --no-print-directory  -C $$(dir $$(PRIVATE_SRC))
		@mv -f cmake-temp $@
...
## Recover from the removal of $@
		...
		if test -f $@; then :; else \
		  $$dry trap 'rm -rf cmake-lock cmake-stamp' 1 2 13 15; \
		  if $$dry mkdir cmake-lock 2>/dev/null; then \
## This code is being executed by the first process.
			$$dry rm -f cmake-stamp; \
			$(MAKE) $(AM_MAKEFLAGS) cmake-stamp; \
			$$dry rmdir cmake-lock; \
		  else \
## This code is being executed by the follower processes.
## Wait until the first process is done.
			while test -d cmake-lock && test -z "$$dry"; do \
			  sleep 1; \
			done; \
## Succeed if and only if the first process succeeded.
			$$dry test -f cmake-stamp; exit $$?; \
		  fi; \
		fi
endef
