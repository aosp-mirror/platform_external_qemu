...
# Macro    : last-but-one
# Arguments: 1: list
# Returns  : Return the last-but-first item in a list.
# Usage    : $(call last-but-one,<list>)
# -----------------------------------------------------------------------------
last-but-one = $(strip \
  $(eval __lbo_len := $$(words $1))\
  $(if $(filter-out 0 1,$(__lbo_len)),\
    $(wordlist $(__lbo_len),$(__lbo_len),xxx $1)))

-test-last-but-one = \
  $(call test-expect,,$(call last-but-one,))\
  $(call test-expect,,$(call last-but-one,foo1))\
  $(call test-expect,foo1,$(call last-but-one,foo1 bar2))\
  $(call test-expect,bar2,$(call last-but-one,foo1 bar2 zoo3))
...
# -----------------------------------------------------------------------------
# Macro    : parent-makefile
# Returns  : the name of the parent Makefile in the inclusion list.
# Usage    : $(parent-makefile)
# -----------------------------------------------------------------------------
parent-makefile = $(call last-but-one,$(MAKEFILE_LIST))
...
assert-defined = $(foreach __varname,$(strip $1),\
  $(if $(strip $($(__varname))),,\
    $(call -build-error, Assertion failure: $(__varname) is not defined)\
  )\
)
...
check-required-vars = $(foreach __varname,$1,\
  $(if $(strip $($(__varname))),,\
    $(call -build-info, Required variable $(__varname) is not defined by $2)\
