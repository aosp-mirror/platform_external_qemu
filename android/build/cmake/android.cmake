...
function(android_compile_for_host
         EXE
         SOURCE
         OUT_PATH)
  if(NOT CROSSCOMPILE)
...
function(android_nasm_compile)
  ...
  cmake_parse_arguments(android_nasm_compile
                        "${options}"
                        "${oneValueArgs}"
                        "${multiValueArgs}"
                        ${ARGN})
...
function(android_target_compile_definitions
         TGT
         OS
         MODIFIER
         ITEMS)
...
# (https://cmake.org/cmake/help/v3.5/command/target_link_libraries.html) T
# he only difference is that the definitions
# will only be applied if the OS parameter matches the ANDROID_TARGET_TAG or Compiler variable.
function(android_target_link_libraries
         TGT
         OS
         MODIFIER
         ITEMS)
...
function(android_target_include_directories
         TGT
         OS
         MODIFIER
         ITEMS)
...
function(android_target_compile_options
         TGT
         OS
         MODIFIER
         ITEMS)
...
function(_register_target)
  ...
  if(DEFINED build_NOTICE AND NOT ${build_NODISTRIBUTE})
    ...
    string(REPLACE "REPO"
                   ...
                   LOCAL_LICENSE
                   ${build_NOTICE})
    android_license(TARGET ${build_TARGET}
                    URL ${build_URL}
                    LIBNAME ${build_LIBNAME}
                    SPDX ${build_LICENSE}
...
#
# ! android_license: Registers the given license, ands adds it to LICENSES.LST for post processing !
#
#  \param:LIBNAME TThe name of the library, this is how it is usually known in the world. for example: libpng
#  \param:URL The location where the source code for this library can be found.
#  \group:TARGET List of targets that are part of the named library.
#  \param:SPDX The spdx license identifier. (See https://spdx.org/)
#  \p
function(android_license)
  ...
  file(APPEND ${CMAKE_BINARY_DIR}/LICENSES.LST
...
function(android_add_test)
  # Parse arguments set(options)
  set(oneValueArgs TARGET SPDX LICENSE)
  set(multiValueArgs
      SRC
      LINUX
      MSVC
...
#~~~
# Registers the given executable, by calculating the source set and setting licensens.
#
# ``SHARED``  Option indicating that this is a shared library.
...
function(android_add_executable)
  ...
  if(${build_INSTALL})
    android_install(${build_TARGET} ${build_INSTALL})
  endif()
endfunction()
...
function(android_generate_hw_config)
  ...
  set_source_files_properties(${ANDROID_HW_CONFIG_H}
                              PROPERTIES
                              GENERATED
                              TRUE
                              SKIP_AUTOGEN
...
function(android_copy_test_files
         TGT
         SOURCE_LIST
         DEST)
...
function(android_copy_test_dir
         TGT
         SRC_DIR
         DST_DIR)
...
function(android_build_qemu_variant)
  ...
  cmake_parse_arguments(qemu_build
                        "${options}"
                        "${oneValueArgs}"
                        "${multiValueArgs}"
                        ${ARGN})
  ...
  android_add_executable(TARGET ${qemu_build_EXE}
                         LICENSE Apache-2.0
                         LIBNAME qemu-system URL
...
function(android_copy_shared_lib
         TGT
         SHARED_LIB
         NAME)
...
function(android_upload_symbols TGT)
  ...
  install(CODE # Upload the symbols, with warnings/error logging only.
