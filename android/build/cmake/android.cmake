...
#
# EXE the name of the target we are interested in. This is the main build product you want to use. SOURCE the location
# of the CMakeList.txt describing the project. OUT_PATH Name of the variable that will contain the resulting executable.
function(android_compile_for_host EXE SOURCE OUT_PATH)
  if(NOT CROSSCOMPILE)
    ...
    get_filename_component(BUILD_PRODUCT
                           ${ANDROID_CROSS_BUILD_DIRECTORY}/${EXE}_ext_cross/src/${EXE}_ext_cross-build/${EXE} ABSOLUTE)
...
function(android_nasm_compile)
  ...
  foreach(asm ${REGISTERED_SRC})
    ...
    add_custom_command(OUTPUT ${DST} COMMAND ${nasm_EXECUTABLE} -f ${ANDROID_ASM_TYPE} -o ${DST} ${asm} ${ASM_INC}
                       WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR} VERBATIM DEPENDS ${nasm_EXECUTABLE} ${asm})
...
# (https://cmake.org/cmake/help/v3.5/command/target_link_libraries.html) T he only difference is that the definitions
...
function(_register_target)
  ...
  if(NOT DEFINED build_LICENSE)
    message(FATAL_ERROR "You must set a license for ${build_TARGET}, or declare NODISTRIBUTE")
  endif()

  if(NOT DEFINED build_NOTICE)
    set(build_URL "https://android.googlesource.com/platform/external/qemu.git/+/refs/heads/emu-master-dev")
    set(build_REPO "INTERNAL")
    set(build_NOTICE "REPO/LICENSES/LICENSE.APACHE2")
  endif()

  if(NOT DEFINED build_URL OR NOT DEFINED build_REPO OR NOT DEFINED build_NOTICE)
    message(FATAL_ERROR "You must set a notice/url/repo for ${build_TARGET}, or declare NODISTRIBUTE")
  endif()
  if(DEFINED build_NOTICE AND NOT ${build_NODISTRIBUTE})
    ...
    android_license(TARGET ${build_TARGET} URL ${build_URL} LIBNAME ${build_LIBNAME} SPDX ${build_LICENSE}
                    LICENSE ${REMOTE_LICENSE} LOCAL ${LOCAL_LICENSE})
  endif()
endfunction()
...
function(android_add_library)
  ...
  if(${build_SHARED})
    ...
    if(ANDROID_TARGET_TAG MATCHES "windows.*")
      set_target_properties(${build_TARGET} PROPERTIES RUNTIME_OUTPUT_DIRECTORY ${CMAKE_LIBRARY_OUTPUT_DIRECTORY})
    endif()
    if(CROSSCOMPILE AND WINDOWS_MSVC_X86_64)
      ...
      # ${CMAKE_ARCHIVE_OUTPUT_DIRECTORY} or we can't link to it. Most windows compilers, including clang, don't allow
      # you to directly link to a dll (unlike mingw), and instead, need to link to it's import library.
      #
...
#
# result: The variable containing all the targets defined by this project subdir: The directory of interest
...
# Adds an external project, transforming all external "executables" to include the runtime properties. On linux for
# example this will set the rpath to find libc++
#
...
define_property(GLOBAL PROPERTY LICENSES_LST BRIEF_DOCS "Global list of license definitions"
                FULL_DOCS "Global list of license definitions")
define_property(GLOBAL PROPERTY INSTALL_TARGETS_LST BRIEF_DOCS "GLobal list of targets that are installed."
...
#
# ``LIBNAME`` The name of the library, this is how it is usually known in the world. for example: libpng
...
function(android_license)
  ...
  set_property(GLOBAL APPEND PROPERTY LICENSES_LST
...
function(finalize_all_licenses)
  ...
  execute_process(
    COMMAND "python" "android/build/python/aemu/licensing.py" "${PROJECT_BINARY_DIR}"
    WORKING_DIRECTORY ${ANDROID_QEMU2_TOP_DIR} RESULT_VARIABLE LICENSE_RES OUTPUT_VARIABLE STD_OUT
    ERROR_VARIABLE STD_ERR)
  if(NOT "${LICENSE_RES}" STREQUAL "0")
    message(FATAL_ERROR "Unable to validate licenses, out: ${STD_OUT}, err: ${STD_ERR}")
  endif()
endfunction()

# Creates the dependency
function(android_install_license tgt targetname)
  set_property(GLOBAL APPEND PROPERTY INSTALL_TARGETS_LST "${tgt}|${targetname}\n")
endfunction()
...
function(android_add_test)
  ...
  # We cannot run tests when we are cross compiling, and for mingw it is very expensive.
  if(WINDOWS_X86_64)
  ...
  add_test(NAME ${build_TARGET}
           COMMAND $<TARGET_FILE:${build_TARGET}> --gtest_output=xml:$<TARGET_FILE_NAME:${build_TARGET}>.xml
                   --gtest_catch_exceptions=0 WORKING_DIRECTORY $<TARGET_FILE_DIR:${build_TARGET}>)
...
# Installs the given target into ${DBG_INFO}/tests/ directory.. This is mainly there so we can preserve it as part of
...
function(android_install_as_debug_info name)
  ...
  add_custom_command(TARGET ${name} POST_BUILD COMMAND ${CMAKE_COMMAND} -E copy_if_different $<TARGET_FILE:${name}>
...
# use execeptions. Protobuf targets will be licensed under the Apache-2.0 license.
#
...
function(android_add_protobuf name protofiles)
  ...
  android_add_library(TARGET ${name} LICENSE Apache-2.0 SRC ${PROTO_SRCS} ${PROTO_HDRS})
...
function(android_generate_hw_config)
  ...
  add_custom_command(
    ...
    COMMAND python ${ANDROID_QEMU2_TOP_DIR}/android/scripts/gen-hw-config.py
            ${ANDROID_QEMU2_TOP_DIR}/android/android-emu/android/avd/hardware-properties.ini ${ANDROID_HW_CONFIG_H}
    DEPENDS ${ANDROID_QEMU2_TOP_DIR}/android/android-emu/android/avd/hardware-properties.ini
    VERBATIM)
  android_add_library(TARGET android-hw-config LICENSE Apache-2.0 SRC ${ANDROID_HW_CONFIG_H} dummy.c)
  set_source_files_properties(${ANDROID_HW_CONFIG_H} PROPERTIES GENERATED TRUE SKIP_AUTOGEN ON HEADER_FILE_ONLY TRUE)
...
function(android_copy_test_files TGT SOURCE_LIST DEST)
  ...
  foreach(SRC_FILE ${SOURCE_LIST})
    ...
    add_custom_command(
      TARGET ${TGT} PRE_BUILD COMMAND ${CMAKE_COMMAND} -E copy_if_different ${TESTDATA_ROOT}/${SRC_FILE}
...
function(android_copy_file TGT SRC DST)
  add_custom_command(TARGET ${TGT} PRE_BUILD COMMAND ${CMAKE_COMMAND} -E copy_if_different ${SRC} ${DST}
                                                     MAIN_DEPENDENCY ${SRC})
endfunction()
...
function(android_copy_test_dir TGT SRC_DIR DST_DIR)
  get_filename_component(TESTDATA_ROOT
                         "${ANDROID_QEMU2_TOP_DIR}/../../prebuilts/android-emulator-build/common/testdata" ABSOLUTE)

  add_custom_command(TARGET ${TGT} POST_BUILD COMMAND ${CMAKE_COMMAND} -E copy_directory ${TESTDATA_ROOT}/${SRC_DIR}
...
function(get_git_version VER)
  execute_process(COMMAND "git" "describe" WORKING_DIRECTORY ${ANDROID_QEMU2_TOP_DIR} RESULT_VARIABLE GIT_RES
  ...
  if(NOT "${GIT_RES}" STREQUAL "0")
    ...
    if(NOT MSVC)
      execute_process(COMMAND "date" "+%Y-%m-%d" WORKING_DIRECTORY ${ANDROID_QEMU2_TOP_DIR} RESULT_VARIABLE DATE_RES
                      OUTPUT_VARIABLE STD_OUT ERROR_VARIABLE STD_ERR)
      if(NOT "${DATE_RES}" STREQUAL "0")
        message(FATAL_ERROR "Unable to retrieve date!")
      endif()
    else()
      execute_process(COMMAND "date" "/T" WORKING_DIRECTORY ${ANDROID_QEMU2_TOP_DIR} RESULT_VARIABLE DATE_RES
                      OUTPUT_VARIABLE STD_OUT ERROR_VARIABLE STD_ERR)
    endif()
  endif()
...
function(get_git_sha VER)
  execute_process(COMMAND "git" "log" "-n" "1" "--pretty=format:'%H'" WORKING_DIRECTORY ${ANDROID_QEMU2_TOP_DIR}
...
#
# ANDROID_AARCH The android architecture name QEMU_AARCH The qemu architecture name. CONFIG_AARCH The configuration
# architecture used. STUBS The set of stub sources to use. CPU The target cpu architecture used by qemu
#
...
function(android_build_qemu_variant)
  ...
  android_add_executable(
    ...
    LICENSE Apache-2.0
    LIBNAME qemu-system URL "https://android.googlesource.com/platform/external/qemu/+/refs/heads/emu-master-dev"
    REPO "${ANDROID_QEMU2_TOP_DIR}"
    NOTICE "REPO/LICENSES/LICENSE.APACHE2"
    SRC ${qemu_build_SOURCES} ${qemu-system-${QEMU_AARCH}_sources} ${qemu-system-${QEMU_AARCH}_generated_sources})
  target_include_directories(${qemu_build_EXE} PRIVATE android-qemu2-glue/config/target-${CONFIG_AARCH} target/${CPU})
  target_compile_definitions(${qemu_build_EXE} PRIVATE ${qemu_build_DEFINITIONS})
  target_link_libraries(${qemu_build_EXE} PRIVATE ${QEMU_COMPLETE_LIB} ${qemu_build_LIBRARIES})
  ...
  if(qemu_build_INSTALL)
    set_target_properties(
      ${qemu_build_EXE} PROPERTIES RUNTIME_OUTPUT_DIRECTORY
...
#
# ANDROID_AARCH The android architecture name STUBS The set of stub sources to use.
function(android_add_qemu_executable ANDROID_AARCH STUBS)
  android_build_qemu_variant(
    ...
    DEFINITIONS -DNEED_CPU_H -DCONFIG_ANDROID -DANDROID_SDK_TOOLS_REVISION=${OPTION_SDK_TOOLS_REVISION}
...
function(android_add_qemu_headless_executable ANDROID_AARCH STUBS)
  android_build_qemu_variant(
    ...
    SOURCES android-qemu2-glue/main.cpp vl.c ${STUBS}
    DEFINITIONS
      -DNEED_CPU_H -DCONFIG_ANDROID -DCONFIG_HEADLESS -DANDROID_SDK_TOOLS_REVISION=${OPTION_SDK_TOOLS_REVISION}
      -DANDROID_SDK_TOOLS_BUILD_NUMBER=${OPTION_SDK_TOOLS_BUILD_NUMBER}
    LIBRARIES libqemu2-glue
              libqemu2-glue-vm-operations
              libqemu2-util
...
function(android_install_shared TGT)
  install(TARGETS ${TGT} RUNTIME DESTINATION lib64 # We don't want windows to binplace dlls in the exe dir
...
function(android_extract_symbols TGT)
  ...
  set(DEST "${ANDROID_SYMBOL_DIR}/${TGT}.sym")
  add_custom_command(TARGET ${TGT} POST_BUILD COMMAND dump_syms "$<TARGET_FILE:${TGT}>" > ${DEST} 2> ${DEV_NULL} DEPENDS
                                                      dump_syms COMMENT "Extracting symbols for ${TGT}" VERBATIM)
endfunction()
