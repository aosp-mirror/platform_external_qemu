...
function(android_target_link_libraries TGT OS MODIFIER ITEMS)
  ...
  if(ANDROID_TARGET_TAG MATCHES "${OS}.*" OR ${OS} STREQUAL "all")
    ...
    target_link_libraries(${TGT}
                          ${MODIFIER}
                          ${ARGV3} ${ARGV4} ${ARGV5} ${ARGV6} ${ARGV7} ${ARGV8} ${ARGV9} ${ARGV10}
...
function(android_add_test name)
  ...
  # We are not registering tests when cross compiling to windows
  # - The build bots do not have wine installed
  # - Some tests are flaky under wine
  if (NOT WIN32 AND ANDROID_TARGET_TAG MATCHES "windows.*")
    return()
  endif()
...
function(android_add_executable name)
  ...
  if (ANDOID_SYMBOLS)
    android_copy_symbols(${name})
  endif()
endfunction()

# Copies the symbols from the given target..
function(android_copy_symbols TGT)
 prebuilt(BREAKPAD)
 set(DEBUG_INFO_DEST ${CMAKE_CURRENT_BINARY_DIR}/build/debug-info/$<TARGET_FILE_NAME:${TGT}>)
 # Let's start by copying the debug-info over.
 if (ANDROID_TARGET_TAG STREQUAL "darwin-x86_64")
   add_custom_command(TARGET ${TGT} POST_BUILD
                      OUTPUT ${DEBUG_INFO_DEST}
                      COMMAND  dsymutil --out=${DEBUG_INFO_DEST}.dsym $<TARGET_FILE:${TGT}>
                     )
  else()
    add_custom_command(TARGET ${TGT} POST_BUILD
                       COMMAND ${CMAKE_COMMAND} -E copy_if_different $<TARGET_FILE:${TGT}> ${DEST}
                       MAIN_DEPENDENCY ${SRC})
  endif()

  # And then dumping the symbols
  # add_custom_command(TARGET ${TGT} POST_BUILD)
endfunction()
...
function(android_validate_sha256 FILE EXPECTED)
  ...
  if (NOT CHECKSUM STREQUAL "${EXPECTED}")
     get_filename_component(DEST "${ANDROID_QEMU2_TOP_DIR}/../../device/generic/goldfish-opengl"
     ABSOLUTE)
     message(FATAL_ERROR "Checksum mismatch for ${FILE} = ${CHECKSUM}, expecting ${EXPECTED}, you need to regenerate the cmake files by executing 'make' in ${DEST}")
  endif()
endfunction()