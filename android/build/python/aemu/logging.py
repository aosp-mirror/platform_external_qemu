...
class LevelSplitter(pylog.StreamHandler):
...
class NinjaLevelSplitter(LevelSplitter):
    ...
    def __init__(self):
        self._prev_handler = logging.get_absl_handler()._python_handler
    ...
    def __enter__(self):
        self._prev_handler = logging.get_absl_handler()._python_handler
        logging.get_absl_handler()._python_handler = self
        logging.get_absl_handler().activate_python_handler()
        return self

    def __exit__(self, exc_type, exc_value, exc_traceback):
        logging.get_absl_handler()._python_handler = self._prev_handler
