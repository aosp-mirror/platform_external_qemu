...
from definitions import CMAKE_DIR, QEMU_ROOT, EXE_POSTFIX
...
def run_tests(out_dir):
    if platform.system() == 'Windows':
        ...
        run([os.path.join(QEMU_ROOT, 'android', 'scripts', 'run_tests.sh'), '--out-dir=%s' % out_dir, '--verbose', '--verbose'])
...
def get_ctest():
    ...
    return os.path.join(CMAKE_DIR, 'ctest%s' % EXE_POSTFIX)
