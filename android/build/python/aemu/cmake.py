...
import process
from definitions import Generator, Crash, BuildConfig, Toolchain, AOSP_ROOT, QEMU_ROOT, CMAKE_DIR, Make, EXE_POSTFIX
...
flags.DEFINE_enum('build', 'check', Make.values(), 'Target we want to build, if any')
...
def get_cmake():
    return os.path.join(CMAKE_DIR, 'cmake%s' % EXE_POSTFIX)
...
def main(argv):
    ...
    logging.info('Running under Python {0[0]}.{0[1]}.{0[2]}, Platform: {1}'.format(version, platform.platform()))
    configure()
    if FLAGS.build == Make.config:
        print('You can now build with: %s ' % ' '.join(get_build_cmd()))
        return

    process.run(get_build_cmd())
    run_tests(FLAGS.out)
