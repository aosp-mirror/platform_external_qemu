...
flags.DEFINE_enum('symbol_dest', 'none', SymbolUris.values(),
                  'Deprecated, the crash server determines the symbol endpoint.')
...
def get_build_cmd():
    ...
    target = 'install'
    # Only strip if we are not cross compiling.
    if FLAGS.symbols and platform.system().lower() == FLAGS.target:
        target += '/strip'
