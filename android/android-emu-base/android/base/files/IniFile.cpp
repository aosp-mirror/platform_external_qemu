...
void IniFile::parseStream(std::istream* in, bool keepComments) {
    ...
    while (std::getline(*in, line)) {
        ...
        if (citer == cend) {
            VLOG(ini) << "Line " << lineno << ": Skipped empty line.";
            if (keepComments) {
        ...
        if (*citer == '#' || *citer == ';') {
            VLOG(ini) << "Line " << lineno << ": Skipped comment line.";
            if (keepComments) {
                mComments.emplace_back(outputLineno, std::move(line));
            }
            continue;
...
int64_t IniFile::getInt64(const string& key, int64_t defaultValue) const {
    ...
    if (errno || *end != 0) {
        VLOG(ini) << "Malformed int64 value " << value << " for key "
                         << key;
        return defaultValue;
    }
    return result;
...
double IniFile::getDouble(const string& key, double defaultValue) const {
    ...
    if (errno || *end != 0) {
        VLOG(ini) << "Malformed double value " << value << " for key "
                         << key;
        return defaultValue;
    }
    return result;
...
bool IniFile::getBool(const string& key, bool defaultValue) const {
    ...
    if (isBoolTrue(value)) {
        ...
        VLOG(ini) << "Malformed bool value " << value << " for key "
                         << key;
        return defaultValue;
    }
}
