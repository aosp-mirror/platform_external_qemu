...
TEST_F(AsyncMetricsReporterTest, reportConditional) {
    ...
    mWriter->mOnWrite =
            [threadId](const android_studio::AndroidStudioEvent& asEvent,
                ...
                // Verify the fields AsyncMetricsReporter is supposed to fill
                // in.
                EXPECT_TRUE(asEvent.has_product_details());
                EXPECT_STREQ(kVersion,
    ...
    mReporter->reportConditional(
            [threadId,
             &reportCallbackCalls](android_studio::AndroidStudioEvent* event) {
...
TEST_F(AsyncMetricsReporterTest, finishPendingReports) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        mReporter->reportConditional(
                [&reported](android_studio::AndroidStudioEvent*) {
                    ++reported;
                    return true;
                });
        mReporter->reportConditional(
