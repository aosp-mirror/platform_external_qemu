...
/*            p = bufprint(p, end, " androidboot.console=ttyS%d", shell_serial );*/
    ...
    /* Set up the interfaces and boot properties for inter-emulator networking */
    if (opts->shared_net_id) {
        unsigned int shared_net_id = atoi(opts->shared_net_id);
        char ip[11];
        snprintf(ip, 11, "10.1.2.%d", shared_net_id);
        boot_property_add("net.shared_net_ip",ip);

        char nic[50];
        args[n++] = "-net";
        sprintf(nic, "nic,vlan=1,macaddr=52:54:00:12:34:%02x", shared_net_id);
        args[n++] = nic;
        args[n++] = "-net";
