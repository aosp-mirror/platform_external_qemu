...
{
    int fd = android_console_open(sockaddr);
    if (fd < 0) {
        return -1;
    } else {
        socket_close(fd);
...
attach_to_core(AndroidOptions* opts) {
     ...
     * structure. */
    sockaddr_list = sock_address_list_create(host_name, port_num, 0);
    ...
    printf("Attaching to core %s\n", sock_address_to_string(&console_socket));

    AndroidConsoleClient* attach_client =
        android_console_client_create(&console_socket);
    if (attach_client != NULL) {
        if (!android_console_client_open(attach_client)) {
            char* handshake = NULL;
            android_console_client_switch_stream(attach_client, "attach UI", &handshake);
            printf("Handshake: %s\n", handshake);
