...
int main(int argc, char **argv)
{
    ...
    if (opts->my_ip) {
        uint32_t dummy_ip;
        if (strncmp("10.1.2.", opts->my_ip, 7) != 0 || inet_strtoip(opts->my_ip, &dummy_ip) < 0) {
            fprintf(stderr, "invalid -my-ip parameter '%s', must be an IP address in the 10.1.2.x range\n", opts->my_ip);
            exit(1);
        }
    }
