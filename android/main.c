...
/* Informational QEMU options, which make QEMU print some information
 * to the console and exit. */
static const char* qemu_info_opts[] = {
    "-h",
    "-help",
    "-version",
    "-audio-help",
...
/*
 * _findQemuInfoOpt: search for informational QEMU options
 *
 * Scans the given command-line options for any informational QEMU
 * option (see |qemu_info_opts| for the list of informational QEMU
 * options). Returns the first matching option, or NULL if no match is
 * found.
 *
 * |qemu_argc| is the number of command-line options in |qemu_argv|.
 * |qemu_argv| is the array of command-line options to be searched. It
 * is the caller's responsibility to ensure that all these options are
 * intended for QEMU.
 */
static char*
_findQemuInfoOpt( int qemu_argc, char** qemu_argv )
{
    int i = 0;

    for (; i < qemu_argc; i++) {
        char* arg = qemu_argv[i];
        const char** oo = qemu_info_opts;
...
int main(int argc, char **argv)
{
    ...
    /* Both |argc| and |argv| have been modified by the big while loop
     * above: |argc| should now be the number of options after '-qemu',
     * and if that is positive, |argv| should point to the first option
     * following '-qemu'.
     * Now we check if any of these QEMU options is an 'informational'
     * option, e.g. '-h', '-version', etc.
     * The extra pair of parentheses is to keep gcc happy. */
    if ((qemu_info_opt = _findQemuInfoOpt(argc, argv))) {
        D("Found informational option '%s' after '-qemu'.\n"
          "All options before '-qemu' will be ignored!", qemu_info_opt);

        /* Copy all QEMU options to |args|, and set |n| to the number of
         * options in |args| (|argc| must be positive here). */
        n = 1;
        do {
            args[n] = argv[n - 1];
        } while (n++ < argc);
