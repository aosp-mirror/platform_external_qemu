...
#define CORE_PORT_TIMEOIT_MS    500
...
check_for_running_core(SockAddress* sockaddr)
{
    ...
    if (connect_status < 0 && errno == EINPROGRESS) {
        ...
        connect_status = iolooper_wait(looper, CORE_PORT_TIMEOIT_MS);
        if (connect_status > 0) {
    ...
    if (connect_status >= 0) {
        ...
        if (iolooper_wait(looper, CORE_PORT_TIMEOIT_MS) > 0 &&
...
list_running_cores(const char* host)
{
    ...
    if (sockaddr_list == NULL) {
        fprintf(stderr, "Unable to build socket address for host %s: %s\n",
                host, errno_str);
        return;
    }
    ...
    if (sockaddr_list[iter] == NULL) {
        fprintf(stderr, "Unix socket is an invalid parameter for -list-cores option.\n");
...
int main(int argc, char **argv)
{
    ...
    if (opts->list_cores) {
        fprintf(stdout, "Enumerating running core processes.\n"
                        ...
                        (MAX_CORE_PROCS * CORE_PORT_TIMEOIT_MS) / 1000 + 1);
