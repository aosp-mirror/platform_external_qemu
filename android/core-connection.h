 ...
 *  set.
 */
SyncSocket* android_open_core_connection_socket(SockAddress* sockaddr);
 ...
 */
CoreConnection* android_create_core_connection(SockAddress* console_socket);

/* Frees descriptor allocated with android_create_core_connection.
 * Param:
 *  desc Descriptor to free. Note that this routine will simply free the memory
 *      used by the descriptor.
 */
void android_free_core_connection(CoreConnection* desc);
 ...
 */
int android_open_core_connection(CoreConnection* desc);

/* Closes a socket handle to the console opened with android_open_core_connection.
 * Param:
 *  desc Console client descriptor opened with android_open_core_connection.
 */
void android_close_core_connection(CoreConnection* desc);
 ...
 */
int android_write_core_connection(CoreConnection* desc,
                                  const void* buffer,
                                  size_t to_write,
 ...
 *  0 on success, or -1 on failure.
 */
int android_read_core_connection(CoreConnection* desc,
                                 void* buffer,
                                 size_t to_read,
                                 size_t* read_bytes);
 ...
 * Param:
 *  desc Console client descriptor opened with android_open_core_connection. Note
 ...
 *  0 on success, or -1 on failure.
 */
int android_switch_core_connection_stream(CoreConnection* desc,
 ...
 *  desc Console client descriptor opened with android_open_core_connection.
 */
void android_detach_core_connection(CoreConnection* desc);
