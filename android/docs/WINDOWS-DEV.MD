...
2. The next step is to obtain a copy of [git-remote-sso.exe](https://x20web.corp.google.com/teams/gerritcodereview/bin/git-remote-sso.exe), and place it in the git\cmd directory (or anywhere else on the PATH). 
...
4. Make sure we have a repo launcher somewhere on the path.
  ...
  6. Sync the repo (and get some coffee, or a have a good nap.)
   ...
   7. Congratulations! You have all the sources you need. Now open
   
      - Open CMakeLists.txt in Visual Studio 2017. 
      - Use Visual Studio code with [CMake Tools](https://vector-of-bool.github.io/docs/vscode-cmake-tools/index.html) extension
      - Use cmake to generate a Visual Studio project: ```cmake -G "Visual Studio 15"```
