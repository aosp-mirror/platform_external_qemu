...
ConsoleAuthToken::ConsoleAuthToken(OpenMode openMode) {
    ...
    if (openMode == OpenMode::CreateOnDemand) {
        ...
        if (fd.get() < 0) {
            ...
            std::string token64 = android::base64Encode(
                    StringView(reinterpret_cast<char*>(tokenBytes), kSize));
    ...
    const size_t bufferLen = ((kSize + 2) / 3) * 4;
    ...
    std::string data;
    if (!android::base64Decode(
                StringView(reinterpret_cast<const char*>(buffer), bufferLen),
                &data)) {
...
bool ConsoleAuthToken::checkAgainst(android::base::StringView str) {
    ...
    std::string data;
    if (!android::base64Decode(str, &data)) {
