...
FilePusher::FilePusher(Looper* looper,
          Looper::Duration progressCheckTimeoutMs) : mLooper(looper),
    mProgressCheckTimeoutMs(progressCheckTimeoutMs), mSys(System::get()){}
...
void FilePusher::pushNextItem() {
    ...
    if (!mSys->pathCanRead(mCurrentItem.first)) {
...
void FilePusher::pushOneFile(vector<string> commandLine, Result* outResult) {

    System::ProcessExitCode exitCode;
    if (!mSys->runCommand(commandLine,
