...
// The console will then output "OK" (test was able to run) or "KO"
// (test failed to run). The test result will be appended to both stdout
// and a file determined by the emulator (there will be a print to stdout
// of the file's location when the test runner initializes).
//
...
typedef struct QAndroidTestAgent {
    ...
    // Gets location of test results
    // Another double-call. This is used internally by the
    // console to generate more helpful messages.
    void (*getResultLocation)(char* buf, unsigned int* sz);
    ...
    // Returns whether or not the test was able to run.
    // The result of the test is appended to a temp file,
    // the name of which can be obtained using listTests.
