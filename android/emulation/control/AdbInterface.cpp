...
public:
    ...
    const std::string& detectedAdbPath() const override final { return mAutoAdbPath; }
...
AdbInterfaceImpl::AdbInterfaceImpl(android::base::Looper* looper)
    ...
    if (!sdk_root_by_env.empty()) {
        ...
        mAutoAdbPath = PathUtils::join(sdk_root_by_env, "platform-tools", "adb");
...
AdbCommandPtr AdbInterfaceImpl::runAdbCommand(
    ...
    auto command = std::shared_ptr<AdbCommand>(
            new AdbCommand(mLooper, mCustomAdbPath.empty() ? mAutoAdbPath : mCustomAdbPath, mSerialString, args,
