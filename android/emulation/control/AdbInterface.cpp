...
bool checkAdbVersion(const std::string& sdk_root_directory) {
    ...
    std::string properties_path =
        PathUtils::join(android::base::System::get()->getRootDirectory(),
                        sdk_root_directory,
                        "platform-tools",
                        "source.properties");
...
AdbInterface::AdbInterface() : mAdbVersionCurrent(false) {
    ...
    auto sdk_root_directory = android::ConfigDirs::getSdkRootDirectoryByEnv();
    if (!sdk_root_directory.empty()) {
        ...
        mAdbPath =
            PathUtils::join(sdk_root_directory, "platform-tools", "adb");
        if (checkAdbVersion(sdk_root_directory)) {
    ...
    sdk_root_directory = android::ConfigDirs::getSdkRootDirectoryByPath();
    if (!sdk_root_directory.empty()) {
        if (checkAdbVersion(sdk_root_directory)) {
            mAdbPath =
                PathUtils::join(sdk_root_directory, "platform-tools", "adb");
