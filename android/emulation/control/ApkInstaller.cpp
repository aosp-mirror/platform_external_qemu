...
const char ApkInstaller::kInstallOutputFileName[] = "adb-install-output.txt";
...
intptr_t ApkInstaller::taskFunction(ApkInstaller::Result* outResult) {
    *outResult = Result::kUnknownError;
    ...
    System::Pid pid;
    string outputFilePath = PathUtils::join(System::get()->getTempDir(),
    ...
    if (!sys->runCommand(command,
                         ...
                         kInstallTimeoutMs, &exitCode, &pid, outputFilePath) ||
        exitCode != 0) {
    ...
    string errorCode;
    if (!parseOutputForFailure(outputFilePath, &errorCode)) {
        ...
        while (getline(file, line)) {
            if (!line.compare(0, 7, "Failure")) {
                ...
                if (openBrace != string::npos && closeBrace != string::npos) {
                    *outErrorString = line.substr(openBrace + 1,
                                                  closeBrace - openBrace - 1);
                } else {
                    *outErrorString = "Could not parse error string";
                }
                return false;
            }
        }
