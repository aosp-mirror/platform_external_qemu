...
struct AdbCommandResult {
    ...
    // NOTE: we're using unique_ptr here because
    // there's a bug in g++ 4 due to which file
    // strreams are unmovable.
    mutable std::unique_ptr<std::ifstream> output;
};
using OptionalAdbCommandResult = android::base::Optional<AdbCommandResult>;

// A utility class used to automatically locate the ADB binary
// and asynchronously run ADB commands.
class AdbInterface {
public:
...
private:
    ...
    android::base::Looper* mLooper;
    std::unique_ptr<android::base::ParallelTask<OptionalAdbCommandResult>> mTask;
