...
#if defined(__X86__) && defined (USE_TSC)
...
int android_twitter_init( void )
{
    ...
    if(twitter_stream == NULL) {
        ...
        fprintf(stdout, "New twitter log @ %s; will be deleted on exit", tfpath);
    }
...
int android_tweet(unsigned long long tsc, const char *msg)
{
    ...
    if(now_mono - event_mono > 1e9) {
      event_mono = now_mono;
      event_gid++;
    }

    ret = fprintf(twitter_stream, "H: %-16llu G: %-16llu I: %ld T: %-16llu %s\n",
                  now_cpu, tsc, event_gid, now_mono, msg);
    D("H: %-16llu G: %-16llu I: %-8ld T: %-16llu %s\n",
      now_cpu, tsc, event_gid, now_mono, msg);

    return ret; // TODO: only count user msg length --- subtract rest of strlen
}
