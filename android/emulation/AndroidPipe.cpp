...
public:
    ...
    virtual int onGuestSend(const AndroidPipeBuffer* buffers,
        ...
        for (; numBuffers > 0; buffers++, numBuffers--) {
            if (buffers[0].size == 0) {
                // Skip empty buffers.
                continue;
            }
            int avail = std::min(sizeof(mBuffer) - mPos, buffers[0].size);
            if (avail > 0) {
                ::memcpy(mBuffer + mPos, buffers[0].data, avail);
                mPos += avail;
                result += avail;
            }
        }
    ...
    virtual void onSave(BaseStream* stream) override {
        ...
        stream->putByte(static_cast<uint8_t>(mPos));
    ...
    bool onLoad(BaseStream* stream) {
        ...
        if (len < 0 || len > static_cast<int>(sizeof(mBuffer))) {
            D("%s: invalid length %d (expected 0 <= len <= %d)", __FUNCTION__,
              len, static_cast<int>(sizeof(mBuffer)));
            return false;
        }
        return (stream->read(mBuffer, mPos) == mPos);
    }
...
public:
    ...
    virtual AndroidPipe* load(void* hwPipe,
        ...
        ConnectorPipe* pipe =
                reinterpret_cast<ConnectorPipe*>(create(hwPipe, args));
...
struct Globals {
    ...
    const Service* loadServiceByName(BaseStream* stream) const {
        ...
        DD("%s: found [%s]", __FUNCTION__, serviceName.value().c_str());
        return this->findServiceByName(serviceName.value().c_str());
    }
};
...
AndroidPipe* loadPipeFromStreamCommon(BaseStream* stream,
    ...
    *pForceClose = 0;
    ...
    if (service->canLoad()) {
        ...
        pipe = service->load(hwPipe, args ? args.value().c_str() : nullptr,
                             stream);
    } else {
...
void android_pipe_guest_wake_on(void* internalPipe, unsigned wakes) {
    auto pipe = static_cast<AndroidPipe*>(internalPipe);
    //DD("%s: host=%p [%s] wakeFlags=%u", __FUNCTION__, pipe, pipe->name(),
    //   wakes);
...
void android_pipe_host_signal_wake(void* hwpipe, unsigned flags) {
    // DD("%s: hwpipe=%p wakeFlags=%d", __FUNCTION__, hwpipe, flags);
