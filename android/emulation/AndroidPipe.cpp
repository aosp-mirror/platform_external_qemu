...
static void pipeWakerDeviceOp(void* data) {
    PipeWakeCommand* wake_cmd = (PipeWakeCommand*)data;
...
static void pipeWakerDeviceOpPost(void* data) {
    PipeWakeCommand* wake_cmd = (PipeWakeCommand*)data;
    delete wake_cmd;
}

class PipeWaker final : public DeviceContextRunner {
public:
    void signalWake(void* hwPipe, int wakeFlags) {
        PipeWakeCommand* wake_cmd = new PipeWakeCommand;
        wake_cmd->hwPipe = hwPipe;
        wake_cmd->wakeFlags = wakeFlags;
        queueDeviceOperation((void*)wake_cmd);
    }
...
void AndroidPipe::initThreading(VmLock* vmLock) {
    ...
    sGlobals->pipeWaker.setDeviceOperations(pipeWakerDeviceOp,
                                            pipeWakerDeviceOpPost);
}
