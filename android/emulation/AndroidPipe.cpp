...
static void pipeWakerDeviceOp(const PipeWakeCommand& wake_cmd) {
...
class PipeWaker final :
    public DeviceContextRunner<PipeWakeCommand> {
public:
...
void AndroidPipe::initThreading(VmLock* vmLock) {
    ...
    sGlobals->pipeWaker.setDeviceOperation(pipeWakerDeviceOp);
}
