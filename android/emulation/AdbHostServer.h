...
struct AdbHostServer {
    ...
    // for
    // connections on |adbEmulatorPort| localhost TCP port. |adbClientPort| is
    // the localhost TCP port the ADB server is listening on for client
    // connections where the message should be sent. Except during unit-testing,
    // this should be the result of getClientPort(). Return true on success,
    // false/errno on error.
