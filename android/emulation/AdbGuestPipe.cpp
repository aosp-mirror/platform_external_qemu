...
AndroidPipe* AdbGuestPipe::Service::create(void* mHwPipe, const char* args) {
    ...
    const_cast<Service*>(this)->onPipeOpen(pipe);
...
void AdbGuestPipe::Service::onPipeClose(AdbGuestPipe* pipe) {
    ...
    if (mActivePipe == pipe) {
        mActivePipe = nullptr;
        if (!mPipes.empty()) {
            // There are pending pipes waiting for a new connection from
            // the ADB Server, so tell the host agent to start listening
            // again.
            mHostAgent->startListening();
