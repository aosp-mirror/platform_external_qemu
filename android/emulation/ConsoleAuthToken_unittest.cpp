...
static void makeUserDir(const TestSystem& sys) {
    makeHomeDir(sys);
    ASSERT_TRUE(sys.pathFileMkDir("/home/.android", 0700));
}

static const char kAuthFile[] =
        "/home/.android/android_emulator_console_auth_token";

static void writeConsoleAuthTokenFile(const TestSystem& sys, StringView token) {
    ScopedFd fd(sys.pathFileCreate(kAuthFile, 0600));
...
TEST(ConsoleAuthToken, NoHomeDirectory) {
    ...
    EXPECT_FALSE(token->isValid());
    EXPECT_STREQ("Could not create user configuration directory!",
...
TEST(ConsoleAuthToken, TokenFileWithBadPermissions) {
    ...
    int fd = sys.pathFileCreate(kAuthFile, 0777);
    EXPECT_GE(fd, 0);
    ::close(fd);
