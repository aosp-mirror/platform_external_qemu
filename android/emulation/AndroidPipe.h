...
// It's important to understand that pipe operations (onGuestXXX methods) are
// performed on the 'device thread', which really means different things
// depending on the emulation engine:
//
...
//   the lock.
//
// All AndroidPipe methods *must* be called from within the device thread
// context.
// TODO(digit): Make some methods callable from any thread.
//
...
public:
    ...
    // PIPE_WAKE_XXX bits in |flags| occurred. NOTE: This must be called from
    // the device thread!
    void signalWake(int flags);

    // Method used to signal the guest that the pipe instance wants to close
    // the pipe. NOTE: This must be called from the device thread!
    void closeFromHost();
