...
//      should probably be ignored (it is suspected to be here for legacy
//      reasons, or for non-emulation usage).
//
...
//      AdbHostAgent instance to it (in practice a AdbHostListener one).
//      Then call AndroidPipe::Service::add() to register it.
//
// NOTE: This must be done at emulation setup time from the QEMU main loop!
//
class AdbGuestPipe : public AndroidPipe, public AdbGuestAgent {
public:
    ...
    public:
        ...
        virtual AndroidPipe* create(void* mHwPipe,
                                    const char* args) const override;
    ...
    // Overriden AndroidPipe methods. Called from the device context.
    ...
    // API called from the AdbServer class.

    // These must be called in a thread that already owns the global VM
    // state lock.

    // Implement AdbGuestPipe::onHostConnection().
    virtual bool onHostConnection(int socket) override;

private:
    AdbGuestPipe(void* mHwPipe, const Service* service, AdbHostAgent* hostAgent)
    ...
    StringView mCommand;     // the command being accepted or reply being sent.
    size_t mCommandPos = 0;  // number of matched command bytes on input/output.
