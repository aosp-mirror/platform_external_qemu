...
static void* selfTestWriteThreadFunction(void* param) {
    // GoldfishSyncTestState* p = static_cast<GoldfishSyncTestState*>(param);
...
static const size_t kSyncDeviceTestNumThreads = 10;
...
static bool goldfishSyncMultiThread() {
    ...
    // Don't do this until the test runner is not on the main thread
    for (size_t i = 0; i < kSyncDeviceTestNumThreads; i++) {
        threads[i].join();
    }
