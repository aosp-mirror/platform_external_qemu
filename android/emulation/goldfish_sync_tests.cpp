...
public:
    GoldfishSyncState() : mLock(), mCount(0) {}
    ~GoldfishSyncState() {}
...
static const size_t kSelfTestNumThreads = 100;

static void goldfishSyncMultiThread() {
    DPRINT("start");
...
void add_goldfish_sync_tests() {
    DPRINT("enter");
    test_runner_add_test("goldfishsyncmulti", goldfishSyncMultiThread);
}
