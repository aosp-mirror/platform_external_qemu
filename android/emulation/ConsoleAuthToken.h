...
public:
    ...
    // false in case of error, call getStatus() to know why.
    ...
    // if this matches the token bytes, or if the token value is the special
    // 'passthrough' value that accepts anything (and disables security).
    bool checkAgainst(android::base::StringView str);

    // Name of auth token file in the user home directory.
    static const android::base::StringView kFileName;
