...
GlobalState gGlobals = { false, false, CPU_ACCELERATOR_NONE, { '\0' }, ANDROID_VM_ACCELERATION_ERROR };
...
VMAccelerationCapability ProbeKVM(String *status) {
    ...
    if (::access("/dev/kvm", F_OK)) {
        ...
        if (!cpu_ok) {
            ...
            return ANDROID_VM_ACCELERATION_NO_CPU_SUPPORT;
        }

        status->assign(
            "/dev/kvm is not found: VT disabled in BIOS or KVM kernel module not loaded");
        return ANDROID_VM_ACCELERATION_DEV_NOT_FOUND;
    }
    ...
    if (::access("/dev/kvm", R_OK)) {
        ...
        return ANDROID_VM_ACCELERATION_DEV_PERMISSION;
    }
    ...
    if (!fd.valid()) {
        ...
        return ANDROID_VM_ACCELERATION_DEV_OPEN_FAILED;
    }
    ...
    if (version < 0) {
        ...
        status->append(strerror(errno));
        return ANDROID_VM_ACCELERATION_DEV_IOCTL_FAILED;
    }
    ...
    if (version < KVM_API_VERSION) {
        StringAppendFormat(status,
                           ...
                           version,
                           KVM_API_VERSION);
        return ANDROID_VM_ACCELERATION_DEV_OBSOLETE;
    }
    ...
    StringAppendFormat(status,
                       ...
                       version);
    return ANDROID_VM_ACCELERATION_READY;
}
...
#define HAXM_INSTALLER_VERSION_MINIMUM 0x01020001
...
/*
 * ProbeHaxCpu: returns ANDROID_VM_ACCELERATION_READY if the CPU supports
 * HAXM requirements.
 *
 * Otherwise returns some other VMAccelerationCapability status and sets
 * |status| to a user-understandable error string
 */
VMAccelerationCapability ProbeHaxCpu(String* status) {
    char vendor_id[16];
    ...
    if (android_get_x86_cpuid_vendor_id_is_vmhost(vendor_id)) {
        ...
        StringAppendFormat(status,
                           ...
                           "Your CPU: '%s'",
                           vendor_id);
        return ANDROID_VM_ACCELERATION_NESTED_NOT_SUPPORTED;
    }
    ...
    if (!android_get_x86_cpuid_vmx_support()) {
        if (android_get_x86_cpuid_is_vcpu()) {
            ...
            return ANDROID_VM_ACCELERATION_HYPERV_ENABLED;
#else // OSX
            StringAppendFormat(status,
                               "Android Emulator does not support nested virtualization.  "
                               "Your CPU: '%s'",
        ...
        return ANDROID_VM_ACCELERATION_NO_CPU_VTX_SUPPORT;
    }
    ...
    if (!android_get_x86_cpuid_nx_support()) {
        status->assign(
            "Android Emulator requires an Intel processor with VT-x and NX support.  "
            "(NX is not supported)");
        return ANDROID_VM_ACCELERATION_NO_CPU_NX_SUPPORT;
    }
...
VMAccelerationCapability ProbeHAX(String* status) {
    status->clear();

    VMAccelerationCapability cpu = ProbeHaxCpu(status);
    if (cpu != ANDROID_VM_ACCELERATION_READY)
        return cpu;
    ...
    if (haxm_installer_version == 0) {
        ...
        return ANDROID_VM_ACCELERATION_ACCEL_NOT_INSTALLED;
    }

    if (haxm_installer_version < HAXM_INSTALLER_VERSION_MINIMUM) {
        status->assign(
            "HAXM must be updated");
        return ANDROID_VM_ACCELERATION_ACCEL_OBSOLETE;
    }
    ...
    if ( (cap.wstatus & HAX_CAP_WORKSTATUS_MASK) == HAX_CAP_STATUS_NOTWORKING )
    {
        if (cap.winfo & HAX_CAP_FAILREASON_VT) {
            ...
            return ANDROID_VM_ACCELERATION_VT_DISABLED;
        }
        else if (cap.winfo & HAX_CAP_FAILREASON_NX) {
            status->assign("NX feature disabled in BIOS/UEFI");
            return ANDROID_VM_ACCELERATION_NX_DISABLED;
        }
    }

    // 4) Profit!
    StringAppendFormat(status,
                       "HAX (version 0x%08x %d) is installed and usable.",
...
VMAccelerationCapability ProbeHAX(String* status) {
    ...
    int32_t version = CpuAccelerator_Darwin_GetHaxVersion(
        kext_dir,
        kext_dir_count,
        version_file
        );
...
int32_t CpuAccelerator_Darwin_ParseVersionScript(const std::string& version_script) {
    int32_t result = 0;
    std::string version_equals("VERSION=");
    size_t offset = version_script.find(version_equals);
    if (offset == std::string::npos) {
        return -1;
    }
    std::string version_string = version_script.substr(offset+version_equals.length());
    const int VALUE_COUNT_MAX = 3; // support 3 numbers max major.minor.rev
    const int bit_offset[VALUE_COUNT_MAX] = { 24, 16, 0 };
    const int value_max[VALUE_COUNT_MAX] = { 127, 255, 65535 };
    for (int i = 0; i < VALUE_COUNT_MAX; i++) {
        const char* end = version_string.c_str();
        unsigned long value = strtoul(version_string.c_str(), (char**)&end, 10);
        if (version_string.c_str() == end) {
...
int32_t CpuAccelerator_Darwin_GetHaxVersion(
    ...
    for (size_t i = 0; i < kext_dir_count; i++) {
        ...
        std::string version_file_abs = std::string(kext_dir[i]) + "/" + version_file;
        FileData fd;
        ...
        int32_t result = CpuAccelerator_Darwin_ParseVersionScript(version_script);
        if (result < -1) {
            // let's try the next directory, just in case
            continue;
        }
...
CpuAccelerator GetCurrentCpuAccelerator() {
    ...
    String status;
#if HAVE_KVM
    VMAccelerationCapability status_code = ProbeKVM(&status);
    if (status_code == ANDROID_VM_ACCELERATION_READY) {
        g->accel = CPU_ACCELERATOR_KVM;
    }
#elif HAVE_HAX
    VMAccelerationCapability status_code = ProbeHAX(&status);
    if (status_code == ANDROID_VM_ACCELERATION_READY) {
