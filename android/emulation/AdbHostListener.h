...
public:
    ...
    AdbHostListener(int adbClientPort = 0) : mAdbClientPort(adbClientPort) {}

    // AdbHostAgent method overrides.
    virtual bool init(int adbServerPort) override;
    virtual void deinit() override;
    virtual void addGuest(AdbGuestAgent* guest) override;
    virtual void deleteGuest(AdbGuestAgent* guest) override;

    // Return port this host server is bound to, or -1 if the server
    // is not initialized.
    int port() const { return mServer.get() ? mServer->port() : -1; }

    // Try to send a message to the ADB host server listening for
    // client connections on TCP localhost port |adbClientPort| that
    // an emulator instance is listening for connections on TCP localhost
    // port |adbPort|. Return true on success, false/errno on error.
    static bool notifyAdbServer(int adbClientPort, int adbPort);

    // The default ADB Client port, to be passed to init().
    static constexpr int kDefaultAdbClientPort = 5037;

private:
    static constexpr size_t kMaxGuests = 4;
