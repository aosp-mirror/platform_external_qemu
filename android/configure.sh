...
if [ "$OPTION_MINGW" = "yes" ] ; then
    ...
    TEST_SHELL=$(which $TEST_SHELL 2>/dev/null || true)
    if [ -z "$WINE_CMD" ]; then
        echo "WARNING: Wine is not installed on this machine!! Unit tests will be ignored and will not run!!"
        TEST_SHELL="true ||"
    fi
...
ecoh "TEST_SHELL            := $TEST_SHELL" >> $config_mk
