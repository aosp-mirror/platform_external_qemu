...
for opt do
  ...
  ;;
  --out-intermediates-dir=*) OPTION_OUT_INTERMEDIATES_DIR=$optarg
  ;;
...
if [ "$OPTION_OUT_INTERMEDIATES_DIR" ]; then
    OUT_INTERMEDIATES_DIR="$OPTION_OUT_INTERMEDIATES_DIR"
    mkdir -p "$OUT_INTERMEDIATES_DIR" || panic "Could not create output directory: $OUT_INTERMEDIATES_DIR"
else
    OUT_INTERMEDIATES_DIR=$OUT_DIR
    log "Auto-config: --out-intermediates-dir=$OUT_DIR"
fi
...
SDK_TOOLCHAIN_DIR=$OUT_INTERMEDIATES_DIR/build/toolchain
...
# and the intermediates dir
mkdir -p $OUT_INTERMEDIATES_DIR
...
if [ -n "${TOOLCHAIN_SYSROOT}" ]; then
    ...
    case $BUILD_OS in
      linux*)
        ...
        copy_toolchain_lib "${OUT_INTERMEDIATES_DIR}/intermediates64" "$(dirname ${LIBCPLUSPLUS})" libc++
...
config_mk=$OUT_INTERMEDIATES_DIR/build/config.make
...
echo "BUILD_INTERMEDIATES   := $OUT_INTERMEDIATES_DIR" >> $config_mk
...
#
config_h=$OUT_INTERMEDIATES_DIR/build/config-host.h
cat > $config_h <<EOF
...
AUTOGENERATED_DIR=$OUT_INTERMEDIATES_DIR/build/qemu1-qapi-auto-generated
