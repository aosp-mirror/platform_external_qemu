...
struct CameraCommand {
    ...
    intptr_t process() const {
        ...
        case CAMERA_CMD_OPEN:
            result = reinterpret_cast<intptr_t>(
                    cmd_camera_device_open(cmd.open.name,
                                           cmd.open.channel));
            break;

        case CAMERA_CMD_START_CAPTURING:
            result =
                cmd_camera_device_start_capturing(cmd.start.device,
                                                  cmd.start.pixel_format,
                                                  cmd.start.frame_width,
                                                  cmd.start.frame_height);
            break;

        case CAMERA_CMD_STOP_CAPTURING:
            result =
                cmd_camera_device_stop_capturing(cmd.stop.device);
            break;

        case CAMERA_CMD_READ_FRAME:
            result =
                cmd_camera_device_read_frame(cmd.read_frame.device,
                                             cmd.read_frame.framebuffers,
                                             cmd.read_frame.fbs_num,
                                             cmd.read_frame.r_scale,
                                             cmd.read_frame.g_scale,
                                             cmd.read_frame.b_scale,
        ...
        case CAMERA_CMD_ENUMERATE_DEVICES:
            result =
                cmd_enumerate_camera_devices(cmd.enumerate.info,
                                             cmd.enumerate.max);
            break;
        }
        return result;
...
public:
    // Constructor, called from the main thread, which also
    // owns the |channel| parameter.
...
static android::base::LazyInstance<CameraThread> sCameraThread = LAZY_INSTANCE_INIT;
