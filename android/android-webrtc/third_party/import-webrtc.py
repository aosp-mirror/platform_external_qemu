...
PLATFORMS = {
    "Darwin": ["apple", "posix", "mac", "_all", "intel_cpu"],
    "Darwin-aarch64": ["apple", "posix", "mac", "_all", "arm64"],
    "Linux-aarch64": ["linux", "posix", "linux_kernel", "arm64", "_all"],
    "Linux": ["linux", "posix", "linux_kernel", "_all"],
    "Windows": ["windows", "windows_x64", "intel_cpu", "_all"],
}
...
DEP_MAP = {
    ...
    "//third_party/libaom:aom_highbd": "webrtc_modules_video_coding_codecs_av1_libaom_av1_encoder",
...
class Target(object):
    def __init__(self, path, name, typ, srcs, hdrs, defs, deps, copts, includes, lopts):
        ...
        self.copts = [x.replace("$(WEBRTC_RTTI)", "") for x in copts]
        self.lopts = [
    ...
    def cmake_snippet(self):
        ...
        snippet += "target_include_directories({} PUBLIC {})\n".format(
            self.name, " ".join(self.includes)
        )

        if self.defs:
            snippet += "target_compile_definitions({} PUBLIC {})\n".format(
...
class BuildFileFunctions(object):
    ...
    def _add_target(self, typ, name, srcs, hdrs, defs, deps, copts, lopts, includes):
        # deps += ["{lib}::{lib}".format(lib=x[:-4]) for x in lopts if x.endswith(".lib")]
        self.targets.add(
...
def write_cmake_project(targets):
    cmake = "# This is an autogenerated file by calling {}".format(" ".join(sys.argv))
    cmake += """
...
def main():
    ...
    out.write(write_cmake_project(cmake))
