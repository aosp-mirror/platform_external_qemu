...
int32_t VideoShareCapture::Init(std::string handle) {
    mSharedMemory = SharedMemory(
        handle, getPixelCount(mSettings) + sizeof(VideoShareInfo::VideoInfo));
    int err = mSharedMemory.open(SharedMemory::AccessMode::READ_ONLY);
    if (err != 0) {
        RTC_LOG(LERROR)
        << "Unable to open memory mapped handle: [" << handle << "], due to:"
        << err;
        return -1;
    }
    mPixelBuffer =
        (uint8_t*) *mSharedMemory + sizeof(VideoShareInfo::VideoInfo);
