...
#include "emulator_controller.pb.h"  // for AudioPacket, AudioFormat, AudioF...
...
public:
    ...
    grpc::Status GetMetadata(
        ...
        metadata->insert(
                std::make_pair(android::emulation::control::BasicTokenAuth::DEFAULT_HEADER, mToken));
...
// TODO(jansene): We should use the emulator discovery mechanism to configure
// the gRPC endpoint.
GrpcAudioSource::GrpcAudioSource(std::string discovery_file) {
    if (initializeGrpcStub(discovery_file)) {
        ...
        audio_streaming_fiber_ = std::thread([this]() { StreamAudio(); });
    }
}
...
bool GrpcAudioSource::initializeGrpcStub(std::string discovery_file) {
    ...
    std::string grpc_address = "localhost:" + iniFile.getString("grpc.port", "8554");
    mEmulatorStub = android::emulation::control::EmulatorController::NewStub(
             grpc::CreateChannel(grpc_address,
                                 grpc::InsecureChannelCredentials()));

    if (iniFile.hasKey("grpc.token")) {
        auto token = iniFile.getString("grpc.token", "");
        auto creds = grpc::MetadataCredentialsFromPlugin( std::make_unique<BasicTokenAuthenticator>(token));
