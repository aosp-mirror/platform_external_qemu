...
void GrpcVideoSource::captureFrames() {
    ...
    request.set_allocated_transport(&transport);
    ...
    while (stream->Read(&img)) {
        auto converted = libyuv::ConvertToI420(
                (uint8_t*)shm.get(), shm.size(),
                mI420Buffer.get()->MutableDataY(), mI420Buffer.get()->StrideY(),
                mI420Buffer.get()->MutableDataU(), mI420Buffer.get()->StrideU(),
                mI420Buffer.get()->MutableDataV(), mI420Buffer.get()->StrideV(),
                /*crop_x=*/0,
    ...
    RTC_LOG(WARNING) << "Completed video stream.";
    tempfile_close(tmp);
}
