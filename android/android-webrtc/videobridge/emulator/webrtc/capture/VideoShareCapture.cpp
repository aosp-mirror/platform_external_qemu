...
bool VideoShareCapture::CaptureProcess() {
    ...
    if (mCaptureStarted && mVideoInfo->frameNumber != mPrevframeNumber) {
        ...
        // We need to swap R<-->B channel as the frames we are getting are:
        // libyuv::FOURCC_ABGR, which the webrtc codec is not fond of.
        colorConvert((uint32_t*)mPixelBuffer,
                     (uint32_t*)mConversionBuffer.data(),
                     mConversionBuffer.size() / 4);
        IncomingFrame(mConversionBuffer.data(), mPixelBufferSize, mSettings);
...
int32_t VideoShareCapture::Init(std::string handle) {
    ...
    mConversionBuffer.resize(mPixelBufferSize);
    return 0;
};
