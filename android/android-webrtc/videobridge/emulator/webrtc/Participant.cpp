...
bool Participant::AddVideoTrack(VideoCapturer* capturer) {
    auto handle = capturer->name();
...
bool Participant::RemoveVideoTrack(VideoCapturer* capturer) {
    auto handle = capturer->name();
    if (mActiveVideoTracks.count(handle) == 0) {
        RTC_LOG(LS_ERROR) << "Track " << handle
    ...
    auto track = mActiveVideoTracks[capturer->name()];
