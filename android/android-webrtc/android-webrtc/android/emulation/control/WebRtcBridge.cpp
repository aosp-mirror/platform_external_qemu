...
static std::string generateUniqueVideoHandle() {
    ...
    char handle[32];
    uint32_t* uniqueId = (uint32_t*) Uuid::generate().bytes();
    snprintf(handle, sizeof(handle),"emuvid%X%X%X%X", uniqueId, uniqueId[1], uniqueId[2], uniqueId[3]);
    return std::string(handle, 32);
}
...
bool WebRtcBridge::start() {
    ...
    // TODO(jansen): We should pause the recorder when no connections are active.
