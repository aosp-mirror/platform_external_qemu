...
static bool operator<(const tm& tm1, const tm& tm2) {
    return std::tie(tm1.tm_year, tm1.tm_mon, tm1.tm_mday, tm1.tm_hour,
                    tm1.tm_min, tm1.tm_sec) < std::tie(tm2.tm_year, tm2.tm_mon,
                                                       tm2.tm_mday, tm2.tm_hour,
...
public:
    ...
    // the implementation of c function tm* localtime(time_t* timer).  Because
    ...
    // at the specific moment of |timer| . The timezone offset includes day
    // light saving.
    long androidTimeZoneOffset(const time_t& timer);

    // Return true only if android timezone is successfully set to tzname
    // This operation will affect the emulated network time in virtual modem.
    // When |tzname| is not
...
private:
    ...
    static bool parseTimeZoneInformationFromRegistry(
            const char* winName,
    ...
    static bool parseZdumpDate(const std::vector<std::string>& tokens,
                               tm* date);
...
bool TimeZone::setAndroidTimeZoneUsingZdump() {
    ...
    std::vector<std::string> shellCmd = {"/bin/bash", "-c",
                                         std::move(zdumpCmd)};
        ...
        if (rules.size() == 4) {
            ...
            for (int i = 1; i < 4; i += 2) {
                ...
                std::vector<std::string> utcTokens(tokens.begin() + 2,
                                                   tokens.begin() + 6);
                if (!parseZdumpDate(utcTokens, i == 1 ? &mStandardDateUtc
                                                      : &mDaylightDateUtc)) {
                    return false;
                }
                tm localDate;
                ...
                if (i == 1) {
                    ...
                    utcOffsetStandard =
                            this->diffTime(localDate, mDaylightDateUtc);
                }
            }
...
bool TimeZone::setAndroidTimeZoneUsingDate() {
    std::string dateCmd =
            StringFormat("TZ=%s date +%%z", mTimeZoneName.c_str());
...
const char* TimeZone::androidTimeZoneName() {
    if (!this->mAndroidTimeZoneInit) {
        ...
        if (tz.empty()) {
            ...
            if (len < 0) {
                dprint("### WARNING: Could not read %s, something is very "
            ...
            if (memcmp(buff, ZONEINFO_DIR, sizeof(ZONEINFO_DIR) - 1)) {
                dprint("### WARNING: %s does not point to %s, can't determine "
                       "zoneinfo timezone name",
                       LOCALTIME_FILE, ZONEINFO_DIR);
                return NULL;
            }
            tz = buff + sizeof(ZONEINFO_DIR) - 1;
            if (!checkTimeZoneIsZoneInfo(tz)) {
                dprint("### WARNING: %s does not point to zoneinfo-compatible "
                       "timezone name\n",
                       LOCALTIME_FILE);
                return NULL;
            }
...
const char* TimeZone::androidTimeZoneName(void) {
    if (!mAndroidTimeZoneInit) {
        ...
        if (tz != NULL && !checkTimeZoneIsZoneInfo(tz)) {
            D("%s: ignoring non zoneinfo formatted TZ environment variable: "
              "'%s'\n",
              __FUNCTION__, tz);
            tz = NULL;
        }
        ...
        if (tz == NULL) {
            ...
            /* determine the correct timezone directory */
            {
                ...
                if (access(env, R_OK) != 0) {
                    if (env == zoneinfo_dir) {
                        fprintf(stderr,
                                "### WARNING: could not find %s directory. "
                                "unable to determine host timezone\n",
                                env);
                    } else {
                        D("%s: TZDIR does not point to valid directory, using "
                          "%s instead\n",
            ...
            if (access(localtimePtr, R_OK) != 0) {
                ...
                if (p >= end || access(temp, R_OK) != 0) {
                    fprintf(stderr,
                            "### WARNING: could not find %s or %s. unable to "
            ...
            if (len >= 0 && len > static_cast<int>(tzdir.size()) + 2) {
                ...
                // verify that the link points to tzdir/<something> where
                // <something> is a valid zoneinfo name
                if (!memcmp(temp, tzdir.c_str(), tzdir.size()) &&
                    ...
                    D("%s: %s link points to non-zoneinfo filename %s, "
                      "comparing contents\n",
            ...
            // otherwise, parse all files under tzdir and see if we have
            // something that looks like it
            {
                ...
                if (stat(localtime.c_str(), &scan->localtime_st) < 0) {
                    fprintf(stderr,
                            "### WARNING: can't access '%s', unable to "
                            "determine host timezone\n",
                            localtime.c_str());
                    goto Exit;
                }
                ...
                scan->path_root = bufprint(scan->path, scan->path_end, "%s",
                                           tzdir.c_str());
...
char* bufprint_zoneinfo_timezone(char* p, char* end) {
    ...
    if (tz == NULL || !sAndroidTimeZone->checkTimeZoneIsZoneInfo(tz)) {
        // If |tz| is invalid, print empty string. In this case, the system
        // image will determine
