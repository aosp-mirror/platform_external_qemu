...
int TimeZone::utccmp(const struct tm& utc1, const struct tm& utc2)
{
...
int TimeZone::getIsdst(const time_t& timeNow)
{
    if (mDaylightOffset == 0) {
        return -1;
    } else {
        struct tm utc = *gmtime(&timeNow);
        if (utccmp(utc, mStandardDateUtc) < 0 || utccmp(utc, mDaylightDateUtc) >= 0) {
            return 0;
        } else {
            return 1;
        }
