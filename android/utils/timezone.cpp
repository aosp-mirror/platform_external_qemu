...
// Return true if tm1 is earlier than
static bool operator<(const tm& tm1, const tm& tm2){
    return std::tie(tm1.tm_year, tm1.tm_mon, tm1.tm_mday, tm1.tm_hour, tm1.tm_min, tm1.tm_sec) <
            std::tie(tm2.tm_year, tm2.tm_mon, tm2.tm_mday, tm2.tm_hour, tm2.tm_min, tm2.tm_sec);
}
...
public:
    ...
    // the implementation of c function tm* localtime(time_t* timer).  Because the c implementation
    // keeps one global variable for struct tm. The caller must de-reference the returned pointer
    ...
    // at the specific moment of |timer| . The timezone offset includes day light saving.
    long androidTimeZoneOffset(const time_t& timer);

    // Return true only if android timezone is successfully set to tzname
    // This operation will affect the emulated network time in virtual modem. When |tzname| is not
    // found or invalid, the AVD's timezone will be the same as host OS.
    bool androidTimeZoneSet(const char* tzname);

    // Return the |tzname| value passed from cmdline if timezone option is enabled.
    // Otherwise, return NULL
    const char* androidTimeZoneName();

    // Return true if |tz| is of the format Area/Location or Area/Location/SubLocation
...
private:
    ...
    //  Return 1 if current timezone with respect to the specific moment at |timer| is in daylight saving,
    ...
    // Return the tm difference in seconds assuming the diff is within 24 hours and
    ...
    // Return true only if |tzi| is successfully retrieved from registry given |winName|.
    // This function will save the retrieved information in |tzi|. This function does NOT work on WINE.
    static bool  parseTimeZoneInformationFromRegistry(const char* winName, TIME_ZONE_INFORMATION* tzi);
#else // !_WIN32
    ...
    // Return true only if the timezone information in |mCurrentYear| is found using zdump cpmmand.
...
bool TimeZone::parseTimeZoneInformationFromRegistry(const char* winName, TIME_ZONE_INFORMATION* tzi)
{
...
bool TimeZone::parseZdumpDate(const std::vector<std::string>& tokens, tm* date)
{
    if (tokens.size() >= 4) {
        int monIdx = -1;
        for(int i = 0; i < 12; i++) {
...
bool
TimeZone::checkTimeZoneIsZoneInfo(StringView tz)
{
...
bool
TimeZone::compareTimeZoneToLocalTime( ScanDataRec*  scan,
                               const char*   path )
{
    struct  stat  st;
    int           fd1, fd2;
...
const char*
TimeZone::scanTimeDir( ScanDataRec*  scan,
                   char*         top,
                   int           depth )
{
...
const char*
TimeZone::androidTimeZoneName( void )
{
...
{
    if (!sAndroidTimeZone->checkTimeZoneIsZoneInfo(tzname)){ return -1; }
...
bufprint_zoneinfo_timezone( char*  p, char*  end )
{
    ...
    if (tz == NULL || !sAndroidTimeZone->checkTimeZoneIsZoneInfo(tz)) {
        // If |tz| is invalid, print empty string. In this case, the system image will determine
