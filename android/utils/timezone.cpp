...
#define D(...)                                  \
  do {                                          \
    if (D_ACTIVE) fprintf(stderr, __VA_ARGS__); \
  } while (0)
#else
#define D(...) ((void)0)
#endif

// Return true if tm1 is earlier than tm2
static bool operator<(const tm& tm1, const tm& tm2) {
  return std::tie(tm1.tm_year, tm1.tm_mon, tm1.tm_mday, tm1.tm_hour, tm1.tm_min,
                  tm1.tm_sec) < std::tie(tm2.tm_year, tm2.tm_mon, tm2.tm_mday,
...
static const Win32Timezone kWin32Timezones[] = {
...
class TimeZone {
 public:
  ...
  // Return local time tm* converted from absolute time value |timer| based on
  // the implementation of c function tm* localtime(time_t* timer).  Because the
  // c implementation
  // keeps one global variable for struct tm. The caller must de-reference the
  // returned pointer
  // before using its value.
  tm* androidLocaltime(const time_t& timer);

  // Return the timezone offset in seconds with respect to UTC in AVD
  // at the specific moment of |timer| . The timezone offset includes day light
  // saving.
  long androidTimeZoneOffset(const time_t& timer);

  // Return true only if android timezone is successfully set to tzname
  // This operation will affect the emulated network time in virtual modem. When
  // |tzname| is not
  // found or invalid, the AVD's timezone will be the same as host OS.
  bool androidTimeZoneSet(const char* tzname);

  // Return the |tzname| value passed from cmdline if timezone option is
  // enabled.
  // Otherwise, return NULL
  const char* androidTimeZoneName();

  // Return true if |tz| is of the format Area/Location or
  // Area/Location/SubLocation
  static bool checkTimeZoneIsZoneInfo(StringView tz);

 private:
  tm mStandardDateUtc = {};
  tm mDaylightDateUtc = {};
  long mStandardOffset = 0;
  long mDaylightOffset = 0;
  int mCurrentYear = 0;
  int mAndroidTimeZoneInit = false;
  std::string mTimeZoneName;
  static const int kSecondsPerDay = 60 * 60 * 24;
  long getTimeZoneDiff(const time_t& timer);

  //  Return 1 if current timezone with respect to the specific moment at
  //  |timer| is in daylight saving,
  //  0 if current timezone is NOT in daylight saving.
  //  and -1 if current timezone does NOT have daylight saving

  int getIsDaylightSavingTime(const time_t& timer);

  // Return the tm difference in seconds assuming the diff is within 24 hours
  // and
  // |end| and |beginning| represent utc time.

  static long diffTime(const tm& end, const tm& beginning);
#ifdef _WIN32
  static void parseSystemTime(const SYSTEMTIME& winTime, tm* androidTime);

  // Return true only if |tzi| is successfully retrieved from registry given
  // |winName|.
  // This function will save the retrieved information in |tzi|. This function
  // does NOT work on WINE.
  static bool parseTimeZoneInformationFromRegistry(const char* winName,
                                                   TIME_ZONE_INFORMATION* tzi);
#else // !_WIN32

  // Return true is |tokens| is successfully parsed and saved into |date|
  // Sample |tokens|: ["Mar", "13", "07:00:00", "2016"]
  static bool parseZdumpDate(const std::vector<std::string>& tokens, tm* date);

  // Return true only if the timezone information in |mCurrentYear| is found
  // using zdump cpmmand.
  // Sample shell command to be run: zdump -v America/New_York | grep 2016
  // Sample output from the shell commnd:
  // America/New_York  Sun Mar 13 06:59:59 2016 UT = Sun Mar 13 01:59:59 2016
  // EST isdst=0 [gmtoff=-18000]
  // America/New_York  Sun Mar 13 07:00:00 2016 UT = Sun Mar 13 03:00:00 2016
  // EDT isdst=1 [gmtoff=-14400]
  // America/New_York  Sun Nov  6 05:59:59 2016 UT = Sun Nov  6 01:59:59 2016
  // EDT isdst=1 [gmtoff=-14400]
  // America/New_York  Sun Nov  6 06:00:00 2016 UT = Sun Nov  6 01:00:00 2016
  // EST isdst=0 [gmtoff=-18000]
  bool setAndroidTimeZoneUsingZdump();

  // Return true if the  timezone information is found using date command
  // Sample shell command: TZ=America/New_York date +%:z
  // Sample output from shell command: -0400
  bool setAndroidTimeZoneUsingDate();
#endif

#if defined(__linux__) || defined(__FreeBSD__)
  static bool compareTimeZoneToLocalTime(ScanDataRec* scan, const char* path);
  static const char* scanTimeDir(ScanDataRec* scan, char* top, int depth);
...
tm* TimeZone::androidLocaltime(const time_t& timer) {
  if (this->mAndroidTimeZoneInit) {
    // reset android timezone if year has changed
    int year = gmtime(&timer)->tm_year + 1900;
    if (year != mCurrentYear) {
    ...
    int isdst = getIsDaylightSavingTime(timer);
    long tzdiff = getTimeZoneDiff(timer);
    time_t localTime = timer + tzdiff;
    tm* local = gmtime(&localTime);
...
int TimeZone::getIsDaylightSavingTime(const time_t& timer) {
  if (mDaylightOffset == 0) {
    return -1;
  }
  tm utc = *gmtime(&timer);
  if (utc < mStandardDateUtc || !(utc < mDaylightDateUtc)) {
    return 0;
  } else {
    return 1;
  }
...
long TimeZone::getTimeZoneDiff(const time_t& timer) {
  int isdst = getIsDaylightSavingTime(timer);
  return (isdst <= 0) ? mStandardOffset : mStandardOffset + mDaylightOffset;
}

long TimeZone::diffTime(const tm& end, const tm& beginning) {
  long endInSecs = end.tm_sec + 60 * (end.tm_min + 60 * end.tm_hour);
  long beginningInSecs =
      beginning.tm_sec + 60 * (beginning.tm_min + 60 * beginning.tm_hour);
...
bool TimeZone::parseTimeZoneInformationFromRegistry(
    const char* winName, TIME_ZONE_INFORMATION* tzi) {
  using android::base::ScopedRegKey;
  using android::base::Win32Utils;
  HKEY hkey = 0;
  std::string registryPath = StringFormat(
      "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Time Zones\\%s",
      winName);

  LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, registryPath.c_str(), 0,
                             KEY_READ, &hkey);
  ...
  result = RegQueryValueEx(timezoneKey.get(), "TZI", NULL, &dataType,
                           (LPBYTE)&binaryTzi, &len);
...
void TimeZone::parseSystemTime(const SYSTEMTIME& winTime, tm* androidTime) {
...
bool TimeZone::parseZdumpDate(const std::vector<std::string>& tokens,
                              tm* date) {
  if (tokens.size() >= 4) {
    int monIdx = -1;
    for (int i = 0; i < 12; i++) {
...
bool TimeZone::setAndroidTimeZoneUsingZdump() {
  std::string zdumpCmd =
      StringFormat("zdump -v %s | grep %d", mTimeZoneName, mCurrentYear);
  ...
  bool commandRan = System::get()->runCommand(shellCmd, runFlags, timeout,
                                              &exitCode, nullptr, outputFile);
  ...
  if (commandRan && !exitCode) {
    ...
    if (rules.size() == 4) {
      ...
      for (int i = 1; i < 4; i += 2) {
        ...
        std::vector<std::string> utcTokens(tokens.begin() + 2,
                                           tokens.begin() + 6);
        if (!parseZdumpDate(utcTokens,
                            i == 1 ? &mStandardDateUtc : &mDaylightDateUtc)) {
          return false;
        }
        tm localDate;
        std::vector<std::string> localTokens(tokens.begin() + 9,
                                             tokens.begin() + 13);
...
bool TimeZone::setAndroidTimeZoneUsingDate() {
  ...
  if (commandRan && !exitCode) {
    ...
    mStandardOffset = sign * (60 * (std::stoi(tzdiff.substr(1, 2)) * 60 +
                                    std::stoi(tzdiff.substr(3, 2))));
    mDaylightOffset = 0;
    return true;
  } else {
    return false;
...
bool TimeZone::androidTimeZoneSet(const char* tzname) {
  mTimeZoneName = std::string(tzname);
  this->mAndroidTimeZoneInit = 0;
#ifdef _WIN32
  const char* winName = NULL;
  for (const auto& win32tz : kWin32Timezones) {
  ...
  if (winName) {
    ...
    if (!parseTimeZoneInformationFromRegistry(winName, &win_tzi)) {
      D("%s: could not retrieve time zone information from registry on "
        "Windows, use host localtime by default.\n",
        __FUNCTION__);
    } else {
      // UTC = localtime + Bias
...
#endif
  return this->mAndroidTimeZoneInit ? true : false;
}

bool TimeZone::checkTimeZoneIsZoneInfo(StringView tz) {
  const int slashCount = std::count(tz.begin(), tz.end(), '/');
...
const char* TimeZone::androidTimeZoneName() {
  if (!this->mAndroidTimeZoneInit) {
    System* system = System::get();
    std::string tz = system->envGet("TZ");
    ...
    if (tz.empty()) {
      ...
      if (len < 0) {
        dprint(
            "### WARNING: Could not read %s, something is very wrong on your "
            "system",
            LOCALTIME_FILE);
        return NULL;
      }
      ...
      if (memcmp(buff, ZONEINFO_DIR, sizeof(ZONEINFO_DIR) - 1)) {
        dprint(
            "### WARNING: %s does not point to %s, can't determine zoneinfo "
            "timezone name",
            LOCALTIME_FILE, ZONEINFO_DIR);
        return NULL;
      }
      tz = buff + sizeof(ZONEINFO_DIR) - 1;
      if (!checkTimeZoneIsZoneInfo(tz)) {
        dprint(
            "### WARNING: %s does not point to zoneinfo-compatible timezone "
            "name\n",
            LOCALTIME_FILE);
        return NULL;
      }
  ...
  D("found timezone %s", mTimeZoneName.c_str());
  return mTimeZoneName.c_str();
}

#endif /* __APPLE__ */
// on Linux, with glibc2, the zoneinfo directory can be changed with TZDIR
// environment variable
// but should be /usr/share/zoneinfo by default. /etc/localtime is not
// guaranteed to exist on
// all platforms, so if it doesn't, try $TZDIR/localtime, then
// /usr/share/zoneinfo/locatime
// ugly, isn't it ?
//
// besides, modern Linux distribution don't make /etc/localtime a symlink but a
// straight copy of
// the original timezone file. the only way to know which zoneinfo name to
// retrieve is to compare
// it with all files in $TZDIR (at least those that match Area/Location or
// Area/Location/SubLocation

#if defined(__linux__) || defined(__FreeBSD__)

bool TimeZone::compareTimeZoneToLocalTime(ScanDataRec* scan, const char* path) {
  struct stat st;
  int fd1, fd2;
  bool result = false;
  ...
  if (st.st_size != scan->localtime_st.st_size) {
    D(" size mistmatch (%zd != %zd)\n", (size_t)st.st_size,
      (size_t)scan->localtime_st.st_size);
    return false;
  }
...
const char* TimeZone::scanTimeDir(ScanDataRec* scan, char* top, int depth) {
  ...
  if (d != NULL) {
    ...
    while ((ent = readdir(d)) != NULL) {
      ...
      if (lstat(scan->path, &ent_st) < 0) continue;

      if (S_ISDIR(ent_st.st_mode) && depth < 2) {
        // D( "%s: directory '%s'\n", __FUNCTION__, scan->path );
        result = scanTimeDir(scan, p, depth + 1);
        if (result != NULL) break;
      } else if (S_ISREG(ent_st.st_mode) && (depth >= 1 && depth <= 2)) {
        char* name = scan->path_root + 1;

        if (checkTimeZoneIsZoneInfo(name)) {
          if (compareTimeZoneToLocalTime(scan, scan->path)) {
            result = strdup(name);
            D("%s: found '%s'\n", __FUNCTION__, result);
            break;
          }
...
const char* TimeZone::androidTimeZoneName(void) {
  if (!mAndroidTimeZoneInit) {
    System* system = System::get();
    const char* tz = system->envGet("TZ").c_str();

    // if we ever allocate into tz, this object will take care of it.
    ScopedCPtr<const char> tzDeleter;

    if (tz != NULL && !checkTimeZoneIsZoneInfo(tz)) {
    ...
    if (tz == NULL) {
      ...
      {
        const char* env = system->envGet("TZDIR").c_str();
        const char* zoneinfo_dir = ZONEINFO_DIR;

        if (env == NULL) env = zoneinfo_dir;

        if (access(env, R_OK) != 0) {
          if (env == zoneinfo_dir) {
            fprintf(stderr,
                    "### WARNING: could not find %s directory. unable to "
      ...
      if (access(localtimePtr, R_OK) != 0) {
        ...
        if (p >= end || access(temp, R_OK) != 0) {
          fprintf(stderr,
                  "### WARNING: could not find %s or %s. unable to determine "
                  "host timezone\n",
      ...
      if (len >= 0 && len > static_cast<int>(tzdir.size()) + 2) {
        ...
        // verify that the link points to tzdir/<something> where <something> is
        // a valid zoneinfo name
        if (!memcmp(temp, tzdir.c_str(), tzdir.size()) &&
            temp[tzdir.size()] == '/') {
          if (checkTimeZoneIsZoneInfo(temp + tzdir.size() + 1)) {
            /* we have it ! */
          ...
          D("%s: %s link points to non-zoneinfo filename %s, comparing "
            "contents\n",
      ...
      // otherwise, parse all files under tzdir and see if we have something
      // that looks like it
      {
        ScanDataRec scan[1];
        ...
        if (stat(localtime.c_str(), &scan->localtime_st) < 0) {
          fprintf(stderr,
                  "### WARNING: can't access '%s', unable to determine host "
                  "timezone\n",
                  localtime.c_str());
          goto Exit;
        }
        ...
        scan->path_root =
            bufprint(scan->path, scan->path_end, "%s", tzdir.c_str());

        tz = scanTimeDir(scan, scan->path_root, 0);
        tzDeleter.reset(tz);
      }
  ...
  if (!mAndroidTimeZoneInit) {
    ...
    for (const auto& win32tz : kWin32Timezones) {
      if (!strcmp(win32tz.win_name, tzname)) {
        mTimeZoneName = win32tz->zoneinfo_name;
        goto Exit;
      }
    }
...
{
  if (!sAndroidTimeZone->checkTimeZoneIsZoneInfo(tzname)) {
    return -1;
  }
  return sAndroidTimeZone->androidTimeZoneSet(tzname) ? 0 : -1;
}
...
tm* android_localtime(time_t* time_p) {
  time_t time = *time_p;
  return sAndroidTimeZone->androidLocaltime(time);
}

char* bufprint_zoneinfo_timezone(char* p, char* end) {
  const char* tz = sAndroidTimeZone->androidTimeZoneName();

  if (tz == NULL || !sAndroidTimeZone->checkTimeZoneIsZoneInfo(tz)) {
    // If |tz| is invalid, print empty string. In this case, the system image
    // will determine
    // the timezone based on tzoffset and dst instead of |tz|.
    return bufprint(p, end, "");
  } else {
