...
bool
iniFile_hasKey( CIniFile* i, const char* key )
{
    return asBaseIniFile(i)->hasKey(key);
}

int
iniFile_getPairCount( CIniFile*  i )
{
...
int
iniFile_saveToFile( CIniFile*  f, const char*  filepath )
{
...
int
iniFile_saveToFileClean( CIniFile*  f, const char*  filepath )
{
    asBaseIniFile(f)->setBackingFile(filepath);
...
int
iniFile_getEntry(CIniFile* f, int index, char** key, char** value)
{
    BaseIniFile* ini = asBaseIniFile(f);
    if (index >= ini->size()) {
        LOG(VERBOSE) << "Index " << index
                     << "exceeds the number of ini file intries "
                     << ini->size();
        return -1;
    }
    ...
    *key = strdup(cit->first.c_str());
    *value = strdup(cit->second.c_str());
    return 0;
}

char* iniFile_getString( CIniFile*  f, const char*  key, const char*
                         defaultValue ) {
...
double
iniFile_getDouble( CIniFile*  f, const char*  key, double  defaultValue )
{
    return asBaseIniFile(f)->getDouble(key, defaultValue);
}

int
iniFile_getBoolean( CIniFile*  f, const char*  key, const char*  defaultValue )
{
    return asBaseIniFile(f)->getBoolStr(key, defaultValue);
}

int64_t
iniFile_getDiskSize( CIniFile*  f, const char*  key, const char*  defaultValue )
{
    return asBaseIniFile(f)->getDiskSizeStr(key, defaultValue);
}

int64_t
iniFile_getInt64( CIniFile*  f, const char*  key, int64_t  defaultValue )
{
    return asBaseIniFile(f)->getInt64(key, defaultValue);
}

void
iniFile_setValue( CIniFile* f, const char* key, const char* value )
{
    asBaseIniFile(f)->setString(key, value);
}

void
iniFile_setInteger( CIniFile* f, const char* key, int value )
{
    asBaseIniFile(f)->setInt(key, value);
}

void
iniFile_setInt64( CIniFile* f, const char* key, int64_t value )
{
    asBaseIniFile(f)->setInt64(key, value);
}

void
iniFile_setDouble( CIniFile* f, const char* key, double value )
{
    asBaseIniFile(f)->setDouble(key, value);
}

void
iniFile_setBoolean( CIniFile* f, const char* key, int value )
{
    asBaseIniFile(f)->setBool(key, value);
}

void
iniFile_setDiskSize( CIniFile* f, const char* key, int64_t size )
{
