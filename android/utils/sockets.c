...
#ifdef __linux__
            if (src->sun_path[0] == '\0') {
                size_t abstract_len = fromlen - sun_path_offset + 1;
                abstract_path = malloc(abstract_len + 2);
                abstract_path[0] = '@';
                memcpy(&abstract_path[1], src->sun_path + 1, abstract_len);
                abstract_path[abstract_len + 1] = '\0';
            }
#endif  // __linux__
