...
public:
    virtual void SetUp() {
        ...
        ASSERT_TRUE(temp_dir_->path());
        stashed_avd_home_ = std::getenv("ANDROID_AVD_HOME");
        setenv("ANDROID_AVD_HOME", temp_dir_->path(), 1);
        std::cout << "tempdir: " << temp_dir_->path() << std::endl;
        std::cout << "ANDROID_AVD_HOME: " << std::getenv("ANDROID_AVD_HOME")
                  << std::endl;
    }
    ...
    void createValidFile(String filename) {
       //ASSERT_TRUE(temp_dir_->makeSubFile(filename.c_str()));
       ofstream ini_file;
       ini_file.open(temp_dir_->makeSubPath(filename.c_str()).c_str());
       ini_file << "tick = 3";
       ini_file.close();
    }
...
#define METRICS_DOUBLE(n, s, d)     ASSERT_EQ(lhs.n, rhs.n);

#include "android/utils/metrics/metrics_fields.h"
}

TEST_F(MetricsReporterTest, empty) {
}
...
TEST_F(MetricsReporterTest, simpleWriteRead) {
    ...
    androidMetrics_free(&metrics_in);
    androidMetrics_free(&metrics_out);
