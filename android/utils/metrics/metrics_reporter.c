...
void
androidMetrics_free( AndroidMetrics *androidMetrics )
{
#undef METRICS_INT
#undef METRICS_STRING
#undef METRICS_DOUBLE
...
androidMetrics_tick()
{
    ...
    ++metrics.tick;
    metrics.user_time = (double) get_user_time_ms() / 1000;
    metrics.system_time = (double) get_system_time_ms() / 1000;

    success = androidMetrics_write(&metrics);
    androidMetrics_free(&metrics);
    return success;
}
