...
bufprint_avd_home_path(char*  buff, char*  end)
{
    ...
    SHGetFolderPath( NULL, CSIDL_PROFILE,
                     NULL, 0, path);
    return bufprint(buff, end, "%s\\%s\\%s", path, _ANDROID_PATH, _ANDROID_AVD_DIR);
#else
    home = getenv("HOME");
    if (home == NULL)
        home = "/tmp";
    return bufprint(buff, end, "%s/%s/%s", home, _ANDROID_PATH, _ANDROID_AVD_DIR);
#endif
}
...
char*
bufprint_ini_file(char*  buff, char*  end, const char*  suffix)
{
    char*   p;
    p = bufprint_avd_home_path(buff, end);
