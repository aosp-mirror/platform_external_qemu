...
# This is an intermediate transition between android make and cmake.
# this basically declares everything under android-emu
# the tests, executables, shared libs etc.
#
# The cmake file will do dependency resolution for all the libraries
# all that is currently used are the cflags & toolchain.

$(call start-cmake-project,android-emu)
# Make sure we pass in all the proper flags to the compiler, not doing this
# will result in odd build failures and breakage.
LOCAL_CFLAGS:=  $(EMULATOR_COMMON_CFLAGS)

# The list of things we build an make available, other targets
...
# Note that we translate the names for now
...
# Third party libs that are generated here as well.
...
# The executables we are producing here.
PRODUCED_EXECUTABLES := emulator_img2simg=emulator$(BUILD_TARGET_SUFFIX)_img2simg \
                  ...
                  android-emu_benchmark=android_emu$(BUILD_TARGET_SUFFIX)_benchmark \
...
# We need to export a lot of variables since we are still building big chunks with gnumake
...
ifeq ($(BUILD_TARGET_OS),windows)
    ...
    # Make sure the memory modules are available.
