    ...
    while (line_end_pos) {
        ...
        if (equals_pos && equals_pos < line_end_pos) {
            ...
            if (wcsstr(key, L"AdapterCompatibility")) {
                ...
                strcpy(gpuinfos[curr_gpu]->make, val_as_bytes);
            } else if (wcsstr(key, L"Caption")) {
                fprintf(stderr, "(found model)\n");
                wcstombs(val_as_bytes, val, val_strlen); val_as_bytes[val_strlen] = '\0';
                strcpy(gpuinfos[curr_gpu]->model, val_as_bytes);
            } else if (wcsstr(key, L"DriverVersion")) {
                fprintf(stderr, "(found driver version)\n");
                wcstombs(val_as_bytes, val, val_strlen); val_as_bytes[val_strlen] = '\0';
                strcpy(gpuinfos[curr_gpu]->version, val_as_bytes);
                ...
                wprintf(L"curr dll=%ls\n", curr_dll);

                wcstombs(dll_as_bytes, curr_dll, curr_dll_len);
                dll_as_bytes[curr_dll_len] = '\0';

                gpuinfos[curr_gpu]->dlls[dll_index] = (char*)malloc(curr_dll_len + 1);
                strncpy(gpuinfos[curr_gpu]->dlls[dll_index],
                        dll_as_bytes, curr_dll_len + 1);
                dll_index++;
