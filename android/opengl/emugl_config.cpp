...
#ifdef _WIN32
static void gpuinfo_extract_val(wchar_t* val_str, int howmuch, wchar_t* out) {
}

static void gpuinfo_extract_val_bytes(wchar_t* val_str, char* out) {
...
static void gpuinfo_add_dll(GpuInfo** gpuinfos, int curr_gpu, wchar_t* dll_str) {
    gpuinfos[curr_gpu]->num_dlls++;
    int dll_index = gpuinfos[curr_gpu]->num_dlls - 1;
    int len = wcslen(dll_str);
    gpuinfos[curr_gpu]->dlls[dll_index] = (char*)malloc(len + 1);
    gpuinfo_extract_val_bytes(dll_str, gpuinfos[curr_gpu]->dlls[dll_index]);
}
...
#define MAX_GPUS 12
    ...
    // PathUtils::join and Uuid:: are just no good
    std::string sep("");
    if (tmp_dir.back() != '/') {
        sep += "/";
    }
    ...
    while (line_loc) {
        ...
        if (kvsep_loc && kvsep_loc < line_loc) {
            ...
            if (strstr(key, "Chipset Model")) {
                ...
                strncpy(gpuinfos[curr_gpu]->model, val, val_strlen + 1);
            } else if (strstr(key, "Vendor")) {
                fprintf(stderr, "(found make)\n");
                strncpy(gpuinfos[curr_gpu]->make, val, val_strlen + 1);
            } else if (strstr(key, "Device ID")) {
                fprintf(stderr, "(found deviceid)\n");
                strncpy(gpuinfos[curr_gpu]->device_id, val, val_strlen + 1);
            } else if (strstr(key, "Revision ID")) {
                strncpy(gpuinfos[curr_gpu]->revision_id, val, val_strlen + 1);
    ...
    auto exitCode = sys.runCommand({"wmic", temp_path_arg, "path", "Win32_VideoController", "get", "/value"}, RunOptions::WaitForCompletion);
    ...
    while (line_end_pos) {
        ...
        if (equals_pos && equals_pos < line_end_pos) {
            ...
            if (wcsstr(key, L"AdapterCompatibility")) {
                ...
                // fprintf(stderr, "(found make)\n");
                wcstombs(val_as_bytes, val, val_strlen); val_as_bytes[val_strlen] = '\0';
                strncpy(gpuinfos[curr_gpu]->make, val_as_bytes, val_strlen + 1);
            } else if (wcsstr(key, L"Caption")) {
                // fprintf(stderr, "(found model)\n");
                wcstombs(val_as_bytes, val, val_strlen); val_as_bytes[val_strlen] = '\0';
                strncpy(gpuinfos[curr_gpu]->model, val_as_bytes, val_strlen + 1);
            } else if (wcsstr(key, L"DriverVersion")) {
                // fprintf(stderr, "(found driver version)\n");
                wcstombs(val_as_bytes, val, val_strlen); val_as_bytes[val_strlen] = '\0';
                strncpy(gpuinfos[curr_gpu]->version, val_as_bytes, val_strlen + 1);
            } else if (wcsstr(key, L"InstalledDisplayDrivers")) {
                if (val_strlen == 0) {
                    continue;
                }
                // Look for the list of DLL's.
                // First, initialize our DLL list.
                ...
                char dll_as_bytes[FIELD_LEN] = {};

                int curr_dll_len = dll_end - dll_pos;
                wcsncpy(curr_dll, dll_pos, curr_dll_len + 1);
                curr_dll[curr_dll_len] = L'\0';

                gpuinfo_add_dll(gpuinfos, curr_gpu, curr_dll);
                ...
                // Add OpenGL_specific DLL's so we can find out which GPU is actualyl running at the moment
                
                if (strstr(gpuinfos[curr_gpu]->make, "NVIDIA")) {
                    gpuinfo_add_dll(gpuinfos, curr_gpu, L"nvoglv32.dll");
                    gpuinfo_add_dll(gpuinfos, curr_gpu, L"nvoglv64.dll");
                } else if (strstr(gpuinfos[curr_gpu]->make, "Advanced Micro Devices, Inc.")) {
                    fprintf(stderr, "should add ati-specific dlls\n");
                } else {
                    fprintf(stderr, "warning: unrecognized GPU make=%s\n", gpuinfos[curr_gpu]->make);
                }


            } else if (wcsstr(key, L"VideoModeDescription")) {
                if (val_strlen > 0) {
                    gpuinfos[curr_gpu]->current_gpu = true;
                }
            } else {
