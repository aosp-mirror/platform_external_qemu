...
#ifdef _WIN32
static void gpuinfo_extract_dllname_bytes(wchar_t* dllstr,
                                          int dll_strlen,
                                          char* out) {
    wcstombs(out, dllstr, dll_strlen);
    out[dll_strlen] = '\0';
}

static void gpuinfo_add_dll(GpuInfo** gpuinfos,
                            int curr_gpu,
                            int dll_index,
                            char* dll_str,
                            int dll_strlen) {

    gpuinfos[curr_gpu]->dlls[dll_index] = (char*)malloc(dll_strlen + 1);
    strncpy(gpuinfos[curr_gpu]->dlls[dll_index],
            dll_str, dll_strlen + 1);
    ...
    while (line_end_pos) {
        ...
        if (equals_pos && equals_pos < line_end_pos) {
            ...
            if (wcsstr(key, L"AdapterCompatibility")) {
                ...
                gpuinfo_extract_dllname_bytes(curr_dll,
                                              curr_dll_len,
                                              dll_as_bytes);
                gpuinfo_add_dll(gpuinfos, curr_gpu, dll_index,
                                dll_as_bytes, curr_dll_len);
                dll_index++;
                ...
                if (strstr(gpuinfos[curr_gpu]->make, "NVIDIA")) {
                    fprintf(stderr, "should add nvidia-specific dlls\n");
