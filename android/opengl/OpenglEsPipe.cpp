...
using android::base::ScopedVmLock;
...
//
// - if the looper is null, PipeWaker runs the pipe operation in the same thread
//   where you called PipeWaker::wake/close, but holds the Qemu global lock for
...
private:
    ...
    base::Lock mWakeMapLock;
    std::unordered_map<void*, int> mWakeMap;  // zero flags mean 'close'
