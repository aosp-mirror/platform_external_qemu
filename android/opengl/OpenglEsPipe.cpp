...
public:
    ...
    public:
        Service() : AndroidPipe::Service("opengles") {
            GLPipeControl::get()->registerRemoveCallback(android_close_opengles_pipe);
        }
    ...
    virtual void onGuestClose() override {
        ...
        GLPipeControl::get()->removePipe((void*)this);
    ...
    void close() {
        ...
        if (mHwPipe) {
            ...
            // Also stop the channel explicitly.
            mChannel->stop();
        }
...
void stopRenderer() {
    if (gRenderer.get()) {
        fprintf(stderr, "%s: stop %p\n", __FUNCTION__, gRenderer.get());
        gRenderer->stop();
    }
}
