...
void GpuInfo::setMake(INPUT_CHAR_TYPE* val_str, int len) {
    fprintf(stderr, "%s: valstr=%s\n", __FUNCTION__, val_str);
...
char* load_gpu_info() {
    ...
    if (tmpfd == -1) {
    fprintf(stderr, "%s:can't make temp file\n", __FUNCTION__);
        return NULL;
    }

    const char* temp_file_path = temp_file_path_template.c_str();
    fprintf(stderr, "%s:path =%s\n", __FUNCTION__, temp_file_path);
    ...
    while (line_end_pos) {
        ...
        if (equals_pos && equals_pos < line_end_pos) {
            ...
            if (wcsstr(key, L"AdapterCompatibility")) {
                ...
                gpulist->currGpu().addDll(gpulist, dll_pos, curr_dll_len);
                ...
                if (strstr(curr_make, "NVIDIA")) {
                    gpulist->currGpu().addConstDll(gpulist, L"nvoglv32.dll");
                    gpulist->currGpu().addConstDll(gpulist, L"nvoglv64.dll");
                } else if (strstr(curr_make, "Advanced Micro Devices, Inc.")) {
                    gpulist->currGpu().addConstDll(gpulist, L"atioglxx.dll");
                    gpulist->currGpu().addConstDll(gpulist, L"atig6txx.dll");
                }
            }
        }
