...
bool gpuinfo_query_blacklist(GpuInfoList* gpulist) {
    ...
    for (auto gpuinfo : gpulist->infos) {
        ...
        for (auto bl_entry : sGpuBlacklist) {
            bl_make = bl_entry.make;
            bl_model = bl_entry.model;
            bl_device_id = bl_entry.device_id;
            bl_revision_id = bl_entry.revision_id;
            bl_version = bl_entry.version;
            bl_renderer = bl_entry.renderer;

            if (bl_make && (gpuinfo.make != bl_make)) continue;
            if (bl_model && (gpuinfo.model != bl_model)) continue;
            if (bl_device_id && (gpuinfo.device_id != bl_device_id)) continue;
            if (bl_revision_id && (gpuinfo.revision_id != bl_revision_id)) continue;
            if (bl_version && (gpuinfo.revision_id != bl_version)) continue;
            if (bl_renderer && (gpuinfo.renderer != bl_renderer)) continue;
...
std::string parse_last_hexbrackets(const std::string& str) {
    ...
    return str.substr(openbrace_p, closebrace_p + 1 - openbrace_p);
}
...
std::string parse_renderer_part(const std::string& str) {
    ...
    return str.substr(prevspace_p, lastspace_p + 1 - prevspace_p);
}
...
void parse_gpu_info_list_linux(const std::string& contents,
    ...
    while (line_loc != NOTFOUND) {
        ...
        if (!lookfor && (key.find("VGA") != NOTFOUND)) {
            ...
            gpulist->currGpu().make =
                parse_last_hexbrackets(key);
        } else if (lookfor && (key.find("Device") != NOTFOUND)) {
            gpulist->currGpu().device_id =
                parse_last_hexbrackets(key);
            lookfor = false;
        } else if (key.find("OpenGL version string") != NOTFOUND) {
            gpulist->currGpu().renderer =
                parse_renderer_part(key);
        } else {
    ...
    while (line_loc != NOTFOUND) {
        ...
        if ((equals_pos != NOTFOUND) &&
            ...
            if (key.find("AdapterCompatibility") != NOTFOUND) {
                ...
                size_t vp = val_pos;
                size_t dll_sep_loc = contents.find(",", vp);
                size_t dll_end = (dll_sep_loc != NOTFOUND) ? dll_sep_loc : line_loc;
                gpulist->currGpu().addDll(contents.substr(vp, dll_end - vp));

                while (dll_sep_loc != NOTFOUND) {
                    vp = dll_sep_loc + 1;
                    dll_sep_loc = contents.find(",", vp);
                    dll_end = (dll_sep_loc != NOTFOUND) ? dll_sep_loc : line_loc;
                    gpulist->currGpu().addDll(contents.substr(vp, dll_end - vp));
                }
