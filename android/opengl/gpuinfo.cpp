...
GpuInfo::GpuInfo() {
    make = new char[FIELD_LEN];
    model = new char[FIELD_LEN];
    device_id = new char[FIELD_LEN];
    revision_id = new char[FIELD_LEN];
    version = new char[FIELD_LEN];
    renderer = new char[FIELD_LEN];
    current_gpu = false;
    dlls = NULL;
    num_dlls = 0;

    memset(make, 0, FIELD_LEN);
    memset(model, 0, FIELD_LEN);
    memset(device_id, 0, FIELD_LEN);
    memset(revision_id, 0, FIELD_LEN);
    memset(version, 0, FIELD_LEN);
    memset(renderer, 0, FIELD_LEN);
}
...
GpuInfoList::GpuInfoList() {
    infos = new GpuInfo*[MAX_GPUS];
    memset(infos, 0, MAX_GPUS);
    count = 0;
}
...
void GpuInfoList::addGpu() {
    count++;
    if (count > MAX_GPUS) return;

    int curr_gpu = count - 1;
    infos[curr_gpu] = new GpuInfo();
}
