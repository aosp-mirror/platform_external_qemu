...
static const BlacklistEntry sGpuBlacklist[] = {
        ...
        // Testing:
        {"10de", nullptr, "13ba", nullptr, nullptr,
         nullptr},  // Linux dev machine
        {"NVIDIA", "NVIDIA Quadro K600", nullptr, nullptr, nullptr,
         nullptr},  // Windows dev machine
        {"Intel (0x8086)", "Intel Iris Pro", nullptr, nullptr, nullptr,
         nullptr},  // OS X dev machine
};
    ...
    for (auto gpuinfo : gpulist->infos) {
        for (auto bl_entry : sGpuBlacklist) {
            ...
            if (bl_version && (gpuinfo.revision_id != bl_version))
                continue;
            if (bl_renderer && (gpuinfo.renderer != bl_renderer))
                continue;

            fprintf(stderr,
                    "There may be a problem with your GPU drivers."
...
void parse_gpu_info_list_linux(const std::string& contents,
    ...
    while (line_loc != NOTFOUND) {
        ...
        if (!lookfor && (key.find("VGA") != NOTFOUND)) {
            ...
            gpulist->currGpu().renderer = parse_renderer_part(key);
        } else {
        }
        if (line_loc == contents.size()) {
            break;
    ...
    while (line_loc != NOTFOUND) {
        ...
        if ((equals_pos != NOTFOUND) && (equals_pos < line_loc)) {
            ...
            if (key.find("AdapterCompatibility") != NOTFOUND) {
                ...
                size_t dll_end =
                        (dll_sep_loc != NOTFOUND) ? dll_sep_loc : line_loc;
