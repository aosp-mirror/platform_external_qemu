...
bool gpuinfo_query_blacklist(GpuInfoList* gpulist) {
    for (unsigned int i = 0; i < gpulist->infos.size(); i++) {
        ...
        for (unsigned int j = 0; j < sBlacklistSize; j++) {
            ...
            if (bl_make && (gpulist->infos[i].make != bl_make))
                continue;
            if (bl_model && (gpulist->infos[i].model != bl_model))
                continue;
            if (bl_device_id && (gpulist->infos[i].device_id != bl_device_id))
                continue;
            if (bl_revision_id &&
                (gpulist->infos[i].revision_id != bl_revision_id))
                continue;
            if (bl_version && (gpulist->infos[i].revision_id != bl_version))
                continue;
            if (bl_renderer && (gpulist->infos[i].renderer != bl_renderer))
                continue;
