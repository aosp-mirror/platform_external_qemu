                           ...
                           int is_fullscreen)
{
    ...
    SDL_RenderSetLogicalSize(renderer, original_w, original_h);

    if (DEBUG && VERBOSE_CHECK(surface)) {
        SDL_RendererInfo info;
        XX(SDL_GetRendererInfo(renderer, &info));
        printf("renderer.name = %s\n", info.name);
        printf("renderer.flags = 0x%x\n", info.flags);
        printf("renderer.num_texture_formats = %d\n", info.num_texture_formats);
        int nn;
        for (nn = 0; nn < info.num_texture_formats; ++nn) {
            printf("   0x%x (%s)\n", info.texture_formats[nn], SDL_GetPixelFormatName(info.texture_formats[nn]));
        }
    ...
    XX(SDL_SetRenderTarget(renderer, texture));
    XX(SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255));
    XX(SDL_RenderClear(renderer));

    XX(SDL_SetRenderTarget(renderer, NULL));
    XX(SDL_SetRenderDrawColor(renderer, 0, 0, 128, 255));
    XX(SDL_RenderClear(renderer));
    XX(SDL_RenderCopy(renderer, texture, NULL, NULL));
                       ...
                       uint32_t argb_premul)
{
    ...
    if (dst->surface) {
        SDL_FillRect(dst->surface, &r, argb_premul);
    }
...
void skin_surface_blit(SkinSurface* dst,
    ...
    SDL_RenderCopy(renderer, src->texture, &srect, &drect);
...
void skin_surface_upload(SkinSurface* surface,
    ...
    SDL_UpdateTexture(surface->texture, &r, pixels, pitch);
}
