...
skin_window_show_opengles( SkinWindow* window )
{
    ...
    // level, but Qt seems to do this transformation on it's own. This means the transformation
...
skin_window_scroll_updated( SkinWindow* window, int dx, int xmax, int dy, int ymax )
{
    ...
    if (left_buf + right_buf >= xmax) {
        ...
        px = (xmax == 0 ? 0 : (float) (dx - left_buf) / (float) (xmax - (left_buf + right_buf)));
    }
    ...
    if (top_buf + bottom_buf >= ymax) {
        ...
        py = (ymax == 0 ? 0 : (float) ((ymax - dy) - bottom_buf) / (float) (ymax - (bottom_buf + top_buf)));
    }
