...
{
    bool OK = false;
    int  id = extendedUi->finger_IdBox->toPlainText().toInt(&OK);

    if (checked && !OK) {
        QErrorMessage *eM = new QErrorMessage;
        eM->showMessage(QString("The \"Fingerprint ID\" number is invalid"));
        extendedUi->finger_IdBox->setEnabled(true);
        extendedUi->finger_touchCkBox->setChecked(false);
        return;
    }

    if (fingerAgent && fingerAgent->setTouch) {
        fingerAgent->setTouch(checked, id);
    }

    // Don't allow changing the ID while a touch is active
    extendedUi->finger_IdBox->setEnabled( !checked );
}
