...
void ExtendedWindow::initSms()
{
    ...
    extendedUi->sms_phoneNumberBox->setValidator(new phoneNumberValidator);
}
...
void ExtendedWindow::on_sms_sendButton_clicked()
{
    ...
    int retVal = sms_address_from_str(&sender,
                                      extendedUi->sms_phoneNumberBox->currentText().toStdString().c_str(),
                                      extendedUi->sms_phoneNumberBox->currentText().length());
    ...
    QString theMessage = extendedUi->sms_messageBox->toPlainText();
