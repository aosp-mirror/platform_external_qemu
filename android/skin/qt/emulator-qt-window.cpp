...
void EmulatorQtWindow::closeEvent(QCloseEvent* event) {
    ...
    if (mMainLoopThread && mMainLoopThread->isRunning()) {
        ...
        // Now the main loop is not running, so any OpenGL pipes
        // that are not closed, will not get closed as they generally rely
        // on a signal from the guest in order to do that.
