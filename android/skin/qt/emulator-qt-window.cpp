        ...
        mMainLoopThread(nullptr)
{
    ...
    // TODO(birenbaum): replace the following with proper cursor assets
    mZoomInPixmap.load(":/dark/zoom");
    mZoomOutPixmap.load(":/dark/zoom");
        ...
        if (event->key() == Qt::Key_Control) {
            if (type == kEventKeyDown) {
                if (event->modifiers() & Qt::ShiftModifier) {
                    mContainer.setCursor(QCursor(mZoomOutPixmap));
                } else {
                    mContainer.setCursor(QCursor(mZoomInPixmap));
                }
