...
Accelerometer3DWidget::Accelerometer3DWidget(QWidget* parent) :
    GLWidget(parent),
    mYawPitchRoll(0,0,0), mTracking(false) {
    toggleAA();
}
...
bool loadTexture(const GLESv2Dispatch* gles2,
...
GLuint create2DTexture(const GLESv2Dispatch* gles2,
                       const char* source_filename,
                       GLenum target,
                       GLenum format,
...
void Accelerometer3DWidget::initGL() {
    ...
    mCameraTransform.lookAt(QVector3D(0.0, 0.0, 8.5), QVector3D(0, 0, 0), QVector3D(0, 1, 0));
...
void Accelerometer3DWidget::repaintGL() {
    ...
    float ypr[3];
    mQuat.getEulerAngles(&ypr[1], &ypr[0], &ypr[2]);
    qWarning("Rotation %f %f %f\n", ypr[2], ypr[1], ypr[0]);
    QMatrix4x4 model_view_transform;
    model_view_transform.setToIdentity();
    model_view_transform.rotate(mQuat);//device_rotation_quat);
