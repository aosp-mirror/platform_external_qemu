...
{
    loc_timer.setSingleShot(true);
    QObject::connect(&loc_timer, &QTimer::timeout, this, &ExtendedWindow::loc_slot_timeout);
}

void ExtendedWindow::on_loc_addRowButton_clicked()
{
    // Add a row below the currently-highlighted row
    int currentRow = extendedUi->loc_pathTable->currentRow();
    if (currentRow < 0) {
        // Nothing selected, act as if the last is selected
        currentRow = extendedUi->loc_pathTable->rowCount() - 1;
    }
    extendedUi->loc_pathTable->insertRow(currentRow + 1);
    // Initialize it the same as the row above
    for (int col = 0; col< extendedUi->loc_pathTable->columnCount(); col++) {
        extendedUi->loc_pathTable->setItem(currentRow+1, col,
                                       extendedUi->loc_pathTable->item(currentRow, col)->clone());
    }

    // We must have more than one row. Allow deletion.
    setButtonEnabled(extendedUi->loc_removeRowButton, true);
}

void ExtendedWindow::on_loc_removeRowButton_clicked()
{
    // Delete the currently-highlighted row
    if (extendedUi->loc_pathTable->rowCount() > 1) {
        ...
        extendedUi->loc_pathTable->removeRow(currentRow);
        if (currentRow == 0) {
    ...
    if (extendedUi->loc_pathTable->rowCount() <= 1) {
        ...
        setButtonEnabled(extendedUi->loc_removeRowButton, false);
    }
}
...
void ExtendedWindow::on_loc_pathTable_cellChanged(int row, int col)
{
    ...
    if (row == 0 && col == 0) {
        extendedUi->loc_pathTable->item(0,0)->setText(QString("0"));
    }

    bool cellOK = loc_cellIsValid(extendedUi->loc_pathTable, row, col);
    QColor newColor = (cellOK ? Qt::white : Qt::red);

    extendedUi->loc_pathTable->item(row, col)->setBackground(QBrush(newColor));
}

void ExtendedWindow::on_loc_playButton_clicked()
{
    // Validate all the table's values
    for (int row = 0; row < extendedUi->loc_pathTable->rowCount(); row++) {
        for (int col = 0; col < extendedUi->loc_pathTable->columnCount(); col++) {
            if (!loc_cellIsValid(extendedUi->loc_pathTable, row, col)) {
    ...
    loc_rowToSend = 0;
    loc_timer.setInterval(0); // Fire when I return
    loc_timer.start();
    setButtonEnabled(extendedUi->loc_playButton,  false);
    setButtonEnabled(extendedUi->loc_pauseButton, true);
    setButtonEnabled(extendedUi->loc_stopButton,  true);
}
...
void ExtendedWindow::loc_slot_timeout()
{
    ...
    if (loc_rowToSend < 0  ||  loc_rowToSend >= extendedUi->loc_pathTable->rowCount()) {
    ...
    theItem = extendedUi->loc_pathTable->item(loc_rowToSend, 1);
    double lat = theItem->text().toDouble(&cellOK);

    theItem = extendedUi->loc_pathTable->item(loc_rowToSend, 2);
    double lon = theItem->text().toDouble(&cellOK);

    theItem = extendedUi->loc_pathTable->item(loc_rowToSend, 3);
    ...
    snprintf(curLoc, 64, "Sent row %2d: %9.4f, %8.4f, %3.0f",
             loc_rowToSend+1, lat, lon, alt);

    extendedUi->loc_currentLocBox->setPlainText(QString(curLoc));

    if (locationAgent  &&  locationAgent->gpsCmd) {
        locationAgent->gpsCmd(lat, lon, alt, nSats, &timeVal);
    }

    // Go on to the next row
    loc_rowToSend++;
    if (loc_rowToSend >= extendedUi->loc_pathTable->rowCount()) {
        // No more to send. Same as clicking Stop.
        on_loc_stopButton_clicked();
        return;
    }
    ...
    theItem = extendedUi->loc_pathTable->item(loc_rowToSend, 0);
    double dTime = theItem->text().toDouble();
    int mSec = dTime * 1000.0;
    if (mSec < 0) mSec = 0;
    loc_timer.setInterval(mSec);
    loc_timer.start();
    loc_nowPaused = false;
}
...
void ExtendedWindow::on_loc_pauseButton_clicked()
{
    if (loc_nowPaused) {
        ...
        // the remaining time
        loc_timer.setInterval(loc_mSecRemaining);
        loc_timer.start();
        loc_mSecRemaining = -1;
        loc_nowPaused = false;
    } else {
        // Pause the timer operation
        if (!loc_timer.isActive()) {
            // Nothing more to do. Treat this as a Stop.
            on_loc_stopButton_clicked();
            return;
        }
        ...
        // the timer
        loc_mSecRemaining = loc_timer.remainingTime();
        if (loc_mSecRemaining < 0) loc_mSecRemaining = 0;
        loc_timer.stop();
        loc_nowPaused = true;
    }
}
...
void ExtendedWindow::on_loc_stopButton_clicked()
{
    ...
    setButtonEnabled(extendedUi->loc_playButton,  true);
    setButtonEnabled(extendedUi->loc_pauseButton, false);
    setButtonEnabled(extendedUi->loc_stopButton,  false);
}
...
bool ExtendedWindow::loc_cellIsValid(QTableWidget *table, int row, int col)
{
    ...
    switch (col) {
        case 0: // Delay
            theItem = extendedUi->loc_pathTable->item(row, col);
...
void ExtendedWindow::on_loc_KmlButton_clicked()
{
    ...
    extendedUi->loc_pathTable->setRowCount(0);

    loc_readKmlFile(&theFile);

    // Ensure that we have at least on row
    if (extendedUi->loc_pathTable->rowCount() == 0) {
        extendedUi->loc_pathTable->insertRow(0);

        extendedUi->loc_pathTable->setItem(0, 0, new QTableWidgetItem("0")); // Delay
        extendedUi->loc_pathTable->setItem(0, 1, new QTableWidgetItem("0")); // Latitute
        extendedUi->loc_pathTable->setItem(0, 2, new QTableWidgetItem("0")); // Longitude
        extendedUi->loc_pathTable->setItem(0, 3, new QTableWidgetItem("0")); // Elevation
        extendedUi->loc_pathTable->setItem(0, 4, new QTableWidgetItem("" )); // Name
        extendedUi->loc_pathTable->setItem(0, 5, new QTableWidgetItem("" )); // Description
    }

    setButtonEnabled(extendedUi->loc_removeRowButton,
                     extendedUi->loc_pathTable->rowCount() > 1 );
}
...
{
    int newRow = extendedUi->loc_pathTable->rowCount();

    extendedUi->loc_pathTable->insertRow(newRow);
    extendedUi->loc_pathTable->setItem(newRow, 0, new QTableWidgetItem("10")); // Delay
    extendedUi->loc_pathTable->setItem(newRow, 1, new QTableWidgetItem(lat));
    extendedUi->loc_pathTable->setItem(newRow, 2, new QTableWidgetItem(lon));
    extendedUi->loc_pathTable->setItem(newRow, 3, new QTableWidgetItem(elev));
    extendedUi->loc_pathTable->setItem(newRow, 4, new QTableWidgetItem(name));
    extendedUi->loc_pathTable->setItem(newRow, 5, new QTableWidgetItem(description));
}
