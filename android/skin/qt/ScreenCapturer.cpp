...
// take... a while.
const System::Duration ScreenCapturer::kScreenCaptureTimeoutMs = 30000;
...
ScreenCapturer::ScreenCapturer(const QString& adbCommand,
    ...
    qRegisterMetaType<ScreenCapturer::Result>("android::qt::ScreenCapturer::Result");
    qRegisterMetaType<ScreenCapturer::Result>("ScreenCapturer::Result");
    qRegisterMetaType<ScreenCapturer::Result>("Result");
...
void ScreenCapturer::start() {
    if (mInFlight) {
        screenCaptureDone(Result::kOperationInProgress);
    }

    mInFlight =
            android::base::async([this]() { return this->doScreenCapture(); });
    if (!mInFlight) {
        screenCaptureDone(Result::kUnknownError,
...
intptr_t ScreenCapturer::doScreenCapture() {
    ...
    if (!sys->runCommand(mCaptureCommand,
        ...
        screenCaptureDone(Result::kCaptureFailed);
        return 0;
    }
    ...
    if (dirName.isEmpty()) {
        screenCaptureDone(Result::kSaveLocationInvalid);
        return 0;
    }
    ...
    if (!sys->runCommand(command,
        ...
        exitCode != 0) {
        // TODO(pprabhu): Capture stderr and return an intelligent error string.
        screenCaptureDone(Result::kPullFailed);
        return 0;
    }
    screenCaptureDone(Result::kSuccess);
    return 0;
}

void ScreenCapturer::screenCaptureDone_slot(Result outResult,
                                            QString errString) {
    mInFlight = false;
