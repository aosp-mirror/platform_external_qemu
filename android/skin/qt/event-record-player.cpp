...
void EventRecordPlayer::follow(QWidget* const wid) {
    QList<QWidget*> to_follow = wid->findChildren<QWidget*>();
    to_follow.append(wid);
    for (auto w : to_follow) {
        std::string name = fullName(static_cast<const QObject*>(w));
        if (name.empty()) {
            derror("Cannot track/replay events to unnamed widget");
            continue;
        }
        auto ret = mFollowed.emplace(name, w);
        if (ret.second == false) {
            D("Widget %s already being followed for input events",
              name.c_str());
        }
    }
}

void EventRecordPlayer::unfollow(const QWidget* const wid) {
...
void EventPlayer::replayEvent() {
    ...
    if (w == NULL) {
        derror("Can't replay event to Widget \"%s\"; no such widget found",
               rec.target_name.c_str());
        derror("Unsupported or malformatted input recording; "
               "please record again");
        // abandon event replay, this recording is likely outdated
        return;
    }
...
void EventRecorder::print() const {
    for (unsigned int i = 0; i < container().size(); ++i) {
        std::cout << "Event: " << i << " dt:" << container()[i].dt
                  << " target: " << container()[i].to_string().c_str()
                  << std::endl;
    }
}
