...
void ExtendedWindow::initKbdShorts() {
   ...
   for (int cmd = SKIN_KEY_COMMAND_NONE + 1;
       ...
       if (bindings_count > 0) {
           ...
           table_widget->setItem(table_row, 0, new QTableWidgetItem(binding_str));
           table_widget->setItem(table_row, 1, new QTableWidgetItem(description));
       }
   }
}
