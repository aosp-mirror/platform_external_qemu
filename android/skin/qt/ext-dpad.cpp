...
#define ON_PRESSED(buttonName) \
    ...
    { \
        setPressed(mExtendedUi->dpad_ ## buttonName ## Button); \
    }

#define ON_RELEASED(buttonName) \
    void ExtendedWindow::on_dpad_ ## buttonName ## Button_released() \
    { \
        setReleased(mExtendedUi->dpad_ ## buttonName ## Button); \
    }
...
void ExtendedWindow::setPressed(QPushButton* button)
{
...
void ExtendedWindow::setReleased(QPushButton* button)
{
    QString iconName = button->property("themeIconName").toString();
    if ( !iconName.isNull() ) {
        QString resName = ":/";
