...
bool UIEventRecordPlayer::setReplayDelay(const char* delay) {
    ...
    if (inRecordMode()) {
        D("Replay delay not applicable while recording");
        return false;
    }
...
bool UIEventRecordPlayer::init(const char* recordPath,
    ...
    if (setReplayPath(replayPath)) {
        return setReplayDelay(delay);
    }

    if (setRecordPath(recordPath)) {
        if (delay != NULL) {
            D("Replay delay not applicable when recording\n");
            return true;
        }
        return true;
    }
...
void UIEventRecordPlayer::unfollow(QWidget* wid) {
    std::string name = wid->metaObject()->className();
    if (!mFollowed.erase(name)) {
