...
public:
    ...
    void startRecording(EventCapturer* ecap, QWidget* widget) {
        mEventCapturerTokens[widget] = ecap->subscribeToEvents(
    ...
    void stopRecording(QWidget* widget) {
        auto it = mEventCapturerTokens.find(widget);
        if (it != mEventCapturerTokens.end()) {
            *it = EventCapturer::SubscriberToken();
        }
    }
...
private:
    ...
    std::unordered_map<QWidget*, EventCapturer::SubscriberToken> mEventCapturerTokens;
};
