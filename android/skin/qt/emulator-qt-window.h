...
private:
    ...
    void runAdbPush(QList<QUrl> urls);
    void adbPushProgress(unsigned progressPercent, bool done);
    ...
    // Can't create an instance when EmulatorQtWindow is created because Qt
    // thread's looper isn't ready yet. So, carry a pointer.
    std::unique_ptr<android::emulation::FilePusher> mFilePusher;
