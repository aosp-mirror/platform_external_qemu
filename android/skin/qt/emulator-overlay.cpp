...
void EmulatorOverlay::mouseMoveEvent(QMouseEvent* event) {
    if (mMode == OverlayMode::Zoom) {
        ...
        generateMouseEvents(kEventMouseMotion, event->buttons());
    }
}
...
void EmulatorOverlay::mousePressEvent(QMouseEvent* event) {
    if (mMode == OverlayMode::Zoom) {
        ...
        if (!androidHwConfig_isScreenMultiTouch(android_hw)) {
            ...
            generateMouseEvents(kEventMouseButtonDown, event->buttons());
        }
    }
}
...
void EmulatorOverlay::mouseReleaseEvent(QMouseEvent* event) {
    if (mMode == OverlayMode::Zoom) {
        ...
        generateMouseEvents(kEventMouseButtonUp, event->buttons());
    }
}
...
void EmulatorOverlay::generateMouseEvents(SkinEventType type, Qt::MouseButtons buttons) {
    if (buttons & Qt::LeftButton) {
        mEmulatorWindow->handleMouseEvent(type, kMouseButtonLeft,
                                          mPrimaryTouchPoint);
        mEmulatorWindow->handleMouseEvent(type, kMouseButtonSecondaryTouch,
                                          getSecondaryPinchPoint());
    } else if (buttons & Qt::RightButton) {
        // TODO: do the other multitouch UI
    }
}
