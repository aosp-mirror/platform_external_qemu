...
// On Linux, this threshold is 0.
    ...
    if (mMode == OverlayMode::Multitouch) {
        ...
        painter.drawImage(getSecondaryPinchPoint(), mTouchImage);
        painter.resetTransform();
...
void EmulatorOverlay::genereateTouchEvents(QMouseEvent* event) {
    ...
    QPoint secondaryPoint;

    if (event->type() == QMouseEvent::MouseButtonPress) {
        eventType = kEventMouseButtonDown;
        secondaryPoint = (event->button() == Qt::LeftButton ? getSecondaryPinchPoint() : getSecondarySwipePoint());
    } else if (event->type() == QMouseEvent::MouseButtonRelease) {
        eventType = kEventMouseButtonUp;
        secondaryPoint = (event->button() == Qt::LeftButton ? getSecondaryPinchPoint() : getSecondarySwipePoint());
    } else if (event->type() == QMouseEvent::MouseMove) {
        eventType = kEventMouseMotion;
        secondaryPoint = (event->buttons() & Qt::LeftButton ? getSecondaryPinchPoint() : getSecondarySwipePoint());
    }

    if (eventType) {
        mEmulatorWindow->handleMouseEvent(eventType, kMouseButtonLeft, mPrimaryTouchPoint);
        mEmulatorWindow->handleMouseEvent(eventType, kMouseButtonSecondaryTouch, secondaryPoint);
    }
}

QPoint EmulatorOverlay::getSecondaryPinchPoint() const {
    return mPrimaryTouchPoint + 2 * (mMultitouchCenter - mPrimaryTouchPoint);
}

QPoint EmulatorOverlay::getSecondarySwipePoint() const {
    // TODO(birenbaum): decide how to implement this best
    return getSecondaryPinchPoint();
}
