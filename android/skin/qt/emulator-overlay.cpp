...
EmulatorOverlay::EmulatorOverlay(EmulatorQtWindow *window, EmulatorContainer *container)
    : QFrame(container),
      ...
      mMode(OverlayMode::Hidden)
{
    ...
    QObject::connect(&mFlashAnimation, SIGNAL(finished()),
                     this, SLOT(slot_animationFinished()));
    QObject::connect(&mFlashAnimation, SIGNAL(valueChanged(QVariant)),
...
void EmulatorOverlay::mouseReleaseEvent(QMouseEvent *event)
{
    if (mMode == OverlayMode::Zoom) {
        ...
        QPoint localPoint = mEmulatorWindow->mapFromGlobal(mapToGlobal(geom.center()));
