...
bool GLWidget::setupContextAndSurface() {
    ...
    EGLint num_config;
    if (s_egl.eglChooseConfig(mEGLState->Display, config_attribs, &(mEGLState->Config), 1, &num_config) == EGL_FALSE) {
        qWarning("Failed to choose EGL config %d", num_config);
        return false;
    }
    ...
    mEGLState->Context =
        s_egl.eglCreateContext(mEGLState->Display, mEGLState->Config, EGL_NO_CONTEXT, context_attribs);
