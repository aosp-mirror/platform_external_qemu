...
#define CHECK_GL_ERROR(error_msg, cleanup_func) \
...
void GLWidget::renderFrame() {
    ...
    mGLES2->glVertexAttribPointer(mAAPositionAttribLocation, 3, GL_FLOAT,
                                  GL_FALSE, 0, 0);
...
void GLWidget::initFramebuffer() {
    ...
    CHECK_GL_ERROR("Failed to create target texture for FBO",
                   destroyFramebuffer);
    mGLES2->glBindTexture(GL_TEXTURE_2D, mTargetTexture);
    mGLES2->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width() * devicePixelRatio(),
                         height() * devicePixelRatio(), 0, GL_RGB,
                         GL_UNSIGNED_BYTE, 0);
    ...
    mGLES2->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                   GL_TEXTURE_2D, mTargetTexture, 0);
    ...
    CHECK_GL_ERROR("Failed to create depth buffer", destroyFramebuffer);
    mGLES2->glBindRenderbuffer(GL_RENDERBUFFER, mDepthBuffer);
    mGLES2->glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, width() * devicePixelRatio(), height() * devicePixelRatio());
    CHECK_GL_ERROR("Failed to initialize depth buffer", destroyFramebuffer);
    mGLES2->glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, mDepthBuffer);
    CHECK_GL_ERROR("Failed to attach depth buffer to FBO", destroyFramebuffer);
...
void GLWidget::initAAProgram() {
    ...
    CHECK_GL_ERROR(
            "Failed to get attributes & uniforms for anti-aliasing shader",
            destroyAAProgram);
    ...
    mGLES2->glBufferData(GL_ARRAY_BUFFER, sizeof(vertex_data), vertex_data,
                         GL_STATIC_DRAW);
...
void GLWidget::bindFramebuffer() {
    mGLES2->glBindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);
    mGLES2->glViewport(0, 0, width() * devicePixelRatio(),
                       height() * devicePixelRatio());
}
