...
void GLWidget::initFramebuffer() {
    ...
    mGLES2->glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT,
                         width() * devicePixelRatio(),
                         height() * devicePixelRatio(), 0, GL_DEPTH_COMPONENT,
                         GL_UNSIGNED_INT, 0);
    ...
    mGLES2->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    mGLES2->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    mGLES2->glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
