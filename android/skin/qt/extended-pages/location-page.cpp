...
static const double GPLEX_LON = -122.084;
static const double GPLEX_LAT =   37.422;
    ...
    mGeoDataLoadingStopRequested(false)
{
    ...
    QSettings settings;
    double altValue = settings.value(Ui::Settings::LOCATION_ALTITUDE, 0.0).toDouble();
    mUi->loc_altitudeInput->setText( QString::number(altValue, 'f', 1) );
    mUi->loc_longitudeInput->setValue(
            settings.value(Ui::Settings::LOCATION_LONGITUDE, GPLEX_LON).toDouble());
    mUi->loc_latitudeInput->setValue(
            settings.value(Ui::Settings::LOCATION_LATITUDE, GPLEX_LAT).toDouble());
...
void LocationPage::updateDisplayedLocation(double lat, double lon, double alt) {
    QString curLoc;
    curLoc += tr("Longitude: ");
    curLoc += QString::number(lon, 'f', 4);
    curLoc += tr("\nLatitude: ");
    curLoc += QString::number(lat, 'f', 4);
    curLoc += tr("\nAltitude: ");
    curLoc += QString::number(alt, 'f', 1);
...
void LocationPage::on_loc_altitudeInput_editingFinished() {
    QSettings settings;
    settings.setValue(Ui::Settings::LOCATION_ALTITUDE,
                      mUi->loc_altitudeInput->text().toDouble());
}
                                     ...
                                     double* pAltitude)
{
    ...
    if (locAgent && locAgent->gpsLoc) {
        // Query the device
        int    nSats;  // Unused
        gotDeviceLoc = locAgent->gpsLoc(pLatitude, pLongitude, pAltitude, &nSats);
    }
                                        ...
                                        double altitude)
{
    ...
    if (locAgent && locAgent->gpsCmd) {
        // Send these to the device
        timeval timeVal = {};
        gettimeofday(&timeVal, nullptr);
        locAgent->gpsCmd(latitude, longitude, altitude, 4, &timeVal);
    }
}
