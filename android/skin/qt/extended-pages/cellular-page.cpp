...
void CellularPage::setCellularAgent(const QAndroidCellularAgent* agent) {
    ...
    if (android_op_netspeed) {
        ...
        mUi->cell_standardBox    ->setEnabled(false);
        mUi->cell_signalStatusBox->setEnabled(false);
        mUi->cell_voiceStatusBox ->setEnabled(false);
        mUi->cell_dataStatusBox  ->setEnabled(false);
