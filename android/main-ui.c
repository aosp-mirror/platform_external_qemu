...
attach_to_core(AndroidOptions* opts) {
    ...
    attach_client = android_create_core_connection(&console_socket);
    if (attach_client != NULL) {
        if (!android_open_core_connection(attach_client)) {
            if (!android_switch_core_connection_stream(attach_client, "attach UI",
                                                       &core_ui_settings)) {
                ...
                derror("Unable to attach to the core %s: %s\n",
                       sock_address_to_string(&console_socket),
                       core_ui_settings);
                android_close_core_connection(attach_client);
                android_free_core_connection(attach_client);
                attach_client = NULL;
                return -1;
            }
        } else {
