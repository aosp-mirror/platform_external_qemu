...
static void _hwNfc_reset_request(HwNfcService* nfc);
//static void _hwNfc_reset_response(HwNfcService* nfc);
...
static void _hwNfc_save(Stream* f, QemudService* sv, void* opaque) {
    ...
    stream_put_be32(f, nfc->response_length);
...
static int _hwNfc_load(Stream* f, QemudService* sv, void* opaque) {
    ...
    HwNfcService* nfc = opaque;
    if (nfc->request_length > 0) {
        _hwNfc_reset_request(nfc);
    }

    size_t sz = stream_get_be32(f);
