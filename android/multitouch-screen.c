...
int multitouch_create_buttons_state(int is_down,
                                       int skip_sync,
                                       int finger_number) {
    int state = 0;
    if (is_down)
    ...
    if (finger_number)
        state |= (1 << (kShiftSecondaryFinger + finger_number - 1));
    return state;
}

int multitouch_is_touch_down(int buttons_state) {
    return buttons_state & (1 << kShiftIsTouchDown);
}

int multitouch_should_skip_sync(int buttons_state) {
    return buttons_state & (1 << kShiftShouldSkipSync);
}

int multitouch_get_finger_number(int buttons_state) {
    int i;
    for (i = 0; i < MTS_POINTERS_NUM; i++) {
        if (buttons_state & (1 << (kShiftSecondaryFinger + i))) {
            return i + 1;
        }
    }
    return 0;
