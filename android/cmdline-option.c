...
android_parse_options( int  *pargc, char**  *pargv, AndroidOptions*  opt )
{
    ...
    while (nargs > 0) {
        ...
        if (!strcmp(arg, "debug")) {
            ...
            nargs--;
            android_parse_debug_tags_option(*aread++, false);
            continue;
        } else if (!strncmp(arg, "debug-", sizeof("debug-") - 1)) {
            // For a "-debug-<tag>" option we only allow a single tag,
            // otherwise it may look strange to someone.
            android_parse_debug_tags_option(arg + sizeof("debug-") - 1, true);
            continue;
        }
...
bool android_parse_debug_tags_option(const char* opt, bool single_tag) {
    if (opt == NULL)
        return false;
    ...
    const char* x = opt;
    while (*x) {
        const char* y = single_tag ? (x + strlen(x)) : strchr(x, ',');
        if (y == NULL)
            y = x + strlen(x);
...
parse_env_debug_tags( void )
{
    ...
    android_parse_debug_tags_option(env, false);
}
