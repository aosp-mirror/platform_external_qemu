...
class MediaVideoToolBoxHevcVideoHelper : public MediaVideoHelper {
public:
    ...
    MediaVideoToolBoxHevcVideoHelper(int w,
                                     int h,
                                     OutputTreatmentMode outMode,
                                     FrameStorageMode fMode);
    ~MediaVideoToolBoxHevcVideoHelper() override;
...
private:
    ...
    struct InputFrame {
        InputFrame(HevcNaluParser::HevcNaluType in_type,
                   const uint8_t* in_data,
                   int in_size)
            : type(in_type), data(in_data), size(in_size) {}

        HevcNaluParser::HevcNaluType type;
        const uint8_t* data;
        int size;
    };
    ...
    std::vector<uint8_t> mVPS;  // vps NALU
...
};  // MediaVideoToolBoxHevcVideoHelper
