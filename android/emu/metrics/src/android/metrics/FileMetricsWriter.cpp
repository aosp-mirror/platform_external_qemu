...
FileMetricsWriter::finalizeAbandonedSessionFilesFromDefault() {
    return finalizeAbandonedSessionFiles(getSpoolDirectory());
}
...
void FileMetricsWriter::openNewFileNoLock() {
    ...
    int fd = -1;
    if (!runFileOperationWithRetries(
                &mFilenameCounter, [this, &newName, &fd]() -> bool {
                    std::string testName = PathUtils::join(
                            mSpoolDir,
                            formatFilename(sessionId(), mFilenameCounter,
                                           kTempExtension));
    ...
    mActiveFile = absl::make_unique<google::protobuf::io::FileOutputStream>(fd);
    if (!mActiveFile) {
...
void FileMetricsWriter::ensureTimerStarted() {
    ...
    mTimer.reset(mLooper->createTimer(
            [](void* weakSelf, Looper::Timer* timer) {
                auto weakPtr = static_cast<FileMetricsWriter::WPtr*>(weakSelf);
    ...
    if (mTimer) {
        ...
        D("started metrics timer with a period of %d ms", mTimeLimitMs);
    }
}
