...
public:
    ...
    [[nodiscard]] auto error() const -> int { return mError; }
    ...
    auto tryRead(void* buffer, size_t len, size_t* readBytes) -> bool {
        *readBytes = 0;
...
// return false.
auto parse_hex8(const char* input, uint32_t* value) -> bool {
    uint32_t result = 0;
    for (int n = 0; n < 8; ++n) {
        int c = input[n];
...
auto android_extractRamdiskFile(const char* ramdiskPath,
    ...
    *outSize = 0;
    ...
    for (;;) {
        ...
        // name.
        size_t skipName = ((sizeof header + nameSize + 3) & ~3) - sizeof header;
        ...
        if ((entrySize == 0 || nameSize != fileNameLen + 1U) &&
            ...
            if (!input.doRead(entryName.data(), nameSize)) {
