...
void android_emulator_bridge_pull(const char* filename) {
    ...
    if (!pipe->state.ready) {
        ...
        pipe->print_callback(pipe->monitor, "Another command is pending. Abort\n");
        return;
    }

    if (strlen(filename) > AEB_COMMAND_WIDTH - 1) {
        D("%s is too long. Abort\n", filename);
        pipe->print_callback(pipe->monitor, "%s is too long. Abort\n", filename);
        return;
    }
    ...
    if (!pipe->state.current_file_handle) {
        ...
        pipe->print_callback(pipe->monitor, "Cannot open file ./%s on the host for writing. Abort.\n", file_basename);
        return;
    }
...
void android_emulator_bridge_push(const char* filename) {
    ...
    if (bin == NULL) {
        ...
        pipe->print_callback(pipe->monitor, "KO:Cannot read file %s from host", filename);
        return;
    }
...
void android_emulator_bridge_register_print_callback(void (*monitor_printf)(void*, const char* fmt, ...)) {
