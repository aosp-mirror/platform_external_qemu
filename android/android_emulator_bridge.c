    ...
    D("%s: DONE -> SUCCESS: %" PRIu64 "ms\n", __FUNCTION__, done_success_ms);
        ...
        case CMD_SPEEDTEST:
            ...
            if (!strcmp(msg, "SUCCESS") || !strcmp(msg, "ERROR")) {
                ...
                fprintf(stderr, "received return msg (after guest received all 10), len=%d us elapsed: %d\n", msglen, get_elapsed_timer_us());
...
void android_emulator_bridge_speedtest() {
    ...
    reset_timer();
    for (i = 0; i < 10; i++) {
        send_host_guest_packet(64);
    }
    fprintf(stderr, "us since sending 10x 64byte packets to guest (no ack):%d\n", get_elapsed_timer_us());
    pipe->state.cmd = CMD_SPEEDTEST;
}
