...
static void enqueue_msg(const char* what, size_t msglen) {
...
static void _aeb_send(const char* what, size_t msglen) {
...
static void send_aeb_payload(const char* msg, size_t msglen) {
    char cmd[AEB_NUMCONST_WIDTH] = {};

    // D("%s: call. payload size=%d\n", __FUNCTION__, msglen);
    snprintf(cmd, sizeof(cmd), "%016x", msglen);
    _aeb_send(cmd, AEB_NUMCONST_WIDTH);
    _aeb_send(msg, msglen);
}


char* load_file(const char* fn, size_t* sz, int* fd) {
#if USE_MMAP
    struct stat sb;
    *fd = open(fn, O_RDONLY);
    ...
    *fd = -1;
    ...
    size_t ret = fread(res, totalsize, 1, fh);
    if (!ret) {
...
float transfer_rate(size_t transfer_size, size_t transfer_ms) {
    return ((float)transfer_size) / 
            ((float)(1024 * 1024)) / 
            transfer_ms *
            1000.0;
}

void android_emulator_bridge_recv(AEBPipe* pipe, char* msg, size_t msglen,
                                  bool need_free) {
    ...
    switch (pipe->state.cmd) {
        case CMD_PUSH:
            // If in CMD_PUSH, we should get an acknowledgement from the device for
            // success/failure
            D("%s: call. msg=%s msglen=%u\n", __FUNCTION__, msg, msglen);
            ...
            if (is_command(pipe, "SUCCESS")) {
                ...
                pipe->state.current_ms = (size_t)(pipe->state.tp.tv_sec * 1000 + pipe->state.tp.tv_usec / 1000 - pipe->state.current_ms);
                D("%s: Time measured since beginning of transfer (including pipe lag): %u ms. Time on host: %u ms\n", __FUNCTION__, pipe->state.current_ms, pipe->state.host_ms);

                D("Transfer rate (end to end + lag in receiving ack): %f MiB/s Setting to idle.\n", 
                  transfer_rate(pipe->state.current_transfer, pipe->state.current_ms));
        ...
        case CMD_PULL:
            if (pipe->state.pull_step == 0 && !strcmp(msg, "ERROR")) {
                ...
                pipe->state.cmd = CMD_IDLE;
                break;
            } else if (pipe->state.pull_step == 0 && !strcmp(msg, "PULL_SIZE")) {
                ...
                sscanf(msg, "%016llx", &pipe->state.current_transfer);
                D("%s: File being pulled is %u bytes\n", __FUNCTION__,
                ...
                // D("%s: PULLING: so far: %llu bytes. total: %llu bytes\n", __FUNCTION__, pipe->state.pulled_so_far, pipe->state.current_transfer);
            }
            ...
            if (pipe->state.pull_step == 2 &&
                ...
                D("%s: PULLING: Time taken to transfer: %u ms\n", __FUNCTION__, pipe->state.current_ms);
...
void android_emulator_bridge_pull(const char* filename) {
    ...
    gettimeofday(&pipe->state.tp, NULL);
    pipe->state.current_ms = (size_t)(pipe->state.tp.tv_sec * 1000 + pipe->state.tp.tv_usec / 1000);
...
void android_emulator_bridge_push(const char* filename) {
    ...
    size_t filesize = to_transfer;

    pipe->state.current_transfer = (uint64_t)filesize;
    D("Transferring %u bytes to virtual device.\n", pipe->state.current_transfer);
    ...
    int max_chunk_size = AEB_PUSH_CHUNK_SIZE;
    size_t j = to_transfer > max_chunk_size ? max_chunk_size : to_transfer;
    ...
    gettimeofday(&pipe->state.tp, NULL);
    pipe->state.host_ms = (size_t)(pipe->state.tp.tv_sec * 1000 + pipe->state.tp.tv_usec / 1000 - pipe->state.current_ms);
...
static int _aebPipe_sendBuffers(void* opaque,
    ...
    android_emulator_bridge_recv(pipe, msg, transferred, need_free);
    return transferred;
}
...
static int _aebPipe_recvBuffers(void* opaque,
    ...
    while (buff != endbuff && *msg_list != NULL) {
        ...
        size_t msg_remaining = msg->msglen - msg->offset;
        size_t buff_remaining = buff->size - off_in_buff;
        size_t to_copy =
