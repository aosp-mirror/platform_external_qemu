...
void android_emulator_bridge_recv(AEBPipe* pipe, char* msg, uint64_t msglen) {
    ...
    switch (pipe->state.cmd) {
        case CMD_PUSH:
            ...
            break;
        case CMD_PULL:
            if (pipe->state.current_transfer == -1 && !strcmp(msg, "ERROR")) {
                D("%s: Error pulling file. Abort.\n", __FUNCTION__);
                pipe->state.cmd = CMD_IDLE;
                break;
            } else if (pipe->state.current_transfer == -1 && !strcmp(msg, "PULL_SIZE")) {
                D("%s: got PULL_SIZE message\n", __FUNCTION__);
                pipe->state.current_transfer = 0;
            } else if (pipe->state.current_transfer == 0) {
                sscanf(msg, "%016" PRIu64, &pipe->state.current_transfer);
...
static int _aebPipe_sendBuffers(void* opaque,
    ...
    if (numBuffers == 1) {
        ...
        memcpy(msg, buffers->data, buffers->size);
    }  else {
