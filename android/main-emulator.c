...
static bool checkAvdKernelForRanchuSupport(const char* avdName);
    ...
    if (ranchu == RANCHU_AUTODETECT) {
        ...
        if (cpuHasRanchu) {
            if (cpuHasGoldfish) {
                ...
                if (checkAvdKernelForRanchuSupport(avdName)) {
...
static bool checkAvdKernelForRanchuSupport(const char* avdName) {
    ...
    asprintf(&kernel_file, "%s/%s", sdkRootPath, "kernel-ranchu");
    result = path_is_regular(kernel_file);
