...
if [ -z "$BUILD_DIR" ]; then
    var_create_temp_dir WORK_DIR "build-wine-temp"
else
    WORK_DIR=$BUILD_DIR
fi
...
if [ -d "$SOURCE_DIR" ]; then
    ...
    (cd $SOURCE_DIR && git fetch && git clean -xfd &&
     git reset --hard origin/$WINE_VERSION) ||
    ...
    git -C $SOURCE_DIR checkout origin/$WINE_VERSION ||
            panic "Unknown version $WINE_VERSION."
fi
...
run_schroot $CHROOT_NAME / apt-get install --yes \
...
run sudo chown -R $USER:eng $PREFIX ||
