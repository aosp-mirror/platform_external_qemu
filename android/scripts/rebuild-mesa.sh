...
option_register_var "--install-dir=<dir>" OPT_INSTALL_DIR \
        "Select installation directory [<prebuilts>/mesa]"
...
if [ "$OPT_INSTALL_DIR" ]; then
    INSTALL_PREFIX=$OPT_INSTALL_DIR
    log "Using install dir: $INSTALL_PREFIX"
else
    INSTALL_PREFIX=$PREBUILTS_DIR/mesa
    log "Auto-config: --install-dir=$INSTALL_PREFIX  [default]"
fi
...
build_windows_mesa () {
    ...
    if [ ! -f "$PKG_TIMESTAMP" -o -n "$OPT_FORCE" ]; then
        (
            ...
            export LLVM=$(builder_install_prefix) &&
...
for SYSTEM in $HOST_SYSTEMS; do
    (
        ...
        case $SYSTEM in
            linux-*)
                build_package util-macros

                build_package xproto

                build_package libXau

                # Need to fix the version of xau.pc, or libpthread-stubs
                # will refuse to use it.
                sed -i -e 's|Version: 0.1.1|Version: 1.1.1|g' "$(builder_install_prefix)"/lib/pkgconfig/xau.pc

                case $SYSTEM in
                    windows-*)
                        ;;
                    *)
                        build_package libpthread-stubs
                esac
        ...
        case $SYSTEM in
            windows-*)
                ...
                ;;
            *)
                # Use configure/make build for other platforms.
                build_package MesaLib \
                        --enable-xlib-glx \
                        --enable-opengl \
                        --disable-gles1 \
                        --disable-gles2 \
        ...
        for SUBDIR in include lib; do
            copy_directory \
                    ...
                    "$INSTALL_PREFIX/$SYSTEM/$SUBDIR"
        done

    ) || panic "[$SYSTEM] Could not build $LLVM_PKG_FULLNAME"
