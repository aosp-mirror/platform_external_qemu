...
build_angle_package () {
    ...
    log "ANGLE build: apply patch for linux-aarch64" &&
    ...
    # gn is provided for each platform in the previously cloned depot_tools?
...
build_angle_libraries () {
    local PKG_BUILD_DIR PKG_SRC_TIMESTAMP PKG_TIMESTAMP PKG_LIB_DIR DYN_LIB_SUFFIX
    ...
    if [ ! -f "$PKG_TIMESTAMP" -o -n "$OPT_FORCE" ]; then
        ...
        copy_directory_files \
                    ...
                    libshadertranslator.${DYN_LIB_SUFFIX} \
