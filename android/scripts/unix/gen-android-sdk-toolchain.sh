    ...
    if [ "$CLANG_BINDIR" ]; then
        ...
        case $PROG in
           cc|gcc|clang)
                ...
                ;;
            cl)
                # These are some hacks for cross compiling.
                printf -v EXTRA_ENV_SETUP 'for arg do\n shift \n [ "$arg" = "-fPIC" ] && continue \n set -- "$@" "$arg" \n done'
                DST_PROG=clang
                DST_PREFIX=$CLANG_BINDIR/
                ;;
            c++|g++|clang++)
...
gen_wrapper_toolchain () {
    ...
    case "$CURRENT_HOST" in
        windows_msvc*)
          local COMPILERS="cc gcc clang c++ cl g++ clang++ cpp ld clang-tidy ar as ranlib strings nm objdump objcopy link lib"
