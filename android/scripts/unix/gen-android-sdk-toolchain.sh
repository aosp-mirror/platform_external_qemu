...
prepare_build_for_darwin() {
    ...
    OSX_SDK_SUPPORTED="${OSX_REQUIRED} 10.15 10.16 11.0 11.1 11.2 11.3 11.4 11.5 11.6"
...
prepare_build_for_darwin_aarch64() {
    ...
    OSX_SDK_SUPPORTED="10.11 10.12 10.13 10.14 10.15 10.16 11.0 11.1 11.2 11.3 11.4 11.5 11.6"
    OSX_SDK_INSTALLED_LIST=$(xcodebuild -showsdks 2>/dev/null | \
...
prepare_build_for_host () {
    ...
    log2 "Building on $BUILD_HOST"
    case $CURRENT_HOST in
        linux-*)
            prepare_build_for_linux_$BUILD_ARCH
            ;;
        darwin-aarch64)
            if [ "$BUILD_HOST"  != "darwin-arm64" ]; then
              OSX_DEPLOYMENT_TARGET=10.16
            ...
            ;;
        darwin-*)
            if [ "$BUILD_HOST"  != "darwin-x86_64" ]; then
