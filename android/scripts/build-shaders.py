...
shader_source_names = {"Etc2Decomp"}
...
for shader_source_name in shader_source_names:
    ...
    os.write(tmp_file, "\n".join(shader_sources))
    os.close(tmp_file)
    os.system(" ".join([glslangValidator_path, "--spirv-val", "-V", tmp_file_name]))
    shutil.move("comp.spv", os.path.join(shader_source_path, shader_source_name + ".spv"))
