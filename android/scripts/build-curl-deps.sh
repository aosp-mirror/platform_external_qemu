...
PROGRAM_DESCRIPTION=\
"Build curl dependencies (openssl, zlib) for Linux, Windows and Darwin."

package_builder_register_options

aosp_dir_register_option
prebuilts_dir_register_option
install_dir_register_option curl-deps

# Disable ccache for this script; build failures have been observed otherwise
if [[ "$@" =~ "--no-ccache" ]]; then
    log "ccache disabled through script options"
    option_parse "$@"
else
    if [[ "$@" =~ "--ccache=" ]]; then
        panic "The \"--ccache\" option is not valid for this script."
    fi
    log "Disabling ccache (necessary to build openssl cleanly)"
    option_parse "$@" "--no-ccache"  
fi
...
package_builder_process_options curl-deps
...
# Build a package openssl
#
# $1: Source directory.
# $2: Build directory.
# $3+: Additional configure options.
build_openssl_package () {
...
build_package () {
    ...
    local PKG_SRCD_NAME=$(package_list_get_unpack_src_dir $1)
...
do_remote_darwin_build () {
    builder_prepare_remote_darwin_build \
            "/tmp/$USER-rebuild-darwin-ssh-$$/curl-deps-build"
...
if [ -z "$OPT_FORCE" ]; then
    builder_check_all_timestamps "$INSTALL_DIR" curl-deps
fi

if [ "$DARWIN_SSH" -a "$DARWIN_SYSTEMS" ]; then
    # Perform remote Darwin build first.
    dump "Remote openssl build for: $DARWIN_SYSTEMS"
