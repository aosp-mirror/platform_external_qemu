...
# Panic if ccache is used for this script; build failures have been observed otherwise
option_parse "${@}"
if [ "$OPT_CCACHE" ]; then
    panic "You cannot use --ccache with this script (OpenSSL would fail to build)."
fi
OPT_NO_CCACHE=true
...
build_package_openssl () {
    ...
    if [ ! -f "$PKG_BLD_TIMESTAMP" -o -n "$OPT_FORCE" ]; then
      ...
      local PKG_FULLNAME="$(basename "${PKGS_SRC_DIR}")"
