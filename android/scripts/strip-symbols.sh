...
case $(get_build_os) in
  linux)
      ROOT=$(find . -maxdepth 1 -type f -executable)
      LIBS=$(find lib64 -type f -executable)
      QEMU=$(find qemu -type f -executable)
      EXES="$ROOT $LIBS $QEMU"
      OPT_HOST=linux-x86_64
      ;;
  darwin)
      ROOT=$(find . -maxdepth 1 -type f -perm +111)
      LIBS=$(find lib64 -type f -perm +111)
      QEMU=$(find qemu -type f -perm +111)
      EXES="$ROOT $LIBS $QEMU"
