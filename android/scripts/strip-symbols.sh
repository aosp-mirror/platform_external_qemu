...
build_symbols () {
    ...
    if [ "$(get_build_os)" = "darwin" ]; then
        ...
        log2 dsymutil --out=$DSYM_BIN_DIR $BINARY
        log2 $SYMTOOL -g $DSYM_BIN_DIR $BINARY > $SYMBOL_DIR/$BINARY.sym 2> $SYMBOL_DIR/$BINARY.err

        dsymutil --out=$DSYM_BIN_DIR $BINARY 2>&1 | grep -q 'no debug symbols' ||
            strip -S $DEST_ROOT/$LIB || panic "Could not strip $DEST_ROOT" &&
            $SYMTOOL -g $DSYM_BIN_DIR $BINARY > $SYMBOL_DIR/$BINARY.sym 2> $SYMBOL_DIR/$BINARY.err ||
...
process_dir() {
    ...
    $FIND_CMD | while IFS= read -r -d $'\0' line; do
