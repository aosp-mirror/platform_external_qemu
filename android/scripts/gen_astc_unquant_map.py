...
def getUnquantizedTritValue(val, numBits):
  bitRange = 1 << numBits
  trit = val // bitRange
  bits = val % bitRange
...
def genWeightTable(tritOrQuint, maxBits):
  tritOrQuintStr = "Trit" if tritOrQuint else "Quint"
  ret = "const uint kUnquant%sWeightMapBitIdx[%d] = {\n" \
      % (tritOrQuintStr, maxBits + 1)
  ret += " " * 3
  totalEntries = 0
  ...
  ret += "\n};\n\n";
  ret += "const uint kUnquant%sWeightMap[%d] = {\n" \
      % (tritOrQuintStr, totalEntries)
  for i in range(0, maxBits + 1):
    ret += " " * 3
...
def genAllTables():
  ret = genWeightTable(True, 3)
  ret += genWeightTable(False, 2)
  return ret
