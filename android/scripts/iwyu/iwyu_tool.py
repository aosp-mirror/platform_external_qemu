...
def is_subpath_of(path, pattern):
    """ Return True if regex pattern matches path.

    Assumes path is canonicalized with os.path.realpath.
    """
    path = normcase(path)
...
def parse_compilation_db(compilation_db_path):
    ...
    for entry in compilation_db:
        # Skip objective-c we cannot handle those
        if entry['file'].endswith('.m'):
            continue
...
def _bootstrap(sys_argv):
    ...
    parser.add_argument('source', nargs='*',
                        help=('Zero or more regular expressions to '
                              'run IWYU on. Defaults to all in compilation '
                              'database.'))
    parser.add_argument('-cflags',
                        help='Additional c-flags passed on to clang', dest='cflags')
    ...
    if args.cflags:
        extra_args += args.cflags.replace('-L','-I').split(' ')
