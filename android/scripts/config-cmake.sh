...
option_register_var "--mingw" OPTION_MING "Build Windows executable on Linux using mingw"
...
if [ "$OPTION_SDK_REVISION" ]; then
    CMAKE_PARAMS+=" -DOPTION_SDK_TOOLS_REVISION=$OPTION_SDK_REVISION"
fi;
if [ "$OPTION_SDK_BUILD" ]; then
    CMAKE_PARAMS+=" -DOPTION_SDK_TOOLS_BUILD_NUMBER=$OPTION_SDK_BUILD"
fi;

if [ "$OPTION_SANITIZER" != "none" ] ; then
   CMAKE_PARAMS+=" -DOPTION_ASAN=$OPTION_SANITIZER"
fi
...
echo "Ready to go. Type ${GREEN}'cd ${OPTION_OUT_DIR} && ${OPTION_GENERATOR}'${RESET} to build emulator, and ${GREEN}'${OPTION_GENERATOR} tests'${RESET} to run the unit tests."
