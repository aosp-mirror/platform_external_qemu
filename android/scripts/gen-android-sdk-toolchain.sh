...
# Configure the darwing toolchain.
...
prepare_build_for_linux() {
    ...
    EXTRA_CXXFLAGS="-m64"
    var_append EXTRA_CXXFLAGS "--gcc-toolchain=$PREBUILT_TOOLCHAIN_DIR"
    var_append EXTRA_CXXFLAGS "--sysroot=${SYSROOT}"
    var_append EXTRA_CXXFLAGS "-stdlib=libc++"
    if [ "$OPT_CXX11" ]; then
        var_append EXTRA_CXXFLAGS "-std=c++14" "-Werror=c++14-compat"
    fi
    ...
    # Note that this can result in warnings of unused flggs if we are not
    # linking and merely translating a .cpp file
    var_append EXTRA_CXXFLAGS "-fuse-ld=${PREBUILT_TOOLCHAIN_DIR}/bin/x86_64-linux-ld"
    var_append EXTRA_CXXFLAGS "-B$PREBUILT_TOOLCHAIN_DIR/lib/gcc/x86_64-linux/4.8/"
    var_append EXTRA_CXXFLAGS "-L$PREBUILT_TOOLCHAIN_DIR/lib/gcc/x86_64-linux/4.8/"
    var_append EXTRA_CXXFLAGS "-L$PREBUILT_TOOLCHAIN_DIR/x86_64-linux/lib64/"
    var_append EXTRA_CXXFLAGS "-L${CLANG_BINDIR}/../lib64/clang/${CLANG_VERSION}/lib/linux/"
    var_append EXTRA_CXXFLAGS "-L${CLANG_BINDIR}/../lib64/"

    if [ $(get_verbosity) -gt 3 ]; then
      # This will get pretty crazy, but useful if you want to debug linker issues.
      var_append EXTRA_CXXFLAGS "-Wl,-verbose"
      var_append EXTRA_CXXFLAGS "-v"
    fi
...
# Prints info based on the passed in parametr
