...
fetch_dependencies_msvc() {
    ...
    MSVC_DIR=${INSTALL_DIR}/msvc
    CIOPFS_DIR=${INSTALL_DIR}/.ciopfs
    mkdir -p $MSVC_DIR
    mkdir -p $CIOPFS_DIR
    CPIOFS="${AOSP_DIR}/prebuilts/android-emulator-build/common/ciopfs/linux-x86_64/ciopfs"
    run $CPIOFS -o use_ino $CIOPFS_DIR $MSVC_DIR

    local DEPOT_TOOLS=$(aosp_depot_tools_dir)
    local MSVC_VER=$(aosp_msvc_version)
    log "Looking to get tools version: ${MSVC_VER}"
    run ${DEPOT_TOOLS}/win_toolchain/get_toolchain_if_necessary.py --force --toolchain-dir=$MSVC_DIR $MSVC_VER 
}
...
prepare_build_for_windows_msvc() {
    ...
    if [ "$OPT_VSDIR" ]; then
      ...
      MSVC_VER=$(aosp_msvc_version)
      MSVC_DIR=$INSTALL_DIR/msvc/vs_files/${MSVC_VER}/
    fi
    ...
    var_append EXTRA_CFLAGS "-isystem $MSVC_DIR/win_sdk/include/${MSVC_VER}/hypervisor"
    ...
    var_append EXTRA_CXXFLAGS "-isystem $MSVC_DIR/win_sdk/include/${MSVC_VER}/hypervisor"
