...
EOF
    ...
    run mv "$INSTALL_DIR/$SYSTEM/include" "$INSTALL_DIR/common/include.new"

    directory_atomic_update \
            "$INSTALL_DIR"/common/include \
            "$INSTALL_DIR"/common/include.new

    (cd "$INSTALL_DIR/$SYSTEM" && ln -f -s ../common/include include)
}
...
for SYSTEM in $LOCAL_HOST_SYSTEMS; do
    (
        ...
        QT_STATIC_LIBS=$(cd "$(builder_install_prefix)" && find . -name "*.a" 2>/dev/null)
        if [ -z "$QT_STATIC_LIBS" ]; then
            panic "Could not find any Qt static library!!"
        fi
