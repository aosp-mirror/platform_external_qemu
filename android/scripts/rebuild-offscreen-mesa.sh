...
PROGRAM_DESCRIPTION=\
"A script used to rebuild the Offscreen Mesa graphics library from sources."
...
case $(get_build_os) in
    linux-*)
        DEFAULT_HOST_SYSTEMS="linux-x86_64,windows-x86_64"
        ;;
esac
...
AOSP_DIR=$PARAMETER_3
if [ ! -d "$AOSP_DIR"/prebuilts/gcc ]; then
    panic "Not a valid AOSP checkout directory: $AOSP_DIR"
fi

package_builder_process_options osmesa
