...
# Return the SHA-1 of a given file
# $1: file path.
get_file_sha1 () {
    sha1sum "$1" | cut -d" " -f1
}
...
download_package () {
    ...
    if [ "$SHA1SUM" ]; then
        ...
        REAL_SHA1SUM=$(get_file_sha1 "$DST_DIR"/$PKG_NAME)
...
library dependencies (e.g. libffi, libz, glib, etc..) and place them
in '<archive-dir>', to be used by other scripts to rebuild them.

If a package file is already in <archive-dir>, the script checks its SHA-1
to avoid re-downloading it. You can however use --force to always download."

PROGRAM_PARAMETERS="<archive-dir>"
...
if [ "$PARAMETER_COUNT" != "1" ]; then
    panic "This script requires one argument! See --help."
fi
...
get_package () {
    ...
    if [ "$PKG_URL" ]; then
        ...
        if [ -f "$PKG_FILE" -a -n "$PKG_SHA1" ]; then
            ...
            ARCHIVE_SHA1=$(get_file_sha1 "$PKG_FILE")
...
package_list_parse "$PACKAGE_LIST" get_package
