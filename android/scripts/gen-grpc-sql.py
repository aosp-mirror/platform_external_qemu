...
"""A very simplistic script to extract the crc32 values that are collected
   ...
   them out, so the simple parse will still pick them up and you don't end up
   with missing/expired data.

   For example:

   // The following methods have been deleted on the given date
   // rpc deprectedCommentedOut (03/12/11)
"""
import argparse
import datetime
import re
...
class ServiceParser(object):
    ...
    def __init__(self, grpc_proto_file, expired=360):
        ...
        self.package, self.service, discovered = self._parse_grpc_file(
        ...
        with open(fname, "r") as pfile:
            for line in pfile.readlines():
                ...
                if m:
                    ...
                    logging.info("Considering deleted entry %s, %s", m.group(1), date)
                    if date > keep_after_date:
                        discovered.append(m.group(1))

        return package, service, discovered
