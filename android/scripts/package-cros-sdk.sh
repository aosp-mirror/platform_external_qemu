...
function pack_for_os() {
  ...
  mkdir -p system-images/chromeos-m"${CROS_MILESTONE}"/google_apis
  ln "${TMP_SDK_DIR}/x86" system-images/chromeos-m"${CROS_MILESTONE}"/google_apis -sf
  # By default, zip derefrences symlink.
  zip -r - emulator system-images >  "${PACKAGE_ROOT_DIR}/cros_sdk_for_${platform}_${VERSION}.zip.tmp"
  cd "${PACKAGE_ROOT_DIR}"
