...
if [ -f build.ninja ]; then
    echo ninja test || FAILURES="$FAILURES unittests"
else
    echo run make test -j$HOST_NUM_CPUS || FAILURES="$FAILURES unittests"
fi
cd ..
...
if [ "$HOST_OS" = "linux" ]; then
   ...
   if [ -d $OPT_OUT/gradle-release ]; then
        ...
        find $OPT_OUT/gradle-release \( -type f -and \( -executable -or -name '*.so.*' \) \) -print0 | while read -d $'\0' file; do
