...
def cleanup_json(data):
    ...
    if (isinstance(data, list)):
        ...
        for elem in data:
            ...
            if (rem is not None and rem not in copy):
                copy.append(rem)
...
class Module:
    ...
    def __init__(self, js, parent):
        ...
        self.libs = [l for l in js.as_list('LOCAL_LDLIBS') if
                     l.startswith('-') and not l.endswith('a')]
        self.libs += [os.path.splitext(os.path.basename(l))[0] for
                      l in js.as_list('LOCAL_LDLIBS') if l.endswith('a')]
...
class ModuleCollection:
    ...
    def to_project(self):
        ...
        target += 'SET(CMAKE_CXX_FLAGS -Wno-reserved-user-defined-literal)\n'
        target += 'SET(HOST "{}")\n'.format(self.host)
