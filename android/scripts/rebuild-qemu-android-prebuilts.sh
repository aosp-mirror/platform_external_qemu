...
OPT_SYSTEM=
option_register_var "--system=<list>" OPT_SYSTEM "List of target systems [$DEFAULT_SYSTEMS]"

PROGRAM_PARAMETERS="<install-dir> <archive-dir> <aosp-dir>"

PROGRAM_DESCRIPTION=\
"Rebuild qemu-android prebuilt libraries from scratch. This requires three
directory paths:

  <install-dir>
      Prefix path where the final binaries will be installed. The script
      will populate the 'include' and 'lib' subdirectories appropriately.

  <archive-dir>
      Directory containing the tarballs of the prebuilt sources needed
      for the build. This is typically populated by calling
      $(program_directory)/download-prebuilts-sources.sh before calling
      this script.
...
if [ "$PARAMETER_COUNT" != 3 ]; then
    panic "This script requires three arguments, see --help for details."
fi

INSTALL_PREFIX=$PARAMETER_1

ARCHIVE_DIR=$PARAMETER_2
if [ ! -d "$ARCHIVE_DIR" ]; then
    panic "Missing source archive directory: $ARCHIVE_DIR"
fi
...
get_source_package_version () {
    package_list_get_package_version "$PACKAGE_LIST" "$1"
}
...
get_source_package_name () {
    package_list_get_package_filename "$PACKAGE_LIST" "$1"
}
...
EOF
    ...
    dump "$CURRENT_TEXT Copying prebuilts to $INSTALL_PREFIX/$CURRENT_HOST"

    BINARY_DIR=$INSTALL_PREFIX/$CURRENT_HOST
    ...
    for SUBDIR in include lib lib32 lib64 bin; do
        ...
        run cp -rp "$PREFIX/$SUBDIR" "$BINARY_DIR/$SUBDIR"
    done
...
\$PROGDIR/scripts/$(program_name) \\
    ...
    $EXTRA_FLAGS \\
    /tmp/$PKG_SUFFIX/install-prefix \\
    /tmp/$PKG_SUFFIX/archive \\
    /tmp/$PKG_SUFFIX/aosp
EOF
    ...
    local BINARY_DIR=$INSTALL_PREFIX
    run mkdir -p "$BINARY_DIR" ||
    panic "Could not create final directory: $BINARY_DIR"
