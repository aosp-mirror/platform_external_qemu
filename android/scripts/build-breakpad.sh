...
PROGRAM_DESCRIPTION=\
"Build Google Breakpad client library and host tools."

package_builder_register_options

aosp_dir_register_option
prebuilts_dir_register_option
install_dir_register_option google-breakpad
...
package_builder_process_options google-breakpad
...
# $1: Package name (e.g. 'libpthread-stubs')
...
if [ "$DARWIN_SSH" -a "$DARWIN_SYSTEMS" ]; then
    ...
    dump "Remote google-breakpad build for: $DARWIN_SYSTEMS"
...
for SYSTEM in $LOCAL_HOST_SYSTEMS; do
    (
        ...
        case $SYSTEM in
            windows*)
                BREAKPAD_CONFIGURE_FLAGS="--disable-processor --disable-tools"
                ;;
            *)
                BREAKPAD_CONFIGURE_FLAGS=
                ;;
        esac
        build_package google-breakpad \
                $BREAKPAD_CONFIGURE_FLAGS

        for SUBDIR in bin include lib; do
