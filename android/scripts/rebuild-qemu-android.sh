...
PROGRAM_PARAMETERS="<qemu-android> <prebuilts-dir> <aosp-dir>"
...
This requires three parameters:
  ...
  <qemu-android>
      Location of the qemu-android source tree to compile.

  <prebuilts-dir>
      Installation prefix of all prebuilt dependency libraries
      (libffi, glib, etc) for qemu-android. One can rebuild these
      with the following script:

        $(program_directory)/rebuild-qemu-android-prebuilts.sh script.

  <aosp-dir>
      Top-level checkout of the AOSP source tree, used for its
      prebuilts/gcc/ toolchains on some platforms.
"
...
OPT_INSTALL_DIR=$(program_directory)/../../objs/qemu
option_register_var "--install-dir=<dir>" OPT_INSTALL_DIR "Install binaries to <dir>"

option_parse "$@"

if [ "$PARAMETER_COUNT" != 3 ]; then
    panic "This script takes 3 arguments. See --help for details."
fi
...
PREBUILTS_DIR=$PARAMETER_2

AOSP_SOURCE_DIR=$PARAMETER_3
if [ ! -d "$AOSP_SOURCE_DIR"/prebuilts/gcc ]; then
    panic "Not a valid AOSP checkout directory: $AOSP_DIR"
fi
...
EOF
    ...
    for QEMU_TARGET in $QEMU_TARGETS; do
        ...
        dump "$(builder_text) Copying $QEMU_EXE to binaries/$CURRENT_HOST"
...
do_remote_darwin_build () {
    ...
    run cp -rp "$PREBUILTS_DIR"/darwin-x86_64 \
               "$PKG_DIR/prebuilts/darwin-x86_64"

    local EXTRA_FLAGS=""

    VERBOSITY=$(get_verbosity)
    while [ "$VERBOSITY" -ge "1" ]; do
...
EOF
    ...
    local BINARY_DIR=$OPT_INSTALL_DIR
    run mkdir -p "$BINARY_DIR" ||
    panic "Could not create final directory: $BINARY_DIR"

    for SYSTEM in $DARWIN_SYSTEMS; do
        run scp -r "$DARWIN_SSH":/tmp/$PKG_SUFFIX/install/$SYSTEM $BINARY_DIR/
    done
}
...
for SYSTEM in $HOST_SYSTEMS; do
    if [ ! -d "$PREBUILTS_DIR/$SYSTEM" ]; then
        var_append BAD_PREBUILTS "$PREBUILTS_DIR/$SYSTEM"
    fi
done
if [ "$BAD_PREBUILTS" ]; then
    dump "Missing prebuilts: $BAD_PREBUILTS"
    panic "Please use rebuild-qemu-android-prebuilts.sh first!"
fi
...
for SYSTEM in $HOST_SYSTEMS; do
    ...
    build_qemu_android $SYSTEM "$AOSP_SOURCE_DIR"
done
