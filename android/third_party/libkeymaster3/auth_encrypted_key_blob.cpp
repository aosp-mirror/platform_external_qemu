...
static keymaster_error_t DeserializeUnversionedBlob(const KeymasterKeyBlob& key_blob,
    ...
    if (!nonce->advance_write(OCB_NONCE_LENGTH) ||
...
keymaster_error_t DeserializeAuthEncryptedBlob(const KeymasterKeyBlob& key_blob,
    ...
    if (version != CURRENT_BLOB_VERSION ||  //
        !nonce->Deserialize(buf_ptr, end) ||
        nonce->available_read() != OCB_NONCE_LENGTH ||
        !encrypted_key_material->Deserialize(buf_ptr, end) ||  //
        !tag->Deserialize(buf_ptr, end) ||
        tag->available_read() != OCB_TAG_LENGTH ||
        !hw_enforced->Deserialize(buf_ptr, end) ||  //
        !sw_enforced->Deserialize(buf_ptr, end)) {
        // This blob failed to parse.  Either it's corrupted or it's a blob
        // generated by an earlier version of keymaster using a previous blob
        // format which did not include the version byte or the nonce or tag
        // length fields. So we try to parse it as that previous version.
        //
        // Note that it's not really a problem if we erronously parse a
        // corrupted blob, because decryption will fail the authentication
        // check.
        //
        // A bigger potential problem is: What if a valid unversioned blob
        // appears to parse correctly as a versioned blob?  It would then be
        // rejected during decryption, causing a valid key to become unusable.
        // If this is a disk encryption key, upgrading to a keymaster version
        // with the new format would destroy the user's data.
        //
        // What is the probability that an unversioned key could be successfully
        // parsed as a version 0 key?  The first 12 bytes of an unversioned key
        // are the nonce, which, in the only keymaster version released with
        // unversioned keys, is chosen randomly.  In order for an unversioned
        // key to parse as a version 0 key, the following must be true about the
        // first five of those random bytes:
        //
        // 1.  The first byte must be zero.  This will happen with probability
        // 1/2^8.
        //
        // 2.  The second through fifth bytes must contain an unsigned integer
        // value equal to
        //     NONCE_LENGTH.  This will happen with probability 1/2^32.
        //
        // Based on those two checks alone, the probability of interpreting an
        // unversioned blob as a version 0 blob is 1/2^40.  That's small enough
        // to be negligible, but there are additional checks which lower it
        // further.
        LOG_D("Failed to deserialize versioned key blob.  Assuming "
              "unversioned.",
              0);
        return DeserializeUnversionedBlob(key_blob, encrypted_key_material,
