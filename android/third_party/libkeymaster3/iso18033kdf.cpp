...
bool Iso18033Kdf::GenerateKey(const uint8_t* info, size_t info_len, uint8_t* output,
     ...
     */
    if ((0xFFFFFFFFULL + start_counter_) * digest_size_ <
    ...
    for (size_t block = 0; block < num_blocks; block++) {
        switch (digest_type_) {
            case KM_DIGEST_SHA1:
                if (!EVP_DigestInit_ex(&ctx, EVP_sha1(),
                                       nullptr /* default digest */)) {
                    return false;
                }
                break;
            case KM_DIGEST_SHA_2_256:
                if (!EVP_DigestInit_ex(&ctx, EVP_sha256(),
                                       nullptr /* default digest */)) {
                    return false;
                }
                break;
        ...
        if (!EVP_DigestUpdate(&ctx, secret_key_.get(), secret_key_len_) ||
            !Uint32ToBigEndianByteArray(block + start_counter_,
                                        counter.get()) ||
        ...
        if (!EVP_DigestFinal_ex(&ctx, digest_result.get(),
                                &uint32_digest_size_) ||
