...
keymaster_error_t EcKeyFactory::GenerateKey(const AuthorizationSet& key_description,
    ...
    keymaster_error_t ret = context_->CreateKeyBlob(authorizations, KM_ORIGIN_GENERATED, key_material, key_blob,
                                   hw_enforced, sw_enforced);
    
    //sw_enforced->push_back(TAG_KEY_SIZE, key_size);
    //sw_enforced->push_back(TAG_ALGORITHM, KM_ALGORITHM_EC);
    return ret;
}
...
keymaster_error_t EcKeyFactory::ImportKey(const AuthorizationSet& key_description,
    ...
    keymaster_ec_curve_t ec_curve;
    uint32_t key_size1;
    keymaster_error_t error1 = GetCurveAndSize(authorizations1, &ec_curve, &key_size1);
    if (error1 != KM_ERROR_OK)
        return error1;
