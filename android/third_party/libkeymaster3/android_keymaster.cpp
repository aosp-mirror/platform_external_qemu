...
void AndroidKeymaster::SupportedAlgorithms(const SupportedAlgorithmsRequest& /* request */,
    ...
    const keymaster_algorithm_t* algorithms =
            context_->GetSupportedAlgorithms(&algorithm_count);
...
void GetSupported(const KeymasterContext& context, keymaster_algorithm_t algorithm,
    ...
    const OperationFactory* factory =
            context.GetOperationFactory(algorithm, purpose);
    if (!factory) {
...
void AndroidKeymaster::SupportedImportFormats(const SupportedImportFormatsRequest& request,
    ...
    if (response == NULL ||
        !check_supported(*context_, request.algorithm, response)) {
        return;
    }
...
void AndroidKeymaster::GetKeyCharacteristics(const GetKeyCharacteristicsRequest& request,
    ...
    response->error = context_->ParseKeyBlob(
            KeymasterKeyBlob(request.key_blob), request.additional_params,
            &key_material, &response->enforced, &response->unenforced);
    if (response->error != KM_ERROR_OK) {
        return;
    }

    response->error = CheckVersionInfo(response->enforced, response->unenforced,
                                       *context_);
}
...
void AndroidKeymaster::BeginOperation(const BeginOperationRequest& request,
    ...
    UniquePtr<Key> key;
    response->error = LoadKey(request.key_blob, request.additional_params,
                              &hw_enforced, &sw_enforced, &key_factory, &key);
    if (response->error != KM_ERROR_OK) {
        return;
    }
    ...
    OperationFactory* factory =
            key_factory->GetOperationFactory(request.purpose);
    if (!factory) {
        return;
    }

    UniquePtr<Operation> operation(factory->CreateOperation(
            *key, request.additional_params, &response->error));
    ...
    if (context_->enforcement_policy()) {
        km_id_t key_id;
        response->error = KM_ERROR_UNKNOWN_ERROR;
        if (!context_->enforcement_policy()->CreateKeyId(request.key_blob,
                                                         &key_id)) {
            return;
        }
        operation->set_key_id(key_id);
        response->error = context_->enforcement_policy()->AuthorizeOperation(
                request.purpose, key_id, key->authorizations(),
                request.additional_params, 0 /* op_handle */,
...
void AndroidKeymaster::UpdateOperation(const UpdateOperationRequest& request,
    ...
    if (context_->enforcement_policy()) {
        response->error = context_->enforcement_policy()->AuthorizeOperation(
                operation->purpose(), operation->key_id(),
                operation->authorizations(), request.additional_params,
                request.op_handle, false /* is_begin_operation */);
        if (response->error != KM_ERROR_OK) {
...
void AndroidKeymaster::ExportKey(const ExportKeyRequest& request, ExportKeyResponse* response) {
    ...
    keymaster_algorithm_t algorithm;
    KeyFactory* key_factory = GetKeyFactory(*context_, hw_enforced, sw_enforced,
                                            &algorithm, &response->error);
    if (!key_factory) {
        return;
    }
    ...
    UniquePtr<Key> key;
    response->error =
            key_factory->LoadKey(key_material, request.additional_params,
                                 hw_enforced, sw_enforced, &key);
    if (response->error != KM_ERROR_OK) {
        return;
    }
    ...
    size_t size;
    response->error =
            key->formatted_key_material(request.key_format, &out_key, &size);
    if (response->error == KM_ERROR_OK) {
...
void AndroidKeymaster::AttestKey(const AttestKeyRequest& request, AttestKeyResponse* response) {
    ...
    response->error = LoadKey(request.key_blob, request.attest_params,
                              &tee_enforced, &sw_enforced, &key_factory, &key);
    if (response->error != KM_ERROR_OK) {
        return;
    }
    ...
    if (request.attest_params.GetTagValue(TAG_ATTESTATION_APPLICATION_ID,
        ...
        sw_enforced.push_back(TAG_ATTESTATION_APPLICATION_ID,
                              attestation_application_id);
    }
...
void AndroidKeymaster::UpgradeKey(const UpgradeKeyRequest& request, UpgradeKeyResponse* response) {
    ...
    response->error =
            context_->UpgradeKeyBlob(KeymasterKeyBlob(request.key_blob),
...
void AndroidKeymaster::ImportKey(const ImportKeyRequest& request, ImportKeyResponse* response) {
    ...
    if (!request.key_description.GetTagValue(TAG_ALGORITHM, &algorithm) ||
        ...
        keymaster_key_blob_t key_material = {request.key_data,
                                             request.key_data_length};
        KeymasterKeyBlob key_blob;
        response->error =
                factory->ImportKey(request.key_description, request.key_format,
...
void AndroidKeymaster::Configure(const ConfigureRequest& request, ConfigureResponse* response) {
    ...
    response->error = context_->SetSystemVersion(request.os_version,
                                                 request.os_patchlevel);
}
