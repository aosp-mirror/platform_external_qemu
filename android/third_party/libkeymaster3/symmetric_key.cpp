...
keymaster_error_t SymmetricKeyFactory::GenerateKey(const AuthorizationSet& key_description,
    ...
    key_description.GetTagValue(TAG_ALGORITHM, &myalgorithm);
    hw_enforced->push_back(TAG_ALGORITHM, myalgorithm);
    keymaster_error_t ret = context_->CreateKeyBlob(key_description, KM_ORIGIN_GENERATED, key_material, key_blob,
...
keymaster_error_t SymmetricKeyFactory::ImportKey(const AuthorizationSet& key_description,
    ...
    hw_enforced->push_back(TAG_KEY_SIZE, key_size_bits);
    hw_enforced->push_back(TAG_ALGORITHM, myalgorithm);
    keymaster_block_mode_t mymode;
    authorizations.GetTagValue(TAG_BLOCK_MODE, & mymode);
    hw_enforced->push_back(TAG_BLOCK_MODE, mymode);
    
    keymaster_digest_t mydigest;
    authorizations.GetTagValue(TAG_DIGEST, & mydigest);
    hw_enforced->push_back(TAG_DIGEST, mydigest);
    
    keymaster_padding_t mypad;
    authorizations.GetTagValue(TAG_PADDING, & mypad);
    hw_enforced->push_back(TAG_PADDING, mypad);
    
    hw_enforced->push_back(TAG_ORIGIN, KM_ORIGIN_IMPORTED);
    
    keymaster_error_t ret = context_->CreateKeyBlob(authorizations, KM_ORIGIN_IMPORTED, input_key_material,
