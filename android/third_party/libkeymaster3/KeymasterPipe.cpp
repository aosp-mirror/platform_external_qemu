...
void keymasterDecodeAndExecute(std::vector<uint8_t> &input, std::vector<uint8_t> *output) {
    keymaster_ipc_call(input, output);
}

void registerKeymasterPipeService() {
    android::AndroidPipe::Service::add(new android::AndroidMessagePipe::Service("KeymasterService", keymasterDecodeAndExecute));
}
} // namespace android
