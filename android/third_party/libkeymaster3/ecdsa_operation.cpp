...
keymaster_error_t EcdsaOperation::StoreData(const Buffer& input, size_t* input_consumed) {
    ...
    if (!data_.write(input.peek_read(),
                     min(data_.available_write(), input.available_read()))) {
...
keymaster_error_t EcdsaSignOperation::Finish(const AuthorizationSet& additional_params,
    ...
    size_t siglen;
    if (digest_ == KM_DIGEST_NONE) {
        UniquePtr<EC_KEY, EC_KEY_Delete> ecdsa(
                EVP_PKEY_get1_EC_KEY(ecdsa_key_));
        if (!ecdsa.get()) {
            return TranslateLastOpenSslError();
        }
        ...
        if (!ECDSA_sign(0 /* type -- ignored */, data_.peek_read(),
                        data_.available_read(), output->peek_write(),
                        &siglen_tmp, ecdsa.get())) {
            return TranslateLastOpenSslError();
        }
        siglen = siglen_tmp;
    } else {
        if (EVP_DigestSignFinal(&digest_ctx_, nullptr /* signature */,
                                &siglen) != 1) {
            return TranslateLastOpenSslError();
        }
        ...
        if (EVP_DigestSignFinal(&digest_ctx_, output->peek_write(), &siglen) <=
            0) {
            return TranslateLastOpenSslError();
        }
    }
...
keymaster_error_t EcdsaVerifyOperation::Begin(const AuthorizationSet& /* input_params */,
    ...
    EVP_PKEY_CTX* pkey_ctx;
    if (EVP_DigestVerifyInit(&digest_ctx_, &pkey_ctx, digest_algorithm_,
                             nullptr /* engine */, ecdsa_key_) != 1) {
        return TranslateLastOpenSslError();
    }
    return KM_ERROR_OK;
