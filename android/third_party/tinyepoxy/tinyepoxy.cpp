...
STUB(glFramebufferTexture3DOES)
...
static void GL_APIENTRY glDrawElementsBaseVertex(GLenum mode,
    ...
    void * orig = s_gles2->glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, indices,
    ...
    GLuint bid = 0;
    s_gles2->glGenBuffers(1, &bid);
    s_gles2->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, bid);
    s_gles2->glBufferData(GL_ELEMENT_ARRAY_BUFFER, osize, tmp, GL_STREAM_DRAW);
    free(tmp);
    s_gles2->glDrawElements(mode, count, GL_UNSIGNED_INT, 0);
    s_gles2->glDeleteBuffers(1, &bid);
