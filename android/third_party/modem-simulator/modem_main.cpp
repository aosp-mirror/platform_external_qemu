...
void start_calling_thread(std::string msg) {
    // connect and send message
    auto monitor_sock = cvd::SharedFD::SocketLocalClient(s_host_server_port);
    if (monitor_sock->IsOpen()) {
        DD("making a call from %s to main host server %d", msg.c_str(),
           s_host_server_port);
        std::string ss("REM0");
        ...
        DD("ERROR: cannot making a call from %s to main host server %d",
           msg.c_str(), s_host_server_port);
    }
}
