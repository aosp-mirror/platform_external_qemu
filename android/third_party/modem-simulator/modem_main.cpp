...
void set_data_network_type(int type) {
    ...
    std::string ss("REM0AT+REMOTECTEC: ");
    ss += ctec;
    ss += "\r";
    mymsg.sdata = ss;
    s_msg_channel.send(mymsg);
...
void set_signal_strength_profile(int quality) {
    ...
    std::string ss("REM0AT+REMOTESIGNAL: ");
...
void start_calling_thread(std::string ss) {
    ...
    auto monitor_sock = cvd::SharedFD::SocketLocalClient(s_host_server_port);
    if (monitor_sock->IsOpen()) {
        DD("making a connection to main host server %d", s_host_server_port);
        monitor_sock->Write(ss.data(), ss.size());
        cvd::WriteAll(monitor_sock, ss);
        std::string ss;
        while (true) {
            std::vector<char> buffer(1024);
            auto bytes_read = monitor_sock->Read(buffer.data(), buffer.size());
            if (bytes_read <= 0) {
...
void process_msgs() {
    while (true) {
        ...
        if (msg.type == MODEM_MSG_CALL) {
            DD("incoming call from %s", msg.sdata.c_str());
            // channel_monitors[0]->SendUnsolicitedCommand(msg.sdata);
            // start_calling_thread(msg.sdata);
            std::thread t1(&start_calling_thread, msg.sdata);
            t1.detach();
        }
        if (msg.type == MODEM_MSG_CTEC) {
            DD("sending %s", msg.sdata.c_str());
            std::thread t1(&start_calling_thread, msg.sdata);
            t1.detach();
        }
        if (msg.type == MODEM_MSG_SIGNAL) {
            DD("sending signal strength %s", msg.sdata.c_str());
            std::thread t1(&start_calling_thread, msg.sdata);
            t1.detach();
        }
    }
