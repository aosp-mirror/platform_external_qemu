...
static cvd::SharedFD s_one_shot_monitor_sock;
static cvd::SharedFD s_current_call_monitor_sock;

void start_calling_thread(std::string ss) {
    // connect and send message
    s_current_call_monitor_sock = cvd::SharedFD::SocketLocalClient(s_host_server_port);
    if (s_current_call_monitor_sock->IsOpen()) {
        ...
        while (true) {
            ...
            auto bytes_read = s_current_call_monitor_sock->Read(buffer.data(), buffer.size());
            if (bytes_read <= 0) {
        ...
        DD("ERROR: cannot making a connection to main host server %d", s_host_server_port);
    }
    s_current_call_monitor_sock = cvd::SharedFD();
...
void process_msgs() {
    while (true) {
        ...
        if (msg.type == MODEM_MSG_SMS) {
            ...
            if (s_current_call_monitor_sock->IsOpen()) {
                s_current_call_monitor_sock->Write(msg.sdata.data(), msg.sdata.size());
            }
        } else if (msg.type == MODEM_MSG_CTEC
                || msg.type == MODEM_MSG_SIGNAL || msg.type == MODEM_MSG_DATA_REG
                || msg.type == MODEM_MSG_VOICE_REG) {
            if (!s_one_shot_monitor_sock->IsOpen()) {
                s_one_shot_monitor_sock = cvd::SharedFD::SocketLocalClient(s_host_server_port);
                if (s_one_shot_monitor_sock->IsOpen()) {
            ...
            if (s_one_shot_monitor_sock->IsOpen()) {
                s_one_shot_monitor_sock->Write(msg.sdata.data(), msg.sdata.size());
            }
        }
    }
}
...
void start_android_modem_simulator(const std::string& modem_port_string,
    ...
    s_host_server_port = start_android_modem_host_server(phone_number_string);
...
int stop_android_modem_simulator() {
    if (s_host_server_port > 0 && !modem_host_servers.empty()) {
        ...
        // send STOP to it
        auto monitor_sock =
                cvd::SharedFD::SocketLocalClient(s_host_server_port);
        std::string msg("STOP");
...
int start_android_modem_simulator_detached(std::string guest_port) {
    ...
    int actual_guest_server_port = start_android_modem_guest_server(guest_port);
...
void main_host_thread() {
    ...
    DD("started main host server at %d", s_host_server_port);
    while (true) {
