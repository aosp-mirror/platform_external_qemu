...
int gPollSelect(SharedFDSet& read_set,
           struct timeval* timeout) {

    std::vector<GPollFD> myfds;
    std::map<int, SharedFD> myreadmap;
    for (const auto& iter : read_set) {
        myfds.emplace_back(GPollFD{iter->getFd(), G_IO_IN, 0});
        myreadmap[iter->getFd()] = iter;
    }
    ...
    read_set.Zero();
    fprintf(stderr, "calling %s with nfds %d\n", __func__, myfds.size());
    int ready = g_poll(myfds.data(), (int)(myfds.size()), mytimeout);
    fprintf(stderr, "done calling %s with nfds %d\n", __func__, myfds.size());
    if (ready > 0) {
        for (const auto& iter : myfds) {
            switch (iter.revents) {
                case G_IO_IN:
...
int WindowsPollFDs(SharedFDSet& read_set,
    ...
    int numFdReady = 0;
    read_set.Zero();
    if (ret >= WAIT_OBJECT_0 && ret < myevents.size()) {
        // setup the fd sets
...
int Select(SharedFDSet* read_set,
    ...
    if (write_set || error_set) {
        // just ignore them
        fprintf(stderr, "WARNING: %s %d ignore write and error\n", __func__,
                __LINE__);
    }
    ...
    return gPollSelect(*read_set, timeout);
