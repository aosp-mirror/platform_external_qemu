...
SharedFD SharedFD::SocketLocalClient(const std::string& name, bool abstract,
                                     int in_type) {
    // this is a bit tricky, as on windows, the unix pipe does not work
    // so we assume the 4 digit port number just work
    const std::string prefix = "modem_simulator";
    if (name.size() != prefix.size() + 4 || name.find(prefix) != 0 || in_type != SOCK_STREAM || !abstract) {
        return SharedFD();
    }
