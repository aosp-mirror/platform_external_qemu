...
/**
  ...
  void ble_npl_callout_init(struct ble_npl_callout *cf, struct ble_npl_eventq
  *evq, ble_npl_event_fn *ev_cb, void *ev_arg); int ble_npl_callout_reset(struct
  ble_npl_callout *, int32_t); int ble_npl_callout_queued(struct ble_npl_callout
  *c); void ble_npl_callout_stop(struct ble_npl_callout *c);
*/
 ...
 */
int test_init(void) {
    ble_npl_callout_init(&s_callout, &s_eventq, on_callout, &s_callout_args);
    return PASS;
}
...
int main(void) {
    // Make sure we have a looper..
    ThreadLooper::get();
    ble_npl_eventq_init(&s_eventq);

    SuccessOrQuit(ble_npl_task_init(&s_task, "s_task", test_task_run, NULL, 1,
                                    0, NULL, 0),
                  "task: error initializing");

    // Push the thread looper.
    while (1) {
