...
/**
  ...
  struct ble_npl_event *ble_npl_eventq_poll(struct ble_npl_eventq **, int,
  ble_npl_time_t); void ble_npl_eventq_remove(struct ble_npl_eventq *, struct
  ble_npl_event *); struct ble_npl_eventq *ble_npl_eventq_dflt_get(void);
*/
...
int test_init(void) {
    // VerifyOrQuit(!ble_npl_eventq_inited(&s_eventq), "eventq: empty q
    // initialized");
    ble_npl_eventq_init(&s_eventq);
    // VerifyOrQuit(ble_npl_eventq_inited(&s_eventq), "eventq: not
    // initialized");
...
int test_get(void) {
    struct ble_npl_event* ev =
            ble_npl_eventq_get(&s_eventq, BLE_NPL_TIME_FOREVER);
...
int main(void) {
    SuccessOrQuit(ble_npl_task_init(&s_task_runner, "task_test_runner",
