...
public:
    ...
    virtual protozero::ContiguousMemoryRange GetNewBuffer() {
        ...
        if (mWritingPacket) {
            ...
            return protozero::ContiguousMemoryRange{mWriter.write_ptr(), mTraceBuffer + (mTraceBufferSize - writtenThisTime) };
        } else {
...
private:
    ...
    inline uint64_t getTimestamp() {
        ...
        if (CC_UNLIKELY(sTraceConfig.needToGetGuestTimeDiff)) {
            if (sTraceConfig.useGuestStartTime) {
                fprintf(stderr, "%s: guest time: %llu host time: %llu\n", __func__,
                        (unsigned long long)(sTraceConfig.guestStartTime),
                        (unsigned long long)(t));
                sTraceConfig.guestTimeDiff = sTraceConfig.guestStartTime - t;
                fprintf(stderr, "%s: adusted time from %llu to %llu\n", __func__,
                        (unsigned long long)(t),
                        (unsigned long long)(t + sTraceConfig.guestTimeDiff));
            }
            sTraceConfig.needToGetGuestTimeDiff = false;
        }
...
void TraceStorage::saveTracesToDisk() {
    FILE* fh = fopen(sTraceConfig.filename, "wb");
    ...
    for (const auto& info : mSavedTraces) {
        if (info.first) {
            fwrite(info.data, info.written, 1, fh);
        }
    }
...
void enableTracing() {
    ...
    sTraceConfig.needToGetGuestTimeDiff = true;
