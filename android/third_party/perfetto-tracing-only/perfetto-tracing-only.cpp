...
public:
    ChunkStore(uint8_t* buffer, size_t size) : mBuffer(buffer), mSize(size), mAbi(mBuffer, mSize, 4096) {
        mHeader.chunk_id = 0;
        mHeader.writer_id = 0;
        ::perfetto::SharedMemoryABI::ChunkHeader::Packets packets;
        packets.count = 1;
        packets.flags = 0;
        mHeader.packets.store(packets, std::memory_order_relaxed);

        if (!mAbi.TryPartitionPage(0, ::perfetto::SharedMemoryABI::PageLayout::kPageDiv1)) {
    ...
    ::perfetto::SharedMemoryABI::Chunk GetNewChunk() {
        ++mHeader.chunk_id;
        ::perfetto::SharedMemoryABI::Chunk res = mAbi.TryAcquireChunkForWriting(0, 0, &mHeader);
        if (!res.is_valid()) {
    ...
    void flush(::perfetto::SharedMemoryABI::Chunk&& chunk) {
...
private:
    ...
    uint8_t* mBuffer;
    size_t mSize;
    ::perfetto::SharedMemoryABI mAbi;
    ::perfetto::SharedMemoryABI::ChunkHeader mHeader;
};
...
public:
    ...
    void forEach(std::function<void(TraceContext*)> func) {
        std::lock_guard<std::mutex> lock(mContextsLock);
...
public:
    ...
    virtual ~TraceContext() {
        sTraceContextStorage.remove(this);
    }
...
private: 
    ...
    void beginPacket() {
        ...
        size_t neededSpace = ::perfetto::SharedMemoryABI::kPacketHeaderSize + 4 + kTrackEventPadding;
        ...
        uint8_t* header = mWriter.ReserveBytes(::perfetto::SharedMemoryABI::kPacketHeaderSize);
        memset(header, 0, ::perfetto::SharedMemoryABI::kPacketHeaderSize);
    ...
    ::perfetto::SharedMemoryABI::Chunk mChunk;
...
void enableTracing() {
    ...
    sTraceConfig.hostStartTime = (uint64_t)(::perfetto::base::GetWallTimeNs().count());
}

void disableTracing() {
    if (!sTraceConfig.tracingDisabled) {
        sTraceContextStorage.forEach([](TraceContext* context) {
            context->finishAndRefresh();
        });
    }
