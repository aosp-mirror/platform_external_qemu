...
static void sProcessOneTracePacket(const char* packetBegin, size_t innerSize, std::vector<char>& postProcessed) {
    size_t begin = postProcessed.size();

    // Each trace packet consists of:
    // ..a preamble...
    size_t preambleBytes = 1;
    size_t sizeFieldBytes = varIntEncodingSize(innerSize);
    size_t additionalBytesTotal = preambleBytes + sizeFieldBytes + innerSize;

    postProcessed.resize(begin + additionalBytesTotal);

    char* dst = postProcessed.data() + begin;

    *dst = 0x0a;
    ++dst;

    dst = (char*)writeVarInt(innerSize, (uint8_t*)dst);

    memcpy(dst, packetBegin, innerSize);
}
...
static std::vector<char> sProcessTrace(const std::vector<char>& trace) {
    ...
    do {
        ...
        sProcessOneTracePacket(current, field_size, res);

        bytesUsed += field_size;
        current += field_size;
    } while (true);

    fprintf(stderr, "%s: Total trace packet mb: %f\n", __func__, ((float)bytesUsed) / 1048576.0f);
    return res;
}
...
PERFETTO_TRACING_ONLY_EXPORT void disableTracing() {
    if (sTracingSession) {
        ...
        fprintf(stderr, "%s: processed size: %zu orig: %zu\n", __func__, processed.size(), trace_data.size());
