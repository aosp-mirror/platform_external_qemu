...
#   define CC_UNLIKELY( exp )  (__builtin_expect( !!(exp), 0 ))
    ...
    void beginTrace(const char* name) {
        if (CC_UNLIKELY(sTracingEnabled)) {
            ...
            uint32_t mCurrentCategoryIid = internCategory(kCategory, &needEmitCategoryIntern);

            bool needEmitEventIntern = false;
            uint32_t mCurrentEventNameIid = internEvent(name, &needEmitEventIntern);
            ...
            if (CC_UNLIKELY(needEmitCategoryIntern)) {
                ...
                category->set_iid(mCurrentCategoryIid);
            ...
            if (CC_UNLIKELY(needEmitEventIntern)) {
                ...
                eventname->set_iid(mCurrentEventNameIid);
            ...
            trackevent->add_category_iids(mCurrentCategoryIid);
            trackevent->set_name_iid(mCurrentEventNameIid);
...
private: 
    ...
    uint32_t mCurrentCategoryIid = 0;
    uint32_t mCurrentEventNameIid = 0;
