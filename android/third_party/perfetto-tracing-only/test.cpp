...
void setPreambleAndPacketHeader(protozero::RootMessage<protos::pbzero::TracePacket>* packet, protozero::ScatteredStreamWriter* writer) {
    // Write the preamble
...
void hello() {
    ...
    packet->Reset(&writer);
    setPreambleAndPacketHeader(packet, &writer);
    ...
    // packet->Reset(&writer); setPreambleAndPacketHeader(packet, &writer);

    // packet->set_timestamp(23);
