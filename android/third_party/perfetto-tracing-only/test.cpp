...
public:
    TestChunkStore(uint8_t* buffer, size_t size) : mBuffer(buffer), mSize(size), mAbi(mBuffer, mSize, 4096) {
        ...
        mFileHandle = fopen("./testtrace.trace", "ab");
    }
    ...
    void flush(SharedMemoryABI::Chunk&& chunk) {
        ...
        fwrite(chunkForReading.payload_begin(), bytesUsedThisChunk, 1, mFileHandle);
...
#   define CC_UNLIKELY( exp )  (__builtin_expect( !!(exp), 0 ))
    ...
    void beginTrace(const char* name) {
        if (CC_LIKELY(sTracingDisabled)) return;
        ...
        if (CC_UNLIKELY(mNeedToSetThreadId)) {
            mThreadId = __atomic_add_fetch(&sCurrentThreadId, 1, __ATOMIC_RELAXED);
