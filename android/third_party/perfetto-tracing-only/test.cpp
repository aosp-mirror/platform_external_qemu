...
void refreshPacket(protozero::RootMessage<protos::pbzero::TracePacket>* packet, protozero::ScatteredStreamWriter* writer) {
    packet->Reset(writer);
...
void beginPacket(protozero::RootMessage<protos::pbzero::TracePacket>* packet, protozero::ScatteredStreamWriter* writer) {
    packet->Reset(writer);
...
void endPacket(protozero::RootMessage<protos::pbzero::TracePacket>* packet) {
    // Finish the previous
    packet->Finalize();
}
...
static __attribute__((aligned(4096))) thread_local uint8_t sThreadLocalTraceBuffer[4096];

class TraceContext {
public:
    TraceContext() :
        mChunkStore(sThreadLocalTraceBuffer, sizeof(sThreadLocalTraceBuffer)),
        mWriterDelegate(&mChunkStore),
        mWriter(&mWriterDelegate) {
    }
...
private: 
    ...
    TestStreamWriterDelegate mWriterDelegate; 
