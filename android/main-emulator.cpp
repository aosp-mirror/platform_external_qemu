...
static std::string emu_dirname(const std::string& launcherDir) {
    char* cstr = path_join(path_parent(launcherDir.c_str(), 1), "emu");
...
"WARNING: 32-bit OSX Android emulator binaries are not supported, use 64bit.\n");
    ...
    std::string emuDirName = emu_dirname(argv0DirName);
