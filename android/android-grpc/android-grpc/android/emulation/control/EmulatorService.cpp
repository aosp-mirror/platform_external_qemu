...
#include "android/emulation/control/waterfall-client.grpc.pb.h"
...
public:
    ...
    Status Echo(
        ...
        std::unique_ptr<WaterfallBridge> bridge(mWaterfall.getBridge());
        auto channel = bridge->getGrpcChannel();

        if (!channel) {
            return Status::CANCELLED;
        }
        auto stub_ = ::waterfall::Waterfall::NewStub(channel);
        grpc::ClientContext deviceContext;

        std::shared_ptr<grpc::ClientReaderWriter<::waterfall::Message,
                                                 ::waterfall::Message>>
                deviceStream(stub_->Echo(&deviceContext));

        std::thread toDeviceForwarder([&stream, &deviceStream, &bridge]() {
            ::waterfall::Message msg;
            while (stream->Read(&msg) && deviceStream->Write(msg)) {
                // We do nothing..
                LOG(INFO) << "Forwarded from client to device: " << msg.DebugString();
            }
            LOG(INFO) << "The client has completed its stream..";
            deviceStream->WritesDone();
        });
        std::thread fromDeviceForwarder([&stream, &deviceStream, &bridge]() {
            ::waterfall::Message msg;
            while (deviceStream->Read(&msg) && stream->Write(msg)) {
                // We do nothing..
                LOG(INFO) << "Forwarded from to device to client" << msg.DebugString();
            }
            LOG(INFO) << "The device has completed its stream..";
        });
        toDeviceForwarder.join();
        fromDeviceForwarder.join();
        return deviceStream->Finish();
    }

private:
    WaterfallService mWaterfall;
