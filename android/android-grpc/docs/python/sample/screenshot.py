...
def _img_producer(queue, lock):
    ...
    fmt = p.ImageFormat(format=p.ImageFormat.RGBA8888, width=SAMPLE_WIDTH, height=SAMPLE_HEIGTH)
    i = 0
    for img in stub.streamScreenshot(fmt):
        lock.acquire()
        i = i + 1
        print("Enquing image {}".format(i))
        queue.append(img)
        lock.release()
...
def main():
    ...
    center_window(root, SAMPLE_WIDTH, SAMPLE_HEIGTH)
    root.mainloop()
