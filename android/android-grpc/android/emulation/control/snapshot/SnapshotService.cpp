...
public:
    ...
    Status LoadSnapshot(ServerContext* context,
        ...
        android::base::ThreadLooper::runOnMainLooper([&snapshot_done, &snapshot_signal,
            &snapshot_lock, &snapshot_success, &slc, &snapshot] () {
            snapshot_success = gQAndroidVmOperations->snapshotLoad(snapshot->name().data(),
                                                 slc.opaque(),
                                                 LineConsumer::Callback);
            snapshot_lock.lock();
            snapshot_done = true;
