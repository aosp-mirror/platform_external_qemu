...
public:
    ...
    // Kills the process.
    static void suicide(void* opaque, android::base::Looper::Timer* timer) {
        ...
        mLooper->scheduleCallback([=]() {
            switch (request->state()) {
                ...
                case VmRunState::TERMINATE:
                    // Try shutdown.. If that doesn't work we will give up.
                    mAgents->vm->vmShutdown();
                    android::base::ThreadLooper::get()
                            ->createTimer(suicide, nullptr)
                            ->startRelative(kWaitToDie.count());
                    break;
