...
protected:
    ...
    void SetUp() {
        ...
        mToken = pj(mTestSystem.getHomeDirectory(), "secret_token");
        writeToken(mToken);
    }
...
TEST_F(GrpcServiceTest, BadSecretsRegistrationFails) {
    auto badfile = pj(mTestSystem.getHomeDirectory(), "doesnot_exist");
...
TEST_F(GrpcServiceTest, InsecureWithGoodTokenAccepts) {
    ...
    auto token = getTokenCredentials("foobar");
    // Note! ::grpc::InsecureChannel WILL NOT INJECT HEADERS!
    auto [msg, status] =
            sayHello(::grpc::experimental::LocalCredentials(LOCAL_TCP), token);
    EXPECT_EQ(HELLO, msg.msg());
    EXPECT_EQ(grpc::StatusCode::OK, status.error_code());
...
TEST_F(GrpcServiceTest, SecureWithGoodTokenAccepts) {
    ...
    auto token = getTokenCredentials("foobar");
    auto [msg, status] = sayHello(creds, token);
    EXPECT_EQ(HELLO, msg.msg());
    EXPECT_EQ(grpc::StatusCode::OK, status.error_code());
