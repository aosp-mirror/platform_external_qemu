...
#if DEBUG >= 1
#define DD(fmt, ...) \
    printf("WaterfallServiceForwarder: %s:%d| " fmt "\n", __func__, __LINE__, ##__VA_ARGS__)
#else
#define DD(...) (void)0
#endif
namespace android {
...
WaterfallServiceForwarder::WaterfallServiceForwarder(WaterfallConnection* con)
...
WaterfallServiceForwarder::getWaterfallStub() {
...
std::shared_ptr<grpc::Channel> WaterfallServiceForwarder::openChannel() {
...
WaterfallServiceForwarder* getDefaultWaterfallServiceForwarder() {
    return new WaterfallServiceForwarder(getDefaultWaterfallConnection());
}
}  // namespace control
}  // namespace emulation
}  // namespace android
