...
uint64_t EventWaiter::next(std::chrono::milliseconds timeoutms) {
    uint64_t missed = next(mLastEvent + 1, timeoutms);
...
uint64_t EventWaiter::next(uint64_t afterEvent,
    ...
    auto future = std::chrono::system_clock::now() + timeoutms;
