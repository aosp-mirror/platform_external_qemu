...
StudioDiscovery::StudioDiscovery(StudioConfig&& config)
...
StudioDiscovery::~StudioDiscovery() {
    garbageCollect();
}

int StudioDiscovery::garbageCollect() {
    int collected = 0;
...
std::string StudioDiscovery::sharedFile() const {
    auto pid = std::to_string(System::get()->getCurrentProcessId());
    return android::base::pj(mSharedDirectory, pid);
}

bool StudioDiscovery::write() {
    auto pidFile = sharedFile();
