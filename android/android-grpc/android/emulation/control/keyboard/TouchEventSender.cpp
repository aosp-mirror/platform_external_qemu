...
void TouchEventSender::doSend(const TouchEvent request) {
    ...
    for (auto touch : request.touches()) {
        ...
        if (mUsedSlots.count(slot) == 0 && touch.pressure() != 0) {
            events.push_back({EV_ABS, LINUX_ABS_MT_TRACKING_ID, slot, displayId});
        ...
        if (touch.touch_major()) {
            events.push_back(
                    {EV_ABS, LINUX_ABS_MT_TOUCH_MAJOR, touch.touch_major(), displayId});
        }

        if (touch.touch_minor()) {
            events.push_back(
                    {EV_ABS, LINUX_ABS_MT_TOUCH_MINOR, touch.touch_minor(), displayId});
        }
        ...
        if (touch.pressure() == 0) {
            ...
            events.push_back({EV_ABS, LINUX_ABS_MT_PRESSURE, touch.pressure(), displayId});
        }
    }
