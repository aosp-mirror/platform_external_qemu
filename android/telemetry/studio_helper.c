...
static studio_xml_t uid = {
    .filename  = "options.xml",
    .nodename  = "property",
    .propname  = "name",
    .propvalue = "installation.uid",
    .keyname   = "value", // assuming hex string format "00000000-0000-0000-0000-000000000000"
};
...
// preferences installation; this value gets assigned to studio_scanner.cmp
int tr_studio_revision(const char *name)
{
    ...
    if( strlen(revstr) != 3 || !isdigit(revstr[0]) ||
        revstr[1] != '.'    || !isdigit(revstr[2])    ) {
        return 0;
    }
...
/* Create a new .AndroidStudio scanner instance, used to parse the directory
 * at |studio_preferences_home|. If this parameter is NULL, this will search
 * for the default AndroidStudio directory per Studio and IntelliJ conventions
 * https://intellij-support.jetbrains.com/hc/en-us/articles/206827437-
 *         Directories-used-by-the-IDE-to-store-settings-caches-plugins-and-logs
 */
studio_scanner* studio_scanner_new(const char* studio_preferences_home) {
    studio_scanner* s;
    ANEW0(s);
...
/* Return the name of the next .AndroidStudio dir detected by the scanner.
 * This will be NULL at the end of the scan.
 * Assumes PATH_MAX path strings
 */
studio_scanner* studio_scanner_next(studio_scanner* s) {
    if (s->scanner) {
        for (;;) {
            ...
            if(path_len >= PATH_MAX) {
                D("Cannot reach %s preferences in path %s",
                  ANDROID_STUDIO_DIR, path);
                continue;
            }
...
// Release an studio_scanner object and associated resources.
// This can be called before the end of a scan.
void studio_scanner_free(studio_scanner* s) {
    if (s) {
        if (s->scanner) {
...
/* Scan through all possible android studio dirs and return path to latest one
 * Assumes PATH_MAX path strings
 */
int path_to_latest_studio_dir(char *path)
{
    studio_scanner* as = studio_scanner_new(NULL);
    studio_scanner* p = NULL;
    int   last = -1;
    
    while ((p = studio_scanner_next(as)) != NULL) {
        if(last < p->cmp) {
            last = p->cmp;
        }
    }
    studio_scanner_free(as);    

    if(last == -1) 
        return -1;
    
    // repeat search for actual selection
    as = studio_scanner_new(NULL);
    while ((p = studio_scanner_next(as)) != NULL) {
        if(p->cmp == last)
            break;
    }
    
    if(strncpy(path, p->name, PATH_MAX) != path) {
...
/* Get the path to one of AndroidStudio's xml config files we support
 * Assumes PATH_MAX path strings
 */
int path_to_latest_studio_config_xml(char *path, const char *filename)
{
    char *p    = path;
    char *end  = p + (sizeof(char) * PATH_MAX);
    
    // append path to config filename (subdirs after ANDROID_STUDIO_DIR are fixed)
    p = &path[strlen(path)];
...
xmlChar *eval_studio_config_xml(xmlDocPtr doc, xmlNodePtr root, studio_xml_t *match)
{
    ...
    for (current = root; current; current = current->next) {
        if (current->type == XML_ELEMENT_NODE) {
            if ((!xmlStrcmp(current->name, (const xmlChar *) match->nodename))) {
                // fprintf(stderr, "%s--%s\n", (const char *) current->name,  (const char *) match->nodename);
                xmlChar *propvalue = xmlGetProp(current, (const xmlChar *) match->propname);
                // fprintf(stderr, "%s:%s\n", (const char *) match->propname, (const char *) propvalue);
                if ((!xmlStrcmp(propvalue, (const xmlChar *) match->propvalue))) {
                    retval = xmlGetProp(current, (const xmlChar *) match->keyname);
                    // fprintf(stderr, "---%s:%s\n", (const char *) match->keyname, (const char *) retval);
                    if(retval != NULL)
                        break;
                }
            }
...
xmlChar *parse_studio_config_xml(studio_xml_t *match)
{
...
int get_android_studio_optins(char *retval)
{
    ...
    if(val == NULL) {
        D("Failed to parse %s preferences file %s", ANDROID_STUDIO_DIR, optins.filename);
        return -1;
    }

    // convert to "bool_t"
    if ((!xmlStrcmp(val, (const xmlChar *) "false")))
        *retval = (char) 0;
    else
        *retval = (char) 1;
...
int get_android_studio_installation_id(char **retval)
{
    ...
    if(val == NULL) {
        D("Failed to parse %s preferences file %s", ANDROID_STUDIO_DIR, uid.filename);
        return -1;
    }

    // no conversion; installation IDs are 128bit so they remain as strings
