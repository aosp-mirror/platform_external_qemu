...
// Tests binding an image, but without dedicated allocation.
TEST_F(VulkanHalTest, AndroidHardwareBufferAllocate_ExportSeparateImage) {
    VkImage testAhbImage;
    createExternalImage(&testAhbImage, 1, 1);

    VkDeviceMemory memory;
    AHardwareBuffer* ahw;
    exportAllocateAndroidHardwareBuffer(
        nullptr,
        getNeededMemorySizeForImage(testAhbImage),
        getFirstMemoryTypeIndexForImage(testAhbImage),
        &memory, &ahw);
