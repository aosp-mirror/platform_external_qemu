...
public:
    ...
    int create(const char* name, size_t size) {
        ...
        int fd = mem->getFd();
        if (fd >= 0) {
            std::unique_ptr<SharedMemory> memPtr(mem);
            mMemories[fd] = std::move(memPtr);
        } else {
    ...
    void clear() {
        auto it = mMemories.begin();
        for (; it != mMemories.end(); ++it) {
            it->second->close();
        }
        mMemories.clear();
    }
    ...
    void close(int fd) {
        if (auto sharedMem = android::base::find(mMemories, fd)) {
            (*sharedMem)->close();
        }
    }
