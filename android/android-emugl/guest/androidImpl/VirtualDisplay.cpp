    ...
    void composeThread() {
        mThread = new FunctorThread([this]() {
            ...
            while (true) {
                ...
                fprintf(stderr, "%s: call glEGLImageTargetTexture2DOES %p vs %p\n", __func__, gl.glEGLImageTargetTexture2DOES, egl.eglGetProcAddress("glEGLImageTargetTexture2DOES"));
                ...
                uint32_t p;
                gl.glReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &p);
                fprintf(stderr, "%s: pixel: 0x%x\n", __func__, p);
