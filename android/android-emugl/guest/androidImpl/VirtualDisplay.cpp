...
public:
    ...
    int dequeueBuffer(ANativeWindowBuffer** buffer, int* fenceFd) {
        fprintf(stderr, "VirtualSurface::%s call\n", __func__);
        assert(mFromSf);
        BQ::Item item;
...
public:
    ...
    void composeThread() {
        mThread = new FunctorThread([this]() {
                fprintf(stderr, "%s: START COMPOSE THREAD\n", __func__);
            auto& egl = emugl::loadGoldfishOpenGL()->egl;
            auto& gl = emugl::loadGoldfishOpenGL()->gles2;

                fprintf(stderr, "composethread %s:%d arrive\n", __func__, __LINE__);
            ...
            fprintf(stderr, "%s: sampler loc: %d\n", __func__, samplerLoc);
            ...
            while (true) {
                ...
                fprintf(stderr, "%s: dequeud app buff\n", __func__);
...
private:
    ...
    EGLImageKHR getEGLImage(ANativeWindowBuffer* buffer) {
        ...
        auto& egl = emugl::loadGoldfishOpenGL()->egl;
        EGLDisplay d = egl.eglGetDisplay(EGL_DEFAULT_DISPLAY);
        EGLContext c = egl.eglGetCurrentContext();
        EGLImageKHR image = egl.eglCreateImageKHR(d, c, EGL_NATIVE_BUFFER_ANDROID, buffer, 0);
    ...
    GLuint compileShader(GLenum shaderType, const char* src) {
        ...
        auto& gl = emugl::loadGoldfishOpenGL()->gles2;
               ...
               fprintf(stderr, "%s: createShader %p\n", __func__, gl.glCreateShader); 
