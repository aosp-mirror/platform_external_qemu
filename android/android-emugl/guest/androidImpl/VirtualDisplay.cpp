...
public:
    VirtualSurface(BQ* _producer) : producer(_producer) {
    ...
    int queueBuffer(ANativeWindowBuffer* buffer, int fenceFd) {
        producer->queueBuffer({ buffer, fenceFd });
        return 0;
    }
...
public:
    ...
    ANativeWindowBuffer* allocateWindowBuffer(int width, int height, int format, int usage) {
        ...
        int stride;
        mGralloc.alloc(mGpu0Device, width, height, format, usage, handle, &stride);
