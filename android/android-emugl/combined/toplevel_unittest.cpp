...
TEST(Toplevel, Basic) {
    ...
    auto readbackWorker = std::make_unique<ExternalReadbackWorker>(*t, width, height, bytesPerPixel, readbackRate);

    eglMakeCurrent(d, s, s, c);
    for (int i = 0; i < 1 * displayRefreshRate; i++) {
