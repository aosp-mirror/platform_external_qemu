...
static const QAndroidEmulatorWindowAgent sQAndroidEmulatorWindowAgent = {
        .getEmulatorWindow =
                [](void) {
                    printf("window-agent-GfxStream-impl: .getEmulatorWindow\n");
                    return (EmulatorWindow*)nullptr;
                },
        .rotate90Clockwise =
                [](void) {
                    printf("window-agent-GfxStream-impl: .rotate90Clockwise\n");
                    return true;
                },
        .rotate =
        ...
        .getRotation =
                [](void) {
                    printf("window-agent-GfxStream-impl: .getRotation\n");
                    return SKIN_ROTATION_0;
                },
        .showMessage =
                [](const char* message, WindowMessageType type, int timeoutMs) {
                    printf("window-agent-GfxStream-impl: .showMessage %s\n", message);
                },
        .showMessageWithDismissCallback =
                [](const char* message, WindowMessageType type,
                   ...
                   void (*func)(void*), int timeoutMs) {
                    printf("window-agent-GfxStream-impl: .showMessageWithDismissCallback %s\n", message);
                },
        .fold =
                [](bool is_fold) {
                    printf("window-agent-GfxStream-impl: .fold %d\n", is_fold);
                    sIsFolded = is_fold;
                    return true;
                },
        .isFolded =
                [](void) -> bool {
                    printf("window-agent-GfxStream-impl: .isFolded ? %d\n", sIsFolded);
                    return sIsFolded;
                },
        .setUIDisplayRegion =
                [](int x_offset, int y_offset, int w, int h) {
                    printf("window-agent-GfxStream-impl: .setUIDisplayRegion %d %d %dx%d\n",
...
GfxStreamAndroidConsoleFactory::android_get_QAndroidEmulatorWindowAgent() const {
