...
// Test draw call rate without drawing on the host driver, which gives numbers
// closer to pure emulator overhead. This is a perfgate benchmark
TEST_F(CombinedGoldfishOpenglTest, DrawCallRateOverheadOnlyShort) {
        constexpr char vshaderSrc[] = R"(#version 300 es
    precision highp float;
...
TEST_F(CombinedGoldfishOpenglTest, DrawWithRoundTripOverhead) {
    ...
    android::perflogger::logDrawCallOverheadTest(
        ...
        (const char*)glGetString(GL_RENDERER),
        (const char*)glGetString(GL_VERSION),
        "drawArrays", "drawWithRoundTripOverhead",
        kDrawCallLimit,
        (long)drawCallHz,
        duration_us,
        duration_cpu_us);
...
// Test very large query (stream buffer stress test)
TEST_F(CombinedGoldfishOpenglTest, VeryLargeGetAttrib) {
    constexpr GLsizei kBufferSize = 524288;
    std::vector<GLchar> buf(kBufferSize);
    memset(buf.data(), 'A', kBufferSize);
    buf[kBufferSize - 1] = '\0';

    // Alternate large and small.
    for (int i = 0; i < 10; ++i) {
        int loc = glGetAttribLocation(238953, buf.data());
        EXPECT_EQ(-1, loc);
        loc = glGetAttribLocation(549412, "some_var");
        EXPECT_EQ(-1, loc);
    }
}
