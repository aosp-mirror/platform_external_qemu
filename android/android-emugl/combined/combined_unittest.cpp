...
// Test change in vertex array + texture bind rate + drawElements
TEST_P(CombinedGoldfishOpenglTest, ComplexDraw) {
        constexpr char vshaderSrc[] = R"(#version 300 es
    precision highp float;
    ...
    while (uploadCount < kUploadLimit) {
        glBindVertexArray(vao);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo);
        glBindBuffer(GL_ARRAY_BUFFER, uploadCount % 2 == 0 ? buffer : buffer2);
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE,
                              sizeof(VertexAttributes), 0);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE,
                                  sizeof(VertexAttributes),
        ...
        glBindTexture(GL_TEXTURE_2D, uploadCount % 2 == 0 ? texture : texture2);
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, matrix);
