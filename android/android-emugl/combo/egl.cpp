...
public:
    EGLGlobals() {
        emugl::setupStandaloneLibrarySearchPaths();

        assert(LazyLoadedEGLDispatch::get());
        assert(LazyLoadedGLESv2Dispatch::get());
...
EGLAPI void* EGLAPIENTRY aemuCreateWindow(int width, int height) {
    struct ANativeWindow* res = new ANativeWindow;
    res->setSwapInterval = hook_setSwapInterval;
    res->dequeueBuffer_DEPRECATED = hook_dequeueBuffer_DEPRECATED;
    res->lockBuffer_DEPRECATED = hook_lockBuffer_DEPRECATED;
    res->queueBuffer_DEPRECATED = hook_queueBuffer_DEPRECATED;
    res->query = hook_query;
    res->perform = hook_perform;
    res->cancelBuffer_DEPRECATED = hook_cancelBuffer_DEPRECATED;
    res->dequeueBuffer = hook_dequeueBuffer;
    res->queueBuffer = hook_queueBuffer;
    res->cancelBuffer = hook_cancelBuffer;
    res->common.incRef = hook_incRef;
    res->common.decRef = hook_decRef;
    fprintf(stderr, "%s: new window %p\n", __func__, res);
    return res;
}
