...
#define DEBUG_REFS_CALLSTACK_PATH       "/tmp"
...
public:
    ...
    void printRefs() const
    {
        ...
        {
            android::base::AutoLock _l(mMutex);
            char buf[128];
...
private:
    ...
    mutable android::base::Lock mMutex;
...
void RefBase::incStrong(const void* id) const
{
    ...
    refs->mStrong.fetch_add(-INITIAL_STRONG_VALUE, std::memory_order_release);
...
bool RefBase::weakref_type::attemptIncStrong(const void* id)
{
    ...
    while (curCount > 0 && curCount != INITIAL_STRONG_VALUE) {
        ...
        if (impl->mStrong.compare_exchange_strong(curCount, curCount + 1, std::memory_order_release, std::memory_order_relaxed) == 0) {
            break;
        }
...
bool RefBase::weakref_type::attemptIncWeak(const void* id)
{
    ...
    ALOG_ASSERT(curCount >= 0, "attemptIncWeak called on %p after underflow",
               this);
    while (curCount > 0) {
        if (impl->mWeak.compare_exchange_strong(curCount, curCount+1, std::memory_order_release, std::memory_order_relaxed) == 0) {
            break;
        }
        curCount = impl->mWeak;
    }
...
{
    mRefs->mFlags.fetch_or(mode, std::memory_order_release);
}
