...
inline int Log2Floor(int max_texture_size) {
    const auto size = static_cast<double>(max_texture_size);
...
static ElementRange GetElementRange(GLsizei count,
                                    GLenum type,
                                    const GLvoid* data) {
    ElementRange range;
    if (count == 0) {
        ...
        const auto* arr = static_cast<const GLubyte*>(data);
        range.first = arr[0];
        range.last = arr[0];
        ...
        const auto* arr = static_cast<const GLushort*>(data);
        range.first = arr[0];
        range.last = arr[0];
        for (int i = 1; i < count; ++i) {
            range.first = std::min<size_t>(arr[i], range.first);
        ...
        const auto* arr = static_cast<const GLuint*>(data);
        range.first = arr[0];
        range.last = arr[0];
        for (int i = 1; i < count; ++i) {
            range.first = std::min<size_t>(arr[i], range.first);
...
bool Light::IsSpot() const {
    return spot_cutoff != 180.f;
}

bool Light::IsPositional() const {
    return position.Get(3) != 0.f;
}
...
// See
// https://www.khronos.org/registry/gles/extensions/OES/OES_texture_cube_map.txt
// The initial values are defined in Section 2.11.4
TexGen::TexGen() = default;
...
void StateCacheLoaderHelper::Load(
        GLenum value,
        GLfloat& data) {  // NOLINT(runtime/references)
    Load(value, &data);
}

void StateCacheLoaderHelper::Load(GLenum value,
                                  GLint& data) {  // NOLINT(runtime/references)
    Load(value, &data);
}

void StateCacheLoaderHelper::Load(GLenum value,
                                  GLuint& data) {  // NOLINT(runtime/references)
    Load(value, &data);
}
...
FullscreenQuad::FullscreenQuad(GlesContext* context)
    ...
    const GLuint fsh =
            context_->CompileShader(GL_FRAGMENT_SHADER, kFragmentShader);

    program_ = context_->CompileProgram(vsh, fsh);
    position_attrib_idx_ =
            PASS_THROUGH(context_, GetAttribLocation, program_, "a_position");
    texcoord_attrib_idx_ =
            PASS_THROUGH(context_, GetAttribLocation, program_, "a_texcoord");
    sampler_uniform_idx_ =
            PASS_THROUGH(context_, GetUniformLocation, program_, "u_sampler");
...
void UniformContext::Bind(ProgramContext* program) {
    ...
    if (force_update || alpha_test_.IsDirty()) {
        program->BindUniform(alpha_test_.Get().value,
                             kAlphaTestConstantUniform);
        alpha_test_.Clean();
    }
...
const emugl::Matrix& UniformContext::GetTextureMatrixByStage(
        GLint stage) const {
...
const Dirtiable<MatrixStack>* UniformContext::GetActiveMatrixStackInternal()
        const {
    switch (matrix_mode_) {
        case GL_PROJECTION:
...
GLint ProgramContext::GetUniformLocation(UniformKey key, int idx) {
    ...
    auto iter = uniform_location_map_.find(entry);
...
void ProgramContext::BindUniform(const float& value,
                                 UniformKey key,
                                 int index) {
    const GLint location = GetUniformLocation(key, index);
    if (location != -1) {
...
void ProgramContext::BindUniform(const emugl::Vector& vector,
                                 UniformKey key,
                                 int index) {
    const GLint location = GetUniformLocation(key, index);
    if (location != -1) {
...
void ProgramContext::BindUniform(const emugl::Matrix& matrix,
                                 UniformKey key,
                                 int index) {
    const GLint location = GetUniformLocation(key, index);
    if (location != -1) {
...
void ProgramContext::BindUniform(const Light& light, int index) {
    const float light_spot_cutoff =
            std::cos(light.spot_cutoff * emugl::kRadiansPerDegree);
    ...
    BindUniform(light.linear_attenuation, kLightLinearAttenuationUniform,
                index);
...
void ProgramContext::BindUniform(const TexEnv& texenv, int index) {
    ...
    //   * A color/alpha scale. For simplicity we generate a four component
    //   scale
    //     here, though we only really need two components.
    BindUniform(texenv.color, kTextureEnvColorUniform, index);
    const emugl::Vector scale =
            emugl::Vector(texenv.rgb_scale, texenv.rgb_scale, texenv.rgb_scale,
                          texenv.alpha_scale);
...
void TextureContext::DeleteTexture(GLuint texture) {
    for (auto& texture_unit : texture_units_) {
        for (GLuint j = 0; j < num_texture_units_; ++j) {
            TextureUnit& unit = texture_unit[j];
...
void TextureContext::SetTargetTexture(GLenum target,
                                      GLuint texture,
                                      GLenum global_target) {
    // The logic below in PrepareTextures/RestoreTextures assumes that we are
    // only ever remapping TEXTURE_EXTERNAL_OES to TEXTURE_2D.
    LOG_ALWAYS_FATAL_IF(
            target != global_target && (global_target != GL_TEXTURE_2D ||
...
GLenum TextureContext::EnsureCorrectBinding(GLenum target) {
    const TextureUnit& unit =
            GetTextureUnit(MapTarget(target), active_texture_unit_);
...
void TextureContext::PrepareTextures(bool gles11,
    ...
    for (GLuint i = 0; i < num_texture_units_; ++i) {
        ...
        if (unit_ext.global_target == GL_TEXTURE_2D) {
            // For the GLES1 code path, we know if the texture target was
            // enabled or not. If it is enabled, the external texture overrides
            // any normal texture 2d according to the specification. For the
            // GLES2+ code path, things are much more interesting, and we need
            // to know what uniform type the shader uses for each texture unit,
            // as valid textures can be bound to each target and the shader
            // defines which one is read.
            // TODO(crbug.com/441940): This really needs to be revisited to not
            // assume that only the first texture unit is the one that needs to
            // be remapped.
            ...
            // bind that unit's data, otherwise the app was using 2D-related
            // unit.
...
void TextureContext::RestoreTextures() {
    ...
    for (GLuint i = 0; i < num_texture_units_; ++i) {
        const TextureUnit& unit_ext = GetTextureUnit(kTextureExternal, i);
        // If we have an external texture that maps to TEXTURE_2D we simply
        // rebind the 2D texture unconditionally rather than duplicate the logic
        // from PrepareTextures. Worst case this texture is already bound.
        if (unit_ext.global_target == GL_TEXTURE_2D) {
            ActiveTextureUnderlying(i);
            current_active_texture_unit = i;
...
TextureContext::TextureUnit& TextureContext::GetTextureUnit(Target t,
                                                            GLuint index) {
    return texture_units_[t][index];
}

const TextureContext::TextureUnit& TextureContext::GetTextureUnit(
        Target t,
        GLuint index) const {
    return texture_units_[t][index];
}
...
void PointerContext::SetPointer(GLuint index,
                                GLint size,
                                GLenum type,
                                GLsizei stride,
...
const GLvoid* PointerContext::PrepareBuffersForDrawElements(
    ...
    if (vbo == nullptr) {
        ...
        if (size > element_array_buffer_size_) {
            PASS_THROUGH(context_, BufferData, GL_ELEMENT_ARRAY_BUFFER, size,
                         NULL, GL_STREAM_DRAW);
    ...
    // calculate the range of the elements we are drawing so that we can copy
    // the client-data into VBOs.  The copying of the data is done in
    // BindPointers. If we have no client-side vertex attributes, then the range
    // does not matter.
    ...
    if (has_client_vertex_attribs) {
        ...
        if (vbo != nullptr) {
            ...
            LOG_ALWAYS_FATAL_IF(buffer_data == nullptr,
    ...
    return vbo != nullptr ? indices : nullptr;
}
...
void PointerContext::BindPointers(GLint first, GLint last) {
    ...
    for (size_t index = 0; index < pointers_.size(); ++index) {
        ...
        if (ptr.buffer_name) {
            const ObjectGlobalName global_name =
                    context_->GetShareGroup()->GetBufferGlobalName(
                            ptr.buffer_name);
            PASS_THROUGH(context_, BindBuffer, GL_ARRAY_BUFFER, global_name);

            // TODO(crbug.com/482070): Convert any elements in the buffer data
            // that are of type GL_FIXED to GL_FLOAT and re-upload that data to
            // the bound buffer.
            if (ptr.type == GL_FIXED) {
                ...
                BufferDataPtr buffer = context_->GetShareGroup()->GetBufferData(
                        ptr.buffer_name);
                const unsigned char* data = buffer->GetData();
                const size_t size = buffer->GetSize();
                const GLintptr stride =
                        ptr.stride ? ptr.stride
                                   : ptr.size * GetTypeSize(ptr.type);

                for (auto marker = reinterpret_cast<size_t>(ptr.pointer);
                     marker < size; marker += stride) {
                    GLfloat dst[4];
                    const auto* src =
                            reinterpret_cast<const GLfixed*>(&data[marker]);
                    for (int i = 0; i < ptr.size; ++i) {
                        dst[i] = X2F(src[i]);
                    }
                    PASS_THROUGH(context_, BufferSubData, GL_ARRAY_BUFFER,
                                 marker, ptr.size * sizeof(GLfloat), dst);
                }
            }
        ...
        size += ptr.stride ? ptr.stride * last
                           : GetTypeSize(ptr.type) * last * ptr.size;
    }
    ...
    for (size_t index = 0; index < pointers_.size(); ++index) {
        ...
        const auto* data = reinterpret_cast<const unsigned char*>(ptr.pointer);

        // TODO(crbug.com/482070): Convert any elements in the buffer data that
        // are of type GL_FIXED to GL_FLOAT before copying it to our buffer
        // object.
        if (ptr.type == GL_FIXED) {
            auto* tmp = new unsigned char[offset_last];
            for (size_t marker = offset_first; marker < offset_last;
                 marker += stride) {
                auto* dst = reinterpret_cast<GLfloat*>(&tmp[marker]);
                const auto* src =
                        reinterpret_cast<const GLfixed*>(&data[marker]);
                for (int i = 0; i < ptr.size; ++i) {
                    dst[i] = X2F(src[i]);
        ...
        PASS_THROUGH(context_, BufferSubData, GL_ARRAY_BUFFER,
                     ...
                     data + offset_first);
        PASS_THROUGH(context_, VertexAttribPointer, index, ptr.size,
                     ptr.type == GL_FIXED ? GL_FLOAT : ptr.type, ptr.normalize,
                     ptr.stride, reinterpret_cast<void*>(offset));
        if (ptr.type == GL_FIXED) {
            delete[] data;
        }
        offset += offset_last;
