...
static const char* sGetGLEnumString(GLenum name) {
#define ENUMSTRINGCASE(e) \
    case e: \
        return #e; \

    switch (name) {
    ENUMSTRINGCASE(GL_COLOR_CLEAR_VALUE)
    ENUMSTRINGCASE(GL_ACTIVE_TEXTURE)
    };
    D("Unknown enum 0x%x", name);
    return "UNKNOWN";
}
...
void GLSnapshotState::getGlobalStateEnum(GLenum name, int size) {
    D("save %s", sGetGLEnumString(name));
