...
static void loadMap_32_string(std::unordered_map<K, std::string>& m, android::base::Stream* stream) {
    uint32_t count = stream->getBe32();
    for (uint32_t i = 0; i < count; i++) {
        K fst; fst = stream->getBe32();
        m[fst] = stream->getString();;
    }
}
...
void GLSnapshotState::onSave(android::base::Stream* stream) {
    ...
    D("UID %u: Saved %zu shaders.", mUid, mShaders.size());
    D("UID %u: Saved %zu programs.", mUid, mPrograms.size());
    D("UID %u: Saved %zu buffers.", mUid, mBuffers.size());
    D("UID %u: Saved %zu FBOs.", mUid, mFBOs.size());
    D("UID %u: Saved %zu Textures.", mUid, mTextures.size());
    D("UID %u: Saved %zu VAOs.", mUid, mVAOs.size());
...
bool GLSnapshotState::onLoad(android::base::Stream* stream) {
    ...
    D("UID %u: Found %zu shaders.", mUid, mShaders.size());
    D("UID %u: Found %zu programs.", mUid, mPrograms.size());
    D("UID %u: Found %zu buffers.", mUid, mBuffers.size());
    D("UID %u: Found %zu FBOs.", mUid, mFBOs.size());
    D("UID %u: Found %zu Textures.", mUid, mTextures.size());
    D("UID %u: Found %zu VAOs.", mUid, mVAOs.size());
