        ...
        switch (opcode)
        {
            ...
            case OP_vkFlushMappedMemoryRanges:
            {
                ...
                for (uint32_t i; i < memoryRangeCount; ++i)
                {
                    ...
                    VkMappedMemoryRange hostFlush = { VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, 0, memory, offset, size };
                    VkResult vkFlushMappedMemoryRanges_VkResult_return = (VkResult)0;
                    vkFlushMappedMemoryRanges_VkResult_return = m_vk->vkFlushMappedMemoryRanges(device, 1, &hostFlush);
                    m_vk->vkUnmapMemory(device, memory);
                }
                vkStream->commitWrite();
                break;
            ...
            case OP_vkInvalidateMappedMemoryRanges:
            {
                ...
                for (uint32_t i; i < memoryRangeCount; ++i)
                {
                    ...
                    VkResult vkMapMemory_VkResult_return = (VkResult)0;
                    vkMapMemory_VkResult_return = m_vk->vkMapMemory(device, memory, offset, size, flags, ppData);
                    VkMappedMemoryRange hostInvalidate = { VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, 0, memory, offset, size };
                    VkResult vkInvalidateMappedMemoryRanges_VkResult_return = (VkResult)0;
                    vkInvalidateMappedMemoryRanges_VkResult_return = m_vk->vkInvalidateMappedMemoryRanges(device, 1, &hostInvalidate);
                    vkReadStream->write(*ppData, size);
                    m_vk->vkUnmapMemory(device, memory);
                }
                vkStream->commitWrite();
                break;
