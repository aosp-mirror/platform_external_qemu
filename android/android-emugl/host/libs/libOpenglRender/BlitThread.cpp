...
#define D(fmt,...) do { \
    fprintf(stderr, "BlitThread: " fmt " curr cxt: 0x%llx surfs 0x%llx 0x%llx\n", ##__VA_ARGS__, \
            (unsigned long long)(uintptr_t)s_egl.eglGetCurrentContext(), \
            (unsigned long long)(uintptr_t)s_egl.eglGetCurrentSurface(EGL_DRAW), \
            (unsigned long long)(uintptr_t)s_egl.eglGetCurrentSurface(EGL_READ)); \
} while (0) \
...
void BlitThread::bindSubwinContext(bool canSkip = true) {
    if (canSkip && mCurrContext == mSubwinContext) return;
    mFb->lock();
    bindSubwinContextLocked(canSkip);
    mFb->unlock();
}

void BlitThread::bindSubwinContextLocked(bool canSkip = true) {
    if (canSkip && mCurrContext == mSubwinContext) return;
...
void BlitThread::closeColorBufferLocked(uint32_t colorBuffer) {
    android::base::AutoLock lock(mCmdLock);
    mCmd.opCode = CloseColorBufferLocked;
    mCmd.colorBuffer = colorBuffer;
    mInput.send(mCmd);
    mOutput.receive(&mCmdRes);
}
...
intptr_t BlitThread::main() {
    ...
    while (!exiting) {
        ...
        case TransformSubwindow:
            // Framebuffer lock is held during this.
            D("cmd: TransformSubwindow");
            mFb->transformSubWindowLocked(
            ...
            mSubwinSurf = mFb->currSubWindowEGLSurface();
            bindSubwinContextLocked(false);
            mOutput.send(1);
            break;
        case Blit:
            D("cmd: Blit");
        ...
        case CloseColorBuffer:
            ...
            break;
        case CloseColorBufferLocked:
            D("cmd: CloseColorBufferLocked");
            mFb->closeColorBufferLocked(cmd.colorBuffer);
            mOutput.send(0);
            break;
