...
void PostWorker::compose(struct composeDevice* p) {
    ...
    printf("worker compose %d layers\n", p->numLayers);
    for (int i = 0; i < p->numLayers; i++, l++) {
        printf("\tcomposeMode %d color %d %d %d %d blendMode "
 ...
 void PostWorker::composeLayer(ColorBuffer* cb, struct composeLayer *l) {
    if (l->composeMode == HWC2_COMPOSITION_DEVICE) {
