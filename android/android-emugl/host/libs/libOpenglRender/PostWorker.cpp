...
void PostWorker::post(ColorBuffer* cb) {
    ...
    if (emugl::get_emugl_multi_display_operations().isMultiDisplayEnabled()) {
        ...
        while(emugl::get_emugl_multi_display_operations().getNextMultiDisplay(start_id, &id,
            ...
            ComposeLayer l;
            hwc_rect_t displayArea = { .left = x,
                                             .top = y,
                                             .right = x + w,
                                             .bottom = y + h };
            hwc_frect_t cropArea = { .left = (float)0,
                                           .top = multiDisplayCb->getHeight(),
                                           .right = multiDisplayCb->getWidth(),
                                           .bottom = 0 }; 
...
//                displayArea.right = windowHeight;
//                displayArea.bottom = windowWidth;
                break;
            case 2:
 ...
 //               displayArea.bottom = windowWidth;
        ...
        fillMultiDisplayPostStruct(&l, displayArea, cropArea, (hwc_transform_t)rotation);
...
//        GLuint tex = cb->scale();
        cb->postWithOverlay(cb->getTexture(), 0.0f, dx, dy);
...
void PostWorker::viewport(int width, int height) {
    ...
    if (emugl::get_emugl_window_operations().isFolded()) {
        int xoffset = m_viewportWidth / 1536 * 696;
        s_gles2.glViewport(-xoffset, 0, m_viewportWidth, m_viewportHeight);
    } else {
