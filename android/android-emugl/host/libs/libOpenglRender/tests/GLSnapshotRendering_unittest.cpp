...
public:
    ...
    void overrideFunctions() {
        this->glEnable = (glEnable_t)test_glEnable;
...
private:
    ...
    static void test_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
        ...
        width = fb->getWidth();
        height = fb->getHeight();
        prePixels.resize(width * height *
                         glUtilsPixelBitSize(GL_RGBA, GL_UNSIGNED_BYTE) / 8);
        ...
        postPixels.resize(width * height *
                          glUtilsPixelBitSize(GL_RGBA, GL_UNSIGNED_BYTE) / 8);
        gl->glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE,
                         postPixels.data());

        EXPECT_TRUE(compareVector<GLubyte>(
                prePixels, postPixels, "Snapshot DrawArrays result pixels"));
        fprintf(stderr, "Finished snapshot DrawArrays comparison!\n");
        // TODO(benzene): print stuff to verify pixels got getted correctly
    }
...
TEST_F(GLTest, OverrideDispatch) {
    ...
    EXPECT_NE(gl->glEnable, LazyLoadedGLESv2Dispatch::get()->glEnable);
...
protected:
    ...
    void draw() {
        HelloTriangle::draw();
        fprintf(stderr, "SnapshotTestTriangle Drew!\n");
    }
};
