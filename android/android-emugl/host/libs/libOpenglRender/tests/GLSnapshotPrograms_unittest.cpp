        ...
        for (int i = 0; i < currentState.uniforms.size(); i++) {
            ...
            for (int j = 0; j < currentState.uniforms[i].size; j++) {
                ...
                if (currentVal.floats.size() > 0 &&
                    ...
                    FAIL() << "Uniform " << currentState.uniforms[i].name.data()
...
protected:
    ...
    void getUniformValues(GLuint location,
                          ...
                          std::vector<GlValues>& values) {
        GlValues val = {};
        // alignas(double) unsigned char val[128];  // enough space for MAT4x4
        switch (type) {
            case GL_FLOAT:
            case GL_FLOAT_VEC2:
            case GL_FLOAT_VEC3:
    ...
    // with the the brackets sliced off.
...
TEST_F(SnapshotGlProgramTest, UseProgramAndUniforms) {
    setStateChanger([this] {
        ...
        GLuint floatMatUnifLocation =
                gl->glGetUniformLocation(m_program_name, "projection");
