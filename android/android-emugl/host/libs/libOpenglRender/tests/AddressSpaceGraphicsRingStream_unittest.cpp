...
protected:
    ...
    void ringWriteFullyWithHangup(
        ...
        uint32_t currAvailRead = ring_buffer_available_read(mToHost.ring, &mToHost.view);
        while (currAvailRead) {
            sched_yield();
            uint32_t nextAvailRead = ring_buffer_available_read(mToHost.ring, &mToHost.view);
...
TEST_F(AddressSpaceGraphicsRingStreamTest, MultiRoundTrips_1k_LargerReads) {
    std::vector<RoundTrip> trips;
    for (uint32_t i = 0; i < 1000; ++i) {
        trips.push_back({ 4, 513 });
    };
    multiRoundTripTest(trips);
}

// Tests 1k round trips with random write and read sizes.
TEST_F(AddressSpaceGraphicsRingStreamTest, MultiRoundTrips_1k_Random) {
