...
protected:
    ...
    void singleEchoTest(size_t echoSize, size_t echoReadbackSize) {
        ...
        AddressSpaceGraphicsContext::ConsumerCreateCallback createFunc =
            [&ringStream, &fakeRenderThread, echoSize, echoReadbackSize](struct ring_buffer_with_view tohost,
               ...
               fakeRenderThread.reset(new FunctorThread(
                   [&ringStream, echoSize, echoReadbackSize] {
                   const size_t wantedRead = echoSize;
                   const size_t wantedWrite = echoReadbackSize;
                   std::vector<char> toRead(wantedRead);
                   std::vector<char> toWrite(wantedRead);
        ...
        std::vector<char> echoBuf(echoSize);
        std::vector<char> echoReadbackBuf(echoReadbackSize);
