...
class SnapshotGlColorMaskTest : public SnapshotSetValueTest<GLboolean*> {
    void stateCheck(GLboolean* expected) override {
        GLboolean mask[4] = {};
        gl->glGetBooleanv(GL_COLOR_WRITEMASK, mask);
        for (int i = 0; i < 4; i++) {
            EXPECT_EQ(expected[i], mask[i]) << " at mask index " << i;
        }
    }
    void stateChange() override {
        GLboolean* mask = *m_changed_value;
        gl->glColorMask(mask[0], mask[1], mask[2], mask[3]);
    }
};

TEST_F(SnapshotGlColorMaskTest, SetColorMask) {
    GLboolean defaultColorMask[] = {true, true, true, true};
    GLboolean testColorMask[] = {false, false, false, false};
    setExpectedValues(defaultColorMask, testColorMask);
    doCheckedSnapshot();
}
...
class SnapshotGlDepthMaskTest : public SnapshotSetValueTest<GLboolean> {
    void stateCheck(GLboolean expected) override {
        GLboolean flag;
        gl->glGetBooleanv(GL_DEPTH_WRITEMASK, &flag);
...
TEST_F(SnapshotGlColorClearValueTest, SetClearColor) {
    setExpectedValues({0, 0, 0, 0}, {0.5f, 0.6f, 0.7f, 0.8f});
    doCheckedSnapshot();
}
