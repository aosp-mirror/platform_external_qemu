...
static void* dlOpenFuncForTesting() {
    auto progPath =
        PathUtils::join(
            TestSystem::getProgramDirectoryFromPlatform(),
            "testlib64");
    
#ifdef _WIN32
    const Win32UnicodeString name(PathUtils::join(progPath, "vulkan-1.dll"));
    ...
    std::string libName = std::string("libvulkan") + suffix;

    auto name = PathUtils::join(progPath, libName);
...
static std::string getPhysicalDevicePropertiesString(
    ...
    for (uint32_t i = 0; i < deviceExtensionCount; ++i) {
        ss << "Device extension: " << deviceExtensionProperties[i].extensionName << "\n";
    }
...
static bool testDeviceGetter(const VulkanDispatch* vk,
                             VkInstance instance,
                             VkPhysicalDevice* physDevice_out,
                             uint32_t* queueFamilyInfoCount_out,
                             VkQueueFamilyProperties* queueFamilyInfos_out,
                             VkDevice* device_out,
                             bool* presentCapable) {
    ...
    if (!bestPhysicalDeviceFound) return false;
    ...
    *physDevice_out = bestPhysicalDevice;
    ...
    uint32_t queueFamilyCount;
    vk->vkGetPhysicalDeviceQueueFamilyProperties(bestPhysicalDevice, &queueFamilyCount, nullptr);

    *queueFamilyInfoCount_out = queueFamilyCount;

    if (queueFamilyInfos_out) {
        std::vector<VkQueueFamilyProperties> queueFamilies(queueFamilyCount);
        vk->vkGetPhysicalDeviceQueueFamilyProperties(bestPhysicalDevice, &queueFamilyCount, queueFamilies.data());
        memcpy(queueFamilyInfos_out, queueFamilies.data(), queueFamilyCount * sizeof(VkQueueFamilyProperties));
...
TEST(Vulkan, Dispatch) {
    ...
    auto ldLibPath = TestSystem::getEnvironmentVariable("LD_LIBRARY_PATH");
    auto icdfile = TestSystem::getEnvironmentVariable("VK_ICD_FILENAMES");

    fprintf(stderr, "%s: ld %s icd %s\n", __func__,
            ldLibPath.c_str(),
            icdfile.c_str());
