...
template <class T>
testing::AssertionResult compareVector(std::vector<T> expected,
                                       std::vector<T> actual,
    ...
    if (mismatches > 0) {
        return testing::AssertionFailure() << description << " mismatched\n"
                                           << message.str() << "  ";
    }
    return testing::AssertionSuccess();
}
...
testing::AssertionResult compareGlobalGlBooleanv(
    ...
    EXPECT_EQ(GL_NO_ERROR, gl->glGetError());
    return compareVector<GLboolean>(expected, current, "GL global booleanv");
}
...
testing::AssertionResult compareGlobalGlIntv(const GLESv2Dispatch* gl,
    ...
    EXPECT_EQ(GL_NO_ERROR, gl->glGetError());
    return compareVector<GLint>(expected, current, "GL global intv");
}
...
testing::AssertionResult compareGlobalGlFloatv(const GLESv2Dispatch* gl,
    ...
    EXPECT_EQ(GL_NO_ERROR, gl->glGetError());
    return compareVector<GLfloat>(expected, current, "GL global floatv");
}
