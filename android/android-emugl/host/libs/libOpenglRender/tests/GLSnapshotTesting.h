...
// integer array with no more than |size| elements, against |expected| values.
testing::AssertionResult compareGlobalGlIntv(const GLESv2Dispatch* gl,
                                             GLenum name,
                                             std::vector<GLint> expected,
                                             GLuint size = 0);

// Compares a vector of global GL values, known by |name| and retrieved as a
// float array of no more than |size| elements, against |expected| values.
