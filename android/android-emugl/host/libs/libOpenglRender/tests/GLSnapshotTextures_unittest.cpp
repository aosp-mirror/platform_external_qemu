...
public:
    ...
    void changedStateCheck() override {
        SCOPED_TRACE(std::to_string(m_object_name) + ": target " +
                     describeGlEnum(m_state.target));
        ...
        auto compareImageFunc = [this](GLenum imageTarget,
            ...
            fprintf(stderr, "Comparing images\n");
            for (int i = 0; i < levels.size(); i++) {
                std::vector<GLubyte> currentTexels = {};
                fprintf(stderr, "Trying to get texture image data %d\n", i);
                getTextureImageData(gl, imageTarget, m_object_name, i, 0, 0,
                                    levels[i].width, levels[i].height,
                                    levels[i].format, levels[i].type,
                                    currentTexels);
                fprintf(stderr, "Got texture image data\n");
                EXPECT_TRUE(compareVector<GLubyte>(
                        levels[i].bytes, currentTexels,
                        "Texture " + std::to_string(m_object_name) +
                                " target " + describeGlEnum(imageTarget) +
                                " level " + std::to_string(i)));
                EXPECT_EQ(GL_NO_ERROR, gl->glGetError());
                fprintf(stderr, "Completed comparison level %d\n", i);
            }
        };
        ...
        switch (m_state.target) {
            ...
            case GL_TEXTURE_CUBE_MAP: {
                ...
                for (int j = 0; j < m_state.imagesCubeMap.size(); j++) {
                    ...
                    compareImageFunc(side, m_state.imagesCubeMap[j]);
                }
            } break;
            default:
                ADD_FAILURE()
