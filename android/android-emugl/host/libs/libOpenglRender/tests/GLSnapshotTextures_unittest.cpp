...
public:
    void defaultStateCheck() override {
        ...
        EXPECT_TRUE(compareParameter(GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR));
    ...
    void changedStateCheck() override {
        ...
        EXPECT_TRUE(compareGlobalGlInt(gl, GL_ACTIVE_TEXTURE, GL_TEXTURE0));
        EXPECT_TRUE(compareGlobalGlInt(gl, GL_TEXTURE_BINDING_2D, m_object_name));
...
protected:
    ...
    testing::AssertionResult compareParameter(GLenum paramName,
        ...
        fprintf(stderr, "For param 0x%x: actual 0x%x, expected 0x%x\n",
...
TEST_F(SnapshotGlTextureObjectTest, SetObjectParameters) {
    setStateChanger([this] {
        const GLenum testMinFilter = GL_LINEAR, testMagFilter = GL_NEAREST,
                     testWrapS = GL_MIRRORED_REPEAT,
                     testWrapT = GL_CLAMP_TO_EDGE;
        gl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, testMinFilter);
        gl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, testMagFilter);
        gl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, testWrapS);
        gl->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, testWrapT);
