    ...
    void stateChange() override {
        ...
        auto initImageFunc = [this](GLenum imageTarget, GlMipmapArray& levels) {
            for (int i = 0; i < levels.size(); i++) {
                levels[i].bytes.resize(levels[i].width * levels[i].height *
                                       glSizeof(GL_UNSIGNED_BYTE));
                gl->glTexImage2D(imageTarget, i, levels[i].format,
                                 levels[i].width, levels[i].height, 0,
                                 levels[i].format, GL_UNSIGNED_BYTE,
                                 levels[i].bytes.data());
                fprintf(stderr, "Created Tex2D for mipmap level %d\n", i);
            }
        };
        ...
        switch (m_state.target) {
            case GL_TEXTURE_2D: {
                ...
                // for (int i = 0; i < m_state.images2D.size(); i++) {
                //      m_state.images2D[i].bytes.resize(m_state.images2D[i].width
                //      *
                //          m_state.images2D[i].height *
                //          glSizeof(GL_UNSIGNED_BYTE));
                //      gl->glTexImage2D(m_state.target, i,
                //      m_state.images2D[i].format,
                //              m_state.images2D[i].width,
                //              m_state.images2D[i].height, 0,
                //              m_state.images2D[i].format, GL_UNSIGNED_BYTE,
                //              m_state.images2D[i].bytes.data());
                //      fprintf(stderr, "Created Tex2D for 2d mipmap level
                //      %d\n", i);
                //  }
            } break;
            case GL_TEXTURE_CUBE_MAP: {
                ...
                for (int j = 0; j < m_state.imagesCubeMap.size(); j++) {
                    ...
                    // for (int i = 0; i < m_state.imagesCubeMap[j].size(); i++)
                    // {
                    //     m_state.imagesCubeMap[j][i].bytes.resize(m_state.imagesCubeMap[j][i].width
                    //     *
                    //         m_state.imagesCubeMap[j][i].height *
                    //         glSizeof(GL_UNSIGNED_BYTE));
                    //     gl->glTexImage2D(side, i,
                    //     m_state.imagesCubeMap[j][i].format,
                    //             m_state.imagesCubeMap[j][i].width,
                    //             m_state.imagesCubeMap[j][i].height, 0,
                    //             m_state.imagesCubeMap[j][i].format,
                    //             GL_UNSIGNED_BYTE,
                    //             m_state.imagesCubeMap[j][i].bytes.data());
                    //     fprintf(stderr, "Created Tex2D for cube face 0x%x
                    //     level %d\n", side, i);
                    // }
                }
            } break;
...
protected:
    ...
    GLenum getTargetBindingName(GLenum target) {
        switch (target) {
            ...
            default:
                ADD_FAILURE()
                        << "Test unsupported texture bind target " << target;
                return 0;
        }
    }
