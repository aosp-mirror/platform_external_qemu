...
static const char kTestVertexShaderSource[] =
...
static const char kTestFragmentShaderSource[] =
        "precision mediump float;\n"
        "void main() {\n"
        "gl_FragColor = vec4(0.0, 1.0, 0.0, 1.0);\n"
        "}\n";

// std::string getSource(std::string filePath) {
...
//         return source;
//     }
//     ADD_FAILURE() << "Could not load shader source " << filePath;
//     return "";
// }
...
public:
    ...
    void loadSource(std::string sourceString) {
        if (m_shader_name == 0) {
            ADD_FAILURE() << "Cannot set source without a shader name";
        }
