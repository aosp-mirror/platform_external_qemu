...
protected:
    virtual void SetUp() override {
        ...
        if (mUseSubWindow) {
            ...
            EXPECT_NE(nullptr, mWindow->getFramebufferNativeWindow());
            mWindow->messageLoop();

            EXPECT_TRUE(FrameBuffer::initialize(width, height, true /* useSubWindow */, !useHostGpu /* egl2egl */));
            mFb = FrameBuffer::getFB();
            EXPECT_NE(nullptr, mFb);

            mFb->setupSubWindow(
                (FBNativeWindowType)(uintptr_t)mWindow->getFramebufferNativeWindow(),
                0, 0, width, height, width, height, 1.0, 0, false);

            mWindow->messageLoop();
        } else {
            EXPECT_TRUE(FrameBuffer::initialize(width, height, false /* useSubWindow */, !useHostGpu /* egl2egl */));
            mFb = FrameBuffer::getFB();
            EXPECT_NE(nullptr, mFb);
            mFb->finalize();
        }
