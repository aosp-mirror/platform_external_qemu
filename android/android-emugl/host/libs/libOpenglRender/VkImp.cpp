...
static std::unordered_map<VkInstance, VkInstanceDispatch*> sInstanceDispatches;
static std::unordered_map<VkPhysicalDevice, VkInstanceDispatch*> sInstanceDispatchesPhysicalDevice;
static std::unordered_map<VkDevice, VkDeviceDispatch*> sDeviceDispatches;
static std::unordered_map<VkQueue, VkDeviceDispatch*> sDeviceDispatchesQueue;
static std::unordered_map<VkCommandBuffer, VkDeviceDispatch*> sDeviceDispatchesCmdBuf;

static VkInstanceDispatch* s_ivk(VkInstance instance) {
    if (sInstanceDispatches.find(instance) == sInstanceDispatches.end()) {
        sInstanceDispatches[instance] = new VkInstanceDispatch;
        *sInstanceDispatches[instance] = initVkInstanceDispatch(instance);
    }
    return sInstanceDispatches[instance];
}

static VkInstanceDispatch* s_ivkPhysdev_new(VkInstance instance, VkPhysicalDevice physdev) {
    sInstanceDispatchesPhysicalDevice[physdev] = sInstanceDispatches[instance];
    return sInstanceDispatchesPhysicalDevice[physdev];
}

static VkInstanceDispatch* s_ivkPhysdev(VkPhysicalDevice physdev) {
    return sInstanceDispatchesPhysicalDevice[physdev];
}

static void s_ivkPhysdev_new_multi(VkInstance instance, uint32_t count, VkPhysicalDevice* devices) {
    if (!devices) return;
...
static VkDeviceDispatch* s_dvk_new(VkPhysicalDevice physdev, VkDevice device) {
    if (sDeviceDispatches.find(device) == sDeviceDispatches.end()) {
        sDeviceDispatches[device] = new VkDeviceDispatch;
        *sDeviceDispatches[device] = initVkDeviceDispatch(*sInstanceDispatchesPhysicalDevice[physdev], device);
    }
    return sDeviceDispatches[device];
}

static VkDeviceDispatch* s_dvk(VkDevice device) {
    return sDeviceDispatches[device];
}

static VkDeviceDispatch* s_dvkQueue_new(VkDevice device, VkQueue queue) {
    sDeviceDispatchesQueue[queue] = sDeviceDispatches[device];
    return sDeviceDispatchesQueue[queue];
}

static VkDeviceDispatch* s_dvkQueue(VkQueue queue) {
    return sDeviceDispatchesQueue[queue];
}

static VkDeviceDispatch* s_dvkCmdBuf_new(VkDevice device, VkCommandBuffer cmdbuf) {
    sDeviceDispatchesCmdBuf[cmdbuf] = sDeviceDispatches[device];
    return sDeviceDispatchesCmdBuf[cmdbuf];
}

static void s_dvkCmdBuf_new_multi(VkDevice device, const VkCommandBufferAllocateInfo* pAllocateInfo, VkCommandBuffer* pCommandBuffers) {
...
static VkDeviceDispatch* s_dvkCmdBuf(VkCommandBuffer cmdbuf) {
    return sDeviceDispatchesCmdBuf[cmdbuf];
}

static void sMapInstanceDispatch(VkInstance instance) {
    if (sInstanceDispatches.find(instance) == sInstanceDispatches.end()) {
        sInstanceDispatches[instance] = new VkInstanceDispatch;
        *sInstanceDispatches[instance] = initVkInstanceDispatch(instance);
    }
...
static VkResult s_vkMapMemoryAEMU(void* self, VkDevice device, VkDeviceMemory memory, VkDeviceSize offset, VkDeviceSize size, VkMemoryMapFlags flags, uint32_t datalen, char* data) {
    fprintf(stderr, "%s: call", __func__);
    return VK_SUCCESS;
}
...
static void s_vkDestroyInstance(VkInstance instance, const VkAllocationCallbacks* pAllocator) {
    fprintf(stderr, "%s: call", __func__);
    vkGlobalDispatcher()->vkDestroyInstance(instance, pAllocator);
}

static VkResult s_vkEnumeratePhysicalDevices(VkInstance instance, u32* pPhysicalDeviceCount, VkPhysicalDevice* pPhysicalDevices) {
    fprintf(stderr, "%s: call", __func__);
    VkResult res = s_ivk(instance)->vkEnumeratePhysicalDevices(instance, pPhysicalDeviceCount, pPhysicalDevices);
    if (res == VK_SUCCESS) { s_ivkPhysdev_new_multi(instance, *pPhysicalDeviceCount, pPhysicalDevices); }
    return res;
}

static void s_vkGetPhysicalDeviceProperties(VkPhysicalDevice physicalDevice, VkPhysicalDeviceProperties* pProperties) {
    fprintf(stderr, "%s: call", __func__);
    s_ivkPhysdev(physicalDevice)->vkGetPhysicalDeviceProperties(physicalDevice, pProperties);
}
