...
static VkResult s_vkCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, u32 createInfoCount, const VkGraphicsPipelineCreateInfo* pCreateInfos, const VkAllocationCallbacks* pAllocator, VkPipeline* pPipelines) {
    fprintf(stderr, "%s: call createInfoCount %u\n", __func__, createInfoCount);

    // guest->host handle conversions
    VkDevice host_device = VkVirtualHandleGetHost<VkDevice>((uint32_t)(uintptr_t)device);
    VkPipelineCache host_pipelineCache = VkVirtualHandleGetHost<VkPipelineCache>((uint32_t)(uintptr_t)pipelineCache);
    fprintf(stderr, "%s: host pipeline cahce? %p\n", __func__, host_pipelineCache);
    ...
    fprintf(stderr, "%s: stageCount %u stages %p vinput %p inputassembly %p tessel %p viewp %p raster %p multisample %p depthstencil %p colorblend %p dyn %p\n", __func__,
    host_pCreateInfos[0].stageCount,
    host_pCreateInfos[0].pStages,
    host_pCreateInfos[0].pVertexInputState,
    host_pCreateInfos[0].pInputAssemblyState,
    host_pCreateInfos[0].pTessellationState,
    host_pCreateInfos[0].pViewportState,
    host_pCreateInfos[0].pRasterizationState,
    host_pCreateInfos[0].pMultisampleState,
    host_pCreateInfos[0].pDepthStencilState,
    host_pCreateInfos[0].pColorBlendState,
    host_pCreateInfos[0].pDynamicState);
