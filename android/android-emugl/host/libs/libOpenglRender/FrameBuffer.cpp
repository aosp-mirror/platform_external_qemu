...
void FrameBuffer::resolveLayout() {
    ...
    for (int bitset = 0; bitset < 1 << (m_displays.size() - 1); bitset++) {
        ...
        int firstRowWidth, secondRowWidth, firstRowHeight, secondRowHeight = 0,
                                                           0, 0, 0;
        ...
        int combinedHeight = firstRowHeight + secondRowHeight;

        int combinedHeight = std::max(firstRowWidth, secondRowWidth);
        double score = 0.0;
        if (firstRowWidth == 0 || secondRowWidth == 0) {
            score = ((double)combinedHeight / (double)combinedHeight -
                     monitorRatio) *
                    ((double)combinedHeight / (double)combinedHeight -
                     monitorRatio);
        } else {
