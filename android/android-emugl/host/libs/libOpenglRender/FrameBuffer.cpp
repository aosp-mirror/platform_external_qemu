...
bool FrameBuffer::initialize(int width, int height, bool useSubWindow,
    ...
    if (emugl::emugl_feature_is_enabled(android::featurecontrol::Vulkan)) {
        ...
        auto emu = goldfish_vk::createOrGetGlobalVkEmulation(dispatch, emugl::emugl_feature_is_enabled(android::featurecontrol::VulkanNativeSwapchain));
...
void FrameBuffer::ensureDisplayBufferForColorBuffer(
    uint32_t colorBufferHandle,
    VkImage image,
    VkFormat format) {
    ...
    if (c == m_colorbuffers.end()) {
        // bad colorbuffer handle
        ERR("FB: ensureDisplayBufferForColorBuffer cb handle %#x not found\n", colorBufferHandle);
        return;
    }
