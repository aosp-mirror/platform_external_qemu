...
FrameBuffer::FrameBuffer(int p_width, int p_height, bool useSubWindow)
    ...
    emugl::get_emugl_window_operations().setMultiDisplay(0, 0, 0, getWidth(), getHeight(), true, 0);
}
...
bool FrameBuffer::onLoad(Stream* stream,
    ...
    std::unordered_map<uint32_t, DisplayInfo> displays_onLoad;
    loadCollection(stream, &displays_onLoad,
...
int FrameBuffer::setDisplayColorBufferLocked(uint32_t displayId, uint32_t colorBuffer) {
    ...
    AutoLock mutex(m_lock);
    return setDisplayColorBuffer(displayId, colorBuffer);
}
...
void FrameBuffer::unlockMultiDisplayOnLoad(void) {
    m_multiDisplayOnLoadLock.unlock();
}
