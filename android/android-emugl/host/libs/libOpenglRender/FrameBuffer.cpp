    ...
    if (m_onPost) {
        if (m_enableAsyncReadback) {
            ...
            if (!m_readbackWorker && !m_readbackBuffers[0]) {
                ...
                // Do async mapping of the other buffer and glreadpixels of this buffer
                // and wait until both finish.
                m_readbackThread.enqueue({ReadbackCmd::Readback,
                        buf2,
                        m_readbackBufferSize, m_fbImage, [this](void* pixels) {
                        m_onPost(m_onPostContext, m_framebufferWidth, m_framebufferHeight, -1, GL_RGBA, GL_UNSIGNED_BYTE, (unsigned char*)pixels); }});
                cb->readbackAsync(buf1);
                m_readbackThread.finish();
                ...
                m_readbackCount++;
                // fprintf(stderr, "%s: stopwatch elapsed %lu us\n", __func__, sw.elapsedUs());
            }
        } else {
