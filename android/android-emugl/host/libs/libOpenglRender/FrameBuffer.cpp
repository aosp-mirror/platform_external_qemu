...
bool FrameBuffer::post(HandleType p_colorbuffer, bool needLockAndBind) {
    ...
    bool ret = false;
    bool waitReadbackWorkerInitialized = false;
        ...
        if (m_enableAsyncReadback) {
            ...
            if (!m_readbackWorker) {
                if (!m_readbackThread.isStarted()) {
                    waitReadbackWorkerInitialized = true;
                    m_readbackWorker = new ReadbackWorker(cb->getWidth(), cb->getHeight());
...
EXIT:
    ...
    if (waitReadbackWorkerInitialized) {
        fprintf(stderr, "%s: wait init\n", __func__);
        m_readbackThread.finish();
        fprintf(stderr, "%s: wait init done\n", __func__);
    }
    return ret;
}
