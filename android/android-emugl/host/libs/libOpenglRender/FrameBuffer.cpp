...
FrameBuffer::sendReadbackWorkerCmd(const Readback& readback) {
    ...
    case ReadbackCmd::Init:
        m_readbackWorker = new ReadbackWorker();
        return WorkerProcessingResult::Continue;
    case ReadbackCmd::Readback:
        m_readbackWorker->readback(readback.bufferId, readback.bytes, readback.callback);
        return WorkerProcessingResult::Continue;
        ...
        if (m_enableAsyncReadback) {
            ...
            if (!m_readbackWorker && !m_readbackBuffers[0]) {
                ...
                m_readbackThread.enqueue({ReadbackCmd::Readback,
                        buf2,
                        m_readbackBufferSize,
                        [this](void* pixels) {
                            m_onPost(m_onPostContext,
                                     m_framebufferWidth, m_framebufferHeight, -1,
