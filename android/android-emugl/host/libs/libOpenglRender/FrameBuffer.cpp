...
/*             if (!iter.second.readbackThread) {
                auto p = new android::base::WorkerThread<Readback>
                    ([](Readback&& readback) {
                        return sendReadbackWorkerCmd(readback);
                    });
                iter.second.readbackThread.reset(p);   
...
void FrameBuffer::getPixels(void* pixels, uint32_t bytes) {
    ...
    iter->second.readbackThread->enqueue({ ReadbackCmd::GetPixels, 0, 0, pixels, bytes });
    iter->second.readbackThread->waitQueuedItems();
}
