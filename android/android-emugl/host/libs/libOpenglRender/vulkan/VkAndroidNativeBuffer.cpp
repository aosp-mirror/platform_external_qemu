...
VkResult syncImageToColorBuffer(
    ...
    *pNativeFenceFd = -1;
    ...
    // If GL texture, just do an empty submit
    // if (anbInfo->isGlTexture) {
    //     fprintf(stderr, "%s: is gl texture, empty submit\n", __func__);
    ...
    //     vk->vkWaitForFences(anbInfo->device, 1, &queueState.fence, VK_TRUE,
    //                         ANB_MAX_WAIT_NS);
    ...
    // } else {
    //     fprintf(stderr, "%s: is not gl texture, empty submit\n", __func__);
    // }
