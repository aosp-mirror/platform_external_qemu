...
bool setupVkColorBuffer(uint32_t colorBufferHandle,
    ...
    //frameworkFormat = FrameworkFormat::FRAMEWORK_FORMAT_GL_COMPATIBLE;
...
bool updateVkImageFromColorBuffer(uint32_t colorBufferHandle) {
    ...
    readRes = FrameBuffer::getFB()->
        readColorBufferContents(
            ...
            sVkEmulation->staging.memory.mappedPtr);
    uint8_t* ptr = (uint8_t*)(sVkEmulation->staging.memory.mappedPtr);
    printf("vk decoded frame %x %x %x %x\n", ptr[0], ptr[1], ptr[2], ptr[3]);
    ...
    if (infoPtr->frameworkFormat == FrameworkFormat::FRAMEWORK_FORMAT_GL_COMPATIBLE) {
        ...
        if (infoPtr->frameworkFormat == FrameworkFormat::FRAMEWORK_FORMAT_YUV_420_888) {
            printf("pushing yuv data %x %x %x\n",
                ptr[0],
                ptr[infoPtr->extent.width * infoPtr->extent.height],
                ptr[infoPtr->extent.width * infoPtr->extent.height
