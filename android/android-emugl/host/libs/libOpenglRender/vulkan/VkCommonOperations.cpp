...
static bool getImageFormatExternalMemorySupportInfo(
    ...
    if (!sVkEmulation->instanceSupportsExternalMemoryCapabilities) {
        ...
        if (res != VK_SUCCESS) {
            if (res == VK_ERROR_FORMAT_NOT_SUPPORTED) {
                ...
                printf("%s: vkGetPhysicalDeviceImageFormatProperties query failed with %d "
        ...
        printf("%s: Supported (not externally): %s %s %s %s\n", __func__,
    ...
    if (res != VK_SUCCESS) {
        if (res == VK_ERROR_FORMAT_NOT_SUPPORTED) {
            ...
            printf("%s: vkGetPhysicalDeviceImageFormatProperties2KHR query "
                   "failed "
                   "for format 0x%x type 0x%x usage 0x%x flags 0x%x\n",
    ...
    printf("%s: Supported: %s %s %s %s extSupported %d requiresDedicated "
           "%d\n",
           __func__, string_VkFormat(info->format),
...
VkEmulation* createOrGetGlobalVkEmulation(VulkanDispatch* vk) {
    ...
    for (size_t i = 0; i < sVkEmulation->imageSupportInfo.size(); ++i) {
        getImageFormatExternalMemorySupportInfo(
                vk, sVkEmulation->physdev, &sVkEmulation->imageSupportInfo[i]);
    }
    ...
    if (!allocExternalMemory(vk, &sVkEmulation->staging.memory, false /* not external */)) {
...
void teardownGlobalVkEmulation(VulkanDispatch* vk) {
    ...
    freeExternalMemory(vk, &sVkEmulation->staging.memory);

    vk->vkDestroyDevice(sVkEmulation->device, nullptr);
    vk->vkDestroyInstance(sVkEmulation->instance, nullptr);
}

// Precondition: sVkEmulation has valid device support info
bool allocExternalMemory(VulkanDispatch* vk, VkEmulation::ExternalMemoryInfo* info, bool actuallyExternal) {
...
bool setupVkColorBuffer(VulkanDispatch* vk, uint32_t colorBufferHandle) {
    ...
    if (createRes != VK_SUCCESS) {
        printf("Failed to create Vulkan image for ColorBuffer 0x%x\n", colorBufferHandle);
        return false;
    }
    ...
    printf("External Vulkan image created for ColorBuffer 0x%x. Reqs: size 0x%x typeBits 0x%x\n",
           colorBufferHandle, (unsigned int)res.memReqs.size, res.memReqs.memoryTypeBits);

    res.memory.size = res.memReqs.size;
    res.memory.typeIndex = lastGoodTypeIndex(res.memReqs.memoryTypeBits);

    printf("Selected type index: %u\n", res.memory.typeIndex);

    bool allocRes = allocExternalMemory(vk, &res.memory);

    if (!allocRes) {
        printf("Failed to allocate ColorBuffer with Vulkan backing\n");
    }
