...
VkEmulation* createOrGetGlobalVkEmulation(VulkanDispatch* vk) {
    ...
    for (int i = 0; i < physdevCount; ++i) {
        ...
        if (sVkEmulation->instanceSupportsExternalSemaphoreCapabilities) {
            ...
            fprintf(stderr, "%s: device supports ext semaphores? %d\n", __func__,
                    deviceInfos[i].supportsExternalSemaphores);
        }
...
bool setupVkColorBuffer(uint32_t colorBufferHandle,
    ...
    if (sVkEmulation->deviceInfo.supportsExternalSemaphores &&
        ...
        LOG(ERROR) << "Failed to import external semaphore for ColorBuffer to GL";
    }
