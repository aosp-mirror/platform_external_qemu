...
bool allocExternalMemory(VulkanDispatch* vk,
    ...
    while (!memoryAllocated) {
        ...
        if (  // don't care about alignment (e.g. device-local memory)
                ...
                // mappedPtr is already aligned to page
                mappedPtrPageOffset == 0u ||
                // clients can set a offset when vkBindImageMemory so that it
                // will be aligned to page.
                deviceAlignment.value() < mappedPtrPageOffset) {
            // allocation success.
...
bool setupVkColorBuffer(uint32_t colorBufferHandle,
    ...
    // Calculated bindOffset is supposed to be always compatible with device
    // alignment requirements.
    //
    // For images with deviceAlignment requirement of more than 4KB, we require
    // the host pointer aligned to 4KB, so bindOffset will be 0; for images
    // with smaller alignment requirement, we require the host pointer have
    // the same alignment as its device alignment so that this calculated
    // bindOffset is compatible with device.
    assert(res.memory.bindOffset % res.memReqs.alignment == 0);
