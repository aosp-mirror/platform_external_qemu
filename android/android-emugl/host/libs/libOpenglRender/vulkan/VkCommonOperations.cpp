...
constexpr size_t kGuestPageBits = 12;
constexpr size_t kGuestPageSize = 1u << kGuestPageBits;
constexpr size_t kGuestPageOffsetMask = kGuestPageSize - 1;
...
bool allocExternalMemory(VulkanDispatch* vk,
    ...
    info->actualSize =
        (info->size + 2 * kGuestPageSize - 1) /
        kGuestPageSize * kGuestPageSize;
    ...
    while (!memoryAllocated) {
        ...
        uint64_t mappedPtrPageOffset =
                reinterpret_cast<uint64_t>(info->mappedPtr) % kGuestPageSize;
        ...
        if (  // don't care about alignment (e.g. device-local memory)
                ...
                // |kGuestPageSize - mappedPtrPageOffset| in vkBindImageMemory to
                // make it aligned to page and compatible with device
                // requirements.
                (kGuestPageSize - mappedPtrPageOffset) % deviceAlignment.value() == 0) {
            // allocation success.
...
bool setupVkColorBuffer(uint32_t colorBufferHandle,
    ...
    res.memory.pageOffset =
            reinterpret_cast<uint64_t>(res.memory.mappedPtr) % kGuestPageSize;
    res.memory.bindOffset =
            res.memory.pageOffset ? kGuestPageSize - res.memory.pageOffset : 0u;
...
int32_t mapGpaToBufferHandle(uint32_t bufferHandle, uint64_t gpa) {
    ...
    memoryInfoPtr->sizeToPage = ((memoryInfoPtr->size +
                                  memoryInfoPtr->pageOffset + kGuestPageSize - 1) >>
                                 kGuestPageBits)
                                << kGuestPageBits;
