...
void VkReconstruction::save(android::base::Stream* stream) {
    ...
    mApiTrace.forEachLiveEntry([stream](bool live, uint64_t handle, ApiInfo& info) {
         stream->putBe32(info.opCode);
         stream->putBe32(info.traceBytes);
         android::base::saveBufferRaw(stream, (char*)info.trace.data(), info.traceBytes);
    });
}
