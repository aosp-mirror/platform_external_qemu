...
void VkReconstruction::save(android::base::Stream* stream) {
    ...
    mHandleReconstructions.forEachLiveComponent(
        ...
        if (backDeps.find(entityHandle) == backDeps.end()) {
            fprintf(stderr, "%s: found root: 0x%llx\n", __func__, (unsigned long long)entityHandle);
        ...
        for (auto handle : topoOrder) {
            ...
            for (auto apiHandle : item->apiRefs) {
                ...
                if (uniqApiRefsToTopoOrder.find(apiHandle) == uniqApiRefsToTopoOrder.end()) {
                    fprintf(stderr, "%s: level %zu: 0x%llx api ref: 0x%llx\n", __func__, topoLevel, (unsigned long long)handle, (unsigned long long)apiHandle);
    ...
    size_t totalApiTraceSize = 4; // 4 bytes to store size of created handles
    size_t totalCreatedHandlesSize = 0;

    for (size_t i = 0; i < topoLevel; ++i) {
        for (auto apiHandle : uniqApiRefsByTopoOrder[i]) {
            fprintf(stderr, "%s: saving api handle 0x%llx\n", __func__, apiHandle);
            auto item = mApiTrace.get(apiHandle);
            size_t createdHandlesSize = item->createdHandles.size() * sizeof(uint64_t);
            totalCreatedHandlesSize += createdHandlesSize;
            totalApiTraceSize += createdHandlesSize;
        }
    }
    ...
    fprintf(stderr, "%s: total api trace size: %zu created handles size: %zu\n", __func__, totalApiTraceSize, totalCreatedHandlesSize);

    stream->putBe32(totalApiTraceSize);
    stream->putBe32(totalCreatedHandlesSize);

    for (size_t i = 0; i < topoLevel; ++i) {
        for (auto apiHandle : uniqApiRefsByTopoOrder[i]) {
            auto item = mApiTrace.get(apiHandle);
            for (auto createdHandle : item->createdHandles) {
                stream->putBe64(createdHandle);
            }
        }
    }
...
void VkReconstruction::load(android::base::Stream* stream) {
    fprintf(stderr, "VkReconstruction::%s: assuming VkDecoderGlobalState has been cleared for loading already\n", __func__);
    fprintf(stderr, "VkReconstruction::%s: clearing current api trace and handle reconstructions\n", __func__);
    mApiTrace.clear();
    mHandleReconstructions.clear();

    fprintf(stderr, "VkReconstruction::%s: loading trace\n", __func__);
    // Load what is to be decoded.
    auto res = android::base::loadBuffer(stream, &mLoadedTrace);
    fprintf(stderr, "%s: loaded trace size: %zu res: %d\n", __func__, mLoadedTrace.size(), res);
    ...
    fprintf(stderr, "VkReconstruction::%s: creating decoder for loading\n", __func__);
    ...
    uint32_t* loadedTraceWords = (uint32_t*)mLoadedTrace.data();
    for (uint32_t i = 0; i < 5; ++i) {
        fprintf(stderr, "%s: %u\n", __func__, loadedTraceWords[i]);
    }

    fprintf(stderr, "VkReconstruction::%s: decoding trace\n", __func__);

    decoderForLoading.decode(mLoadedTrace.data(), mLoadedTrace.size(), &trivialStream);

    fprintf(stderr, "VkReconstruction::%s: finished decoding trace\n", __func__);
}
