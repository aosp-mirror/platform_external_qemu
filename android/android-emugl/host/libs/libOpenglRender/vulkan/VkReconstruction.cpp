...
void VkReconstruction::save(android::base::Stream* stream) {
    ...
    for (size_t i = 0; i < topoLevel; ++i) {
        for (auto apiHandle : uniqApiRefsByTopoOrder[i]) {
            ...
            totalApiTraceSize += 4; // buffer size of created handles
            totalApiTraceSize += item->createdHandles.size() * sizeof(uint64_t); // created handles
    ...
    for (size_t i = 0; i < topoLevel; ++i) {
        for (auto apiHandle : uniqApiRefsByTopoOrder[i]) {
            auto item = mApiTrace.get(apiHandle);
            // Buffer where created handles are recorded
            android::base::saveBufferRaw(stream, (char*)item->createdHandles.data(), item->createdHandles.size() * sizeof(uint64_t));
