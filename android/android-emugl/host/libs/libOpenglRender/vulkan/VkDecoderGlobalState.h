...
#define BOXED_DISPATCHABLE_UNWRAP_IMPL(type_name) \
    MAKE_HANDLE_MAPPING_FOREACH(type_name, \
        if (handles[i]) { handles[i] = VkDecoderGlobalState::get()->unbox_##type((uint64_t)(uintptr_t)handles[i]); } else { handles[i] = nullptr; } ;, \
        if (handles[i]) { handle_u64s[i] = (uint64_t)VkDecoderGlobalState::get()->unbox_##type((uint64_t)(uintptr_t)handles[i]); } else { handle_u64s[i] = 0; }, \
        if (handle_u64s[i]) { handles[i] = VkDecoderGlobalState::get()->unbox_##type(handle_u64s[i]); } else { handles[i] = nullptr; })
