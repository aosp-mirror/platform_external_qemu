...
{
    fprintf(stderr, "%s: begin (host)\n", __func__);
    vkStream->write((VkStructureType*)&forMarshaling->sType, sizeof(VkStructureType));
    fprintf(stderr, "%s: begin pNext (host)\n", __func__);
    marshal_extension_struct(vkStream, forMarshaling->pNext);
    fprintf(stderr, "%s: end pNext (host)\n", __func__);
    marshal_VkPhysicalDeviceFeatures(vkStream, (VkPhysicalDeviceFeatures*)(&forMarshaling->features));
    fprintf(stderr, "%s: end (host)\n", __func__);
}
    ...
    VkPhysicalDeviceFeatures2* forUnmarshaling)
{
    ...
    size_t pNext_size;
    pNext_size = vkStream->getBe32();
    fprintf(stderr, "%s: guest pNext size %zu (host)\n", __func__, pNext_size);
    forUnmarshaling->pNext = nullptr;
    if (pNext_size)
    {
        vkStream->alloc((void**)&forUnmarshaling->pNext, sizeof(VkStructureType));
...
void marshal_extension_struct(
    ...
    const void* structExtension)
{
    fprintf(stderr, "%s: call for %p (host)\n", __func__, structExtension);
    VkInstanceCreateInfo* structAccess = (VkInstanceCreateInfo*)(structExtension);
    size_t currExtSize = goldfish_vk_extension_struct_size(structExtension);
    fprintf(stderr, "%s: write back %zu\n", __func__, currExtSize);
    vkStream->putBe32(currExtSize);
    if (!currExtSize)
    {
        fprintf(stderr, "%s: unknown struct (host). skip, recurse (top) ??? struct ptr: %p\n", __func__, structExtension);
        if (structAccess && structAccess->pNext)
        {
            fprintf(stderr, "%s: unknown struct (host). skip, recurse!!!!!!!!!!!!!!!! (top)\n", __func__);
    ...
    fprintf(stderr, "%s: do actual stuff\n", __func__);
    switch(structType)
    {
#ifdef VK_VERSION_1_1
        ...
        default:
        {
            fprintf(stderr, "%s: unknown struct (host). skip?\n", __func__);
            if (structAccess && structAccess->pNext)
            {
                fprintf(stderr, "%s: unknown struct (host). skip, recurse\n", __func__);
                marshal_extension_struct(vkStream, (void*)structAccess->pNext);
            }
            break;
        }
...
void unmarshal_extension_struct(
    ...
    void* structExtension_out)
{
    fprintf(stderr, "%s: call for %p (host)\n", __func__, structExtension_out);
    VkInstanceCreateInfo* structAccess = (VkInstanceCreateInfo*)(structExtension_out);
    size_t currExtSize = goldfish_vk_extension_struct_size(structExtension_out);
    if (!currExtSize)
    {
        fprintf(stderr, "%s: host says 0 for ext size. struct %p\n", __func__, structExtension_out);
        if (structAccess && structAccess->pNext)
        {
