    ...
    const void* structExtension)
{
    ...
    if (!currExtSize && structExtension) {
        // unknown struct extension.
        marshal_extension_struct(vkStream, (void*)structAccess->pNext);
        return;
    } else {
        // either known that the struct extension is nullptr,
        // or it's a known struct extension
        vkStream->putBe32(currExtSize);
        if (!currExtSize) {
            // stop if that was a nullptr
            return;
        }
    }

    // Known extension structs
    ...
    switch(structType)
    {
        ...
        default:
        {
            fprintf(stderr, "%s: fatal: this path only handles known extension structs\n", __func__);
            abort();
        }
    }
}
    ...
    void* structExtension_out)
{
    ...
    if (!currExtSize && structExtension_out) {
        // unmarshaling, but the guest's pointer to extension struct is not know
        // need to skip
        // Look at its pNext field, could have known stuff down the line.
        unmarshal_extension_struct(vkStream, (void*)structAccess->pNext);
        return;
    } else {
        // receive size from host
        // size_t hostSize = vkStream->getBe32();
        if (!currExtSize) return;
    }
