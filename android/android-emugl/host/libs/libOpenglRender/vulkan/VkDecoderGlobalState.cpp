...
public:
    ...
    VkResult on_vkCreateImage(
        ...
        if (cmpInfo.isCompressed && deviceInfoIt->second.emulateTextureEtc2) {
            printf("creating decompressed image\n");
    ...
    VkResult on_vkBindImageMemory(android::base::Pool* pool,
        ...
        printf("emulateTextureEtc2 %d isCompressed %d\n",
               deviceInfoIt->second.emulateTextureEtc2, cmp.isCompressed);
        if (!deviceInfoIt->second.emulateTextureEtc2 || !cmp.isCompressed) {
            return VK_SUCCESS;
        }
        // TODO: clean up cmp.decompImgMemory if it already exist
        printf("bind decompressed image memory %p\n", cmp.decompImgMemory);
        if (cmp.decompImgMemory == 0) {
    ...
    VkResult on_vkCreateImageView(
        ...
        if (deviceInfoIt->second.emulateTextureEtc2) {
            ...
            if (cmpInfo.isCompressed) {
                ...
                pCreateInfo = &createInfo;
                printf("create compressed image view\n");
            }
        }
    ...
    void on_vkCmdCopyBufferToImage(
        ...
        {
            printf("vkCmdCopyBufferToImage size compatible regions\n");
    ...
    VkResult on_vkBeginCommandBuffer(
        ...
        auto vk = dispatch_VkCommandBuffer(boxed_commandBuffer);
        mCmdBufferInfo[commandBuffer].preprocessFuncs.clear();
        printf("clearing command buffer %p\n", commandBuffer);
...
private:
    ...
    void decompressBufferToImage(CompressedImageInfo cmp,
        ...
        printf("decodedPixelSize %d__ srcBuffer %p\n", decodedPixelSize, srcBuffer);

        auto srcBufferInfoIt = mBufferInfo.find(srcBuffer);
        if (srcBufferInfoIt == mBufferInfo.end()) {
            printf("srcBuffer not found!\n");
            return;
        }
                    ...
                    for (int w = 0; w < dstRegion.imageExtent.width; w++) {
                        ...
                        switch (cmp.compFormat) {
                            ...
                            case VK_FORMAT_ETC2_R8G8B8_SRGB_BLOCK:
                                ...
                                dstPixel[0] = 11;
                                memcpy(dstPixel, srcPixel, decodedPixelSize);
                                //if ((int64_t)(dstPixel - dstPtr + decodedPixelSize) > 
                                break;
                            default:
