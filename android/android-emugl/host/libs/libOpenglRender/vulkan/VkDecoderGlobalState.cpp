...
public:
    ...
    VkResult on_vkBindImageMemory(android::base::Pool* pool,
        ...
        if (cmp.sizeCompImgMemory) {
            return VK_SUCCESS;
        }
        VkPhysicalDevice physicalDevice = mDeviceInfo[device].physicalDevice;
        VkMemoryRequirements memRequirements;
        vk->vkGetImageMemoryRequirements(device, cmp.sizeCompImg,
                                         &memRequirements);
        int32_t memIdx =
        ...
        cmp.sizeCompImgMemory = imgMemory;
        vk->vkBindImageMemory(device, cmp.sizeCompImg, imgMemory, 0);
