    ...
    VkResult on_vkCreateImage(
        ...
        if (cmpInfo.isCompressed && deviceInfoIt->second.emulateTextureEtc2) {
            if (VK_SUCCESS != vk->vkCreateImage(device, &decompInfo, pAllocator,
                                                &cmpInfo.decompImg)
                || VK_SUCCESS != cmpInfo.initDecomp(vk, device, *pImage)) {
                fprintf(stderr,
    ...
    void on_vkCmdCopyBufferToImage(
        ...
        // Handle size-compatible compressed data
        {
            ...
            for (uint32_t r = 0; r < regionCount; r++) {
                if (pRegions[r].imageSubresource.mipLevel >= cmp.mipLevels) {
                    continue;
                }
    ...
    void on_vkCmdPipelineBarrier(
        ...
        for (uint32_t i = 0; i < imageMemoryBarrierCount; i++) {
            ...
            if (barrier.newLayout != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
                ...
                fprintf(stderr, "WARNING: unexpected usage to transfer "
                    "compressed image layout from %d to %d\n",
                    barrier.oldLayout, barrier.newLayout);
            }
            VkResult result = it->second.cmpInfo.initDecomp(vk,
                cmdBufferInfoIt->second.device,
                image);
            if (result != VK_SUCCESS) {
            ...
            preDecompBarriers[0] = barrier;
            preDecompBarriers[0].dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
            preDecompBarriers[0].newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

            preDecompBarriers[1] = barrier;
            ...
            vk->vkCmdPipelineBarrier(commandBuffer,
                    ...
                    0, nullptr,
                    2, preDecompBarriers);
            it->second.cmpInfo.cmdDecompress(vk, commandBuffer, image);

            VkImageMemoryBarrier postDecompBarrier = barrier;
...
#define _RETURN_ON_FAILURE(cmd)                                                \
    {                                                                          \
        ...
        if (VK_SUCCESS != result) {                                            \
            ...
            imageView =
                    createDefaultImageView(vk, device, image, sizeCompFormat);
        ...
        void cmdDecompress(goldfish_vk::VulkanDispatch* vk,
            ...
            preImageBarriers[0].subresourceRange.aspectMask =
                    VK_IMAGE_ASPECT_COLOR_BIT;
            ...
            // TODO: handle mipmap and other situations
            preImageBarriers[1].subresourceRange.aspectMask =
                    VK_IMAGE_ASPECT_COLOR_BIT;
            ...
            vk->vkCmdDispatch(commandBuffer, extent.width,
            ...
            // TODO: handle mipmap and other situations
            postImageBarrier[0].subresourceRange.aspectMask =
                    VK_IMAGE_ASPECT_COLOR_BIT;
            ...
            // VK_PIPELINE_BIND_POINT_COMPUTE, cmdBufferInfo.computePipeline);
        }
    };
