...
private:
    ...
    VkDecoderComponent<VkDevice, VkPhysicalDevice> mPhysicalDevices;
    // Keep the physical device memory properties around.
    VkDecoderComponent<VkPhysicalDevice, VkPhysicalDeviceMemoryProperties>
        mPhysicalDeviceMemoryProperties;
    // Queue family properties
    VkDecoderComponent<VkPhysicalDevice, std::vector<VkQueueFamilyProperties>>
        mPhysicalDeviceQueueFamilyProperties;

    VkDecoderComponent<VkDeviceMemory, MappedMemoryInfo> mMapInfo;

    // Images that correspond to gralloc buffers and thus
    // needing some kind of custom backing of some sort.
    VkDecoderComponent<VkDevice, DeviceStaging> mStaging;
    VkDecoderComponent<VkImage, NativeBufferInfo> mNativeBufferImages;
    // Correlate devices with queues
    VkDecoderComponent<VkDevice, std::vector<DeviceQueueInfo>> mDeviceQueues;
    VkDecoderComponent<VkQueue, VkDevice> mQueueToDevice;
};
