...
public:
    ...
    void initDescriptorSetInfoLocked(
        ...
        poolInfo->allocedSetsToBoxed[descriptorSet] = (VkDescriptorSet)descriptorSetPoolId;
...
#include "VkSubDecoder.cpp"
    ...
    VkDescriptorSet getOrAllocateDescriptorSetFromPoolAndId(
        ...
        if (setHandleInfo->underlying) {
            if (pendingAlloc) {
                ...
                *didAlloc = true;
                // fprintf(stderr, "%s: new free+alloc case. got %p\n", __func__, allocedSet);
            } else {
                *didAlloc = false;
                fprintf(stderr, "%s: existing on host and no pending alloc case. got %p\n", __func__, (VkDescriptorSet)(setHandleInfo->underlying));
            ...
            if (pendingAlloc) {
                ...
                initDescriptorSetInfoLocked(pool, setLayout, poolId, allocedSet);
                *didAlloc = true;
                // fprintf(stderr, "%s: new alloc w/o  case. got %p\n", __func__, allocedSet);
                return allocedSet;
            } else {
    ...
    void on_vkQueueCommitDescriptorSetUpdatesGOOGLE(
        ...
        for (uint32_t i = 0; i < descriptorSetCount; ++i) {
            ...
            fprintf(stderr, "%s: pool id: 0x%llx\n", __func__, (unsigned long long)poolId);
        ...
        if (didAlloc) {
            ...
            for (uint32_t i = 0; i < descriptorSetCount; ++i) {
                ...
                fprintf(stderr, "%s: starting index: %u\n", __func__, writeStartIndex);
                ...
                for (uint32_t j = writeStartIndex; j < writeEndIndex; ++j) {
                    ...
                    fprintf(stderr, "%s: use set to update @ %d: %p this write has dstbinding %u dstArrayStart %u count %u\n", __func__, j, setsToUpdate[i],
                            writeDescriptorSetsForHostDriver[j].dstBinding,
                            writeDescriptorSetsForHostDriver[j].dstArrayElement,
                            writeDescriptorSetsForHostDriver[j].descriptorCount);
                }
            }
            ...
            fprintf(stderr, "%s: skipping reassignment of dstSet in VkWriteDescriptorSet's as they should already be there.\n", __func__);
            this->on_vkUpdateDescriptorSetsImpl(
                pool, vk, device,
