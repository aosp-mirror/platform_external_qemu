...
public:
    ...
    VkResult on_vkCreateImage(
        ...
        if (cmpInfo.isCompressed && deviceInfoIt->second.emulateTextureEtc2) {
            ...
            /*if (VK_SUCCESS != vk->vkCreateImage(device, &sizeCompInfo, pAllocator,
                                                &cmpInfo.sizeCompImg)) {
                fprintf(stderr,
        ...
        if (info.anbInfo.image) {
            ...
            if (info.cmpInfo.isCompressed) {
                ...
                if (image != info.cmpInfo.sizeCompImg) {
                    vk->vkDestroyImage(device, info.cmpInfo.sizeCompImg, pAllocator);
                    vk->vkFreeMemory(device, info.cmpInfo.sizeCompImgMemory, nullptr);
                }
            }
    ...
    VkResult on_vkBindImageMemory(android::base::Pool* pool,
        ...
        return VK_SUCCESS;
        // TODO: Create and bind memory for size-compatible texture
    ...
    VkResult on_vkCreateImageView(
        ...
        if (deviceInfoIt->second.emulateTextureEtc2) {
            ...
            if (cmpInfo.isCompressed) {
                ...
                // TODO: lazy create size compatible image
            }
        }
    ...
    void on_vkCmdCopyBufferToImage(
        ...
        if (cmp.sizeCompImg) {
            ...
            for (uint32_t r = 0; r < regionCount; r++) {
                if (pRegions[r].imageSubresource.mipLevel >= cmp.mipLevels - 2) {
                    continue;
                }
            ...
            if (regions.size() > 0) {
                vk->vkCmdCopyBufferToImage(commandBuffer, srcBuffer, cmp.sizeCompImg,
