...
public:
    ...
    void on_vkDestroyInstance(VkInstance boxed_instance, const VkAllocationCallbacks* pAllocator) {
        ...
        while (it != mPhysicalDeviceToInstance.end()) {
            if (it->second == instance) {
                auto it2 = android::base::find(mPhysdevInfo, it->first);

                if (it2) {
                    delete_boxed_VkPhysicalDevice(it2->boxed, false /* not owned */);
                }
    ...
    VkResult on_vkAllocateCommandBuffers(
        ...
        for (uint32_t i = 0; i < pAllocateInfo->commandBufferCount; i++) {
            ...
            mCmdBufferInfo[pCommandBuffers[i]].boxed =
                new_boxed_VkCommandBuffer(pCommandBuffers[i], vk);
        }
        return result;
    }
...
private:
    ...
    void removeCommandBufferInfo(
        ...
        for (const auto& cmdBuffer : cmdBuffers) {
            auto cmdbufInfo = android::base::find(mCmdBufferInfo, cmdBuffer);
            if (cmdbufInfo) {
                delete_boxed_VkCommandBuffer(cmdbufInfo->boxed, false /* Not owned */);
            }
