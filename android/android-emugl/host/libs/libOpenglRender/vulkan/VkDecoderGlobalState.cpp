...
public:
    ...
    void on_vkDestroyImage(
        ...
        if (info.anbInfo.image) {
            ...
            if (info.cmpInfo.isCompressed) {
                ...
                for (const auto& buffer : cmpInfo.decompBuffer) {
                    vk->vkDestroyBuffer(device, buffer, nullptr);
                }
                for (const auto& memory : cmpInfo.decompMemory) {
                ...
                vk->vkFreeMemory(device, cmpInfo.sizeCompImgMemory, nullptr);
                ...
                if (imageInfoIt->second.cmpInfo.sizeCompImgs.size()) {
                    ...
                    createInfo.image =
                            imageInfoIt->second.cmpInfo
                                    .sizeCompImgs[pCreateInfo->subresourceRange
                                                          .baseMipLevel];
    ...
    void on_vkCmdCopyImageToBuffer(android::base::Pool* pool,
        ...
        for (uint32_t r = 0; r < regionCount; r++) {
            ...
            region.imageExtent.width = (region.imageExtent.width + 3) / 4;
            region.imageExtent.height = (region.imageExtent.height + 3) / 4;
    ...
    void on_vkCmdPipelineBarrier(
        ...
        for (uint32_t i = 0; i < imageMemoryBarrierCount; i++) {
            ...
            // srcBarrier.srcAccessMask, srcBarrier.dstAccessMask); printf("old
            // layout %d new layout %d\n", srcBarrier.oldLayout,
            // srcBarrier.newLayout); if
...
#define _RETURN_ON_FAILURE(cmd)                                                \
    ...
    void lazyCreateSizeCompImg(goldfish_vk::VulkanDispatch* vk,
        ...
        for (size_t i = 0; i < cmpInfo->sizeCompImgs.size(); i++) {
            vk->vkBindImageMemory(device, cmpInfo->sizeCompImgs[i], imgMemory,
                                  memoryOffset[i]);
        }
    }
