...
#define _RETURN_ON_FAILURE(cmd)                                                \
            ...
            printf("load shader source success\n");
        ...
        void cmdDecompress(goldfish_vk::VulkanDispatch* vk,
            ...
            if (isEtc2) {
                ...
                printf("pusing astc srgb %d smallBlock %d block width %d block height %d\n",
                    pushConstant.sRGB, pushConstant.smallBlock, pushConstant.blockSize[0], pushConstant.blockSize[1]);
                if (extent.depth > 1) {
                    // 3D texture
                    pushConstant.baseLayer = 0;
                    dispatchZ = extent.depth;
