...
public:
    ...
    VkResult on_vkFreeDescriptorSets(
        ...
        auto res = vk->vkFreeDescriptorSets(device, descriptorPool, descriptorSetCount, pDescriptorSets);

        if (res == VK_SUCCESS) {
            AutoLock lock(mLock);

            for (uint32_t i = 0; i < descriptorSetCount; ++i) {
                auto pool = android::base::find(mDescriptorSetToPool, pDescriptorSets[i]);
                ...
                auto descSetAllocedEntry = android::base::find(poolInfo->allocedSetsToBoxed, pDescriptorSets[i]);
