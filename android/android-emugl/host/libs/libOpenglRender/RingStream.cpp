...
static bool getAssistThreadEnabledFromEnv() {
    auto threadEnabled = System::getEnvironmentVariable("ANDROID_EMUGL_RING_STREAM_ASSIST_THREAD");
    if (threadEnabled == "1") return true;
    return false;
}

static bool getBenchmarkEnabledFromEnv() {
    auto threadEnabled = System::getEnvironmentVariable("ANDROID_EMUGL_RENDERTHREAD_STATS");
    if (threadEnabled == "1") return true;
    return false;
}
...
RingStream::RingStream(
    ...
    size_t bufsize) :
        ...
        mAssistThreadEnabled(getAssistThreadEnabledFromEnv()),
        mFromGuest(8192, mLock),
        mFromGuestReadingThread([this] { if (mMode == Mode::PtrsWithDataSameThread) { threadFuncAssist(); } else { threadFunc(); } }),
        mBenchmarkEnabled(getBenchmarkEnabledFromEnv()) {
            if (mMode == Mode::PtrsWithDataSameThread) {
                if (mAssistThreadEnabled) {
...
RingStream::~RingStream() {
    if (mMode == Mode::PtrsWithDataSameThread && mAssistThreadEnabled) {
        AssistThreadMessage msg = AssistThreadMessage::Exit;
        mToAssistThreadMessages.send(msg);
        mFromAssistThreadMessages.receive(&msg);
    }
...
const unsigned char* RingStream::readRaw(void* buf, size_t* inout_len) {
    if (mMode == Mode::PtrsWithData) {
        ...
        return readRaw_sameThreadNoBuffer(buf, inout_len);
    }
}

const unsigned char* RingStream::readRaw_sameThreadNoBuffer(void* buf, size_t* inout_len) {
...
//             if (mFromHost.ring->state == 1) {
//                 ring_buffer_yield();
//                 continue;
//             }

            if (!mOnUnavailableReadFunc) {
        ...
        *(ConsumerState*)(&mToHost.ring->state) = ConsumerState::CanConsume;
        ...
        if (ringAvailable && mFromGuest.canPushLocked()) {
            auto start = currUs();
            if (mMode != Mode::PtrsWithDataSameThread) {
            ...
            uint32_t xferCountTotal = ringAvailable / 8;

            if (mOutstandingXfers.size() < xferCountTotal) {
            ...
            ring_buffer_copy_contents(
                ...
                ringAvailable, (uint8_t*)xfersPtr);

            bool quit = false;

            for (size_t i = 0; i < xferCountTotal; ++i) {
                mGetPtrAndSizeFunc(
                    xfersPtr[i],
                    &((decodedPtr + i)->ptr),
                    &((decodedPtr + i)->size));
                ...
                if (current + (decodedPtr + i)->size > ptrEnd) {
                    quit = true;
                    break;
                }
        ...
        if (!wasRingAvailable) {
            if (ringDataAvailable && mFromGuest.canPushLocked()) {
                ...
                uint32_t actuallyCanRead = (uint32_t)(uintptr_t)(ptrEnd - current);
                uint32_t willRead = actuallyCanRead < wantToReadThisTime ? actuallyCanRead : wantToReadThisTime;
...
const unsigned char* RingStream::readRaw_sameThread(void* buf, size_t* inout_len) {
    ...
    while (count < wanted) {
        ...
        *(ConsumerState*)(&mToHost.ring->state) = ConsumerState::CanConsume;
        ...
        bool wasRingAvailable = ringAvailable != 0;
        const size_t totalOutstandingMax = 256 * 1048576;
        size_t currentOutstanding = 0;

        while (ringAvailable && mFromGuest.canPushLocked() && currentOutstanding < totalOutstandingMax) {
            auto start = currUs();
            if (mMode != Mode::PtrsWithDataSameThread) {
                fprintf(stderr, "%s: wrong mode\n", __func__);
                abort();
            ...
            auto prevSize = mReadBuffer.size();
            mReadBuffer.resize_noinit(mReadBuffer.size() + totalNeeded);
            ...
            for (size_t i = 0; i < xferCount; ++i) {
                auto ptr = (decodedPtr + i)->ptr;
                auto size = (decodedPtr + i)->size;
                memcpy(mReadBuffer.data() + prevSize + byteOff, ptr, size);
                mReadBufferLeft += size;
                ring_buffer_view_read(
                        mToHost.ring, &mToHost.view,
                        mOutstandingXfers.data(),
                        8, 1);
                byteOff += size;
                currentOutstanding = mReadBuffer.size();
            }
        ...
        size_t dataRingByteOff = mReadBuffer.size();

        if (ringDataAvailable && mFromGuest.canPushLocked()) {
            auto start = currUs();
            uint32_t wantToReadThisTime = mToHostData.ring->state;

            mReadBuffer.resize_noinit(dataRingByteOff + wantToReadThisTime);
            ring_buffer_read_fully(
                mToHostData.ring, &mToHostData.view,
                mReadBuffer.data() + dataRingByteOff,
                wantToReadThisTime);
            dataRingByteOff += wantToReadThisTime;
...
void RingStream::threadFuncAssist() {
    fprintf(stderr, "%s: start\n", __func__);
    bool shouldExit = false;
    uint32_t readbackWaitCount = 0;
    uint32_t writeWaitCount = 0;
    ...
    while (1) {
        mToAssistThreadMessages.receive(&msg);

        if (msg == AssistThreadMessage::Exit) {
            mFromAssistThreadMessages.send(msg);
            return;
        }

        if (msg == AssistThreadMessage::Stop) {
            mFromAssistThreadMessages.send(msg);
            continue;
        }

        if (msg == AssistThreadMessage::Start) {
            ...
            while (1) {
                ...
                if (!ringAvailable && !ringDataAvailable) {
                    ring_buffer_yield();
                    if (mToAssistThreadMessages.tryReceive(&msg)) {
                        if (msg == AssistThreadMessage::Stop ||
                                msg == AssistThreadMessage::Exit) {
                            mFromAssistThreadMessages.send(msg);
                            if (msg == AssistThreadMessage::Exit) return;
                            // fprintf(stderr, "%s: break 1\n", __func__);
                            break;
                        }
                    }
                    continue;
                ...
                // Empty it into the read buffer
                if (!mFromGuest.canPushLocked()) {
                    RenderChannel::Buffer toReadBuffer;
                    size_t byteOff = mReadBuffer.size();
                    bool done = false;
                    while (!done) {
                        auto popResult = mFromGuest.tryPopSameThread(&toReadBuffer);
                        if (popResult == RenderChannel::IoResult::Ok) {
                            auto newSz = toReadBuffer.size();
                            mReadBuffer.resize_noinit(
                                byteOff + newSz);
                            memcpy(
                                mReadBuffer.data() + byteOff,
                                toReadBuffer.data(), newSz);
                            byteOff += newSz;
                            mReadBufferLeft += newSz;
                        }
                        else {
                            done = true;
                        }
                ...
                auto end = currUs();
                m_assistBufPushTimeUs += end - start;
            }
        }
...
void RingStream::printStats() {
    printf("RingStream::%s: push time %f ms pop time %f ms copyout %f ms unavail %f ms sleep %f ms assist %f ms (%f ms, %f ms)\n", __func__,
            ...
            (float)m_assistBufPushTimeUs / 1000.0f,
            (float)m_startAssistTimeUs / 1000.0f,
            (float)m_endAssistTimeUs / 1000.0f);
    m_bufPushTimeUs = 0;
    m_bufPopTimeUs = 0;
    m_bufCopyOutTimeUs = 0;
    m_bufUnavailableTimeUs = 0;
    m_bufUnavailableSleepTimeUs = 0;
    m_assistBufPushTimeUs = 0;
    m_startAssistTimeUs = 0;
    m_endAssistTimeUs = 0;
}

void RingStream::startAssist() {
    if (mMode != Mode::PtrsWithDataSameThread) return;
    if (!mAssistThreadEnabled) return;
    auto start = System::get()->getHighResTimeUs();
    AssistThreadMessage msg = AssistThreadMessage::Start;
    mToAssistThreadMessages.send(msg);
    auto end = System::get()->getHighResTimeUs();
    m_startAssistTimeUs += end - start;
}

void RingStream::endAssist() {
    if (mMode != Mode::PtrsWithDataSameThread) return;
    if (!mAssistThreadEnabled) return;
    auto start = System::get()->getHighResTimeUs();
    AssistThreadMessage msg = AssistThreadMessage::Stop;
    mToAssistThreadMessages.send(msg);
    mFromAssistThreadMessages.receive(&msg);
    auto end = System::get()->getHighResTimeUs();
    m_endAssistTimeUs += end - start;
}
