    ...
    while (count < wanted) {
        ...
        *(ConsumerState*)(&mToHost.ring->state) = ConsumerState::CanConsume;
        ...
        if (ringDataAvailable) {
            if (mMode == Mode::PtrsWithData) {
                fprintf(stderr, "%s: unimplemented, abort\n", __func__);
                abort();
            }
            uint32_t todo = wanted - count;
            uint32_t wantToReadThisTime = todo < ringDataAvailable ? todo : ringDataAvailable;
            mReadBuffer.resize_noinit(wantToReadThisTime);
            ring_buffer_read_fully(
                mToHostData.ring, &mToHostData.view, mReadBuffer.data(), wantToReadThisTime);
