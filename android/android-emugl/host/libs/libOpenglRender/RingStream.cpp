...
const unsigned char* RingStream::readRaw(void* buf, size_t* inout_len) {
    ...
    while (count < wanted) {
        ...
        if (!ringAvailable && (count > 0)) {
            break;
        } else if (!ringAvailable) {
            mOnUnavailableReadFunc();
        }

        if (mReadBuffer.size() < ringAvailable) {
            mReadBuffer.resize_noinit(ringAvailable << 1);
        }

        ring_buffer_read_fully(
            mToHost.ring, &mToHost.view, mReadBuffer.data(), ringAvailable);

        mReadBufferLeft = ringAvailable;
    }
