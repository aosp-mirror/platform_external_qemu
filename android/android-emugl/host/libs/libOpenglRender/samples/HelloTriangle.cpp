...
int main(int argc, char** argv) {
    ...
    gl->glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)vertexAttrs);
    ...
    for (int i = 0; i < 60; i++) {
        ...
        GLint err = gl->glGetError();
        if (err) fprintf(stderr, "%s: gl err 0x%x\n", __func__, err);
        android::base::System::get()->sleepMs(30);
        window.post();
    }
