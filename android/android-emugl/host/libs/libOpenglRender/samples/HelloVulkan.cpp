...
protected:
    ...
    #define PLATFORM_SURFACE_INSTANCE_EXT "VK_KHR_xlib_surface"
        ...
        static constexpr char* wantedExts[] = {
            "VK_KHR_surface",
        ...
        for (int i = 0; i < physdevCount; i++) {
            ...
            bool hasPlatformSurfaceExt = false;
            bool extensionsGood = false;
            for (int j = 0; j < devExts.size(); j++) {
                if (!strcmp(devExts[j].extensionName, "VK_KHR_surface")) {
                    hasSurfaceExt = true;
                }
                if (!strcmp(devExts[j].extensionName, PLATFORM_SURFACE_INSTANCE_EXT)) {
                    hasPlatformSurfaceExt = true;
                }
            }
            if (hasSurfaceExt && hasPlatformSurfaceExt) {
                extensionsGood = true;
            }
...
#define ROUND_UP(n, d) (((n) + (d) - 1) & -(0 ? (n) : (d)))
        ...
        if (bufferMemReqs.memoryTypeBits & (1 << bestDeviceMemoryTypeIndex)) {
            fprintf(stderr, "%s: buffer usable with devie memory type index\n", __func__);
        } else {
            abort();
        }
        ...
        VkPushConstantRange pushConstantRange = {
            VK_SHADER_STAGE_VERTEX_BIT,
            0,
            4 * sizeof(float) /* enough for our modelview matrix */,
        };
        ...
        VkPipelineRasterizationStateCreateInfo rasterizationInfo = {
            ...
            VK_TRUE /* enable rasterizer discard */,
