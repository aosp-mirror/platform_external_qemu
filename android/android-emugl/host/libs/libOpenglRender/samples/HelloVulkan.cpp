...
protected:
    ...
    void initialize() override {
        ...
        vk->vkEnumerateInstanceExtensionProperties(nullptr, &extPropCount, nullptr);
        extProps.resize(extPropCount);
        vk->vkEnumerateInstanceExtensionProperties(nullptr, &extPropCount, extProps.data());
    ...
    #define PLATFORM_SURFACE_INSTANCE_EXT "VK_KHR_xcb_surface"
        ...
        if (!hasSurfaceCapability || !hasPlatformSurfaceCapability) {
            fprintf(stderr, "%s: doesn't have needed surface capabilities; abort\n", __func__);
            abort();
        }
        ...
        vk->vkEnumeratePhysicalDevices(mInstance, &physdevCount, physdevs.data());
        fprintf(stderr, "%s: found %u physical devices\n", __func__, physdevCount);
        ...
        for (int i = 0; i < physdevCount; i++) {
            ...
            // Need a device with queue families capable of:
            // a. transfer
            // b. graphics
            ...
            for (int j = 0; j < queueFamilyProps.size(); j++) {
                ...
                if ((props.queueFlags & VK_QUEUE_GRAPHICS_BIT) &&
                    ...
                    fprintf(stderr,
                            "%s: queue family %d is capable of graphics + transfer, with %u "
                            "queues.\n",
                            __func__, j, props.queueCount);
            ...
            if (queuesGood && extensionsGood) {
                ...
                fprintf(stderr, "%s: found suitable physical device. index: %d\n", __func__, i);
                break;
            }
        }
        ...
        ASSERT_VK_RESULT(VK_SUCCESS,
                         vk->vkCreateDevice(mPhysicalDevice,
                                            &dCi, nullptr, &mDevice));
        ...
        for (uint32_t i = 0; i < memProps.memoryTypeCount; ++i) {
            ...
            if (!foundStaging && (type.propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)) {
                foundStaging = true;
                bestStagingMemoryTypeIndex = i;
                fprintf(stderr, "%s: best staging memory type index: %u\n", __func__, bestStagingMemoryTypeIndex);
            }
            if (!foundDevice && (type.propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)) {
                foundDevice = true;
        ...
        ASSERT_VK_RESULT(VK_SUCCESS,
                         vk->vkAllocateMemory(mDevice, &stagingAllocInfo, nullptr,
                                              &mStagingMemory));
        ASSERT_VK_RESULT(VK_SUCCESS,
                         vk->vkAllocateMemory(mDevice, &deviceAllocInfo, nullptr,
                                              &mDeviceMemory));
        fprintf(stderr, "%s: allocated staging and device memory. time to map\n", __func__);
        ASSERT_VK_RESULT(VK_SUCCESS,
                         vk->vkMapMemory(mDevice, mStagingMemory, 0, kAllocSize, 0,
                                         &mMappedStagingMemory));
        ...
        void main() {
            gl_Position = vec4(pos, 0.0, 1.0);
            color_varying = (transformPC.mvp * vec4(color, 0.0, 1.0)).xy;
        }
        )";
        ...
        fprintf(stderr, "%s: compiled vert and frag shaders. creating shader modules\n", __func__);

        fprintf(stderr, "%s: spv buffs: %p %p\n", __func__, vertSpvBuff->data(), fragSpvBuff->data());
        ...
        ASSERT_VK_RESULT(VK_SUCCESS,
                         vk->vkCreateShaderModule(mDevice, &vertModuleCi, nullptr,
                                                  &mVshaderModule));
        ASSERT_VK_RESULT(VK_SUCCESS,
                         vk->vkCreateShaderModule(mDevice, &fragModuleCi, nullptr,
                                                  &mFshaderModule));
        ...
        vk->vkGetBufferMemoryRequirements(mDevice, mVkStagingBuffer, &stagingBufMemReqs);
        if (stagingBufMemReqs.size > kAllocSize) {
            fprintf(stderr,
                    "%s: fatal oops: staging buf actually required greater size (0x%llx) "
                    "than expected (0x%llx)\n",
                    __func__,
                    (unsigned long long)stagingBufMemReqs.size,
                    (unsigned long long)kAllocSize);
            abort();
        }
        if (stagingBufMemReqs.memoryTypeBits & (1 << bestStagingMemoryTypeIndex)) {
            fprintf(stderr,
        ...
        ASSERT_VK_RESULT(VK_SUCCESS, vk->vkBindBufferMemory(mDevice, mVkStagingBuffer,
                                                            mStagingMemory, 0));
...
#define ROUND_UP(n, d) (((n) + (d) - 1) & -(0 ? (n) : (d)))
        ...
        ASSERT_VK_RESULT(VK_SUCCESS, vk->vkBindBufferMemory(mDevice, mVkVertexBuffer,
                                                            mDeviceMemory, 0));
        ...
        vk->vkGetImageMemoryRequirements(mDevice, mVkImageToPaint, &imageMemReqs);
        ...
        currentDeviceMemoryOffset =
                ROUND_UP(currentDeviceMemoryOffset, imageMemReqs.alignment);
        fprintf(stderr, "%s: image: round device memory offset to 0x%llx\n",
                __func__, (unsigned long long)currentDeviceMemoryOffset);
        ASSERT_VK_RESULT(
                VK_SUCCESS,
                vk->vkBindImageMemory(mDevice, mVkImageToPaint, mDeviceMemory,
                                      currentDeviceMemoryOffset));
        ...
        ASSERT_VK_RESULT(VK_SUCCESS, vk->vkAllocateDescriptorSets(mDevice, &dsai,
                                                                  &mDescriptorSet));
        ...
        VkPipelineDepthStencilStateCreateInfo depthStencilInfo = {
            ...
            { VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_COMPARE_OP_EQUAL,
              0x0, 0x0, 0x0 },
            { VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_COMPARE_OP_EQUAL,
              0x0, 0x0, 0x0 },
        ...
        vk->vkCmdCopyBuffer(mCommandBuffer_transfer, mVkStagingBuffer, mVkVertexBuffer, 1, &region);
        ...
        ASSERT_VK_RESULT(VK_SUCCESS, vk->vkQueueSubmit(mTransferQueue, 1, &vboCopySi, mFence));
    ...
    void draw() override {
        ...
        clearColorValue.float32[3] = 0.2f;
        ...
        // We're not actually using the descriptor set here as it's not needed
        // (using the push_constant instead of UBO), but here is the command
        ...
        VkMappedMemoryRange invalidateRange = {
            ...
            mStagingMemory, 0, VK_WHOLE_SIZE,
        };
    ...
    static constexpr VkDeviceSize kAllocSize = 32 * 1024 * 1024; // 32 mb
