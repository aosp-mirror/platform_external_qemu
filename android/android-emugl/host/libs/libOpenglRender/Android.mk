...
ifeq ($(BUILD_TARGET_OS),linux)
    standalone_common_SRC_FILES += tests/angle-util/x11/X11Window.cpp
    LOCAL_LDLIBS += -lX11 -lrt
endif

ifeq ($(BUILD_TARGET_OS),darwin)
    standalone_common_SRC_FILES += tests/angle-util/osx/OSXWindow.mm
    LOCAL_LDLIBS += -Wl,-framework,AppKit
	LOCAL_LDFLAGS += -Wl,-headerpad_max_install_names
endif

ifeq ($(BUILD_TARGET_OS),windows)
    standalone_common_SRC_FILES += tests/angle-util/windows/WindowsTimer.cpp
    standalone_common_SRC_FILES += tests/angle-util/windows/win32/Win32Window.cpp
    LOCAL_LDLIBS += -lgdi32
endif
...
standalone_common_C_INCLUDES := \
    ...
    $(EMUGL_PATH)/host/libs/libOpenglRender/tests \
    $(EMUGL_PATH)/host/libs/libOpenglRender/tests/angle-util \
