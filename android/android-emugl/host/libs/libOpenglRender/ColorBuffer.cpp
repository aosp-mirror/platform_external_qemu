...
ColorBuffer::ColorBuffer(EGLDisplay display, HandleType hndl, Helper* helper)
    : m_wait_emulation_func([]{}), m_display(display), m_helper(helper), mHndl(hndl) {}
...
void ColorBuffer::setPendingSemaphoreSignalEmulated(std::function<void()> waitEmulationFunc) {
    m_wait_emulation_func = waitEmulationFunc;
    m_pending_semaphore_signal = true;
    m_pending_semaphore_signal_emulated = true;
}

void ColorBuffer::setPendingSemaphoreSignal() {
    m_pending_semaphore_signal = true;
}
...
void ColorBuffer::waitSemaphore(GLenum textureSrcLayout) {
    ...
    if (m_pending_semaphore_signal_emulated) {
        m_wait_emulation_func();
        m_pending_semaphore_signal = false;
        m_pending_semaphore_signal_emulated = false;
    } else {
