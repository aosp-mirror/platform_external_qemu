...
// static
bool ColorBuffer::setGLFormat(GLenum p_internalFormat,
                              GLenum* texFormat,
                              GLenum* pixelType,
                              int* bytesPerPixel) {
...
ColorBuffer* ColorBuffer::create(EGLDisplay p_display,
    ...
    GLenum texFormat, pixelType = GL_UNSIGNED_BYTE;
    int bytesPerPixel = 3;
    if (!setGLFormat(p_internalFormat, &texFormat, &pixelType,
                     &bytesPerPixel)) {
        return NULL;
    }
...
void ColorBuffer::initialize() {
    ...
    int bytesPerPixel = 3;
    ColorBuffer::setGLFormat(m_internalFormat, &m_format, &m_type,
                             &bytesPerPixel);
