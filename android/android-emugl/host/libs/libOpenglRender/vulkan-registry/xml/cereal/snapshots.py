        ...
        if category in ["struct", "union"] and not alias:
            ...
            def messageGenerator(cgen):
                ...
                for (i, member) in enumerate(structInfo.members):
                    ...
                    self.codegen.line("// %s msg? %d stringarray? %d string? %d hasLenInfo? %d protoType %s origType %s %s" % \
                        (member.typeName, protoInfo.needsMessage, protoInfo.isRepeatedString, protoInfo.isString, protoInfo.lengthInfo != None, protoInfo.protobufType, member.typeName, protoInfo.origTypeCategory))
