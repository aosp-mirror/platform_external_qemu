...
class VulkanReservedMarshalingCodegen(VulkanTypeIterator):
    ...
    def genMemcpyAndIncr(self, varname, cast, toStreamExpr, sizeExpr):
        if self.direction == "write":
    ...
    def onString(self, vulkanType):
        ...
        self.genMemcpyAndIncr(self.ptrVar, "(uint32_t*)" ,"&l", "sizeof(uint32_t)")
    ...
    def onStringArray(self, vulkanType):
        ...
        self.genMemcpyAndIncr(self.ptrVar, "(uint32_t*)" ,"&c", "sizeof(uint32_t)")
    ...
    def onStructExtension(self, vulkanType):
        ...
        if self.direction == "read" and self.dynAlloc:
            ...
            self.genMemcpyAndIncr(self.ptrVar, "(uint32_t*)", "&" + sizeVar, "sizeof(uint32_t)")
...
class VulkanReservedMarshaling(VulkanWrapperGenerator):
    ...
    def __init__(self, module, typeInfo, variant="host"):
        ...
        self.ptrVarName = "ptr"
        self.ptrVarType = makeVulkanTypeSimple(False, "uint8_t", 1, self.ptrVarName)
        self.ptrVarTypeUnmarshal = makeVulkanTypeSimple(True, "uint8_t", 1, self.ptrVarName)
    ...
    def doExtensionStructMarshalingCodegen(self, cgen, retType, extParam, forEach, funcproto, direction):
        ...
        if direction == "write":
            cgen.stmt("memcpy(%s, &%s, sizeof(uint32_t)); %s += sizeof(uint32_t)" % (self.ptrVarName, sizeVar, self.ptrVarName))
        elif not self.dynAlloc:
            cgen.stmt("memcpy(&%s, %s, sizeof(uint32_t)); %s += sizeof(uint32_t)" % (sizeVar, self.ptrVarName, self.ptrVarName))
