...
def emit_impl(typeInfo, api, cgen):
    ...
    for p in api.parameters:
        ...
        if p.isDestroyedBy(api):
            ...
            cgen.stmt("forEachReconstructionDeleteApiRefs_%s(%s, %s)" % (p.typeName, access, lenExpr))
    ...
    if traceCreate:
        ...
        cgen.stmt("forEachReconstructionAddApiRef_%s(%s, %s, apiHandle)" % (p.typeName, access, lenExpr))
