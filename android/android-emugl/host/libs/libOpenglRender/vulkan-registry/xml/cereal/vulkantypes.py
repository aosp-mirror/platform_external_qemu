...
class VulkanTypeInfo(object):
    ...
    def onGenStruct(self, typeinfo, typeName, alias):
        if not alias:
            ...
            if self.categoryOf(typeName) == "struct":
                self.structs[typeName] = VulkanCompoundType(typeName, members)
            elif self.categoryOf(typeName) == "union":
                self.unions[typeName] = VulkanCompoundType(typeName, members, isUnion = True)
