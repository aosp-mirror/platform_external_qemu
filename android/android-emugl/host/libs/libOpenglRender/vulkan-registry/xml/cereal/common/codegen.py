...
class CodeGen(object):
    ...
    def streamPrimitive(self, typeInfo, streamVar, accessExpr, accessTypeName, direction="write"):
        if not self.validPrimitive(typeInfo, accessTypeName):
        ...
        if direction == "read":
            self.stmt("%s = (%s)%s->%s()" % (accessExpr, accessTypeName, streamVar, streamMethod))
        else:
            self.stmt("%s %s = (%s)%s" %
                      (streamStorageVarType, streamStorageVar,
                       streamStorageVarType, accessExpr))
