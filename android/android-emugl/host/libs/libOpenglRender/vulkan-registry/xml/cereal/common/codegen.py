...
class CodeGen(object):
    ...
    def makeStructAccess(self, vulkanType, structVarName, asPtr = True, structAsPtr = True, accessIndex = None):

        deref = "->" if structAsPtr else "."

        indexExpr = (" + %s" % accessIndex) if accessIndex else ""

        addrOfExpr = "" if vulkanType.accessibleAsPointer() or (not asPtr) else "&"

        return "%s%s%s%s%s" % (addrOfExpr, structVarName, deref, vulkanType.paramName, indexExpr)
    ...
    def makeLengthAccessFromStruct(self, structInfo, vulkanType, structVarName, asPtr = True):
        ...
        specialCaseAccess = handleSpecialCases(structInfo, vulkanType, structVarName, asPtr)
        ...
        if lenExpr == "null-terminated":
            return "strlen(%s%s%s)" % (structVarName, deref, vulkanType.paramName)
    ...
    def sizeofExpr(self, vulkanType):
        return "sizeof(%s)" % (self.makeCTypeDecl(vulkanType, useParamName = False))

    def generalAccess(self, vulkanType, parentVarName = "parent", asPtr = True, structAsPtr = True):
        if vulkanType.parent is None:
            return self.accessParameter(vulkanType, asPtr = asPtr)

        if type(vulkanType.parent) is VulkanCompoundType:
            return self.makeStructAccess(vulkanType, parentVarName, asPtr = asPtr, structAsPtr = structAsPtr)

        if type(vulkanType.parent) is VulkanAPI:
            return self.accessParameter(vulkanType, asPtr = asPtr)
    ...
    def generalLengthAccess(self, vulkanType, parentVarName = "parent"):
        ...
        if type(vulkanType.parent) is VulkanCompoundType:
            return self.makeLengthAccessFromStruct(vulkanType.parent, vulkanType, parentVarName, asPtr = True)
...
# Class to wrap a Vulkan API call. The user gives a generic callback, |codegenDef|, that takes a CodeGen
# object and a VulkanAPI object as arguments, and the purpose is to fill in the body of the function.
