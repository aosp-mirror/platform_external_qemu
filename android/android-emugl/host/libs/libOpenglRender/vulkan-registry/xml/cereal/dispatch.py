...
""")
        ...
        self.cgenHeader.line("VkQueue graphicsQueue;")
        self.cgenHeader.line("VkQueue computeQueue;")
        self.cgenHeader.line("VkQueue presentQueue;")
...
void init_vulkan_dispatch_from_system_loader(
    ...
    VulkanDispatch* out)""")
        ...
        self.cgenImpl.stmt("out->graphicsQueue = nullptr")
        self.cgenImpl.stmt("out->computeQueue = nullptr")
        self.cgenImpl.stmt("out->presentQueue = nullptr")
        ...
        for vulkanApi, typeDecl, feature in self.apisToGet["global"]:
            ...
            self.cgenImpl.stmt("out->%s = (%s)dlSymFunc(lib, \"%s\")" % (vulkanApi.name, typeDecl, vulkanApi.name))
        ...
        for vulkanApi, typeDecl, feature in self.apisToGet["global-instance"]:
            self.syncFeature(self.cgenImpl, feature)
            self.cgenImpl.stmt("out->%s = (%s)out->vkGetInstanceProcAddr(nullptr, \"%s\")" % (vulkanApi.name, typeDecl, vulkanApi.name))
            self.cgenImpl.stmt("if (!out->%s) fprintf(stderr, \"not found: %s\\n\")" % (vulkanApi.name, vulkanApi.name))
        ...
        for vulkanApi, typeDecl, feature in self.apisToGet["instance"]:
            self.syncFeature(self.cgenImpl, feature)
            self.cgenImpl.stmt("out->%s = (%s)out->vkGetInstanceProcAddr(out->instance, \"%s\")" % (vulkanApi.name, typeDecl, vulkanApi.name))

        self.syncFeature(self.cgenImpl, "")
        self.cgenImpl.stmt("if (!deviceGetter(out, out->instance, &out->physicalDevice, &out->device)) return")

        for vulkanApi, typeDecl, feature in self.apisToGet["device"]:
            self.syncFeature(self.cgenImpl, feature)
            self.cgenImpl.stmt("out->%s = (%s)out->vkGetDeviceProcAddr(out->device, \"%s\")" % (vulkanApi.name, typeDecl, vulkanApi.name))
    ...
    def onGenCmd(self, cmdinfo, name, alias):
        ...
        self.cgenHeader.line("// %s proc addr by %s" % (name, procAddressType))
