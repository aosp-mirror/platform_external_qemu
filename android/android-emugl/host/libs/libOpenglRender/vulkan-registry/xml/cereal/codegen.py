...
class CodeGen(object):
    ...
    def makeStructAccess(self, vulkanType, structVarName, asPtr = True, accessIndex = None):

        deref = "->" if asPtr else "."

        indexExpr = (" + %s" % accessIndex) if accessIndex else ""

        addrOfExpr = "" if vulkanType.accessibleAsPointer() else "&"
    ...
    def accessParameterAsPtr(self, param):
    ...
    def generalAccessAsPtr(self, vulkanType, parentVarName = "parent"):
        if vulkanType.parent is None:
            return self.accessParameterAsPtr(vulkanType)

        if type(vulkanType.parent) is VulkanCompoundType:
            return self.makeStructAccess(vulkanType, parentVarName, asPtr = True)
