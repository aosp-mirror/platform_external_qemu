...
def decode_vkMapMemory(typeInfo, api, cgen):
    device = api.getParameter("device")
    memory = api.getParameter("memory")
    offset = api.getParameter("offset")
    size = api.getParameter("size")
    flags = api.getParameter("flags")
    ppData = api.getParameter("ppData")
    ret = api.retType
