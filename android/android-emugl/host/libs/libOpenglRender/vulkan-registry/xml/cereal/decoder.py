...
from .wrapperdefs import ACQUIRE_APIS, RELEASE_APIS, DELAYED_DECODER_DELETES
...
def emit_destroyed_handle_cleanup(api, cgen):
    ...
    for p in paramsToRead:
        if p.dispatchHandle:
            ...
            if destroy:
                if None == lenAccess or "1" == lenAccess:
                    if p.typeName in DELAYED_DECODER_DELETES:
                        cgen.stmt("delayed_delete_%s(boxed_%s_preserve)" % (p.typeName, p.paramName))
                    else:
                        cgen.stmt("delete_%s(boxed_%s_preserve)" % (p.typeName, p.paramName))
                else:
                    cgen.beginFor("uint32_t i = 0", "i < %s" % lenAccess, "++i")
                    if p.typeName in DELAYED_DECODER_DELETES:
                        cgen.stmt("delayed_delete_%s(boxed_%s_preserve[i])" % (p.typeName, p.paramName))
                    else:
