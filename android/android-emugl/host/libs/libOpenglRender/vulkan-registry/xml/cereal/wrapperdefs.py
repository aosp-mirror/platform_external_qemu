...
class VulkanStreamCodegen(object):
    ...
    def compoundIter(self, cgen, vulkanType, toProcessExpr, loopVar = None):
        ...
        castedExpr = "(%s)(%s)" % (typeCast, indexedExpr)
        ...
        if vulkanType.pointerIndirectionLevels > 0:
            ...
            checkCastExpr = \
                "(%s)" % (cgen.makeCTypeDecl( \
                    vulkanType.getTransformed( \
                        isConstChoice = (self.direction == "write"),
                        ptrIndirectionChoice = vulkanType.pointerIndirectionLevels + 1), useParamName = False))
            ...
            compensateCastedType = vulkanType.getTransformed(isConstChoice = (self.direction == "write"), ptrIndirectionChoice = vulkanType.pointerIndirectionLevels + 0)
            compensatedCast = cgen.makeCTypeDecl(compensateCastedType, useParamName = False)
            compensateCastExpr = "(%s)(%s)" % (compensatedCast, indexedExpr)
            cgen.funcCall(None, apiPrefix + vulkanType.typeName, [VULKAN_STREAM_VAR_NAME, compensateCastExpr])
            cgen.endIf()
        else:
            cgen.funcCall(None, apiPrefix + vulkanType.typeName, [VULKAN_STREAM_VAR_NAME, castedExpr])
    ...
    def onGenType(self, typeXml, name, alias):
        ...
        if category in ["struct", "union"] and not alias:
            ...
            def structUnmarshalingDef(cgen):
                ...
                # self.streamReadCodegen.processSimple(cgen, 
                #     makeVulkanTypeSimple(False, structInfo.name, 1, UNMARSHAL_INPUT_VAR_NAME))

                # cgen.stmt("if (!%s) return" % UNMARSHAL_INPUT_VAR_NAME)
