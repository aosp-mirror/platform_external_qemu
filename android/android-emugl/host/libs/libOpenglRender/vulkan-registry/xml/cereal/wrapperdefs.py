...
PARAMETERS_MARSHAL = [makeVulkanTypeSimple(False, "void", 1, "vkStream")]
PARAMETERS_ENCODE = [makeVulkanTypeSimple(False, "void", 1, "vkStream")]
PARAMETERS_VALIDATE = [makeVulkanTypeSimple(False, "VkResult", 1, "validateResult")]

VOID_TYPE = makeVulkanTypeSimple(False, "void", 0)
STREAM_RET_TYPE = makeVulkanTypeSimple(False, "uint32_t", 0)

MARSHAL_INPUT_VAR_NAME = "marshalIn"
            ...
            def structMarshalingDef(cgen):
                for member in structInfo.members:
                    if self.typeInfo.isCompoundType(member.typeName):
                        cgen.line("// marshal %s (call next marshaler)" % (cgen.makeCTypeDecl(member)))

                        neededStaticArrReps = None
                        neededCustomLength = None

                        if member.pointerIndirectionLevels > 0:
                            addrOf = ""
                            deref = "->"
                            # Start out with a nullptr guard
                            neededCustomLength = "nullptr-check"
                            arrIndexTemplate = " + %s"
                        elif member.staticArrExpr != "":
                            addrOf = ""
                            deref = "->"
                            neededStaticArrReps = member.staticArrExpr
                            arrIndexTemplate = " + %s"
                        else:
                            addrOf = "&"
                            deref = "->"
                        ...
                        if member.lenExpr:
                            neededCustomLength = member.lenExpr
                        
                        structAccess = "%s%s%s" % (addrOf, MARSHAL_INPUT_VAR_NAME, deref) + member.paramName
                      
                        needLoop = False

                        if neededCustomLength == "nullptr-check":
                            cgen.beginIf("%s" % structAccess)
                        elif neededCustomLength is not None:
                            needLoop = True
                            lenAccess = "%s->%s" % (MARSHAL_INPUT_VAR_NAME, neededCustomLength)
                            cgen.beginFor("uint32_t i = 0", "i < (uint32_t)%s /* customLen */" % lenAccess, "++i")
                        elif neededStaticArrReps is not None:
                            needLoop = True
                            cgen.beginFor("uint32_t i = 0", "i < (uint32_t)%s /* staticArr */" % neededStaticArrReps, "++i")

                        if needLoop:
                            structAccess = (structAccess + (arrIndexTemplate % "i"))

                        cgen.funcCall(None, API_PREFIX_MARSHAL + member.typeName, ["vkStream", structAccess])
                        ...
                        cgen.line("// marshal %s (simple or array)" % (cgen.makeCTypeDecl(member)))
                cgen.stmt("return 0")
...
class VulkanFrontend(VulkanWrapperGenerator):
    def __init__(self, module, typeInfo):
        ...
        def frontendDefFunc(codegen, api):
            ...
            codegen.stmt("VkResult validateResult = VK_SUCCESS")
            codegen.funcCall(None, API_PREFIX_VALIDATE + api.origName, ["&validateResult"] + list(map(lambda p: p.paramName, api.parameters)))
    
            codegen.beginIf("validateResult != VK_SUCCESS")
            if retTypeName == 'VkResult':
                codegen.stmt("return validateResult")
            ...
            codegen.stmt("void* vkStream = 0")
   
            retLhs = None
            if retTypeName != 'void':
                retLhs = retTypeName + " res"

            codegen.funcCall(retLhs, API_PREFIX_ENCODE + api.origName, ["vkStream"] + list(map(lambda p: p.paramName, api.parameters)))
