...
class VulkanStreamCodegen(object):
    ...
    def simpleIter(self, cgen, vulkanType, toProcessExpr, lenExpr):
        processString = "putString" if self.direction == "write" else "getString"
        processStringArray = "putStringArray" if self.direction == "write" else "getStringArray"
        processOther = "write" if self.direction == "write" else "read"

        if vulkanType.isString():
            cgen.stmt("%s->%s(%s)" % (VULKAN_STREAM_VAR_NAME, processString, toProcessExpr))
        elif vulkanType.isArrayOfStrings():
            cgen.stmt("%s->%s(%s, %s)" % (VULKAN_STREAM_VAR_NAME, processStringArray, toProcessExpr, lenExpr))
        elif lenExpr:
            finalLenExpr = "%s * %s" % (lenExpr, cgen.sizeofExpr(vulkanType))
            cgen.stmt("%s->%s(%s, %s)" % (VULKAN_STREAM_VAR_NAME, processOther, toProcessExpr, finalLenExpr))
        else:
