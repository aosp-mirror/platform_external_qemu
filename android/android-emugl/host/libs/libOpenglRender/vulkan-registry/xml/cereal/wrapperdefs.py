        ...
        def apiMarshalingDef(cgen, api):
            for param in api.parameters:
                ...
                if self.typeInfo.isCompoundType(param.typeName):
                    ...
                    paramAccess = cgen.accessParameterAsPtr(param)
                    lenAccess = cgen.makeLengthAccessFromApi(api, param)

                    if param.isString():
                        cgen.stmt("%s->putString(%s)" % (VULKAN_STREAM_VAR_NAME, paramAccess))
                    elif param.isArrayOfStrings():
                        cgen.stmt("%s->putStringArray(%s, %s)" % (VULKAN_STREAM_VAR_NAME, paramAccess, lenAccess))
                    elif lenExpr:
                        fullLenExpr = "%s * %s" % (lenAccess, self.safeMarshalingSizeof(param))
                        cgen.stmt("%s->write(%s, %s)" % (VULKAN_STREAM_VAR_NAME, paramAccess, fullLenExpr))
                    else:
                        fullLenExpr = self.safeMarshalingSizeof(param)
                        cgen.stmt("%s->write(%s, %s)" % (VULKAN_STREAM_VAR_NAME, paramAccess, fullLenExpr))

            if api.retType.isVoidWithNoSize():
                pass
            else:
                cgen.line("// TODO: Output: %s" % cgen.makeCTypeDecl(api.retType))
                cgen.stmt("return (%s)0" % api.retType.typeName)
    ...
    def onGenType(self, typeXml, name, alias):
        ...
        if category in ["struct", "union"] and not alias:
            ...
            def structMarshalingDef(cgen):
                ...
                cgen.stmt("%s->write(%s, sizeof(%s*))" % (VULKAN_STREAM_VAR_NAME, MARSHAL_INPUT_VAR_NAME, structInfo.name))
                ...
                for member in structInfo.members:
                    ...
                    if not member.isArrayOfStrings():
                    ...
                    if self.typeInfo.isCompoundType(member.typeName):
                        ...
                        elif member.isArrayOfStrings():
                            lenAccess = cgen.makeLengthAccessFromStruct(structInfo, member, MARSHAL_INPUT_VAR_NAME, asPtr = True)
                            cgen.stmt("%s->putStringArray(%s, %s)" % (VULKAN_STREAM_VAR_NAME, structAccess, lenAccess))
                        elif lenExpr:
                            lenAccess = cgen.makeLengthAccessFromStruct(structInfo, member, MARSHAL_INPUT_VAR_NAME, asPtr = True)
