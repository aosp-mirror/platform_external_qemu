...
class VulkanStreamCodegen(object):
    def __init__(self, direction = "write", forApiOutput = False):
    ...
    def compoundIter(self, cgen, vulkanType, toProcessExpr, loopVar = None):
        ...
        if vulkanType.pointerIndirectionLevels > 0:
            ...
            needCheck = vulkanType.isOptional and not self.forApiOutput
    ...
    def simpleIter(self, cgen, vulkanType, toProcessExpr, lenExpr):
        ...
        castedForStreamingDeclPtr = \
            cgen.makeCTypeDecl( \
                vulkanType.getTransformed( \
                    isConstChoice = (self.direction == "write"),
                    ptrIndirectionChoice = vulkanType.pointerIndirectionLevels),
                useParamName = False)

        castedForStreamingExprPtr = "(%s)" % castedForStreamingDeclPtr

        castedForStreamingDeclDeref = \
            cgen.makeCTypeDecl( \
                vulkanType.getTransformed( \
                    isConstChoice = (self.direction == "write"),
                    ptrIndirectionChoice = vulkanType.pointerIndirectionLevels - 1),
        ...
        castedForStreamingExprDeref = "(%s)" % castedForStreamingDeclDeref
        ...
        castedForStreamingExprValue = "(%s)" % castedForStreamingDeclValue
        ...
        if vulkanType.isString():
            ...
            cgen.stmt("%s->%s(%s%s, %s)" % (VULKAN_STREAM_VAR_NAME, self.processOther, castedForStreamingExprValue, toProcessExpr, finalLenExpr))
            ...
            if not skipStreamInternal:
                if needCheck:
                    cgen.beginIf("%s" % toProcessExpr)

                if lenExpr:
                    finalLenExpr = "%s * sizeof(%s)" % (lenExpr, castedForStreamingDeclDeref)
                else:
                    finalLenExpr = "sizeof(%s)" % (castedForStreamingDeclDeref)

                cgen.stmt("%s->%s(%s%s, %s)" % (VULKAN_STREAM_VAR_NAME, self.processOther, castedForStreamingExprPtr, toProcessExpr, finalLenExpr))
...
class VulkanMarshaling(VulkanWrapperGenerator):
    ...
    def __init__(self, module, typeInfo):
        ...
        self.streamWriteCodegenForApiOutput = VulkanStreamCodegen("write", forApiOutput = True)
        self.streamReadCodegenForApiOutput = VulkanStreamCodegen("read", forApiOutput = True)
            ...
            for param in api.parameters:
                ...
                if param.possiblyOutput():
                    ...
                    iterateVulkanType(typeInfo,
                                      ...
                                      self.streamReadCodegenForApiOutput.compoundIter,
                                      self.streamReadCodegenForApiOutput.simpleIter)
                else:
                    iterateVulkanType(typeInfo,
                                      cgen,
                                      param,
            ...
            for param in api.parameters:
                ...
                if param.possiblyOutput():
                    iterateVulkanType(typeInfo,
                                      ...
                                      lambda t : cgen.accessParameterAsPtr(t),
                                      lambda t: cgen.makeLengthAccessFromApi(api, t),
                                      self.streamWriteCodegenForApiOutput.compoundIter,
                                      self.streamWriteCodegenForApiOutput.simpleIter)
                else:
                    iterateVulkanType(typeInfo,
                                      cgen,
                                      param,
