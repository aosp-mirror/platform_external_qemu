...
class VulkanMarshaling(VulkanWrapperGenerator):
    ...
    def onGenType(self, typeXml, name, alias):
        ...
        if category in ["struct", "union"] and not alias:
            ...
            prototypeParameters = \
                PARAMETERS_MARSHAL + \
                [makeVulkanTypeSimple(True, name, 1, "input")]

            prototype = VulkanAPI("marshal_%s" % name, STREAM_RET_TYPE, prototypeParameters)

            structInfo = self.typeInfo.structs[name]

            def structMarshalingDef(cgen):
                for member in structInfo.members:
                    cgen.line("// marshal %s" % (cgen.makeCTypeDecl(member)))
