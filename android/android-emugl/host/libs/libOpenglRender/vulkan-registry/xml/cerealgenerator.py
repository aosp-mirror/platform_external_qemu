...
"""% (autogeneratedMkTemplate % banner_command(sys.argv), cppFiles)
        ...
        self.host_decoder_cmake_generator = lambda cppFiles: """%s
set(vulkan_dec_src %s)
android_add_library(vulkan_dec)
    ...
    def beginFile(self, genOpts):
        ...
        guestAndroidMkPath = \
            os.path.join( \
                self.guest_abs_destination,
                "Android.mk")
        hostDecoderCMakePath = \
            os.path.join( \
                self.host_abs_decoder_destination,
                "CMakeLists.txt")

        guestAndroidMkFile = open(guestAndroidMkPath, "w", encoding="utf-8")
        hostDecoderCMakeFile = open(hostDecoderCMakePath, "w", encoding="utf-8")

        write(self.guest_android_mk_generator(self.guestAndroidMkCppFiles),
              file = guestAndroidMkFile)
        write(self.host_decoder_cmake_generator(self.hostDecoderCMakeCppFiles),
              file = hostDecoderCMakeFile)

        guestAndroidMkFile.close()
        hostDecoderCMakeFile.close()
