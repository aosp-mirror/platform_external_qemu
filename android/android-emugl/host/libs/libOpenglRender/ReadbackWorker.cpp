...
void ReadbackWorker::initFlushContext() {
    const FbConfig* config = mFb->getConfigs()->get(0 /* p_config */);
    if (!config) {
        // Surprise!
        return;
    }
    ...
    mFlushContext =
            s_egl.eglCreateContext(mFb->getDisplay(), config->getEglConfig(),
                                   mContext, contextAttribs);
}
...
void ReadbackWorker::flushPipeline() {
    ...
    // the context.
    s_egl.eglMakeCurrent(mFb->getDisplay(), mSurf, mSurf, mFlushContext);
