...
void ReadbackWorker::doNextReadback(ColorBuffer* cb, void* fbImage) {
    ...
    if (isCopying) {
        switch (mMapCopyIndex.load(std::memory_order_relaxed)) {
        ...
        mReadPixelsIndexEven = 0;
        mReadPixelsIndexOdd = 1;
        mMapCopyIndex.store(mPrevReadPixelsIndex, std::memory_order_relaxed);
    }
...
void ReadbackWorker::getPixels(void* buf, uint32_t bytes) {
    ...
    GLuint buffer = mBuffers[mMapCopyIndex.load(std::memory_order_relaxed)];
