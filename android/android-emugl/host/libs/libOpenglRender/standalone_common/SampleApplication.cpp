...
public:
    ScopedRender(unsigned int context, unsigned int surface) {
        ...
        fprintf(stderr, "%s: finished init scoped render\n", __func__);
    }
...
void SampleApplication::surfaceFlingerComposerLoop() {
    ...
    mFb->bindContext(appContext, appSurface, appSurface);
        ...
        while (true) {
            ...
            app2sfQueue.queueBuffer({ sfItem.colorBuffer, nullptr }); 
    ...
    mFb->bindContext(sfContext, sfSurface, sfSurface);
        ...
        GLint blitProgram = compileAndLinkShaderProgram(blitVshaderSrc, blitFshaderSrc);
        ...
        gl->glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), (GLvoid*)(uintptr_t)(2 * sizeof(GLfloat)));
        ...
        while (true) {
            ...
            sf2appQueue.queueBuffer({ appItem.colorBuffer, nullptr});
