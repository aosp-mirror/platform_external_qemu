...
void TextureDraw::drawLayer(struct composeLayer* l, int frameWidth, int frameHeight,
   ...
   if (!mProgram) {
    ...
    edges[1] = 1 - 2.0 * (frameHeight - l->displayFrame.bottom)/frameHeight;
    edges[2] = 1 - 2.0 * (frameWidth - l->displayFrame.right)/frameWidth;
    edges[3] = 1- 2.0 * (frameHeight - l->displayFrame.top)/frameHeight;
    ...
    Vertex vertices[] = {
        ...
        {{ edges[0], edges[3], +0 }, { crop[1], crop[3] }},
    };
