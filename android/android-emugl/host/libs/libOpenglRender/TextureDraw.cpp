...
void TextureDraw::drawSelf(const float edges[4], const float crop[4],
                           int mode, float alpha, GLuint texture,
...
//    Vertex vertices[] = {
//    {{ edges[2], edges[3], +0 }, { +1, +1 }},
//    {{ edges[2], edges[1], +0 }, { +1, +0 }},
//    {{ edges[0], edges[1], +0 }, { +0, +0 }},
//    {{ edges[0], edges[3], +0 }, { +0, +1 }},
//    };
    ...
    //TODO: use enum name and handle HWC2_BLEND_MODE_COVERAGE
    if (mode == 2) {
        s_gles2.glUniform1f(mAlpha, alpha);
        s_gles2.glEnable(GL_BLEND);
        s_gles2.glBlendFunc((mode == 2) ? GL_ONE : GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    }
