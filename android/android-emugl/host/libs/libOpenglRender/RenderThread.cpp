    ...
    while (1) {
        ...
        if (packetSize > (int)readBuf.validData()) {
            ...
            if (stat <= 0) {
                ...
                fprintf(stderr, "%s: need restore from snapshot, do a bind curr context draw read? %p %p %p\n", __func__,
                        bool(tInfo.currContext),
                        bool(tInfo.currDrawSurf),
                        bool(tInfo.currReadSurf));
                tInfo.postLoadRefreshCurrentContextSurfacePtrs();
                fprintf(stderr, "%s: after post load refresh\n", __func__);
