...
void RenderThread::postSnapshot(){
    android::base::AutoLock lock(mPostSnapshotLock);
    mResume = true;
    mPostSnapshot.signalAndUnlock(&lock);
}
        ...
        if (stat <= 0) {
            {
                ...
                if (m_snapshotStream) {
                    ...
                    mPostSnapshotLock.lock();
                    while (!mResume) {
                        mPostSnapshot.wait(&mPostSnapshotLock);
                    }
                    mPostSnapshotLock.unlock();
                    continue;
                }
            }
