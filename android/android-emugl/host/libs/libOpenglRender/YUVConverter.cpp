...
void YUVConverter::drawConvert(int x, int y,
    ...
    switch (mFormat) {
        ...
        fprintf(stderr, "fomrat is NV12 %s %d\n", __func__, __LINE__);
            if (callback) {
                if (!pixels) {
                    fprintf(stderr, "%s %d actually doing subUpdateYUVGLTexCallback UV\n", __func__, __LINE__);
