...
void main() {
    ...
    mat3 mvInvTr = mat3(transpose(inverse(modelview)));
    normal_varying = mvInvTr * normal;

    if (enable_rescale_normal) {
        float rescale = 1.0;
        vec3 rescaleVec = vec3(mvInvTr[2]);
...
void main() {
    ...
    for (int i = 0; i < kMaxLights; i++) {
        ...
        vec3 h = toLight + vec3(0, 0, 1);
        float ndotL = clamp(dot(norm, normalize(toLight)), 0, 1);
        float ndoth = clamp(dot(norm, normalize(h)), 0, 1);
        ...
        float spotAngleCos = cos(spotAngle * 3.1415926535 / 180);
        vec3 toSurfaceDir = -normalize(toLight);
        float spotDot = clamp(dot(toSurfaceDir, normalize(lightDir)), 0, 1);

        if (spot == 180 || lightPos.w == 0) {
            spot = 1.0;
        } else {
