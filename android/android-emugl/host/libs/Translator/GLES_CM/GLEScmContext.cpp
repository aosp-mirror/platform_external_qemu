...
const GLEScmContext::DrawTexOESCoreState& GLEScmContext::getDrawTexOESCoreState() {
    ...
    if (!m_drawTexOESCoreState.vao) {
        s_glDispatch.glGenVertexArrays(1, &m_drawTexOESCoreState.vao);
        s_glDispatch.glBindVertexArray(m_drawTexOESCoreState.vao);

        // Initialize VBO
        // Save IBO, attrib arrays/pointers to VAO
        s_glDispatch.glGenBuffers(1, &m_drawTexOESCoreState.ibo);
        s_glDispatch.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_drawTexOESCoreState.ibo);
        s_glDispatch.glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(sDrawTexIbo), sDrawTexIbo, GL_STATIC_DRAW);

        s_glDispatch.glGenBuffers(1, &m_drawTexOESCoreState.vbo);
        s_glDispatch.glBindBuffer(GL_ARRAY_BUFFER, m_drawTexOESCoreState.vbo);

        s_glDispatch.glEnableVertexAttribArray(0); // pos
        s_glDispatch.glEnableVertexAttribArray(1); // texcoord

        s_glDispatch.glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (GLvoid*)0);
        s_glDispatch.glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(float),
                                 (GLvoid*)(uintptr_t)(2 * sizeof(float)));

        s_glDispatch.glBindVertexArray(0);

        s_glDispatch.glBindBuffer(GL_ARRAY_BUFFER, 0);
