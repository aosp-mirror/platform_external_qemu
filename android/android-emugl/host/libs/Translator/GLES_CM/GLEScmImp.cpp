...
static uint32_t sDrawTexIbo[] = {
    0, 1, 2, 0, 2, 3,
};

template <class T, GLenum TypeName>
void glDrawTexOES_core (T x, T y, T z, T width, T height) {
    GET_CTX_CM()
    ...
    for (int i = 0; i < ctx->getMaxTexUnits(); i++) {
        if (ctx->isTextureUnitEnabled(GL_TEXTURE0 + i)) {
            ...
            if (objData) {
                ...
                gl.glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertexAttrs.size(), &vertexAttrs[0], GL_STATIC_DRAW);
            }
...
void glDrawTexOES (T x, T y, T z, T width, T height) {
    ...
    if (ctx->isCoreProfile()) {
        glDrawTexOES_core<T, TypeName>(x, y, z, width, height);
    } else {
        int numClipPlanes;
