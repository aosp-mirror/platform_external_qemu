...
CoreProfileEngine::CoreProfileEngine(GLEScmContext* ctx, bool isGles) :
    mCtx(ctx),
    mGles(isGles) {
    ...
    if (!m_drawTexOESCoreState.program) {
        m_drawTexOESCoreState.vshader =
            GLEScontext::compileAndValidateCoreShader(
                GL_VERTEX_SHADER,
                mGles ? kDrawTexOESGles2_vshader : kDrawTexOESCore_vshader);
        m_drawTexOESCoreState.fshader =
            GLEScontext::compileAndValidateCoreShader(
                GL_FRAGMENT_SHADER,
                mGles ? kDrawTexOESGles2_fshader : kDrawTexOESCore_fshader);
...
const CoreProfileEngine::GeometryDrawState& CoreProfileEngine::getGeometryDrawState() {
    ...
    if (!m_geometryDrawState.program) {
        ...
        m_geometryDrawState.vshader =
            GLEScontext::compileAndValidateCoreShader(
                GL_VERTEX_SHADER,
                sMakeGeometryDrawShader(mGles, GL_VERTEX_SHADER, false /* no flat shading */).c_str());

        m_geometryDrawState.fshader =
            GLEScontext::compileAndValidateCoreShader(
                GL_FRAGMENT_SHADER,
                sMakeGeometryDrawShader(mGles, GL_FRAGMENT_SHADER, false /* no flat shading */).c_str());
        ...
        m_geometryDrawState.vshaderFlat =
            GLEScontext::compileAndValidateCoreShader(
                    GL_VERTEX_SHADER,
                    sMakeGeometryDrawShader(mGles, GL_VERTEX_SHADER, true /* flat shading */).c_str());
        m_geometryDrawState.fshaderFlat =
            GLEScontext::compileAndValidateCoreShader(
                    GL_FRAGMENT_SHADER,
                    sMakeGeometryDrawShader(mGles, GL_FRAGMENT_SHADER, true /* flat shading */).c_str());
        m_geometryDrawState.programFlat =
            GLEScontext::linkAndValidateProgram(m_geometryDrawState.vshaderFlat,
                                                m_geometryDrawState.fshaderFlat);

        if (mGles) {
            m_geometryDrawState.posAttribLoc =
                gl.glGetAttribLocation(m_geometryDrawState.program, "pos");
            m_geometryDrawState.normAttribLoc =
                gl.glGetAttribLocation(m_geometryDrawState.program, "normal");
            m_geometryDrawState.colorAttribLoc =
                gl.glGetAttribLocation(m_geometryDrawState.program, "color");
            m_geometryDrawState.pointsizeAttribLoc =
                gl.glGetAttribLocation(m_geometryDrawState.program, "pointsize");
            m_geometryDrawState.texcoordAttribLoc =
                gl.glGetAttribLocation(m_geometryDrawState.program, "texcoord");
        }
...
void CoreProfileEngine::setupArrayForDraw(
    ...
    if (mGles) {
        switch (arrayType) {
            case GL_VERTEX_ARRAY:
                attribNum = m_geometryDrawState.posAttribLoc;
                break;
            case GL_NORMAL_ARRAY:
                attribNum = m_geometryDrawState.normAttribLoc;
                break;
            case GL_COLOR_ARRAY:
                attribNum = m_geometryDrawState.colorAttribLoc;
                break;
            case GL_POINT_SIZE_ARRAY_OES:
                attribNum = m_geometryDrawState.pointsizeAttribLoc;
                break;
            case GL_TEXTURE_COORD_ARRAY:
                attribNum = m_geometryDrawState.texcoordAttribLoc;
                break;
            default:
                emugl_crash_reporter(
                    "invalid gles1 array type: 0x%x\n", arrayType);
        }
    } else {
...
void CoreProfileEngine::preDrawTextureUnitEmulation() {
    ...
    gl.glUniform1i(m_geometryDrawState.enableTextureLoc,
                   (mCtx->isEnabled(GL_TEXTURE_2D) &&
                    mCtx->isArrEnabled(GL_TEXTURE_COORD_ARRAY)) ? 1 : 0);
