...
static char shaderPrintBuf[65535] = {};
const char* sSetAndReturnShader(GLenum shaderType, bool flat) {
    switch (shaderType) {
    case GL_VERTEX_SHADER:
        snprintf(shaderPrintBuf, sizeof(shaderPrintBuf),
                kGeometryDrawVShaderSrcTemplate, flat ? "flat" : "");
        break;
    case GL_FRAGMENT_SHADER:
        snprintf(shaderPrintBuf, sizeof(shaderPrintBuf),
                kGeometryDrawFShaderSrcTemplate, flat ? "flat" : "");
        break;
    default:
        fprintf(stderr, "%s: unknown shader type 0x%x\n", __func__, shaderType);
        abort();
    }
    return shaderPrintBuf;
}
    ...
    if (!m_geometryDrawState.program) {
        ...
        m_geometryDrawState.vshader =
            GLEScontext::compileAndValidateCoreShader(
                GL_VERTEX_SHADER,
                sSetAndReturnShader(GL_VERTEX_SHADER, false /* no flat shading */));

        snprintf(shaderPrintBuf, sizeof(shaderPrintBuf), kGeometryDrawFShaderSrcTemplate, "");
        m_geometryDrawState.fshader =
            GLEScontext::compileAndValidateCoreShader(
                GL_FRAGMENT_SHADER,
                sSetAndReturnShader(GL_FRAGMENT_SHADER, false /* no flat shading */));
        ...
        m_geometryDrawState.vshaderFlat =
            GLEScontext::compileAndValidateCoreShader(
                    GL_VERTEX_SHADER,
                    sSetAndReturnShader(GL_VERTEX_SHADER, true /* flat shading */));
        m_geometryDrawState.fshaderFlat =
            GLEScontext::compileAndValidateCoreShader(
                    GL_FRAGMENT_SHADER,
                    sSetAndReturnShader(GL_FRAGMENT_SHADER, true /* flat shading */));
...
void CoreProfileEngine::drawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid* indices) {
    ...
    gl.glUniform1i(m_geometryDrawState.enableTextureLoc,
                   isEnabled(GL_TEXTURE_2D) &&
                   mCtx->isArrEnabled(GL_TEXTURE_COORD_ARRAY));
    gl.glUniform1i(m_geometryDrawState.textureSamplerLoc, mCurrTextureUnit);
    gl.glUniform1i(m_geometryDrawState.textureCubeSamplerLoc, mCurrTextureUnit + 1);
