...
void CoreProfileEngine::setupLighting() {
    ...
    GLint lightEnables[GLEScmContext::kMaxLights];
    GLfloat lightAmbients[4 * GLEScmContext::kMaxLights];
    GLfloat lightDiffuses[4 * GLEScmContext::kMaxLights];
    GLfloat lightSpeculars[4 * GLEScmContext::kMaxLights];
    GLfloat lightPositions[4 * GLEScmContext::kMaxLights];
    GLfloat lightDirections[3 * GLEScmContext::kMaxLights];
    GLfloat spotlightExponents[GLEScmContext::kMaxLights];
    GLfloat spotlightCutoffAngles[GLEScmContext::kMaxLights];
    GLfloat attenuationConsts[GLEScmContext::kMaxLights];
    GLfloat attenuationLinears[GLEScmContext::kMaxLights];
    GLfloat attenuationQuadratics[GLEScmContext::kMaxLights];

    for (int i = 0; i < GLEScmContext::kMaxLights; i++) {
        lightEnables[i] = mCtx->isEnabled(GL_LIGHT0 + i);
        const auto& light = mCtx->getLightInfo(i);
        memcpy(lightAmbients + 4 * i, &light.ambient, 4 * sizeof(GLfloat));
        memcpy(lightDiffuses + 4 * i, &light.diffuse, 4 * sizeof(GLfloat));
        memcpy(lightSpeculars + 4 * i, &light.specular, 4 * sizeof(GLfloat));
        memcpy(lightPositions + 4 * i, &light.position, 4 * sizeof(GLfloat));
        memcpy(lightDirections + 3 * i, &light.direction, 3 * sizeof(GLfloat));
    ...
    gl.glUniform1iv(m_geometryDrawState.lightEnablesLoc, GLEScmContext::kMaxLights, lightEnables);
    gl.glUniform4fv(m_geometryDrawState.lightAmbientsLoc, GLEScmContext::kMaxLights, lightAmbients);
    gl.glUniform4fv(m_geometryDrawState.lightDiffusesLoc, GLEScmContext::kMaxLights, lightDiffuses);
    gl.glUniform4fv(m_geometryDrawState.lightSpecularsLoc, GLEScmContext::kMaxLights, lightSpeculars);
    gl.glUniform4fv(m_geometryDrawState.lightPositionsLoc, GLEScmContext::kMaxLights, lightPositions);
    gl.glUniform3fv(m_geometryDrawState.lightDirectionsLoc, GLEScmContext::kMaxLights, lightDirections);
    gl.glUniform1fv(m_geometryDrawState.lightSpotlightExponentsLoc, GLEScmContext::kMaxLights, spotlightExponents);
    gl.glUniform1fv(m_geometryDrawState.lightSpotlightCutoffAnglesLoc, GLEScmContext::kMaxLights, spotlightCutoffAngles);
    gl.glUniform1fv(m_geometryDrawState.lightAttenuationConstsLoc, GLEScmContext::kMaxLights, attenuationConsts);
    gl.glUniform1fv(m_geometryDrawState.lightAttenuationLinearsLoc, GLEScmContext::kMaxLights, attenuationLinears);
    gl.glUniform1fv(m_geometryDrawState.lightAttenuationQuadraticsLoc, GLEScmContext::kMaxLights, attenuationQuadratics);
}
