...
void ShaderParser::convertESSLToGLSL(int glesMajorVersion, int glesMinorVersion) {
    std::string infolog;
    std::string parsedSource;
    m_valid = ANGLEShaderParser::translate(glesMajorVersion, glesMinorVersion, m_originalSrc.c_str(), m_type, &infolog, &parsedSource);
...
void ShaderParser::setSrc(int glesMajorVersion, int glesMinorVersion, GLsizei count, const GLchar* const* strings, const GLint* length){
    m_src.clear();
    ...
    convertESSLToGLSL(glesMajorVersion, glesMinorVersion);
}
