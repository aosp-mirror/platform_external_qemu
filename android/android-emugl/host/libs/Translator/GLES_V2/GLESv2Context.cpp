...
GLESv2Context::GLESv2Context(int maj, int min, GlobalNameSpace* globalNameSpace,
    ...
    if (stream) {
        ...
        size_t numEmulatedClientVBOs = stream->getBe32();
...
void GLESv2Context::onSave(android::base::Stream* stream) const {
    ...
    stream->putBe32(m_emulatedClientVBOs.size());
}
...
void GLESv2Context::drawWithEmulations(const GLESv2Context::DrawCallParams& callParams) {
    ...
    DrawCallCmd cmd = callParams.cmd;
    GLenum mode = callParams.mode;
    GLuint first = callParams.first;
    GLsizei count = callParams.count;
    GLenum type = callParams.type;
    const GLvoid* indices = callParams.indices;
    GLuint primcount = callParams.primcount;
    GLuint start = callParams.start;
    GLuint end = callParams.end;
    ...
    if (needClientIBOSetup) {
        ...
        s_glDispatch.glBufferData(GL_ELEMENT_ARRAY_BUFFER, dataSize, indices, GL_STATIC_DRAW);
    }

    const GLvoid* indicesOrOffset =
        needClientIBOSetup ? (GLvoid*)0 : indices;
