...
void TransformFeedbackData::restore(ObjectLocalName localName,
    ...
    int globalName = getGlobalName(NamedObjectType::TRANSFORM_FEEDBACK,
            localName);
    ...
    for (unsigned int i = 0; i < m_indexedTransformFeedbackBuffers.size();
        ...
        if (!bufferBinding.buffer) continue;
        auto bufferName = getGlobalName(NamedObjectType::VERTEXBUFFER,
                bufferBinding.buffer);
        if (bufferBinding.isBindBase) {
            dispatcher.glBindBufferBase(GL_TRANSFORM_FEEDBACK, i,
                    bufferName);
        } else {
            dispatcher.glBindBufferRange(GL_TRANSFORM_FEEDBACK,
