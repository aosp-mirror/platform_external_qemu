...
static void getShaderLinkInfo(int esslVersion,
    ...
    for (int i = 0; i < uniformCount; i++) {
        fprintf(stderr, "%s: map uniform %s -> %s\n", __func__,
                          linkInfo->uniforms[i].name, linkInfo->uniforms[i].mapped_name);
        linkInfo->nameMap[linkInfo->uniforms[i].name] = linkInfo->uniforms[i].mapped_name;
    }

    for (int i = 0; i < varyingsCount; i++) {
        fprintf(stderr, "%s: map varying %s -> %s\n", __func__,
                          linkInfo->varyings[i].name, linkInfo->varyings[i].mapped_name);
        linkInfo->nameMap[linkInfo->varyings[i].name] = linkInfo->varyings[i].mapped_name;
    }

    for (int i = 0; i < attributesCount; i++) {
        fprintf(stderr, "%s: map attribute name %s -> %s\n", __func__,
                          linkInfo->attributes[i].name, linkInfo->attributes[i].mapped_name);

        linkInfo->nameMap[linkInfo->attributes[i].name] = linkInfo->attributes[i].mapped_name;
    }

    for (int i = 0; i < outputVarsCount; i++) {
        fprintf(stderr, "%s: map output %s -> %s\n", __func__,
                          linkInfo->outputVars[i].name, linkInfo->outputVars[i].mapped_name);
