...
GL_APICALL void  GL_APIENTRY glCompressedTexImage2D(GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border, GLsizei imageSize, const GLvoid* data)
{
    ...
    if ((isEtcFormat(internalformat) && ctx->getCaps()->hasEtc2Support)
...
GL_APICALL void  GL_APIENTRY glCompressedTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLsizei imageSize, const GLvoid* data){
	...
	if ((isEtcFormat(format) && ctx->getCaps()->hasEtc2Support)
    ...
    if (ctx->shareGroup().get()) {
        ...
        if (texData && level == 0) {
            ...
            if (GLESv2Validate::isCompressedFormat(internalformat)) {
                texData->compressed = true;
                texData->compressedFormat = internalformat;
                texData->internalFormat = ((isEtcFormat(internalformat) && ctx->getCaps()->hasEtc2Support) || (isAstcFormat(internalformat) && ctx->getCaps()->hasAstcSupport)) ? internalformat : decompressedInternalFormat(ctx, internalformat);
            } else {
                texData->internalFormat = internalformat;
            }
