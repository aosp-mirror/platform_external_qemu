...
GL_APICALL int GL_APIENTRY glGetUniformLocation(GLuint program, const GLchar* name){
    ...
    if(ctx->shareGroup().get()) {
        ...
        // Sort it out here.
        int hostLoc = ctx->dispatcher().glGetUniformLocation(
            globalProgramName, pData->getTranslatedName(name).c_str());
...
GL_APICALL void  GL_APIENTRY glUniform1i(GLint location, GLint x){
    ...
    int hostLoc = s_getHostLocOrSetError(location);
    if (hostLoc >= 0) {
        ctx->dispatcher().glUniform1i(hostLoc,count,v);
    }
}
...
GL_APICALL void  GL_APIENTRY glUniform2f(GLint location, GLfloat x, GLfloat y){
    ...
    int hostLoc = s_getHostLocOrSetError(location);
    if (hostLoc >= 0) {
        ctx->dispatcher().glUniform2f(hostLoc,count,v);
    }
}
...
GL_APICALL void  GL_APIENTRY glUniform2i(GLint location, GLint x, GLint y){
    ...
    int hostLoc = s_getHostLocOrSetError(location);
    if (hostLoc >= 0) {
        ctx->dispatcher().glUniform2i(hostLoc,count,v);
    }
}
