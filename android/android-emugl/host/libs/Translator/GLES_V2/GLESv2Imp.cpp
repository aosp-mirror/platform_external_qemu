...
GL_APICALL void  GL_APIENTRY glGetActiveUniform(GLuint program, GLuint index, GLsizei bufsize, GLsizei* length, GLint* size, GLenum* type, GLchar* name){
    ...
    if(ctx->shareGroup().get()) {
        ...
        ProgramData* pData = (ProgramData *)objData;
        // ctx->dispatcher().glGetActiveUniform(
        //     globalProgramName, index, bufsize, length, size, type,
        //     name);
        ...
        result[0] = watch_val;
        ctx->dispatcher().glGetActiveUniform(
            globalProgramName, index, bufsize, length, size, type, &result[0]);
        // Don't even touch it if name query didn't touch result,
        // or garbage will result.
        if (result[0] != watch_val &&
            (!length || (length && *length))) { // don't touch if length known 0
            memcpy(name, pData->getDetranslatedName(result).c_str(), bufsize);
        }
    }
}
