...
GL_APICALL int GL_APIENTRY glGetUniformLocation(GLuint program, const GLchar* name){
    ...
    if(ctx->shareGroup().get()) {
        ...
        int loc = pData->getGuestUniformLocation(name);
        int driverLoc = ctx->dispatcher().glGetUniformLocation(
            globalProgramName, pData->getTranslatedName(name).c_str());
        if (loc < 0) {
            //printf("%s (%s) not found.", name, pData->getTranslatedName(name).c_str());
            //printf(" Driver loc %d\n", driverLoc);
            assert(driverLoc < 0);
        }
        int hostLoc = pData->getHostUniformLocation(loc);
        if (hostLoc != driverLoc) {
            printf("%s (%s) loc changed (%d -> %d)\n",
                name, pData->getTranslatedName(name).c_str(),
                hostLoc, driverLoc);
        }
        assert(hostLoc == driverLoc);
        return loc;
    }
    return -1;
}
