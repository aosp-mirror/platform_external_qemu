...
#define VALIDATE(cond, err) do { if (cond) { *err_out = err; return; } } while(0) \
    ...
    if (!isCompressedFormat) {
        ...
        VALIDATE((format == GL_DEPTH_COMPONENT || internalformat == GL_DEPTH_COMPONENT) &&
                (type != GL_UNSIGNED_SHORT && type != GL_UNSIGNED_INT), GL_INVALID_OPERATION);

        VALIDATE((type == GL_UNSIGNED_SHORT || type == GL_UNSIGNED_INT) &&
