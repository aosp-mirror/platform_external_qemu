...
GL_APICALL void  GL_APIENTRY glCompressedTexImage2D(GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLint border, GLsizei imageSize, const GLvoid* data)
{
    ...
    if (ctx->getCaps()->hasFullCompressedSupport) {
    ...
    if (texData) {
        ...
        if (ctx->getCaps()->hasFullCompressedSupport) texData->internalFormat = internalformat;
    }
}
    ...
    if (ctx->shareGroup().get()) {
        ...
        if (texData && level == 0) {
            ...
            if (GLESv2Validate::isCompressedFormat(internalformat)) {
                texData->compressed = true;
                texData->compressedFormat = internalformat;
                texData->internalFormat = ctx->getCaps()->hasFullCompressedSupport ? internalformat : decompressedInternalFormat(ctx, internalformat);
            } else {
