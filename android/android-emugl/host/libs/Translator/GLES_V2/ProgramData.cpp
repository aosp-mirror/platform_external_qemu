...
GLUniformDesc::GLUniformDesc(GLsizei count, GLboolean transpose, GLenum type,
            GLsizei size, unsigned char* val)
        : mCount(count), mTranspose(transpose), mType(type), mSize(size)
        , mVal(val) { }
...
void ProgramData::addUniform(GLuint loc, GLUniformDesc&& uniform) {
    uniforms.emplace(loc, std::move(uniform));
}
