...
static void getUniformValue(GLuint pname, GLchar *name, GLenum type,
...
static std::unordered_map<GLuint, GLUniformDesc> collectUniformInfo(GLuint pname) {
    ...
    for (int i = 0; i < uniform_count; i++) {
        ...
        GLenum type;
        GLsizei length;
        // 13-digit decimal number enough to represent GLint, plus 2 bytes for "[]"
        std::unique_ptr<char[]> name(new char[nameLength + 15]);
        dispatcher.glGetActiveUniform(pname, i, nameLength, &length, &size, &type, (GLchar *)name.get());
        if (size > 1) {
            ...
            if(strncmp(name.get() + length -3, "[0]", 3) == 0) {
                length -= 3;
            }
            for (int arrayIndex = 0; arrayIndex < size; arrayIndex++) {
                sprintf(name.get() + length, "%c%d%c%c", '[', arrayIndex, ']', '\0');
                getUniformValue(pname, name.get(), type, uniformsOnSave);
            }
        }
        else {
            getUniformValue(pname, name.get(), type, uniformsOnSave);
