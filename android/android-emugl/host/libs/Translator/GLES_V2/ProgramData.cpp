...
bool ProgramData::validateLink(ShaderParser* frag, ShaderParser* vert) {
    ...
    if (!res) {
        // Also print error message to the user immediately.
...
static bool sCheckLimits(
    ...
    for (const auto& elt : vertShaderLinkInfo->attributes) {
        if (pData->boundAttribLocs.find(elt.name) !=
            pData->boundAttribLocs.end()) {
            explicitlyBound.insert(pData->boundAttribLocs[elt.name]);
        } else {
            numImplicitlyBound++;
        }
    }
