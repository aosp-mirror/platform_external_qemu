...
class TextureGlobal {
public:
    TextureGlobal() = default;
    TextureGlobal(TextureGlobal&&) = default;
    TextureGlobal& operator=(TextureGlobal&&) = default;
    TextureGlobal(const EglImage& eglImage);
    TextureGlobal(const TextureData& texture);
    // precondition: a context must be properly bound
    TextureGlobal(android::base::Stream* stream,
        GlobalNameSpace* globalNameSpace, GLDispatch* dispatcher);
    // precondition: a context must be properly bound
    void onSave(android::base::Stream* stream, unsigned int globalName,
        GLDispatch* dispatcher) const;
