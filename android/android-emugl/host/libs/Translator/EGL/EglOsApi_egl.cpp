...
#define DECLARE_EGL_POINTER(return_type, function_name, signature) \
    ...
    EglOsEglDispatcher() {
        static const char kLibName[] = "libEGL.dll";
        D("loading %s\n", kLibName);
        char error[256];
...
public:
    EglOsGlLibrary() {
        const char* kLibName = "libGLESv2.dll";
        char error[256];
        mLib = emugl::SharedLibrary::open(kLibName, error, sizeof(error));
        if (!mLib) {
            ERR("%s: Could not open GL library %s [%s]\n",
...
Surface* EglOsEglDisplay::createWindowSurface(PixelFormat* pf,
    ...
    D("%s\n", __FUNCTION__);
    EGLSurface surface = mDispatcher.eglCreateWindowSurface(mDisplay,
        ((EglOsEglPixelFormat*)pf)->mConfigId, win, nullptr);
    CHECK_EGL_ERR
    if (surface == EGL_NO_SURFACE) {
...
bool EglOsEglDisplay::makeCurrent(Surface* read, Surface* draw,
    ...
    bool ret = mDispatcher.eglMakeCurrent(mDisplay,
        drawSfc ? drawSfc->getHndl() : EGL_NO_SURFACE,
        readSfc ? readSfc->getHndl() : EGL_NO_SURFACE,
        ctx ? ctx->mNativeCtx : EGL_NO_CONTEXT);
    if (readSfc) {
        D("make current surface type %d %d\n", readSfc->type(), drawSfc->type());
    }
    D("make current %d\n", ret);
    CHECK_EGL_ERR
    return ret;
