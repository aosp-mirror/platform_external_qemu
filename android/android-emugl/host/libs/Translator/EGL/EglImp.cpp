...
static EGLContext eglCreateOrLoadContext(EGLDisplay display, EGLConfig config,
    ...
    if (stream) {
        EGLint configId;
        assert(EglContext::onLoad(stream, configId, shareGroupId));
        config = dpy->getConfig(configId);
    }
    ...
    if(nativeContext) {
        ContextPtr ctx(new EglContext(dpy, nativeContext,shareGroupId,cfg,glesCtx,glesVersion,dpy->getManager(glesVersion)));
...
EGLAPI EGLBoolean EGLAPIENTRY eglSaveContext(EGLDisplay display, EGLContext contex, EGLStream stream) {
    printf("eglSaveContext\n");
    VALIDATE_DISPLAY(display);
    VALIDATE_CONTEXT(contex);
    ctx->onSave((emugl::Stream*)stream);
    printf("eglSaveContext done\n");
    return EGL_TRUE;
}

EGLAPI EGLContext EGLAPIENTRY eglLoadContext(EGLDisplay display, const EGLint *attrib_list, EGLStream stream) {
    return eglCreateOrLoadContext(display, (EGLConfig)0, EGL_NO_CONTEXT, attrib_list, (emugl::Stream*)stream);
}