...
EGLAPI EGLBoolean EGLAPIENTRY eglPreSaveImages(EGLDisplay display, EGLImageKHR image, EGLStream stream);
...
EGLAPI EGLBoolean EGLAPIENTRY eglPreSaveImage(EGLDisplay display, EGLImageKHR image, EGLStream stream) {
    if (!image) return true;
    VALIDATE_DISPLAY(display);
    dpy->getGlobalNameSpace()->preSaveAddEglImage(
            dpy->getImage(image).get());
    return EGL_TRUE;
}
