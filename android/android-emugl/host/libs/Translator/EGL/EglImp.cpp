...
EGLAPI EGLBoolean eglPostSaveContext(EGLDisplay display, EGLContext context, EGLStream stream);
...
EGLAPI EGLBoolean EGLAPIENTRY eglMakeCurrent(EGLDisplay display,
    ...
    if(releaseContext) { //releasing current context
       if(prevCtx.get()) {
        ...
        //newCtx->getShareGroup()->postLoadRestore();
...
EGLBoolean eglPostSaveContext(EGLDisplay display, EGLContext context, EGLStream stream) {
    VALIDATE_DISPLAY(display);
    VALIDATE_CONTEXT(context);
