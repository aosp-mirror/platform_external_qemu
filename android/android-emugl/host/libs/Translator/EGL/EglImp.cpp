...
EGLAPI EGLBoolean EGLAPIENTRY eglSaveAllImages(EGLDisplay display, EGLStream stream, const char* snapshotDir);
EGLAPI EGLBoolean EGLAPIENTRY eglLoadAllImages(EGLDisplay display, EGLStream stream, const char* snapshotDir);
...
EGLAPI EGLBoolean EGLAPIENTRY eglSaveAllImages(EGLDisplay display,
        EGLStream stream, const char* snapshotDir) {
    const GLESiface* iface = g_eglInfo->getIface(GLES_2_0);
    assert(iface->saveTexture);
