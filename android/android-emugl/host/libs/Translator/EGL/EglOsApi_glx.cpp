...
#include "GlxCoreProfileConfigs.h"
...
public:
    ...
    virtual void queryConfigs(int renderableType,
        ...
        if (frmtList) {
            mFBConfigs.resize(n);
            memcpy(&mFBConfigs[0], frmtList, n * sizeof(GLXFBConfig));
            for(int i = 0; i < n; i++) {
                pixelFormatToConfig(
                        mDisplay,
                        renderableType,
        ...
        if (successQueryVersion) {
            fprintf(stderr, "%s: GLX version: %d %d\n", __func__, glxMaj, glxMin);
            if (glxMaj < 1 || (glxMaj >= 1 && glxMin < 4)) {
                // core profile not supported in this GLX.
                mCoreProfileSupported = false;
            } else {
                fprintf(stderr, "%s: querying core profile support.\n", __func__);
    ...
    virtual emugl::SmartPtr<EglOS::Context> createContext(
        ...
        bool isCoreProfile = profileMask & EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT_KHR;

        ErrorHandler handler(mDisplay);

        GLXContext ctx;
        if (mCoreProfileSupported) {
            fprintf(stderr, "%s: core profile supprted, creating context.\n", __func__);
...
private:
    ...
    void queryCoreProfileSupport() {
        ...
        if (!mCreateContextAttribs) {
            fprintf(stderr, "%s: no create context attribs func!\n", __func__);
        } else {
            ...
            for (int i = 0; i < getNumCoreProfileCtxAttribs(); i++) {
                ...
                if (testContext) {
                    fprintf(stderr, "%s: succeeded on context attribs %d.\n", __func__);
                    mCoreProfileSupported = true;
                    mCoreProfileCtxAttribs = attribs;
                    int glmaj, glmin;
                    getCoreProfileCtxAttribsVersion(attribs, &glmaj, &glmin);
                    fprintf(stderr, "%s: gl major minor version %d %d\n", __func__, glmaj, glmin);
