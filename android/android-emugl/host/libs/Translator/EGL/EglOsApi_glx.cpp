...
#define PROFILE_SLOW_EGL 1
#define DEBUG_PBUF_POOL 1
...
public:
    ...
    virtual emugl::SmartPtr<EglOS::Context> createContext(
        ...
        for (const auto it : mFreePbufs) {
            totalPbufs += it.second.size();
        }

        size_t totalPbufsLive = 0;
        for (const auto it : mLivePbufs) {
            totalPbufsLive += it.second.size();
        }
    ...
    virtual EglOS::Surface* createPbufferSurface(
        ...
        if (needPrime) {
            ...
            auto& newPbufs = mFreePbufs[config];
            // Double the # pbufs of this config, or create |mPbufPrimingCount|
            // of them, whichever is higher.
            int toCreate = std::max((int)mLivePbufs[config].size(), mPbufPrimingCount);
            for (int i = 0; i < toCreate; i++) {
                newPbufs.push_back(createPbufferSurfaceImpl(config));
            }
        }
