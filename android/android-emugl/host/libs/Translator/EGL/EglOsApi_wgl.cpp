...
public:
    ...
    virtual emugl::SmartPtr<EglOS::Context> createContext(
        ...
        bool useCoreProfile = profileMask & EGL_CONTEXT_OPENGL_CORE_PROFILE_BIT_KHR;
        ...
        if (useCoreProfile) {
            ...
            ctx = mDispatch->wglCreateContextAttribsARB(dpy, WinContext::from(sharedContext), attribs);
            fprintf(stderr, "%s: using core profile\n", __func__);
        } else {
...
private:
    ...
    void queryCoreProfileSupport() {
        ...
        if (!mDispatch->wglCreateContextAttribsARB) {
            fprintf(stderr, "%s: create context attribs not found\n", __func__);
            return;
        }
        ...
        for (int i = 0; i < getNumCoreProfileCtxAttribs(); i++) {
            ...
            if (testContext) {
                ...
                fprintf(stderr, "%s: core profile supported @ %d.%d\n", __func__, glmaj, glmin);
                return;
            }
        }
    }
