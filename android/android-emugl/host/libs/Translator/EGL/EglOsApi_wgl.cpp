...
public:
    ...
    virtual void queryConfigs(int renderableType,
        ...
        fprintf(stderr, "%s: query swap interval\n", __func__);
        if (mDispatch->wglGetSwapIntervalEXT) {
            fprintf(stderr, "%s: swap interval: %d\n", __func__, mDispatch->wglGetSwapIntervalEXT());
        }
        if (mDispatch->wglSwapIntervalEXT) {
            int interval = 0;
            fprintf(stderr, "%s: set swap interval to %d\n", __func__, interval);
            mDispatch->wglSwapIntervalEXT(interval);
        }
    }
