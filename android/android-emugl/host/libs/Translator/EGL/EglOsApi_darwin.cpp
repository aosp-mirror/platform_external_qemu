...
void initNativeConfigs(){
    ...
    s_nativeFormats.resize(nConfigs);
    std::iota(s_nativeFormats.begin(),
              s_nativeFormats.end(), 0);
...
public:
    ...
    virtual void queryConfigs(int renderableType,
                              ...
                              void* addConfigOpaque) {
        initNativeConfigs();
        for (const auto& nativeFormatId : s_nativeFormats) {
            pixelFormatToConfig(
                nativeFormatId,
                addConfigFunc,
                addConfigOpaque);
    ...
    virtual EglOS::Context* createContext(
        ...
        return new MacContext(isCoreProfile,
