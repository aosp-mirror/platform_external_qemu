...
SaveableTexture::SaveableTexture(const EglImage& eglImage)
    : m_width(eglImage.width),
      ...
      m_numLevels(eglImage.numLevels),
    ...
    if (m_target == GL_TEXTURE_2D || m_target == GL_TEXTURE_CUBE_MAP ||
        ...
        if (m_numLevels) {
            ...
            switch (m_target) {
                case GL_TEXTURE_2D:
                case GL_TEXTURE_CUBE_MAP:
                    dispatcher.glTexStorage2D(m_target, m_numLevels,
                            m_internalFormat,
                ...
                case GL_TEXTURE_3D:
                case GL_TEXTURE_2D_ARRAY:
                    dispatcher.glTexStorage3D(m_target, m_numLevels,
                            m_internalFormat,
                            m_levelData[0].get()[0].m_width,
                            m_levelData[0].get()[0].m_height,
...
void SaveableTexture::fillEglImage(EglImage* eglImage) {
    ...
    eglImage->numLevels = m_numLevels;
}
