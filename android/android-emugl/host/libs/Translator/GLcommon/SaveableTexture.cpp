...
SaveableTexture::SaveableTexture(const TextureData& texture)
    : m_target(texture.target),
      ...
      m_isDirty(true) {
    // fprintf(stderr, "Constructed saveabletexture with name 0x%x\n", m_globalName);
}
...
SaveableTexture::SaveableTexture(GlobalNameSpace* globalNameSpace,
    ...
    : m_loader(std::move(loader)),
      ...
      m_isDirty(false) {
    mNeedRestore = true;
    // fprintf(stderr, "Constructed saveabletexture from namespace, globalname 0x%x\n", m_globalName);
}
...
void SaveableTexture::onSave(
    ...
    if (m_target == GL_TEXTURE_2D || m_target == GL_TEXTURE_CUBE_MAP ||
        ...
        fprintf(stderr, "saveable Binding texture named 0x%x\n", m_globalName);
...
void SaveableTexture::restore() {
    ...
    fprintf(stderr, "Restoring saveabletexture 0x%x\n", m_globalTexObj->getGlobalName());
    if (m_target == GL_TEXTURE_2D || m_target == GL_TEXTURE_CUBE_MAP ||
        m_target == GL_TEXTURE_3D || m_target == GL_TEXTURE_2D_ARRAY) {
        // restore the texture
        GLDispatch& dispatcher = GLEScontext::dispatcher();
