...
SaveableTexture::SaveableTexture(android::base::Stream* stream,
    ...
    if (m_target == GL_TEXTURE_2D || m_target == GL_TEXTURE_CUBE_MAP
        ...
        static constexpr GLenum pixelStorIndexes[] = {
        ...
        static constexpr GLint pixelStorDesired[] = {0, 0, 0, 0, 0, 1};

        GLint pixelStorPrev[android::base::arraySize(pixelStorIndexes)];

        for (int i = 0; i != android::base::arraySize(pixelStorIndexes); ++i) {
            dispatcher.glGetIntegerv(pixelStorIndexes[i], &pixelStorPrev[i]);
            if (pixelStorPrev[i] != pixelStorDesired[i]) {
                dispatcher.glPixelStorei(pixelStorIndexes[i], pixelStorDesired[i]);
            }
        }
        ...
        for (int i = 0; i != android::base::arraySize(pixelStorIndexes); ++i) {
            if (pixelStorPrev[i] != pixelStorDesired[i]) {
                dispatcher.glPixelStorei(pixelStorIndexes[i], pixelStorPrev[i]);
            }
        }
...
void SaveableTexture::onSave(android::base::Stream* stream) const {
    ...
    if (m_target == GL_TEXTURE_2D || m_target == GL_TEXTURE_CUBE_MAP
        ...
        static constexpr GLint pixelStorDesired[] = { 0,0,0,1 };
        GLint pixelStorPrev[android::base::arraySize(pixelStorIndexes)];
