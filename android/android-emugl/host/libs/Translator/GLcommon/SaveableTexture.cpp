...
static void writeBmp(const char* fileName, int width, int height, int channels,
    ...
    for(int i=0; i<w; i++)
    {
        for(int j=0; j<h; j++)
        {
            int x=i;
            int y=(h-1)-j;
            int r = data[(i + j * w) * channels + 0];
            int g = data[(i + j * w) * channels + 1];
            int b = data[(i + j * w) * channels + 2];
            if (r > 255) r=255;
            if (g > 255) g=255;
            if (b > 255) b=255;
    ...
    if (m_target == GL_TEXTURE_2D || m_target == GL_TEXTURE_CUBE_MAP ||
        ...
        switch (m_target) {
            case GL_TEXTURE_2D:
                ...
                {
                    std::vector<unsigned char> rgb(m_width * m_height * 4);
                    //dispatcher.glReadPixels(0, 0, m_width, m_height, GL_RGBA,
                    //        GL_UNSIGNED_BYTE, rgb.data());
                    dispatcher.glGetTexImage(GL_TEXTURE_2D, 0, GL_RGB,
                            GL_UNSIGNED_BYTE, rgb.data());
                    char name[1000];
                    sprintf(name, "tmp/%d.bmp", m_globalName);
                    writeBmp(name, m_width, m_height, 3, rgb.data());
                }
                break;
            case GL_TEXTURE_CUBE_MAP:
