...
void TextureData::restore(ObjectLocalName localName,
    ...
    if (sourceEGLImage) {
        ...
        printf("restoring tex: %llu(%d) internalformat  %d width %d height %d border %d format %d type %d (%d, %d)\n", localName, globalName,
                internalFormat, width, height, border, format, type, data[0], data[data.size()-1]);
    }
