...
void FramebufferData::separateDepthStencilWorkaround(GLEScontext* ctx) {
    ...
    GLint widthDepth;
    GLint heightDepth;
    getAttachmentDimensions(ctx, GL_DEPTH_ATTACHMENT, &widthDepth, &heightDepth);
    ...
    gl.glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, widthDepth, heightDepth);

    gl.glBindFramebuffer(GL_DRAW_FRAMEBUFFER, ctx->getFBOGlobalName(m_fbName));
    gl.glFramebufferRenderbuffer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_separateDSEmulationRbo);
