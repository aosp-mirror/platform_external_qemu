...
void GLEScontext::initCapsLocked(const GLubyte * extensionString)
{
    ...
    if (numCompressedFormats > 0) {
        ...
        for (int i = 0; i < numCompressedFormats; ++i) {
            ...
            forEachEtc2Format([&numEtc2FormatsSupported, &found, format](GLenum etcFormat) {
                if (format == etcFormat) { ++numEtc2FormatsSupported; found[format] = true;} });
            forEachAstcFormat([&numAstcFormatsSupported, &found, format](GLenum astcFormat) {
                if (format == astcFormat) { ++numAstcFormatsSupported;found[format] = true; } });
        }

        if (numEtc2Formats == numEtc2FormatsSupported) {
            // Supports ETC2 underneath
        } else {
        ...
        if ((numEtc2Formats == numEtc2FormatsSupported) &&
            ...
            hasEtc2AstcSupport = true;
        }
    }
    s_glSupport.hasEtc2AstcSupport = hasEtc2AstcSupport;
