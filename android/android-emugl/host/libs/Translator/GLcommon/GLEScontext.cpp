...
void GLEScontext::blitFromReadBufferToTextureFlipped(
    ...
    const GLuint globalProgramName =
        shareGroup()->getGlobalName(NamedObjectType::SHADER_OR_PROGRAM,
                m_useProgram);
    // ??????????????????????
    gl.glBindBuffer(GL_ARRAY_BUFFER,
            shareGroup()->getGlobalName(NamedObjectType::VERTEXBUFFER,
                getBuffer(GL_ARRAY_BUFFER)));
    ...
    gl.glBindFramebuffer(GL_DRAW_FRAMEBUFFER,
                         getFBOGlobalName(getFramebufferBinding(GL_DRAW_FRAMEBUFFER)));
    gl.glBindFramebuffer(GL_READ_FRAMEBUFFER,
                         getFBOGlobalName(getFramebufferBinding(GL_READ_FRAMEBUFFER)));
