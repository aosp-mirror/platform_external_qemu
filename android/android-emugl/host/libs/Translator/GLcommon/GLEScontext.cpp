...
void GLEScontext::restore() {
    ...
    if (!m_needRestoreFromSnapshot) {
        //fprintf(stderr, "%s Doesn't need restore from snapshot!?!?!\n", __FILE__);
    }
    if (m_needRestoreFromSnapshot) {
        postLoadRestoreCtx();
        fprintf(stderr, "\n================== restoring context =========================\n\n");
...
bool GLEScontext::needRestore() {
    ...
    if (m_needRestoreFromSnapshot) {
        fprintf(stderr, "%s context asked needRestore, NEEDS SNAPSHOT %d\n", __FILE__, m_needRestoreFromSnapshot);
    }
    else {
        fprintf(stderr, "%s needRestore returning %d\n", __FILE__, ret);
    }
    return ret;
}
