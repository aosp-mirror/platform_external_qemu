...
void doCompressedTexImage2D(GLEScontext* ctx, GLenum target, GLint level,
    ...
    if (isEtcFormat(internalformat)) {
        ...
        AlignedBuf<uint8_t, 64> alignedCompressedData(imageSize);
        memcpy(alignedCompressedData.data(), data, imageSize);
        AlignedBuf<uint8_t, 64> alignedUncompressedData(size);

        astc_decode_rgba_func((uint8_t*)alignedCompressedData.data(), width, height, xblocks, yblocks, alignedUncompressedData.data());

        glTexImage2DPtr(target, level, srgb? GL_SRGB8_ALPHA8 : GL_RGBA8,
                        width, height, border, GL_RGBA, GL_UNSIGNED_BYTE, alignedUncompressedData.data());
