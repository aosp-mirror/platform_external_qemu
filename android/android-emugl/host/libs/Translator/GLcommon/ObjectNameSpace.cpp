...
void GlobalNameSpace::onLoad(android::base::Stream* stream,
    ...
    auto weakLoader = std::weak_ptr<TextureLoader>(textureLoader);
    loadCollection(
            stream, &m_textureMap,
            [this, creator, &weakLoader](android::base::Stream* stream) {
                ...
                SaveableTexture* saveableTexture = creator(
                        this, [globalName,
                               weakLoader](SaveableTexture* saveableTexture) {
                            if (auto loader = weakLoader.lock()) {
                                loader->loadTexture(
                                        globalName,
                                        [saveableTexture](
                                                android::base::Stream* stream) {
                                            saveableTexture->loadFromStream(
                                                    stream);
                                        });
                            }
                        });
