                              ...
                              void* library_out)
{
    ...
    *(emugl::SharedLibrary**)library_out = lib;
    ...
    return gles2_dispatch_init_from_lib(lib, egl_dispatch, dispatch_table, false);
}

bool gles2_dispatch_init_from_lib(void* _lib, EGLDispatch* egl_dispatch, GLESv2Dispatch* dispatch_table, bool useEgl) {
...
#define LOOKUP_SYMBOL(return_type,function_name,signature,callargs) \
    ...
    if (useEgl || ((!dispatch_table-> function_name) && egl_dispatch->eglGetProcAddress)) \
        dispatch_table-> function_name = reinterpret_cast< function_name ## _t >( \
