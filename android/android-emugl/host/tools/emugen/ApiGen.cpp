    ...
    for (size_t i = 0; i < size(); i++) {
        ...
        for (size_t j = 0; j < n; j++) {
            if (!evars[j].isVoid()) {
                ...
                evars[j].printTypeName(fp);
            }
            if (e->customDecoder() && e->notApi()) {
                if (evars[j].isPointer() &&
                    (evars[j].pointerDir() & Var::POINTER_IN)) {
                    fprintf(fp, ", size_t encSize_%s", evars[j].name().c_str());
                }
            }
        }
        fprintf(fp, ");\n");
...
R"(    const size_t checksumSize = checksumCalc ? checksumCalc->checksumByteSize() : 0;
    ...
    fprintf(fp,
"\twhile (end - ptr >= 8) {\n\
\t\tif (ptr >= end) return ptr - (unsigned char*)buf; \n\
...
R"(        // Do this on every iteration, as some commands may change the checksum
    ...
    for (size_t f = 0; f < n; f++) {
        ...
        // fprintf(fp, "\t\tfprintf(stderr, \"run OP_%s\\n\");\n", e->name().c_str());
                    ...
                    if (v->pointerDir() & Var::POINTER_IN) {
                        ...
                        if (pass == PASS_FunctionCall &&
                            ...
                            if (e->customDecoder() && e->notApi()) {
                                if (v->nullAllowed()) {
                                    fprintf(fp,
                                            "size_%s == 0 ? nullptr : (%s)(inptr_%s.get()), size_%s",
                                            var_name,
                                            var_type_name,
                                            var_name,
                                            var_name);
