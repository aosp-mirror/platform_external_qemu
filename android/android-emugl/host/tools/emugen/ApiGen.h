...
public:
    ...
    ApiGen(std::string basename)
        : m_basename(std::move(basename)),
          m_maxEntryPointsParams(0),
          m_baseOpcode(0) {}
    virtual ~ApiGen() = default;
    ...
    void updateMaxEntryPointsParams(size_t val) {
        if (m_maxEntryPointsParams == 0 || val > m_maxEntryPointsParams)
            m_maxEntryPointsParams = val;
    }
...
protected:
    ...
    size_t m_maxEntryPointsParams;  // record the maximum number of parameters
                                    // in the entry points;
    int m_baseOpcode;
