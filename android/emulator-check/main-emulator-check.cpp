...
static const int badReturn = 100;
...
static CommandReturn getWindowManager() {
    const std::string name = android::base::getWindowManagerName();
    return std::make_pair(name.empty() ? badReturn : 0, name);
}
...
static int processArguments(int argc, const char* const* argv) {
    ...
    return retCode.valueOr(badReturn);
}
