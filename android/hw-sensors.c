...
static void _hwSensorClient_tick(void* opaque, LoopTimer* unused) {
    ...
    // Grab the guest time before sending any sensor data: the CTS requires
    // sync times to be no greater than the time of the sensor event arrival.
    // Since the CTS enforces this property, other code may also rely on it.
    // Note: failing tests used to include:
    //  - "android.hardware.cts.test%SensorName%_fastest"
    //  - "android.hardware.cts.test%SensorName%_200hz"
    //  - "android.hardware.cts.test%SensorName%_fastest_batching"
    //  ... and some others, they were quite flaky because of the very nature of
    //  the test, dependent on the emulator and device timings.
