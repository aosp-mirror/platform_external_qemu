...
AConfig* aconfig_node(const char* name, const char* value) {
    AConfig* n;
...
void aconfig_node_free(AConfig* root) {
    AConfig *node, *next;
...
static int parse_block(cstate* cs, AConfig* node) {
    for (;;) {
        switch (lex(cs, 0)) {
            case T_TEXT:
                if (parse_expr(cs, node))
                    return -1;
                continue;
...
void aconfig_load(AConfig* root, char* data) {
    if (data != 0) {
        cstate cs;
        cs.data = data;
        cs.next = 0;
...
int aconfig_load_file(AConfig* root, const char* fn) {
    char* data;
...
static int writer_init(Writer* w, const char* fn) {
    w->p = w->buff;
...
static void writer_write(Writer* w, const char* src, int len) {
    while (len > 0) {
        int avail = w->end - w->p;
...
static void writer_done(Writer* w) {
    if (w->p > w->buff) {
...
static void writer_margin(Writer* w, int margin) {
...
static void writer_c(Writer* w, char c) {
    writer_write(w, &c, 1);
}

static void writer_str(Writer* w, const char* str) {
