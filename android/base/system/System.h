...
public:
    ...
    // Returns: if |wait| == false, returns child process ID on success
    // and RunFailed on failure. For |wait| == true, returns process' exit code
    // (or RunFailed if failed to launch it)
    virtual int runSilentCommand(const StringVector& commandLine,
                                 bool wait = false) = 0;
