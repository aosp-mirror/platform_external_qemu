...
#include "android/base/system/PlatformInfo.h"
...
static std::string getLinuxWindowManagerName() {
    ...
    auto supWindowPtr = getXProperty(disp.get(), DefaultRootWindow(disp.get()),
                                     XA_WINDOW, "_NET_SUPPORTING_WM_CHECK");
    if (supWindowPtr.empty()) {
        supWindowPtr = getXProperty(disp.get(), DefaultRootWindow(disp.get()),
    ...
    if (supWindowPtr.size() < sizeof(Window)) {
        // Bad size returned, has to be at least of a Window type size
        return {};
    }

    const auto supWindow = reinterpret_cast<const Window*>(supWindowPtr.data());
