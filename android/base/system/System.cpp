    ...
    virtual const String& getHomeDirectory() const {
        if (mHomeDir.empty()) {
            ...
            if (SUCCEEDED(
                ...
                mHomeDir = Win32UnicodeString(path).toString();
            } else {
    ...
    virtual const String& getAppDataDirectory() const {
        ...
        if (mAppDataDir.empty()) {
            ...
            if (SUCCEEDED(
                ...
                mAppDataDir = Win32UnicodeString(path).toString();
            } else {
                const wchar_t* appdata = _wgetenv(L"APPDATA");
                if(appdata != NULL) {
                    mAppDataDir = Win32UnicodeString(appdata).toString();
                }
            }
        }
#elif defined (__APPLE__)
    ...
    */
    ...
    virtual String envGet(const char* varname) const {
        ...
        if (!value) {
            ...
            return Win32UnicodeString(value).toString();
        }
#else
    ...
    bool runSilentCommand(const StringVector& commandLine) {
        ...
        if (
                !CreateProcessW(comspec,                /* program path */
    ...
    if (findIndex >= 0) {
        do {
            ...
            if (wcscmp(name, L".") != 0 && wcscmp(name, L"..") != 0) {
                result.append(Win32UnicodeString(name).toString());
            }
