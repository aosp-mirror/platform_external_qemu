...
#error "Unsupported platform!"
    ...
    */
    ...
    bool runCommandPosix(const std::vector<std::string>& commandLine,
        ...
        ScopedCPtr<char*>  params {new char*[commandLine.size() + 1]};
...
int runViaPosixSpawn(
    ...
    unique_ptr<posix_spawnattr_t, decltype(attrDeleter)>
        scopedAttr(&attr, attrDeleter);
    ...
    // Global variable |environ| is defined only for executables (as opposed to
    // shared libraries). OTOH, _NSGetEnviron is always available on OSX.
    auto childEnviron = const_cast<char* const*>(*_NSGetEnviron());

    int pid;
    if (posix_spawnp(&pid, command, &fileActions, &attr, params.get(), childEnviron)) {
