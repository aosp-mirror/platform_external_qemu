    ...
    */
    ...
    virtual int runCommand(const StringVector& commandLine,
        ...
        if (execvp(commandLine[0].c_str(), params) == -1) {
            ...
            // emulator doesn't really like exit calls from a forked proces,
    ...
    int runSilentCommand(const StringVector& commandLine, bool wait) override {
        return runCommand(commandLine,
                 wait
                   ? RunOptions::WaitForCompletion | RunOptions::ReturnExitCode
                   : RunOptions::Default);
    }
