...
#error "Unsupported platform!"
    ...
    */
    ...
    bool runCommand(const std::vector<std::string>& commandLine,
        ...
        if ((options & RunOptions::DontWait) != RunOptions::None) {
            LOG(ERROR) << "RunOptions::DontWait no longer supported.";
            return false;
        }

        Process::Options procOptions;
        Process::RunOptions procRunOptions;
        ...
        return Process::runCommand(
            commandLine, procOptions, procRunOptions, outputFile,
            timeoutMs, outExitCode, outChildPid);
    }
