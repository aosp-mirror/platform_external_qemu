...
TEST(Win32UnicodeString, convertToUtf8) {
    ...
    for (size_t n = 0; n < kDataSize; ++n) {
        ...
        char out[256];
        int len1 = Win32UnicodeString::convertToUtf8(out, sizeof(out),
                                                     kData[n].utf16);
        EXPECT_EQ(strlen(kData[n].utf8) + 1, len1);
        EXPECT_STREQ(kData[n].utf8, out);

        int len2 = Win32UnicodeString::convertToUtf8(out, 1,
                                                     kData[n].utf16);
        EXPECT_EQ(strlen(kData[n].utf8) + 1, len2);

        int len3 = Win32UnicodeString::convertToUtf8(nullptr, 0,
                                                     kData[n].utf16);
        EXPECT_EQ(strlen(kData[n].utf8) + 1, len3);

        int len4 = Win32UnicodeString::convertToUtf8(nullptr, 0,
                                                     kData[n].utf16,
                                                     wcslen(kData[n].utf16));
        EXPECT_EQ(strlen(kData[n].utf8), len4);
    }
}
...
TEST(Win32UnicodeString, convertFromUtf8) {
    ...
    for (size_t n = 0; n < kDataSize; ++n) {
        wchar_t out[256];
        int len1 = Win32UnicodeString::convertFromUtf8(out, ARRAY_SIZE(out),
                                                       kData[n].utf8);
        EXPECT_EQ(wcslen(kData[n].utf16) + 1, len1);
        EXPECT_STREQ(kData[n].utf16, out);

        int len2 = Win32UnicodeString::convertFromUtf8(out, 1, kData[n].utf8);
        EXPECT_EQ(wcslen(kData[n].utf16) + 1, len2);

        int len3 = Win32UnicodeString::convertFromUtf8(nullptr, 0,
                                                       kData[n].utf8);
        EXPECT_EQ(wcslen(kData[n].utf16) + 1, len3);

        int len4 = Win32UnicodeString::convertFromUtf8(nullptr, 0,
                                                       kData[n].utf8,
                                                       strlen(kData[n].utf8));
        EXPECT_EQ(wcslen(kData[n].utf16), len4);
    }
}
