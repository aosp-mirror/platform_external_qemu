...
void Win32UnicodeString::reset(const char* str, size_t len) {
    ...
    const int utf16Len = convertFromUtf8(nullptr, 0, str, len);
...
String Win32UnicodeString::convertToUtf8(const wchar_t* str, int len) {
    String result;
    const int utf8Len = convertToUtf8(nullptr, 0, str, len);
    if (utf8Len > 0) {
...
int Win32UnicodeString::convertToUtf8(char* outStr, int outLen,
    ...
    // first, try to just convert it as is
    ...
    // Failure. If the outLen was 0, we're done
    if (outLen == 0) {
        return utf8Len;
    }

    // otherwise, query the needed output buffer length to return it
    const int bufLen = WideCharToMultiByte(CP_UTF8, 0, str, len, nullptr, 0,
                                           nullptr, nullptr);
    return bufLen;
}
...
int Win32UnicodeString::convertFromUtf8(wchar_t* outStr, int outLen,
    ...
    // otherwise, query the needed output buffer length to return it
    const int bufLen = MultiByteToWideChar(CP_UTF8, 0, str, len, nullptr, 0);
    return bufLen;
}
