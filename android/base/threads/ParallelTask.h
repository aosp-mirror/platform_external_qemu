...
// loop. Additionally, the |onJoined| function is called with a result of type
// |Result| returned from the launched thread.
//
...
//               mEarth(looper,
//                      &LifeUniverseAndEverythingInParallel::compute,
...
//
//   private:
//       ParallelTask<LifeUniverseAndEverythingInParallel, int> mEarth;
//   };
//
//   int main() {
...
public:
    ...
    using TaskFunction = std::function<void(ContextType*, ResultType*)>;
    using TaskDoneFunction = std::function<void(ContextType*, const ResultType&)>;
    ...
    bool start(ContextType* context) {
        mContext = context;
        return ParallelTaskBase::start();
    }
...
protected:
    void taskImpl() override final {
        mTaskFunction(mContext, &mResultBuffer);
    }

    void taskDoneImpl() override final {
        mTaskDoneFunction(mContext, mResultBuffer);
    }
...
class SelfDeletingParallelTask {
    public:
     SelfDeletingParallelTask(
         ...
         mParallelTask(looper,
                       &SelfDeletingParallelTask::taskFunction,
                       &SelfDeletingParallelTask::taskDoneFunction,
                       checkTimeoutMs) {}

     bool start() { return mParallelTask.start(this); };

     void taskFunction(ResultType* result) {
        mTaskFunction(result);
     }
    ...
    private:
     std::function<void(ResultType*)> mTaskFunction;
         std::function<void(const ResultType&)> mTaskDoneFunction;
         ParallelTask<SelfDeletingParallelTask<ResultType>,ResultType> mParallelTask;
