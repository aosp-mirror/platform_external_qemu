...
TEST_F(IniFileTest, readWrite) {
    ...
    ASSERT_EQ(intData.size() + int64Data.size() + doubleData.size() +
                      boolData.size() + diskSizeData.size(),
              mIni->size());

    // Mix-up the order a bit.
    for (const auto& keyval : boolData) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_EQ(keyval.second, mIni->getBool(keyval.first, 99));
    }
    for (const auto& keyval : diskSizeData) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_EQ(keyval.second, mIni->getDiskSize(keyval.first, 99));
    }
    for (const auto& keyval : int64Data) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_EQ(keyval.second, mIni->getInt64(keyval.first, 99));
    }
    for (const auto& keyval : intData) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_EQ(keyval.second, mIni->getInt(keyval.first, 99));
    }
    for (const auto& keyval : doubleData) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_EQ(keyval.second, mIni->getDouble(keyval.first, 99));
    }
}
...
TEST_F(IniFileTest, duplicateAndMisingKeys) {
    ...
    ASSERT_EQ(0, mIni->size());
    ASSERT_TRUE(mIni->read());
    ASSERT_NE(0, mIni->size());

    ASSERT_EQ(1, mIni->getInt("int", 99));
    ASSERT_EQ(1ULL, mIni->getInt64("int64", 99));
    ASSERT_EQ(1.1, mIni->getDouble("double", 99));
    ASSERT_EQ(true, mIni->getBool("bool", false));
    ASSERT_EQ(1ULL, mIni->getDiskSize("ds", 99ULL));

    ASSERT_EQ(-11, mIni->getInt("missing", -11));
    ASSERT_EQ(22ULL, mIni->getInt64("missing", 22ULL));
    ASSERT_EQ(3.3, mIni->getDouble("missing", 3.3));
    ASSERT_EQ(true, mIni->getBool("missing", true));
    ASSERT_EQ(44ULL, mIni->getInt("missing", 44ULL));
}
...
TEST_F(IniFileTest, readMalformedFile) {
    ...
    ASSERT_TRUE(mIni->read());
    ASSERT_EQ(validEntries.size(), mIni->size());
    for (const auto& keyval : validEntries) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_EQ(keyval.second, mIni->getInt(keyval.first, defaultInt));
    }
}
...
TEST_F(IniFileTest, boolFormat) {
    ...
    ASSERT_TRUE(mIni->read());
    ASSERT_EQ(lines.size(), mIni->size());
    for (const auto& keyval : validTrues) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_TRUE(mIni->getBool(keyval.first, false));
    }
    for (const auto& keyval : validFalses) {
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_FALSE(mIni->getBool(keyval.first, true));
    }
    for (const auto& keyval : invalidTrues) {
        // The keyval exists, it's just not a valid bool value.
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_FALSE(mIni->getBool(keyval.first, false));
    }
    for (const auto& keyval : invalidFalses) {
        // The keyval exists, it's just not a valid bool value.
        ASSERT_TRUE(mIni->hasKey(keyval.first));
        ASSERT_TRUE(mIni->getBool(keyval.first, true));
    }
}
...
TEST_F(IniFileTest, diskSizeFormat) {
    ...
    ASSERT_TRUE(mIni->read());
    ASSERT_EQ(lines.size(), mIni->size());
    ASSERT_EQ(30ULL, mIni->getDiskSize("ThirtyB", 99));
    ASSERT_EQ(1024ULL, mIni->getDiskSize("OneKilo", 99));
    ASSERT_EQ(1024 * 1024ULL, mIni->getDiskSize("OneMega", 99));
    ASSERT_EQ(1024 * 1024 * 1024ULL, mIni->getDiskSize("OneGiga", 99));
    ASSERT_EQ(5 * 1024ULL, mIni->getDiskSize("FiveKilo", 99));
    ASSERT_EQ(5 * 1024 * 1024ULL, mIni->getDiskSize("FiveMega", 99));
    ASSERT_EQ(5 * 1024 * 1024 * 1024ULL, mIni->getDiskSize("FiveGiga", 99));

    ASSERT_EQ(99L, mIni->getDiskSize("WrongUnit", 99L));
    ASSERT_EQ(99L, mIni->getDiskSize("FractionalNumber", 99L));
    ASSERT_EQ(99L, mIni->getDiskSize("FractionalKilo", 99L));
    ASSERT_EQ(99L, mIni->getDiskSize("smiley_really", 99L));
}
...
TEST_F(IniFileTest, discardEmpty) {
    ...
    ASSERT_EQ(0, mIni->size());
    ASSERT_TRUE(mIni->read());
    ASSERT_EQ(2, mIni->size());
    ASSERT_EQ("someValue", mIni->getString("nonEmpty", "defaultString"));
    ASSERT_EQ("", mIni->getString("empty", "defaultString"));

    ASSERT_TRUE(mIni->writeDiscardingEmpty());
    mIni.reset(new IniFile(mIniFilePath));
    ASSERT_EQ(0, mIni->size());
    ASSERT_TRUE(mIni->read());
    ASSERT_EQ(1, mIni->size());
    ASSERT_EQ("someValue", mIni->getString("nonEmpty", "defaultString"));
    ASSERT_EQ("defaultString", mIni->getString("empty", "defaultString"));
}
...
TEST_F(IniFileTest, iteratorTest) {
    ...
    for (const auto& keyval : cIni) {
        ASSERT_NE(std::end(keys), keys.find(keyval.first));
        ASSERT_EQ("dummyValue", keyval.second);
    }
}
