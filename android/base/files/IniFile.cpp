...
static inline CIterator eat(CIterator citer, CIterator cend, Pred pred) {
...
bool IniFile::writeCommon(bool discardEmpty) {
    ...
    for (const auto& key : mKeys) {
        ...
        for (;
             ...
             ++commentIter, ++lineno) {
            outFile << commentIter->second << std::endl;
        }
...
static inline bool isBoolTrue(const string& value) {
    const char* cstr = value.c_str();
    return strcasecmp("yes", cstr) == 0 || strcasecmp("true", cstr) == 0 ||
           strcasecmp("1", cstr) == 0;
}

static inline bool isBoolFalse(const string& value) {
    const char* cstr = value.c_str();
...
static inline IniFile::DiskSize parseDiskSize(const string& valueStr,
