...
private:
    ...
    // return true if we can still read data from socket
    bool readData();
};
...
void start_socket_drainer(Looper* looper) {
    if (!s_socket_drainer) {
        s_socket_drainer = new android::base::SocketDrainer(looper);
    }
}
...
void socket_drainer_add(int socket_fd) {
    ...
    if (s_socket_drainer) {
        ...
        // when socket drainer is not started, just do a hard shutdown
        android::base::Drainer drainer(0, socket_fd);
    }
}
