...
SRWLockSupportLoader::SRWLockSupportLoader() {
    sLibrary = LoadLibraryW(L"kernel32.dll");
    if (!sLibrary) { return; }
    sModuleLoaded = true;
...
ReadWriteLock::ReadWriteLock() {
    ...
    if (!sSRWSupported) {
        initialize_srw_lock((SRWLock*)&mLock.fallback_lock_obj);
    } else {
        initialize_srw_lock(&mLock.srw_lock);
    }
}

ReadWriteLock::~ReadWriteLock() {
    if (!sSRWSupported) {
        DeleteCriticalSection(&mLock.fallback_lock_obj.write_lock);
        DeleteCriticalSection(&mLock.fallback_lock_obj.read_lock);
    }
}
