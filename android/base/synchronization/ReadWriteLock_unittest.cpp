...
TEST(ReadWriteLock, SyncWrite) {
    ...
    TestThread* write_threads[kNumWriteThreads];
    RWThreadParams p;

    // Create and launch all threads.
    for (size_t n = 0; n < kNumWriteThreads; ++n) {
        // stack size needs to be adjusted to fit
        write_threads[n] = new TestThread(writeThreadFunction, &p);
    }

    // Wait until their completion.
    for (size_t n = 0; n < kNumWriteThreads; ++n) {
        write_threads[n]->join();
        delete write_threads[n];
    }
...
TEST(ReadWriteLock, SyncRead) {
    const size_t kNumThreads = 1000;
    TestThread* write_threads[kNumThreads];
    TestThread* read_threads[kNumThreads];
    ...
    for (size_t n = 0; n < kNumThreads; ++n) {
        // stack size needs to be adjusted to fit
        write_threads[n] = new TestThread(writeThreadFunction, &p);
        read_threads[n] = new TestThread(readThreadFunction, &p);
    }
...
TEST(ReadWriteLock, SyncRandomRW) {
    ...
    TestThread* threads[kNumThreadsPerTrial];
