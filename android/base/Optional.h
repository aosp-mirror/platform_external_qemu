...
class Optional : private details::OptionalBase<sizeof(T),
    ...
    // make sure all optionals are buddies
...
public:
    ...
    Optional(const Optional& other) : base_type(other.mConstructed) {
        if (this->mConstructed) {
            new (&get()) T(other.get());
        }
    }
    Optional(Optional&& other) : base_type(other.mConstructed) {
        if (this->mConstructed) {
    ...
    Optional(const Optional<U>& other) : base_type(other.mConstructed) {
        if (this->mConstructed) {
            new (&get()) T(other.get());
        }
    }
    ...
    template <class U,
              class = enable_if_c<
                  !std::is_same<T, U>::value
                  && std::is_constructible<T, U>::value>>
    Optional(Optional<U>&& other) : base_type(other.mConstructed) {
        if (this->mConstructed) {
            new (&get()) T(std::move(other.get()));
        }
    }
    ...
    constexpr T valueOr(U&& defaultValue) const {
        return this->mConstructed ? get() : std::move(defaultValue);
    }
