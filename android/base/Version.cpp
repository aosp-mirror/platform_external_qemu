...
// parse the current part of version string and return if there's anything else left
namespace {
...
static VersionParseResult parseVersionPart(const char* start, char*& end,
                                           unsigned int& value) {
...
Version::Version(const char *ver)
    ...
    if (res == Error) {
        *this = Invalid;
        ...
        dwarning("Version: unexpected version string '%s'", ver);
    }
}
