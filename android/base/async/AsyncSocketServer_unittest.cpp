...
struct TestData {
    ...
    bool startServer(AsyncSocketServer::LoopbackMode mode) {
        auto callback = std::bind(&TestData::onConnection, this,
                                std::placeholders::_1);
        mServer.reset(AsyncSocketServer::createTcpLoopbackServer(
                            mPort, callback, mode, mLooper));
        if (!mServer.get()) {
            return false;
        }
        mPort = mServer->port();
    ...
    bool onConnection(int socket) {
        //LOG(INFO) << "Connected through socket " << socket;
        this->mCounter++;
        char c = 0;
    ...
    static intptr_t connectAndWriteTo4(int port) {
        //LOG(INFO) << "Connecting and writing to 127.0.0.1:" << port;
...
TEST(AsyncSocketServer, createTcpLoopbackServerIPv4) {
    ...
    EXPECT_TRUE(data.startServer(AsyncSocketServer::kIPv4));
...
TEST(AsyncSocketServer, createTcpLoopbackServerIPv6) {
    ...
    TestData data(looper, kPort);
    EXPECT_TRUE(data.startServer(AsyncSocketServer::kIPv6));
    EXPECT_EQ(0, data.mCounter);
...
TEST(AsyncSocketServer, createTcpLoopbackServerIPv4AndIPv6) {
    ...
    TestData data(looper, kPort);
    EXPECT_TRUE(data.startServer(AsyncSocketServer::kIPv4AndIPv6));
    EXPECT_EQ(0, data.mCounter);
