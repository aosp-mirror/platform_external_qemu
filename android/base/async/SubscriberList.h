...
private:
    ...
    typedef struct {
        ...
        typename SubscriberInfoList::iterator infoIterator;
    } MetaData;
    using MetaDataList = std::list<MetaData>;
    using MetaDataListIterator = typename MetaDataList::iterator;

public:
    using iterator = typename SubscriberInfoList::iterator;
    ...
    SubscriptionToken insert(SubscriberInfo&& info) {
        mSubscriberInfos.insert_back(std::forward<SubscriberInfo>(info));
        return insertInternal(std::prev(mSubscriberInfos.end()));
    }
    ...
    iterator begin() { return mSubscriberInfos.begin(); }
    iterator end() { return mSubscriberInfos.end(); }
    size_t size() { return mSubscriberInfos.size(); }
...
private:
    ...
    SubscriptionToken insertInternal(iterator infoIterator) {
