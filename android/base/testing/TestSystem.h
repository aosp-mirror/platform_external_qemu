...
public:
    ...
    typedef bool(ShellCommand)(void* opaque, const StringVector& commandLine);
    ...
    bool runCommand(const StringVector& commandLine,
                    ...
                    System::Pid* outChildPid = nullptr) override {
        if (!commandLine.size()) {
            return false;
        }
        ...
        ;
        if (mShellFunc) {
            result = (*mShellFunc)(mShellOpaque, commandLine);
        }

        if (outExitCode) {
            *outExitCode = result ? 0 : 1;
        }
        return result;
    }
