...
public:
    virtual int resolveName(StringView dns_server_name,
        ...
        std::string hostname(dns_server_name.c_str(), dns_server_name.size());
        struct addrinfo* res;
        ...
        for (struct addrinfo* r = res; r != NULL; r = r->ai_next) {
            ...
            switch (r->ai_family) {
                case AF_INET: {
                    auto sin = reinterpret_cast<struct sockaddr_in *>(r->ai_addr);
                    out->push_back(IpAddress(ntohl(sin->sin_addr.s_addr)));
                    break;
                }

                case AF_INET6: {
                    auto sin6 = reinterpret_cast<struct sockaddr_in6 *>(r->ai_addr);
    ...
    // TODO(digit): Find a Win32 API to return the list of IPv6
    // DNS servers being used by the system.
    size_t count = 0;
    for (IP_ADDR_STRING* ipAddr = &fixedInfo->DnsServerList;
            ipAddr; ipAddr = ipAddr->Next) {
    ...
    // in some situations, the symlink can be destroyed and the system will not
...
Dns::Resolver* s_resolver = nullptr;
...
Dns::Resolver* Dns::Resolver::get() {
    if (!s_resolver) {
        s_resolver = new SystemResolver();
    }
    return s_resolver;
}

// static
Dns::Resolver* Dns::Resolver::setForTesting(
        Dns::Resolver* resolver) {
    Dns::Resolver* result = s_resolver;
    s_resolver = resolver;
    return result;
}
