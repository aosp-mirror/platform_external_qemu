...
// Note: doesn't work for multimap/multiset, as there's no single value
...
bool exists(const T& c, const typename T::key_type& key) {
...
template <class T,
          class = enable_if_c<is_any_map<T>::value || is_any_set<T>::value>>
bool existsAnyOf(const T& c, std::initializer_list<typename T::key_type> keys) {
    for (const auto& key : keys) {
        if (exists(c, key)) {
            return true;
        }
