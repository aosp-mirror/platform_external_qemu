...
// Studio preferences file. StudioXml structs are define statically
...
static std::string getLegacyInstallationId() {
    {
    ...
    // Couldn't find uuid in the android specific location. Try even more
    // legacy uuid locations.
    ...
    if (retval.empty()) {
        ...
        retval = kAndroidStudioUuidHexPattern;
    }
    return retval;
}
...
std::string extractInstallationId() {
    ...
    if (optionalId) {
        ...
        return getLegacyInstallationId();
    }
}
