...
//
static const char * const  ANDROID_STUDIO_UUID_DIR = "JetBrains";
static const char * const  ANDROID_STUDIO_UUID = "PermanentUserID";
...
#ifdef _WIN32
// static
String android::StudioHelper::pathToStudioUUIDWindows() {
    ...
    if(!appDataPath.empty()) {
        ...
        vpath.append(String(ANDROID_STUDIO_UUID_DIR));
        vpath.append(String(ANDROID_STUDIO_UUID));
...
static String parseStudioXML(const StudioXml* const match) {
    ...
    String appDataPath = sys->getAppDataDirectory();
#else
    String appDataPath = sys->getHomeDirectory();
#endif // __APPLE__
    if(appDataPath.empty())
        return NULL;

    String studio = android::StudioHelper::latestAndroidStudioDir(appDataPath);
    if (studio.empty())
        return NULL;
    String xml_path = android::StudioHelper::pathToStudioXML(studio, String(match->filename));
    if(xml_path.empty()) {
    ...
    String retval = eval_studio_config_xml(doc, root, match);
...
static String android_studio_get_Windows_uuid()
{
    ...
    char *val = strdup(ANDROID_STUDIO_UUID_HEXPATTERN);
    String uuid_file_path = android::StudioHelper::pathToStudioUUIDWindows();

    // Read UUID from file
    std::ifstream uuid_file(uuid_file_path.c_str());
    uuid_file.getline(val, strlen(val)+1);
...
char *android_studio_get_installation_id() {
    ...
    if(retval.empty()) {
        ...
        retval = String(ANDROID_STUDIO_UUID_HEXPATTERN);
    }
