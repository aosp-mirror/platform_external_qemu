...
// static
const Version StudioHelper::extractAndroidStudioVersion(const char * const dirName) {
    if(dirName == NULL)
...
static String parseStudioXML(const StudioXml* const match) {
    ...
    String studio = StudioHelper::latestAndroidStudioDir(appDataPath);
    if (studio.empty())
        return NULL;
    String xml_path = StudioHelper::pathToStudioXML(studio, String(match->filename));
    if(xml_path.empty()) {
