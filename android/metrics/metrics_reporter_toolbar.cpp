...
template <class Val>
static void urlAddKeyVal(String* dst, const char* format,
                         const char* key, const Val& val) {
    url_limit_append(
            dst,
            Uri::FormatEncodeArguments(format, key, val));
}

static void url_addstrval(String* dst,
                          const char* key, const char* val) {
    urlAddKeyVal(dst, "&%s=%s", key, val);
}

static void url_addintval(String* dst,
                          const char* key, int val) {
    urlAddKeyVal(dst, "&%s=%d", key, val);
}

static void url_addi64val(String* dst,
