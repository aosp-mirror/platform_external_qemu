...
void url_addstrval(String* dst, const char* key, const char* val) {
    url_limit_append(
            dst,
            Uri::FormatEncodeArguments(
                "&%s=%s", key, val));
}

void url_addintval(String* dst, const char* key, int val) {
    url_limit_append(
            dst,
            Uri::FormatEncodeArguments(
                "&%s=%d", key, val));
...
void url_addi64val(String* dst, const char* key, uint64_t val) {
    url_limit_append(
            dst,
            Uri::FormatEncodeArguments(
                "&%s=%" PRId64, key, val));
...
int formatToolbarGetUrl(char** ptr,
    ...
    url_limit_append(
            &fullUrl,
            Uri::FormatEncodeArguments("?as=%s", product_name));
    ...
    if (metrics->guest_gpu_enabled > 0) {
        ...
        url_addstrval(&fullUrl, guest_gl_renderer_key,
                      metrics->guest_gl_renderer);
        url_addstrval(&fullUrl, guest_gl_version_key,
