...
#define URL_GPU_FORMAT(dst, gpu)                                              \
    do {                                                                      \
        URL_APPEND(dst, "&%s=%s", #gpu "_make", metrics->gpu##_make);         \
        URL_APPEND(dst, "&%s=%s", #gpu "_model", metrics->gpu##_model);       \
        URL_APPEND(                                                           \
                dst, "&%s=%s", #gpu "_device_id", metrics->gpu##_device_id);  \
        URL_APPEND(dst,                                                       \
                   "&%s=%s",                                                  \
                   #gpu "_revision_id",                                       \
                   metrics->gpu##_revision_id);                               \
        URL_APPEND(dst, "&%s=%s", #gpu "_version", metrics->gpu##_version);   \
        URL_APPEND(dst, "&%s=%s", #gpu "_renderer", metrics->gpu##_renderer); \
    } while (0);
...
int formatToolbarGetUrl(char** ptr,
    ...
    URL_APPEND(&fullUrl,
               "&%s=%d",
               exit_started_key,
               metrics->exit_started ? 1 : 0);

    if (metrics->guest_gpu_enabled > 0) {
        URL_APPEND(&fullUrl,
                   "&%s=%s",
                   guest_gl_vendor_key,
                   metrics->guest_gl_vendor);
        URL_APPEND(&fullUrl,
                   "&%s=%s",
                   guest_gl_renderer_key,
                   metrics->guest_gl_renderer);
        URL_APPEND(&fullUrl,
                   "&%s=%s",
                   guest_gl_version_key,
