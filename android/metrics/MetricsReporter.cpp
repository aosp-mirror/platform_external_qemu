...
public:
    ReporterHolder() : mPtr(new NullMetricsReporter()) {}

    void reset(MetricsReporter::Ptr newPtr) {
        if (newPtr) {
            mPtr = std::move(newPtr);
        } else {
            mPtr.reset(new NullMetricsReporter());
        }
    }

    const MetricsReporter::Ptr& reporter() const { return mPtr; }

private:
    MetricsReporter::Ptr mPtr;
};
...
void MetricsReporter::start(const std::string& sessionId,
    ...
    if (!optIn) {
        sInstance->reset(Ptr(new NullMetricsReporter()));
    } else {
