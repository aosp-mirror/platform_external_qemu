...
typedef struct {
  DirScanner* scanner; // next
  char temp[PATH_MAX]; // partial path to studio preferences
  char name[PATH_MAX]; // final path to studio preferences
  int cmp; // numeric equivalent to studio version for sorting reasons
} StudioScanner;
...
//      .AndroidStudio1.3        --> 132 # releases get higher priority
 ...
 *     $HOME/Library/Preferences/AndroidStudio<Preview>X.Y in MacOSX
 *     $HOME/.AndroidStudio<Preview>X.Y in Windows
...
/* Scan through all possible .AndroidStudio dirs and updates |path|
 * return-value to the latest one. If successful, the full path to latest
 * android studio preferences directory will be saved to |path| and 0 will
 * be returned. Otherwise, a value < 0 is returned and |path| might be
 * partial.
 * Assumes PATH_MAX path strings
 */
...
/* This functions appends (value-return) to |path| --- which is the
 * full path to an .AndroidStudio directory --- the subpath to |filename|,
 * where filename one of the xml files storing Studio installation data,
 * like user opt-ins and installation id.
 * Assumes PATH_MAX path strings.
 ...
 * AndroidStudio/config/options in MacOSX
 */
int path_to_latest_studio_config_xml(char* path, const char* filename) {
	...
	p = bufprint(p, end, PATH_SEP ANDROID_STUDIO_DIR_INFIX);
#endif
