...
bool android_metrics_start(const char* emulatorVersion,
    ...
    PeriodicReporter::get().addTask(
            ...
            [](android_studio::AndroidStudioEvent* event) {
                ...
                event->mutable_emulator_details()->set_system_time(
                        times.systemMs);
                event->mutable_emulator_details()->set_user_time(times.userMs);
                event->mutable_emulator_details()->set_wall_time(
                        times.wallClockMs);
                return true;
            });
...
void android_metrics_report_common_info(bool openglAlive) {
    MetricsReporter::get().report([=](
        ...
        // TODO: add renderer
        // set_renderer(emuglConfig_get_renderer(android_hw->hw_gpu_mode));
