...
iniFile_getDuration( IniFile* f, const char* key, Duration defaultValue )
{
    return (Duration) iniFile_getInt64(f, key, (int64_t) defaultValue);
}

static inline void
iniFile_setDuration( IniFile* f, const char* key, Duration value )
{
    iniFile_setInt64(f, key, (int64_t) value);
}
...
void
androidMetrics_fini( AndroidMetrics *androidMetrics )
{
#undef METRICS_INT
#undef METRICS_STRING
#undef METRICS_DURATION
...
#define METRICS_DURATION(n, s, d)   iniFile_setDuration(ini, s, am->n);
...
#define METRICS_DURATION(n, s, d)   am->n = iniFile_getDuration(ini, s, d);
