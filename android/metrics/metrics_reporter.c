...
ABool
androidMetrics_moduleInit(const char* avdHome)
{
...
ABool androidMetrics_uploadMetrics(const AndroidMetrics* metrics) {
    ...
    mwarning("[xkcd] Posting: %s", fields);
    escaped_fields = curl_easy_escape(curl, fields, 0);
    if (escaped_fields == NULL) {
        mwarning("Could not url-escape the post data.");
        android_free(fields);
        curl_easy_cleanup(curl);
        return 0;
    }
    ...
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, escaped_fields);
