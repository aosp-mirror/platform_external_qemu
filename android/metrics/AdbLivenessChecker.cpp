...
void AdbLivenessChecker::reportCheckResult(const CheckResult& result) {
    // |mIsOnline| starts off set to false. We set it to true if we successfully talk to the
    // emulator at least once. This allows us to distinguish the cases where adb is missing /
    // emulator doesn't boot at all from cases where emulator boots and then disappears from adb.
    if (!mIsOnline) {
        switch (result) {
