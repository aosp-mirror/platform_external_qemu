...
TEST(MetricsReporterGa, defaultMetrics) {
    ...
    ASSERT_LT(0, formatGAPostData(&post_data, &metrics));
    ASSERT_EQ(0, strcmp("v=1&tid=UA-19996407-3&an=Android Studio&av=unknown&"
...
TEST(MetricsReporterGa, cleanRun) {
    ...
    ASSERT_LT(0, formatGAPostData(&post_data, &metrics));
    ASSERT_EQ(0, strcmp("v=1&tid=UA-19996407-3&an=Android Studio&av=standalone&"
...
TEST(MetricsReporterGa, dirtyRun) {
    ...
    ASSERT_LT(0, formatGAPostData(&post_data, &metrics));
    ASSERT_EQ(0,
              strcmp("v=1&tid=UA-19996407-3&an=Android Studio&av=standalone&"
                     "cid=default-client&cd6=x86_64&t=event&ec=emulator&"
