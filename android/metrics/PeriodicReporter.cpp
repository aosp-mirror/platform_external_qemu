...
void PeriodicReporter::stop() {
    ...
    // this way we expire all weak pointers in TashGuards and they won't
                                 ...
                                 Looper* looper)
{
    ...
    for (auto& periodAndData : mPeriodDataByPeriod) {
        ...
        createPerPeriodTimerNoLock(&periodAndData.second,
                                   periodAndData.first);
    }
}
...
void PeriodicReporter::createPerPeriodTimerNoLock(PerPeriodData* const data,
    ...
    data->task.emplace(mLooper,
                       [this, data]() {
                           mMetricsReporter->reportConditional([this, data](
