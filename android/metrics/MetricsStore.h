...
//
//   std::uniqe_ptr<IMetricsStore> mstore = IMetricsStore::createFileBackedStore(
//                  "path/to/my/metrics/file",
//                  looper_getForCurrentThread(),
//                  "stupid.metrics.");
...
public:
    ...
    // Data accessors.
    // Default values are returned if the key doesn't exist. Use |hasKey| if you
    // want to exlicitly check for existence of key.
    // Default: std::string("")
    virtual const std::string& getString(const std::string& key) const = 0;
    // Default: 0
    virtual const int getInt(const std::string& key) const = 0;
    // Default: 0
    virtual const int64_t getInt64(const std::string& key) const = 0;
    // Default: 0.0
    virtual const double getDouble(const std::string& key) const = 0;
    ...
    // If the key doesn't exist in the store yet, update* functions behave as if
    // the default values existed before the update.
    virtual void updateInt(const std::string& key, int delta) = 0;
    virtual void updateInt64(const std::string& key, int64_t delta) = 0;
    virtual void updateDouble(const std::string& key, double delta) = 0;
    ...
    // Static methods to obtain a file backed metrics store.
    // Use this function to obtain instances of |IMetricsStore|.
    // See also |FakeMetricsStore| for a testing implementation of this
    // interface.
    // |persistentStorePath| is the path to a backing file on disk. This file
    // need not exist, but the path must be writable.
    // Both |set*| and |get*| methods prefix
    // all key searches with the |keyPrefix|. Use empty string to not use
    // prefixes.
    static IMetricsStore* createFileBackedStore(
            const std::string& persistentStorePath,
            android::base::Looper* looper,
            const std::string& keyPrefix);

protected:
    // Use |IMetricsStore::createFileBackedStore| for creating instances.
    // For testing, create |FakeMetricsStore| directly.
