...
void ConfirmDialog::setSwGpu() {
         ...
         mReportingDir                )
    {
        ...
        std::string avdInfoPath =
                PathUtils::join(mReportingDir, "avd_info.txt").c_str();

        CIniFile* iniF = iniFile_newFromFile(avdInfoPath.c_str());

        // Set hw.gpu.mode=guest
        iniFile_setString(iniF, "hw.gpu.mode", "guest");

        // Get the path to hardware-qemu.ini
        StringView diskPartDir = iniFile_getString(
                iniF, "disk.dataPartition.path", nullptr);
        if ( !diskPartDir.empty() ) {
            ...
            if (isOK) {
                std::string hwQemuPath = PathUtils::join(
                        outputDir, "hardware-qemu.ini").c_str();
                // Write the modified configuration to the hardware-qemu.ini
                iniFile_saveToFileClean(iniF, hwQemuPath.c_str());
            }
        }
    }
}
