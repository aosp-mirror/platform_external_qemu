...
namespace {
    ...
    bool dumpCallback(const google_breakpad::MinidumpDescriptor &descriptor,
            ...
            bool succeeded)
    {
        derror("Crash detected. Minidump saved to: %s\n", descriptor.path());
        BreakpadUtils::dumpIniFile(descriptor.path());
#ifdef CRASHUPLOAD
        //Taken from breakpad
        ...
        if (stack) {
            ...
            const pid_t child = sys_clone(processCrash, stack, CLONE_VFORK | CLONE_FILES | CLONE_FS | CLONE_UNTRACED | CLONE_VM, (void *)descriptor.path(), NULL, NULL, NULL);
            if (child == -1) {
                derror("Could not spawn clone for crash upload\n");
            }
            ...
            bool succeeded)
    {
        ...
        path.append(".dmp");
        derror("Crash detected. Minidump saved to: %s\n", path.c_str());
        BreakpadUtils::dumpIniFile(path.c_str());
#ifdef CRASHUPLOAD
        const StringVector cmdvector = BreakpadUtils::get()->getCrashCmdLine(path);
