...
public:
    ...
    bool waitServicePipeReady(const std::string& pipename,
        ...
        const int kWaitNamedPipeTimeout = 1;
        bool serviceReady = false;
        static_assert(kWaitIntervalMS - kWaitNamedPipeTimeout >= 0,
            "kWaitIntervalMS must be greater or equal to kWaitNamedPipeTimeout");
        for (int i = 0; i < timeout_ms / kWaitIntervalMS; i++) {
            ...
            // WaitNamedPipe does not have a timeout of 0, so set to minimum 1ms
            if (WaitNamedPipe(pipename.c_str(),kWaitNamedPipeTimeout) != 0) {
                serviceReady = true;
            ...
            if (ERROR_SEM_TIMEOUT == GetLastError()) {
                ...
                // A timeout means that we waited 1 ms already
                if (kWaitIntervalMS >= kWaitNamedPipeTimeout) {
