...
UserSuggestions::UserSuggestions(google_breakpad::ProcessState* process_state) {
    ...
    suggestions.resize(CRASHSERVICE_NUM_SUGGESTIONS, false);

    gfx_driver_list.push_back(std::string("qemu"));
    ...
    for (std::size_t i = 0; i < gfx_driver_list.size(); i++) {
        flagged_gfx_drivers[gfx_driver_list[i]] = false;
    }
...
void UserSuggestions::generate() {
    ...
    for (std::size_t i = 0; i < gfx_driver_list.size(); i++) {
        ...
        if (pos != std::string::npos) {
            flagged_gfx_drivers[gfx_driver_list[i]] = true;
            suggestions_exist = true;
            suggestions[CRASHSERVICE_SUGGEST_UPDATE_GFX_DRIVERS] = true;
        }
    }
}
