...
bool HostCrashService::getHWInfo() {
    ...
    String utf8Path = PathUtils::join(dataDirectory, "hw_info.txt");
...
bool HostCrashService::getMemInfo() {
    ...
    String path = PathUtils::join(data_directory, "mem_info.txt");
    ...
    if (!GlobalMemoryStatusEx(&mem)) {
        E("Failed to get global memory status: %lu", GetLastError());
        fout << "ERROR: Failed to get global memory status: " << GetLastError()
             << "\n";
        return false;
    }
