...
static bool uploadCrash(const std::string& crashdumpFilePath, std::string& reportID) {
...
static void cleanupCrash(const std::string& crashdumpFilePath) {
    remove(crashdumpFilePath.c_str());
}

static std::string getCrashDetails(const char* crashPath) {
    std::string details;
...
#if CONFIG_QT
static bool displayConfirmDialog(const char* crashPath) {
...
void OnClientDumpRequest(void* context,
    ...
    static_cast<DumpRequestContext*>(context)->file_path = *file_path;
}
...
int main(int argc, char** argv) {
    ...
    if (dump_file) {
        ...
        ::android::base::Win32UnicodeString crashdir_unicode(
                ::android::crashreport::CrashSystem::get()->getCrashDirectory().c_str());
