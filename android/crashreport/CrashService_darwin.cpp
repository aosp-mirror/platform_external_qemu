...
bool HostCrashService::getHWInfo() {
    String file_path = PathUtils::join(getDataDirectory(), "hw_info");
...
bool HostCrashService::getMemInfo() {
    ...
    String file_path = PathUtils::join(data_directory, "mem_info");
    ...
    if (!fout.good()) {
        E("Unable to open mem_info file for writing");
        return false;
    }
