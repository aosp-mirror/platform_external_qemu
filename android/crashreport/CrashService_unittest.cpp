...
static std::string getTestCrasher(void) {
    std::string path =
            ::android::base::System::get()->getLauncherDirectory().c_str();
    path += System::kDirSeparator;
    path += "emulator";
    if (::android::base::System::get()->getProgramBitness() == 64) {
        path += "64";
    }
    path += "_test_crasher";
#ifdef _WIN32
...
static StringVector getTestCrasherCmdLine(std::string pipe) {
    ...
    cmdline.append(::android::base::StringView(getTestCrasher().c_str()));
...
TEST(CrashService, uploadCrash) {}

TEST(CrashService, getCrashDetails) {}
...
TEST(CrashService, startAttachWaitNoCrash) {
    ...
    crash->startCrashServer(crashpipe.mServer);
    StringVector cmdline = getTestCrasherCmdLine(crashpipe.mClient);
    cmdline.append(::android::base::StringView("-nocrash"));
    int pid = CrashSystem::spawnService(cmdline);
...
TEST(CrashService, startWaitNoAttach) {
    ...
    crash->startCrashServer(crashpipe.mServer);
    StringVector cmdline = getTestCrasherCmdLine(crashpipe.mClient);
    cmdline.append(::android::base::StringView("-noattach"));
    int pid = CrashSystem::spawnService(cmdline);
...
TEST(CrashService, startAttachWaitTimeout) {
    ...
    crash->startCrashServer(crashpipe.mServer);
    StringVector cmdline = getTestCrasherCmdLine(crashpipe.mClient);
    cmdline.append(::android::base::StringView("-delay_ms"));
    cmdline.append(::android::base::StringView("100"));
    int pid = CrashSystem::spawnService(cmdline);
    int64_t waitduration = crash->waitForDumpFile(pid, 1);
    EXPECT_EQ(waitduration, -1);
