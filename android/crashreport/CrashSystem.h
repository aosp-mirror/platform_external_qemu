...
public:
    ...
    struct CrashPipe {
        ...
        CrashPipe(std::string name) : mServer(name), mClient(name), mValid(true) {}
        CrashPipe(int server_fd, int client_fd) :
          mServer(std::to_string(server_fd)), mClient(std::to_string(client_fd)), mValid(true) {}
        CrashPipe(int fd) :
          mServer(std::to_string(fd)), mClient(std::to_string(fd)), mValid(true) {}
        bool isValid(void) { return mValid; }
        std::string mServer;
        std::string mClient;
     private:
        bool mValid;
    ...
    virtual const std::string& getCaBundlePath(void);

    // Returns predefined path to crash-service
    virtual const std::string& getCrashServicePath(void);

    // Returns a string representation of the process used for passing between
    // processes
    virtual const std::string& getProcessId(void);

    // Returns pipe struct for crash reporting
    virtual const CrashPipe& getCrashPipe(void);
    ...
    // are valid.
    virtual bool validatePaths(void);
