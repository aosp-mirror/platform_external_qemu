...
public:
    ...
    virtual const std::string& getCrashURL(void) override {
        if (mCrashURL.empty()) {
...
// All - Returns pid of spawned process on success, 0 otherwise
// No - Win32 - Inheritable handles so we could get access to the parent process
//        handle
...
const std::string& CrashSystem::getCaBundlePath(void) {
...
const std::string& CrashSystem::getCrashServicePath(void) {
...
const std::string& CrashSystem::getProcessId(void) {
    if (mProcessId.empty()) {
...
if (!rSuccess) {
    E("Couldn't duplicate proc handle\n");
    return "";
}
mProcessId = std::to_string(reinterpret_cast<std::size_t>(procHandle));
*/
#endif
    }
    return mProcessId;
...
const CrashSystem::CrashPipe& CrashSystem::getCrashPipe(void) {
    if (!mCrashPipe.isValid()) {
        ...
        pipename.append(".");
        pipename.append(std::to_string(getpid()));
...
bool CrashSystem::validatePaths(void) {
    bool valid = true;
