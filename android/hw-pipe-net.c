...
netPipe_free( NetPipe*  pipe )
{
    ...
    // Don't, someone might use during/after
...
#else
    pipe->shared_mem->lock = NULL;
    pipe->shared_mem->cv_ready = NULL;
    pipe->shared_mem->cv_valid = NULL;
    aemu_mutex_init(&pipe->shared_mem->lock);
    aemu_cond_init(&pipe->shared_mem->cv_ready);
    aemu_cond_init(&pipe->shared_mem->cv_valid);
#endif
...
static int shared_mem_write(NetPipe_SharedMemState* sm, 
    ...
    if (len > 0) { aemu_cond_signal(sm->cv_valid); }
    // fprintf(stderr, "%s: done. len=%d\n", __FUNCTION__, len);
