...
static int do_debug(ControlClient client, char* args) {
    if (!args) {
        control_write(client, "KO: argument missing, try 'debug <flags>'\r\n");
        return -1;
    }
    if (!android_parse_debug_option(args)) {
        control_write(client, "KO: bad flags, see command line help for "
...
static const CommandDefRec   main_commands[] =
{
    ...
    { "debug", "control the emulator debug output flags",
      NULL, NULL, do_debug },
