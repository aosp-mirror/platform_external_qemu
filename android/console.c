...
do_nfc_message_send(ControlClient client, char* args)
{
    if (args) {
		...
		if (args_len % 2 == 1)
		{
			control_write(client, "KO: invalid message\r\n");
		}
		...
		for (i = 0, j = 0; i < args_len; i += 2, j++)
		{
			...
			{
				control_write(client, "KO: message copy failed\r\n");
				return -1;
			}
		}
		
		client->global->nfc_agent->sendNFCMessage(message, message_length);
        return 0;
    }
...
{
    { "send", "send <message> to NFC hal",
      "'send <message>' send <message> to nfc hal.\r\n",
...
static const CommandDefRec   main_commands[] =
{
	...
	{ "nfc", "manage emulator nfc",
      "allows to send message to emulator nfc\r\n", NULL,
      NULL, nfc_commands},
