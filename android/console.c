...
static void control_global_accept(void* opaque,
    ...
    if (client) {
        char emulator_console_auth_token_path[1024];
        android_console_auth_get_token_path(
                emulator_console_auth_token_path,
                sizeof(emulator_console_auth_token_path));
...
#define ANDROID_CONSOLE_COPY_AGENT(type, name) \
    ...
    if (console_auth_status == CONSOLE_AUTH_STATUS_ERROR) {
        // Banners don't get sent reliably in QEMU2, so we output the error
        // message
        // on stderr there.  Do the same here.
        char emulator_console_auth_token_path[1024];
        android_console_auth_get_token_path(
                emulator_console_auth_token_path,
                sizeof(emulator_console_auth_token_path));
        fprintf(stderr,
                "ERROR: Unable to access '%s' emulator console will not work\n",
                emulator_console_auth_token_path);
    }
...
const char* android_console_auth_banner_get() {
    ...
    if (!s_ready) {
        strncpy(s_android_monitor_auth_banner,
                "Android Console: Authentication required\n"
        ...
        android_console_auth_get_token_path(
                emulator_console_auth_token_path,
                sizeof(emulator_console_auth_token_path));
        strncat(s_android_monitor_auth_banner, emulator_console_auth_token_path,
                sizeof(s_android_monitor_auth_banner));
        strncat(s_android_monitor_auth_banner, "'\nOK",
                sizeof(s_android_monitor_auth_banner));
        s_ready = true;
    }
