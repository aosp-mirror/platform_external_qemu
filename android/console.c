...
{
    if (!(vmopers(client)->isVMRunning())) {
        control_write( client, "KO: virtual device already stopped\r\n" );
        return -1;
    }
    return vmopers(client)->stopVMInterrupt() ? 0 : -1;
}
...
do_avd_start( ControlClient  client, char*  args )
{
    ...
    return vmopers(client)->startVMInterrupt() ? 0 : -1;
}
...
do_avd_status( ControlClient  client, char*  args )
{
    control_write(client, "virtual device is %s\r\n",
                  vmopers(client)->isVMRunning() ? "running" : "stopped");
    return 0;
}
...
int control_console_start(int port, QAndroidVMOperations* vm_operations) {
