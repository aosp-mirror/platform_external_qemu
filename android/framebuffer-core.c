...
typedef struct FBUpdateNotify {
    ...
    /* I/O associated with this message. */
    LoopIo                  io;
...
{
    FBUpdateNotify* fb_update_desc = opaque;
    ...
    while (fb_update_head != NULL) {
        ...
        const AsyncStatus status =
            asyncWriter_write(&fb_update_writer, &current_update->io);
        switch (status) {
            case ASYNC_COMPLETE:
            ...
            case ASYNC_ERROR:
                // Done with the current update. Move on to the next one.
                loopIo_dontWantWrite(&current_update->io);
                break;
        ...
        if (fb_update_head != NULL) {
            ...
            asyncWriter_init(&fb_update_writer, &fb_update_head->message,
                             fb_update_head->message_size, &fb_update_head->io);
        }
    }
}
                      ...
                      int x, int y, int w, int h)
{
    ...
    loopIo_init(&ret->io, core_fb->looper, core_fb->sock, corefb_io_func, ret);
    ret->core_fb = core_fb;
              ...
              struct QFrameBuffer* fb, int x, int y, int w, int h)
{
    ...
    asyncWriter_init(&fb_update_writer, &fb_update_head->message,
                     fb_update_head->message_size, &fb_update_head->io);
    status = asyncWriter_write(&fb_update_writer, &fb_update_head->io);
    switch (status) {
        case ASYNC_COMPLETE:
