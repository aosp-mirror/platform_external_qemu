...
corefb_io_func(void* opaque, int fd, unsigned events)
{
    ...
    while (core_fb->fb_update_head != NULL) {
        ...
        if (core_fb->fb_update_head != NULL) {
            ...
            asyncWriter_init(&fb_update_writer, &core_fb->fb_update_head->message,
...
corefb_destroy(CoreFramebuffer* core_fb)
{
    if (core_fb != NULL) {
        if (core_fb->looper != NULL) {
            ...
            while (core_fb->fb_update_head != NULL) {
                FBUpdateNotify* peding_update = core_fb->fb_update_head;
                core_fb->fb_update_head = peding_update->next_fb_update;
                fbupdatenotify_delete(peding_update);
            }
            core_fb->fb_update_tail = NULL;
