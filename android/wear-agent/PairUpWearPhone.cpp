...
void PairUpWearPhoneImpl::onAdbReply() {
    ...
    switch(mState) {
        case TO_READ_PROPERTY_1:
            ...
            asyncQueryAdb(PROPERTY, "ro.product.name");
        ...
        case TO_READ_PROPERTY_2:
            ...
            assert(!mdevs.empty());
            if (mreply.find("clockwork") != std::string::npos &&
                    !strncmp(mdevs.front().c_str(), "emulator-", 9)) {
                DPRINT("found wear %s\n\n", mdevs.front().c_str());
        ...
        case TO_READ_WEARABLE_APP_PKG_1:
            ...
            asyncQueryAdb(WEARABLE_APP_PKG, kWearableAppName);
        ...
        case TO_READ_WEARABLE_APP_PKG_2:
            ...
            assert(!mdevs.empty());
            if (mreply.find(kWearableAppName) !=  std::string::npos) {
...
void PairUpWearPhoneImpl::cleanUp() {
    ...
    if (PAIRUP_SUCCESS == mState) {
        DPRINT("\nSUCCESS\n");
    } else if (PAIRUP_INVALID != mState) {
        DPRINT("\nFAIL\n");
    }
    mState = PAIRUP_INVALID;
    DPRINT("\nclean up\n\n");
