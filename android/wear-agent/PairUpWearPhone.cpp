...
static PairUpWearPhoneImpl  *sPairUpWearPhoneImpl = 0;
...
public:
    ...
    static void reset() {
        if (sPairUpWearPhoneImpl) {
            delete sPairUpWearPhoneImpl;
            sPairUpWearPhoneImpl = 0;
        }
    }
    bool done() const {
        return FORWARDIP_DONE == mstate ||  PAIRUP_ERROR == mstate;
    }
private:
...
void PairUpWearPhoneImpl::onAdbReply() {
    ...
    switch(mstate) {
        case TO_READ_XFER_OKAY:
            ...
            makeQueryMsg(mWriteBuffer, sizeof(mWriteBuffer), mdevs.front().c_str(), PROPERTY,
        ...
        case TO_READ_PROPERTY:
            ...
            if (!strncmp("KKWT", pch, 4) && mdevs.size()) {
                ...
                closeAdbConnection();
                openAdbConnection();
                makeQueryMsg(mWriteBuffer, sizeof(mWriteBuffer), mdevs.front().c_str(), TRANSFER);
                asyncWriter_init(mAsyncWriter, mWriteBuffer, ::strlen(mWriteBuffer), mAdbIo);
                DPRINT("sending query '%s' to adb\n", mWriteBuffer);
        ...
        case TO_READ_WEARABLE_APP_PKG_1:
            ...
            makeQueryMsg(mWriteBuffer, sizeof(mWriteBuffer), mdevs.front().c_str(), WEARABLE_APP_PKG, 
                    " wearablepreview");
            asyncWriter_init(mAsyncWriter, mWriteBuffer, ::strlen(mWriteBuffer), mAdbIo);
            DPRINT("sending query '%s' to adb\n", mWriteBuffer);
            break;
...
void PairUpWearPhoneImpl::checkQueryCompletion() {
    mdevs.pop();
    closeAdbConnection();
    if (mdevs.size() && (mwears.empty() || mphones.empty())) {
...
void PairUpWearPhoneImpl::onConsoleReply() {
    ...
    char buff[1024]={'\0'};
    const int size = socket_recv(mConsoleSocket, buff, sizeof(buff));
    if (size == 0) {
...
PairUpWearPhoneImpl::PairUpWearPhoneImpl(Looper* looper, const char* devicelist, int adbHostPort) :
...
void
PairUpWearPhoneImpl::parse(const char* msg, StrQueue& tokens)
{
    const char* delims = " \t\r\n";
    char* buf = strdup(msg);
...
{
    DPRINT("connecting to console %d\n", mConsolePort);
...
PairUpWearPhone::PairUpWearPhone(Looper* looper, const char* devicelist, int adbHostPort) {
    PairUpWearPhoneImpl::reset();
    sPairUpWearPhoneImpl = new PairUpWearPhoneImpl(looper, devicelist, adbHostPort);
}
