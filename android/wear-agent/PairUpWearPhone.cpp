...
private:
    ...
    void makeQueryMsg(char buff[], int sz, const char* serial, MSG_TYPE type, const char* property = NULL);
...
void
PairUpWearPhoneImpl::onAdbReply()
{
    if (mrestart) {
        restart();
        return;
...
void
PairUpWearPhoneImpl::checkQueryCompletion() {
        fflush(stdout);
        mdevs.pop_back();
        closeAdbConnection();
...
void
PairUpWearPhoneImpl::onQueryAdb()
{
        if (mrestart) {
            restart();
            return;
        ...
        switch (mstate) {
            case TO_QUERY_XFER:
                if (mdevs.empty())
                    return;
                makeQueryMsg(buff, sizeof(buff), mdevs.back().c_str(), TRANSFER );
                size = socket_send(mAdbSocket, buff, ::strlen(buff));
            ...
            case TO_QUERY_PROPERTY:
                makeQueryMsg(buff, sizeof(buff), mdevs.back().c_str(), PROPERTY, "ro.build.version.release");
                size = socket_send(mAdbSocket, buff, ::strlen(buff));
                DPRINT("sending query '%s' to adb\n", buff);
                if (size <=  0) {
                    DPRINT("Error: cannot read device property \n");
                    mstate = PAIRUP_ERROR;
                    return;
                }
                mreply.clear();
            ...
            case TO_QUERY_WEARABLE_APP_PKG_2:
                makeQueryMsg(buff, sizeof(buff), mdevs.back().c_str(), WEARABLE_APP_PKG, 
                        " wearablepreview");
                size = socket_send(mAdbSocket, buff, ::strlen(buff));
                DPRINT("sending query '%s' to adb\n", buff);
                if (size <=  0) {
            ...
            case TO_QUERY_FORWARDIP: 
                ...
                makeQueryMsg(buff, sizeof(buff), mphones[0].c_str(), FORWARDIP);
                size = socket_send(mAdbSocket, buff, ::strlen(buff));
                DPRINT("sending query '%s' to adb\n", buff);
                if (size <=  0) {
                    DPRINT("Error: cannot setup port forwarding\n");
                    mstate = PAIRUP_ERROR;
                    break;
                }
        ...
        //DPRINT("1pairup->mAdbIo->impl: %p\n", mAdbIo->impl);
        loopIo_dontWantWrite(mAdbIo);
        //DPRINT("2pairup->mAdbIo->impl: %p\n", mAdbIo->impl);
        loopIo_wantRead(mAdbIo);
}

void
PairUpWearPhoneImpl::onConsoleReply() {
        if (mrestart) {
            restart();
            return;
        }
...
void
PairUpWearPhoneImpl::onQueryConsole() {
    if (mrestart) {
        restart();
        return;
    }
    ...
    socket_send(mConsoleSocket, buff, ::strlen(buff));
    DPRINT("send to console %s", buff);
...
void
PairUpWearPhoneImpl::connectWearToPhone(const char* s_devicesupdate_msg)
{
...
void
PairUpWearPhoneImpl::forwardIp(const char* phone)
{
...
//-----------------------            helper methods
void
PairUpWearPhoneImpl::openConsoleConnection()
{
...
void
PairUpWearPhoneImpl::openConnection(int port, int& so, LoopIo* io) {
    if (so < 0) {
...
void
PairUpWearPhoneImpl::makeQueryMsg(char buff[], int sz, const char* serial, MSG_TYPE type, const char* property)
{
    assert(serial);
    char buf2[1024];
    switch(type) {
...
void
PairUpWearPhone::connectWearToPhone(const char* devices) {
