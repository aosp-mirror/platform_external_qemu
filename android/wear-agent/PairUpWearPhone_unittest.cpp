...
int testPairUpWearPhone(int adbServerSocket, int consoleServerSocket, const char* wearDevice,
    ...
    if (error) return error;
    error = testSendToSocket(agentSocketQuery, "OKAYclockwork"); // pretend to be watch
    if (error) return error;
    ...
    error = testExpectMessageFromSocket(agentSocketQuery, kProperty);
    if (error) return error;
    error = testSendToSocket(agentSocketQuery, "OKAYsdk"); // pretend to be phone
    if (error) return error;
    socket_close(agentSocketQuery);
    ...
    error = testSendToSocket(agentSocketQuery, "OKAY");
    if (error) return error;
    const char* kPackages = "shell:pm list packages com.google.android.wearable";
    error = testExpectMessageFromSocket(agentSocketQuery, kPackages);
    if (error) return error;
    error = testSendToSocket(agentSocketQuery, "com.google.android.wearable.app");
    if (error) return error;
    socket_close(agentSocketQuery);
...
#ifndef _WIN32
static void runPairUp(const std::vector<std::string>& deviceList, int adbHostPort) {
