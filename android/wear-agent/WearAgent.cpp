...
_talk_to_adb_host(void* opaque, int fd, unsigned events) {
    android::wear::WearAgentImpl* agent = (android::wear::WearAgentImpl*)opaque;
    assert(agent);
...
,mExpectReplayType(OKAY)
{
    ...
    const char* msg_to_send = "host:track-devices";
    snprintf(mWriteBuffer, sizeof(mWriteBuffer), "%04x%s", (unsigned)(::strlen(msg_to_send)), msg_to_send);
...
void
WearAgentImpl::connectLater(int verbose) {
...
void
WearAgentImpl::cleanupConnection() {
    if (mSocket >=  0) {
...
WearAgentImpl::onRead() {
    ...
    switch (status) {
        case ASYNC_COMPLETE:
            ...
            } else if(expectLength() && 1 == sscanf(mReadBuffer, "%x", &msgsize) && msgsize > 0 && msgsize < READ_BUFFER_SIZE) {
