...
SyncSocket*
android_open_core_connection_socket(SockAddress* sockaddr)
{
    SyncSocket* ssocket;
    int status;
    int64_t deadline;
    ...
    // Console sockets are always INET sockets at this point.
...
CoreConnection*
android_create_core_connection(SockAddress* console_address)
{
    CoreConnection* desc = malloc(sizeof(CoreConnection));
    if (desc == NULL) {
...
void
android_free_core_connection(CoreConnection* desc)
{
    if (desc == NULL) {
        return;
    }
...
int
android_open_core_connection(CoreConnection* desc)
{
    if (desc == NULL) {
        errno = EINVAL;
        return -1;
    ...
    desc->ssocket = android_open_core_connection_socket(&desc->console_address);

    return (desc->ssocket != NULL) ? 0 : -1;
}

void
android_close_core_connection(CoreConnection* desc)
{
    if (desc == NULL) {
        return;
    }
...
int
android_write_core_connection(CoreConnection* desc,
                              const void* buffer,
                              size_t to_write,
...
int
android_read_core_connection(CoreConnection* desc,
                             void* buffer,
                             size_t to_read,
                             size_t* read_bytes)
{
...
int
android_switch_core_connection_stream(CoreConnection* desc,
                                      ...
                                      char** handshake)
{
    ...
    *handshake = NULL;
    ...
    if (android_write_core_connection(desc, buf, strlen(buf), NULL)) {
        return -1;
    }
...
void
android_detach_core_connection(CoreConnection* desc)
{
    android_write_core_connection(desc, "\n", 1, NULL);
}
