...
TEST(KmlParserInternal, FindCoordinates) {
    ...
    ASSERT_NE(findCoordinates(cur), (void *) NULL);
...
TEST(KmlParserInternal, FindNoCoordinates) {
    ...
    EXPECT_EQ(findCoordinates(cur), (void *) NULL);
...
TEST(KmlParserInternal, ParseEmptyCoordinates) {
    ...
    EXPECT_FALSE(parseCoordinates(cur, &locations));
    EXPECT_EQ(0, locations.size());
...
TEST(KmlParserInternal, ParseOneCoordinate) {
    ...
    GpsFixArray locations;
    EXPECT_EQ(0, locations.size());
    EXPECT_TRUE(parseCoordinates(cur, &locations));
...
TEST(KmlParserInternal, ParseLocationNormal) {
    ...
    string error;
    EXPECT_EQ(0, locations.size());
    EXPECT_TRUE(traverseSubtree(cur, &locations, &error));
...
TEST(KmlParserInternal, ParseLocationMissingRequiredFields) {
    ...
    string error;
    EXPECT_EQ(0, locations.size());
    EXPECT_FALSE(traverseSubtree(cur, &locations, &error));
    EXPECT_EQ(string("Location found with missing or malformed coordinates"), error);
    EXPECT_EQ(0, locations.size());
