...
xmlNode * findCoordinates(xmlNode * current) {
...
bool parseCoordinates(xmlNode * current, GpsFixArray * fixes) {
...
bool parsePlacemark(xmlNode * current, GpsFixArray * fixes) {
    string description = "";
    string name = "";
    int ind = -1;
...
bool traverseSubtree(xmlNode * current, GpsFixArray * fixes, string * error) {
    for (; current; current = current->next) {
        if (current->name != NULL &&
                !strcmp((const char *) current->name, "Placemark")) {

            if (!parsePlacemark(current->xmlChildrenNode, fixes)) {
                *error = string("Location found with missing or malformed coordinates");
                return false;
            }
...
bool parseFile(const char * filePath, GpsFixArray * fixes, string * error) {
    ...
    if (doc == NULL) {
        *error = string("KML document not parsed successfully.");
        xmlFreeDoc(doc);
        return false;
    }
    ...
    if (cur == NULL) {
        *error = string("Could not get root element of parsed KML file.");
        xmlFreeDoc(doc);
        xmlCleanupParser();
        return false;
    }
    bool wellFormed = traverseSubtree(cur, fixes, error);
