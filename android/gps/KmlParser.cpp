...
static bool parseCoordinates(xmlNode* current, GpsFixArray* fixes) {
    ...
    std::string coordinates {(const char *) coordinates_node->xmlChildrenNode->content};
    int offset = 0, n = 0;
    GpsFix new_fix;
    while(3 == sscanf(coordinates.c_str() + offset,
                      "%f , %f , %f%n",
                      &new_fix.longitude,
                      &new_fix.latitude,
                      &new_fix.elevation,
    ...
    for (int i = offset; i < coordinates.length() && result; ++i) {
