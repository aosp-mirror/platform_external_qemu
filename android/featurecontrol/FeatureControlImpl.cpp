...
static android::featurecontrol::FeatureControlImpl* s_featureControl = nullptr;
...
#endif  // _WIN32
    if (_STRICMP(str.c_str(), "on")) {
        return ON;
    } else if (_STRICMP(str.c_str(), "off")) {
        return OFF;
    } else if (_STRICMP(str.c_str(), "default")) {
        return DEFAULT;
    } else {
        return ERR;
    }
...
static void loadUserOverrideFeature(
    ...
    switch (ParseIniStr(val)) {
        ...
        case OFF:
            ...
            break;
        case DEFAULT:
            s_featureControl->resetEnabledToDefault(featureName);
            break;
        default:
...
#define FEATURE_CONTROL_ITEM(item)                                         \
    initFeatureAndParseDefault(                                            \
            defaultIni, item, #item, [](Feature feature, bool isEnabled) { \
                s_featureControl->initEnabledDefault(feature, isEnabled);  \
            });
