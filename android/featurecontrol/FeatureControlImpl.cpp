...
void FeatureControlImpl::init(std::string defaultIniPath,
...
#undef FEATURE_CONTROL_ITEM
    ...
    std::ifstream inFile(userIniPath);
    if (inFile) {
        inFile.close();
...
bool FeatureControlImpl::isEnabled(AndroidFeatureControlFeature feature) {
    return mFeatures[feature].currentVal;
}

void FeatureControlImpl::setEnabledOverride(AndroidFeatureControlFeature feature, bool isEnabled) {
    FeatureOption& currFeature = mFeatures[feature];
    currFeature.currentVal = isEnabled;
    currFeature.isOverridden = true;
}

void FeatureControlImpl::resetEnabledToDefault(AndroidFeatureControlFeature feature) {
    FeatureOption& currFeature = mFeatures[feature];
    currFeature.currentVal = currFeature.defaultVal;
    currFeature.isOverridden = false;
}

void FeatureControlImpl::initEnabledDefault(AndroidFeatureControlFeature feature, bool isEnabled) {
    FeatureOption& currFeature = mFeatures[feature];
