...
enum { REPORT_CONSOLE_SERVER = (1 << 0), REPORT_CONSOLE_MAX = (1 << 1) };
...
static void report_console(const char* proto_port, int console_port) {
    int s = -1, s2;
    int maxtries = 10;
    int flags = 0;
    ...
    if (!strncmp(proto_port, "tcp:", 4)) {
        ...
        if (flags & REPORT_CONSOLE_SERVER) {
            ...
            if (s < 0) {
                fprintf(stderr,
                        "could not create server socket on TCP:%ld: %s\n", port,
                        errno_str);
                exit(3);
            }
        } else {
...
#ifdef _WIN32
        fprintf(stderr,
                "sorry, the unix: protocol is not supported on Win32\n");
        exit(3);
#else
        ...
        if (flags & REPORT_CONSOLE_SERVER) {
            ...
            if (s < 0) {
                fprintf(stderr,
                        "could not connect to unix socket on '%s': %s\n", path,
                        errno_str);
                exit(3);
            }
        }
        ...
        fprintf(stderr,
                "-report-console must be followed by a 'tcp:<port>' or "
                "'unix:<path>'\n");
        exit(3);
    }
    ...
    if (flags & REPORT_CONSOLE_SERVER) {
        ...
        if (s2 < 0) {
            fprintf(stderr,
                    "could not accept console-reporting client connection: "
                    "%s\n",
                    errno_str);
            exit(3);
        }
...
static int qemu_control_console_start(
        int port,
    ...
    return control_console_start(port, batteryAgent, fingerAgent, locationAgent,
                                 userEventAgent, vmOperations, netAgent);
}
...
void android_emulation_setup(const QAndroidBatteryAgent* batteryAgent,
    ...
    if (s_support_configurable_ports) {
        ...
        int base_port = 5554;
        int legacy_adb =
                avdInfo_getAdbdCommunicationMode(android_avdInfo) ? 0 : 1;
        ...
        if (android_op_ports) {
            ...
            // Set up redirect from host to guest system. adbd on the guest
            // listens
            // on 5555.
            if (legacy_adb) {
            ...
            if (qemu_control_console_start(
                        console_port, batteryAgent, fingerAgent, locationAgent,
                        userEventAgent, vmOperations, netAgent) < 0) {
                if (legacy_adb) {
            ...
            if (android_op_port) {
                ...
                if ((port & 1) != 0) {
                    port &= ~1;
                    dwarning(
                            "option -port must be followed by an even integer, "
                            "using  port number %d\n",
                            port);
                }
                base_port = port;
            ...
            for (; tries > 0; tries--, base_port += 2) {
                ...
                if (qemu_control_console_start(
                            base_port, batteryAgent, fingerAgent, locationAgent,
                            userEventAgent, vmOperations, netAgent) < 0) {
                    if (legacy_adb) {
                        netAgent->slirpUnredir(false, adb_port);
                    }
                ...
                D("control console listening on port %d, ADB on port %d",
                  base_port, adb_port);
                success = 1;
                break;
            }
            ...
            if (!success) {
                fprintf(stderr,
                        "it seems too many emulator instances are running on "
        ...
        * started.
        * it should be listening on port 5037. if we can't reach it, don't
        * bother
        */
        ...
        if (s < 0) {
            ...
            // Where <port> is the decimal adb port number, and <hex4> is the
            // length
            // of the payload that follows it in hex.
            int len =
                    snprintf(tmp, sizeof tmp, "0000host:emulator:%d", adb_port);
    ...
    do {
        ...
        if (p != 0) {
            ...
            if (q == NULL) {
            BadHttpProxyFormat:
                dprint("http_proxy format unsupported, try 'proxy:port' or "
        ...
        D("setting up http proxy:  server=%.*s port=%d", proxy_name_len,
         ...
         */
        if (proxy_check_connection(proxy_name, proxy_name_len, proxy_port,
                                   1000) < 0) {
        ...
        if (proxy_http_setup(proxy_name, proxy_name_len, proxy_port,
            ...
            dprint("Http proxy setup failed for '%.*s:%d': %s", proxy_name_len,
                   proxy_name, proxy_port, errno_str);
            dprint("Proxy will be ignored !");
        }
    } while (0);
    ...
    /* initialize fingperprint sensor */
