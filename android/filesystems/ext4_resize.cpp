...
#ifdef _WIN32
    auto& sys = *System::get();
    auto retCode = sys.runCommand({executable, "-f", partitionPath, size_in_MB}, RunOptions::WaitForCompletion
                                  | RunOptions::ReturnExitCode | RunOptions::ShowOutput);

    DWORD exitCode = (retCode == System::RunFailed) ? 1 : 0;
#else
    int32_t exitCode = 0;
    pid_t pid;
    pid_t child = fork();
