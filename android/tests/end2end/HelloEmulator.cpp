...
protected:
    ...
    bool didEmulatorKernelStartup(const std::string& output) {
        ...
        // Kernel command line will show up during actual kernel startup.
        bool hasKernelCommandLine =
            output.find("Kernel command line:") != std::string::npos;

        return hasMainLoopStartup && hasColdBootChoice && hasKernelCommandLine;
    }
