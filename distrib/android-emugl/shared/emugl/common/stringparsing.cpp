...
std::string replace_with(const std::string& rm,
    ...
    while (p < str.size()) {
        ...
        if (match_pos != kStdStringNotFound) {
...
std::string remove_in_out(const std::string& src,
    ...
    while (p < src.size()) {
        ...
        size_t out_pos;
        if (in_pos != kStdStringNotFound) {
            out_pos = src.find(out, in_pos + in.size());
            if (out_pos != kStdStringNotFound) {
                found_match = true;
            } else {
                *err = true;
                return src;
