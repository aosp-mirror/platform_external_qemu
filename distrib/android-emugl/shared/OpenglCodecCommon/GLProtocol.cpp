...
bool GLProtocol::setVersion(uint32_t version) {
    ...
    m_version = version;
    LOG_GLPROTOCOL("%s: GLprotocol Set Version %d\n", __FUNCTION__,
                m_version);
    return true;
}
...
const char* GLProtocol::getMaxVersionString() {
    if (maxVersionStr.length() == 0) {
        ...
        maxVersionStr = std::string("ANDROID_EMU_GL_PROTOCOL_v") + buffer;
        LOG_GLPROTOCOL("%s: GLprotocol Max Version %s\n", __FUNCTION__,
                maxVersionStr.c_str());
    }
...
void GLProtocol::writeChecksum(void *outputChecksum) {
...
bool GLProtocol::validate(const void *buf, size_t packetLen) {
    ...
    switch (m_version) {
        case 1: {
            ...
            memcpy(checksumBuf, &val, 4);
            memcpy(checksumBuf+4, &m_numRead, 4);
            break;
        }
    }
