...
static const uint32_t kV2CRCPoly = 0x04c11db7;
...
static constexpr uint32_t v2ComputeCRC(uint32_t crc, int k, uint32_t crcPoly) {
    return k == 0 ? crc :
        v2ComputeCRC(crc >> 31 & 1 ? crc << 1 ^ crcPoly : crc << 1, k-1, crcPoly);
}
...
constexpr V2CRCTable v2ComputeCRCTable(uint32_t crcPoly,
    ...
    return  v2ComputeCRCTable<Bits, Size-1>(crcPoly, true,
            v2ComputeCRC((Size - 1) << 24, Bits, crcPoly), table...);
}
...
static unsigned char reverse(unsigned char i) {
...
void ChecksumCalculator::addBufferV2(const void* buf, size_t bufLen) {
    ...
    for (size_t i=0; i<bufLen; i++) {
        ...
        m_v2PartialCRC = (m_v2PartialCRC << 8) ^ kV2CRCTable.val[(m_v2PartialCRC >> 24 & 0xff) ^ ptr[i]];
    }
}

// TODO: for debug
void ChecksumCalculator::crcTest() {
    printf("crc 0: %x\n", v2ComputeCRC(0, 8, kV2CRCPoly) );
    printf("crc 1: %x\n", v2ComputeCRC(1, 8, kV2CRCPoly) );
    printf("\n");
    for (int i=0; i<256; i++) {
        printf("%x\t", kV2CRCTable.val[i]);
    }
    printf("\n");
