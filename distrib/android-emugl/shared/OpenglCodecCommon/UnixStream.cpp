...
UnixStream::~UnixStream()
{
    if (bound_socket_path != NULL) {
        if (unlink(bound_socket_path) < 0) {
            ERR("Failed to unlink UNIX socket at \"%s\"\n", bound_socket_path);
            perror("UNIX socket could not be unlinked");
        } else {
            DBG("Unlinked UNIX socket at \"%s\"\n", bound_socket_path);
            delete bound_socket_path;
        }
    }
}
...
int UnixStream::listen(char addrstr[MAX_ADDRSTR_LEN])
{
    ...
    bound_socket_path = new char[MAX_ADDRSTR_LEN];
    if(bound_socket_path == NULL)
        return -1;
    if(bound_socket_path != strcpy(bound_socket_path, addrstr)) {
        delete bound_socket_path;
        bound_socket_path = NULL;
