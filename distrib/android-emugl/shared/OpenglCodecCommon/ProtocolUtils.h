...
// Helper classess GenericInputBuffer and GenericOutputBuffer used to ensure
...
// if needed. If a no-op if |ptr| was aligned.
//
// Both classes try to minimize heap usage as much as possible - first template
// argument defines the size of an internal array Generic*Buffer-s use if the
// |ptr|'s |size| is small enough. If it doesn't fit into the internal array,
// aligned copy of is allocated on the heap and freed in dtor.
...
public:
    ...
    ~GenericOutputBuffer() {
        if (mPtr != mOrgBuff && mPtr != &mArray[0]) {
            free(mPtr);
        }
    }
