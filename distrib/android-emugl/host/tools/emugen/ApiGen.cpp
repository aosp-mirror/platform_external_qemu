...
void ApiGen::genProcTypes_forEntryPoint(FILE* fp,
                                       SideType side,
                                       EntryPoint* e,
    ...
    fprintf(fp, " (%s_APIENTRY *%s_%s_proc_t) (", basename, usedName.c_str(), sideString(side));
...
int ApiGen::genProcTypes(const std::string &filename, SideType side)
{
    ...
    for (size_t i = 0; i < size(); i++) {
        EntryPoint *e = &at(i);
        genProcTypes_forEntryPoint(fp, side, e, e->name());
        if (e->hasBottomHalf()) {
            genProcTypes_forEntryPoint(fp, side, e, e->name() + "_bh");
        }
    }
    fprintf(fp, "\n\n#endif\n");
    return 0;
...
int ApiGen::genFuncTable(const std::string &filename, SideType side)
{
    ...
    for (size_t i = 0; i < size(); i++) {
        ...
        if (e->hasBottomHalf()) {
            fprintf(fp, "\t{\"%s\", (void*)%s},\n", (e->name() + "_bh").c_str(), (e->name() + "_bh").c_str());
        }
    }
    fprintf(fp, "};\n");
...
int ApiGen::genContext(const std::string & filename, SideType side)
{
    ...
    for (size_t i = 0; i < size(); i++) {
        ...
        if (e->hasBottomHalf()) {
            fprintf(fp, "\t%s_%s_proc_t %s;\n", (e->name() + "_bh").c_str(), sideString(side), (e->name() + "_bh").c_str());
        }
    }
...
\t\tswitch(opcode) {\n");
    ...
    for (size_t f = 0; f < n; f++) {
        ...
        for (int pass = PASS_FIRST; pass < PASS_LAST; pass++) {
            bool inBottomHalf = pass == PASS_BottomHalf && e->hasBottomHalf();
            ...
            if (pass == PASS_FunctionCall ||
                inBottomHalf) {
                if (inBottomHalf) {
                    fprintf(fp, "\t\t\tthis->%s(", (e->name() + "_bh").c_str());
                } else {
