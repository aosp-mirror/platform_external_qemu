...
int ApiGen::genEncoderHeader(const std::string &filename)
{
    ...
    fprintf(fp, "#include \"ChecksumHelper.h\"\n");
    ...
    fprintf(fp, "\tChecksumHelper *m_checksumHelper;\n\n");

    fprintf(fp, "\t%s(IOStream *stream, ChecksumHelper *checksumHelper);\n", classname.c_str());
    fprintf(fp, "};\n\n");
...
static void writeVarLargeEncodingExpression(Var& var, FILE* fp)
{
    ...
    fprintf(fp, "\tchecksumHelper->addBuffer(&__size_%s,4);\n", varname);
    if (var.nullAllowed()) {
	...
	fprintf(fp, "\t\tchecksumHelper->addBuffer(%s, __size_%s);\n", varname, varname);
    }
...
//      fprintf(fp, "\n\tDBG(\">>>> %s\\n\");\n", e->name().c_str());
	...
	fprintf(fp, "\tChecksumHelper *checksumHelper = ctx->m_checksumHelper;\n\n");
        VarsArray & evars = e->vars();
	...
	fprintf(fp, "\t const size_t checksumSize = checksumHelper->checksumBitSize();\n");
	...
	fprintf(fp, "\t const size_t totalSize = sizeWithoutChecksum + checksumSize;\n");
        ...
        while (nvars < maxvars || maxvars == 0) {
		    ...
		    hasLargeFields = true;
                ...
                if (maxvars == 0) {
		    fprintf(fp, "\n\tchecksumHelper->addBuffer(buf, ptr-buf);\n");
                    break;
		}
	...
	if (hasLargeFields) {
	    ...
	    fprintf(fp, "\tchecksumHelper->writeChecksum(buf);\n");
	} else {
	    fprintf(fp, "\tchecksumHelper->writeChecksum(ptr); ptr += checksumSize;\n");
	}
...
//XXX       fprintf(fp, "\n\tDBG(\"<<<< %s\\n\");\n", e->name().c_str());
    ...
    // constructor
    fprintf(fp, "%s::%s(IOStream *stream, ChecksumHelper *checksumHelper)\n{\n", classname.c_str(), classname.c_str());
    fprintf(fp, "\tm_stream = stream;\n");
    fprintf(fp, "\tm_checksumHelper = checksumHelper;\n\n");
...
int ApiGen::genDecoderImpl(const std::string &filename)
{
    ...
    fprintf(fp, "#include \"ChecksumHelperThreadInfo.h\"\n\n");
    fprintf(fp, "#include <stdio.h>\n\n");
    ...
    for (size_t f = 0; f < n; f++) {
        ...
        for (int pass = PASS_FIRST; pass < PASS_LAST; pass++) {
            ...
            if (pass == PASS_Protocol) {
                fprintf(fp,
                        "\t\t\tChecksumHelperThreadInfo::validOrDie(ptr, %s, \"%s::decode, OP_%s: GL checksumHelper failure\\n\");\n",
			varoffset.c_str(),
			classname.c_str(),
			e->name().c_str()
			);
