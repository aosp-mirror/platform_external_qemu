...
int ApiGen::genEncoderHeader(const std::string &filename)
{
    ...
    fprintf(fp, "#include \"GLProtocol.h\"\n");
    ...
    fprintf(fp, "\tGLProtocol *m_protocol;\n\n");

    fprintf(fp, "\t%s(IOStream *stream, GLProtocol *protocol);\n", classname.c_str());
    fprintf(fp, "};\n\n");
...
static void writeVarLargeEncodingExpression(Var& var, FILE* fp)
{
    ...
    fprintf(fp, "\tprotocol->addBuffer(&__size_%s,4);\n", varname);
    if (var.nullAllowed()) {
	...
	fprintf(fp, "\t\tprotocol->addBuffer(%s, __size_%s);\n", varname, varname);
    }
...
//      fprintf(fp, "\n\tDBG(\">>>> %s\\n\");\n", e->name().c_str());
	...
	fprintf(fp, "\tGLProtocol *protocol = ctx->m_protocol;\n\n");
        VarsArray & evars = e->vars();
	...
	fprintf(fp, "\t const size_t checksumSize = protocol->checksumStrSize();\n");
	...
	fprintf(fp, "\t const size_t totalSize = sizeWithoutChecksum + checksumSize;\n");
        ...
        while (nvars < maxvars || maxvars == 0) {
		    ...
		    hasLargeFields = true;
                ...
                if (maxvars == 0) {
		    fprintf(fp, "\n\tprotocol->addBuffer(buf, ptr-buf);\n");
                    break;
		}
	...
	if (hasLargeFields) {
	    ...
	    fprintf(fp, "\tprotocol->checksumStrFromBuffers(buf);\n");
	} else {
	    fprintf(fp, "\tprotocol->checksumStrFromBuffers(ptr); ptr += checksumSize;\n");
	}
...
//XXX       fprintf(fp, "\n\tDBG(\"<<<< %s\\n\");\n", e->name().c_str());
    ...
    // constructor
    fprintf(fp, "%s::%s(IOStream *stream, GLProtocol *protocol)\n{\n", classname.c_str(), classname.c_str());
    fprintf(fp, "\tm_stream = stream;\n");
    fprintf(fp, "\tm_protocol = protocol;\n\n");
        ...
        for (int pass = PASS_FIRST; pass < PASS_LAST; pass++) {
            ...
            if (pass == PASS_Protocol) {
                fprintf(fp,
                        ...
                        "\t\t\t\tGLProtocolThreadInfo::validOrDie(ptr, %s, \"%s::decode, OP_%s: GL protocol failure\\n\");\n"
                        "\t\t\t}\n",
			varoffset.c_str(),
			classname.c_str(),
			e->name().c_str()
