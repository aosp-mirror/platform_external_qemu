...
int ApiGen::genEncoderImpl(const std::string &filename)
{
    ...
    // Conversion value needed for timestamp with KVM clock
    fprintf(fp, "static constexpr long sec_to_nsec = pow(10, 9);\n\n");
...
//      fprintf(fp, "\n\tDBG(\">>>> %s\\n\");\n", e->name().c_str());
        ...
        if (hasLargeFields) { // TODO; allocating but not using??
            ...
            fprintf(fp, "\tif (useTimestamp) { memcpy(buf, &time_nsec, timestampSize); buf += timestampSize; }\n");
            fprintf(fp, "\tif (useTimestamp && useChecksum) checksumCalculator->addBuffer(&time_nsec, timestampSize);\n");
            fprintf(fp, "\tif (useChecksum) checksumCalculator->writeChecksum(buf, checksumSize);\n\n");
        } else {
            fprintf(fp, "\tif (useTimestamp) { memcpy(ptr, &time_nsec, timestampSize); ptr += timestampSize; }\n");
            fprintf(fp, "\tif (useTimestamp && useChecksum) checksumCalculator->addBuffer(&time_nsec, timestampSize);\n");
...
int ApiGen::genDecoderImpl(const std::string &filename)
{
    ...
    fprintf(fp, "#include \"TimestampLoggerThreadInfo.h\"\n\n");
    fprintf(fp, "#include <stdio.h>\n\n");
    ...
    for (size_t f = 0; f < n; f++) {
        ...
        for (int pass = PASS_FIRST; pass < PASS_LAST; pass++) {
            ...
            if (pass == PASS_Protocol) {
                fprintf(fp,
                        "\t\t\tif (useTimestamp) {\n"
                        "\t\t\t\tTimestampLoggerThreadInfo::logTimestamp(ptr + %s, timestampSize);\n"
                        "\t\t\t}\n",
                        varoffset.c_str()
                        );
                fprintf(fp,
