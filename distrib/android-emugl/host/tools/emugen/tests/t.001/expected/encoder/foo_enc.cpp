...
void fooAlphaFunc_enc(void *self , FooInt func, FooFloat ref)
{
	...
	GLProtocol *protocol = ctx->m_protocol;

	 unsigned char *ptr;
	 unsigned char *buf;
	 const size_t sizeWithoutChecksum = 8 + 4 + 4;
	 const size_t checksumSize = protocol->checksumStrSize();
	...
	protocol->checksumStrFromBuffers(ptr); ptr += checksumSize;
}
...
foo_encoder_context_t::foo_encoder_context_t(IOStream *stream, GLProtocol *protocol)
{
	m_stream = stream;
	m_protocol = protocol;
