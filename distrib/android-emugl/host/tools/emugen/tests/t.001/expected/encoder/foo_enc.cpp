...
void fooAlphaFunc_enc(void *self , FooInt func, FooFloat ref)
{
	...
	ChecksumHelper *checksumHelper = ctx->m_checksumHelper;

	 unsigned char *ptr;
	 unsigned char *buf;
	 const size_t sizeWithoutChecksum = 8 + 4 + 4;
	 const size_t checksumSize = checksumHelper->checksumBitSize();
	...
	checksumHelper->writeChecksum(ptr); ptr += checksumSize;
}
...
foo_encoder_context_t::foo_encoder_context_t(IOStream *stream, ChecksumHelper *checksumHelper)
{
	m_stream = stream;
	m_checksumHelper = checksumHelper;
