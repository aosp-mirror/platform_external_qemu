...
size_t foo_decoder_context_t::decode(void *buf, size_t len, IOStream *stream)
{
	...
	while ((len - pos >= 8) && !unknownOpcode) {   
		...
		case OP_fooAlphaFunc: {
			...
			ChecksumHelperThreadInfo::validOrDie(ptr, 8 + 4 + 4, "foo_decoder_context_t::decode, OP_fooAlphaFunc: GL checksumHelper failure\n");
		...
		case OP_fooIsBuffer: {
			...
			ChecksumHelperThreadInfo::validOrDie(ptr, 8 + 4 + size_stuff, "foo_decoder_context_t::decode, OP_fooIsBuffer: GL checksumHelper failure\n");
		...
		case OP_fooUnsupported: {
			...
			ChecksumHelperThreadInfo::validOrDie(ptr, 8 + 4 + size_params, "foo_decoder_context_t::decode, OP_fooUnsupported: GL checksumHelper failure\n");
		...
		case OP_fooDoEncoderFlush: {
			...
			ChecksumHelperThreadInfo::validOrDie(ptr, 8 + 4, "foo_decoder_context_t::decode, OP_fooDoEncoderFlush: GL checksumHelper failure\n");
		...
		case OP_fooTakeConstVoidPtrConstPtr: {
			...
			ChecksumHelperThreadInfo::validOrDie(ptr, 8 + 4 + size_param, "foo_decoder_context_t::decode, OP_fooTakeConstVoidPtrConstPtr: GL checksumHelper failure\n");
