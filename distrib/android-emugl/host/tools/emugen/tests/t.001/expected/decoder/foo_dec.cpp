...
size_t foo_decoder_context_t::decode(void *buf, size_t len, IOStream *stream)
{
	...
	while ((len - pos >= 8) && !unknownOpcode) {   
		...
		uint32_t glProtocolVersion = GLProtocolThreadInfo::getVersion();
		if (len - pos < packetLen)  return pos; 
		switch(opcode) {
		case OP_fooAlphaFunc: {
			FooInt var_func = Unpack<FooInt,uint32_t>(ptr + 8);
			FooFloat var_ref = Unpack<FooFloat,uint32_t>(ptr + 8 + 4);
			if (glProtocolVersion >= 1) {
				GLProtocolThreadInfo::validOrDie(ptr, ptr + 8 + 4 + 4, "foo_decoder_context_t::decode, OP_fooAlphaFunc: GL protocol failure\n");
			}
		...
		case OP_fooIsBuffer: {
			...
			if (glProtocolVersion >= 1) {
				GLProtocolThreadInfo::validOrDie(ptr, ptr + 8 + 4 + size_stuff, "foo_decoder_context_t::decode, OP_fooIsBuffer: GL protocol failure\n");
			}
		...
		case OP_fooUnsupported: {
			...
			if (glProtocolVersion >= 1) {
				GLProtocolThreadInfo::validOrDie(ptr, ptr + 8 + 4 + size_params, "foo_decoder_context_t::decode, OP_fooUnsupported: GL protocol failure\n");
			}
		...
		case OP_fooDoEncoderFlush: {
			...
			if (glProtocolVersion >= 1) {
				GLProtocolThreadInfo::validOrDie(ptr, ptr + 8 + 4, "foo_decoder_context_t::decode, OP_fooDoEncoderFlush: GL protocol failure\n");
			}
		...
		case OP_fooTakeConstVoidPtrConstPtr: {
			...
			if (glProtocolVersion >= 1) {
				GLProtocolThreadInfo::validOrDie(ptr, ptr + 8 + 4 + size_param, "foo_decoder_context_t::decode, OP_fooTakeConstVoidPtrConstPtr: GL protocol failure\n");
			}
