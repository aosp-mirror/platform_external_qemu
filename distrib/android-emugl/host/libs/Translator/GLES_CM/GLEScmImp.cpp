...
GL_API GLboolean GL_APIENTRY glIsRenderbufferOES(GLuint renderbuffer) {
    ...
    if(renderbuffer && ctx->shareGroup().get()){
        return ctx->shareGroup()->isObject(NamedObjectType::RENDERBUFFER,renderbuffer) ? GL_TRUE :GL_FALSE;
    }
...
GL_API void GLAPIENTRY glBindRenderbufferOES(GLenum target, GLuint renderbuffer) {
    ...
    //if buffer wasn't generated before,generate one
    if(renderbuffer && ctx->shareGroup().get() && !ctx->shareGroup()->isObject(NamedObjectType::RENDERBUFFER,renderbuffer)){
    ...
    int globalBufferName = (renderbuffer != 0) ? ctx->shareGroup()->getGlobalName(NamedObjectType::RENDERBUFFER,renderbuffer) : 0;
...
GL_API GLboolean GLAPIENTRY glIsFramebufferOES(GLuint framebuffer) {
    ...
    if (framebuffer && ctx->shareGroup().get()) {
        return ctx->shareGroup()->isObject(NamedObjectType::FRAMEBUFFER,framebuffer) ? GL_TRUE : GL_FALSE;
    }
...
GL_API void GLAPIENTRY glBindFramebufferOES(GLenum target, GLuint framebuffer) {
    ...
    if (framebuffer && ctx->shareGroup().get() && !ctx->shareGroup()->isObject(NamedObjectType::FRAMEBUFFER,framebuffer)) {
    ...
    int globalBufferName = (framebuffer!=0) ? ctx->shareGroup()->getGlobalName(NamedObjectType::FRAMEBUFFER,framebuffer) : 0;
...
GL_API void GLAPIENTRY glFramebufferTexture2DOES(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level) {
    ...
    if(texture) {
        ...
        globalTexName = ctx->shareGroup()->getGlobalName(NamedObjectType::TEXTURE,texname);
    }
...
GL_API void GLAPIENTRY glFramebufferRenderbufferOES(GLenum target, GLenum attachment,GLenum renderbuffertarget, GLuint renderbuffer) {
    ...
    if (renderbuffer) {
        ...
        globalBufferName = ctx->shareGroup()->getGlobalName(NamedObjectType::RENDERBUFFER,renderbuffer);
    }
