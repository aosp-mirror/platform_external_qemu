...
#define CHECK_SHADER_NAME_ERROR_AND_RETURN(globalShaderName, shader, ret) \
            RET_AND_SET_ERROR_IF(globalShaderName==0                            \
                    && ctx->shareGroup()->getGlobalName(NamedObjectType::PROGRAM,\
                        shader) != 0, GL_INVALID_OPERATION, ret);               \
            RET_AND_SET_ERROR_IF(globalShaderName==0, GL_INVALID_VALUE, ret);
