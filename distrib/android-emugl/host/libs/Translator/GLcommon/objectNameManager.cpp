...
ShareGroup::ShareGroup(GlobalNameSpace *globalNameSpace) {
    for (int i=0; i < static_cast<int>(NamedObjectType::NUM_OBJECT_TYPES); i++) {
        m_nameSpace[i] = new NameSpace(static_cast<NamedObjectType>(i), globalNameSpace);
    }
}
...
ShareGroup::~ShareGroup()
{
    ...
    for (int t = 0; t < static_cast<int>(NamedObjectType::NUM_OBJECT_TYPES); t++) {
...
ShareGroup::genName(GenNameInfo genNameInfo,
                    ...
                    bool genLocal)
{
    if (static_cast<int>(genNameInfo.m_type) >= static_cast<int>(NamedObjectType::NUM_OBJECT_TYPES)) return 0;

    emugl::Mutex::AutoLock _lock(m_lock);
    ObjectLocalName localName =
            m_nameSpace[static_cast<ObjectDataType>(genNameInfo.m_type)]->genName(genNameInfo, p_localName,
                                                   genLocal);
    if (genNameInfo.m_type == NamedObjectType::TEXTURE) {
        incTexRefCounterNoLock(m_nameSpace[static_cast<int>(NamedObjectType::TEXTURE)]->getGlobalName(localName));
    }
    return localName;
}
...
{
    if (static_cast<int>(p_type) >= static_cast<int>(NamedObjectType::NUM_OBJECT_TYPES)) return;
...
{
    if (static_cast<int>(p_type) >= static_cast<int>(NamedObjectType::NUM_OBJECT_TYPES)) return 0;
                              ...
                              unsigned int p_globalName)
{
    ...
    emugl::Mutex::AutoLock _lock(m_lock);
    m_nameSpace[static_cast<int>(p_type)]->replaceGlobalName(p_localName, p_globalName);
}
                          ...
                          ObjectLocalName p_localName)
{
    ...
    if (static_cast<int>(p_type) >= static_cast<int>(NamedObjectType::NUM_OBJECT_TYPES)) return ret;
...
ShareGroup::decTexRefCounterAndReleaseIf0(unsigned int p_globalName) {
    ...
    map->erase(iterator);
    m_nameSpace[static_cast<int>(NamedObjectType::TEXTURE)]->m_globalNameSpace->deleteName(NamedObjectType::TEXTURE, p_globalName);
    return 0;
}
