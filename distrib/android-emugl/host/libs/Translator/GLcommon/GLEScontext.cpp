...
const GLvoid* GLEScontext::setPointer(GLenum arrType,GLint size,GLenum type,GLsizei stride,const GLvoid* data,bool normalize) {
    ...
    if(bufferName) {
        ...
        GLESbuffer* vbo = static_cast<GLESbuffer*>(m_shareGroup->getObjectData(NamedObjectType::VERTEXBUFFER,bufferName).get());
...
void GLEScontext::drawValidate(void)
{
    ...
    ObjectDataPtr fbObj = m_shareGroup->getObjectData(NamedObjectType::FRAMEBUFFER,m_framebuffer);
    if (fbObj.get() == NULL)
        return;
