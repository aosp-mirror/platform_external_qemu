...
{
    if ( genNameInfo.m_type >= NamedObjectType::NUM_OBJECT_TYPES ) return 0;
    unsigned int name = 0;
    ...
    case NamedObjectType::SHADER:
        name = GLEScontext::dispatcher().glCreateShader(genNameInfo.m_shaderType);
        break;
    ...
    if (!m_deleteInitialized) {
        ...
        m_glDelete[static_cast<int>(NamedObjectType::VERTEXBUFFER)] = [](GLuint lname)
                { GLEScontext::dispatcher().glDeleteBuffers(1, &lname);};
        m_glDelete[static_cast<int>(NamedObjectType::TEXTURE)] = [](GLuint lname)
                { GLEScontext::dispatcher().glDeleteTextures(1, &lname);};
        m_glDelete[static_cast<int>(NamedObjectType::RENDERBUFFER)] = [](GLuint lname)
                { GLEScontext::dispatcher().glDeleteRenderbuffersEXT(1, &lname);};
        m_glDelete[static_cast<int>(NamedObjectType::FRAMEBUFFER)] = [](GLuint lname)
                { GLEScontext::dispatcher().glDeleteFramebuffersEXT(1, &lname);};
        m_glDelete[static_cast<int>(NamedObjectType::PROGRAM)] = GLEScontext::dispatcher().glDeleteProgram;
        m_glDelete[static_cast<int>(NamedObjectType::SHADER)] = GLEScontext::dispatcher().glDeleteShader;
    }
    return name;
}
