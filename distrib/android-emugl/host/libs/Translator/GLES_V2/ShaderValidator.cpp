...
static void validate_keywords_in_decls(const std::vector<std::string>& decls,
    ...
    for (size_t i = 0; i < decls.size(); i++) {
        ...
        if (storage_pos != kStdStringNotFound &&
                precision_pos != kStdStringNotFound) {
            if (storage_pos < precision_pos) {
                if (parameter_pos != kStdStringNotFound) {
        ...
        } else if (precision_pos != kStdStringNotFound &&
                parameter_pos != kStdStringNotFound) {
            if (parameter_pos > precision_pos) {
                *err = true;
            }
        }
...
static void validate_glsles_variable_decls(
    ...
    for (size_t i = 0; i < raw_decls.size(); i++) {
        if (raw_decls[i].find(openpar) == kStdStringNotFound) {
