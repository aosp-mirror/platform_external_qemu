...
void validate_keywords_in_decls(const std::vector<std::string>& decls,
                                bool* err) {
    ...
    for (size_t i = 0; i < decls.size(); i++) {
        ...
        variance_pos = multi_find(decls[i], variance_keywords);
...
void validate_glsles_function_parameters(const std::vector<std::string>& raw_decls,
    ...
    for (size_t i = 0; i < raw_decls.size(); i++) {
        if (raw_decls[i].find(openpar) != kNotFound) {
            std::vector<std::string> func_params = isolate_in_out(raw_decls[i], openpar, closepar, &parse_err);
    ...
    for (size_t i = 0; i < func_param_decls.size(); i++) {
        fprintf(stderr, "funcparam: %s\n", func_param_decls[i].c_str());
    }
