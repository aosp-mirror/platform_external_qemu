...
static void s_attachShader(GLEScontext* ctx, GLuint program, GLuint shader) {
    if (ctx && program && shader && ctx->shareGroup().get()) {
        ObjectDataPtr shaderData = ctx->shareGroup()->getObjectData(NamedObjectType::SHADER,shader);
...
static TextureData* getTextureData(ObjectLocalName tex) {
    ...
    ObjectDataPtr objData = ctx->shareGroup()->getObjectData(NamedObjectType::TEXTURE,tex);
    if(!objData.get()){
        texData = new TextureData();
        ctx->shareGroup()->setObjectData(NamedObjectType::TEXTURE, tex, ObjectDataPtr(texData));
    } else {
...
GL_APICALL void  GL_APIENTRY glAttachShader(GLuint program, GLuint shader){
    ...
    if(ctx->shareGroup().get()) {
        const GLuint globalProgramName = ctx->shareGroup()->getGlobalName(NamedObjectType::PROGRAM,program);
        SET_ERROR_IF(globalProgramName==0, GL_INVALID_VALUE);
        const GLuint globalShaderName  = ctx->shareGroup()->getGlobalName(NamedObjectType::SHADER,shader);
        SET_ERROR_IF(globalShaderName==0, GL_INVALID_VALUE);

        ObjectDataPtr programData = ctx->shareGroup()->getObjectData(NamedObjectType::PROGRAM,program);
        ObjectDataPtr shaderData = ctx->shareGroup()->getObjectData(NamedObjectType::SHADER,shader);
...
GL_APICALL void  GL_APIENTRY glBindAttribLocation(GLuint program, GLuint index, const GLchar* name){
    ...
    if(ctx->shareGroup().get()) {
        const GLuint globalProgramName = ctx->shareGroup()->getGlobalName(NamedObjectType::PROGRAM,program);
        SET_ERROR_IF(globalProgramName==0, GL_INVALID_VALUE);
        ObjectDataPtr objData = ctx->shareGroup()->getObjectData(NamedObjectType::PROGRAM,program);
...
GL_APICALL void  GL_APIENTRY glBindBuffer(GLenum target, GLuint buffer){
    ...
    if(buffer && ctx->shareGroup().get() && !ctx->shareGroup()->isObject(NamedObjectType::VERTEXBUFFER,buffer)){
        ctx->shareGroup()->genName(NamedObjectType::VERTEXBUFFER, buffer);
        ctx->shareGroup()->setObjectData(NamedObjectType::VERTEXBUFFER,buffer,ObjectDataPtr(new GLESbuffer()));
    }
    ctx->bindBuffer(target,buffer);
    if (buffer) {
        GLESbuffer* vbo = (GLESbuffer*)ctx->shareGroup()->getObjectData(NamedObjectType::VERTEXBUFFER,buffer).get();
        vbo->setBinded();
    }
}
...
GL_APICALL void  GL_APIENTRY glBindFramebuffer(GLenum target, GLuint framebuffer){
    ...
    if(framebuffer && ctx->shareGroup().get()){
        globalFrameBufferName = ctx->shareGroup()->getGlobalName(NamedObjectType::FRAMEBUFFER,framebuffer);
        ...
        if(!globalFrameBufferName){
            ...
            ctx->shareGroup()->setObjectData(NamedObjectType::FRAMEBUFFER, framebuffer,
...
GL_APICALL void  GL_APIENTRY glBindRenderbuffer(GLenum target, GLuint renderbuffer){
    ...
    if(renderbuffer && ctx->shareGroup().get()){
        globalRenderBufferName = ctx->shareGroup()->getGlobalName(NamedObjectType::RENDERBUFFER,renderbuffer);
...
GL_APICALL void  GL_APIENTRY glBindTexture(GLenum target, GLuint texture){
    ...
    if(ctx->shareGroup().get()){
        globalTextureName = ctx->shareGroup()->getGlobalName(NamedObjectType::TEXTURE,localTexName);
...
GL_APICALL void  GL_APIENTRY glCompileShader(GLuint shader){
    ...
    if(ctx->shareGroup().get()) {
        const GLuint globalShaderName = ctx->shareGroup()->getGlobalName(NamedObjectType::SHADER,shader);
        SET_ERROR_IF(globalShaderName==0, GL_INVALID_VALUE);
        ObjectDataPtr objData = ctx->shareGroup()->getObjectData(NamedObjectType::SHADER,shader);
...
GL_APICALL GLuint GL_APIENTRY glCreateProgram(void){
    ...
    if(ctx->shareGroup().get()) {
        ...
        const GLuint localProgramName = ctx->shareGroup()->genName(NamedObjectType::PROGRAM, 0, true);
        ctx->shareGroup()->setObjectData(NamedObjectType::PROGRAM,localProgramName,ObjectDataPtr(programInfo));
...
GL_APICALL GLuint GL_APIENTRY glCreateShader(GLenum type){
    ...
    if(ctx->shareGroup().get()) {
        ...
        ctx->shareGroup()->setObjectData(NamedObjectType::SHADER,localShaderName,ObjectDataPtr(sp));
...
GL_APICALL void  GL_APIENTRY glDeleteBuffers(GLsizei n, const GLuint* buffers){
    ...
    if(ctx->shareGroup().get()) {
        for(int i=0; i < n; i++){
            ctx->shareGroup()->deleteName(NamedObjectType::VERTEXBUFFER,buffers[i]);
...
GL_APICALL void  GL_APIENTRY glDeleteFramebuffers(GLsizei n, const GLuint* framebuffers){
    ...
    if (ctx->shareGroup().get()) {
        for (int i = 0; i < n; i++) {
           ctx->shareGroup()->deleteName(NamedObjectType::FRAMEBUFFER, framebuffers[i]);
        }
    }
}

static void s_detachFromFramebuffer(NamedObjectType bufferType, GLuint texture) {
    GET_CTX();
    GLuint fbName = ctx->getFramebufferBinding();
    if (!fbName) return;
    ObjectDataPtr fbObj = ctx->shareGroup()->getObjectData(NamedObjectType::FRAMEBUFFER,fbName);
    ...
    for (size_t i = 0; i < sizen; ++i ) {
        ...
        if (NamedObjectType::TEXTURE == bufferType && GLESv2Validate::textureTargetEx(target)) {
            glFramebufferTexture2D(GL_FRAMEBUFFER, kAttachments[i], target, 0, 0);
        } else if (NamedObjectType::RENDERBUFFER == bufferType && GLESv2Validate::renderbufferTarget(target)) {
...
GL_APICALL void  GL_APIENTRY glDeleteRenderbuffers(GLsizei n, const GLuint* renderbuffers){
    ...
    if(ctx->shareGroup().get()) {
        for(int i=0; i < n; i++){
           ctx->shareGroup()->deleteName(NamedObjectType::RENDERBUFFER,renderbuffers[i]);
           s_detachFromFramebuffer(NamedObjectType::RENDERBUFFER, renderbuffers[i]);
        }
    }
}
        ...
        for(int i=0; i < n; i++){
            if (textures[i]!=0) {
                ...
                if (!tData || tData->sourceEGLImage == 0) {
                    const GLuint globalTextureName = ctx->shareGroup()->getGlobalName(NamedObjectType::TEXTURE,textures[i]);
                    ctx->shareGroup()->decTexRefCounterAndReleaseIf0(globalTextureName);
                }
                ctx->shareGroup()->deleteName(NamedObjectType::TEXTURE,textures[i]);
...
GL_APICALL void  GL_APIENTRY glFramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer){
    ...
    if(renderbuffer) {
        if (!ctx->shareGroup()->isObject(NamedObjectType::RENDERBUFFER,renderbuffer)) {
            ctx->shareGroup()->genName(NamedObjectType::RENDERBUFFER, renderbuffer);
            ...
            obj = ctx->shareGroup()->getObjectData(NamedObjectType::RENDERBUFFER, renderbuffer);
        }
...
GL_APICALL void  GL_APIENTRY glFramebufferTexture2D(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level){
    ...
    if(texture) {
        ...
        globalTextureName = ctx->shareGroup()->getGlobalName(NamedObjectType::TEXTURE,texname);
    }
...
GL_APICALL void  GL_APIENTRY glGenBuffers(GLsizei n, GLuint* buffers){
    ...
    if(ctx->shareGroup().get()) {
        for(int i=0; i<n ;i++) {
            buffers[i] = ctx->shareGroup()->genName(NamedObjectType::VERTEXBUFFER, 0, true);
            //generating vbo object related to this buffer name
            ctx->shareGroup()->setObjectData(NamedObjectType::VERTEXBUFFER,buffers[i],ObjectDataPtr(new GLESbuffer()));
        }
    }
}
...
GL_APICALL void  GL_APIENTRY glGenFramebuffers(GLsizei n, GLuint* framebuffers){
    ...
    if(ctx->shareGroup().get()) {
        for(int i=0; i<n ;i++) {
            framebuffers[i] = ctx->shareGroup()->genName(NamedObjectType::FRAMEBUFFER, 0 , true);
            ctx->shareGroup()->setObjectData(NamedObjectType::FRAMEBUFFER, framebuffers[i],
...
GL_APICALL void  GL_APIENTRY glGenRenderbuffers(GLsizei n, GLuint* renderbuffers){
    ...
    if(ctx->shareGroup().get()) {
        for(int i=0; i<n ;i++) {
            renderbuffers[i] = ctx->shareGroup()->genName(NamedObjectType::RENDERBUFFER, 0, true);
            ctx->shareGroup()->setObjectData(NamedObjectType::RENDERBUFFER,
                                         renderbuffers[i],
                                         ObjectDataPtr(new RenderbufferData()));
        }
...
GL_APICALL void  GL_APIENTRY glGenTextures(GLsizei n, GLuint* textures){
    ...
    if(ctx->shareGroup().get()) {
        for(int i=0; i<n ;i++) {
            textures[i] = ctx->shareGroup()->genName(NamedObjectType::TEXTURE, 0, true);
        }
    }
}
...
GL_APICALL void  GL_APIENTRY glGetAttachedShaders(GLuint program, GLsizei maxcount, GLsizei* count, GLuint* shaders){
    ...
    if(ctx->shareGroup().get()) {
        ...
        for(int i=0 ; i < maxcount && i<numShaders ;i++){
           shaders[i] = ctx->shareGroup()->getLocalName(NamedObjectType::SHADER,shaders[i]);
        }
    }
}
...
GL_APICALL void  GL_APIENTRY glGetIntegerv(GLenum pname, GLint* params){
    ...
    case GL_CURRENT_PROGRAM:
        if (ctx->shareGroup().get()) {
            ...
            *params = ctx->shareGroup()->getLocalName(NamedObjectType::PROGRAM,i);
        }
        break;
    case GL_FRAMEBUFFER_BINDING:
        if (ctx->shareGroup().get()) {
            ctx->dispatcher().glGetIntegerv(pname,&i);
            *params = ctx->shareGroup()->getLocalName(NamedObjectType::FRAMEBUFFER,i);
        }
        break;
    case GL_RENDERBUFFER_BINDING:
        if (ctx->shareGroup().get()) {
            ctx->dispatcher().glGetIntegerv(pname,&i);
            *params = ctx->shareGroup()->getLocalName(NamedObjectType::RENDERBUFFER,i);
        }
        break;
...
GL_APICALL void  GL_APIENTRY glGetRenderbufferParameteriv(GLenum target, GLenum pname, GLint* params){
    ...
    if (rb) {
        ObjectDataPtr objData = ctx->shareGroup()->getObjectData(NamedObjectType::RENDERBUFFER,rb);
...
GL_APICALL GLboolean    GL_APIENTRY glIsBuffer(GLuint buffer){
    ...
    if(buffer && ctx->shareGroup().get()) {
       ObjectDataPtr objData = ctx->shareGroup()->getObjectData(NamedObjectType::VERTEXBUFFER,buffer);
...
GL_APICALL GLboolean    GL_APIENTRY glIsFramebuffer(GLuint framebuffer){
    GET_CTX_RET(GL_FALSE)
    if(framebuffer && ctx->shareGroup().get()){
        return (ctx->shareGroup()->isObject(NamedObjectType::FRAMEBUFFER,framebuffer) &&
...
GL_APICALL GLboolean    GL_APIENTRY glIsRenderbuffer(GLuint renderbuffer){
    GET_CTX_RET(GL_FALSE)
    if(renderbuffer && ctx->shareGroup().get()){
        return (ctx->shareGroup()->isObject(NamedObjectType::RENDERBUFFER,renderbuffer) &&
...
GL_APICALL void  GL_APIENTRY glLinkProgram(GLuint program){
    ...
    if(ctx->shareGroup().get()) {
        ...
        if (vertexShader != 0 && fragmentShader!=0) {
            ...
            GLuint fragmentShaderGlobal = ctx->shareGroup()->getGlobalName(NamedObjectType::SHADER,fragmentShader);
            GLuint vertexShaderGlobal = ctx->shareGroup()->getGlobalName(NamedObjectType::SHADER,vertexShader);
...
GL_APICALL void  GL_APIENTRY glShaderBinary(GLsizei n, const GLuint* shaders, GLenum binaryformat, const GLvoid* binary, GLsizei length){
    ...
    if(ctx->shareGroup().get()){
        for(int i=0; i < n ; i++){
            const GLuint globalShaderName = ctx->shareGroup()->getGlobalName(NamedObjectType::SHADER,shaders[i]);
            SET_ERROR_IF(globalShaderName == 0,GL_INVALID_VALUE);
    ...
    if (ctx->shareGroup().get() && level == 0){
        ...
        if(texData) {
            ...
            if (texData->sourceEGLImage != 0) {
                ...
                unsigned int tex = ctx->getBindedTexture(target);
                ctx->shareGroup()->genName(NamedObjectType::TEXTURE, tex, false);
                unsigned int globalTextureName =
                    ctx->shareGroup()->getGlobalName(NamedObjectType::TEXTURE, tex);
                ctx->dispatcher().glBindTexture(GL_TEXTURE_2D,
                                                globalTextureName);
...
static void s_unUseCurrentProgram() {
    ...
    ObjectDataPtr objData = ctx->shareGroup()->getObjectData(NamedObjectType::PROGRAM,localCurrentProgram);
    SET_ERROR_IF(objData.get()->getDataType()!=PROGRAM_DATA,GL_INVALID_OPERATION);
    ProgramData* programData = (ProgramData*)objData.get();
    programData->setInUse(false);
    if (programData->getDeleteStatus()) {
        ctx->shareGroup()->deleteName(NamedObjectType::PROGRAM,localCurrentProgram);
    }
}
...
GL_APICALL void GL_APIENTRY glEGLImageTargetTexture2DOES(GLenum target, GLeglImageOES image)
{
    ...
    if (img) {
        ...
        if (ctx->shareGroup().get()) {
            ...
            unsigned int oldGlobal = ctx->shareGroup()->getGlobalName(NamedObjectType::TEXTURE, tex);
            ...
            ctx->shareGroup()->replaceGlobalName(NamedObjectType::TEXTURE, tex,img->globalTexName);
