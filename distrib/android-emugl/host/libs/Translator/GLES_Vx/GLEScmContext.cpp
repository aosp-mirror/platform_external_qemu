...
void GLEScmContext::init() {
  ...
  if(!m_initialized) {
    GLESv2Context::init(); // will init issueFuncs GLES_2_0, set m_initialized
    // TODO: load issue-fucntions to libGL for extensios not implemented in GLES2
    //
    // s_glIssue.issueFuncs(GLES_1_to_2); // load only extensions
    //
    // s_glIssue.issueFuncs(GLES_1_1);    // load the whole GLES_1_1 interface
