...
static void initGLESx() {
  DBG("Init GLESv1 by loading routing table to GLES_V2\n");
...
GL_API void GL_APIENTRY  glDisable(GLenum cap) {
  ...
  if (cap==GL_TEXTURE_GEN_STR_OES) {
    es1xDisable(ctx->getES1xContext(), GL_TEXTURE_GEN_S);
    es1xDisable(ctx->getES1xContext(), GL_TEXTURE_GEN_T);
    es1xDisable(ctx->getES1xContext(), GL_TEXTURE_GEN_R);
  }
  else
...
GL_API void GL_APIENTRY  glEnable(GLenum cap) {
  ...
  DBG("v1: %s ctx=%p ---> ", __func__, ctx);
  if (cap==GL_TEXTURE_GEN_STR_OES) {
    es1xEnable(ctx->getES1xContext(), GL_TEXTURE_GEN_S);
    es1xEnable(ctx->getES1xContext(), GL_TEXTURE_GEN_T);
    es1xEnable(ctx->getES1xContext(), GL_TEXTURE_GEN_R);
  }
  else
  ...
  if (ctx->shareGroup().Ptr()){
    ...
    if(texData) {
      ...
      if (texData->sourceEGLImage != 0) {
        ...
        es1xBindTexture(ctx->getES1xContext(), GL_TEXTURE_2D, texData->oldGlobal);
...
GL_API void GL_APIENTRY  glTexParameterxv(GLenum target, GLenum pname, const GLfixed *params) {
  ...
  if (pname==GL_TEXTURE_CROP_RECT_OES) {
    ...
    es1xTexParameterfv(ctx->getES1xContext(), target, pname, &param);
  }
...
GL_API void GL_APIENTRY glGetTexGenxvOES (GLenum coord, GLenum pname, GLfixed *params) {
  ...
  // __kostas__ : is this a bug ? tmpParams length is 1
...
void glDrawTexOES (T x, T y, T z, T width, T height) {
  ...
  DBG("v1: %s ctx=%p ______________ glDrawTexOES @ ______________ \n", __func__, ctx);
  ...
  for (int i=0;i<ctx->getMaxTexUnits();++i) {
    if (ctx->isTextureUnitEnabled(GL_TEXTURE0+i)) {
      ...
      if (objData.Ptr()) {
        ...
        printf("____________________ texels [%f %f %f %f %f %f %f %f ]\n",
               (float)(texData->crop_rect[0]), (float)(texData->crop_rect[1]),
               (float)(texData->crop_rect[2]), (float)(texData->crop_rect[3]),
               (float)(texData->crop_rect[4]), (float)(texData->crop_rect[5]),
               (float)(texData->crop_rect[6]), (float)(texData->crop_rect[7]));
        fflush(stdout);
