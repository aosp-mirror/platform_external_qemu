...
public:
    explicit SyncThread(EGLContext cxt = EGL_NO_CONTEXT);
    int waitWithFenceFd(void* glsync);
    void triggerWait(RcSync* glsync, uint64_t timeline);
    void cleanup();
private:
    ...
    android::base::MessageChannel<SyncThreadCmd, 512U> mInput;
    // holds result of cmds
    // (This will either be 0 or
    // the result of executing glClientWaitSync)
    android::base::MessageChannel<GLint, 512U> mOutput;
