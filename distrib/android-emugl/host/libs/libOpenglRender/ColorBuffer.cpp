...
void ColorBuffer::subUpdate(int x,
    ...
    if (conversionFbo) { // convert |pixels| from YUV format.
        ...
        if (!conversion) {
            ...
            s_gles2.glGenTextures(1, &convertSrcTex);
            s_gles2.glActiveTexture(GL_TEXTURE0);
            s_gles2.glBindTexture(GL_TEXTURE_2D, convertSrcTex);
            s_gles2.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
            s_gles2.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            ...
            const char kFragmentShaderSource[] =
                "varying highp vec2 outCoord;\n"
                "uniform sampler2D yv12_sampler;\n"
                "void main(void) {\n"
                "  gl_FragColor = texture2D(yv12_sampler, outCoord);\n"
                "}\n";
        ...
        CHECK_GL(s_gles2.glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, width, height, p_format, p_type, pixels));
        // // use the texture sampler
        GLuint samplerLoc = s_gles2.glGetUniformLocation(program, "yv12_sampler");
        CHECK_GL(s_gles2.glUniform1i(samplerLoc, 0));
