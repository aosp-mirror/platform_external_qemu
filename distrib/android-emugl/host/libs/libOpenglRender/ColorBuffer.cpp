...
ColorBuffer* ColorBuffer::create(EGLDisplay p_display,
    ...
    ColorBuffer *cb = new ColorBuffer(p_display);
...
ColorBuffer::ColorBuffer(EGLDisplay display) :
        m_tex(0),
        m_blitTex(0),
        m_eglImage(NULL),
        m_blitEGLImage(NULL),
...
bool ColorBuffer::draw(TextureDraw* textureDraw) {
    if (!bindFbo(&m_fbo, m_tex)) {
        return false;
    }
...
bool ColorBuffer::post(TextureDraw* textureDraw, float rotation) {
    textureDraw->draw(m_tex, rotation);
    return true;
}
