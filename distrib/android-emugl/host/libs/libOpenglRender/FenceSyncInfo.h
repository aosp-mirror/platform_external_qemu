...
// FenceSync object corresponding to the input handle,
// and check whether it is already signaled by checking
// for its presence in the map.
//
// If it is already signaled, the object is not present in the map,
// and we can save the cost of calling out to the host OpenGL driver.
// Otherwise, we will call glClientWaitSync.
class FenceSyncInfo {
public:
