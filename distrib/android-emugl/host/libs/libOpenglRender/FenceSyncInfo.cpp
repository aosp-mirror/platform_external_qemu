...
void FenceSyncInfo::addSync(FenceSync* fencesync) {
    ...
    if (!fencesync) {
        ...
        info.signaled = true;
        info.fence_sync.reset();
    } else {
        SyncHandledInfo& info = mSyncMap[fencesync->getHandle()];
        info.signaled = false;
        info.fence_sync.reset(fencesync);
    }
}
...
FenceSync* FenceSyncInfo::findNonSignaledSync(uint64_t handle) {
    ...
    return it->second.fence_sync.get();
}

::android::base::LazyInstance<FenceSyncInfo> sFenceSyncInfo = LAZY_INSTANCE_INIT;
