...
public:
    ...
    // |textureDraw| is used to perform the blit.
    bool draw(TextureDraw* textureDraw);

    // Post this ColorBuffer to the host native sub-window.
    // |textureDraw| is used to perform the blit.
    // |rotation| is the rotation angle in degrees, clockwise in the GL
    // coordinate space.
    bool post(TextureDraw* textureDraw, float rotation);
    ...
    // to later call post() to display the content.
...
private:
    ...
    explicit ColorBuffer(EGLDisplay display);
