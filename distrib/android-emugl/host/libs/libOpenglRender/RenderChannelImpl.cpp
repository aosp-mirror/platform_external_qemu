...
RenderChannelImpl::RenderChannelImpl()
    : mLock(), mFromGuest(1024, mLock), mToGuest(16, mLock) {
    updateStateLocked();
}
...
void RenderChannelImpl::stop() {
    D("enter");
    AutoLock lock(mLock);
    // Only close the guest->host queue, to avoid losing in-transit
    // packets that the host might want to read.
    mFromGuest.closeLocked();

    // On the other hand, there is no point in keeping the packets
    // buffered from host to guest now.
    mToGuest.closeAndClearLocked();
}
...
IoResult RenderChannelImpl::readFromGuest(Buffer* buffer, bool blocking) {
    ...
    if (blocking) {
        ...
        updateStateLocked();
        DD("mFromGuest.popLocked() return %d, buffer size %d, state %d",
           (int)result, (int)buffer->size(), (int)mState);
    } else {
        result = mFromGuest.tryPopLocked(buffer);
        updateStateLocked();
        DD("mFromGuest.tryPopLocked() return %d, buffer size %d, state %d",
           (int)result, (int)buffer->size(), (int)mState);
    }
    notifyStateChangeLocked();
    return result;
...
void RenderChannelImpl::stopFromHost() {
    ...
    AutoLock lock(mLock);
    mFromGuest.closeAndClearLocked();
