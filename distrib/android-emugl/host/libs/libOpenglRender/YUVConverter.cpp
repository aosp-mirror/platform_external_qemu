...
static void doYUVConversionDraw(GLuint program,
    ...
    GLuint posLoc = s_gles2.glGetAttribLocation(program, "position");
    GLuint coordLoc = s_gles2.glGetAttribLocation(program, "inCoord");
...
YUVConverter::YUVConverter(int width, int height) {
    ...
    createYUVGLFullscreenQuad(&mVbuf, &mIbuf);
}
