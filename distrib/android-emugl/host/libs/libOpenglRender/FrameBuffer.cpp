...
void FrameBuffer::finalize(){
    if(s_theFrameBuffer){
        ScopedBind bind(s_theFrameBuffer);
                                            ...
                                            bool p_isGL2)
{
    ...
    RenderContextPtr rctx(RenderContext::create(
        getDisplay(), config->getEglConfig(), sharedContext, p_isGL2));
    if (rctx.Ptr() != NULL) {
        ret = genHandle();
        m_contexts[ret] = rctx;
...
void FrameBuffer::closeColorBuffer(HandleType p_colorbuffer)
{
    ...
    if (--(*c).second.refcount == 0) {
        ScopedBind bind(this);
        // Don't check bind.isValid() here since we need to delete the
        // ColorBuffer anyway.
...
bool FrameBuffer::flushWindowSurfaceColorBuffer(HandleType p_surface)
{
    ...
    ScopedBind bind(this);
    if (bind.isValid()) {
        surface->getColorBuffer()->draw(m_textureDraw);
    }
    return true;
}
...
bool FrameBuffer::bindColorBufferToTexture(HandleType p_colorbuffer)
{
    ...
    // NOTE: Do not bind the framebuffer here!
...
bool FrameBuffer::post(HandleType p_colorbuffer, bool needLock)
{
    ...
    ret = (*c).second.cb->post(m_textureDraw, m_zRot);
    if (ret) {
