...
uint32_t curr_ms() {
    ...
    return tv.tv_usec * 1000 + tv.tv_sec;
}
...
#define DPRINT(fmt, ...) do { \
    ...
    VERBOSE_TID_FUNCTION_DPRINT(syncthreads, "@ time=%lu: " fmt, curr_ms(), ##__VA_ARGS__); \
} while(0)
#else
#define DPRINT(...)
#endif
...
// - waitWithFenceFd: async wait on the given GL sync object. It returns
// a fence FD that is signaled when that GL sync object is signaled.
// - cleanup: for use with destructors and other cleanup functions. it destroys
// the sync context and exits the sync thread.

static const uint32_t kTimelineInterval = 1;
static const uint64_t kDefaultTimeoutNsecs = 5ULL * 1000ULL * 1000ULL * 1000ULL;

SyncThread::SyncThread(EGLContext cxt) :
    mParentContext(cxt),
    mSyncContext(EGL_NO_CONTEXT) {

    DPRINT("ctor");

    if (mParentContext == EGL_NO_CONTEXT) {
        fprintf(stderr, "error: cannot create a sync context "
                        "with parent context == EGL_NO_CONTEXT\n");
        return;
    }
...
int SyncThread::waitWithFenceFd(void* glsync) {
    return 0;
    // triggerWait(glsync, timelineHandle());
    // android::base::AutoLock(mTimelineInfo.lock);
    // DPRINT("wait on timeline at %" PRIu32 " (curr %" PRIu32 ")",
    //        nextTimelinePt(),
    //        currTimelinePt());
    // return emugl_sync_create_fence(
    //         timelineHandle(),
    //         0);
}
...
void SyncThread::triggerWait(RcSync* glsync,
                             uint64_t timeline) {
    DPRINT("glsync=0x%llx timeline=0x%lx ...",
            glsync, timeline);
    SyncThreadCmd to_send;
    to_send.opCode = SYNC_THREAD_WAIT;
    to_send.glsync = glsync;
...
void SyncThread::sendAsync(SyncThreadCmd& cmd) {
    DPRINT("send with opcode=%u glsync=0x%llx", cmd.opCode, cmd.glsync);
    cmd.needReply = false;
    mInput.send(cmd);
}
...
void SyncThread::doSyncWait(SyncThreadCmd* cmd) {
    DPRINT("enter");
    DPRINT("wait on sync obj: 0x%llx", sync_obj);

    GLint wait_result = 0x0;
    if (cmd->glsync) {
        wait_result = s_gles2.glClientWaitSync
            (cmd->glsync->sync,
             GL_SYNC_FLUSH_COMMANDS_BIT,
             kDefaultTimeoutNsecs);
    } else {
        fprintf(stderr, "WARNING: syncthread waiting on already signaled!\n");
    }
    ...
    if (wait_result != GL_CONDITION_SATISFIED &&
        ...
        fprintf(stderr, "error: glClientWaitSync abnormal exit 0x%x",
                        wait_result);
    }
...
uint64_t SyncThread::timelineHandle() {
    if (mTimelineInfo.timeline_handle) return mTimelineInfo.timeline_handle;
...
SyncThread* get_sync_thread() {
    DPRINT("call.");
    RenderThreadInfo* tInfo = RenderThreadInfo::get();
    DPRINT("info=%p", tInfo);

    if (tInfo->syncThread) return tInfo->syncThread;

    EGLContext currentContext = s_egl.eglGetCurrentContext();

    SyncThread* res = new SyncThread(currentContext);
    tInfo->syncThread = res;
