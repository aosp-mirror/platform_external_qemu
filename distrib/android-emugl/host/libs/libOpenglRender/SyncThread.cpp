...
#define DPRINT(fmt, ...) do { \
    ...
    VERBOSE_TID_FUNCTION_DPRINT(syncthreads, "@ time=%lu: " fmt, ##__VA_ARGS__); \
} while(0)
#else
#define DPRINT(...)
#endif
...
void SyncThread::doSyncWait(SyncThreadCmd* cmd) {
    DPRINT("enter");
    // android::base::AutoLock(mTimelineInfo.lock);
    void* sync_obj = cmd->glsync;
    DPRINT("wait on sync obj: %p", sync_obj);
    ...
    emugl_sync_timeline_inc(cmd->timeline,
                            kTimelineInterval);
    DPRINT("exit");
}
