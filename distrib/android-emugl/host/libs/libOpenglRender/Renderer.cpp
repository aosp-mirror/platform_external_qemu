...
bool Renderer::initialize(int width, int height, bool useSubWindow) {
    if (!mRenderLib || mRenderWindow) {
        return false;
    }
...
void Renderer::setPostCallback(IRenderer::OnPostCallback onPost,
                               void* context) {
    assert(mRenderWindow);
    mRenderWindow->setPostCallback(onPost, context);
}

bool Renderer::showOpenGLSubwindow(FBNativeWindowType window,
                                   int wx,
                                   int wy,
                                   int ww,
                                   int wh,
...
void Renderer::setOpenGLDisplayRotation(float zRot) {
    assert(mRenderWindow);
    mRenderWindow->setRotation(zRot);
}

void Renderer::setOpenGLDisplayTranslation(float px, float py) {
    assert(mRenderWindow);
    mRenderWindow->setTranslation(px, py);
}

void Renderer::repaintOpenGLDisplay() {
    assert(mRenderWindow);
