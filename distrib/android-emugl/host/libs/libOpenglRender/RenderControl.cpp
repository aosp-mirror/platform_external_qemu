...
static EGLint rcClientWaitSyncKHR(uint64_t handle, EGLint flags, uint64_t timeout) {
    ...
    // Big TODO: Need to have two threads for every context, one to render,
    // the other to wait on fences
    if (!tInfo->currContext) {
        ...
        DPRINT("%s: context created, cxt@%p\n", __FUNCTION__, cxt);
        DPRINT("%s: cxt->eglcxt=0x%lx origcxt=0x%lx\n", cxt->getEGLContext(), orig_cxt->getEGLContext());
    } else {
    ...
    if (cxt->isGL2()) {
        ...
        gl_wait_res = s_gles1.glClientWaitSync(glsync, GL_SYNC_FLUSH_COMMANDS_BIT, timeout);
    }
    ...
    case GL_TIMEOUT_EXPIRED:
        ...
        break;
    case GL_WAIT_FAILED:
        DPRINT("%s: wait finished with GL_WAIT_FAILED. glGetError=0x%x\n", __FUNCTION__, s_gles2.glGetError());
        egl_res = EGL_CONDITION_SATISFIED_KHR;
        break;
    default:
        egl_res = EGL_CONDITION_SATISFIED_KHR;
