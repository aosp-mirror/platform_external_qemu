...
static uint64_t rcCreateSyncKHR(EGLenum type, EGLint* attribs, uint32_t num_attribs,
    ...
    bool use_android_native_fence_fd = true;
    ...
    if (RenderThreadInfo::get()->currContext->isGL2()) {
        ...
        if (use_android_native_fence_fd) {
            int fencefd = get_sync_thread()->waitWithFenceFd(sync);
            SYNC_DPRINT("created fence fd %d", fencefd);
            if (fence_fd_out) *fence_fd_out = fencefd;
            SYNC_DPRINT("set fence fd output");
        }
