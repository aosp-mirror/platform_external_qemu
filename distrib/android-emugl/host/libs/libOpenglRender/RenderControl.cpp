...
static void rcTriggerWait(uint64_t eglsync_ptr,
    ...
    RcSync* rcsync = sync_info->findNonSignaledSync(eglsync_ptr);
    EGLSYNC_DPRINT("eglsync=0x%llx rcsync=0x%llx thread_ptr=0x%llx timeline=0x%llx",
                eglsync_ptr, rcsync, thread_ptr, timeline);
...
static EGLint rcClientWaitSyncKHR(uint64_t handle, EGLint flags, uint64_t timeout) {
    ...
    EGLSyncKHR eglsync = rcsync->sync;
