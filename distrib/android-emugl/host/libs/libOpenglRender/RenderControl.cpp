...
static void rcCreateSyncKHR(EGLenum type,
    ...
    FenceSync* fenceSync = new FenceSync(sync,
                                         (void*)RenderThreadInfo::get()->currContext.get());
...
static EGLint rcClientWaitSyncKHR(uint64_t handle, EGLint flags, uint64_t timeout) {
    ...
    EGLSyncKHR eglsync = fenceSync->mGLSync;
    RenderContext* orig_cxt = (RenderContext*)fenceSync->mContext;
