...
static void rcTriggerWait(uint64_t glsync_ptr,
                          uint64_t thread_ptr,
                          uint64_t timeline) {
    SYNC_DPRINT("glsync=0x%llx thread_ptr=0x%llx timeline=0x%llx",
                glsync_ptr, thread_ptr, timeline);
    void* glsync = (void*)(uintptr_t)glsync_ptr;
    SyncThread* syncThread = (SyncThread*)(uintptr_t)thread_ptr;
    syncThread->triggerWait(glsync, timeline);
}
...
static uint64_t rcCreateSyncKHR(EGLenum type, EGLint* attribs, uint32_t num_attribs, uint64_t* glsync_out, uint64_t* syncthread_out) {
    ...
    if (RenderThreadInfo::get()->currContext->isGL2()) {
        ...
        s_gles2.glFlush();
        if (glsync_out) *glsync_out = (uint64_t)(uintptr_t)(void*)(sync);
