...
static EGLint rcQueryEGLString(EGLenum name, void* buffer, EGLint bufferSize)
{
    ...
    if (asyncSwapEnabled &&
        eglStr.find("EGL_ANDROID_native_fence_sync") == std::string::npos) {
        eglStr += "EGL_ANDROID_native_fence_sync ";
    }
...
static uint32_t rcCreateColorBufferPid(uint32_t width, uint32_t height,
                                       GLenum internalFormat, uint64_t pid) {
    FrameBuffer *fb = FrameBuffer::getFB();
    if (!fb) {
        return 0;
    }

    return fb->createColorBufferPid(width, height, internalFormat, pid);
}
...
static int rcOpenColorBuffer2Pid(uint32_t colorbuffer, uint64_t pid)
{
    FrameBuffer *fb = FrameBuffer::getFB();
    if (!fb) {
        return -1;
    }
    return fb->openColorBufferPid(colorbuffer, pid);
}
...
static void rcCloseColorBufferPid(uint32_t colorbuffer, uint64_t pid) {
    FrameBuffer *fb = FrameBuffer::getFB();
    if (!fb) {
        return;
    }
    fb->closeColorBufferPid(colorbuffer, pid);
}
...
static void get_sync_error(EGLSyncKHR sync,
                           RenderContext* cxt,
                           FenceSync* fenceSync,
                           GLint* err) {
#if DEBUG
...
static void rcTriggerWait(uint64_t eglsync_ptr,
    ...
    EGLSYNC_DPRINT("eglsync=0x%llx fenceSync=0x%llx thread_ptr=0x%llx timeline=0x%llx",
...
static uint64_t rcCreateSyncKHR(EGLenum type,
    ...
    EGLSyncKHR sync = s_egl.eglCreateSyncKHR(fb->getDisplay(), EGL_SYNC_FENCE_KHR, NULL);
    FenceSync* fenceSync = new FenceSync(sync, (void*)RenderThreadInfo::get()->currContext.get());
    sync_info->addSync(fenceSync);

    GLint err;
    get_sync_error(sync, RenderThreadInfo::get()->currContext.get(), fenceSync, &err);
    ...
    if (eglsync_out) *eglsync_out = res;
...
static EGLint rcClientWaitSyncKHR(uint64_t handle, EGLint flags, uint64_t timeout) {
    ...
    EGLSyncKHR eglsync = fenceSync->sync;
    RenderContext* orig_cxt = (RenderContext*)fenceSync->gl_cxt;
    ...
    s_egl.eglClientWaitSyncKHR(fb->getDisplay(), eglsync, 0, timeout);
    return egl_wait_res;
}
...
void initRenderControlContext(renderControl_decoder_context_t *dec)
{
    ...
    dec->rcCreateColorBufferPid = rcCreateColorBufferPid;
    dec->rcCloseColorBufferPid = rcCloseColorBufferPid;
    dec->rcOpenColorBuffer2Pid = rcOpenColorBuffer2Pid;
