...
#if DEBUG_EGL_SYNC
#define EGLSYNC_DPRINT EGLSYNC_DPRINT
#else
...
static void check_sync_error(EGLSyncKHR sync,
                             RenderContext* cxt,
                             FenceSync* fenceSync,
                             GLint* err) {
#if DEBUG
...
static EGLint rcClientWaitSyncKHR(uint64_t handle,
    ...
    egl_wait_res = s_egl.eglClientWaitSyncKHR(fb->getDisplay(),
                                              eglsync,
                                              /* flags */ 0,
                                              timeout);
