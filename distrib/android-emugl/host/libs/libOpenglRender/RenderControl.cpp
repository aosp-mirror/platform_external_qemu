...
static EGLint rcClientWaitSyncKHR(uint64_t handle,
    ...
    if (!tInfo->currContext) {
        ...
        fb->createTrivialContext(0, // There is context to share.
                                 &gralloc_sync_cxt,
                                 &gralloc_sync_surf);
...
// "int" is for synchronization purposes; we want this command to complete
// before the guest proceeds any further.
static int rcDestroySyncKHR(uint64_t handle) {
    FrameBuffer *fb = FrameBuffer::getFB();
    FenceSyncInfo* sync_info = FenceSyncInfo::get();
    FenceSync* fenceSync = sync_info->findNonSignaledSync(handle);
    if (fenceSync) {
        s_egl.eglDestroySyncKHR(fb->getDisplay(),
                                (EGLSyncKHR)fenceSync->mGLSync);
        sync_info->setSignaled(handle);
    }
    return 0;
}
