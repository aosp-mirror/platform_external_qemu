...
public:
    ...
    HandleType createColorBuffer(
        int p_width, int p_height, GLenum p_internalFormat);
    HandleType createColorBufferPid(
        int p_width, int p_height, GLenum p_internalFormat, uint64_t pid);
    ...
    int openColorBufferPid(HandleType p_colorbuffer, uint64_t pid);
    ...
    void closeColorBufferPid(HandleType p_colorbuffer, uint64_t pid);

    void cleanupProcColorbuffers(uint64_t pid);
...
private:
    ...
    // We discussed PID reuse issue when designing this component, but we
    // considered it not to be an issue because as new processes fork in, PIDs
    // will increase to a system-dependent limit and then wrap around.
