...
public:
    ...
    static std::unique_ptr<RenderThread> create(
            std::weak_ptr<emugl::Renderer> renderer,
            std::shared_ptr<emugl::RenderingChannel> channel,
            emugl::Mutex* lock);
...
private:
    RenderThread(std::weak_ptr<emugl::Renderer> renderer,
                 std::shared_ptr<emugl::RenderingChannel> channel,
                 emugl::Mutex* mutex);
    ...
    emugl::Mutex* m_lock;
    std::shared_ptr<emugl::RenderingChannel> mChannel;
    std::weak_ptr<emugl::Renderer> mRenderer;
};
