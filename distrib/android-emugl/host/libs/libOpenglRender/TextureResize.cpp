...
void attachShaders(TextureResize::Framebuffer* fb, const char* factorDefine,
    ...
    {
        char dimensionConst[100];
        sprintf(dimensionConst, "const vec2 kDimension = vec2(%d, %d);\n", width, height);
        const char* source[5] = {
            factorDefine, dimensionDefine, kCommonShaderSource, dimensionConst, kVertexShaderSource
        };
        s_gles2.glShaderSource(vShader, 5, source, NULL);
    }
