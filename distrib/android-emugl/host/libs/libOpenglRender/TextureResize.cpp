...
const char kVertexShaderSource[] =
    ...
    "  vec2 uv = ((aPosition + 1.0) / 2.0) + 0.5 / uDimension;\n"
    "  vUV00 = uv;\n"
    "  #ifdef HORIZONTAL\n"
...
void attachShaders(TextureResize::Framebuffer* fb, const char* factorDefine, const char* dimensionDefine) {
    ...
    if (success == GL_FALSE) {
        GLchar messages[4096];
        s_gles2.glGetShaderInfoLog(fShader, sizeof(messages), 0, messages);
...
void TextureResize::setupFramebuffers(unsigned int factor) {
    ...
    s_gles2.glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RGBA, mWidth / factor, mHeight, 0, GL_RGBA, GL_FLOAT, NULL);
