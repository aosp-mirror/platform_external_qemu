...
# Note that the linux version is ((major << 16) | (minor << 8) | patch)
KERNEL_VERSION=$(awk '$2 == "LINUX_VERSION_CODE" { \
  CODE=$3; \
  MAJOR=int(CODE / 65536); \
  MINOR=int(CODE / 256) % 256; \
  PATCH=int(CODE % 256); \
  print MAJOR "." MINOR "." PATCH; }' include/linux/version.h)
