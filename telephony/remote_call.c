...
remote_number_string_to_port( const char*  number )
{
    ...
    if (len > 0 && number[len-1] == ';')
        len--;
    if (len == 11 && !strncmp(number, "1555521", 7))
        temp += 7;
    ...
    if (rcall != NULL) {
        ...
        switch (type) {
            case REMOTE_CALL_DIAL:
                p = bufprint(p, end, "gsm call 1555521%d\n", from_num );
                break;

            case REMOTE_CALL_BUSY:
                p = bufprint(p, end, "gsm busy 1555521%d\n", from_num);
                break;

            case REMOTE_CALL_HOLD:
                p = bufprint(p, end, "gsm hold 1555521%d\n", from_num);
                break;

            case REMOTE_CALL_ACCEPT:
                p = bufprint(p, end, "gsm accept 1555521%d\n", from_num);
                break;

            case REMOTE_CALL_HANGUP:
                p = bufprint(p, end, "gsm cancel 1555521%d\n", from_num );
                break;
