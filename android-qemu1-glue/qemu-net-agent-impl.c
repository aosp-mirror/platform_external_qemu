...
static int slirpRedir(bool isUdp, int hostPort,
                      uint32_t guestAddr, int guestPort) {
    return slirp_redir(isUdp, hostPort, guestAddr, guestPort);
}

int slirpUnredir(bool isUdp, int hostPort) {
    return slirp_unredir(isUdp, hostPort);
}
