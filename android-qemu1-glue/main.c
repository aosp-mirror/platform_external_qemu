...
#error "Unknown target architecture"
    ...
    int ret = handleEmulatorCommandLine(&argc, &argv, kTargetArch, false, opts, hw, &avd);
    if (ret <= 0) {
        // Program exit, -ret is the status code.
        return -ret;
    } else if (ret == +2) {
        // Positional QEMU option detected.
    ...
    if (!handleEmulatorUiConfiguration(opts, avd, hw)) {
      return 1;
    }
    ...
    pthread_sigmask(SIG_SETMASK, &set, NULL);
#endif
    if (!initEmulatorUi(opts, &uiEmuAgent)) {
      return 1;
    }
