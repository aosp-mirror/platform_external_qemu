...
static bool needCreateTmp(const char* id,
        android::base::FileShare shareMode) {
    return shareMode == android::base::FileShare::Read
            && strcmp("system", id)
            && strcmp("vendor", id)
...
static int drive_init(void* opaque, QemuOpts* opts,
    ...
    if (id) {
        ...
        fprintf(stderr, "id %s file new path %s\n", id, qemu_opt_get(opts, "file"));
    }
...
const char* kSnapshotName = "testCheckpoint";

static int drive_try_commit(void* opaque, QemuOpts* opts,
        Error** errp) {
    const char* id = opts->id;
    fprintf(stderr, "id %s file %s\n", id, qemu_opt_get(opts, "file"));
    assert(id);
    ...
    if (needCreateTmp(id, param->shareMode)) {
        ...
        int ret = bdrv_snapshot_goto(bs, kSnapshotName);
...
static int drive_reinit(void* opaque, QemuOpts* opts,
    ...
    BlockBackend* blk = blk_by_name(id);
    assert(blk);
    // The following needs to be run on its owner(?) thread
    // TODO: figure out which thread owns it
    ...
    if (needCreateTmp(id, param->shareMode)) {
        bdrv_flush(oldbs);
        int ret = bdrv_snapshot_goto(oldbs, kSnapshotName);
        assert(ret == 0);
    }
    bdrv_unref(oldbs);
    ...
    drive_opts = qemu_opts_create(&qemu_common_drive_opts, id, 1,
            nullptr);
    assert(drive_opts);
    assert(bs_opts);
    ...
    if (needCreateTmp(id, param->shareMode)) {
        ...
        pstrcpy(sn.name, sizeof(sn.name), kSnapshotName);
        ...
        int res = bdrv_snapshot_create(bs, &sn);
        assert(res == 0);
        if (res) {
            bdrv_unref(bs);
            return 1;
...
static void updateDriveShareMode(android::base::FileShare shareMode) {
    ...
    //if (qemu_opts_foreach(qemu_find_opts("drive"), drive_try_commit,
    //                      &param, NULL)) {
    //    assert(false);
    //}
