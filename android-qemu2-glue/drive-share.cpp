...
static int drive_init(void* opaque, QemuOpts* opts, Error** errp) {
    ...
    if (id) {
        ...
        if (needCreateTmp(id, param->shareMode) && param->snapshotName) {
            // handle the snapshot
            BlockBackend* blk = blk_new_open(sDriveShare->srcImagePaths[id].c_str(),
                    NULL, qdict_new(), BDRV_O_RDWR, nullptr);
            assert(blk);
            BlockDriverState* bs = blk_bs(blk);
            assert(bs);
            // bdrv_snapshot_goto can fail during first boot or wipe data
            int res = bdrv_snapshot_goto(bs, param->snapshotName, nullptr);
            if (res == 0) {
                // Create an empty snapshot in the qcow2-on-qcow2
                BlockBackend* blkNew = blk_new_open(path.c_str(),
                    NULL, qdict_new(), BDRV_O_RDWR, nullptr);
                assert(blkNew);
