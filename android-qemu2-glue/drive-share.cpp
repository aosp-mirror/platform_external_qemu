...
#include "android/android-emu/android/avd/util.h"
    ...
    for (p = 0; p < count; p++) {
        ...
        if (opts) {
            ...
            sDriveShare->srcImagePaths.emplace(images[p].drive,
                                               qcow2_image_path);
            printf("qcow2 path %s: %s\n", images[p].drive, qcow2_image_path);
        }
        ...
        if (android_hw->hw_arc) {
            ...
            if (!android::base::PathUtils::isAbsolute(qcow2_image_path)) {
                char* abs_path_buffer = path_join(avd_data_dir, qcow2_image_path);
                qcow2_path_buffer = abs_path_buffer;
                sDriveShare->srcImagePaths[images[p].drive] = qcow2_path_buffer;
                printf("updated path\n");
            }
            printf("should updated path\n");
        }
...
static void mirrorTmpCache(const char* dst, const char* src) {
    ...
    //qdict_set_default_str(bs_opts, "backing", android_hw->disk_cachePartition_path);
    BlockDriverState* bs = bdrv_open(dst, nullptr, bs_opts, BDRV_O_RDWR | BDRV_O_NO_BACKING, &local_err);
    if (!bs) {
        printf("drive %s open failure: %s\n", dst, error_get_pretty(local_err));
        error_setg(&local_err, "drive %s open failure", dst);
        return;
    }
    char* absPath = nullptr;
    char* backingFile = android_hw->disk_cachePartition_path;
    D("path %s exist %d\n", backingFile, path_exists(backingFile));
    //if (!path_exists(android_hw->disk_cachePartition_path)) {
    if (!android::base::PathUtils::isAbsolute(backingFile)) {
        absPath = path_join(avdInfo_getContentPath(android_avdInfo), backingFile);
        backingFile = absPath;
    }
    D("backing cache.img path: %s\n", backingFile);
    int res = bdrv_change_backing_file(bs,
            backingFile, NULL);
...
static int drive_reinit(void* opaque, QemuOpts* opts, Error** errp) {
    ...
    BlockDriverState* bs = bdrv_open(path.c_str(), nullptr, bs_opts, 0, &local_err);
    if (!bs) {
        printf("drive %s open failure: %s", path.c_str(), error_get_pretty(local_err));
