...
static int drive_reinit(void* opaque, QemuOpts* opts,
    ...
    if (needCreateTmp(id, param->shareMode)) {
        ...
        int res = bdrv_snapshot_goto(oldbs, snapshotName);
        if (res) {
            error_setg(errp, "bdrv_snapshot_goto failed %s <- %s",
                    id, snapshotName);
            return 1;
        }
    }
    ...
    // Please refer to drive_new() for what needs to be set.
...
static bool updateDriveShareMode(const char* snapshotName,
    ...
    return !qemu_opts_foreach(qemu_find_opts("drive"), drive_reinit,
                          &param, NULL);
}
