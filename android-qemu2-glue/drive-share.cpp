...
static bool needRemount(const char* id) {
    ...
    //return strcmp("system", id) && strcmp("vendor", id) &&
    //       strcmp("cache", id);  // Cache never really got mounted
}
...
static int drive_init(void* opaque, QemuOpts* opts, Error** errp) {
    ...
    if (id) {
        ...
        if (needCreateTmp(id, param->shareMode) && param->snapshotName) {
            if (strcmp(id, "cache")) {
                int res = 0;
                printf("baseNeedApplySnapshot %d\n", param->baseNeedApplySnapshot);
                if (param->baseNeedApplySnapshot) {
                    ...
                    if (!res) {
                        fprintf(stderr, "snapshot goto failed %s\n", id);
                    }
                    blk_flush(blk);
                    blk_unref(blk);
                }
                if (res == 0) {
                    ...
                    assert(blkNew);
                    bool success = createEmptySnapshot(blk_bs(blkNew), param->snapshotName);
                    if (!success) {
                        fprintf(stderr, "createEmptySnapshot failed %s\n", id);
                    }
                    blk_unref(blkNew);
                }
            } else {
...
static bool isBaseOnDifferentSnapshot(const char* snapshot_name) {
    ...
    if (!baseSnapshotNameFile) {
        printf("baseSnapshotFile open failure\n");
        return true;
    }
...
extern "C" int android_drive_share_init(bool wipe_data,
    ...
    //const char* semName = "blahblahblahblah";
    //sem_t* sem = sem_open(semName, O_CREAT, 0666, 1);
    //printf("sem open errno %d\n", errno);
    //assert(sem);
    //sem_wait(sem);
    //printf("locked sem\n");
    ...
    if (needApplySnapshot || !read_only) {
        ...
        baseSnapshotNameFile = fsopenWithTimeout(baseSnapshotFileName.c_str(),
            "w", android::base::FileShare::Write, 10000);
        printf("locking\n");
        if (!baseSnapshotNameFile) {
            printf("open snapshot file failed\n");
        }
        /*if (needApplySnapshot && !baseSnapshotNameFile) {
            // failed to acquire the lock, let's proceed without snapshot load
            param.baseNeedApplySnapshot = false;
            param.snapshotName = "";
        }*/
    }
    ...
    if (baseSnapshotNameFile) {
        if (needApplySnapshot) {
            // Don't put \n
            fprintf(baseSnapshotNameFile, "%s", snapshot_name);
        } else {
            fprintf(baseSnapshotNameFile, "\n");
        }
        android::base::Thread::sleepMs(100);
        printf("unlocking\n");
