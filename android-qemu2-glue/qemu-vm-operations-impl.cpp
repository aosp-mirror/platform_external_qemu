...
static bool import_snapshot(const char* name,
    ...
    for (auto qcow2 : android::snapshot::getQcow2Files(datadir)) {
        ...
        // There is a corner case if we are on an imported snapshot, we might not be able to delete dest.
        // path_delete_file returns 0 on success
        for (int i = 0; path_exists(dest.c_str()) && path_delete_file(dest.c_str()) != 0; i++) {
            dest = pj(avd, qcow2) + std::to_string(i) + kImportSuffix;
        }
