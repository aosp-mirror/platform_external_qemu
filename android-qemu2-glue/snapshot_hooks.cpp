...
public:
    ...
    void loadPage(void* ptr) {
        ...
        if (!page.loaded) {
            ...
            mprotect(pagePtr(page), pageSize(page), PROT_READ|PROT_WRITE|PROT_EXEC);
            iovec vec = { pagePtr(page), (size_t)pageSize(page) };
            ...
            munlock(pagePtr(page), pageSize(page));
            page.loaded = true;
        }
    }
...
private:
    ...
    Page& page(void* ptr) {
        auto it = std::lower_bound(
                    ...
                    [this](const Page& page, void* ptr) {
                        return pagePtr(page) + pageSize(page) < ptr;
                    });
    ...
    void readingWorker() {
        ...
        //        break;
        //    }
        //    bool readBlock = (offset & kNewBlockStart) != 0;
        //    if (readBlock) {
        //        uint8_t len = mStream->getByte();
        //        char blockName[256];
        //        mStream->read(blockName, len);
        //        blockName[len] = 0;
        //        mCurrentBlock = qemu_ram_block_by_name(blockName);
        //        if (!mCurrentBlock) {
        //            done();
        //            break;
        //        }
        //    }
        //    uint32_t size = mStream->getBe32();
        //    uint32_t pageSize = qemu_ram_pagesize(mCurrentBlock);
        //    uint64_t realOffset = (offset & ~kNewBlockStart) * pageSize;
        //    auto ptr = (uint8_t*)qemu_host_from_ram_block(mCurrentBlock) + realOffset;
        //    if (!size) {
        ...
        //    }
        //}
        //assert(!ferror(mStream->get()));
        //assert(!feof(mStream->get()));
        //mStream->getByte();
        //assert(feof(mStream->get()));
...
void RamLoader::setupProtection() {
    ...
    for (const Page& page : mIndex.pages) {
        ...
        if (ptr == startPtr + curSize) {
            ...
            if (startPtr) {
                mlock(startPtr, curSize);
                mprotect(startPtr, curSize, PROT_NONE);
            }
            startPtr = ptr;
            curSize = size;
        }
