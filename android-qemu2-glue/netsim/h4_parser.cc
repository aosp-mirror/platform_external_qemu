...
#include "h4_parser.h"  // for H4Parser, PacketType, H4Pars...
...
void H4Parser::OnPacketReady() {
  switch (hci_packet_type_) {
    ...
    case PacketType::ISO:
      ...
      break;
    default:
      dwarning("Unimplemented packet type %d",
...
bool H4Parser::Consume(const uint8_t* buffer, int32_t bytes_read) {
  ...
  if ((uint32_t)bytes_read > BytesRequested()) {
    dwarning("More bytes read (%u) than expected (%u)!",
  ...
  switch (state_) {
    ...
    case HCI_RECOVERY: {
      ...
      dwarning("Received byte in recovery state : 0x%x",
      ...
      if (packet_.size() == reset_command.size()) {
        dwarning("Received HCI Reset command, exiting recovery state");
    ...
    case HCI_TYPE:
      ...
      if (hci_packet_type_ != PacketType::ACL &&
        ...
        if (!enable_recovery_state_) {
          dwarning("Received invalid packet type 0x%x",
                   static_cast<unsigned>(packet_type_));
          std::abort();
        }
        derror("Received invalid packet type 0x%x, entering recovery state",
                  static_cast<unsigned>(packet_type_));
        state_ = HCI_RECOVERY;
        hci_packet_type_ = PacketType::COMMAND;
        bytes_wanted_ = 1;
