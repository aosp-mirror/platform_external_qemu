...
static int createUserData(AvdInfo* avd,
    ...
    if (path_exists(initDir.get())) {
        ...
        if (needCopyDataPartition) {
            const char* tempDataFolder =
                    PathUtils::join(System::get()->getTempDir(), "data")
                            .c_str();
            prepareDataFolder(tempDataFolder, initDir.get());
            D("Creating ext4 userdata.img from %s folder", tempDataFolder);
            needCopyDataPartition = !creatUserDataExt4Img(hw, tempDataFolder);
            path_delete_file(tempDataFolder);
        }
    }
