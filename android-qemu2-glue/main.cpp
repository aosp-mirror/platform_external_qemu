...
static int createUserData(AvdInfo* avd,
    ...
    if (path_exists(initDir.get())) {
        D("Creating ext4 userdata.img from %s folder", dataPath);
        ...
        if (needCopyDataPartition) {
            std::string tempDataFolder =
                    PathUtils::join(System::get()->getTempDir(), "data");
            prepareDataFolder(tempDataFolder.c_str(), initDir.get());
            D("Creating ext4 userdata.img from %s folder",
              tempDataFolder.c_str());
            needCopyDataPartition =
                    !creatUserDataExt4Img(hw, tempDataFolder.c_str());
            path_delete_file(tempDataFolder.c_str());
        }
    }
