...
// Dedicated IOThread for all disk IO
    ...
    // USB
    #define usb_passthrough_driver "Google WinUsb Assistant Driver"
    if (targetIsX86 && apiLevel >= 29) {
        // Android R Kernel interrupt bug on XHCI, use EHCI
        if (apiLevel == 30)
            args.add2("-device", "usb-ehci,id=ehci");
        else
            args.add2("-device", "qemu-xhci,id=xhci");
