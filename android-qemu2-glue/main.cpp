 ...
 */
class PartitionParameterCreator {
public:
    android::ParameterList createPartionParameters(AndroidHwConfig* hw,
                                                   ImageType type,
                                                   bool writable,
                                                   AvdInfo* avd) {
        ...
        switch (type) {
            ...
            case IMAGE_TYPE_SD_CARD:
                if (hw->hw_sdCard_path != NULL &&
                    strcmp(hw->hw_sdCard_path, "")) {
            ...
            case IMAGE_TYPE_ENCRYPTION_KEY:
                if (fc::isEnabled(fc::EncryptUserData) &&
                    hw->disk_encryptionKeyPartition_path != NULL &&
                    strcmp(hw->disk_encryptionKeyPartition_path, "")) {
...
extern "C" int main(int argc, char** argv) {
    ...
    if (!emulator_parseCommonCommandLineOptions(
        ...
        if (exitStatus == EMULATOR_EXIT_STATUS_POSITIONAL_QEMU_PARAMETER) {
            ...
            // This is needed due to the fact that emulator_parse...
            // has side effects that can modify argc/ argv.
...
// Dedicated IOThread for all disk IO
    ...
    PartitionParameterCreator ppc;
    for (auto type : kTarget.imagePartitionTypes) {
        bool writable = (type == IMAGE_TYPE_SYSTEM || type == IMAGE_TYPE_VENDOR)
                                ? android_op_writable_system
                                : true;
        args.add(ppc.createPartionParameters(hw, type, writable, avd));
    }
