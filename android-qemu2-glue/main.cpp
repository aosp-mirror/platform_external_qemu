...
static void prepareDataFolder(const char* destDirectory, const char* srcDirectory) {
...
static int createUserData(AvdInfo* avd,
    ...
    if (path_exists(initDir.get())) {
        D("Creating ext4 userdata.img from %s folder", dataPath.c_str());
        prepareDataFolder(dataPath.c_str(), initDir.get());
        needCopyDataPartition =  !creatUserDataExt4Img(hw, dataPath.c_str());
        // Creating data partition can fail because of insufficient disk space
        // or non-ascii path on Windows.
        // Give another try by having the /data folder in temp directory.

        if (needCopyDataPartition) {
            std::string tempDataFolder = PathUtils::join(System::get()->getTempDir(), "data");
            prepareDataFolder(tempDataFolder.c_str(), initDir.get());
            D("Creating ext4 userdata.img from %s folder", tempDataFolder.c_str());
            needCopyDataPartition = !creatUserDataExt4Img(hw, tempDataFolder.c_str());
        }
    }
