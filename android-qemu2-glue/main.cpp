...
bool handleCpuAccelerationForMinConfig(int argc, char** argv,
    ...
    *accel_mode = ACCEL_ON;
    ...
    if (hasEnableKvm) {
        printf("%s: Selecting KVM for CPU accelerating\n", __func__);
        androidCpuAcceleration_resetCpuAccelerator(
                ANDROID_CPU_ACCELERATOR_KVM);
    } else if (hasEnableHax) {
        printf("%s: Selecting HAXM for CPU accelerating\n", __func__);
        androidCpuAcceleration_resetCpuAccelerator(
                ANDROID_CPU_ACCELERATOR_HAX);
    } else if (hasEnableHvf) {
        printf("%s: Selecting HVF for CPU accelerating\n", __func__);
        androidCpuAcceleration_resetCpuAccelerator(
                ANDROID_CPU_ACCELERATOR_HVF);
    } else if (hasEnableWhpx) {
        printf("%s: Selecting WHPX for CPU accelerating\n", __func__);
        androidCpuAcceleration_resetCpuAccelerator(
                ANDROID_CPU_ACCELERATOR_WHPX);
    } else if (hasEnableGvm) {
        printf("%s: Selecting GVM for CPU accelerating\n", __func__);
        androidCpuAcceleration_resetCpuAccelerator(
