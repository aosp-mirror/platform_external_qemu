...
static std::string getAdbKeyPath(const char* adbKeyFileName) {
    ...
    if (path_is_regular(adbKeyPath.c_str()) && path_can_read(adbKeyPath.c_str())) {
        return adbKeyPath;
    }
    ...
    auto guessedSrcAdbKeyPub = PathUtils::join(home, ".android", adbKeyFileName);
    path_copy_file(adbKeyPath.c_str(),
...
static void prepareDataFolder(const char* destDirectory,
    ...
    if (adbKeyPubPath == "") {
        ...
        std::string pubKey;
        pubkey_from_privkey(&pubKey, adbKeyPrivPath);
