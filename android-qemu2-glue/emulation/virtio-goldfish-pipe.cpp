...
#define PIPE_BIND_QUERY_BUFFER         (1 << 18) /* get_query_result_resource */
    ...
    int resourceMap(uint32_t res_handle, void** hvaOut, uint64_t* sizeOut) {
        ...
        fprintf(stderr, "%s: got result for res %u hva %p\n", __func__, res_handle, alignedHva);
        if (hvaOut) *hvaOut = (void*)(uintptr_t)alignedHva;
        if (sizeOut) *sizeOut = alignedSize;

        fprintf(stderr, "%s: got result for res %u hva %p\n", __func__, res_handle, *hvaOut);
...
private:
    void allocResource(PipeResEntry& entry, iovec* iov, int num_iovs, uint64_t* addrs = nullptr) {
        ...
        if (addrs) {
            ...
            for (int i = 0; i < num_iovs; ++i) {
                ...
                fprintf(stderr, "%s: add addr: 0x%llx\n", __func__, (unsigned long long)addrs[i]);
            }
        }
    }
    ...
    void saveResource(QEMUFile* file, VirglResId resId, const PipeResEntry& pipeResEntry) {
        fprintf(stderr, "%s: save resource %u\n", __func__, resId);
    ...
    void saveContextResourceList(QEMUFile* file, VirglCtxId ctxId, const std::vector<VirglResId>& resourceIds) {
        fprintf(stderr, "%s: save context resource list for ctx %u\n", __func__, ctxId);
    ...
    void saveResourceContextList(QEMUFile* file, VirglResId resId, const std::vector<VirglCtxId>& contextIds) {
        fprintf(stderr, "%s: save resource context list for res %u\n", __func__, resId);
        qemu_put_be32(file, resId);
    ...
    void saveFenceDeque(QEMUFile* file, const std::deque<int>& deque) {
        ...
        for (auto val : mFenceDeque) {
            fprintf(stderr, "%s: save fence val %u\n", __func__, val);
    ...
    void loadResource(QEMUFile* file) {
        ...
        fprintf(stderr, "%s: load resource %u\n", __func__, resId);
        ...
        for (uint32_t i = 0; i < pipeResEntry.numIovs; ++i) {
            ...
            fprintf(stderr, "%s: cpu_physical_memory_map for gpa 0x%llx: %p\n", __func__,
                    (unsigned long long)pipeResEntry.addrs[i],
                    pipeResEntry.iov[i].iov_base);
        }
        ...
        fprintf(stderr, "%s: blob res id %u hva %p\n", __func__, resId, entry.hva);
    ...
    std::vector<VirglResId> loadContextResourceList(QEMUFile* file, uint32_t* ctxIdOut) {
        ...
        fprintf(stderr, "%s: load context resource list for ctx %u\n", __func__, ctxId);
        *ctxIdOut = ctxId;
    ...
    std::vector<VirglCtxId> loadResourceContextList(QEMUFile* file, uint32_t* resIdOut) {
        uint32_t resId = qemu_get_be32(file);
        fprintf(stderr, "%s: load resource context list for res %u\n", __func__, resId);
        *resIdOut = resId;
        size_t count = qemu_get_be32(file);
    ...
    void loadFenceDeque(QEMUFile* file) {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            fprintf(stderr, "%s: load fence val %u\n", __func__, mFenceDeque.back());
        }
    }
