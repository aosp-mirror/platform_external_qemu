...
static inline uint32_t virgl_format_to_gl(uint32_t virgl_format) {
    switch (virgl_format) {
        ...
        case VIRGL_FORMAT_R8G8B8A8_UNORM:
            return kGlRgba;
        case VIRGL_FORMAT_B5G6R5_UNORM:
            fprintf(stderr, "%s: warning: not expecting to create a color buffer here (GL_RGB565)\n", __func__);
            return kGlRgb565;
        case VIRGL_FORMAT_R8_UNORM:
            fprintf(stderr, "%s: warning: not expecting to create a color buffer here (GL_R8)\n", __func__);
            return kGlR8;
        case VIRGL_FORMAT_R8G8_UNORM:
            fprintf(stderr, "%s: warning: not expecting to create a color buffer here (GL_RG8)\n", __func__);
            return kGlRg8;
        case VIRGL_FORMAT_NV12:
        case VIRGL_FORMAT_YV12:
...
static inline uint32_t virgl_format_to_fwk_format(uint32_t virgl_format) {
    switch (virgl_format) {
        case VIRGL_FORMAT_NV12:
            fprintf(stderr, "%s: using NV12 converter\n", __func__);
            return kFwkFormatNV12;
            fprintf(stderr, "%s: using YV12 converter\n", __func__);
        case VIRGL_FORMAT_YV12:
