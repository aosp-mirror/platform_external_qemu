...
public:
    ...
    void writeWordsToFirstIovPageLocked(uint32_t* dwords, size_t dwordCount, uint32_t resId) {
        ...
        auto resEntryIt = mResources.find(resId);
        if (resEntryIt == mResources.end()) {
            fprintf(stderr, "%s: fatal: kVirtioGpuAddressSpaceHostmemAlloc: resp_resid %u not found\n", __func__, resId);
            abort();
        }
        ...
        if (!resEntry.iov) {
            fprintf(stderr, "%s: fatal: kVirtioGpuAddressSpaceHostmemAlloc: resp_resid %u had empty iov\n", __func__, resId);
            abort();
        }
