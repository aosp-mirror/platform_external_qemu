...
void VirtioWifiForwarder::onHostApd(void* opaque, int fd, unsigned events) {
    ...
    if (frame->isBeacon()) {
        forwarder->mBeaconFrame = frame.release();
...
void VirtioWifiForwarder::resetBeaconTask() {
    ...
    mBeaconTask.emplace(
            mLooper,
            [this]() {
                auto framePtr = mBeaconFrame;
                if (framePtr && framePtr->isBeacon()) {
                    auto beaconFrame = std::make_unique<Ieee80211Frame>(
                                        framePtr->data(), framePtr->size(),
                                        framePtr->info());
                    if (!beaconFrame->encrypt(mHostapd->getCipherScheme())) {
                        LOG(ERROR) << "Unable to encrypt the IEEE80211 frame with CCMP.";
                        return true;
                    }
                    sendToGuest(this, std::move(beaconFrame));
                }
                return true;
            },
            mBeaconIntMs);
