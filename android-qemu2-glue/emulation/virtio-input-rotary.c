...
static void virtio_input_rotary_add_wheel(VirtIOInput* vinput) {
    ...
    virtio_input_config test = {};
    memset(&test, 0, sizeof(virtio_input_config));
}
...
void virtio_send_rotary_event(int delta) {
    ...
    if (vinput) {
        virtio_input_event evt;
        evt.type = cpu_to_le16(EV_REL);
        evt.code = cpu_to_le16(REL_WHEEL);
        evt.value = cpu_to_le32(delta);
        virtio_input_send(vinput, &evt);
