...
struct VSockStream {
    ...
    bool load(android::base::Stream *stream) {
        ...
        if (!mPipe || force_close) {
            fprintf(stderr, "rkir555 VSockStream::%s:%d this=%p mPipe=%p force_close=%d return\n",
                    __func__, __LINE__, this, mPipe, force_close);
            return false;
        } else {
            vtblPtr = &vtblMain;
        }
...
struct VirtIOVSockDev {
    VirtIOVSockDev(VirtIOVSock *s) : mS(s) {
        fprintf(stderr, "rkir555 VirtIOVSockDev::%s:%d this=%p\n", __func__, __LINE__, this);
    }
...
int virtio_vsock_is_enabled() {
    bool r = feature_is_enabled(kFeature_VirtioVsockPipe);
