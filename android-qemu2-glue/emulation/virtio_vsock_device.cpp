...
struct VSockStream {
    ...
    bool signalWake(unsigned flags);
...
bool VSockStream::signalWake(unsigned flags) {
    while (true) {
        uint8_t data[1024];
        AndroidPipeBuffer abuf;
        abuf.data = data;
...
VirtIOVSockDev::createStreamLocked(const struct virtio_vsock_hdr *request,
                                   uint32_t *buf_alloc,
                                   uint32_t *fwd_cnt) {
...
size_t VirtIOVSockDev::vqWriteHostToGuestImpl(const uint8_t *buf8,
    ...
    while ((rem > 0) && virtio_queue_ready(vq)) {
        ...
        if (!e) {
            fprintf(stderr, "rkir555 %s:%d vq is full\n", __func__, __LINE__);
            AvqFull = true;
            break;
        }
        ...
        fprintf(stderr, "rkir555 %s:%d sz=%zu rem=%zu size=%zu\n", __func__, __LINE__, sz, rem, size);
...
bool VirtIOVSockDev::sendRWHostToGuest(VSockStream *stream) {
    while (true) {
        ...
        fprintf(stderr, "rkir555 %s:%d strem=%p src_cid=%u dst_cid=%u src_port=%u dst_port=%u len=%u mGuestBufAlloc=%u mGuestFwdCnt=%u mHostSentCnt=%u mHostFwdCnt=%u\n",
                __func__, __LINE__,
                stream,
                (uint32_t)hdr.src_cid, (uint32_t)hdr.dst_cid,
                hdr.src_port, hdr.dst_port,
                hdr.len,
                stream->mGuestBufAlloc, stream->mGuestFwdCnt,
                stream->mHostSentCnt, stream->mHostFwdCnt);
...
void VirtIOVSockDev::vqParseGuestToHostRequest(const struct virtio_vsock_hdr *request) {
    ...
    if (request->type != VIRTIO_VSOCK_TYPE_STREAM) {
        fprintf(stderr, "rkir555 %s:%d bad request type (%d), src_port=%u dst_port=%u\n",
                __func__, __LINE__, request->type, request->src_port, request->dst_port);
        return;
    }
    ...
    if (request->op == VIRTIO_VSOCK_OP_REQUEST) {
        switch (request->dst_port) {
        case 5555: {
                auto stream = createStreamLocked(request, &buf_alloc, &fwd_cnt);
                if (stream) {
                    vqWriteOpHostToGuest(request, VIRTIO_VSOCK_OP_RESPONSE, buf_alloc, fwd_cnt);
                } else {
                    fprintf(stderr, "rkir555 %s:%d {src_port=%u dst_port=%u} could not create stream - already exists\n",
        ...
        default:
            fprintf(stderr, "rkir555 %s:%d {src_port=%u dst_port=%u} unexpected dst_port\n",
                    __func__, __LINE__, request->src_port, request->dst_port);
        ...
        if (stream) {
            ...
            default:
                fprintf(stderr, "rkir555 %s:%d unexpected op, op=%s (%d)\n",
                        __func__, __LINE__,
            ...
            fprintf(stderr, "rkir555 %s:%d {src_port=%u dst_port=%u} stream not found for op=%s (%d)\n",
                    __func__, __LINE__,
                    request->src_port, request->dst_port,
                    op2str(request->op), request->op);
            vqWriteOpHostToGuest(request, VIRTIO_VSOCK_OP_RST, 0, 0);
...
void VirtIOVSockDev::vqParseGuestToHost() {
    while (true) {
        ...
        fprintf(stderr, "rkir555 %s:%d consumed requestSize=%zu request->len=%u mVqGuestToHostBuf.size()=%zu\n",
                __func__, __LINE__, requestSize, request->len, mVqGuestToHostBuf.size());
    }
}
...
void VirtIOVSockDev::vqRecvGuestToHost(VirtQueueElement *e) {
    ...
    fprintf(stderr, "rkir555 %s:%d received sz=%zu\n", __func__, __LINE__, sz);
