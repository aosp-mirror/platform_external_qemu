...
struct VSockStream {
    ...
    void writeGuestToHost(const void *buf, size_t size) {
        if (mPipe) {
            ...
            mHostCallbacks->receive(buf, size);
        } else {
    ...
    void guestConnected() {
        if (mHostCallbacks) {
            mHostCallbacks->setConnected();
        } else {
            fprintf(stderr, "rkir555 %s:%d mHostCallbacks is nullptr\n",
                    __func__, __LINE__);
        }
    }
...
struct VirtIOVSockDev {
    ...
    bool sendRWHostToGuestImpl(VSockStream *stream, const void *data, size_t size) {
        fprintf(stderr, "%s:%d mHostCid=%u mGuestCid=%u src_port=%u dst_port=%u size=%zu\n",
                __func__, __LINE__,
                uint32_t(stream->mHostCid), uint32_t(stream->mGuestCid),
                stream->mHostPort, stream->mGuestPort,
                size);
    ...
    bool sendRWHostToGuest(VSockStream *stream) {
        while (true) {
            ...
            if (b.second == 0) {
                if (stream->mHostCallbacks) {
                    stream->mHostCallbacks->sent();
                }
    ...
    uint64_t hostToGuestOpen(const uint32_t guestPort,
                             const uint32_t hostPortOffset,
                             IVsockHostCallbacks *callbacks) {
        std::lock_guard<std::mutex> lock(mMtx);

        const uint64_t hostCid = 2; // VMADDR_CID_HOST
        const uint64_t guestCid = mS->guest_cid;
        const uint32_t hostPort = getNextSrcHostPort() + hostPortOffset;
        ...
        const bool full = vqWriteHostToGuest(&request, sizeof(request));

//        fprintf(stderr, "%s:%d guestPort=%u hostPort=%u full=%d\n",
//                __func__, __LINE__, guestPort, hostPort, full);
    ...
    bool hostToGuestClose(const uint64_t key) {
        ...
        fprintf(stderr, "%s:%d src_port=%u dst_port=%u\n",
                __func__, __LINE__, stream->mHostPort, stream->mGuestPort);
    ...
    size_t hostToGuestSend(const uint64_t key, const void *data, size_t size) {
        fprintf(stderr, "%s:%d key=%llu size=%zu data='%*s'\n",
                __func__, __LINE__, (unsigned long long)key, size, int(size), (const char *)data);
...
private:
    ...
    std::shared_ptr<VSockStream> createStreamLocked(const struct virtio_vsock_hdr *request,
        ...
        auto stream = std::make_shared<VSockStream>(this,
                                                    ...
                                                    request->fwd_cnt);
        if (stream->ok()) {
            const auto key = makeStreamKey(request->src_port, request->dst_port);
            const auto r = mStreams.insert({key, {}});
            if (r.second) {
//                fprintf(stderr, "%s:%d key=%llu\n",
//                        __func__, __LINE__, (unsigned long long)key);
    ...
    std::shared_ptr<VSockStream> findStreamLocked(uint64_t key) {
        ...
        if (i == mStreams.end()) {
            fprintf(stderr, "%s:%d key=%llu nullptr\n",
                    __func__, __LINE__, (unsigned long long)key);
    ...
    bool closeStreamLocked(const uint64_t key) {
        ...
        bool r = (mStreams.erase(key) > 0);

//        fprintf(stderr, "%s:%d key=%llu r=%d backtrace=<%s>\n",
//                __func__, __LINE__, (unsigned long long)key, r, backtrace.c_str());
    ...
    void closeStreamLocked(const uint32_t guestPort, const uint32_t hostPort) {
        if (!closeStreamLocked(makeStreamKey(guestPort, hostPort))) {
            fprintf(stderr, "%s:%d could not find the stream to close, {guestPort=%u hostPort=%u}\n",
                    __func__, __LINE__, guestPort, hostPort);
        }
    }
    ...
    size_t vqWriteHostToGuestImpl(const uint8_t *buf8, size_t size) {
        ...
        fprintf(stderr, "%s:%d size=%zu rem=%zu\n", __func__, __LINE__, size, rem);
        return size - rem;
    }
    ...
    void vqParseGuestToHostRequest(const struct virtio_vsock_hdr *request) {
        ...
        fprintf(stderr, "%s:%d src_port=%u dst_port=%u op=%s\n",
                __func__, __LINE__,
                request->src_port, request->dst_port, op2str(request->op));
...
uint64_t virtio_vsock_host_to_guest_open(uint32_t guest_port,
                                         ...
                                         IVsockHostCallbacks *callbacks) {
    return g_impl ? g_impl->hostToGuestOpen(guest_port, host_port_offset, callbacks) : 0;
}
