...
private:
    ...
    void prepareFrameHeader(struct virtio_vsock_hdr *hdr,
                            const VSockStream *stream,
                            const enum virtio_vsock_op op,
                            const uint32_t len) {
        hdr->src_cid = stream->mHostCid;
        hdr->dst_cid = stream->mGuestCid;
        hdr->src_port = stream->mHostPort;
        hdr->dst_port = stream->mGuestPort;
        hdr->len = len;
        hdr->type = VIRTIO_VSOCK_TYPE_STREAM;
        hdr->op = static_cast<uint16_t>(op);
        hdr->flags = 0;
        hdr->buf_alloc = kHostBufAlloc;
        hdr->fwd_cnt = stream->mHostFwdCnt;
    }
    ...
    void vqWriteControlFrame(VirtQueue *vq,
        ...
        prepareFrameHeader(&hdr, stream, op, 0);
    ...
    void vqWriteHostToGuestLocked() {
        ...
        if (virtio_queue_ready(vq)) {
            ...
            fprintf(stderr, "%s:%d vq=%p is not ready\n", __func__, __LINE__, vq);
        }
    }
    ...
    if (op == VIRTIO_VSOCK_OP_SHUTDOWN) {
        if (mIsConnected) {
            const struct virtio_vsock_hdr hdr = {
                ...
                .len = 0,
                .type = VIRTIO_VSOCK_TYPE_STREAM,
                .op = static_cast<uint16_t>(VIRTIO_VSOCK_OP_SHUTDOWN),
