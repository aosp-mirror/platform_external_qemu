...
VSockStream::VSockStream(struct VirtIOVSockDev *d,
    ...
    fprintf(stderr, "rkir555 %s:%d this=%p mDev=%p mPipe=%p mSrcPort=%u mDstPort=%u\n",
            __func__, __LINE__, this, mDev, mPipe, mSrcPort, mDstPort);
}
...
void VSockStream::resetPipe(void *pipe) {
    mPipe = pipe;
}
...
void virtio_vsock_pipe_reset(void* hwPipe, void* hostPipe) {
    fprintf(stderr, "rkir555 %s:%d hwPipe=%p hostPipe=%p\n", __func__, __LINE__, hwPipe, hostPipe);
    static_cast<VSockStream *>(hwPipe)->resetPipe(hostPipe);
}
