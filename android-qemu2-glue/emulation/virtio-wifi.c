...
static ssize_t ieee80211_data_from_8023(const uint8_t* buf,
    ...
    size_t hdrlen = IEEE80211_HDRLEN + (qos ? 2 : 0);
    ...
    total = iov_from_buf(sg, elem->in_num, 0, (const void*)&hdr,
                         IEEE80211_HDRLEN) +
            (qos ? 2 : 0);
    memset(sg->iov_base + IEEE80211_HDRLEN, 0, 2);
    if (encaps_len) {
        total +=
...
static ssize_t virtio_wifi_receive_rcu(NetClientState* nc,
    ...
    while (offset < size) {
        ...
        int qos = 1;
        offset = ieee80211_data_from_8023(buf, elem, size, qos);
        if (offset) {
            /* signal other side */
...
static struct iovec* ieee80211_data_to_8023(struct iovec* out_sg,
                                            ...
                                            struct iovec* sg) {
    u16 hdrlen = sizeof(rfc1042_header) + IEEE80211_HDRLEN + 2;
    u16 ethertype;
    u8 dst[ETH_ALEN];
    u8 src[ETH_ALEN];
    uint8_t buf[64];
    ...
    *out_num = iov_copy(&sg[1], VIRTQUEUE_MAX_SIZE, out_sg, *out_num,
                        hdrlen + 2, iov_size(out_sg, *out_num)) +
               1;
    return sg;
}
