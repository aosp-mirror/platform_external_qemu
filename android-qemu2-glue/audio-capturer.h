...
// Qemu engine to capture audio, singleton
// QemuAudioCaptureEngine uses QEMU's builtin audio capture api
// to capture audio and send the stream to the caller.
//
// Usage:
//    1) At qemu glue starts up, register this enigne by calling the AudioCaptureEngine::set() static method
//       bool qemu_android_emulation_early_setup() {
//          ...
//          QemuAudioCaptureEngine::set(new android::qemu::QemuAudioCaptureEngine());
//          ...
//       }
//    2) Create a new AudioCapturer instance, which implements onSample() method
//
//    3) Call QemuAudioCaptureEngine::start() with the instance created in (1) to start capture.
//       After this call, audio byte stream will arrive in AudioCapturer::onSample() method
//
//    4) Call QemuAudioCaptureEngine::stop() with the instance created in (1) to stop capture
//
