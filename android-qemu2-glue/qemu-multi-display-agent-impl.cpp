        ...
        .setMultiDisplay =
            [](uint32_t id, uint32_t x, uint32_t y, uint32_t w,
               uint32_t h, uint32_t dpi, uint32_t flag, bool add) {
                auto pipe = MultiDisplayPipe::getPipe();
                if (pipe) {
                    if (add) {
                        pipe->addMultiDisplay(id, x, y, w, h, dpi, flag);
                    } else {
                        pipe->delMultiDisplay(id);
                    }
                }
            }
};

extern const QAndroidMultiDisplayAgent* const gQAndroidMultiDisplayAgent = &sMultiDisplayAgent;
