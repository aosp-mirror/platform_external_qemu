#!/usr/bin/env python
#
# Script used to create the cmake source depedencies for qemu
#
...
class GitFiles(object):
  IGNORED_OBJECTS = [
      ...
      'hw/i386/acpi-build',  # These have to be compiled with specific flags, and cannot be included
...
def main(_):
  ...
  for host in args.hosts:
    ...
    cmake = '## AUTO-GENERATED AT SHA: %s ##\n' % git_fs.get_revision()
    cmake += '# Do not edit, instead run gen-cmake.py!\n'
    cmake += '# Defines targets: ' + '\n#   '.join(targets)
    cmake += '\n\n'
    for (target, _, files, libdeps, shared) in deps:
      logging.info('Considering target: %s - files: %d deps: %s', target, len(files), libdeps)
      all_sources |= set(files)
      if 'qemu-system' in target:
        if shared_dependencies is None:
          shared_dependencies = SetLikeList(shared)
        else:
          shared_dependencies &= SetLikeList(shared)
