...
int qemu_setup_grpc() {
    ...
    auto builder = EmulatorControllerService::Builder()
                           ...
                           .withCertAndKey(android_cmdLineOptions->grpc_pem,
                                           android_cmdLineOptions->grpc_key,
                                           android_cmdLineOptions->grpc_ca)
                           .withAddress(address)
    ...
    if (grpcService) {
        ...
        if (android_cmdLineOptions->grpc_pem) {
            props["grpc.server_cert"] = android_cmdLineOptions->grpc_pem;
        }
        if (android_cmdLineOptions->grpc_ca) {
            props["grpc.ca_root"] = android_cmdLineOptions->grpc_ca;
        }
    }
    if (!grpcService && android_cmdLineOptions->grpc ||
        android_cmdLineOptions->grpc_ca || android_cmdLineOptions->grpc_key ||
        android_cmdLineOptions->grpc_ca) {
        fprintf(stderr,
