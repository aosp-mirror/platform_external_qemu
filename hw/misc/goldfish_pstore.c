...
typedef struct goldfish_pstore {
    ...
    MemoryRegion memory;
    uint64_t size;   /* Size of the region, keep in mind the larger this gets, the longer writes on exit take */
...
static void goldfish_pstore_save_restore(goldfish_pstore *store, bool write)
{
    ...
    if (write)  {
      qemu_put_buffer(file, (uint8_t*) memory_region_get_ram_ptr(&store->memory), memory_region_size(&store->memory));
    }
    else {
      size_t read = qemu_get_buffer(file, (uint8_t*) memory_region_get_ram_ptr(&store->memory), memory_region_size(&store->memory));
      assert(read == memory_region_size(&store->memory));
    }

    if (qemu_file_get_error(file)) {
        const char* flag_msg = write ? "write" : "read";
        error_setg_errno(&local_err, errno, "Unable to %s memory using file: %s", flag_msg, store->fname);
        goto Error;
    }
...
static void goldfish_pstore_realize(DeviceState *dev, Error **errp)
{
    ...
    memory_region_init_ram(&s->memory, OBJECT(s), GOLDFISH_PSTORE_DEV_ID, s->size, errp);
