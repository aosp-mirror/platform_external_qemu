...
void goldfish_hostmem_send_command(uint64_t gpa,
    ...
    DPRINT("incoming hostmem cmd. "
           ...
           "status_code=%llu "
           "cmd=%llu ",
           id, host_ptr, size, status_code, cmd);
...
#define GOLDFISH_HOSTMEM(obj) \
    OBJECT_CHECK(struct goldfish_hostmem_state, (obj), TYPE_GOLDFISH_HOSTMEM)
...
goldfish_hostmem_read(void *opaque,
    ...
    default:
        DPRINT("INVALID hostmem_read! reg=%u", offset);
        assert(false);
        return 0;
    }
    return 0;
...
goldfish_hostmem_write(void *opaque,
    ...
    case HOSTMEM_REG_INIT:
        ...
        break;
    default:
        DPRINT("INVALID hostmem_write! reg=%u", offset);
        assert(false);
    }
}
...
static void goldfish_hostmem_realize(DeviceState *dev, Error **errp) {
    struct goldfish_hostmem_state* s = GOLDFISH_HOSTMEM(dev);

    s_goldfish_hostmem_dev = s;
...
static Property goldfish_hostmem_properties[] = {
    DEFINE_PROP_UINT64("addr", goldfish_hostmem_state, addr, 0),
    DEFINE_PROP_SIZE("size", goldfish_hostmem_state, size, 0),
...
static void goldfish_hostmem_class_init(ObjectClass *klass, void *data)
{
    ...
    dc->realize = goldfish_hostmem_realize;
    dc->desc = "goldfish_hostmem";
    dc->props = goldfish_hostmem_properties;
}
