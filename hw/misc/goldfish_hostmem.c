...
#define HOSTMEM_REG_BATCH_COMMAND_ADDR           0x00
#define HOSTMEM_REG_BATCH_COMMAND_ADDR_HIGH      0x04
...
goldfish_hostmem_write(void *opaque,
    ...
    case HOSTMEM_REG_BATCH_COMMAND_ADDR:
        DPRINT("write HOSTMEM_REG_BATCH_COMMAND_ADDR, physmem: 0x%x", (uint32_t)val);
        s->batch_cmd_addr = val;
        break;
    case HOSTMEM_REG_BATCH_COMMAND_ADDR_HIGH:
        DPRINT("write HOSTMEM_REG_BATCH_COMMAND_ADDR_HIGH, high part: 0x%x", (uint32_t)val);
        s->batch_cmd_addr |= (uint64_t)(val << 32);
        DPRINT("got phys batch addr: 0x%llx", (unsigned long long)(s->batch_cmd_addr));
        break;
    default:
