...
goldfish_hostmem_read(void *opaque,
                   hwaddr offset,
                   unsigned size) {
    fprintf(stderr, "opaque=%p offset=0x%lx sz=0x%x\n", opaque, offset, size);
...
static void goldfish_hostmem_realize(PCIDevice *pci_dev, Error **errp) {
    ...
    memory_region_init_io(&goldfish_state->iomem_port,
                          OBJECT(goldfish_state),
                          &goldfish_hostmem_iomem_port_ops,
                          goldfish_state,
                          "goldfish_hostmem_pio",
                          0x100);

    pci_register_bar(pci_dev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY, &goldfish_state->iomem);
    pci_register_bar(pci_dev, 1, PCI_BASE_ADDRESS_SPACE_IO, &goldfish_state->iomem_port);
