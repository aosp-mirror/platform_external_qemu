...
static void smc91c111_save(QEMUFile *f, void *opaque)
{
     ...
     * initialized when the emulator starts. Saving/restoring those values
     * is therefore useless and may even be harmful, so they are omitted.
     */
    ...
    qemu_put_buffer(f, (uint8_t*) s->tx_fifo, NUM_PACKETS * 4);  // buffer size in bytes, 4 bytes to a 32-bit int
    qemu_put_be32(f, s->rx_fifo_len);
    qemu_put_buffer(f, (uint8_t*) s->rx_fifo, NUM_PACKETS * 4);
    qemu_put_be32(f, s->tx_fifo_done_len);
    qemu_put_buffer(f, (uint8_t*) s->tx_fifo_done, NUM_PACKETS * 4);

    /* Packet buffer memory.  */
    qemu_put_buffer(f, (uint8_t*) s->data, NUM_PACKETS * BUFFER_PER_PACKET);
...
static int smc91c111_load(QEMUFile *f, void *opaque, int version_id)
{
    ...
    qemu_get_buffer(f, (uint8_t*) s->tx_fifo, NUM_PACKETS * 4);  // buffer size in bytes, 4 bytes to a 32-bit int
    s->rx_fifo_len = qemu_get_be32(f);
    qemu_get_buffer(f, (uint8_t*) s->rx_fifo, NUM_PACKETS * 4);
    s->tx_fifo_done_len = qemu_get_be32(f);
    qemu_get_buffer(f, (uint8_t*) s->tx_fifo_done, NUM_PACKETS * 4);

    /* Packet buffer memory.  */
    qemu_get_buffer(f, (uint8_t*) s->data, NUM_PACKETS * BUFFER_PER_PACKET);
