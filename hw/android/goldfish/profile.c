...
static void dump_profile () {
  ...
  int len = strlen(profile_dirname);
  char filename[BUF_SIZE];
  assert(len + 2 < BUF_SIZE);
  strcpy(filename, profile_dirname);
  filename[len] = '/';
  filename[len + 1] = 0;
    ...
    *(i+4) = 0;
    ...
    while (r) {
      fprintf (f, "%llx-%llx:%llu\n", (unsigned long long) r->start,
               (unsigned long long) r->end, (unsigned long long) r->count);
      r = r->next;
    }
...
static void add_branch_count (BinaryProfile *profile, target_ulong from, target_ulong to) {
...
static void add_range_count (BinaryProfile *profile, target_ulong start, target_ulong end) {
...
static const MmapData *get_mmap_data(long start, target_ulong end, int pid)
{
    const MmapData *data = mmaps[pid];
    while (data) {
        if (start >= data->start && start < data->end) {
            return data;
        }
        data = data->next;
    }
                 ...
                 target_ulong offset, const char *path, unsigned pid)
{
    ...
    data->end = vend;
    data->offset = offset;
    data->name = (char *) malloc (strlen(path) + 1);
...
void profile_bb_helper(target_ulong pc, uint32_t size)
{
  ...
  if (bb_counter++ == SAMPLE_PERIOD) {
    ...
    prev_mmap = get_mmap_data(pc, pc + size, prev_pid);
    if (prev_mmap == NULL) {
      bb_counter = 0;
      return;
    }
    ...
    const MmapData * data =
        get_mmap_data(pc, pc + size, prev_pid);
    if (data != prev_mmap) {
      bb_counter = 0;
      return;
    }
