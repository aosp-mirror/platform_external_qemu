...
void events_dev_init(uint32_t base, qemu_irq irq)
{
    ...
    if (!androidHwConfig_isScreenNoTouch(config)) {
        ...
        if (androidHwConfig_isScreenMultiTouch(config)) {
            ...
            abs_values[LINUX_ABS_MT_TRACKING_ID].max = abs_values[LINUX_ABS_MT_SLOT].max + 1;
            abs_values[LINUX_ABS_MT_POSITION_X].max = abs_values[ABS_X].max;
            abs_values[LINUX_ABS_MT_POSITION_Y].max = abs_values[ABS_Y].max;
            abs_values[LINUX_ABS_MT_TOUCH_MAJOR].max = 0x7fffffff; // TODO: Make it less random
            abs_values[LINUX_ABS_MT_PRESSURE].max = 0x100; // TODO: Make it less random
        }
    }
