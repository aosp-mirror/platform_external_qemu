...
void nand_add_dev(const char *arg)
{
	...
	//    size=<size>              # size of NAND partition in bytes
	//    pagesize=<size>          # page size in bytes [2048]
	//    extrasize=<size>         # extra page data size in bytes [64]
	//    erasepages=<count>       # count of erase pages [64]
	//    file=<path>              # backing file.
    while(arg) {
        next_arg = strchr(arg, ',');
        value = strchr(arg, '=');
        if(next_arg != NULL) {
    ...
    // if file=<path> is not used, create a temporary file.
    if (rwfilename == NULL) {
