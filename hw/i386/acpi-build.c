...
static void acpi_get_pm_info(AcpiPmInfo *pm) {
  ...
  pm->acpi_enable_cmd =
      object_property_get_int(obj, ACPI_PM_PROP_ACPI_ENABLE_CMD, NULL);
  pm->acpi_disable_cmd =
      object_property_get_int(obj, ACPI_PM_PROP_ACPI_DISABLE_CMD, NULL);
  pm->io_base = object_property_get_int(obj, ACPI_PM_PROP_PM_IO_BASE, NULL);
  pm->gpe0_blk = object_property_get_int(obj, ACPI_PM_PROP_GPE0_BLK, NULL);
  pm->gpe0_blk_len =
      object_property_get_int(obj, ACPI_PM_PROP_GPE0_BLK_LEN, NULL);
  pm->pcihp_bridge_en = object_property_get_bool(
      obj, "acpi-pci-hotplug-with-bridge-support", NULL);
}
...
static Object *acpi_get_i386_pci_host(void) {
  ...
  if (!host) {
    host = OBJECT_CHECK(PCIHostState, object_resolve_path("/machine/q35", NULL),
                        TYPE_PCI_HOST_BRIDGE);
  }
...
static void acpi_get_pci_holes(Range *hole, Range *hole64) {
  ...
  range_set_bounds1(
      hole,
      object_property_get_int(pci_host, PCI_HOST_PROP_PCI_HOLE_START, NULL),
      object_property_get_int(pci_host, PCI_HOST_PROP_PCI_HOLE_END, NULL));
  range_set_bounds1(
      hole64,
      object_property_get_int(pci_host, PCI_HOST_PROP_PCI_HOLE64_START, NULL),
      object_property_get_int(pci_host, PCI_HOST_PROP_PCI_HOLE64_END, NULL));
}
...
/* FACS */
static void build_facs(GArray *table_data, BIOSLinker *linker) {
...
static void fadt_setup(AcpiFadtDescriptorRev1 *fadt, AcpiPmInfo *pm) {
  ...
  fadt->flags =
      cpu_to_le32((1 << ACPI_FADT_F_WBINVD) | (1 << ACPI_FADT_F_PROC_C1) |
                  (1 << ACPI_FADT_F_SLP_BUTTON) | (1 << ACPI_FADT_F_RTC_S4));
...
/* FADT */
static void build_fadt(GArray *table_data, BIOSLinker *linker, AcpiPmInfo *pm,
  ...
  bios_linker_loader_add_pointer(linker, ACPI_BUILD_TABLE_FILE, fw_ctrl_offset,
  ...
  bios_linker_loader_add_pointer(linker, ACPI_BUILD_TABLE_FILE,
                                 dsdt_entry_offset, sizeof(fadt->dsdt),
                                 ACPI_BUILD_TABLE_FILE, dsdt_tbl_offset);

  build_header(linker, table_data, (void *)fadt, "FACP", sizeof(*fadt), 1,
               oem_id, oem_table_id);
}
...
static void *acpi_set_bsel(PCIBus *bus, void *opaque) {
  ...
  if (qbus_is_hotpluggable(BUS(bus))) {
    ...
    *bus_bsel = (*bsel_alloc)++;
    object_property_add_uint32_ptr(OBJECT(bus), ACPI_PCIHP_PROP_BSEL, bus_bsel,
                                   &error_abort);
  }
...
static void build_append_pci_bus_devices(Aml *parent_scope, PCIBus *bus,
  ...
  for (i = 0; i < ARRAY_SIZE(bus->devices); i += PCI_FUNC_MAX) {
     ...
     */
    bridge_in_acpi =
        pc->is_bridge && pcihp_bridge_en && !DEVICE(pdev)->hotplugged;
    ...
    if (pc->class_id == PCI_CLASS_DISPLAY_VGA) {
      ...
      method = aml_method("_EJ0", 1, AML_NOTSERIALIZED);
      aml_append(method, aml_call2("PCEJ", aml_name("BSEL"), aml_name("_SUN")));
      aml_append(dev, method);
 ...
 */
static Aml *initialize_route(Aml *route, const char *link_name, Aml *lnk_idx,
                             int idx) {
  ...
  while_ctx = aml_while(aml_lless(pin, aml_int(128)));
  {
    ...
    if (is_pci0_prt) {
      ...
      if_device_1 = aml_if(aml_equal(lnk_idx, aml_int(1)));
      {
        ...
        else_pin_4 = aml_else();
        { aml_append(else_pin_4, aml_store(build_prt_entry("LNKA"), route)); }
    ...
    aml_append(while_ctx, aml_store(aml_or(aml_shiftleft(slot, aml_int(16)),
                                           aml_int(0xFFFF), NULL),
                                    aml_index(route, aml_int(0))));
    /* route[1] = pin & 3 */
    aml_append(while_ctx, aml_store(aml_and(pin, aml_int(3), NULL),
...
static void crs_range_set_init(CrsRangeSet *range_set) {
  ...
  range_set->mem_64bit_ranges = g_ptr_array_new_with_free_func(crs_range_free);
}
 ...
 */
static void crs_replace_with_free_ranges(GPtrArray *ranges, uint64_t start,
                                         uint64_t end) {
...
static Aml *build_crs(PCIHostState *host, CrsRangeSet *range_set) {
  ...
  for (devfn = 0; devfn < ARRAY_SIZE(host->bus->devices); devfn++) {
    ...
    for (i = 0; i < PCI_NUM_REGIONS; i++) {
      ...
      if (r->type & PCI_BASE_ADDRESS_SPACE_IO) {
        crs_range_insert(temp_range_set.io_ranges, range_base, range_limit);
      } else { /* "memory" */
        crs_range_insert(temp_range_set.mem_ranges, range_base, range_limit);
      }
    }
    ...
    if (type == PCI_HEADER_TYPE_BRIDGE) {
      ...
      range_base = pci_bridge_get_base(dev, PCI_BASE_ADDRESS_SPACE_MEMORY);
      range_limit = pci_bridge_get_limit(dev, PCI_BASE_ADDRESS_SPACE_MEMORY);
      ...
      if (range_base && range_base <= range_limit) {
        ...
        if (range_limit <= UINT32_MAX && length <= UINT32_MAX) {
          ...
          crs_range_insert(temp_range_set.mem_64bit_ranges, range_base,
                           range_limit);
        }
      }

      range_base = pci_bridge_get_base(dev, PCI_BASE_ADDRESS_MEM_PREFETCH);
      range_limit = pci_bridge_get_limit(dev, PCI_BASE_ADDRESS_MEM_PREFETCH);
  ...
  for (i = 0; i < temp_range_set.io_ranges->len; i++) {
    ...
    aml_append(crs, aml_word_io(AML_MIN_FIXED, AML_MAX_FIXED, AML_POS_DECODE,
                                AML_ENTIRE_RANGE, 0, entry->base, entry->limit,
  ...
  for (i = 0; i < temp_range_set.mem_ranges->len; i++) {
    ...
    aml_append(
        crs, aml_dword_memory(AML_POS_DECODE, AML_MIN_FIXED, AML_MAX_FIXED,
                              AML_NON_CACHEABLE, AML_READ_WRITE, 0, entry->base,
                              entry->limit, 0, entry->limit - entry->base + 1));
  ...
  for (i = 0; i < temp_range_set.mem_64bit_ranges->len; i++) {
    ...
    aml_append(
        crs, aml_qword_memory(AML_POS_DECODE, AML_MIN_FIXED, AML_MAX_FIXED,
                              AML_NON_CACHEABLE, AML_READ_WRITE, 0, entry->base,
                              entry->limit, 0, entry->limit - entry->base + 1));
    crs_range_insert(range_set->mem_64bit_ranges, entry->base, entry->limit);
  }

  crs_range_set_free(&temp_range_set);

  aml_append(
      crs, aml_word_bus_number(AML_MIN_FIXED, AML_MAX_FIXED, AML_POS_DECODE, 0,
                               pci_bus_num(host->bus), max_bus, 0,
...
static void build_hpet_aml(Aml *table) {
  ...
  if_ctx = aml_if(aml_lor(aml_equal(id, zero), aml_equal(id, aml_int(0xffff))));
...
static void build_android_dt_aml(Aml *scope, const char *dev_name,
                                 const char *hid_name, const char *str_name) {
  Aml *dev = aml_device(dev_name);
  Aml *properties, *dsd;
  ...
  aml_append(dev, aml_name_decl("_STR", aml_unicode(str_name)));
	...
	const char* acpiIniFile = avdInfo_getAcpiIniPath(android_avdInfo);
	...
	if (acpiIniFile == NULL) {
		...
		if (system_device_in_guest) {
			append_string_property(properties, "android.fstab.system.compatible", "android,system");
			append_string_property(properties, "android.fstab.system.dev", system_device_in_guest);
		...
		CIniFile* acpiIni = iniFile_newFromFile(acpiIniFile);
...
static Aml *build_fdinfo_aml(int idx, FloppyDriveType type) {
  ...
  aml_append(fdi, aml_int(cmos_get_fd_drive_type(type))); /* Device Type */
  /*
...
#define ACPI_FDE_MAX_FD 4
  ...
  aml_append(dev, aml_name_decl(
                      "_FDE", aml_buffer(sizeof(fde_buf), (uint8_t *)fde_buf)));
...
static Aml *build_iqcr_method(bool is_piix4) {
  ...
  if (is_piix4) {
    ...
    aml_append(method, aml_store(aml_and(aml_arg(0), aml_int(0xF), NULL),
                                 aml_name("PRRI")));
  }
...
static void build_q35_pci0_int(Aml *table) {
  ...
  aml_append(pci0_scope, aml_name_decl("PRTP", build_q35_routing_table("LNK")));
  aml_append(pci0_scope, aml_name_decl("PRTA", build_q35_routing_table("GSI")));
...
static void build_q35_isa_bridge(Aml *table) {
  ...
  aml_append(dev,
             aml_operation_region("PIRQ", AML_PCI_CONFIG, aml_int(0x60), 0x0C));

  aml_append(dev,
             aml_operation_region("LPCD", AML_PCI_CONFIG, aml_int(0x80), 0x02));
  ...
  aml_append(dev,
             aml_operation_region("LPCE", AML_PCI_CONFIG, aml_int(0x82), 0x02));
  /* enable bits */
...
static void build_piix4_pm(Aml *table) {
  ...
  aml_append(dev,
             aml_operation_region("P13C", AML_PCI_CONFIG, aml_int(0x00), 0xff));
  aml_append(scope, dev);
  aml_append(table, scope);
}
...
static void build_piix4_isa_bridge(Aml *table) {
  ...
  aml_append(dev,
             aml_operation_region("P40C", AML_PCI_CONFIG, aml_int(0x60), 0x04));
  /* enable bits */
...
static void build_piix4_pci_hotplug(Aml *table) {
  ...
  aml_append(scope, aml_operation_region("PCST", AML_SYSTEM_IO, aml_int(0xae00),
                                         0x08));
  ...
  aml_append(scope, aml_operation_region("BNMR", AML_SYSTEM_IO, aml_int(0xae10),
                                         0x04));
  ...
  aml_append(method, aml_store(aml_shiftleft(aml_int(1), aml_arg(1)),
                               aml_name("B0EJ")));
...
static void build_dsdt(GArray *table_data, BIOSLinker *linker, AcpiPmInfo *pm,
                       AcpiMiscInfo *misc, Range *pci_hole, Range *pci_hole64,
  ...
  if (misc->is_piix4) {
    ...
    sb_scope = aml_scope("_SB");
    aml_append(sb_scope, aml_operation_region("PCST", AML_SYSTEM_IO,
                                              aml_int(0xae00), 0x0c));
    aml_append(sb_scope, aml_operation_region("PCSB", AML_SYSTEM_IO,
                                              aml_int(0xae0c), 0x01));
  ...
  if (pcmc->legacy_cpu_hotplug) {
    ...
    CPUHotplugFeatures opts = {.apci_1_compatible = true,
                               .has_legacy_cphp = true};
    build_cpus_aml(dsdt, machine, opts, pm->cpu_hp_io_base, "\\_SB.PCI0",
                   "\\_GPE._E02");
  }
  ...
  scope = aml_scope("_GPE");
  {
    ...
    if (misc->is_piix4) {
      ...
      aml_append(method, aml_acquire(aml_name("\\_SB.PCI0.BLCK"), 0xFFFF));
    ...
    if (pcms->acpi_nvdimm_state.is_enabled) {
      ...
      aml_append(method, aml_notify(aml_name("\\_SB.NVDR"), aml_int(0x80)));
      aml_append(scope, method);
    }
  }
  ...
  aml_append(crs, aml_word_bus_number(
                      AML_MIN_FIXED, AML_MAX_FIXED, AML_POS_DECODE, 0x0000, 0x0,
                      root_bus_limit, 0x0000, root_bus_limit + 1));
  aml_append(crs, aml_io(AML_DECODE16, 0x0CF8, 0x0CF8, 0x01, 0x08));

  aml_append(crs, aml_word_io(AML_MIN_FIXED, AML_MAX_FIXED, AML_POS_DECODE,
                              AML_ENTIRE_RANGE, 0x0000, 0x0000, 0x0CF7, 0x0000,
                              0x0CF8));
  ...
  for (i = 0; i < crs_range_set.io_ranges->len; i++) {
    ...
    aml_append(crs,
               aml_word_io(AML_MIN_FIXED, AML_MAX_FIXED, AML_POS_DECODE,
                           AML_ENTIRE_RANGE, 0x0000, entry->base, entry->limit,
                           0x0000, entry->limit - entry->base + 1));
  }

  aml_append(crs, aml_dword_memory(AML_POS_DECODE, AML_MIN_FIXED, AML_MAX_FIXED,
                                   AML_CACHEABLE, AML_READ_WRITE, 0, 0x000A0000,
                                   0x000BFFFF, 0, 0x00020000));

  crs_replace_with_free_ranges(crs_range_set.mem_ranges, range_lob(pci_hole),
                               range_upb(pci_hole));
  ...
  crs = aml_resource_template();
  aml_append(crs, aml_io(AML_DECODE16, pm->gpe0_blk, pm->gpe0_blk, 1,
                         pm->gpe0_blk_len));
  aml_append(dev, aml_name_decl("_CRS", crs));
  aml_append(scope, dev);
  ...
  if (pm->pcihp_io_len) {
    dev = aml_device("PHPR");
    aml_append(dev, aml_name_decl("_HID", aml_string("PNP0A06")));
    aml_append(dev, aml_name_decl("_UID", aml_string("PCI Hotplug resources")));
    /* device present, functioning, decoding, not shown in UI */
    aml_append(dev, aml_name_decl("_STA", aml_int(0xB)));
    crs = aml_resource_template();
    aml_append(crs, aml_io(AML_DECODE16, pm->pcihp_io_base, pm->pcihp_io_base,
                           1, pm->pcihp_io_len));
    aml_append(dev, aml_name_decl("_CRS", crs));
    aml_append(scope, dev);
  }
  ...
  /* create fw_cfg node, unconditionally */
  {
    ...
    uint8_t io_size =
        object_property_get_bool(OBJECT(pcms->fw_cfg), "dma_enabled", NULL)
    ...
    crs = aml_resource_template();
    aml_append(crs, aml_io(AML_DECODE16, FW_CFG_IO_BASE, FW_CFG_IO_BASE, 0x01,
                           io_size));
    aml_append(dev, aml_name_decl("_CRS", crs));
  ...
  if (misc->applesmc_io_base) {
    ...
    aml_append(crs,
               aml_io(AML_DECODE16, misc->applesmc_io_base,
                      misc->applesmc_io_base, 0x01, APPLESMC_MAX_DATA_LENGTH));
    aml_append(crs, aml_irq_no_flags(6));
    aml_append(dev, aml_name_decl("_CRS", crs));
  ...
  if (misc->pvpanic_port) {
    ...
    crs = aml_resource_template();
    aml_append(crs, aml_io(AML_DECODE16, misc->pvpanic_port, misc->pvpanic_port,
                           1, 1));
    aml_append(dev, aml_name_decl("_CRS", crs));
...
static void build_hpet(GArray *table_data, BIOSLinker *linker) {
  Acpi20Hpet *hpet;
  ...
  build_header(linker, table_data, (void *)hpet, "HPET", sizeof(*hpet), 1, NULL,
               NULL);
}

static void build_tpm_tcpa(GArray *table_data, BIOSLinker *linker,
                           GArray *tcpalog) {
  ...
  bios_linker_loader_add_pointer(linker, ACPI_BUILD_TABLE_FILE, log_addr_offset,
                                 log_addr_size, ACPI_BUILD_TPMLOG_FILE, 0);

  build_header(linker, table_data, (void *)tcpa, "TCPA", sizeof(*tcpa), 2, NULL,
               NULL);
}

static void build_tpm2(GArray *table_data, BIOSLinker *linker) {
  Acpi20TPM2 *tpm2_ptr;
  ...
  build_header(linker, table_data, (void *)tpm2_ptr, "TPM2", sizeof(*tpm2_ptr),
               4, NULL, NULL);
}
...
static void build_srat(GArray *table_data, BIOSLinker *linker,
  ...
  ram_addr_t hotplugabble_address_space_size =
      object_property_get_int(OBJECT(pcms), PC_MACHINE_MEMHP_REGION_SIZE, NULL);
  ...
  for (i = 1; i < pcms->numa_nodes + 1; ++i) {
    ...
    numamem = acpi_data_push(table_data, sizeof *numamem);
    build_srat_memory(numamem, mem_base, mem_len, i - 1, MEM_AFFINITY_ENABLED);
  }
  ...
  build_header(linker, table_data, (void *)(table_data->data + srat_start),
               "SRAT", table_data->len - srat_start, 1, NULL, NULL);
}

static void build_mcfg_q35(GArray *table_data, BIOSLinker *linker,
                           AcpiMcfgInfo *info) {
  AcpiTableMcfg *mcfg;
  const char *sig;
 ...
 */
static void build_dmar_q35(GArray *table_data, BIOSLinker *linker) {
 ...
 */
static void build_amd_iommu(GArray *table_data, BIOSLinker *linker) {
...
static GArray *build_rsdp(GArray *rsdp_table, BIOSLinker *linker,
  ...
  bios_linker_loader_add_pointer(linker, ACPI_BUILD_RSDP_FILE, rsdt_pa_offset,
                                 rsdt_pa_size, ACPI_BUILD_TABLE_FILE,
                                 rsdt_tbl_offset);
...
static void acpi_build(AcpiBuildTables *tables, MachineState *machine) {
  ...
  bios_linker_loader_alloc(tables->linker, ACPI_BUILD_TABLE_FILE, tables_blob,
  ...
  build_dsdt(tables_blob, tables->linker, &pm, &misc, &pci_hole, &pci_hole64,
             machine);
  ...
  build_fadt(tables_blob, tables->linker, &pm, facs, dsdt, slic_oem.id,
             slic_oem.table_id);
  ...
  if (vmgenid_dev) {
    acpi_add_table(table_offsets, tables_blob);
    vmgenid_build_acpi(VMGENID(vmgenid_dev), tables_blob, tables->vmgenid,
                       tables->linker);
  }
  ...
  build_rsdt(tables_blob, tables->linker, table_offsets, slic_oem.id,
             slic_oem.table_id);
  ...
  if (pcmc->legacy_acpi_table_size) {
     ...
     */
    int legacy_aml_len = pcmc->legacy_acpi_table_size +
                         ACPI_BUILD_LEGACY_CPU_AML_SIZE * pcms->apic_id_limit;
    int legacy_table_size = ROUND_UP(
        tables_blob->len - aml_len + legacy_aml_len, ACPI_BUILD_ALIGN_SIZE);
    ...
    if (tables_blob->len > ACPI_BUILD_TABLE_SIZE / 2) {
      ...
      error_report("Warning: migration may not work.");
      error_report(
          "Warning: please remove CPUs, NUMA nodes, "
...
static MemoryRegion *acpi_add_rom_blob(AcpiBuildState *build_state,
                                       ...
                                       uint64_t max_size) {
  return rom_add_blob(name, blob->data, acpi_data_len(blob), max_size, -1, name,
                      acpi_build_update, build_state, NULL, true);
}
...
void acpi_setup(void) {
  ...
  build_state->table_mr =
      acpi_add_rom_blob(build_state, tables.table_data, ACPI_BUILD_TABLE_FILE,
                        ACPI_BUILD_TABLE_MAX_SIZE);
  assert(build_state->table_mr != NULL);

  build_state->linker_mr = acpi_add_rom_blob(
      build_state, tables.linker->cmd_blob, "etc/table-loader", 0);

  fw_cfg_add_file(pcms->fw_cfg, ACPI_BUILD_TPMLOG_FILE, tables.tcpalog->data,
                  acpi_data_len(tables.tcpalog));

  vmgenid_dev = find_vmgenid_dev();
  if (vmgenid_dev) {
    vmgenid_add_fw_cfg(VMGENID(vmgenid_dev), pcms->fw_cfg, tables.vmgenid);
  }
  ...
  if (!pcmc->rsdp_in_ram) {
    ...
    fw_cfg_add_file_callback(pcms->fw_cfg, ACPI_BUILD_RSDP_FILE,
                             acpi_build_update, build_state, build_state->rsdp,
                             rsdp_size, true);
