 ...
 *
 *  This implementation uses the classic device assignment interface of GVM
 *  and is only available on x86 hosts. It is expected to be obsoleted by VFIO
 *  based device assignment.
 *
 *  Adapted for GVM (qemu-gvm) by Qumranet. QEMU version was based on qemu-gvm
...
#define TYPE_PCI_ASSIGN "gvm-pci-assign"
...
static void free_msi_virqs(AssignedDevice *dev)
{
    ...
    for (i = 0; i < dev->msi_virq_nr; i++) {
        if (dev->msi_virq[i] >= 0) {
            gvm_irqchip_release_virq(gvm_state, dev->msi_virq[i]);
            dev->msi_virq[i] = -1;
        }
    }
    g_free(dev->msi_virq);
...
{
    uint32_t flags = GVM_DEV_ASSIGN_ENABLE_IOMMU;
    int r;

    /* Only pass non-zero PCI segment to capable module */
    if (!gvm_check_extension(gvm_state, GVM_CAP_PCI_SEGMENT) &&
        dev->host.domain) {
        error_setg(errp, "Can't assign device inside non-zero PCI segment "
                   "as this GVM module doesn't support it.");
        return;
    }

    if (!gvm_check_extension(gvm_state, GVM_CAP_IOMMU)) {
    ...
    if (dev->features & ASSIGNED_DEVICE_SHARE_INTX_MASK &&
        gvm_has_intx_set_mask()) {
        flags |= GVM_DEV_ASSIGN_PCI_2_3;
    }

    r = gvm_device_pci_assign(gvm_state, &dev->host, flags, &dev->dev_id);
    if (r < 0) {
        switch (r) {
        case -EBUSY: {
            char *cause;
...
static void verify_irqchip_in_kernel(Error **errp)
{
    ...
    error_setg(errp, "pci-assign requires GVM with in-kernel irqchip enabled");
}
...
static int assign_intx(AssignedDevice *dev, Error **errp)
{
    ...
    case ASSIGNED_IRQ_INTX_HOST_MSI:
        ...
        r = gvm_device_intx_deassign(gvm_state, dev->dev_id, intx_host_msi);
        break;
    case ASSIGNED_IRQ_MSI:
        r = gvm_device_msi_deassign(gvm_state, dev->dev_id);
        break;
    case ASSIGNED_IRQ_MSIX:
        r = gvm_device_msix_deassign(gvm_state, dev->dev_id);
        break;
    default:
        r = 0;
        break;
...
retry:
    ...
    r = gvm_device_intx_assign(gvm_state, dev->dev_id, intx_host_msi,
                               intx_route.irq);
    if (r < 0) {
...
static void deassign_device(AssignedDevice *dev)
{
    ...
    r = gvm_device_pci_deassign(gvm_state, dev->dev_id);
    assert(r == 0);
}
...
static void assigned_dev_update_msi(PCIDevice *pci_dev)
{
    ...
    if (assigned_dev->assigned_irq_type == ASSIGNED_IRQ_MSI ||
        ...
        r = gvm_device_msi_deassign(gvm_state, assigned_dev->dev_id);
    ...
    if (ctrl_byte & PCI_MSI_FLAGS_ENABLE) {
        ...
        virq = gvm_irqchip_add_msi_route(gvm_state, 0, pci_dev);
        if (virq < 0) {
            perror("assigned_dev_update_msi: gvm_irqchip_add_msi_route");
            return;
        }
        ...
        if (gvm_device_msi_assign(gvm_state, assigned_dev->dev_id, virq) < 0) {
            perror("assigned_dev_update_msi: gvm_device_msi_assign");
        }
...
static void assigned_dev_update_msi_msg(PCIDevice *pci_dev)
{
    ...
    gvm_irqchip_update_msi_route(gvm_state, assigned_dev->msi_virq[0],
...
static int assigned_dev_update_msix_mmio(PCIDevice *pci_dev)
{
    ...
    r = gvm_device_msix_init_vectors(gvm_state, adev->dev_id, entries_nr);
    if (r != 0) {
    ...
    for (i = 0; i < adev->msix_max; i++, entry++) {
        ...
        r = gvm_irqchip_add_msi_route(gvm_state, i, pci_dev);
        if (r < 0) {
            return r;
        }
        adev->msi_virq[i] = r;
        ...
        r = gvm_device_msix_set_vector(gvm_state, adev->dev_id, i,
                                       adev->msi_virq[i]);
        if (r) {
...
static void assigned_dev_update_msix(PCIDevice *pci_dev)
{
    ...
    if ((assigned_dev->assigned_irq_type == ASSIGNED_IRQ_MSIX) ||
        ...
        r = gvm_device_msix_deassign(gvm_state, assigned_dev->dev_id);
        /* -ENXIO means no assigned irq */
        if (r && r != -ENXIO) {
    ...
    if (ctrl_word & PCI_MSIX_FLAGS_ENABLE) {
        ...
        if (assigned_dev->msi_virq_nr > 0) {
            if (gvm_device_msix_assign(gvm_state, assigned_dev->dev_id) < 0) {
                                          ...
                                          uint32_t val, int len)
{
    ...
    if (gvm_has_intx_set_mask() &&
        ...
        if (intx_masked != !!(old_cmd & PCI_COMMAND_INTX_DISABLE)) {
            ret = gvm_device_intx_set_mask(gvm_state, assigned_dev->dev_id,
                                           intx_masked);
            if (ret) {
...
static int assigned_device_pci_cap_init(PCIDevice *pci_dev, Error **errp)
{
    ...
    if (pos != 0 && gvm_check_extension(gvm_state, GVM_CAP_ASSIGN_DEV_IRQ)) {
        verify_irqchip_in_kernel(&local_err);
        if (local_err) {
            error_propagate(errp, local_err);
            return -ENOTSUP;
    ...
    if (pos != 0 && gvm_device_msix_supported(gvm_state)) {
        int bar_nr;
        ...
        msix_max = MIN(msix_max, GVM_MAX_MSIX_PER_DEV);
                                         ...
                                         uint64_t val, unsigned size)
{
    ...
    if (ctrl & PCI_MSIX_FLAGS_ENABLE) {
        ...
        if (!assigned_dev_msix_masked(&orig) &&
             ...
             * to mask or disable the interrupt.  GVM doesn't have
             * support for pending bits and gvm_assign_set_msix_entry
            ...
            if (i >= adev->msi_virq_nr || adev->msi_virq[i] < 0) {
                ...
                ret = gvm_irqchip_update_msi_route(gvm_state,
                                                   adev->msi_virq[i], msg,
                                                   pdev);
                if (ret) {
...
static void assigned_realize(struct PCIDevice *pci_dev, Error **errp)
{
    ...
    if (!gvm_enabled()) {
        error_setg(&local_err, "pci-assign requires GVM support");
...
static void assign_class_init(ObjectClass *klass, void *data)
{
    ...
    dc->desc        = "GVM-based PCI passthrough";
}
