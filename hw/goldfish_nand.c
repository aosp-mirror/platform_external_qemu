...
typedef struct {
    ...
    size_t     devname_len;
    char*      data;         /* buffer for read/write actions to underlying image */
    int        fd;
    uint32_t   flags;
    uint32_t   page_size;
    uint32_t   extra_size;
...
static void  nand_dev_save_disk_state(QEMUFile* f, nand_dev *dev)
{
    ...
    uint64_t actual_contents = total_copied;
...
static int  nand_dev_load_disk_state(QEMUFile* f, nand_dev* dev)
{
    ...
    if (total_size > dev->max_size) {
        XLOG("nand_dev_load_disk_state, restore failed: size required (%lld) exceeds device limit (%lld)\n",
             total_size, dev->max_size);
        return -EIO;
    }
    ...
    while (next_offset < total_size) {
        ...
        qemu_get_buffer(f, buffer, buf_size);
        int ret = do_write(dev->fd, buffer, buf_size);
        if (ret != buf_size) {
            XLOG("nand_dev_load_disk_state, write failed: %s\n", strerror(errno));
            return -EIO;
        }
