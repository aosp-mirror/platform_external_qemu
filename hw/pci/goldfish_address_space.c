...
enum address_space_command_id {
	...
	ADDRESS_SPACE_COMMAND_IRQ_HANDLED = 3,
};
                                          ...
                                          enum address_space_command_id cmd)
{
    ...
    case ADDRESS_SPACE_COMMAND_ALLOCATE_BLOCK:
        {
        uint32_t alloc_res = address_space_allocate_block(state);
        fprintf(stderr, "%s: alloc res finished\n", __func__);
        fprintf(stderr, "%s: irq RAISE\n", __func__);
    ...
    case ADDRESS_SPACE_COMMAND_DEALLOCATE_BLOCK:
        {
        uint32_t dealloc_res = address_space_deallocate_block(state);
        fprintf(stderr, "%s: dealloc res finished\n", __func__);
        fprintf(stderr, "%s: irq RAISE\n", __func__);
        qemu_irq_raise(state->irq);
        return dealloc_res;
        }
    case ADDRESS_SPACE_COMMAND_IRQ_HANDLED:
        {
            fprintf(stderr, "%s: irq LOWER....\n", __func__);
            qemu_irq_lower(state->irq);
            return 0;
        }
    }

    fprintf(stderr, "%s: ENOSYS\n", __func__);
    return ENOSYS;
}
