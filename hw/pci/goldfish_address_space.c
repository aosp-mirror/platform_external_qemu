...
struct memory_block *address_space_allocator_split_block(struct memory_block *big_block,
                                                         uint64_t size)
{
    struct memory_block *next_block = big_block->next;
...
static void address_space_pci_realize(PCIDevice *dev, Error **errp) {
    ...
    memory_region_init_ram_user_backed(&state->area_mem_root,
                                       OBJECT(state),
                                       GOLDFISH_ADDRESS_SPACE_AREA_NAME,
                                       GOLDFISH_ADDRESS_SPACE_AREA_SIZE);
    /*memory_region_add_subregion(get_system_memory(),
                                GOLDFISH_ADDRESS_SPACE_AREA_BASE,
                                &state->area_mem_root);*/
    pci_register_bar(dev,
