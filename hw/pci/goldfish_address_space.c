...
static uint64_t address_space_allocator_allocate(struct address_space_allocator *allocator,
                                                 uint64_t size)
{
    uint64_t offset = allocator->offset;
...
static void address_space_allocator_stat(struct address_space_allocator *allocator,
                                         uint64_t *available, uint64_t *allocated)
{
    *available = 0;
    *allocated = 0;
}
...
static void address_space_pci_unrealize(PCIDevice* dev) {
    ...
    address_space_allocator_stat(&address_space->allocator,
                                 &available, &allocated);
    g_assert(allocated == 0);
