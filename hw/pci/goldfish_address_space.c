...
struct memory_block *allocate_memory_block(struct memory_block *prev,
                                           struct memory_block *next,
                                           uint64_t offset,
                                           uint64_t size,
                                           bool available)
...
struct memory_block *address_space_allocator_find_available_block(struct address_space_allocator *allocator,
                                                                  uint64_t size_at_least)
{
    struct memory_block *result = NULL;

    for (struct memory_block *i = allocator->head; i; i = i->next) {
        if (i->available && (i->size >= size_at_least) &&
            (!result || i->size < result->size)) {
            result = i;
        }
    }
...
struct memory_block *address_space_allocator_split_block(struct memory_block *big_block,
                                                         uint64_t size)
{
    struct memory_block *next = big_block->next;
    uint64_t big_block_new_size = big_block->size - size;

    struct memory_block *new_block =
        allocate_memory_block(big_block,
                              next,
                              big_block->offset + big_block_new_size,
                              size,
                              TRUE);

    big_block->size = big_block_new_size;
    big_block->next = next;
...
void address_space_allocator_merge_with_next(struct memory_block *block, struct memory_block *next)
{
    struct memory_block *next2 = next->next;
...
void address_space_allocator_release_block(struct memory_block *block)
{
    ...
    if (next && next->available) {
        address_space_allocator_merge_with_next(block, next);
    }

    if (prev && prev->available) {
        address_space_allocator_merge_with_next(prev, block);
    }
}
...
static uint64_t address_space_allocator_allocate(struct address_space_allocator *allocator,
                                                 uint64_t size)
{
    struct memory_block *block = address_space_allocator_find_available_block(allocator, size);
    if (!block) {
        return BAD_OFFSET;
    } else {
        ...
        if (block->size > size) {
            block = address_space_allocator_split_block(block, size);
        }
    ...
    for (struct memory_block *i = allocator->head; i; i = i->next) {
        if (i->offset == offset) {
            if (i->available) {
                ...
                address_space_allocator_release_block(i);
                return 0;
            }
        }
    }
...
static void address_space_allocator_init(struct address_space_allocator *allocator,
                                         uint64_t size)
{
    allocator->head = allocate_memory_block(NULL, NULL, 0, size, true);
}

static void address_space_allocator_destroy(struct address_space_allocator *allocator)
{
    struct memory_block *head = allocator->head;

    g_assert(head->prev == NULL);
    g_assert(head->next == NULL);
    g_free(head);
}

/* We to cover 'size' bytes given at any offset with aligned pages. */
static uint64_t address_space_align_size(uint64_t page_size, uint64_t size)
{
    if (size == 0) {
        return 0;
    } else {
        return (size + page_size * 2 - 2) / page_size;
    }
}
...
static uint32_t address_space_allocate_block(struct address_space_state *state)
{
    ...
    uint64_t offset = address_space_allocator_allocate(&state->allocator, aligned_size);
...
static void address_space_control_write(void *opaque,
    ...
    DF("write offset=%d value=%u", (int)offset, (uint32_t)val);
...
static void address_space_pci_realize(PCIDevice *dev, Error **errp) {
    ...
    memory_region_init_ram_user_backed(&state->area_mem,
                                       ...
                                       GOLDFISH_ADDRESS_SPACE_AREA_SIZE);
    memory_region_add_subregion(pci_address_space(dev), 0, &state->area_mem);
    pci_register_bar(dev,
