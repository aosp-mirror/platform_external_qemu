...
static uint32_t address_space_allocate_block(struct address_space_state *state)
{
    ...
    uint64_t offset = address_space_allocator_allocate(&state->allocator, size);
    ...
    if (offset == BAD_OFFSET) {
        ...
        // memory_region_add_subregion(&address_space->area_mem_root, , )
        return 0;
    }
}
...
static uint32_t address_space_deallocate_block(struct address_space_state *state)
{
    ...
    uint32_t res = address_space_allocator_deallocate(&state->allocator, offset);
...
static void address_space_pci_realize(PCIDevice *dev, Error **errp) {
    ...
    memory_region_init(&state->area_mem_root,
                       OBJECT(state),
