...
static uint32_t address_space_allocate_block(struct address_space_state *state)
{
    ...
    DF("aligned_size=%lu offset=%lu allocator.size=%d", aligned_size, offset, state->allocator.size);
...
static uint32_t address_space_deallocate_block(struct address_space_state *state)
{
    ...
    res = address_space_allocator_deallocate(&state->allocator, offset);

    DF("offset=%lu res=%u allocator.size=%d", offset, res, state->allocator.size);
