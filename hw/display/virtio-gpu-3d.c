...
static void virtio_gpu_map_slot(
    ...
    memory_region_add_subregion(parent, gpa, mr);
...
static void virgl_cmd_resource_map(VirtIOGPU *g,
    ...
    uint64_t host_coherent_start =
        memory_region_get_ram_addr(&g->host_coherent_memory);

    fprintf(stderr, "%s: host coherent start: 0x%llx\n", __func__,
            (unsigned long long)host_coherent_start);
