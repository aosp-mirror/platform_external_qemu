...
static VSockStream *virtio_vsock_create_stream_default(uint32_t src_port,
                                                       uint32_t dst_port) {
    return NULL;
}

static void virtio_vsock_close_stream_default(VSockStream *stream) {}

static VSockStream *virtio_vsock_find_stream_default(uint32_t src_port,
                                                     uint32_t dst_port) {
    return NULL;
}

static int virtio_vsock_write_stream_default(VSockStream *stream,
                                             const void *buf,
                                             size_t size) {
    return -1;
}

static void virtio_vsock_read_streams_default(virtio_vsock_read_streams_sink_t sink) {}

const VirtIOVSockOps default_ops = {
    .create = &virtio_vsock_create_stream_default,
    .close = &virtio_vsock_close_stream_default,
    .find = &virtio_vsock_find_stream_default,
    .write_to_stream = &virtio_vsock_write_stream_default,
    .read_from_streams = &virtio_vsock_read_streams_default,
};

static const VirtIOVSockOps* g_ops = &default_ops;
static VirtIOVSock *g_sock = NULL;
static QemuMutex g_mutex;
...
static VSockStream *virtio_vsock_create_stream(uint32_t src_port,
                                               uint32_t dst_port,
                                               uint32_t remote_buf_alloc,
                                               uint32_t remote_fwd_cnt,
                                               uint32_t *buf_alloc) {
    VSockStream *stream = (*g_ops->create)(src_port, dst_port);
    if (stream) {
...
static VSockStream *virtio_vsock_find_stream(uint32_t src_port,
                                             uint32_t dst_port) {
...
virtio_vsock_read_streams_sink(VSockStream *stream,
    ...
    const size_t vq_space_available = 77; // TODO how much space vq has?

    if (vq_space_available < (2 * sizeof(struct virtio_vsock_hdr))) {
        return 0;
    } else {
        const size_t size = MIN(buf_size,
