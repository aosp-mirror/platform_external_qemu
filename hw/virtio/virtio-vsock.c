...
static void virtio_vsock_stream_read(void *s, void *vq,
                                     virtio_vsock_stream_read_cb_t cb) {
    VSockStream *stream = &stream1;

    if (stream->dst_port) {
        const size_t uncorfirmed =
            (stream->sent_cnt >= stream->remote_fwd_cnt) ?
            (stream->sent_cnt - stream->remote_fwd_cnt) : 0;

        const size_t available = MIN(strlen(stream->msg),
                                     stream->remote_buf_alloc - uncorfirmed);

        if (available > 0) {
            size_t consumed = (*cb)(s, vq,
                                    stream->src_port, stream->dst_port,
                                    stream->buf_alloc, stream->fwd_cnt,
                                    stream->msg, available);

            stream->sent_cnt += consumed;
            stream->msg += consumed;
        }
    }
}
