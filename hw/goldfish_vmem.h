...
// replaces cpu-all.h cpu_memory_rw_debug to nicely handle proper register
// synchronization from kvm when necessary.
int safe_memory_rw_debug(CPUState *env, target_ulong addr, uint8_t *buf,
                         int len, int is_write);

// replaces cpu-all.h cpu_get_phys_page_debug to handle proper register
// synchronization from kvm when necessary.
