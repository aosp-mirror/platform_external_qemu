...
static int aspeed_i3c_device_ibi_handle(I3CBus *bus, uint8_t addr, bool is_recv)
{
...
static void aspeed_i3c_device_ibi_queue_push(AspeedI3CDevice *s)
{
    ...
    ARRAY_FIELD_DP32(s->regs, QUEUE_STATUS_LEVEL, IBI_STATUS_CNT,                                              ibi_status_count);
                                     ...
                                     bool is_i2c)
{
    ...
    while (bytes_sent < num) {
        word.w = aspeed_i3c_device_pop_tx(s);
        for (uint8_t i = 0; i < sizeof(word.w); i++) {
            if (aspeed_i3c_device_send_byte(s, word.b[i], is_i2c)) {
                return bytes_sent;
            }
            bytes_sent++;
                                    ...
                                    uint64_t value, unsigned size)
{
    ...
    trace_aspeed_i3c_device_write(s->id, offset, val32);
