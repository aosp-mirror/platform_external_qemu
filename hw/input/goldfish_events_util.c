/*
 * This file contains methods common to goldfish_events and goldfish_roatry.
 * It should only be used by files implementing the goldfish_events-like
 * devices.
 */

#include "hw/input/goldfish_events_util.h"
...
void enqueue_event(GoldfishEvDevState *s,
...
uint64_t events_read(void *opaque, hwaddr offset, unsigned size)
{
...
void events_write(void *opaque, hwaddr offset,
 ...
 */
void events_set_bits(GoldfishEvDevState *s, int type, int bitl, int bith)
{
    uint8_t *bits;
    uint8_t maskl, maskh;
    int il, ih;
...
void events_set_bit(GoldfishEvDevState *s, int  type, int  bit)
{
