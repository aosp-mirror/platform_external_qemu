...
constexpr int max_num_fds_per_item = 64;
...
void divideWorkItems(GPollFD* fds, int nfds) {
  ...
  for (auto& item : s_real_works) {
    item.gfds.resize(max_num_fds_per_item);
    item.nfds = 0;
  }
  for (int i = 0; i < nfds; ++i) {
    int index = i / num_fds_per_item;
    auto& item = s_real_works[index];
    int idx2 = i % num_fds_per_item;
    item.gfds[idx2] = (fds[i]);
    item.nfds++;
  }
}
