...
static void coroutine_trampoline(int i0, int i1)
{
    ...
    if (!sigsetjmp(self->env, 0)) {
        ...
        UC_TRACE("Jump to co %p caller fiber %p", co, self->tsan_caller_fiber);
                      ...
                      CoroutineAction action)
{
    ...
    if (ret == 0) {
        ...
        UC_TRACE("Jump to co %p fiber %p. Terminate? %d", to, to->tsan_co_fiber, action == COROUTINE_TERMINATE);
