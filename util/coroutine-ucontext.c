...
static void on_new_fiber(CoroutineUContext* co) {
#ifdef CONFIG_TSAN
...
static void coroutine_trampoline(int i0, int i1)
{
    ...
    if (!sigsetjmp(self->env, 0)) {
        ...
        start_switch_fiber(&fake_stack_save,
                           leader.stack, leader.stack_size, self->tsan_caller_fiber);
...
Coroutine *qemu_coroutine_new(void)
{
    ...
    if (!sigsetjmp(old_env, 0)) {
        start_switch_fiber(&fake_stack_save, co->stack, co->stack_size, co->tsan_co_fiber);
                      ...
                      CoroutineAction action)
{
    ...
    if (ret == 0) {
        start_switch_fiber(action == COROUTINE_TERMINATE ?
                           NULL : &fake_stack_save, to->stack, to->stack_size, to->tsan_co_fiber);
