...
int wait_async(std::vector<WorkItem>& items, int timeout, bool need_mt) {
  ...
  for (int i = s_thread_done_signals.size(); i < items.size(); ++i) {
  ...
  if (need_mt) {
    ...
    DD("step4");
    int ret = WaitForMultipleObjects(s_thread_done_signals.size(),
                                     s_thread_done_signals.data(), FALSE,
                                     items[0].timeout);
    ...
    ret = WaitForMultipleObjects(s_thread_done_signals.size(),
                                 s_thread_done_signals.data(), TRUE, INFINITE);
  }
