...
int divideWorkItems(GPollFD* fds, int nfds, std::vector<WorkItem>& real_works) {
  ...
  if (real_works.size() < nwork) {
    real_works.resize(nwork);
  }
  DD("size of real_works %d", real_works.size());

  for (int i = 0; i < nwork; ++i) {
    auto& item = real_works[i];
...
int combineWork(std::vector<WorkItem>& items, int nwork) {
  int numReady = 0;
  for (int i = 0; i < nwork; ++i) {
...
void start_work_thread(std::vector<WorkItem>& items, int nwork) {
  const int num_threads = nwork;
  if (s_threads.size() < num_threads) {
    DD("increase number of threads to %d", num_threads);
    for (int i = s_threads.size(); i < num_threads; ++i) {
      // start additional threads, they will be blocked right away
...
int wait_async(std::vector<WorkItem>& items, int nwork, int timeout,
               bool need_mt) {
  if (s_works.empty()) {
    // one time only
    ...
    s_works.resize(max_num_fds_per_item, nullptr);

    // the one and only one stop signal
    s_stopSignal = CreateEventW(NULL, TRUE, FALSE, NULL);
  }

  for (int i = s_thread_done_signals.size(); i < nwork; ++i) {
    auto handle = CreateEventW(NULL, TRUE, FALSE, NULL);
    s_thread_done_signals.push_back(handle);
  }

  // need to reset s_stopSignal each time
  ResetEvent(s_stopSignal);

  for (int i = 0; i < nwork; ++i) {
    DD("prepare for %d th item", i);
    auto handle = s_thread_done_signals[i];
    ResetEvent(handle);
    items[i].stopSignal = s_stopSignal;
  ...
  if (need_mt) {
    DD("trigger mt wait ");
    start_work_thread(items, nwork);
  }

  if (need_mt) {
    DD("wait for any thread to complete");
    // wait for any threads to signal done
    int ret = WaitForMultipleObjects(nwork, s_thread_done_signals.data(), FALSE,
                                     items[0].timeout);

    // if there is any thread signaled, stop all the others
    SetEvent(s_stopSignal);

    DD("wait for all threads to complete");
    // wait for all to signal
    ret = WaitForMultipleObjects(nwork, s_thread_done_signals.data(), TRUE,
                                 INFINITE);
  }
...
int wait_driver(GPollFD* fds, int nfds, int timeout) {
  ...
  int nwork = divideWorkItems(fds, nfds, s_real_works);
  ...
  wait_async(s_real_works, nwork, timeout, need_mt);

  DD("start combine");
  int numReady = combineWork(s_real_works, nwork);
