...
def FindQualifiedEvents(results):
  ...
  for line in lines:
    if line.startswith(INPUT_EVENT_TAG):
      ...
      inputs.append(Event(int(sec) * MILLION + int(nsec), type))
    elif line.startswith(DISPLAY_EVENT_TAG):
      tag, sec, nsec = line.split()
      displays.append(int(sec) * MILLION + int(nsec))
...
def RecordLatency(results, input_time, input_type, display_time):
  # print 'inupt(us)  ', input_time, input_type
  # print 'display(us)', display_time, 'latency(milliseconds)', (
  #     display_time - input_time) / THOUSAND
  result = 0
